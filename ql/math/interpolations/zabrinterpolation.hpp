...
/*! \file zabrinterpolation.hpp
    \brief ZABR interpolation between discrete volatility points
*/
        ...
        struct ZabrSpecs
        {
                               ...
                               const std::vector<Real>& addParams)
            {
                ...
                if (params[0] == Null<Real>())
                    ...
                    params[0] = 0.2 * (params[1] < 0.9999 ? std::pow(forward, 1.0 - params[1]) : Real(1.0));
            ...
            {
                Array x(5);
                x[0] = y[0] < 25.0 + eps1() ? std::sqrt(y[0] - eps1()) : Real((y[0] - eps1() + 25.0) / 10.0);
            ...
            {
                Array y(5);
                y[0] = std::fabs(x[0]) < 5.0 ? Real(x[0] * x[0] + eps1()) : (10.0 * std::fabs(x[0]) - 25.0) + eps1();
                y[1] = std::fabs(x[1]) < std::sqrt(-std::log(eps1())) ? std::exp(-(x[1] * x[1])) : eps1();
                // limit nu to 5.00
                y[2] = (std::atan(x[2]) / M_PI + 0.5) * 5.0;
                y[3] =
                    std::fabs(x[3]) < 2.5 * M_PI ? eps2() * std::sin(x[3]) : Real(eps2() * (x[3] > 0.0 ? 1.0 : (-1.0)));
                // limit gamma to 1.9
      ...
      public:
                          ...
                          const Size maxGuesses = 50)
        {
            impl_ = ext::shared_ptr<Interpolation::Impl>(
                new detail::XABRInterpolationImpl<I1, I2, detail::ZabrSpecs<Evaluation>>(
                    xBegin, xEnd, yBegin, t, forward, {alpha, beta, nu, rho, gamma},
                    {alphaIsFixed, betaIsFixed, nuIsFixed, rhoIsFixed, gammaIsFixed}, vegaWeighted, endCriteria,
                    optMethod, errorAccept, useMaxError, maxGuesses));
        }
    ...
    class Zabr
    {
      public:
        Zabr(Time t,
             ...
             const bool useMaxError = false,
             const Size maxGuesses = 50)
        : t_(t), forward_(forward), alpha_(alpha), beta_(beta), nu_(nu), rho_(rho), gamma_(gamma),
          alphaIsFixed_(alphaIsFixed), betaIsFixed_(betaIsFixed), nuIsFixed_(nuIsFixed), rhoIsFixed_(rhoIsFixed),
          gammaIsFixed_(gammaIsFixed), vegaWeighted_(vegaWeighted), endCriteria_(std::move(endCriteria)),
        ...
        {
            return ZabrInterpolation<Evaluation>(xBegin, xEnd, yBegin, t_, forward_, alpha_, beta_, nu_, rho_, gamma_,
                                                 alphaIsFixed_, betaIsFixed_, nuIsFixed_, rhoIsFixed_, gammaIsFixed_,
                                                 vegaWeighted_, endCriteria_, optMethod_, errorAccept_, useMaxError_,
                                                 maxGuesses_);
        }
