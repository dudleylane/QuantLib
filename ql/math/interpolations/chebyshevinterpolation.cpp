...
namespace QuantLib
{
    ...
    ChebyshevInterpolation::ChebyshevInterpolation(const Array& y, PointsType pointsType)
    : x_(ChebyshevInterpolation::nodes(y.size(), pointsType)), y_(y)
    {

        impl_ = ext::make_shared<detail::LagrangeInterpolationImpl<Array::const_iterator, Array::const_iterator>>(
            std::begin(x_), std::end(x_), std::begin(y_));

        impl_->update();
    }

    ChebyshevInterpolation::ChebyshevInterpolation(Size n, const std::function<Real(Real)>& f, PointsType pointsType)
    : ChebyshevInterpolation(chebyshev_interpolation_detail::apply(ChebyshevInterpolation::nodes(n, pointsType), f),
                             pointsType)
    {
    }
    ...
    {
        QL_REQUIRE(y.size() == y_.size(), "interpolation override has the wrong length");
