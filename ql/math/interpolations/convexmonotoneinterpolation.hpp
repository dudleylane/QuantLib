    ...
    */
    ...
    class ConvexMonotoneInterpolation : public Interpolation
    {
      ...
      public:
        ...
        {
            impl_ = ext::shared_ptr<Interpolation::Impl>(new detail::ConvexMonotoneImpl<I1, I2>(
                xBegin, xEnd, yBegin, quadraticity, monotonicity, forcePositive, flatFinalPeriod, preExistingHelpers));
            impl_->update();
        }

        ConvexMonotoneInterpolation(Interpolation& interp) : Interpolation(interp) {}
    ...
    class ConvexMonotone
    {
      public:
        ...
        : quadraticity_(quadraticity), monotonicity_(monotonicity), forcePositive_(forcePositive)
        {
        }
        ...
        {
            return ConvexMonotoneInterpolation<I1, I2>(xBegin, xEnd, yBegin, quadraticity_, monotonicity_,
                                                       forcePositive_, false);
        }
                                       ...
                                       Size finalSize) const
        {
            ...
            {
                return ConvexMonotoneInterpolation<I1, I2>(xBegin, xEnd, yBegin, quadraticity_, monotonicity_,
                                                           forcePositive_, false, interp.getExistingHelpers());
            }
            else
            {
                return ConvexMonotoneInterpolation<I1, I2>(xBegin, xEnd, yBegin, quadraticity_, monotonicity_,
                                                           forcePositive_, true, interp.getExistingHelpers());
            }
        }
    ...
    namespace detail
    {
        ...
        class ConvexMonotoneImpl final : public Interpolation::templateImpl<I1, I2>
        {
          ...
          public:
            ...
            : Interpolation::templateImpl<I1, I2>(xBegin, xEnd, yBegin, ConvexMonotone::requiredPoints),
              preSectionHelpers_(preExistingHelpers), forcePositive_(forcePositive),
              constantLastPeriod_(constantLastPeriod), quadraticity_(quadraticity), monotonicity_(monotonicity),
              length_(xEnd - xBegin)
            {

                QL_REQUIRE(monotonicity_ >= 0 && monotonicity_ <= 1, "Monotonicity must lie between 0 and 1");
                QL_REQUIRE(quadraticity_ >= 0 && quadraticity_ <= 1, "Quadraticity must lie between 0 and 1");
                QL_REQUIRE(length_ >= 2, "Single point provided, not supported by convex "
                                         "monotone method as first point is ignored");
                QL_REQUIRE((length_ - preExistingHelpers.size()) > 1, "Too many existing helpers have been supplied");
            }
            ...
            Real derivative(Real) const override { QL_FAIL("Convex-monotone spline derivative not implemented"); }
        ...
        class ComboHelper : public SectionHelper
        {
          public:
            ComboHelper(ext::shared_ptr<SectionHelper>& quadraticHelper,
                        ...
                        Real quadraticity)
            : quadraticity_(quadraticity), quadraticHelper_(quadraticHelper), convMonoHelper_(convMonoHelper)
            {
                QL_REQUIRE(quadraticity < 1.0 && quadraticity > 0.0, "Quadratic value must lie between 0 and 1");
            }
            ...
            Real primitive(Real x) const override
            {
                return (quadraticity_ * quadraticHelper_->primitive(x) +
                        (1.0 - quadraticity_) * convMonoHelper_->primitive(x));
            }
        ...
        class ConvexMonotone2Helper : public SectionHelper
        {
          public:
            ConvexMonotone2Helper(
                Real xPrev, Real xNext, Real gPrev, Real gNext, Real fAverage, Real eta2, Real prevPrimitive)
            : xPrev_(xPrev), xScaling_(xNext - xPrev), gPrev_(gPrev), gNext_(gNext), fAverage_(fAverage), eta2_(eta2),
            ...
            Real value(Real x) const override
            {
                ...
                {
                    return (fAverage_ + gPrev_ +
                            (gNext_ - gPrev_) / ((1 - eta2_) * (1 - eta2_)) * (xVal - eta2_) * (xVal - eta2_));
                }
            }
            ...
            Real primitive(Real x) const override
            {
                ...
                {
                    return (prevPrimitive_ +
                            xScaling_ * (fAverage_ * xVal + gPrev_ * xVal +
                                         (gNext_ - gPrev_) / ((1 - eta2_) * (1 - eta2_)) *
                                             (1.0 / 3.0 * (xVal * xVal * xVal - eta2_ * eta2_ * eta2_) -
                                              eta2_ * xVal * xVal + eta2_ * eta2_ * xVal)));
                }
            }
        ...
        class ConvexMonotone3Helper : public SectionHelper
        {
          public:
            ConvexMonotone3Helper(
                Real xPrev, Real xNext, Real gPrev, Real gNext, Real fAverage, Real eta3, Real prevPrimitive)
            : xPrev_(xPrev), xScaling_(xNext - xPrev), gPrev_(gPrev), gNext_(gNext), fAverage_(fAverage), eta3_(eta3),
              prevPrimitive_(prevPrimitive)
            {
            }
            ...
            Real primitive(Real x) const override
            {
                ...
                if (xVal <= eta3_)
                {
                    return (prevPrimitive_ + xScaling_ * (fAverage_ * xVal + gNext_ * xVal +
                                                          (gPrev_ - gNext_) / (eta3_ * eta3_) *
                                                              (1.0 / 3.0 * xVal * xVal * xVal - eta3_ * xVal * xVal +
                                                               eta3_ * eta3_ * xVal)));
                }
                else
                {
                    return (prevPrimitive_ +
                            xScaling_ * (fAverage_ * xVal + gNext_ * xVal +
                                         (gPrev_ - gNext_) / (eta3_ * eta3_) * (1.0 / 3.0 * eta3_ * eta3_ * eta3_)));
                }
            }
            Real fNext() const override { return (fAverage_ + gNext_); }
        ...
        class ConvexMonotone4Helper : public SectionHelper
        {
          public:
            ConvexMonotone4Helper(
                Real xPrev, Real xNext, Real gPrev, Real gNext, Real fAverage, Real eta4, Real prevPrimitive)
            : xPrev_(xPrev), xScaling_(xNext - xPrev), gPrev_(gPrev), gNext_(gNext), fAverage_(fAverage), eta4_(eta4),
              prevPrimitive_(prevPrimitive)
            {
            ...
            Real value(Real x) const override
            {
                ...
                {
                    return (fAverage_ + A_ +
                            (gNext_ - A_) * (xVal - eta4_) * (xVal - eta4_) / ((1 - eta4_) * (1 - eta4_)));
                }
            }
            ...
            Real primitive(Real x) const override
            {
                ...
                if (xVal <= eta4_)
                {
                    retVal = prevPrimitive_ + xScaling_ *
                                                  (fAverage_ + A_ +
                                                   (gPrev_ - A_) / (eta4_ * eta4_) *
                                                       (eta4_ * eta4_ - eta4_ * xVal + 1.0 / 3.0 * xVal * xVal)) *
                                                  xVal;
                }
                else
                {
                    retVal = prevPrimitive_ +
                             xScaling_ * (fAverage_ * xVal + A_ * xVal + (gPrev_ - A_) * (1.0 / 3.0 * eta4_) +
                                          (gNext_ - A_) / ((1 - eta4_) * (1 - eta4_)) *
                                              (1.0 / 3.0 * xVal * xVal * xVal - eta4_ * xVal * xVal +
                                               eta4_ * eta4_ * xVal - 1.0 / 3.0 * eta4_ * eta4_ * eta4_));
                }
                return retVal;
            }
            Real fNext() const override { return (fAverage_ + gNext_); }
        ...
        class ConvexMonotone4MinHelper : public ConvexMonotone4Helper
        {
          public:
            ...
            Real primitive(Real x) const override
            {
                ...
                {
                    xVal /= xRatio_;
                    return (prevPrimitive_ + xScaling_ * xRatio_ *
                                                 (fAverage_ + A_ +
                                                  (gPrev_ - A_) / (eta4_ * eta4_) *
                                                      (eta4_ * eta4_ - eta4_ * xVal + 1.0 / 3.0 * xVal * xVal)) *
                                                 xVal);
                ...
                else if (x <= x3_)
                {
                    return (prevPrimitive_ +
                            xScaling_ * xRatio_ *
                                (fAverage_ * eta4_ + A_ * eta4_ +
                                 (gPrev_ - A_) / (eta4_ * eta4_) * (1.0 / 3.0 * eta4_ * eta4_ * eta4_)));
                }
                else
                {
                    xVal = 1.0 - (1.0 - xVal) / xRatio_;
                    return (prevPrimitive_ + xScaling_ * xRatio_ *
                                                 (fAverage_ * xVal + A_ * xVal + (gPrev_ - A_) * (1.0 / 3.0 * eta4_) +
        ...
        class ConstantGradHelper : public SectionHelper
        {
          public:
            ...
            : fPrev_(fPrev), prevPrimitive_(prevPrimitive), xPrev_(xPrev), fGrad_((fNext - fPrev) / (xNext - xPrev)),
              fNext_(fNext)
            {
            }
        ...
        class QuadraticHelper : public SectionHelper
        {
          public:
            ...
            : xPrev_(xPrev), xNext_(xNext), fPrev_(fPrev), fNext_(fNext), fAverage_(fAverage),
              prevPrimitive_(prevPrimitive)
            {
        ...
        class QuadraticMinHelper : public SectionHelper
        {
          public:
            ...
            : x1_(xPrev), x4_(xNext), primitive1_(prevPrimitive), fAverage_(fAverage), fPrev_(fPrev), fNext_(fNext)
            {
                a_ = 3 * fPrev_ + 3 * fNext_ - 6 * fAverage_;
                b_ = -(4 * fPrev_ + 2 * fNext_ - 6 * fAverage_);
                c_ = fPrev_;
                ...
                if (d > 0)
                {
                    ...
                    if (dAv >= 0.0)
                    {
                        ...
                        primitive2_ = primitive1_ + xScaling_ * (a_ / 3 * xRoot * xRoot + b_ / 2 * xRoot + c_) * xRoot;
                    }
                }
            }
        ...
        void ConvexMonotoneImpl<I1, I2>::update()
        {
            ...
            { // single period
                ext::shared_ptr<SectionHelper> singleHelper(
                    new EverywhereConstantHelper(this->yBegin_[1], 0.0, this->xBegin_[0]));
            ...
            for (Size i = startPoint; i < length_ - 1; ++i)
            {
                ...
                f[i] = dx / (dx + dxPrev) * this->yBegin_[i] + dxPrev / (dx + dxPrev) * this->yBegin_[i + 1];
            }
            ...
            for (Size i = startPoint; i < endPoint; ++i)
            {
                ...
                if (std::fabs(gPrev) < 1.0E-14 && std::fabs(gNext) < 1.0E-14)
                {
                    ext::shared_ptr<SectionHelper> singleHelper(
                        new ConstantGradHelper(f[i - 1], primitive, this->xBegin_[i - 1], this->xBegin_[i], f[i]));
                ...
                else
                {
                    ...
                    if (quadraticity_ > 0.0)
                    {
                        ...
                        {
                            quadraticHelper = ext::shared_ptr<SectionHelper>(new QuadraticMinHelper(
                                this->xBegin_[i - 1], this->xBegin_[i], f[i - 1], f[i], this->yBegin_[i], primitive));
                        }
                        else
                        {
                            quadraticHelper = ext::shared_ptr<SectionHelper>(new QuadraticHelper(
                                this->xBegin_[i - 1], this->xBegin_[i], f[i - 1], f[i], this->yBegin_[i], primitive));
                        }
                    }
                    if (quadraticity_ < 1.0)
                    {
                            ...
                            (gPrev < 0.0 && -0.5 * gPrev <= gNext && gNext <= -2.0 * gPrev))
                        {
                            ...
                            if (quadraticity_ == 0)
                            {
                                if (forcePositive_)
                                {
                                    quadraticHelper = ext::shared_ptr<SectionHelper>(
                                        new QuadraticMinHelper(this->xBegin_[i - 1], this->xBegin_[i], f[i - 1], f[i],
                                                               this->yBegin_[i], primitive));
                                }
                                else
                                {
                                    quadraticHelper = ext::shared_ptr<SectionHelper>(
                                        new QuadraticHelper(this->xBegin_[i - 1], this->xBegin_[i], f[i - 1], f[i],
                                                            this->yBegin_[i], primitive));
                                }
                            }
                        }
                        else if ((gPrev < 0.0 && gNext > -2.0 * gPrev) || (gPrev > 0.0 && gNext < -2.0 * gPrev))
                        {
                            ...
                            if (eta < b2)
                            {
                                convMonotoneHelper = ext::shared_ptr<SectionHelper>(
                                    new ConvexMonotone2Helper(this->xBegin_[i - 1], this->xBegin_[i], gPrev, gNext,
                                                              this->yBegin_[i], eta, primitive));
                            }
                            else
                            {
                                if (forcePositive_)
                                {
                                    convMonotoneHelper = ext::shared_ptr<SectionHelper>(
                                        new ConvexMonotone4MinHelper(this->xBegin_[i - 1], this->xBegin_[i], gPrev,
                                                                     gNext, this->yBegin_[i], b2, primitive));
                                }
                                else
                                {
                                    convMonotoneHelper = ext::shared_ptr<SectionHelper>(
                                        new ConvexMonotone4Helper(this->xBegin_[i - 1], this->xBegin_[i], gPrev, gNext,
                                                                  this->yBegin_[i], b2, primitive));
                                }
                            }
                        }
                                 ...
                                 (gPrev < 0.0 && gNext > 0.0 && gNext < -0.5 * gPrev))
                        {
                            ...
                            {
                                convMonotoneHelper = ext::shared_ptr<SectionHelper>(
                                    new ConvexMonotone3Helper(this->xBegin_[i - 1], this->xBegin_[i], gPrev, gNext,
                                                              this->yBegin_[i], eta, primitive));
                            }
                            else
                            {
                                if (forcePositive_)
                                {
                                    convMonotoneHelper = ext::shared_ptr<SectionHelper>(
                                        new ConvexMonotone4MinHelper(this->xBegin_[i - 1], this->xBegin_[i], gPrev,
                                                                     gNext, this->yBegin_[i], b3, primitive));
                                }
                                else
                                {
                                    convMonotoneHelper = ext::shared_ptr<SectionHelper>(
                                        new ConvexMonotone4Helper(this->xBegin_[i - 1], this->xBegin_[i], gPrev, gNext,
                                                                  this->yBegin_[i], b3, primitive));
                                }
                            }
                        }
                        else
                        {
                            ...
                            {
                                convMonotoneHelper = ext::shared_ptr<SectionHelper>(
                                    new ConvexMonotone4MinHelper(this->xBegin_[i - 1], this->xBegin_[i], gPrev, gNext,
                                                                 this->yBegin_[i], eta, primitive));
                            }
                            else
                            {
                    ...
                    {
                        sectionHelpers_[this->xBegin_[i]] = ext::shared_ptr<SectionHelper>(
                            new ComboHelper(quadraticHelper, convMonotoneHelper, quadraticity));
                    }
                }
                primitive += this->yBegin_[i] * (this->xBegin_[i] - this->xBegin_[i - 1]);
            }

            if (constantLastPeriod_)
            {
                sectionHelpers_[this->xBegin_[length_ - 1]] = ext::shared_ptr<SectionHelper>(
                    new EverywhereConstantHelper(this->yBegin_[length_ - 1], primitive, this->xBegin_[length_ - 2]));
                extrapolationHelper_ = sectionHelpers_[this->xBegin_[length_ - 1]];
            }
            else
            {
                extrapolationHelper_ = ext::shared_ptr<SectionHelper>(new EverywhereConstantHelper(
                    (sectionHelpers_.rbegin())->second->value(*(this->xEnd_ - 1)), primitive, *(this->xEnd_ - 1)));
            }
        }
