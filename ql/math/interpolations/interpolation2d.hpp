      ...
      public:
        ...
        class templateImpl : public Impl
        {
          public:
            ...
            : xBegin_(xBegin), xEnd_(xEnd), yBegin_(yBegin), yEnd_(yEnd), zData_(zData)
            {
                QL_REQUIRE(xEnd_ - xBegin_ >= 2, "not enough x points to interpolate: at least 2 "
                                                 "required, "
                                                     << xEnd_ - xBegin_ << " provided");
                QL_REQUIRE(yEnd_ - yBegin_ >= 2, "not enough y points to interpolate: at least 2 "
                                                 "required, "
            ...
            bool isInRange(Real x, Real y) const override
            {
                ...
                bool xIsInrange = (x >= x1 && x <= x2) || close(x, x1) || close(x, x2);
                if (!xIsInrange)
                    return false;
      ...
      protected:
        void checkRange(Real x, Real y, bool extrapolate) const
        {
            QL_REQUIRE(extrapolate || allowsExtrapolation() || impl_->isInRange(x, y),
                       "interpolation range is [" << impl_->xMin() << ", " << impl_->xMax() << "] x [" << impl_->yMin()
                                                  << ", " << impl_->yMax() << "]: extrapolation at (" << x << ", " << y
                                                  << ") not allowed");
        }
    };
