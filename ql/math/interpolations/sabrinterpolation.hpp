    ...
    namespace detail
    {
        ...
        class SABRWrapper
        {
          public:
            ...
            {
                QL_REQUIRE(forward_ + shift_ > 0.0, "forward+shift must be positive: " << forward_ << " with shift "
                                                                                       << shift_ << " not allowed");
            ...
            {
                return shiftedSabrVolatility(x, forward_, t_, params_[0], params_[1], params_[2], params_[3], shift_,
                                             volatilityType);
            }
        ...
        struct SABRSpecs
        {
                               ...
                               const std::vector<Real>& addParams)
            {
                ...
                if (params[0] == Null<Real>())
                    ...
                    params[0] =
                        0.2 * (params[1] < 0.9999 ?
                                   Real(std::pow(forward + (addParams.empty() ? 0.0 : addParams[0]), 1.0 - params[1])) :
                                   1.0);
            ...
            Array direct(const Array& x, const std::vector<bool>&, const std::vector<Real>&, const Real)
            {
                Array y(4);
                y[0] =
                    std::fabs(x[0]) < 5.0 ? Real(x[0] * x[0] + eps1()) : Real((10.0 * std::fabs(x[0]) - 25.0) + eps1());
                // y_[1] = std::atan(dilationFactor_*x[1])/M_PI + 0.5;
                y[1] = std::fabs(x[1]) < std::sqrt(-std::log(eps1())) ? std::exp(-(x[1] * x[1])) : eps1();
                y[2] =
                    std::fabs(x[2]) < 5.0 ? Real(x[2] * x[2] + eps1()) : Real((10.0 * std::fabs(x[2]) - 25.0) + eps1());
                y[3] = std::fabs(x[3]) < 2.5 * M_PI ? Real(eps2() * std::sin(x[3])) :
            ...
            {
                return blackFormulaStdDevDerivative(strike, forward, stdDev, 1.0, addParams[0]);
            }
    ...
    */
    class SABRInterpolation : public Interpolation
    {
      public:
                          ...
                          const VolatilityType volatilityType = VolatilityType::ShiftedLognormal)
        {
            ...
            impl_ = ext::shared_ptr<Interpolation::Impl>(new detail::XABRInterpolationImpl<I1, I2, detail::SABRSpecs>(
                xBegin, xEnd, yBegin, t, forward, {alpha, beta, nu, rho},
                {alphaIsFixed, betaIsFixed, nuIsFixed, rhoIsFixed}, vegaWeighted, endCriteria, optMethod, errorAccept,
    ...
    class SABR
    {
      public:
        ...
        {
            return SABRInterpolation(xBegin, xEnd, yBegin, t_, forward_, alpha_, beta_, nu_, rho_, alphaIsFixed_,
                                     betaIsFixed_, nuIsFixed_, rhoIsFixed_, vegaWeighted_, endCriteria_, optMethod_,
                                     errorAccept_, useMaxError_, maxGuesses_, shift_);
        }
