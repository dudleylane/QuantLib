    ...
    namespace detail
    {
        ...
        class AbcdCoeffHolder : public Interpolation::Impl
        {
          public:
            ...
            : a_(a), b_(b), c_(c), d_(d), error_(Null<Real>()), maxError_(Null<Real>())
            {
                if (a_ != Null<Real>())
                    aIsFixed_ = aIsFixed;
                else
        ...
        class AbcdInterpolationImpl final : public Interpolation::templateImpl<I1, I2, AbcdCoeffHolder>
        {
          public:
            ...
            : Interpolation::templateImpl<I1, I2, AbcdCoeffHolder>(
              ...
              endCriteria_(std::move(endCriteria)), optMethod_(std::move(optMethod)), vegaWeighted_(vegaWeighted)
            {
            }
            ...
            void update() override
            {
                ...
                abcdCalibrator_ = ext::shared_ptr<AbcdCalibration>(new AbcdCalibration(
                    times, blackVols, this->a_, this->b_, this->c_, this->d_, this->aIsFixed_, this->bIsFixed_,
                    this->cIsFixed_, this->dIsFixed_, vegaWeighted_, endCriteria_, optMethod_));
            ...
            {
                QL_REQUIRE(x >= 0.0, "time must be non negative: " << x << " not allowed");
            ...
            Real secondDerivative(Real) const override { QL_FAIL("Abcd secondDerivative not implemented"); }
            Real k(Time t) const
            {
    ...
    */
      ...
      public:
                          ...
                          const ext::shared_ptr<OptimizationMethod>& optMethod = ext::shared_ptr<OptimizationMethod>())
        {
            ...
            impl_ = ext::shared_ptr<Interpolation::Impl>(
                new detail::AbcdInterpolationImpl<I1, I2>(xBegin, xEnd, yBegin, a, b, c, d, aIsFixed, bIsFixed,
                                                          cIsFixed, dIsFixed, vegaWeighted, endCriteria, optMethod));
            impl_->update();
        }
        //! \name Inspectors
        //@{
    ...
    class Abcd
    {
      public:
        ...
        {
            return AbcdInterpolation(xBegin, xEnd, yBegin, a_, b_, c_, d_, aIsFixed_, bIsFixed_, cIsFixed_, dIsFixed_,
