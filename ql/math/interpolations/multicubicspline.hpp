...
namespace QuantLib
{
    ...
    namespace detail
    {
        ...
        struct EmptyArg
        {
        }; // arg_type termination marker
        struct EmptyRes
        {
        }; // res_type termination marker
        struct EmptyDim
        {
        }; // size_t termination marker
        ...
        struct DataTable
        {
            ...
            Size size() const { return data_table_.size(); }
        ...
        struct DataTable<Real>
        {
            ...
            DataTable(const std::vector<Size>::const_iterator& i) : data_table_(*i) {}
            DataTable(const SplineGrid::const_iterator& i) : data_table_(i->size()) {}
            template <class U>
            DataTable(const std::vector<U>& v)
            {
                DataTable temp(v.begin());
        ...
        struct Data
        {
            ...
            Data(const SplineGrid::const_iterator& i) : first(*i), second(i + 1) {}
            Data(const SplineGrid& v) : first(v[0]), second(v.begin() + 1) {}
        ...
        {
            Data() = default;
            Data(const SplineGrid::const_iterator& i) : first(*i) {}
            Data(const SplineGrid& v) : first(v[0]) {}
            Data(std::vector<Real> v) : first(std::move(v)) {}
            void swap(Data<std::vector<Real>, EmptyArg>& d) noexcept { first.swap(d.first); }
        ...
        {
            typedef X data_type;
            Point() : first(data_type()), second(Y()) {}
            Point(const std::vector<Real>::const_iterator& i) : first(*i), second(i + 1) {}
            Point(const std::vector<Real>& v) : first(v[0]), second(v.begin() + 1) {}
            Point(const SplineGrid::const_iterator& i) : first(i->size()), second(i + 1) {}
            Point(const SplineGrid& grid) : first(grid[0].size()), second(grid.begin() + 1) {}
        ...
        struct Point<Real, EmptyArg>
        {
            ...
            Point(data_type s) : first(s) {}
            Point(const std::vector<Real>::const_iterator& i) : first(*i) {}
            Point(const std::vector<Real>& v) : first(v[0]) {}
            operator data_type() const { return first; }
        ...
        struct Point<base_data_table, EmptyRes>
        {
            ...
            Point(const SplineGrid::const_iterator& i) : first(i->size()) {}
            Point(const SplineGrid& grid) : first(grid[0].size()) {}
            Real operator[](Size n) const { return first[n]; }
            Real& operator[](Size n) { return first[n]; }
            data_type first;
            EmptyRes second;
        ...
        class base_cubic_spline
        {
          public:
            ...
            base_cubic_spline(const data& d, const data& d2, const data_table& y, data_table& y2, output_data& v)
            {
                ...
                Size j = 1, k = 2, l = 3;
                result_type &w = ((y2[dim] = y[1]) -= y[0]) /= d[0], &u = ((y2[0] = y[2]) -= y[1]) /= d[1], &t = v[dim];
        ...
        class n_cubic_splint
        {
          public:
            ...
            n_cubic_splint(const return_type& a,
                           ...
                           output_data& v2,
                           result_type& r)
            : a_(a), b_(b), a2_(a2), b2_(b2), i_(i), d_(d), d2_(d2), y_(y), y2_(y2), v_(v), v1_(v1), v2_(v2)
            {
                for (Size j = 0, dim = y_.size(); j < dim; ++j)
                    X(a_.second, b_.second, a2_.second, b2_.second, i_.second, d_.second, d2_.second, y_[j], y2_[j],
                      v_.second, v1_.second, v2_.second, v1_.first[j]);
                base_cubic_spline(d_.first, d2_.first, v1_.first.first, v2_.first.first, v_.first);
                base_cubic_splint(a_.first, b_.first, a2_.first, b2_.first, i_.first, d_.first, d2_.first,
                                  v1_.first.first, v2_.first.first, v_.first, v1_.first, v2_.first, r);
            }
    ...
    */
    ...
    class MultiCubicSpline
    {
      ...
      public:
        ...
        MultiCubicSpline(const SplineGrid& grid, const data_table& y, std::vector<bool> ae = {})
        : grid_(grid), y_(y), ae_(std::move(ae)), v_(grid), v1_(grid), v2_(grid), y2_(grid)
        {
            if (ae_.empty())
        ...
        result_type operator()(const argument_type& x) const
        {
            ...
            c_splint(a_, b_, a2_, b2_, i_, d_, d2_, y_, y2_, v_, v1_, v2_, res_);
            return res_;
        }
    ...
    void MultiCubicSpline<i>::set_shared_increments() const
    {
        ...
        for (Size j = 0; j < i; k = 0, ++j)
        {
            ...
            QL_REQUIRE(dim >= 3, "Dimension " << j << " : not enough points for interpolation");
            QL_REQUIRE(k >= dim, "Dimension " << j << " : invalid data");
        }
    ...
    // exception is thrown BEFORE the main body of interpolation begins
    template <Size i>
    void MultiCubicSpline<i>::set_shared_coefficients(const typename MultiCubicSpline<i>::argument_type& x) const
    {
        for (Size j = 0; j < i; ++j)
        {
            ...
            if (x[j] < v[0] || x[j] >= v[sz])
            {
                QL_REQUIRE(ae_[j], "Dimension " << j << ": extrapolation is not allowed.");
                a_[j] = 1.0, a2_[j] = b_[j] = b2_[j] = 0.0;
                k = x[j] < v[0] ? 0 : sz;
            }
            else
            {
                k = v[k] <= x[j] && x[j] < v[k + 1] ? k : std::upper_bound(v.begin(), v.end(), x[j]) - v.begin() - 1;
                Real h = v[k + 1] - v[k];
