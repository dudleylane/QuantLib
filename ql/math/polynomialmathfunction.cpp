...
namespace QuantLib
{
    ...
    void PolynomialFunction::initializeEqs_(Time t, Time t2) const
    {
        Time dt = t2 - t;
        Real tau;
        for (Size i = 0; i < order_; ++i)
    ...
    std::vector<Real> PolynomialFunction::definiteIntegralCoefficients(Time t, Time t2) const
    {
    ...
    std::vector<Real> PolynomialFunction::definiteDerivativeCoefficients(Time t, Time t2) const
    {
        Array k(c_.begin(), c_.end());
        initializeEqs_(t, t2);
