...
namespace QuantLib
{
    ...
    GMRESResult GMRES::solveWithRestart(Size restart, const Array& b, const Array& x0) const
    {
    ...
    GMRESResult GMRES::solveImpl(const Array& b, const Array& x0) const
    {
        ...
        {
            y[i] = (z[i] - std::inner_product(h[i].begin() + i + 1, h[i].begin() + k, y.begin() + i + 1, Real(0.0))) /
                   h[i][i];
        }
