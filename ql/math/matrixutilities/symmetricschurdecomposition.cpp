...
namespace QuantLib
{
    ...
    : diagonal_(s.rows()), eigenVectors_(s.rows(), s.columns(), 0.0)
    {
                ...
                */
                ...
                for (j = 0; j < size - 1; j++)
                {
                    for (k = j + 1; k < size; k++)
                    {
                        ...
                        if (ite > 5 && smll < epsPrec * std::fabs(diagonal_[j]) &&
                        ...
                        else if (std::fabs(ss[j][k]) > threshold)
                        {
                            ...
                            else
                            {
                                ...
                                tang = 1.0 / (std::fabs(beta) + std::sqrt(1 + beta * beta));
                                if (beta < 0)
                                    tang = -tang;
                            }
        ...
        QL_ENSURE(ite <= maxIterations, "Too many iterations (" << maxIterations << ") reached");
        ...
        for (col = 0; col < size; col++)
        {
            ...
            diagonal_[col] = (std::fabs(temp[col].first / maxEv) < 1e-16 ? 0.0 : temp[col].first);
            Real sign = 1.0;
