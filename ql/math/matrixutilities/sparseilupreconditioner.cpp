...
namespace QuantLib
{
    ...
    SparseILUPreconditioner::SparseILUPreconditioner(const SparseMatrix& A, Integer lfil)
    : L_(A.size1(), A.size2()), U_(A.size1(), A.size2())
    {

        QL_REQUIRE(A.size1() == A.size2(), "sparse ILU preconditioner works only with square matrices");
    ...
    Array SparseILUPreconditioner::forwardSolve(const Array& b) const
    {
        ...
        {
            y[i] = b[i] / L_(i, i);
            for (Integer j = lBands_.size() - 1; j >= 0 && i - Integer(lBands_[j]) <= i - 1; --j)
            {
