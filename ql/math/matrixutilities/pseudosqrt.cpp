...
namespace QuantLib
{
    ...
    namespace
    {
            ...
            for (Size i = 0; i < size; ++i)
                for (Size j = 0; j < i; ++j)
                    QL_REQUIRE(close(matrix[i][j], matrix[j][i]),
                               "non symmetric matrix: " << "[" << i << "][" << j << "]=" << matrix[i][j] << ", [" << j
                                                        << "][" << i << "]=" << matrix[j][i]);
        }
#endif
        ...
        {
            Size size = matrix.rows();
            QL_REQUIRE(size == pseudo.rows(), "matrix/pseudo mismatch: matrix rows are "
                                                  << size << " while pseudo rows are " << pseudo.columns());
        ...
        class HypersphereCostFunction : public CostFunction
        {
          ...
          public:
            HypersphereCostFunction(const Matrix& targetMatrix, Array targetVariance, bool lowerDiagonal)
            : size_(targetMatrix.rows()), lowerDiagonal_(lowerDiagonal), targetMatrix_(targetMatrix),
              targetVariance_(std::move(targetVariance)), currentRoot_(size_, size_), tempMatrix_(size_, size_),
              currentMatrix_(size_, size_)
            {
            }
            Array values(const Array&) const override { QL_FAIL("values method not implemented"); }
        ...
        Matrix hypersphereOptimize(const Matrix& targetMatrix, const Matrix& currentRoot, const bool lowerDiagonal)
        {
            ...
            HypersphereCostFunction costFunction(targetMatrix, variance, lowerDiagonal);
        ...
        {
            Size size = M.rows();
            QL_REQUIRE(size == M.columns(), "matrix not square");
        ...
        Matrix projectToPositiveSemidefiniteMatrix(Matrix& M)
        {
            ...
            Matrix result = jd.eigenvectors() * diagonal * transpose(jd.eigenvectors());
            return result;
        }
        ...
        Matrix highamImplementation(const Matrix& A, const Size maxIterations, const Real& tolerance)
        {
            ...
            for (Size i = 0; i < maxIterations; ++i)
            {
                ...
                // convergence test
                if (std::max({normInf(X - lastX) / normInf(X), normInf(Y - lastY) / normInf(Y),
                              normInf(Y - X) / normInf(Y)}) <= tolerance)
                {
                    break;
                }
                lastX = X;
        ...
        switch (sa)
        {
            case SalvagingAlgorithm::None:
                ...
                QL_REQUIRE(jd.eigenvalues()[size - 1] >= -1e-16,
                           "negative eigenvalue(s) (" << std::scientific << jd.eigenvalues()[size - 1] << ")");
            ...
            case SalvagingAlgorithm::Principal:
            {
                QL_REQUIRE(jd.eigenvalues().back() >= -10 * QL_EPSILON,
                           "negative eigenvalue(s) (" << std::scientific << jd.eigenvalues().back() << ")");

                Array sqrtEigenvalues(size);
                std::transform(jd.eigenvalues().begin(), jd.eigenvalues().end(), sqrtEigenvalues.begin(),
                               [](Real lambda) -> Real { return std::sqrt(std::max<Real>(lambda, 0.0)); });

                for (Size i = 0; i < size; ++i)
                    std::transform(sqrtEigenvalues.begin(), sqrtEigenvalues.end(), jd.eigenvectors().row_begin(i),
                                   diagonal.column_begin(i), std::multiplies<>());
    ...
    Matrix
    rankReducedSqrt(const Matrix& matrix, Size maxRank, Real componentRetainedPercentage, SalvagingAlgorithm::Type sa)
    {
        Size size = matrix.rows();
        ...
        QL_REQUIRE(componentRetainedPercentage > 0.0, "no eigenvalues retained");

        QL_REQUIRE(componentRetainedPercentage <= 1.0, "percentage to be retained > 100%");

        QL_REQUIRE(maxRank >= 1, "max rank required < 1");
        ...
        switch (sa)
        {
            case SalvagingAlgorithm::None:
                ...
                QL_REQUIRE(eigenValues[size - 1] >= -1e-16,
                           "negative eigenvalue(s) (" << std::scientific << eigenValues[size - 1] << ")");
                break;
            case SalvagingAlgorithm::Spectral:
                // negative eigenvalues set to zero
                for (Size i = 0; i < size; ++i)
        ...
        // factor reduction
        Real enough = componentRetainedPercentage * std::accumulate(eigenValues.begin(), eigenValues.end(), Real(0.0));
