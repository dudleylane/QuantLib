...
namespace QuantLib
{
        ...
        for (Size k = n - 1; k >= 1; --k)
        {
            while (!offDiagIsZero(k, e))
            {
                ...
                while (--l > 0 && !offDiagIsZero(l, e))
                    ; // NOLINT(bugprone-inc-dec-in-conditions)
                iter_++;
                ...
                if (strategy != NoShift)
                {
                    ...
                    const Real t1 = std::sqrt(0.25 * (d_[k] * d_[k] + d_[k - 1] * d_[k - 1]) - 0.5 * d_[k - 1] * d_[k] +
                                              e[k] * e[k]);
                    const Real t2 = 0.5 * (d_[k] + d_[k - 1]);

                    const Real lambda =
                        (std::fabs(t2 + t1 - d_[k]) < std::fabs(t2 - t1 - d_[k])) ? Real(t2 + t1) : Real(t2 - t1);
    ...
    {
        return std::fabs(d_[k - 1]) + std::fabs(d_[k]) == std::fabs(d_[k - 1]) + std::fabs(d_[k]) + std::fabs(e[k]);
    }
