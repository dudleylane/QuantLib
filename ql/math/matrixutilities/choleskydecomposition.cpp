...
namespace QuantLib
{
    ...
    Matrix CholeskyDecomposition(const Matrix& S, bool flexible)
    {
        ...
        QL_REQUIRE(size == S.columns(), "input matrix is not a square matrix");
#if defined(QL_EXTRA_SAFETY_CHECKS)
        for (i = 0; i < S.rows(); i++)
            for (j = 0; j < i; j++)
                QL_REQUIRE(S[i][j] == S[j][i], "input matrix is not symmetric");
#endif
        ...
        for (i = 0; i < size; i++)
        {
            for (j = i; j < size; j++)
            {
                ...
                if (i == j)
                {
                    QL_REQUIRE(flexible || sum > 0.0, "input matrix is not positive definite");
    ...
    Array CholeskySolveFor(const Matrix& L, const Array& b)
    {
        ...
        QL_REQUIRE(L.columns() == n && L.rows() == n, "Size of input matrix and vector does not match.");
