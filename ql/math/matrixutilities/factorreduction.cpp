...
namespace QuantLib
{
    ...
    std::vector<Real> factorReduction(Matrix mtrx, Size maxIters)
    {
        ...
        QL_REQUIRE(mtrx.rows() == mtrx.columns(), "Input matrix is not square");
        ...
        for (Size iRow = 0; iRow < mtrx.rows(); iRow++)
            for (Size iCol = 0; iCol < iRow; iCol++)
                QL_REQUIRE(mtrx[iRow][iCol] == mtrx[iCol][iRow], "input matrix is not symmetric");
        ...
        do
        {
            ...
            Array::const_iterator itMaxEval = std::max_element(eigenVals.begin(), eigenVals.end());
            ...
            for (Size iCol = 0; iCol < n; iCol++)
            {
                ...
                distances.push_back(std::abs(thisCorrel - previousCorrels[iCol]) / previousCorrels[iCol]);
            }
        ...
        QL_REQUIRE(iteration < maxIters, "convergence not reached after " << iteration << " iterations");

#if defined(QL_EXTRA_SAFETY_CHECKS)
        QL_REQUIRE(*std::max_element(previousCorrels.begin(), previousCorrels.end()) <= 1 &&
                       *std::min_element(previousCorrels.begin(), previousCorrels.end()) >= -1,
