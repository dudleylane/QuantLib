...
namespace QuantLib
{
    ...
    */
    ...
    Matrix getCovariance(DataIterator stdDevBegin, DataIterator stdDevEnd, const Matrix& corr, Real tolerance = 1.0e-12)
    {
        ...
        QL_REQUIRE(corr.rows() == size, "dimension mismatch between volatilities ("
                                            << size << ") and correlation rows (" << corr.rows() << ")");
        QL_REQUIRE(corr.columns() == size,
                   "correlation matrix is not square: " << size << " rows and " << corr.columns() << " columns");
        ...
        for (i = 0, iIt = stdDevBegin; i < size; ++i, ++iIt)
        {
            ...
            QL_REQUIRE(std::fabs(corr[i][i] - 1.0) <= tolerance, "invalid correlation matrix, "
