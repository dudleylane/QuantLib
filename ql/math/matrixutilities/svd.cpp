        ...
        */
        ...
        while (p > 0)
        {
            ...
            for (k = p - 2; k >= -1; --k)
            {
                ...
                if (std::fabs(e[k]) <= eps * (std::fabs(s_[k]) + std::fabs(s_[k + 1])))
                {
                    e[k] = 0.0;
                    break;
                }
            ...
            else
            {
                ...
                for (ks = p - 1; ks >= k; --ks)
                {
                    ...
                    Real t = (ks != p ? Real(std::fabs(e[ks])) : 0.) + (ks != k + 1 ? Real(std::fabs(e[ks - 1])) : 0.);
            ...
            { // NOLINT(bugprone-switch-missing-default-case)
                ...
                case 3:
                {
                    ...
                    // Calculate the shift.
                    Real scale = std::max({std::fabs(s_[p - 1]), std::fabs(s_[p - 2]), std::fabs(e[p - 2]),
                                           std::fabs(s_[k]), std::fabs(e[k])});
