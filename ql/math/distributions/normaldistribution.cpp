...
namespace QuantLib
{
    ...
    Real CumulativeNormalDistribution::operator()(Real z) const
    {
        ...
        { // todo: investigate the threshold level
            ...
            Real sum = 1.0, zsqr = z * z, i = 1.0, g = 1.0, x, y, a = QL_MAX_REAL, lasta;
            do
            {
                lasta = a;
                x = (4.0 * i - 3.0) / zsqr;
    ...
    Real InverseCumulativeNormal::tail_value(Real x)
    {
        if (x <= 0.0 || x >= 1.0)
        {
            ...
            {
                QL_FAIL("InverseCumulativeNormal(" << x << ") undefined: must be 0 < x < 1");
            }
        }
    ...
    {
        QL_REQUIRE(x > 0.0 && x < 1.0, "MoroInverseCumulativeNormal(" << x << ") undefined: must be 0<x<1");
        ...
        if (std::fabs(temp) < 0.42)
        {
            ...
            result = temp * temp;
            result = temp * (((a3_ * result + a2_) * result + a1_) * result + a0_) /
                ...
                result *
                    (c1_ +
                     result *
                         (c2_ +
                          result * (c3_ +
                                    result * (c4_ + result * (c5_ + result * (c6_ + result * (c7_ + result * c8_)))))));
            if (x < 0.5)
                result = -result;
        }
    ...
    MaddockInverseCumulativeNormal::MaddockInverseCumulativeNormal(Real average, Real sigma)
    ...
    {
        return boost::math::quantile(boost::math::normal_distribution<Real>(average_, sigma_), x);
    }

    MaddockCumulativeNormal::MaddockCumulativeNormal(Real average, Real sigma) : average_(average), sigma_(sigma) {}

    Real MaddockCumulativeNormal::operator()(Real x) const
    {
        return boost::math::cdf(boost::math::normal_distribution<Real>(average_, sigma_), x);
    }
}
