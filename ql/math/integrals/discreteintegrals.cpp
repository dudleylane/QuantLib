...
namespace QuantLib
{
    ...
    Real DiscreteTrapezoidIntegral::operator()(const Array& x, const Array& f) const
    {
    ...
    Real DiscreteSimpsonIntegral::operator()(const Array& x, const Array& f) const
    {
    ...
    Real DiscreteTrapezoidIntegrator::integrate(const std::function<Real(Real)>& f, Real a, Real b) const
    {
    ...
    Real DiscreteSimpsonIntegrator::integrate(const std::function<Real(Real)>& f, Real a, Real b) const
    {
        const Size n = maxEvaluations() - 1;
        const Real d = (b - a) / n, d2 = d * 2;

        Real sum = 0.0, x = a + d;
        for (Size i = 1; i < n; i += 2)
        { // to time 4
            sum += f(x);
            x += d2;
        }
