...
namespace QuantLib
{
    namespace exponential_integrals_helper
    {
        ...
        Real f(Real x)
        {
            ...
            return (1 + x2 * (7.44437068161936700618e2 +
                   ...
                   (x * (1 + x2 * (7.46437068161927678031e2 +
    ...
    namespace ExponentialIntegral
    {
        ...
        std::complex<Real> Ei(const std::complex<Real>& z, const std::complex<Real>& acc)
        {
            ...
            if (z.real() == 0.0 && z.imag() == 0.0 && std::numeric_limits<Real>::has_infinity)
            {
            ...
            const auto match = [=](const std::complex<Real>& z1, const std::complex<Real>& z2) -> bool
            {
            ...
            if (abs_z > DIST && (z.real() < 0 || std::abs(z.imag()) > DIST))
            {
                ...
                return (acc + std::complex<Real>(0.0, sign(z.imag()) * M_PI)) - std::exp(z) / (1.0 - z + ei);
            }
            ...
            const std::complex<Real> r = (M_EULER_MASCHERONI + acc) + std::log(z) + std::exp(0.5 * z) * s;
        ...
        std::complex<Real> Si(const std::complex<Real>& z)
        {
            ...
            else
            {
                ...
                return 0.5 * i *
                       (E1(-i * z) - E1(i * z) -
                        std::complex<Real>(
                            0.0, ((z.real() >= 0 && z.imag() >= 0) || (z.real() > 0 && z.imag() < 0)) ? M_PI : -M_PI));
            }
        }
