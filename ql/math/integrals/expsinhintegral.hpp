...
namespace QuantLib
{
    ...
    */
    ...
    {
      public:
        explicit ExpSinhIntegral(Real relTolerance = std::sqrt(std::numeric_limits<Real>::epsilon()),
                                 Size maxRefinements = 9)
        : Integrator(QL_MAX_REAL, Null<Size>()), relTolerance_(relTolerance), exp_sinh_(maxRefinements)
        {
        }
