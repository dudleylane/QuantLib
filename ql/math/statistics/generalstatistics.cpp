...
namespace QuantLib
{
    ...
    {
        Size N = samples();
        QL_REQUIRE(N > 2, "sample number <=2, unsufficient");

        Real m = mean();
        Real X = expectationValue(
                     [=](Real x) -> Real
                     {
                         Real d = x - m;
                         return d * d * d;
    ...
    {
        Size N = samples();
        QL_REQUIRE(N > 3, "sample number <=3, unsufficient");
    ...
    Real GeneralStatistics::percentile(Real percent) const
    {
        ...
        QL_REQUIRE(percent > 0.0 && percent <= 1.0, "percentile (" << percent << ") must be in (0.0, 1.0]");
