...
namespace QuantLib
{
    ...
    class NoConstraint : public Constraint
    {
      ...
      public:
        NoConstraint() : Constraint(ext::shared_ptr<Constraint::Impl>(new NoConstraint::Impl)) {}
    };
      ...
      private:
        class Impl final : public Constraint::Impl
        {
          public:
            ...
            Array lowerBound(const Array& params) const override { return Array(params.size(), 0.0); }
        };

      public:
        PositiveConstraint() : Constraint(ext::shared_ptr<Constraint::Impl>(new PositiveConstraint::Impl)) {}
    };
    ...
    class BoundaryConstraint : public Constraint
    {
      private:
        ...
        {
          public:
            Impl(Real low, Real high) : low_(low), high_(high) {}
            bool test(const Array& params) const override
            {
                return std::all_of(params.begin(), params.end(), [this](Real p) { return low_ <= p && p <= high_; });
            }
            Array upperBound(const Array& params) const override { return Array(params.size(), high_); }
            Array lowerBound(const Array& params) const override { return Array(params.size(), low_); }
      ...
      public:
        ...
        : Constraint(ext::shared_ptr<Constraint::Impl>(new BoundaryConstraint::Impl(low, high)))
        {
        }
    };
    ...
    class CompositeConstraint : public Constraint
    {
      private:
        class Impl final : public Constraint::Impl
        {
          public:
            ...
            bool test(const Array& params) const override { return c1_.test(params) && c2_.test(params); }
            Array upperBound(const Array& params) const override
            {
      ...
      public:
        ...
        : Constraint(ext::shared_ptr<Constraint::Impl>(new CompositeConstraint::Impl(c1, c2)))
        {
        }
    };
    ...
    class NonhomogeneousBoundaryConstraint : public Constraint
    {
      private:
        class Impl final : public Constraint::Impl
        {
          public:
            ...
            {
                QL_ENSURE(low_.size() == high_.size(), "Upper and lower boundaries sizes are inconsistent.");
            }
            bool test(const Array& params) const override
            {
                QL_ENSURE(params.size() == low_.size(), "Number of parameters and boundaries sizes are inconsistent.");
      ...
      public:
        ...
        : Constraint(ext::shared_ptr<Constraint::Impl>(new NonhomogeneousBoundaryConstraint::Impl(low, high)))
        {
        }
    };
