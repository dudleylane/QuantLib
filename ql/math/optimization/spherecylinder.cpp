...
namespace QuantLib
{
    ...
    namespace
    {
        ...
        template <class F>
        Real BrentMinimize(Real low, Real mid, Real high, Real tolerance, Size maxIt, const F& objectiveFunction)
        {
            ...
            while (high - low > tolerance && iterations < maxIt)
            {
                ...
                { // left interval is bigger
                    ...
                    Real tentativeNewMidValue = objectiveFunction(tentativeNewMid);
    ...
    SphereCylinderOptimizer::SphereCylinderOptimizer(
        Real r, Real s, Real alpha, Real z1, Real z2, Real z3, Real zweight)
    ...
    void SphereCylinderOptimizer::findClosest(Size maxIterations, Real tolerance, Real& y1, Real& y2, Real& y3) const
    {
        Real x1, x2, x3;
        findByProjection(x1, x2, x3);

        y1 = BrentMinimize(bottomValue_, x1, topValue_, tolerance, maxIterations,
    ...
    bool SphereCylinderOptimizer::findByProjection(Real& y1, Real& y2, Real& y3) const
    {
    ...
    std::vector<Real> sphereCylinderOptimizerClosest(
        Real r, Real s, Real alpha, Real z1, Real z2, Real z3, Natural maxIterations, Real tolerance, Real zweight)
    {

        SphereCylinderOptimizer optimizer(r, s, alpha, z1, z2, z3, zweight);
        std::vector<Real> y(3);

        QL_REQUIRE(optimizer.isIntersectionNonEmpty(), "intersection empty so no solution");
