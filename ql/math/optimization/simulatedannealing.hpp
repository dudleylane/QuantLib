    ...
    */
    ...
    class SimulatedAnnealing : public OptimizationMethod
    {
      ...
      public:
        ...
        : scheme_(ConstantFactor), lambda_(lambda), T0_(T0), epsilon_(epsilon), alpha_(0.0), K_(0), rng_(rng), m_(m)
        {
        }
        ...
        */
        ...
        : scheme_(ConstantBudget), lambda_(lambda), T0_(T0), epsilon_(0.0), alpha_(alpha), K_(K), rng_(rng)
        {
        }
    ...
    template <class RNG>
    Real SimulatedAnnealing<RNG>::simplexSize()
    { // this is taken from
      // simplex.cpp
    ...
    template <class RNG>
    EndCriteria::Type SimulatedAnnealing<RNG>::minimize(Problem& P, const EndCriteria& ec)
    {
        ...
        do
        {
            ...
            do
            {
                ...
                if (ec.checkStationaryPoint(simplexSize(), 0.0, stationaryStateIterations_, ecType) ||
                        ...
                        if (ytry_ >= ysave_)
                        {
                            for (i_ = 0; i_ < n_ + 1; i_++)
                            {
                                if (i_ != ilo_)
                                {
                                    for (j_ = 0; j_ < n_; j_++)
                                    {
                                        sum_[j_] = 0.5 * (vertices_[i_][j_] + vertices_[ilo_][j_]);
            ...
            } while (iteration_ < iterationT_ + (scheme_ == ConstantFactor ? m_ : 1));
