...
namespace QuantLib
{
    ...
    LevenbergMarquardt::LevenbergMarquardt(Real epsfcn, Real xtol, Real gtol, bool useCostFunctionsJacobian)
    : epsfcn_(epsfcn), xtol_(xtol), gtol_(gtol), useCostFunctionsJacobian_(useCostFunctionsJacobian)
    {
    }
    ...
    EndCriteria::Type LevenbergMarquardt::minimize(Problem& P, const EndCriteria& endCriteria)
    {
        ...
        QL_REQUIRE(m >= n, "less functions (" << m << ") than available variables (" << n << ")");
        ...
        MINPACK::lmdif(m, n, xx.begin(), fvec.get(), endCriteria.functionEpsilon(), xtol_, gtol_, maxfev, epsfcn_,
                       diag.get(), mode, factor, nprint, &info, &nfev, fjac.get(), ldfjac, ipvt.get(), qtf.get(),
                       wa1.get(), wa2.get(), wa3.get(), wa4.get(), lmdifCostFunction, lmdifJacFunction);
        // for the time being
        info_ = info;
