...
namespace QuantLib
{
    ...
    Array BFGS::getUpdatedDirection(const Problem& P, Real, const Array& oldGradient)
    {
        if (inverseHessian_.rows() == 0)
        {
            // first time in this update, we create needed structures
            inverseHessian_ = Matrix(P.currentValue().size(), P.currentValue().size(), 0.);
        ...
        if (fac > std::sqrt(1e-8 * sumdg * sumxi)) // skip update if fac not sufficiently positive
        {
            ...
            for (Size i = 0; i < P.currentValue().size(); ++i)
                ...
                {
                    inverseHessian_[i][j] +=
                        fac * lineSearch_->searchDirection()[i] * lineSearch_->searchDirection()[j];
                    inverseHessian_[i][j] -=
                        fad * diffGradientWithHessianApplied[i] * diffGradientWithHessianApplied[j];
