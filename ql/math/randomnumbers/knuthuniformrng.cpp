...
namespace QuantLib
{
    ...
    KnuthUniformRng::KnuthUniformRng(long seed) : ranf_arr_buf(QUALITY), ran_u(QUALITY)
    {
    ...
    void KnuthUniformRng::ranf_start(long seed)
    {
        ...
        while (t != 0)
        {
            ...
            for (j = KK + KK - 2; j >= KK; --j)
                if (ul[j] != 0.0)
                {
                    ul[j - (KK - LL)] = ulp - ul[j - (KK - LL)], u[j - (KK - LL)] = mod_sum(u[j - (KK - LL)], u[j]);
    ...
    void KnuthUniformRng::ranf_array(std::vector<double>& aa, int n) const
    {
        int i, j;
        for (j = 0; j < KK; j++)
            aa[j] = ran_u[j];
