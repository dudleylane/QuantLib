...
namespace QuantLib
{
    ...
    class FastFourierTransform
    {
      public:
        ...
        {
            return static_cast<std::size_t>(std::ceil(std::log(static_cast<Real>(inputSize)) / M_LN2));
        }

        FastFourierTransform(std::size_t order) : cs_(order), sn_(order)
        {
        ...
        std::size_t output_size() const { return (static_cast<std::size_t>(1) << cs_.size()); }
      ...
      private:
        ...
        {
            typedef typename std::iterator_traits<RandomAccessIterator>::value_type complex;
