...
namespace QuantLib
{
    ...
    */
    class GeneralLinearLeastSquares
    {
      public:
        ...
        GeneralLinearLeastSquares(
            xIterator xBegin, xIterator xEnd, yIterator yBegin, yIterator yEnd, vIterator vBegin, vIterator vEnd);
    ...
    template <class xIterator, class yIterator, class vIterator>
    inline GeneralLinearLeastSquares::GeneralLinearLeastSquares(
        xIterator xBegin, xIterator xEnd, yIterator yBegin, yIterator yEnd, vIterator vBegin, vIterator vEnd)
    : a_(std::distance(vBegin, vEnd), 0.0), err_(a_.size(), 0.0), residuals_(std::distance(yBegin, yEnd)),
        ...
        xIterator xBegin, xIterator xEnd, yIterator yBegin, yIterator yEnd, vIterator vBegin)
    {
        ...
        QL_REQUIRE(n == Size(std::distance(yBegin, yEnd)), "sample set need to be of the same size");
        ...
        for (i = 0; i < m; ++i)
        {
            if (w[i] > threshold)
            {
                const Real u = std::inner_product(U.column_begin(i), U.column_end(i), yBegin, Real(0.0)) / w[i];
        ...
        const Real chiSq = std::inner_product(residuals_.begin(), residuals_.end(), residuals_.begin(), Real(0.0));
