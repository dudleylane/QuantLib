...
namespace QuantLib
{
    ...
    : r_(r), dc_(std::move(dc)), comp_(comp), freqMakesSense_(false)
    {
        ...
        if (comp_ == Compounded || comp_ == SimpleThenCompounded || comp_ == CompoundedThenSimple)
        {
            ...
            QL_REQUIRE(freq != Once && freq != NoFrequency, "frequency not allowed for this interest rate");
            freq_ = Real(freq);
        }
    }
    ...
    InterestRate
    InterestRate::impliedRate(Real compound, const DayCounter& resultDC, Compounding comp, Frequency freq, Time t)
    {
                ...
                switch (ir.frequency())
                {
                    ...
                    case Once:
                        QL_FAIL(ir.frequency() << " frequency not allowed "
                                                  "for this interest rate");
                    default:
                        out << "simple compounding up to " << Integer(12 / ir.frequency()) << " months, then "
                ...
                switch (ir.frequency())
                {
                    ...
                    case Once:
                        QL_FAIL(ir.frequency() << " frequency not allowed "
                                                  "for this interest rate");
                    default:
                        out << "compounding up to " << Integer(12 / ir.frequency()) << " months, then "
