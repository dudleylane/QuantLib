...
namespace QuantLib
{
    ...
    : swapLengths_(std::move(swapLengths)), swapIndexes_(std::move(swapIndexes)), iborIndex_(std::move(iborIndex)),
      bidAskSpreads_(bidAskSpreads), pricers_(pricers), discTS_(std::move(discountingTS)),

      nExercise_(swapLengths_.size()), nSwapIndexes_(swapIndexes_.size()), swapTenors_(nSwapIndexes_),
    ...
    {
        QL_REQUIRE(2 * nSwapIndexes_ == bidAskSpreads[0].size(), "2*nSwapIndexes_ (" << 2 * nSwapIndexes_
        ...
        for (Size i = 0; i < nExercise_; ++i)
        {
            ...
            for (Size j = 0; j < nSwapIndexes_; ++j)
            {
                ...
                spotSwaps_[i][j] = MakeCms(swapLengths_[i], swapIndexes_[j], iborIndex_, 0.0, Period())
                                       .operator ext::shared_ptr<Swap>();
                fwdSwaps_[i][j] = MakeCms(swapLengths_[i] - start, swapIndexes_[j], iborIndex_, 0.0, start)
    ...
    void CmsMarket::performCalculations() const
    {
        for (Size j = 0; j < nSwapIndexes_; ++j)
        {
            ...
            for (Size i = 0; i < nExercise_; ++i)
            {
                ...
                spotFloatLegNPV_[i][j] = CashFlows::npv(spotFloatLeg, **discTS_, false, discTS_->referenceDate());
                spotFloatLegBPS_[i][j] = CashFlows::bps(spotFloatLeg, **discTS_, false, discTS_->referenceDate());

                // imply the spot CMS leg NPV from the spot ibor floating leg NPV
                mktSpotCmsLegNPV_[i][j] = -(spotFloatLegNPV_[i][j] + spotFloatLegBPS_[i][j] * mktSpreads_[i][j] / 1e-4);
                ...
                errFwdCmsLegNPV_[i][j] = mdlFwdCmsLegNPV_[i][j] - mktFwdCmsLegNPV_[i][j];
                ...
                errSpotCmsLegNPV_[i][j] = mdlSpotCmsLegNPV_[i][j] - mktSpotCmsLegNPV_[i][j];
    ...
    void CmsMarket::reprice(const Handle<SwaptionVolatilityStructure>& v, Real meanReversion)
    {
        Handle<Quote> meanReversionQuote(ext::make_shared<SimpleQuote>(meanReversion));
        for (Size j = 0; j < nSwapIndexes_; ++j)
        {
            // ??
            ...
            {
                ext::shared_ptr<MeanRevertingPricer> p = ext::dynamic_pointer_cast<MeanRevertingPricer>(pricers_[j]);
        ...
        for (Size j = 0; j < nSwapIndexes_; ++j)
        {
            for (Size i = 0; i < nExercise_; ++i)
            {
                ...
                if (mdlSpreads_[i][j] > mktAskSpreads_[i][j])
                    result[j * nExercise_ + i][7] = (mdlSpreads_[i][j] - mktAskSpreads_[i][j]) * 10000;
                else if (mdlSpreads_[i][j] < mktBidSpreads_[i][j])
                    result[j * nExercise_ + i][7] = (mktBidSpreads_[i][j] - mdlSpreads_[i][j]) * 10000;
                else
