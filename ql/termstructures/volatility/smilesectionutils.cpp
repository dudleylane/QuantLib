...
namespace QuantLib
{
                                         ...
                                         const bool deleteArbitragePoints)
    {
        ...
        {
            QL_REQUIRE(section.volatilityType() == Normal || moneynessGrid[0] >= 0.0,
                       "moneyness grid should only contain non negative values (" << moneynessGrid[0] << ")");
            for (Size i = 0; i < moneynessGrid.size() - 1; i++)
            {
                QL_REQUIRE(moneynessGrid[i] < moneynessGrid[i + 1], "moneyness grid should contain strictly increasing "
                                                                    "values ("
                                                                        << moneynessGrid[i] << ","
        ...
        {
            f_ = section.atmLevel();
            QL_REQUIRE(f_ != Null<Real>(), "atm level must be provided by source section or given "
                                           "in the constructor");
        }
        else
        {
        ...
        static const Real defaultMoneyNormal[] = {
            -0.20,  -0.15,  -0.10,  -0.075, -0.05, -0.04, -0.03, -0.02, -0.015, -0.01, -0.0075, -0.0050, -0.0025, 0.0,
            0.0025, 0.0050, 0.0075, 0.01,   0.015, 0.02,  0.03,  0.04,  0.05,   0.075, 0.10,    0.15,    0.20};

        if (moneynessGrid.empty())
        {
            tmp = section.volatilityType() == Normal ? std::vector<Real>(defaultMoneyNormal, defaultMoneyNormal + 27) :
        ...
        for (Real& i : tmp)
        {
            ...
            { // if the section provides a limited strike range
              ...
              // in order to not loose too much information
                ...
                {
                    m_.push_back(section.volatilityType() == Normal ? Real(section.minStrike() - f_) :
                ...
                {
                    m_.push_back(section.volatilityType() == Normal ? Real(section.maxStrike() - f_) :
        ...
        for (Size i = (section.volatilityType() == Normal ? 0 : 1); i < k_.size(); i++)
        {
            c_.push_back(section.optionPrice(k_[i], Option::Call, 1.0));
        }

        Size centralIndex =
            std::upper_bound(m_.begin(), m_.end(), (section.volatilityType() == Normal ? 0.0 : 1.0) - QL_EPSILON) -
            m_.begin();
        ...
        QL_REQUIRE(centralIndex < k_.size(), "central index is at right boundary");
        ...
        QL_REQUIRE(rightIndex_ > leftIndex_, "arbitrage free region must at least contain two "
