...
namespace QuantLib
{
    ...
    unsafeSabrLogNormalVolatility(Rate strike, Rate forward, Time expiryTime, Real alpha, Real beta, Real nu, Real rho)
    {
        ...
        const Real d =
            1.0 + expiryTime * (oneMinusBeta * oneMinusBeta * alpha * alpha / (24.0 * A) +
                                0.25 * rho * beta * nu * alpha / sqrtA + (2.0 - 3.0 * rho * rho) * (nu * nu / 24.0));
    ...
    unsafeSabrNormalVolatility(Rate strike, Rate forward, Time expiryTime, Real alpha, Real beta, Real nu, Real rho)
    {
        ...
        const Real d =
            1.0 + expiryTime * (minusBeta * (2 - beta) * alpha * alpha / (24.0 * A) +
                                0.25 * rho * beta * nu * alpha / sqrtA + (2.0 - 3.0 * rho * rho) * (nu * nu / 24.0));
    ...
    Real sabrVolatility(Rate strike,
                        ...
                        VolatilityType volatilityType)
    {
        QL_REQUIRE(strike > 0.0, "strike must be positive: " << io::rate(strike) << " not allowed");
    ...
    Real shiftedSabrVolatility(Rate strike,
                               ...
                               VolatilityType volatilityType)
    {
        QL_REQUIRE(strike + shift > 0.0,
                   "strike+shift must be positive: " << io::rate(strike) << "+" << io::rate(shift) << " not allowed");
        QL_REQUIRE(forward + shift > 0.0, "at the money forward rate + shift must be "
                                          "positive: "
                                              << io::rate(forward) << " " << io::rate(shift) << " not allowed");
        QL_REQUIRE(expiryTime >= 0.0, "expiry time must be non-negative: " << expiryTime << " not allowed");
        validateSabrParameters(alpha, beta, nu, rho);
        return unsafeShiftedSabrVolatility(strike, forward, expiryTime, alpha, beta, nu, rho, shift, volatilityType);
    }
    ...
    namespace
    {
        struct SabrFlochKennedyVolatility
        {
            ...
            Real y(Real k) const { return -1.0 / (1.0 - beta) * (std::pow(F, 1 - beta) - std::pow(k, 1 - beta)); }

            Real Dint(Real k) const
            {
                return 1 / nu *
                       std::log((std::sqrt(1 + 2 * rho * nu / alpha * y(k) + squared(nu / alpha * y(k))) - rho -
                                 nu / alpha * y(k)) /
                                (1 - rho));
            }
            ...
            Real D(Real k) const
            {
                return std::sqrt(alpha * alpha + 2 * alpha * rho * nu * y(k) + squared(nu * y(k))) * std::pow(k, beta);
            }

            Real omega0(Real k) const { return std::log(F / k) / Dint(k); }
            ...
            Real operator()(Real k) const
            {
                ...
                {
                    return omega0(k) *
                               (1 + 0.25 * rho * nu * alpha * (std::pow(k, beta) - std::pow(F, beta)) / (k - F) * t) -
                           omega0(k) / squared(Dint(k)) *
                               (std::log(omega0(k)) + 0.5 * std::log((F * k / (D(F) * D(k))))) * t;
                }
                else
                {
            ...
            Real taylorExpansion(Real k) const
            {
                ...
                return (alpha * std::pow(F, -3 + beta) *
                        (alpha2 * squared(-1 + beta) * std::pow(F, 2 * beta) * t +
                         6 * alpha * beta * nu * std::pow(F, 1 + beta) * rho * t +
                         F2 * (24 + nu * nu * (2 - 3 * rho2) * t))) /
                           24.0 +
                       (3 * alpha2 * alpha * std::pow(-1 + beta, 3) * std::pow(F, 3 * beta) * t +
                        3 * alpha2 * (-1 + beta) * (-1 + 5 * beta) * nu * std::pow(F, 1 + 2 * beta) * rho * t +
                        nu * F2 * F * rho * (24 + nu * nu * (-4 + 3 * rho2) * t) +
                        alpha * std::pow(F, 2 + beta) *
                            (24 * (-1 + beta) + nu * nu * (2 * (-1 + beta) + 3 * (1 + beta) * rho2) * t)) /
                           (48. * F2 * F2) * (k - F) +
                       (std::pow(F, -5 - beta) *
                        (alpha2 * alpha2 * std::pow(-1 + beta, 3) * (-209 + 119 * beta) * std::pow(F, 4 * beta) * t +
                         30 * alpha2 * alpha * (-1 + beta) * (9 + beta * (-37 + 18 * beta)) * nu *
                             std::pow(F, 1 + 3 * beta) * rho * t -
                         30 * alpha * nu * std::pow(F, 3 + beta) * rho *
                             (24 + nu * nu * (-4 * (1 + beta) + 3 * (1 + 2 * beta) * rho2) * t) +
                         10 * alpha2 * std::pow(F, 2 + 2 * beta) *
                             (24 * (-4 + beta) * (-1 + beta) +
                              nu * nu * (2 * (-1 + beta) * (-7 + 4 * beta) + 3 * (-4 + beta * (-7 + 5 * beta)) * rho2) *
                                  t) +
    ...
    {
        const SabrFlochKennedyVolatility v = {forward, alpha, beta, nu, rho, expiryTime};
                                  ...
                                  VolatilityType volatilityType)
    {
        ...
        switch (volatilityType)
        {
            case ShiftedLognormal:
            {
                ...
                Real nu2 = 3 * sigma_0 * sigma_2 - 0.5 * squared(1 - beta) * sigma_0 * sigma_0 +
            ...
            case Normal:
            {
                ...
                Real nu2 =
                    squared(1 / (f + b)) *
                    (3 * sigma_0 * sigma_2 - 0.5 * (beta * beta + beta) * (sigma_0 * sigma_0) -
                     3 * sigma_0 * (sigma_1 - 0.5 * beta * sigma_0) + 1.5 * squared(2 * sigma_1 - beta * sigma_0));
                Real nu, rho;
                if (nu2 > 0.0)
                {
                    nu = std::sqrt(nu2);
    ...
    std::array<Real, 4>
    sabrGuess(Real, Volatility, Real, Volatility, Real, Volatility, Rate, Time, Real, Real, VolatilityType)
    {
    ...
    bool sabrIsRiskyRegime(Real forward, Time expiryTime, Real alpha, Real beta, Real nu, Real /*rho*/)
    {
        ...
        const Real localVol = alpha * std::pow(forward, std::max(beta - 1.0, -1.0));
        const Real scaledNu = nu * std::sqrt(expiryTime) / std::max(localVol, QL_EPSILON);
