...
namespace QuantLib
{
    ...
    OptionletStripper1::OptionletStripper1(const ext::shared_ptr<CapFloorTermVolSurface>& termVolSurface,
    ...
    : OptionletStripper(termVolSurface, index, discount, type, displacement, optionletFrequency),
      floatingSwitchStrike_(switchStrike == Null<Rate>()), switchStrike_(switchStrike), accuracy_(accuracy),
      maxIter_(maxIter), dontThrow_(dontThrow)
    {
    ...
    void OptionletStripper1::performCalculations() const
    {
        ...
        ext::shared_ptr<BlackCapFloorEngine> dummy(new BlackCapFloorEngine( // discounting does not matter here
        ...
        for (Size i = 0; i < nOptionletTenors_; ++i)
        {
            ...
            ext::shared_ptr<FloatingRateCoupon> lFRC = temp.lastFloatingRateCoupon();
        ...
        if (volatilityType_ == ShiftedLognormal)
        {
            capFloorEngine = ext::make_shared<BlackCapFloorEngine>(
                ...
                discountCurve, Handle<Quote>(volQuote), dc, displacement_);
        }
        else if (volatilityType_ == Normal)
        {
            capFloorEngine = ext::make_shared<BachelierCapFloorEngine>(

                discountCurve, Handle<Quote>(volQuote), dc);
        }
        else
        {
        ...
        for (Size j = 0; j < nStrikes_; ++j)
        {
            ...
            for (Size i = 0; i < nOptionletTenors_; ++i)
            {
                ...
                ext::shared_ptr<CapFloor> capFloor =
                    MakeCapFloor(capFloorType, capFloorLengths_[i], iborIndex_, strikes[j], -0 * Days)
                ...
                try
                {
                    ...
                    {
                        optionletStDevs_[i][j] = std::sqrt(optionletTimes_[i]) *
