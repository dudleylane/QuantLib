...
namespace QuantLib
{
    ...
    StrippedOptionlet::StrippedOptionlet(Natural settlementDays,
                                         ...
                                         VolatilityType type,
                                         Real displacement)
    : calendar_(calendar), settlementDays_(settlementDays), businessDayConvention_(bdc), dc_(std::move(dc)),
      iborIndex_(std::move(iborIndex)), type_(type), displacement_(displacement),
      nOptionletDates_(optionletDates.size()), optionletDates_(optionletDates), optionletTimes_(nOptionletDates_),
      optionletAtmRates_(nOptionletDates_), optionletStrikes_(strikes), optionletVolQuotes_(std::move(v))
    {
        checkInputs();
        ...
        Date refDate = calendar.advance(Settings::instance().evaluationDate(), settlementDays, Days);
    ...
    void StrippedOptionlet::checkInputs() const
    {
        ...
        QL_REQUIRE(nOptionletDates_ == optionletVolQuotes_.size(),
                   "mismatch between number of option tenors ("
                       << nOptionletDates_ << ") and number of volatility rows (" << optionletVolQuotes_.size() << ")");
        ...
        for (Size i = 1; i < nOptionletDates_; ++i)
            QL_REQUIRE(optionletDates_[i] > optionletDates_[i - 1],
                       "non increasing option dates: " << io::ordinal(i) << " is " << optionletDates_[i - 1] << ", "
        ...
        QL_REQUIRE(nOptionletDates_ == optionletStrikes_.size(), "mismatch between number of option tenors ("
                                                                     << nOptionletDates_ << ") and number of strikes ("
            ...
            for (Size j = 1; j < optionletStrikes_[i].size(); ++j)
                QL_REQUIRE(optionletStrikes_[i][j - 1] < optionletStrikes_[i][j],
                           "non increasing strikes at date "
                               << i << ": " << io::ordinal(j) << " is " << io::rate(optionletStrikes_[0][j - 1]) << ", "
    ...
    const vector<Volatility>& StrippedOptionlet::optionletVolatilities(Size i) const
    {
        calculate();
