...
namespace QuantLib
{
    ...
    AbcdFunction::AbcdFunction(Real a, Real b, Real c, Real d) : AbcdMathFunction(a, b, c, d) {}
    ...
    Real AbcdFunction::primitive(Time t, Time T, Time S) const
    {
        ...
        return (b_ * b_ *
                    (-1 - 2 * c_ * c_ * S * T - c_ * (S + T) +
                     k1 * k1 * (1 + c_ * (S + T - 2 * t) + 2 * c_ * c_ * (S - t) * (T - t))) +
                2 * c_ * c_ *
                    (2 * d_ * a_ * (k2 + k3) * (k1 - 1) + a_ * a_ * (k1 * k1 - 1) + 2 * c_ * d_ * d_ * k2 * k3 * t) +
                2 * b_ * c_ *
                    (a_ * (-1 - c_ * (S + T) + k1 * k1 * (1 + c_ * (S + T - 2 * t))) -
                     2 * d_ *
                         (k3 * (1 + c_ * S) + k2 * (1 + c_ * T) - k1 * k3 * (1 + c_ * (S - t)) -
                          k1 * k2 * (1 + c_ * (T - t))))) /
               (4 * c_ * c_ * c_ * k2 * k3);
    }
    ...
    : abcd_(new AbcdFunction(a, b, c, d)), T_(T), S_(S)
    {
    }
