...
namespace QuantLib
{
    ...
    BlackVolatilitySurfaceDelta::BlackVolatilitySurfaceDelta(
        ...
        SmileInterpolationMethod im,
        bool flatStrikeExtrapolation,
        BlackVolTimeExtrapolation::Type timeExtrapolationType,
        const Period& switchTenor,
        DeltaVolQuote::DeltaType longTermDeltaType,
        DeltaVolQuote::AtmType longTermAtmType,
    ...
    : BlackVolatilityTermStructure(referenceDate, cal, Following, dayCounter), dates_(dates), times_(dates.size(), 0),
      ...
      foreignTS_(foreignTS), deltaType_(deltaType), atmType_(atmType),
      atmDeltaType_(atmDeltaType ? *atmDeltaType : deltaType), interpolationMethod_(im),
      ...
      longTermAtmDeltaType_(longTermAtmDeltaType ? *longTermAtmDeltaType : longTermDeltaType)
    {
        ...
        for (Size i = 0; i < n; i++)
        {
            ...
            interpolators_.push_back(ext::make_shared<BlackVarianceCurve>(
                referenceDate, dates, vols, dayCounter, forceMonotoneVariance, timeExtrapolationType_));
        }
        ...
        else
        {
            ...
            if (interpolationMethod_ == SmileInterpolationMethod::Linear)
                return ext::make_shared<InterpolatedSmileSection<Linear>>(
                    t, strikes, stdDevs, atmLevel, Linear(), dayCounter(), VolatilityType::ShiftedLognormal, 0.0,
                ...
                return ext::make_shared<InterpolatedSmileSection<Cubic>>(
                    t, strikes, stdDevs, atmLevel, Cubic(CubicInterpolation::Kruger), dayCounter(),
    ...
    Volatility BlackVolatilitySurfaceDelta::blackVolImpl(Time t, Real strike) const
    {
        ...
        Time tme = (t > times_.back() && timeExtrapolationType_ == BlackVolTimeExtrapolation::FlatVolatility) ?
                       times_.back() :
                       t;
