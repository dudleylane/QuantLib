...
namespace QuantLib
{
    ...
    {
      public:
        AndreasenHugeCostFunction(Array marketNPVs,
                                  Array marketVegas,
        ...
        : marketNPVs_(std::move(marketNPVs)), marketVegas_(std::move(marketVegas)),
          lnMarketStrikes_(std::move(lnMarketStrikes)), previousNPVs_(std::move(previousNPVs)), mesher_(mesher),
          nGridPoints_(mesher->layout()->size()), dT_(dT),
          interpolationType_((lnMarketStrikes_.size() > 1) ? interpolationType :
                                                             AndreasenHugeVolatilityInterpl::PiecewiseConstant),
          dxMap_(FirstDerivativeOp(0, mesher_)), dxxMap_(SecondDerivativeOp(0, mesher_)),
          d2CdK2_(dxMap_.mult(Array(mesher->layout()->size(), -1.0)).add(dxxMap_)), mapT_(0, mesher_)
        {
        }

        Array d2CdK2(const Array& c) const { return d2CdK2_.apply(c); }
        ...
        Array solveFor(Time dT, const Array& sig, const Array& b) const
        {
            ...
            switch (interpolationType_)
            {
                case AndreasenHugeVolatilityInterpl::CubicSpline:
                    sigInterpl = CubicNaturalSpline(lnMarketStrikes_.begin(), lnMarketStrikes_.end(), sig.begin());
                    break;
                case AndreasenHugeVolatilityInterpl::Linear:
                    sigInterpl = LinearInterpolation(lnMarketStrikes_.begin(), lnMarketStrikes_.end(), sig.begin());
                    break;
            ...
            for (const auto& iter : *mesher_->layout())
            {
                ...
                const Real vol =
                    sigInterpl(std::min(std::max(lnStrike, lnMarketStrikes_.front()), lnMarketStrikes_.back()), true);
        ...
        Array apply(const Array& c) const { return -mapT_.apply(c); }

        Array values(const Array& sig) const override
        {
            Array newNPVs = solveFor(dT_, sig, previousNPVs_);

            const std::vector<Real>& gridPoints = mesher_->getFdm1dMeshers().front()->locations();

            const MonotonicCubicNaturalSpline interpl(gridPoints.begin(), gridPoints.end(), newNPVs.begin());
        ...
        Array vegaCalibrationError(const Array& sig) const { return values(sig) / marketVegas_; }

        Array initialValues() const { return Array(lnMarketStrikes_.size(), 0.25); }
      ...
      private:
        ...
        const Time dT_;
        const AndreasenHugeVolatilityInterpl::InterpolationType interpolationType_;
    ...
    class CombinedCostFunction : public CostFunction
    {
      public:
        ...
        {
            if ((putCostFct_ != nullptr) && (callCostFct_ != nullptr))
                return 0.5 * (putCostFct_->initialValues() + callCostFct_->initialValues());
            else if (putCostFct_ != nullptr)
    ...
    : spot_(std::move(spot)), rTS_(std::move(rTS)), qTS_(std::move(qTS)), interpolationType_(interplationType),
      calibrationType_(calibrationType), nGridPoints_(nGridPoints), minStrike_(_minStrike), maxStrike_(_maxStrike),
      optimizationMethod_(std::move(optimizationMethod)), endCriteria_(endCriteria)
    {
        ...
        for (const auto& i : calibrationSet)
        {
            ...
            QL_REQUIRE(exercise->type() == Exercise::European, "European option required");
        ...
        calibrationMatrix_ =
            std::vector<std::vector<Size>>(expiries.size(), std::vector<Size>(strikes.size(), Null<Size>()));

        for (Size i = 0; i < calibrationSet.size(); ++i)
        {
            const Date expiry = calibrationSet[i].first->exercise()->lastDate();

            const Size l = std::distance(expiries.begin(), expiries.lower_bound(expiry));

            const Real strike =
                ext::dynamic_pointer_cast<PlainVanillaPayoff>(calibrationSet[i].first->payoff())->strike();

            const Size k = std::distance(strikes_.begin(),
                                         std::find_if(strikes_.begin(), strikes_.end(), close_enough_to(strike)));
    ...
    ext::shared_ptr<AndreasenHugeCostFunction> AndreasenHugeVolatilityInterpl::buildCostFunction(
        Size iExpiry, Option::Type optionType, const Array& previousNPVs) const
    {

        if (calibrationType_ != CallPut && ((calibrationType_ == Call && optionType == Option::Put) ||
        ...
        Size null = Null<Size>();
        const Size nOptions = std::count_if(calibrationMatrix_[iExpiry].begin(), calibrationMatrix_[iExpiry].end(),
                                            [=](Size n) { return n != null; });

        Array lnMarketStrikes(nOptions), marketNPVs(nOptions), marketVegas(nOptions);
        ...
        for (Size j = 0, k = 0; j < strikes_.size(); ++j)
        {
            ...
            if (idx != null)
            {
                ...
                const BlackCalculator calculator(optionType, strikes_[j], fwd, stdDev, discount);
        ...
        return ext::make_shared<AndreasenHugeCostFunction>(marketNPVs, marketVegas, lnMarketStrikes, previousNPVs,
                                                           mesher_, dT_[iExpiry], interpolationType_);
    }


    void AndreasenHugeVolatilityInterpl::performCalculations() const
    {
        QL_REQUIRE(maxStrike() > minStrike(), "max strike must be greater than min strike");

        const DayCounter dc = rTS_->dayCounter();
        for (Size i = 0; i < expiryTimes_.size(); ++i)
        {
            expiryTimes_[i] = dc.yearFraction(rTS_->referenceDate(), expiries_[i]);
            dT_[i] = expiryTimes_[i] - ((i == 0) ? 0.0 : expiryTimes_[i - 1]);
        }

        mesher_ = ext::make_shared<FdmMesherComposite>(ext::make_shared<Concentrating1dMesher>(
            std::log(minStrike() / spot_->value()), std::log(maxStrike() / spot_->value()), nGridPoints_,
        ...
        {
            const ext::shared_ptr<AndreasenHugeCostFunction> putCostFct = buildCostFunction(i, Option::Put, npvPuts);
            const ext::shared_ptr<AndreasenHugeCostFunction> callCostFct = buildCostFunction(i, Option::Call, npvCalls);

            CombinedCostFunction costFunction(putCostFct, callCostFct);

            PositiveConstraint positiveConstraint;
            Problem problem(costFunction, positiveConstraint, costFunction.initialValues());
            ...
            const SingleStepCalibrationResult calibrationResult = {
                npvPuts, npvCalls, sig, (calibrationType_ == Call) ? callCostFct : putCostFct};
            ...
            switch (calibrationType_)
            {
                case CallPut:
                {
                    const Array vegaPutDiffs = putCostFct->vegaCalibrationError(sig);
                    const Array vegaCallDiffs = callCostFct->vegaCalibrationError(sig);

                    const Real fwd = spot_->value() * qTS_->discount(expiryTimes_[i]) / rTS_->discount(expiryTimes_[i]);

                    for (Size j = 0; j < vegaDiffs.size(); ++j)
                        vegaDiffs[j] = std::fabs((fwd > gridInFwd_[j]) ? vegaPutDiffs[j] : vegaCallDiffs[j]);
                }
                break;
                case Put:
            ...
            avgError_ += std::accumulate(vegaDiffs.begin(), vegaDiffs.end(), Real(0.0));
            minError_ = std::min(minError_, *std::min_element(vegaDiffs.begin(), vegaDiffs.end()));
            maxError_ = std::max(maxError_, *std::max_element(vegaDiffs.begin(), vegaDiffs.end()));
    ...
    {
        return (minStrike_ == Null<Real>()) ? 1 / 8.0 * strikes_.front() : minStrike_;
    }

    Real AndreasenHugeVolatilityInterpl::maxStrike() const
    {
        return (maxStrike_ == Null<Real>()) ? 8.0 * strikes_.back() : maxStrike_;
    }
    ...
    const Handle<YieldTermStructure>& AndreasenHugeVolatilityInterpl::riskFreeRate() const
    {
        return rTS_;
    }

    std::tuple<Real, Real, Real> AndreasenHugeVolatilityInterpl::calibrationError() const
    {
        calculate();
    ...
    Size AndreasenHugeVolatilityInterpl::getExerciseTimeIdx(Time t) const
    {
        return std::min<Size>(
            expiryTimes_.size() - 1,
            std::distance(expiryTimes_.begin(), std::upper_bound(expiryTimes_.begin(), expiryTimes_.end(), t)));
    }

    Real AndreasenHugeVolatilityInterpl::getCacheValue(Real strike, const TimeValueCacheType::const_iterator& f) const
    {

        const Real fwd = std::get<0>(f->second);
        const Real k = std::log(strike / fwd);

        const Real s = std::max(gridPoints_[1], std::min(*(gridPoints_.end() - 2), k));

        return (*(std::get<2>(f->second)))(s);
    }

    Array AndreasenHugeVolatilityInterpl::getPriceSlice(Time t, Option::Type optionType) const
    {

        const Size iu = getExerciseTimeIdx(t);

        return calibrationResults_[iu].costFunction->solveFor(
            (iu == 0) ? t : t - expiryTimes_[iu - 1], calibrationResults_[iu].sigmas,
            (optionType == Option::Call) ? calibrationResults_[iu].callNPVs : calibrationResults_[iu].putNPVs);
    }

    Real AndreasenHugeVolatilityInterpl::optionPrice(Time t, Real strike, Option::Type optionType) const
    {
        ...
        if (f != priceCache_.end())
        {
            ...
            if (optionType == Option::Put && (calibrationType_ == Call || calibrationType_ == CallPut))
        ...
        ext::shared_ptr<Array> prices(ext::make_shared<Array>(gridPoints_));
        ...
        priceCache_[t] = std::make_tuple(
            fwd, prices,
            ext::make_shared<CubicNaturalSpline>(gridPoints_.begin() + 1, gridPoints_.end() - 1, prices->begin() + 1));

        return this->optionPrice(t, strike, optionType);
    }

    Array AndreasenHugeVolatilityInterpl::getLocalVolSlice(Time t, Option::Type optionType) const
    {
        ...
        const ext::shared_ptr<AndreasenHugeCostFunction> costFunction = calibrationResults_[iu].costFunction;
        ...
        const Array dCdT =
            costFunction->solveFor(dt, sig, costFunction->apply(costFunction->solveFor(dt, sig, previousNPVs)));
    ...
    Volatility AndreasenHugeVolatilityInterpl::localVol(Time t, Real strike) const
    {
        ...
        ext::shared_ptr<Array> localVol(ext::make_shared<Array>(gridPoints_.size()));
        ...
        localVolCache_[t] = std::make_tuple(fwd, localVol,
                                            ext::make_shared<LinearInterpolation>(
                                                gridPoints_.begin() + 1, gridPoints_.end() - 1, localVol->begin() + 1));
