...
namespace QuantLib
{
    ...
    {
        Real blackValue(
            Option::Type optionType, Real strike, Real forward, Real maturity, Volatility vol, Real discount, Real npv)
        {

            return blackFormula(optionType, strike, forward, std::max(0.0, vol) * std::sqrt(maturity), discount) - npv;
        }
    }
    ...
    : BlackVolTermStructure(hestonModel->process()->riskFreeRate()->referenceDate(),
      ...
      hestonModel_(hestonModel), cpxLogFormula_(cpxLogFormula), integration_(std::move(integration))
    {
    ...
    {
        AnalyticHestonEngine hestonEngine(hestonModel_.currentLink(), cpxLogFormula_, integration_);
        ...
        const Real fwd = process->s0()->value() * process->dividendYield()->discount(t, true) / df;

        const ext::shared_ptr<PlainVanillaPayoff> payoff =
            ext::make_shared<PlainVanillaPayoff>(fwd > strike ? Option::Put : Option::Call, strike);
        ...
        return solver.solve([&](Volatility _v)
                            { return blackValue(payoff->optionType(), strike, fwd, t, _v, df, npv); }, accuracy, guess,
                            0.01);
    }
}
