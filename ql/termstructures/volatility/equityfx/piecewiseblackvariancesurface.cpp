...
namespace QuantLib
{
      ...
      smileSections_(std::move(smileSections))
    {
        ...
        QL_REQUIRE(!dates.empty(), "at least one date is required");
        QL_REQUIRE(dates.size() == smileSections_.size(),
                   "mismatch between " << dates.size() << " dates and " << smileSections_.size() << " smile sections");
        ...
        for (Size i = 1; i < dates.size(); ++i)
        {
            ...
            QL_REQUIRE(times_[i] > times_[i - 1], "dates must be sorted and unique, but date "
                                                      << dates[i] << " (t=" << times_[i] << ") is not after date "
                                                      << dates[i - 1] << " (t=" << times_[i - 1] << ")");
        }

        for (Size i = 0; i < smileSections_.size(); ++i)
        {
            QL_REQUIRE(smileSections_[i], "null smile section at index " << i);
            registerWith(smileSections_[i]);
        }
    }

    PiecewiseBlackVarianceSurface::PiecewiseBlackVarianceSurface(const Date& referenceDate,
                                                                 const Date& date,
                                                                 ext::shared_ptr<SmileSection> smileSection,
                                                                 DayCounter dayCounter)
    : PiecewiseBlackVarianceSurface(referenceDate,
    ...
    Real PiecewiseBlackVarianceSurface::sectionVariance(Size i, Real strike) const
    {
        const auto& s = smileSections_[i];
        QL_REQUIRE(allowsExtrapolation() || (strike >= s->minStrike() && strike <= s->maxStrike()),
                   "strike (" << strike << ") is outside the range of smile section " << i << " [" << s->minStrike()
                              << ", " << s->maxStrike() << "]");
        return s->variance(strike);
    }

    Real PiecewiseBlackVarianceSurface::blackVarianceImpl(Time t, Real strike) const
    {
    ...
    PiecewiseBlackVarianceSurface::makeFromGrid(const Date& referenceDate,
                                                const std::vector<Date>& dates,
                                                ...
                                                const DayCounter& dc)
    {
        ...
        QL_REQUIRE(blackVols.rows() == strikes.size(),
                   "mismatch between " << strikes.size() << " strikes and " << blackVols.rows() << " matrix rows");
        QL_REQUIRE(blackVols.columns() == dates.size(),
                   "mismatch between " << dates.size() << " dates and " << blackVols.columns() << " matrix columns");
        ...
        for (Size j = 0; j < dates.size(); ++j)
        {
            ...
            QL_REQUIRE(t > 0.0, "date " << dates[j] << " must be after reference date " << referenceDate);
            Real sqrtT = std::sqrt(t);
            for (Size i = 0; i < strikes.size(); ++i)
                stdDevs[i] = blackVols[i][j] * sqrtT;

            sections[j] = ext::make_shared<InterpolatedSmileSection<Linear>>(dates[j], strikes, stdDevs, Null<Real>(),
                                                                             dc, Linear(), referenceDate);
        }

        return ext::make_shared<PiecewiseBlackVarianceSurface>(referenceDate, dates, std::move(sections), dc);
    }
