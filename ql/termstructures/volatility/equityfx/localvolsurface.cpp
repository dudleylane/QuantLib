...
namespace QuantLib
{
    ...
    : LocalVolTermStructure(blackTS->businessDayConvention(), blackTS->dayCounter()), blackTS_(blackTS),
      riskFreeTS_(std::move(riskFreeTS)), dividendTS_(std::move(dividendTS)), underlying_(std::move(underlying))
    {
    ...
    LocalVolSurface::LocalVolSurface(const Handle<BlackVolTermStructure>& blackTS,
                                     ...
                                     Real underlying)
    : LocalVolTermStructure(blackTS->businessDayConvention(), blackTS->dayCounter()), blackTS_(blackTS),
      riskFreeTS_(std::move(riskFreeTS)), dividendTS_(std::move(dividendTS)),
    ...
    Volatility LocalVolSurface::localVolImpl(Time t, Real underlyingLevel) const
    {
        ...
        else
        {
            ...
            QL_ENSURE(result >= 0.0, "negative local vol^2 at strike "
                                         << strike << " and time " << t
