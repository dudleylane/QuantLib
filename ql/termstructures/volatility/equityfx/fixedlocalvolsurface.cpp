...
namespace QuantLib
{
    ...
    : LocalVolTermStructure(referenceDate, NullCalendar(), Following, dayCounter), maxDate_(dates.back()),
      localVolMatrix_(std::move(localVolMatrix)), strikes_(dates.size(), ext::make_shared<std::vector<Real>>(strikes)),
      localVolInterpol_(dates.size()), lowerExtrapolation_(lowerExtrapolation), upperExtrapolation_(upperExtrapolation)
    {
    ...
    : LocalVolTermStructure(referenceDate, NullCalendar(), Following, dayCounter),
      ...
      localVolMatrix_(std::move(localVolMatrix)), strikes_(times.size(), ext::make_shared<std::vector<Real>>(strikes)),
      localVolInterpol_(times.size()), lowerExtrapolation_(lowerExtrapolation), upperExtrapolation_(upperExtrapolation)
    {
    ...
    : LocalVolTermStructure(referenceDate, NullCalendar(), Following, dayCounter),
      maxDate_(yearFractionToDate(dayCounter, referenceDate, times.back())), times_(times),
      localVolMatrix_(std::move(localVolMatrix)), strikes_(strikes), localVolInterpol_(times.size()),
      lowerExtrapolation_(lowerExtrapolation), upperExtrapolation_(upperExtrapolation)
    {

        QL_REQUIRE(times_[0] >= 0, "cannot have times[0] < 0");
        QL_REQUIRE(times.size() == strikes.size(), "need strikes for every time step");
        checkSurface();
        setInterpolation<Linear>();
    }
    ...
    {
        QL_REQUIRE(times_.size() == localVolMatrix_->columns(), "mismatch between date vector and vol matrix colums");
        for (const auto& strike : strikes_)
        {
            QL_REQUIRE(strike->size() == localVolMatrix_->rows(), "mismatch between money-strike vector and "
                                                                  "vol matrix rows");
        }
    ...
    Volatility FixedLocalVolSurface::localVolImpl(Time t, Real strike) const
    {
        ...
        const Size idx = std::distance(times_.begin(), std::lower_bound(times_.begin(), times_.end(), t));
        ...
        else
        {
            ...
            const Real earlyVol = (strikes_[idx - 1]->front() < strikes_[idx - 1]->back()) ?
            ...
            return earlyVol + (laterVol - earlyVol) / (times_[idx] - times_[idx - 1]) * (t - times_[idx - 1]);
        }
    }
