...
namespace QuantLib
{
    ...
    : SmileSection(*source), source_(source), moneynessGrid_(moneynessGrid), gap_(gap), interpolate_(interpolate),
      exponentialExtrapolation_(exponentialExtrapolation), forcedLeftIndex_(forcedLeftIndex),
      forcedRightIndex_(forcedRightIndex)
    {
        ...
        ssutils_ = ext::make_shared<SmileSectionUtils>(*source, moneynessGrid, atm, deleteArbitragePoints);
    ...
    void KahaleSmileSection::compute()
    {
        ...
        cFunctions_ = std::vector<ext::shared_ptr<cFunction>>(rightIndex_ - leftIndex_ + 2);
            ...
            try
            {
                ...
                Real s = brent.solve(sh1, QL_KAHALE_ACC, 0.20, 0.00,
                                     QL_KAHALE_SMAX); // numerical parameters
                                                      // hardcoded here
                sh1(s);
                ext::shared_ptr<cFunction> cFct1(new cFunction(sh1.f_, s, 0.0, sh1.b_));
                cFunctions_[0] = cFct1;
        ...
        QL_REQUIRE(leftIndex_ < rightIndex_,
                   "can not extrapolate to left, right index of af region reached (" << rightIndex_ << ")");
        ...
        if (interpolate_)
        {
            ...
            for (Size i = leftIndex_; i < rightIndex_; i++)
            {
                ...
                try
                {
                    a = brent.solve(ah, QL_KAHALE_ACC, 0.5 * (cp1 + (1.0 + cp0)), cp1 + QL_KAHALE_EPS,
                ...
                {
                    ah(a);
                    ext::shared_ptr<cFunction> cFct(new cFunction(ah.f_, ah.s_, a, ah.b_));
        ...
        do
        {
            ...
            try
            {
                ...
                Real cp0;
                if (interpolate_)
                    cp0 = 0.5 * (c_[rightIndex_] - c_[rightIndex_ - 1]) / (k_[rightIndex_] - k_[rightIndex_ - 1]);
                else
                {
                    cp0 = -source_->digitalOptionPrice(k0 - shift() - gap_ / 2.0, Option::Call, 1.0, gap_);
                }
                ...
                if (exponentialExtrapolation_)
                {
                    QL_REQUIRE(-cp0 / c0 > 0.0, "dummy"); // this is caught
                                                          // below
                    cFct = ext::make_shared<cFunction>(-cp0 / c0, std::log(c0) - cp0 / c0 * k0);
                }
                else
                {
                    ...
                    s = brent.solve(sh, QL_KAHALE_ACC, 0.20, 0.0,
                                    QL_KAHALE_SMAX); // numerical parameters
                                                     // hardcoded here
                    sh(s);
                    cFct = ext::make_shared<cFunction>(sh.f_, s, 0.0, 0.0);
                }
        ...
        QL_REQUIRE(leftIndex_ < rightIndex_,
                   "can not extrapolate to right, left index of af region reached (" << leftIndex_ << ")");
    }
    ...
    Real KahaleSmileSection::optionPrice(Rate strike, Option::Type type, Real discount) const
    {
        ...
        if (interpolate_ || (i == 0 || i == (int)(rightIndex_ - leftIndex_ + 1)))
            return discount * (type == Option::Call ? (*cFunctions_[i])(shifted_strike) :
    ...
    Real KahaleSmileSection::volatilityImpl(Rate strike) const
    {
        ...
        try
        {
            ...
            vol = blackFormulaImpliedStdDev(type, shifted_strike, f_, type == Option::Put ? strike - f_ + c : c) /
                  sqrt(exerciseTime());
        }
        catch (...)
        {
    ...
    {
        int i = static_cast<int>(std::upper_bound(k_.begin(), k_.end(), strike) - k_.begin()) -
                static_cast<int>(leftIndex_);
        return std::max(std::min(i, static_cast<int>(rightIndex_ - leftIndex_ + 1)), 0);
    }
}
