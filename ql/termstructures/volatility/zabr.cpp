...
namespace QuantLib
{
    ...
    ZabrModel::ZabrModel(const Real expiryTime,
                         const Real forward,
                         const Real alpha,
                         const Real beta,
                         const Real nu,
                         const Real rho,
                         const Real gamma)
    : expiryTime_(expiryTime), forward_(forward), alpha_(alpha), beta_(beta), nu_(nu * std::pow(alpha_, 1.0 - gamma)),
      rho_(rho), gamma_(gamma)
    {
    ...
    Real ZabrModel::localVolatilityHelper(const Real f, const Real x) const
    {
        return alpha_ * std::pow(std::fabs(f), beta_) /
               F(y(f), std::pow(alpha_, gamma_ - 1.0) * x); // TODO optimize this, y is comoputed together
                                                            // with x already
    }
    ...
    std::vector<Real> ZabrModel::fdPrice(const std::vector<Real>& strikes) const
    {
        ...
        const Size steps = (Size)std::ceil(expiryTime_ * 24);        // number of steps in dimension t
...
#    pragma GCC diagnostic ignored "-Warray-bounds"
        ...
        const ext::shared_ptr<FdmLinearOpLayout> layout(new FdmLinearOpLayout(dim));
...
#    pragma GCC diagnostic pop
        ...
        const ext::shared_ptr<Fdm1dMesher> m1(
            new Concentrating1dMesher(start, end, size, std::pair<Real, Real>(forward_, density), true));
        ...
        const ext::shared_ptr<FdmMesher> mesher(new FdmMesherComposite(layout, meshers));
        ...
        FdmBackwardSolver solver(map, boundaries, ext::shared_ptr<FdmStepConditionComposite>(),
        ...
        ext::shared_ptr<Interpolation> solution(new CubicInterpolation(
            k.begin(), k.end(), rhs.begin(), CubicInterpolation::Spline, true, CubicInterpolation::SecondDerivative,
    ...
    Real ZabrModel::fullFdPrice(const Real strike) const
    {
        ...
        const Size sizefa = std::max<Size>(4, (Size)std::ceil(((x0 + x1) / 2.0 - f0) / (f1 - f0) * (Real)sizef));
        const Size sizefb = sizef - sizefa + 1; // common point, so we can spend
        // one more here
        const ext::shared_ptr<Fdm1dMesher> mfa(
            new Concentrating1dMesher(f0, (x0 + x1) / 2.0, sizefa, std::pair<Real, Real>(x0, densityf), true));
        const ext::shared_ptr<Fdm1dMesher> mfb(
            new Concentrating1dMesher((x0 + x1) / 2.0, f1, sizefb, std::pair<Real, Real>(x1, densityf), true));
        ...
        const ext::shared_ptr<Fdm1dMesher> mv(
            new Concentrating1dMesher(v0, v1, sizev, std::pair<Real, Real>(alpha_, densityv), true));
        ...
        ext::shared_ptr<FdmZabrOp> map(new FdmZabrOp(mesher, beta_, nu_, rho_, gamma_));
        FdmBackwardSolver solver(map, boundaries, ext::shared_ptr<FdmStepConditionComposite>(),
        ...
        ext::shared_ptr<BicubicSpline> interpolation =
            ext::make_shared<BicubicSpline>(f_.begin(), f_.end(), v_.begin(), v_.end(), result);
    ...
    std::vector<Real> ZabrModel::x(const std::vector<Real>& strikes) const
    {
        ...
        QL_REQUIRE(strikes[0] > 0.0 || beta_ < 1.0, "strikes must be positive (" << strikes[0] << ") if beta = 1");
        ...
        std::transform(strikes.rbegin(), strikes.rend(), y.begin(), [&](Real _k) { return this->y(_k); });
        ...
        else
        {
            ...
            for (int dir = 1; dir >= -1; dir -= 2)
            {
                ...
                {
                    Real u = rk([&](Real _y, Real _u) { return F(_y, _u); }, u0, y0, y[m]);
    ...
    Real ZabrModel::y(const Real strike) const
    {
        ...
        {
            return (strike < 0.0 ? Real(std::pow(forward_, 1.0 - beta_) + std::pow(-strike, 1.0 - beta_)) :
                                   Real(std::pow(forward_, 1.0 - beta_) - std::pow(strike, 1.0 - beta_))) *
