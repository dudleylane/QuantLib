...
namespace QuantLib
{
    ...
    MultiCurve::MultiCurve(Real accuracy) : multiCurveBootstrap_(ext::make_shared<MultiCurveBootstrap>(accuracy)) {}
    ...
    Handle<YieldTermStructure> MultiCurve::addBootstrappedCurve(RelinkableHandle<YieldTermStructure>& internalHandle,
                                                                ext::shared_ptr<YieldTermStructure>&& curve)
    {
        QL_REQUIRE(internalHandle.empty(), "internal handle must be empty; was the curve added already?");
    ...
    Handle<YieldTermStructure> MultiCurve::addNonBootstrappedCurve(RelinkableHandle<YieldTermStructure>& internalHandle,
                                                                   ext::shared_ptr<YieldTermStructure>&& curve)
    {
        QL_REQUIRE(internalHandle.empty(), "internal handle must be empty; was the curve added already?");
    ...
    Handle<YieldTermStructure> MultiCurve::addCurve(RelinkableHandle<YieldTermStructure>& internalHandle,
                                                    ext::shared_ptr<YieldTermStructure>&& curve)
    { // NOLINT(cppcoreguidelines-rvalue-reference-param-not-moved)
        internalHandle.linkTo(ext::shared_ptr<YieldTermStructure>(curve.get(), null_deleter()), false);
