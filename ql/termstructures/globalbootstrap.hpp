...
namespace QuantLib
{
    ...
    class MultiCurveBootstrapContributor
    {
      public:
        ...
        virtual void setParentBootstrapper(const ext::shared_ptr<MultiCurveBootstrap>& b) const = 0;
    ...
    */
    ...
    class GlobalBootstrap final : public MultiCurveBootstrapContributor
    {
      ...
      private:
        ...
        void setParentBootstrapper(const ext::shared_ptr<MultiCurveBootstrap>& b) const override;
    ...
    GlobalBootstrap<Curve>::GlobalBootstrap(Real accuracy,
                                            ...
                                            std::vector<Real> instrumentWeights)
    : ts_(nullptr), accuracy_(accuracy), optimizer_(std::move(optimizer)), endCriteria_(std::move(endCriteria)),
      instrumentWeights_(std::move(instrumentWeights))
    {
    }

    template <class Curve>
    GlobalBootstrap<Curve>::GlobalBootstrap(std::vector<ext::shared_ptr<typename Traits::helper>> additionalHelpers,
                                            std::function<std::vector<Date>()> additionalDates,
                                            AdditionalPenalties additionalPenalties,
                                            Real accuracy,
                                            ext::shared_ptr<OptimizationMethod> optimizer,
                                            ...
                                            std::vector<Real> instrumentWeights)
    : ts_(nullptr), accuracy_(accuracy), optimizer_(std::move(optimizer)), endCriteria_(std::move(endCriteria)),
      additionalHelpers_(std::move(additionalHelpers)), additionalDates_(std::move(additionalDates)),
      additionalPenalties_(std::move(additionalPenalties)), additionalVariables_(std::move(additionalVariables)),
      instrumentWeights_(std::move(instrumentWeights))
    {
    }
    ...
    : GlobalBootstrap(
          ...
          additionalPenalties ? [f = std::move(additionalPenalties)](const std::vector<Time>&, const std::vector<Real>&)
              { return f(); } : AdditionalPenalties(),
          accuracy,
          std::move(optimizer),
    ...
    void GlobalBootstrap<Curve>::setup(Curve* ts)
    {
        ...
        QL_REQUIRE(instrumentWeights_.empty() || instrumentWeights_.size() == ts_->instruments_.size(),
                   "GlobalBootstrap: number of instrument weights (" << instrumentWeights_.size()
                                                                     << ") must match number of instruments ("
    ...
    template <class Curve>
    void GlobalBootstrap<Curve>::initialize() const
    {
        ...
        std::copy_if(additionalHelpers_.begin(), additionalHelpers_.end(), std::back_inserter(aliveAdditionalHelpers_),
        ...
        {
            additionalDates.erase(std::remove_if(additionalDates.begin(), additionalDates.end(),
        ...
        std::transform(aliveInstruments_.begin(), aliveInstruments_.end(), std::back_inserter(dates),
        ...
        QL_REQUIRE(dates.size() >= Interpolator::requiredPoints,
                   "GlobalBootstrap: not enough curve points ("
                       << dates.size() << ") for interpolation requiring at least " << Interpolator::requiredPoints);
    ...
    template <class Curve>
    Array GlobalBootstrap<Curve>::setupCostFunction() const
    {
        ...
        for (auto const& helper : aliveInstruments_)
        {
            ...
            QL_REQUIRE(helper->quote()->isValid(), "instrument (maturity: " << helper->maturityDate()
        ...
        for (auto const& helper : aliveAdditionalHelpers_)
        {
            QL_REQUIRE(helper->quote()->isValid(),
                       "additional instrument (maturity: " << helper->maturityDate() << ") has an invalid quote");
            helper->setTermStructure(const_cast<Curve*>(ts_));
        }
        ...
        std::copy(additionalGuesses.begin(), additionalGuesses.end(), guess.begin() + ts_->times_.size() - 1);
        return guess;
    }
    ...
    Array GlobalBootstrap<Curve>::evaluateCostFunction() const
    {
        ...
        std::copy(additionalErrors.begin(), additionalErrors.end(), result.begin() + aliveInstruments_.size());
        return result;
    }
