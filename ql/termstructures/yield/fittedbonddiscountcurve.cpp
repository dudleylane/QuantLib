...
namespace QuantLib
{
    ...
    class FittedBondDiscountCurve::FittingMethod::FittingCost : public CostFunction
    {
        friend class FittedBondDiscountCurve::FittingMethod;

      public:
        explicit FittingCost(FittedBondDiscountCurve::FittingMethod* fittingMethod);
    ...
    FittedBondDiscountCurve::FittedBondDiscountCurve(Natural settlementDays,
    ...
    : YieldTermStructure(settlementDays, calendar, dayCounter), accuracy_(accuracy), maxEvaluations_(maxEvaluations),
      simplexLambda_(simplexLambda), maxStationaryStateIterations_(maxStationaryStateIterations),
      guessSolution_(std::move(guess)), bondHelpers_(std::move(bondHelpers)), fittingMethod_(fittingMethod)
    {
    ...
    FittedBondDiscountCurve::FittedBondDiscountCurve(const Date& referenceDate,
                                                     vector<ext::shared_ptr<BondHelper>> bondHelpers,
                                                     const DayCounter& dayCounter,
                                                     const FittingMethod& fittingMethod,
                                                     Real accuracy,
                                                     ...
                                                     Real simplexLambda,
                                                     Size maxStationaryStateIterations)
    : YieldTermStructure(referenceDate, Calendar(), dayCounter), accuracy_(accuracy), maxEvaluations_(maxEvaluations),
      simplexLambda_(simplexLambda), maxStationaryStateIterations_(maxStationaryStateIterations),
      guessSolution_(std::move(guess)), bondHelpers_(std::move(bondHelpers)), fittingMethod_(fittingMethod)
    {
    ...
    FittedBondDiscountCurve::FittedBondDiscountCurve(Natural settlementDays,
                                                     ...
                                                     Date maxDate,
                                                     const DayCounter& dayCounter)
    : YieldTermStructure(settlementDays, calendar, dayCounter), accuracy_(1e-10), maxEvaluations_(0),
      guessSolution_(std::move(parameters)), maxDate_(maxDate), fittingMethod_(fittingMethod)
    {
    ...
    FittedBondDiscountCurve::FittedBondDiscountCurve(const Date& referenceDate,
                                                     ...
                                                     Date maxDate,
                                                     const DayCounter& dayCounter)
    : YieldTermStructure(referenceDate, Calendar(), dayCounter), accuracy_(1e-10), maxEvaluations_(0),
      guessSolution_(std::move(parameters)), maxDate_(maxDate), fittingMethod_(fittingMethod)
    {
    ...
    void FittedBondDiscountCurve::performCalculations() const
    {
        ...
        if (maxEvaluations_ == 0)
        {
            ...
            QL_REQUIRE(maxDate_ != Date() || !bondHelpers_.empty(), "no bond helpers or max date given");
        }
        ...
        if (!bondHelpers_.empty())
        {
            ...
            for (Size i = 0; i < bondHelpers_.size(); ++i)
            {
                ...
                QL_REQUIRE(bondHelpers_[i]->quote()->isValid(), io::ordinal(i + 1)
                                                                    << " bond (maturity: " << bond->maturityDate()
                                                                    << ") has an invalid price quote");
                Date bondSettlement = bond->settlementDate();
                QL_REQUIRE(bondSettlement >= refDate, io::ordinal(i + 1)
                                                          << " bond settlemente date (" << bondSettlement
                                                          << ") before curve reference date (" << refDate << ")");
                QL_REQUIRE(BondFunctions::isTradable(*bond, bondSettlement),
                           io::ordinal(i + 1) << " bond non tradable at " << bondSettlement
                                              << " settlement date (maturity"
                                                 " being "
                                              << bond->maturityDate() << ")");
                maxDate_ = std::max(maxDate_, bondHelpers_[i]->pillarDate());
                bondHelpers_[i]->setTermStructure(const_cast<FittedBondDiscountCurve*>(this));
            }
        }
    ...
    : constrainAtZero_(constrainAtZero), weights_(weights), l2_(std::move(l2)), calculateWeights_(weights.empty()),
      optimizationMethod_(std::move(optimizationMethod)), constraint_(std::move(constraint)),
    ...
    void FittedBondDiscountCurve::FittingMethod::init()
    {
        ...
        if (calculateWeights_)
        {
            ...
            for (Size i = 0; i < curve_->bondHelpers_.size(); ++i)
            {
                ...
                Date bondSettlement = bond->settlementDate();
                Rate ytm = BondFunctions::yield(*bond, price, yieldDC, yieldComp, yieldFreq, bondSettlement);

                Time dur = BondFunctions::duration(*bond, ytm, yieldDC, yieldComp, yieldFreq, Duration::Modified,
                                                   bondSettlement);
                weights_[i] = 1.0 / dur;
        ...
        if (!l2_.empty())
        {
            QL_REQUIRE(l2_.size() == size(), "Given penalty factors do not cover all parameters");
    ...
    void FittedBondDiscountCurve::FittingMethod::calculate()
    {
        ...
        if (curve_->maxEvaluations_ == 0)
        {
            ...
            QL_REQUIRE(curve_->guessSolution_.size() == size(), "wrong number of parameters");
        ...
        EndCriteria endCriteria(curve_->maxEvaluations_, curve_->maxStationaryStateIterations_, rootEpsilon,
    ...
    Real FittedBondDiscountCurve::FittingMethod::FittingCost::value(const Array& x) const
    {
