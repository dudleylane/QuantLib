...
namespace QuantLib
{
    ...
    namespace
    {
        ...
        Time DetermineYearFraction(const Date& earliestDate, const Date& maturityDate, const DayCounter& dayCounter)
        {
            return dayCounter.yearFraction(earliestDate, maturityDate, earliestDate, maturityDate);
        }
    ...
    : RateHelper(price), convAdj_(handleFromVariant(convAdj))
    {
        ...
        maturityDate_ = calendar.advance(iborStartDate, lengthInMonths * Months, convention, endOfMonth);
    ...
    : RateHelper(price), convAdj_(handleFromVariant(convAdj))
    {
        ...
        const auto determineMaturityDate = [&iborStartDate, &iborEndDate](const auto nextDateCalculator) -> Date
        {
            ...
            {
                QL_REQUIRE(iborEndDate > iborStartDate, "end date (" << iborEndDate
        ...
        switch (type)
        {
            case Futures::IMM:
                maturityDate_ =
                    determineMaturityDate([](const Date date) -> Date { return IMM::nextDate(date, false); });
                break;
            case Futures::ASX:
                maturityDate_ =
                    determineMaturityDate([](const Date date) -> Date { return ASX::nextDate(date, false); });
                break;
            case Futures::Custom:
    ...
    : RateHelper(price), convAdj_(handleFromVariant(convAdj))
    {
        ...
        maturityDate_ = cal.advance(iborStartDate, index->tenor(), index->businessDayConvention());
    ...
    Real FuturesRateHelper::impliedQuote() const
    {
        ...
        Rate forwardRate =
            (termStructure_->discount(earliestDate_) / termStructure_->discount(maturityDate_) - 1.0) / yearFraction_;
    ...
    : RelativeDateRateHelper(rate)
    {
        iborIndex_ = ext::make_shared<IborIndex>("no-fix", // never take fixing into account
                                                 tenor, fixingDays, Currency(), calendar, convention, endOfMonth,
                                                 dayCounter, termStructureHandle_);
    ...
    FraRateHelper::FraRateHelper(const std::variant<Rate, Handle<Quote>>& rate,
                                 ...
                                 Date customPillarDate,
                                 bool useIndexedCoupon)
    : RelativeDateRateHelper(rate), periodToStart_(periodToStart), pillarChoice_(pillarChoice),
    ...
    FraRateHelper::FraRateHelper(const std::variant<Rate, Handle<Quote>>& rate,
                                 ...
                                 Date customPillarDate,
                                 bool useIndexedCoupon)
    : RelativeDateRateHelper(rate, false), pillarChoice_(pillarChoice), useIndexedCoupon_(useIndexedCoupon)
    {
        // take fixing into account
        iborIndex_ = i->clone(termStructureHandle_);
        // see above
        ...
        if (updateDates_)
        {
            ...
            { // NOLINT(readability-implicit-bool-conversion)
                earliestDate_ = iborIndex_->fixingCalendar().advance(
                    spotDate, *periodToStart_, iborIndex_->businessDayConvention(), iborIndex_->endOfMonth());
                // maturity date is calculated from spot date
                maturityDate_ =
                    iborIndex_->fixingCalendar().advance(spotDate, *periodToStart_ + iborIndex_->tenor(),
    ...
    : SwapRateHelper(rate,
                     swapIndex->tenor(),
                     swapIndex->fixingCalendar(),
                     swapIndex->fixedLegTenor().frequency(),
                     swapIndex->fixedLegConvention(),
                     swapIndex->dayCounter(),
                     swapIndex->iborIndex(),
                     std::move(spread),
                     fwdStart,
                     std::move(discount),
    ...
    : RelativeDateRateHelper(rate), settlementDays_(settlementDays), tenor_(tenor), pillarChoice_(pillarChoice),
      calendar_(std::move(calendar)), fixedConvention_(fixedConvention), fixedFrequency_(fixedFrequency),
    ...
    SwapRateHelper::SwapRateHelper(const std::variant<Rate, Handle<Quote>>& rate,
                                   ...
                                   const ext::optional<bool>& useIndexedCoupons,
                                   const ext::optional<BusinessDayConvention>& floatConvention)
    : RelativeDateRateHelper(rate, false), startDate_(startDate), endDate_(endDate), pillarChoice_(pillarChoice),
      calendar_(std::move(calendar)), fixedConvention_(fixedConvention), fixedFrequency_(fixedFrequency),
      fixedDayCount_(std::move(fixedDayCount)), spread_(std::move(spread)), endOfMonth_(endOfMonth),
      discountHandle_(std::move(discount)), useIndexedCoupons_(useIndexedCoupons), floatConvention_(floatConvention)
    {
        QL_REQUIRE(fixedFrequency != Once, "fixedFrequency == Once is not supported when passing explicit "
                                           "startDate and endDate");
        initialize(iborIndex, customPillarDate);
    }

    void SwapRateHelper::initialize(const ext::shared_ptr<IborIndex>& iborIndex, Date customPillarDate)
    {
        // take fixing into account
    ...
    void SwapRateHelper::initializeDates()
    {
        ...
        ext::shared_ptr<IborCoupon> lastCoupon = ext::dynamic_pointer_cast<IborCoupon>(swap_->floatingLeg().back());
    ...
    : RelativeDateRateHelper(liborFraction), tenor_(tenor), settlementDays_(settlementDays),
      ...
      bmaDayCount_(std::move(bmaDayCount)), bmaIndex_(std::move(bmaIndex)), iborIndex_(std::move(iborIndex))
    {
        registerWith(iborIndex_);
    ...
    void BMASwapRateHelper::initializeDates()
    {
        // if the evaluation date is not a business day
        // then move to the next business day
        JointCalendar jc(calendar_, iborIndex_->fixingCalendar());
        Date referenceDate = jc.adjust(evaluationDate_);
        earliestDate_ = calendar_.advance(referenceDate, settlementDays_ * Days, Following);
        ...
        swap_ = ext::make_shared<BMASwap>(Swap::Payer, 100.0, liborSchedule,
                                          0.75, // arbitrary
                                          0.0, iborIndex_, iborIndex_->dayCounter(), bmaSchedule, clonedIndex,
                                          bmaDayCount_);
        swap_->setPricingEngine(
            ext::shared_ptr<PricingEngine>(new DiscountingSwapEngine(iborIndex_->forwardingTermStructure())));
    ...
    : RelativeDateRateHelper(fwdPoint), spot_(std::move(spotFx)), tenor_(tenor), fixingDays_(fixingDays),
      cal_(std::move(calendar)), conv_(convention), eom_(endOfMonth),
      isFxBaseCurrencyCollateralCurrency_(isFxBaseCurrencyCollateralCurrency), collHandle_(std::move(coll)),
      tradingCalendar_(std::move(tradingCalendar))
    {
        registerWith(spot_);
    ...
    void FxSwapRateHelper::initializeDates()
    {
        ...
        if (!tradingCalendar_.empty())
        {
            ...
            latestDate_ = jointCalendar_.advance(earliestDate_, tenor_, conv_, eom_);
        }
        else
        {
