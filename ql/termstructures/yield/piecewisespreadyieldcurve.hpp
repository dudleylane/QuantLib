...
namespace QuantLib
{
    ...
    class PiecewiseSpreadYieldCurve : public PiecewiseYieldCurve<detail::SpreadTraits<Traits>, Interpolator, Bootstrap>
    {
      public:
      ...
      public:
        //! \name Constructors
        //@{
        PiecewiseSpreadYieldCurve(Handle<YieldTermStructure> baseCurve,
                                  std::vector<ext::shared_ptr<typename traits_type::helper>> instruments,
                                  const Interpolator& i = {},
                                  typename base_curve::bootstrap_type bootstrap = {})
        : base_curve(std::move(instruments), std::move(bootstrap), std::move(baseCurve), i)
        {
        }
        //@}
    };
