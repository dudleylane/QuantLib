...
namespace QuantLib
{
    ...
    : FittedBondDiscountCurve::FittingMethod(
          constrainAtZero, weights, optimizationMethod, l2, minCutoffTime, maxCutoffTime, std::move(constraint)),
    ...
    : ExponentialSplinesFitting(
          constrainAtZero, weights, {}, l2, minCutoffTime, maxCutoffTime, numCoeffs, fixedKappa, std::move(constraint))
    {
    }

    ExponentialSplinesFitting::ExponentialSplinesFitting(
        bool constrainAtZero, const Size numCoeffs, const Real fixedKappa, const Array& weights, Constraint constraint)
    : ExponentialSplinesFitting(
          constrainAtZero, weights, {}, Array(), 0.0, QL_MAX_REAL, numCoeffs, fixedKappa, std::move(constraint))
    {
    }

    std::unique_ptr<FittedBondDiscountCurve::FittingMethod> ExponentialSplinesFitting::clone() const
    {
    ...
    DiscountFactor ExponentialSplinesFitting::discountFunction(const Array& x, Time t) const
    {
    ...
    NelsonSiegelFitting::NelsonSiegelFitting(const Array& weights,
                                             ...
                                             Constraint constraint)
    : FittedBondDiscountCurve::FittingMethod(
          true, weights, optimizationMethod, l2, minCutoffTime, maxCutoffTime, std::move(constraint))
    {
    }
    ...
    std::unique_ptr<FittedBondDiscountCurve::FittingMethod> NelsonSiegelFitting::clone() const
    {
    ...
    DiscountFactor NelsonSiegelFitting::discountFunction(const Array& x, Time t) const
    {
        Real kappa = x[size() - 1];
        Real zeroRate = x[0] +
    ...
    std::unique_ptr<FittedBondDiscountCurve::FittingMethod> SvenssonFitting::clone() const
    {
    ...
    DiscountFactor SvenssonFitting::discountFunction(const Array& x, Time t) const
    {
        Real kappa = x[size() - 2];
      ...
      splines_(3, knots.size() - 5, knots)
    {
        ...
        QL_REQUIRE(knots.size() >= 8, "At least 8 knots are required");
        ...
        if (constrainAtZero)
        {
            ...
            QL_REQUIRE(std::abs(splines_(N_, 0.0)) > QL_EPSILON, "N_th cubic B-spline must be nonzero at t=0");
        }
        else
        {
    ...
    CubicBSplinesFitting::CubicBSplinesFitting(const std::vector<Time>& knots,
                                               ...
                                               const Real maxCutoffTime,
                                               Constraint constraint)
    : CubicBSplinesFitting(knots, constrainAtZero, weights, {}, l2, minCutoffTime, maxCutoffTime, std::move(constraint))
    {
    }
    ...
    std::unique_ptr<FittedBondDiscountCurve::FittingMethod> CubicBSplinesFitting::clone() const
    {
    ...
    DiscountFactor CubicBSplinesFitting::discountFunction(const Array& x, Time t) const
    {
        DiscountFactor d = 0.0;
    ...
    NaturalCubicFitting::NaturalCubicFitting(const std::vector<Time>& knotTimes,
                                             const Array& weights,
                                             const ext::shared_ptr<OptimizationMethod>& optimizationMethod,
                                             const Array& l2,
                                             Real minCutoffTime,
      ...
      knotTimes_(knotTimes)
    {
        ...
        auto last =
            std::unique(knotTimes_.begin(), knotTimes_.end(), [](Time a, Time b) { return std::fabs(a - b) <= 1e-14; });
        ...
        for (Size i = 0; i + 1 < n; ++i)
        {
            ...
            QL_REQUIRE(h > 1e-14, "NaturalCubicFitting: knot times must be strictly increasing (non-zero spacing)");
            QL_REQUIRE(std::isfinite(h), "NaturalCubicFitting: non-finite knot spacing");
        }
    }
    ...
    NaturalCubicFitting::NaturalCubicFitting(const std::vector<Time>& knotTimes,
                                             ...
                                             Real maxCutoffTime,
                                             Constraint constraint)
    : NaturalCubicFitting(knotTimes, weights, {}, l2, minCutoffTime, maxCutoffTime, std::move(constraint))
    {
    }

    std::unique_ptr<FittedBondDiscountCurve::FittingMethod> NaturalCubicFitting::clone() const
    {
    ...
    DiscountFactor NaturalCubicFitting::discountFunction(const Array& x, Time t) const
    {
        ...
        QL_REQUIRE(x.size() == expected, "NaturalCubicFitting::discountFunction(): parameter size mismatch: expected "
                                             << expected << " got " << x.size());
    ...
    : SimplePolynomialFitting(
          degree, constrainAtZero, weights, {}, l2, minCutoffTime, maxCutoffTime, std::move(constraint))
    {
    }

    std::unique_ptr<FittedBondDiscountCurve::FittingMethod> SimplePolynomialFitting::clone() const
    {
    ...
    DiscountFactor SimplePolynomialFitting::discountFunction(const Array& x, Time t) const
    {
        DiscountFactor d = 0.0;
    ...
    SpreadFittingMethod::SpreadFittingMethod(const ext::shared_ptr<FittingMethod>& method,
                                             ...
                                             const Real minCutoffTime,
                                             const Real maxCutoffTime)
    : FittedBondDiscountCurve::FittingMethod(method != nullptr ? method->constrainAtZero() : true,
                                             method != nullptr ? method->weights() : Array(),
                                             method != nullptr ? method->optimizationMethod() :
    ...
    std::unique_ptr<FittedBondDiscountCurve::FittingMethod> SpreadFittingMethod::clone() const
    {
