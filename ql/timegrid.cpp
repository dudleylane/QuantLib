...
namespace QuantLib
{
    ...
    TimeGrid::TimeGrid(Time end, Size steps)
    {
        ...
        QL_REQUIRE(end > 0.0, "negative times not allowed");
        Time dt = end / steps;
        ...
        else
        {
            if (t < times_.front())
            {
                QL_FAIL("using inadequate time grid: all nodes "
                        ...
                        << std::setprecision(12) << t << " (earliest node is t1 = " << std::setprecision(12)
                        << times_.front() << ")");
            }
            else if (t > times_.back())
            {
                QL_FAIL("using inadequate time grid: all nodes "
                        ...
                        << std::setprecision(12) << t << " (latest node is t1 = " << std::setprecision(12)
                        << times_.back() << ")");
            }
            else
            {
                ...
                QL_FAIL("using inadequate time grid: the nodes closest "
                        ...
                        << std::setprecision(12) << t << " are t1 = " << std::setprecision(12) << times_[j]
                        << " and t2 = " << std::setprecision(12) << times_[k]);
            }
        }
    }
