...
namespace QuantLib
{
    ...
    Period PeriodParser::parseOnePeriod(const std::string& str)
    {
        ...
        QL_REQUIRE(iPos == str.length() - 1, "unknown '" << str.substr(str.length() - 1, str.length()) << "' unit");
        TimeUnit units = Days;
        ...
        {
            QL_FAIL("unable to parse the number of units of " << units << " in '" << str << "'. Error:" << e.what());
        }

        return {n, units};
    }

    Date DateParser::parseFormatted(const std::string& str, const std::string& fmt)
    {
#ifndef QL_PATCH_SOLARIS
    ...
    {
        QL_REQUIRE(str.size() == 10 && str[4] == '-' && str[7] == '-', "invalid format");
