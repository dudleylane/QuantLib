...
namespace QuantLib
{
    ...
    */
    ...
    {
      public:
        RelinkableHandle() : RelinkableHandle(ext::shared_ptr<T>()) {}
        explicit RelinkableHandle(const ext::shared_ptr<T>& p, bool registerAsObserver = true);
        explicit RelinkableHandle(ext::shared_ptr<T>&& p, bool registerAsObserver = true);
    ...
    template <class T>
    inline void Handle<T>::Link::linkTo(ext::shared_ptr<T> h, bool registerAsObserver)
    {
    ...
    template <class T>
    inline RelinkableHandle<T>::RelinkableHandle(const ext::shared_ptr<T>& p, bool registerAsObserver)
    : Handle<T>(p, registerAsObserver)
    {
    }

    template <class T>
    inline RelinkableHandle<T>::RelinkableHandle(ext::shared_ptr<T>&& p, bool registerAsObserver)
    : Handle<T>(std::move(p), registerAsObserver)
    {
    }

    template <class T>
    inline void RelinkableHandle<T>::linkTo(const ext::shared_ptr<T>& h, bool registerAsObserver)
    {
        this->link_->linkTo(h, registerAsObserver);
    }

    template <class T>
    inline void RelinkableHandle<T>::linkTo(ext::shared_ptr<T>&& h, bool registerAsObserver)
    {
