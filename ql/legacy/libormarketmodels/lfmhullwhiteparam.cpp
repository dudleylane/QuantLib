...
namespace QuantLib
{
    ...
    LfmHullWhiteParameterization::LfmHullWhiteParameterization(
        ...
        Size factors)
    : LfmCovarianceParameterization(process->size(), factors), diffusion_(size_ - 1, factors_),
      fixingTimes_(process->fixingTimes())
    {
        ...
        {
            QL_REQUIRE(factors_ == 1, "correlation matrix must be given for "
                                      "multi factor models");
        }
        else
        {
            QL_REQUIRE(correlation.rows() == size_ - 1 && correlation.rows() == correlation.columns(),
                       "wrong dimesion of the correlation matrix");

            QL_REQUIRE(factors_ <= size_ - 1, "too many factors for given LFM process");

            Matrix tmpSqrtCorr = pseudoSqrt(correlation, SalvagingAlgorithm::Spectral);
            ...
            {
                Real p =
                    std::sqrt(std::inner_product(tmpSqrtCorr[i], tmpSqrtCorr[i] + factors_, tmpSqrtCorr[i], Real(0.0)));
                std::transform(tmpSqrtCorr[i], tmpSqrtCorr[i] + factors_, sqrtCorr[i],
        ...
        for (Size i = 1; i < size_; ++i)
        {
            ...
            for (Size j = 1; j < i; ++j)
            {
                cumVar += lambda[i - j - 1] * lambda[i - j - 1] * (fixingTimes[j + 1] - fixingTimes[j]);
            }

            const Volatility vol = capletVol->volatility(fixingDates[i], 0.0);
            const Volatility var = vol * vol * capletVol->dayCounter().yearFraction(fixingDates[0], fixingDates[i]);

            lambda.push_back(std::sqrt((var - cumVar) / (fixingTimes[1] - fixingTimes[0])));
    ...
    Matrix LfmHullWhiteParameterization::integratedCovariance(Time t, const Array&) const
    {
        ...
        Size last = std::lower_bound(fixingTimes_.begin(), fixingTimes_.end(), t) - fixingTimes_.begin();

        for (Size i = 0; i < last; ++i)
        {
            const Time dt = ((i + 1 < last) ? fixingTimes_[i + 1] : t) - fixingTimes_[i];
