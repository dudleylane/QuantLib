...
namespace QuantLib
{
    ...
    : StochasticProcess(ext::shared_ptr<discretization>(new EulerDiscretization)), size_(size),
      ...
      accrualStartTimes_(size), accrualEndTimes_(size), accrualPeriod_(size_), m1(size_), m2(size_)
    {
        ...
        const Date startDate = ext::dynamic_pointer_cast<IborCoupon>(flows[0])->fixingDate();

        for (Size i = 0; i < size_; ++i)
        {
            const ext::shared_ptr<IborCoupon> coupon = ext::dynamic_pointer_cast<IborCoupon>(flows[i]);

            QL_REQUIRE(coupon->date() == coupon->accrualEndDate(), "irregular coupon types are not suppported");
    ...
    Array LiborForwardModelProcess::drift(Time t, const Array& x) const
    {
        Array f(size_, 0.0);
        ...
        for (Size k = m; k < size_; ++k)
        {
            ...
            f[k] = std::inner_product(m1.begin() + m, m1.begin() + k + 1, covariance.column_begin(k) + m, Real(0.0)) -
                   0.5 * covariance[k][k];
        }
    ...
    Array LiborForwardModelProcess::evolve(Time t0, const Array& x0, Time dt, const Array& dw) const
    {
        ...
        */
        ...
        for (Size k = m; k < size_; ++k)
        {
            ...
            m1[k] = y / (1 + y);
            const Real d =
                (std::inner_product(m1.begin() + m, m1.begin() + k + 1, covariance.column_begin(k) + m, Real(0.0)) -
                 0.5 * covariance[k][k]) *
                dt;

            const Real r = std::inner_product(diff.row_begin(k), diff.row_end(k), dw.begin(), Real(0.0)) * sdt;

            const Real x = y * std::exp(d + r);
            m2[k] = x / (1 + x);
            f[k] = x0[k] * std::exp(0.5 * (d + (std::inner_product(m2.begin() + m, m2.begin() + k + 1,
    ...
    void LiborForwardModelProcess::setCovarParam(const ext::shared_ptr<LfmCovarianceParameterization>& param)
    {
        lfmParam_ = param;
    }

    ext::shared_ptr<LfmCovarianceParameterization> LiborForwardModelProcess::covarParam() const
    {
        return lfmParam_;
    }

    ext::shared_ptr<IborIndex> LiborForwardModelProcess::index() const
    {
        return index_;
    }

    Leg LiborForwardModelProcess::cashFlows(Real amount) const
    {
        Date refDate = index_->forwardingTermStructure()->referenceDate();
        Schedule schedule(refDate, refDate + Period(index_->tenor().length() * size_, index_->tenor().units()),
                          index_->tenor(), index_->fixingCalendar(), index_->businessDayConvention(),
    ...
    const std::vector<Time>& LiborForwardModelProcess::accrualStartTimes() const
    {
        return accrualStartTimes_;
    }

    const std::vector<Time>& LiborForwardModelProcess::accrualEndTimes() const
    {
    ...
    {
        return std::upper_bound(fixingTimes_.begin(), fixingTimes_.end(), t) - fixingTimes_.begin();
    }

    std::vector<DiscountFactor> LiborForwardModelProcess::discountBond(const std::vector<Rate>& rates) const
    {
        ...
        {
            discountFactors[i] = discountFactors[i - 1] / (1.0 + rates[i] * accrualPeriod_[i]);
        }
