...
namespace QuantLib
{
    ...
    : LfmCovarianceParameterization(corrModel->size(), corrModel->factors()), volaModel_(std::move(volaModel)),
      corrModel_(corrModel)
    {

        QL_REQUIRE(volaModel_->size() == corrModel_->size(), "different size for the volatility ("
                                                                 << volaModel_->size() << ") and correlation ("
                                                                 << corrModel_->size() << ") models");
    }

    ext::shared_ptr<LmVolatilityModel> LfmCovarianceProxy::volatilityModel() const
    {
        return volaModel_;
    }

    ext::shared_ptr<LmCorrelationModel> LfmCovarianceProxy::correlationModel() const
    {
        return corrModel_;
    }
    ...
    Matrix LfmCovarianceProxy::diffusion(Time t, const Array& x) const
    {
        ...
        {
            std::transform(pca.row_begin(i), pca.row_end(i), pca.row_begin(i),
    ...
    LfmCovarianceProxy::Var_Helper::Var_Helper(const LfmCovarianceProxy* proxy, Size i, Size j)
    : i_(i), j_(j), volaModel_(proxy->volaModel_.get()), corrModel_(proxy->corrModel_.get())
    {
    }
    ...
    Real LfmCovarianceProxy::integratedCovariance(Size i, Size j, Time t, const Array& x) const
    {
