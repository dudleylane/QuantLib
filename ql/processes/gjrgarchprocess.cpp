...
namespace QuantLib
{
    ...
    : StochasticProcess(ext::shared_ptr<discretization>(new EulerDiscretization)),
      ...
      omega_(omega), alpha_(alpha), beta_(beta), gamma_(gamma), lambda_(lambda), daysPerYear_(daysPerYear),
      discretization_(d)
    {
    ...
    Array GJRGARCHProcess::drift(Time t, const Array& x) const
    {
        ...
        return {riskFreeRate_->forwardRate(t, t, Continuous).rate() -
                ...
                daysPerYear_ * daysPerYear_ * omega_ + daysPerYear_ * (beta_ + alpha_ * q2 + gamma_ * q3 - 1.0) *
    ...
    Matrix GJRGARCHProcess::diffusion(Time, const Array& x) const
    {
        ...
        */
        ...
        const Real q3 = lambda_ * n + N + lambda_ * lambda_ * N;
        const Real Eml_e4 = lambda_ * lambda_ * lambda_ * n + 5.0 * lambda_ * n + 3.0 * N +
        ...
        const Real rho1 = std::sqrt(daysPerYear_) * (alpha_ * sigma12 + gamma_ * sigma13) * vol * vol;
        const Real rho2 =
            vol * vol * std::sqrt(daysPerYear_) *
            std::sqrt(alpha_ * alpha_ * (sigma2 - sigma12 * sigma12) + gamma_ * gamma_ * (sigma3 - sigma13 * sigma13) +
    ...
    Array GJRGARCHProcess::apply(const Array& x0, const Array& dx) const
    {
        return {x0[0] * std::exp(dx[0]), x0[1] + dx[1]};
    }

    Array GJRGARCHProcess::evolve(Time t0, const Array& x0, Time dt, const Array& dw) const
    {
        Array retVal(2);
        Real vol, mu, nu;
        ...
        const Real rho2 = std::sqrt(daysPerYear_) * std::sqrt(alpha_ * alpha_ * (sigma2 - sigma12 * sigma12) +
        ...
        switch (discretization_)
        {
            ...
            case Reflection:
                ...
                retVal[0] = x0[0] * std::exp(mu * dt + vol * dw[0] * sdt);
                retVal[1] = vol * vol + nu * dt + sdt * vol * vol * (rho1 * dw[0] + rho2 * dw[1]);
                break;
            default:
