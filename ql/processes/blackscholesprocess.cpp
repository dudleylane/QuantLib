...
namespace QuantLib
{
    ...
    GeneralizedBlackScholesProcess::GeneralizedBlackScholesProcess(Handle<Quote> x0,
    ...
    : StochasticProcess1D(ext::make_shared<EulerDiscretization>()), x0_(std::move(x0)),
      ...
      blackVolatility_(std::move(blackVolTS)), externalLocalVolTS_(std::move(localVolTS)), forceDiscretization_(false),
      hasExternalLocalVol_(true), updated_(false), isStrikeIndependent_(false)
    {
        registerWith(x0_);
    ...
    Real GeneralizedBlackScholesProcess::expectation(Time t0, Real x0, Time dt) const
    {
        ...
        if (isStrikeIndependent_ && !forceDiscretization_)
        {
            ...
            return x0 * std::exp(dt * (riskFreeRate_->forwardRate(t0, t0 + dt, Continuous, NoFrequency, true).rate() -
                                       dividendYield_->forwardRate(t0, t0 + dt, Continuous, NoFrequency, true).rate()));
        }
        else
        {
    ...
    Real GeneralizedBlackScholesProcess::variance(Time t0, Real x0, Time dt) const
    {
        ...
        {
            // exact value for curves
            return blackVolatility_->blackVariance(t0 + dt, 0.01) - blackVolatility_->blackVariance(t0, 0.01);
        }
        else
        {
    ...
    Real GeneralizedBlackScholesProcess::evolve(Time t0, Real x0, Time dt, Real dw) const
    {
        localVolatility(); // trigger update
        if (isStrikeIndependent_ && !forceDiscretization_)
        {
            ...
            Real drift = (riskFreeRate_->forwardRate(t0, t0 + dt, Continuous, NoFrequency, true).rate() -
                          dividendYield_->forwardRate(t0, t0 + dt, Continuous, NoFrequency, true).rate()) *
                             dt -
                         0.5 * var;
    ...
    const Handle<Quote>& GeneralizedBlackScholesProcess::stateVariable() const
    {
        return x0_;
    }

    const Handle<YieldTermStructure>& GeneralizedBlackScholesProcess::dividendYield() const
    {
        return dividendYield_;
    }

    const Handle<YieldTermStructure>& GeneralizedBlackScholesProcess::riskFreeRate() const
    {
        return riskFreeRate_;
    }

    const Handle<BlackVolTermStructure>& GeneralizedBlackScholesProcess::blackVolatility() const
    {
        return blackVolatility_;
    }

    const Handle<LocalVolTermStructure>& GeneralizedBlackScholesProcess::localVolatility() const
    {
        ...
        if (!updated_)
        {
            ...
            ext::shared_ptr<BlackConstantVol> constVol =
                ext::dynamic_pointer_cast<BlackConstantVol>(*blackVolatility());
            if (constVol != nullptr)
            {
                // ok, the local vol is constant too.
                localVolatility_.linkTo(ext::make_shared<LocalConstantVol>(
                    constVol->referenceDate(), constVol->blackVol(0.0, x0_->value()), constVol->dayCounter()));
                updated_ = true;
            ...
            ext::shared_ptr<BlackVarianceCurve> volCurve =
                ext::dynamic_pointer_cast<BlackVarianceCurve>(*blackVolatility());
            if (volCurve != nullptr)
            {
                // ok, we can use the optimized algorithm
                localVolatility_.linkTo(ext::make_shared<LocalVolCurve>(Handle<BlackVarianceCurve>(volCurve)));
                updated_ = true;
                return localVolatility_;
            }
            ...
            localVolatility_.linkTo(
                ext::make_shared<LocalVolSurface>(blackVolatility_, riskFreeRate_, dividendYield_, x0_->value()));
            updated_ = true;
            isStrikeIndependent_ = false;
            return localVolatility_;
        }
    ...
    BlackScholesProcess::BlackScholesProcess(const Handle<Quote>& x0,
    ...
    BlackScholesMertonProcess::BlackScholesMertonProcess(const Handle<Quote>& x0,
                                                         const Handle<YieldTermStructure>& dividendTS,
                                                         const Handle<YieldTermStructure>& riskFreeTS,
                                                         const Handle<BlackVolTermStructure>& blackVolTS,
                                                         const ext::shared_ptr<discretization>& d,
                                                         bool forceDiscretization)
    : GeneralizedBlackScholesProcess(x0, dividendTS, riskFreeTS, blackVolTS, d, forceDiscretization)
    {
    }
    ...
    BlackProcess::BlackProcess(const Handle<Quote>& x0,
                               ...
                               const ext::shared_ptr<discretization>& d,
                               bool forceDiscretization)
    : GeneralizedBlackScholesProcess(x0, riskFreeTS, riskFreeTS, blackVolTS, d, forceDiscretization)
    {
    }


    GarmanKohlagenProcess::GarmanKohlagenProcess(const Handle<Quote>& x0,
                                                 const Handle<YieldTermStructure>& foreignRiskFreeTS,
                                                 const Handle<YieldTermStructure>& domesticRiskFreeTS,
                                                 const Handle<BlackVolTermStructure>& blackVolTS,
                                                 const ext::shared_ptr<discretization>& d,
                                                 bool forceDiscretization)
    : GeneralizedBlackScholesProcess(x0, foreignRiskFreeTS, domesticRiskFreeTS, blackVolTS, d, forceDiscretization)
    {
    }
