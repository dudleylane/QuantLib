...
namespace QuantLib
{
    ...
    void MfStateProcess::checkTimesVols() const
    {
        ...
        for (Size i = 0; i < vols_.size(); i++)
            QL_REQUIRE(vols_[i] >= 0.0, "volatilities must be non negative (" << vols_[i] << "@" << i << ")");
    }
    ...
    Real MfStateProcess::drift(Time, Real) const
    {
        return 0.0;
    }
    ...
    Real MfStateProcess::variance(Time t, Real, Time dt) const
    {
        ...
        if (reversionZero_)
            v += vols_[j] * vols_[j] * (t + dt - std::max(j > 0 ? times_[j - 1] : 0.0, t));
        else
