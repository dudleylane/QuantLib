...
namespace QuantLib
{
    ...
    {
        return {xProcess_->drift(t, x[0]), yProcess_->drift(t, x[1])};
    }
    ...
    Matrix G2Process::diffusion(Time, const Array&) const
    {
        ...
        */
        ...
        tmp[0][1] = 0.0;
        tmp[1][0] = rho_ * sigma1;
        tmp[1][1] = std::sqrt(1.0 - rho_ * rho_) * sigma2;
        return tmp;
    }

    Array G2Process::expectation(Time t0, const Array& x0, Time dt) const
    {
        return {xProcess_->expectation(t0, x0[0], dt), yProcess_->expectation(t0, x0[1], dt)};
    }
    ...
    Matrix G2Process::stdDeviation(Time t0, const Array& x0, Time dt) const
    {
        ...
        */
        ...
        Real den = (0.5 * sigma_ * eta_) * std::sqrt((1 - expa * expa) * (1 - expb * expb) / (a_ * b_));
        Real newRho = H / den;
        tmp[0][0] = sigma1;
        tmp[0][1] = 0.0;
    ...
    {
        return {xProcess_->drift(t, x[0]) + xForwardDrift(t, T_), yProcess_->drift(t, x[1]) + yForwardDrift(t, T_)};
    }
    ...
    Array G2ForwardProcess::expectation(Time t0, const Array& x0, Time dt) const
    {
        return {xProcess_->expectation(t0, x0[0], dt) - Mx_T(t0, t0 + dt, T_),
    ...
    Real G2ForwardProcess::xForwardDrift(Time t, Time T) const
    {
        ...
        return -(sigma_ * sigma_ / a_) * (1 - expatT) - (rho_ * sigma_ * eta_ / b_) * (1 - expbtT);
    }
    ...
    Real G2ForwardProcess::yForwardDrift(Time t, Time T) const
    {
        ...
        return -(eta_ * eta_ / b_) * (1 - expbtT) - (rho_ * sigma_ * eta_ / a_) * (1 - expatT);
    }

    Real G2ForwardProcess::Mx_T(Real s, Real t, Real T) const
    {
        Real M;
        M = ((sigma_ * sigma_) / (a_ * a_) + (rho_ * sigma_ * eta_) / (a_ * b_)) * (1 - std::exp(-a_ * (t - s)));
        M += -(sigma_ * sigma_) / (2 * a_ * a_) * (std::exp(-a_ * (T - t)) - std::exp(-a_ * (T + t - 2 * s)));
    ...
    {
        Real M;
        M = ((eta_ * eta_) / (b_ * b_) + (rho_ * sigma_ * eta_) / (a_ * b_)) * (1 - std::exp(-b_ * (t - s)));
        M += -(eta_ * eta_) / (2 * b_ * b_) * (std::exp(-b_ * (T - t)) - std::exp(-b_ * (T + t - 2 * s)));
