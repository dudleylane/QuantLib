...
namespace QuantLib
{
    ...
    JointStochasticProcess::JointStochasticProcess(std::vector<ext::shared_ptr<StochasticProcess>> l, Size factors)
    ...
    Array JointStochasticProcess::slice(const Array& x, Size i) const
    {
    ...
    Array JointStochasticProcess::initialValues() const
    {
        ...
        for (auto iter = l_.begin(); iter != l_.end(); ++iter)
        {
            ...
            std::copy(pInitValues.begin(), pInitValues.end(), retVal.begin() + vsize_[iter - l_.begin()]);
        }
    ...
    Array JointStochasticProcess::drift(Time t, const Array& x) const
    {
        Array retVal(size());
        ...
        for (Size i = 0; i < l_.size(); ++i)
        {
            ...
            std::copy(pDrift.begin(), pDrift.end(), retVal.begin() + vsize_[i]);
        }

        return retVal;
    }

    Array JointStochasticProcess::expectation(Time t0, const Array& x0, Time dt) const
    {
        Array retVal(size());
        ...
        for (Size i = 0; i < l_.size(); ++i)
        {
            ...
            std::copy(pExpectation.begin(), pExpectation.end(), retVal.begin() + vsize_[i]);
        }
    ...
    Matrix JointStochasticProcess::covariance(Time t0, const Array& x0, Time dt) const
    {
        ...
        for (Size j = 0; j < l_.size(); ++j)
        {
            ...
            {
                std::copy(pCov.row_begin(i), pCov.row_end(i), retVal.row_begin(vs + i) + vs);
            }
        }
    ...
    Matrix JointStochasticProcess::stdDeviation(Time t0, const Array& x0, Time dt) const
    {
        return pseudoSqrt(covariance(t0, x0, dt));
    }


    Array JointStochasticProcess::apply(const Array& x0, const Array& dx) const
    {
        Array retVal(size());
        ...
        for (Size i = 0; i < l_.size(); ++i)
        {
            ...
            std::copy(pApply.begin(), pApply.end(), retVal.begin() + vsize_[i]);
        }

        return retVal;
    }

    Array JointStochasticProcess::evolve(Time t0, const Array& x0, Time dt, const Array& dw) const
    {
        Array dv(modelFactors_);

        if (correlationIsStateDependent() || correlationCache_.count(CachingKey(t0, dt)) == 0)
        {
            ...
            for (Size j = 0; j < l_.size(); ++j)
            {
                ...
                for (Size i = 0; i < stdDev.rows(); ++i)
                {
                    const Volatility vol = std::sqrt(
                        std::inner_product(stdDev.row_begin(i), stdDev.row_end(i), stdDev.row_begin(i), Real(0.0)));
                    if (vol > 0.0)
                    {
                        std::transform(stdDev.row_begin(i), stdDev.row_end(i), stdDev.row_begin(i),
                                       [=](Real x) -> Real { return x / vol; });
                    }
                    else
                    {
                        // keep the svd happy
                        std::fill(stdDev.row_begin(i), stdDev.row_end(i), 100 * i * QL_EPSILON);
                    }
                }
                ...
                for (Size i = 0; i < stdDev.rows(); ++i)
                {
                    std::copy(inv.row_begin(i), inv.row_end(i), diff.row_begin(i + vs) + vf);
                }
            }

            Matrix rs = rankReducedSqrt(cov, factors_, 1.0, SalvagingAlgorithm::Spectral);
            ...
            if (rs.columns() < factors_)
            {
                ...
                for (Size i = 0; i < cov.rows(); ++i)
                {
                    std::copy(rs.row_begin(i), rs.row_end(i), tmp.row_begin(i));
                }
                rs = tmp;
            }
        ...
        for (auto iter = l_.begin(); iter != l_.end(); ++iter)
        {
            ...
            std::copy(dv.begin() + vfactors_[i], dv.begin() + vfactors_[i] + (*iter)->factors(), dz.begin());
            Array x((*iter)->size());
            std::copy(x0.begin() + vsize_[i], x0.begin() + vsize_[i] + (*iter)->size(), x.begin());
    ...
    const std::vector<ext::shared_ptr<StochasticProcess>>& JointStochasticProcess::constituents() const
    {
        return l_;
    }
