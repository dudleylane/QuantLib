...
namespace QuantLib
{
    ...
    : StochasticProcess(ext::shared_ptr<discretization>(new EulerDiscretization)),
      riskFreeRate_(std::move(riskFreeRate)), dividendYield_(std::move(dividendYield)), s0_(std::move(s0)), v0_(v0),
      kappa_(kappa), theta_(theta), sigma_(sigma), rho_(rho), discretization_(d)
    {
    ...
    Size HestonProcess::factors() const
    {
        return (discretization_ == BroadieKayaExactSchemeLobatto ||
                ...
                discretization_ == BroadieKayaExactSchemeLaguerre) ?
                   3 :
                   2;
    }
    ...
    Array HestonProcess::drift(Time t, const Array& x) const
    {
        ...
        return {riskFreeRate_->forwardRate(t, t, Continuous).rate() -
                    dividendYield_->forwardRate(t, t, Continuous).rate() - 0.5 * vol * vol,
    ...
    Array HestonProcess::apply(const Array& x0, const Array& dx) const
    {
    ...
    namespace
    {
        ...
        std::complex<Real> Phi(const HestonProcess& process, const std::complex<Real>& a, Real nu_0, Real nu_t, Time dt)
        {
            ...
            const std::complex<Real> ga = std::sqrt(kappa * kappa - 2 * sigma2 * a * std::complex<Real>(0.0, 1.0));
            const Real d = 4 * theta * kappa / sigma2;

            const Real nu = 0.5 * d - 1;
            const std::complex<Real> z = ga * std::exp(-0.5 * ga * dt) / (1.0 - std::exp(-ga * dt));
            const std::complex<Real> log_z = -0.5 * ga * dt + std::log(ga / (1.0 - std::exp(-ga * dt)));

            const std::complex<Real> alpha =
                4.0 * ga * std::exp(-0.5 * ga * dt) / (sigma2 * (1.0 - std::exp(-ga * dt)));
            const std::complex<Real> beta =
                4.0 * kappa * std::exp(-0.5 * kappa * dt) / (sigma2 * (1.0 - std::exp(-kappa * dt)));
            ...
            return ga * std::exp(-0.5 * (ga - kappa) * dt) * (1 - std::exp(-kappa * dt)) /
                   ...
                   ((nu_t > 1e-8) ? modifiedBesselFunction_i(nu, std::sqrt(nu_0 * nu_t) * alpha) /
                                        modifiedBesselFunction_i(nu, std::sqrt(nu_0 * nu_t) * beta) :
                                    std::pow(alpha / beta, nu));
        }

        Real ch(const HestonProcess& process, Real x, Real u, Real nu_0, Real nu_t, Time dt)
        {
            return M_2_PI * std::sin(u * x) / u * Phi(process, u, nu_0, nu_t, dt).real();
        }

        Real ph(const HestonProcess& process, Real x, Real u, Real nu_0, Real nu_t, Time dt)
        {
            return M_2_PI * std::cos(u * x) * Phi(process, u, nu_0, nu_t, dt).real();
        }

        Real int_ph(const HestonProcess& process, Real a, Real x, Real y, Real nu_0, Real nu_t, Time t)
        {
            ...
            return gaussLaguerreIntegration([&](Real u) { return ph(process, y, u, nu_0, nu_t, t); }) /
                   std::sqrt(2 * M_PI * (1 - rho * rho) * y) *
                   std::exp(-0.5 * squared(x - x0 - a + y * (0.5 - rho * kappa / sigma)) / (y * (1 - rho * rho)));
        }
        ...
        Real Si(Real x)
        {
            if (x <= 4.0)
            {
                const Real n[] = {-4.54393409816329991e-2, 1.15457225751016682e-3,   -1.41018536821330254e-5,
                                  9.43280809438713025e-8,  -3.53201978997168357e-10, 7.08240282274875911e-13,
                                  -6.05338212010422477e-16};
                const Real d[] = {1.01162145739225565e-2,
            ...
            {
                const Real y = 1 / (x * x);
                const Real fn[] = {7.44437068161936700618e2,  1.96396372895146869801e5,  2.37750310125431834034e7,
                                   1.43073403821274636888e9,  4.33736238870432522765e10, 6.40533830574022022911e11,
                                   4.20968180571076940208e12, 1.00795182980368574617e13, 4.94816688199951963482e12,
                                   -4.94701168645415959931e11};
                const Real fd[] = {7.46437068161927678031e2,  1.97865247031583951450e5,
                ...
                const Real gn[] = {8.1359520115168615e2,   2.35239181626478200e5,  3.12557570795778731e7,
                                   2.06297595146763354e9,  6.83052205423625007e10, 1.09049528450362786e12,
                                   7.57664583257834349e12, 1.81004487464664575e13, 6.43291613143049485e12,
                                   -1.36517137670871689e12};
                const Real gd[] = {8.19595201151451564e2,  2.40036752835578777e5,
        ...
        Real cornishFisherEps(const HestonProcess& process, Real nu_0, Real nu_t, Time dt, Real eps)
        {
            // use moment generating function to get the
            // first,second, third and fourth moment of the distribution
            const Real d = 1e-2;
            const Real p2 = Phi(process, std::complex<Real>(0, -2 * d), nu_0, nu_t, dt).real();
            const Real p1 = Phi(process, std::complex<Real>(0, -d), nu_0, nu_t, dt).real();
            const Real p0 = Phi(process, std::complex<Real>(0, 0), nu_0, nu_t, dt).real();
            const Real pm1 = Phi(process, std::complex<Real>(0, d), nu_0, nu_t, dt).real();
            const Real pm2 = Phi(process, std::complex<Real>(0, 2 * d), nu_0, nu_t, dt).real();
            ...
            const Real skew = (m3 - 3 * var * avg - avg * avg * avg) / (var * stdDev);

            const Real m4 = (pm2 - 4 * pm1 + 6 * p0 - 4 * p1 + p2) / (d * d * d * d);
            const Real kurt = (m4 - 4 * m3 * avg + 6 * m2 * avg * avg - 3 * avg * avg * avg * avg) / (var * var);
        ...
        {
            const Real eps = 1e-4;
            const Real u_eps = std::min(100.0, std::max(0.1, cornishFisherEps(process, nu_0, nu_t, dt, eps)));
            ...
            switch (discretization)
            {
                case HestonProcess::BroadieKayaExactSchemeLaguerre:
                {
                    ...
                    return (x < upper) ?
                               std::max(0.0,
                                        std::min(1.0, gaussLaguerreIntegration(
                ...
                case HestonProcess::BroadieKayaExactSchemeLobatto:
                {
                    ...
                    return (x < upper) ?
                               std::max(0.0,
                                        std::min(1.0, GaussLobattoIntegral(Null<Size>(), eps)(
    ...
    Real HestonProcess::pdf(Real x, Real v, Time t, Real eps) const
    {
        ...
        const Real a = std::log(dividendYield_->discount(t) / riskFreeRate_->discount(t)) +
            ...
            df = 1 / std::sqrt(2 * M_PI * (1 - rho_ * rho_)) *
                 (-0.5 / (upper * std::sqrt(upper)) * std::exp(f2) +
                  1 / std::sqrt(upper) * std::exp(f2) * (-0.5 / (1 - rho_ * rho_)) *
                      (-1 / (upper * upper) * f1 * f1 + 2 / upper * f1 * (0.5 - rho_ * kappa_ / sigma_)));
        ...
        return SegmentIntegral(100)([&](Real xi) { return int_ph(*this, a, x, xi, v0_, v, t); }, QL_EPSILON, upper) *
               boost::math::pdf(
                   boost::math::non_central_chi_squared_distribution<Real>(
                       4 * theta_ * kappa_ / (sigma_ * sigma_),
                       4 * kappa_ * std::exp(-kappa_ * t) / ((sigma_ * sigma_) * (1 - std::exp(-kappa_ * t))) * v0_),
                   v / k) /
               k;
    }

    Array HestonProcess::evolve(Time t0, const Array& x0, Time dt, const Array& dw) const
    {
        Array retVal(2);
        ...
        switch (discretization_)
        {
            ...
            case PartialTruncation:
                ...
                mu = riskFreeRate_->forwardRate(t0, t0 + dt, Continuous).rate() -
                     dividendYield_->forwardRate(t0, t0 + dt, Continuous).rate() - 0.5 * vol * vol;
            ...
            case Reflection:
                ...
                retVal[0] = x0[0] * std::exp(mu * dt + vol * dw[0] * sdt);
                retVal[1] = vol * vol + nu * dt + vol2 * sdt * (rho_ * dw[0] + sqrhov * dw[1]);
                break;
            case NonCentralChiSquareVariance:
                ...
                dy = (mu - rho_ / sigma_ * kappa_ * (theta_ - vol * vol)) * dt + vol * sqrhov * dw[0] * sdt;
            ...
            case QuadraticExponentialMartingale:
            {
                ...
                if (psi < 1.5)
                {
                    ...
                    if (discretization_ == QuadraticExponentialMartingale)
                    {
                        ...
                        k0 = -A * b2 * a / (1 - 2 * A * a) + 0.5 * std::log(1 - 2 * A * a) - (k1 + 0.5 * k3) * x0[1];
                    }
            ...
            case BroadieKayaExactSchemeTrapezoidal:
            {
                ...
                const Real x = std::min(1.0 - QL_EPSILON, std::max(0.0, CumulativeNormalDistribution()(dw[2])));

                const Real vds = Brent().solve(
                    [&](Real xi) { return cdf_nu_ds_minus_x(*this, xi, nu_0, nu_t, dt, discretization_, x); }, 1e-5,
                    theta_ * dt, 0.1 * theta_ * dt);

                const Real vdw = (nu_t - nu_0 - kappa_ * theta_ * dt + kappa_ * vds) / sigma_;
    ...
    Real HestonProcess::varianceDistribution(Real v, Real dw, Time dt) const
    {
        ...
        const Real ncp = 4 * kappa_ * std::exp(-kappa_ * dt) / (sigma_ * sigma_ * (1 - std::exp(-kappa_ * dt))) * v;

        const Real p = std::min(1.0 - QL_EPSILON, std::max(0.0, CumulativeNormalDistribution()(dw)));
