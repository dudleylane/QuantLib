...
namespace QuantLib
{
    ...
    : mixingFactor_(mixingFactor), hestonProcess_(hestonProcess), leverageFct_(std::move(leverageFct))
    {
    ...
    Array HestonSLVProcess::drift(Time t, const Array& x) const
    {
        ...
        const Volatility vol = std::max(1e-8, std::sqrt(x[1]) * leverageFct_->localVol(t, x[0], true));

        tmp[0] = riskFreeRate()->forwardRate(t, t, Continuous).rate() -
                 dividendYield()->forwardRate(t, t, Continuous).rate() - 0.5 * vol * vol;
    ...
    Matrix HestonSLVProcess::diffusion(Time t, const Array& x) const
    {
        ...
        const Real vol = std::max(1e-8, std::sqrt(x[1]) * leverageFct_->localVol(t, x[0], true));
    ...
    Array HestonSLVProcess::evolve(Time t0, const Array& x0, Time dt, const Array& dw) const
    {
        Array retVal(2);
        ...
        retVal[0] =
            x0[0] * std::exp(mu * dt - 0.5 * v_0 * dt +
                             rho_ / mixedSigma_ * l_0 *
                                 (retVal[1] - kappa_ * theta_ * dt + 0.5 * (x0[1] + retVal[1]) * kappa_ * dt - x0[1]) +
