...
namespace QuantLib::detail
{
    ...
    GsrProcessCore::GsrProcessCore(Array times, Array vols, Array reversions, const Real T)
    : times_(std::move(times)), vols_(std::move(vols)), reversions_(std::move(reversions)), T_(T),
      revZero_(reversions_.size(), false)
    {
        flushCache();
    ...
    {
        QL_REQUIRE(times_.size() == vols_.size() - 1, "number of volatilities ("
        ...
        QL_REQUIRE(times_.size() == reversions_.size() - 1 || reversions_.size() == 1,
                   "number of reversions (" << vols_.size() << ") compared to number of times (" << times_.size()
                                            << " must be bigger by one, or exactly "
                                               "1 reversion must be given");
        for (int i = 0; i < ((int)times_.size()) - 1; i++)
            QL_REQUIRE(times_[i] < times_[i + 1], "times must be increasing (" << times_[i] << "@" << i << " , "
                                                                               << times_[i + 1] << "@" << i + 1 << ")");
    }
    ...
    Real GsrProcessCore::expectation_x0dep_part(const Time w, const Real xw, const Time dt) const
    {
        Real t = w + dt;
    ...
    Real GsrProcessCore::expectation_rn_part(const Time w, const Time dt) const
    {
        ...
        for (int k = lowerIndex(w); k <= upperIndex(t) - 1; k++)
        {
            ...
            for (int l = 0; l <= k - 1; l++)
            {
                ...
                // alpha_l
                res2 *= revZero(l) ?
                            Real(vol(l) * vol(l) * (time2(l + 1) - time2(l))) :
                            vol(l) * vol(l) / (2.0 * rev(l)) * (1.0 - exp(-2.0 * rev(l) * (time2(l + 1) - time2(l))));
                // zeta_i (i>k)
                ...
                // zeta_k beta_k
                res2 *= revZero(k) ? Real(2.0 * time2(k) - flooredTime(k, w) - cappedTime(k + 1, t) -
                                          2.0 * (time2(k) - cappedTime(k + 1, t))) :
                                     Real((exp(rev(k) * (2.0 * time2(k) - flooredTime(k, w) - cappedTime(k + 1, t))) -
            ...
            Real res2 = 1.0;
            // alpha_k zeta_k
            res2 *= revZero(k) ? Real(vol(k) * vol(k) / 4.0 *
                                      (4.0 * pow(cappedTime(k + 1, t) - time2(k), 2.0) -
                                       (pow(flooredTime(k, w) - 2.0 * time2(k) + cappedTime(k + 1, t), 2.0) +
                                        pow(cappedTime(k + 1, t) - flooredTime(k, w), 2.0)))) :
                                 Real(vol(k) * vol(k) / (2.0 * rev(k) * rev(k)) *
                                      (exp(-2.0 * rev(k) * (cappedTime(k + 1, t) - time2(k))) + 1.0 -
                                       (exp(-rev(k) * (flooredTime(k, w) - 2.0 * time2(k) + cappedTime(k + 1, t))) +
                                        exp(-rev(k) * (cappedTime(k + 1, t) - flooredTime(k, w))))));
            // zeta_i (i>k)
            for (int i = k + 1; i <= upperIndex(t) - 1; i++)
                res2 *= exp(-rev(i) * (cappedTime(i + 1, t) - time2(i)));
    ...
    Real GsrProcessCore::expectation_tf_part(const Time w, const Time dt) const
    {
        ...
        for (int k = lowerIndex(w); k <= upperIndex(t) - 1; k++)
        {
            ...
            for (int l = k + 1; l <= upperIndex(T_) - 1; l++)
            {
                Real res3 = 1.0;
                // eta_l
                res3 *= revZero(l) ? Real(cappedTime(l + 1, T_) - time2(l)) :
                                     (1.0 - exp(-rev(l) * (cappedTime(l + 1, T_) - time2(l)))) / rev(l);
                // zeta_i (i>k)
                for (int i = k + 1; i <= upperIndex(t) - 1; i++)
                ...
                res3 *= revZero(k) ?
                            ...
                            Real((exp(rev(k) * (cappedTime(k + 1, t) - time2(k + 1))) -
                                  exp(rev(k) * (2.0 * flooredTime(k, w) - cappedTime(k + 1, t) - time2(k + 1)))) /
                                 (2.0 * rev(k)));
                // add to sum
                res2 += res3;
            }
            ...
            res3 *=
                revZero(k) ?
                    Real((-pow(cappedTime(k + 1, t) - cappedTime(k + 1, T_), 2.0) -
                          ...
                          pow(2.0 * flooredTime(k, w) - cappedTime(k + 1, T_) - cappedTime(k + 1, t), 2.0)) /
                         4.0) :
                    Real((2.0 - exp(rev(k) * (cappedTime(k + 1, t) - cappedTime(k + 1, T_))) -
                          (2.0 * exp(-rev(k) * (cappedTime(k + 1, t) - flooredTime(k, w))) -
                           exp(rev(k) * (2.0 * flooredTime(k, w) - cappedTime(k + 1, T_) - cappedTime(k + 1, t))))) /
                         (2.0 * rev(k) * rev(k)));
            // zeta_i (i>k)
            for (int i = k + 1; i <= upperIndex(t) - 1; i++)
                res3 *= exp(-rev(i) * (cappedTime(i + 1, t) - time2(i)));
    ...
    Real GsrProcessCore::variance(const Time w, const Time dt) const
    {
        ...
        for (int k = lowerIndex(w); k <= upperIndex(t) - 1; k++)
        {
            ...
            res2 *= revZero(k) ?
                        ...
                        (1.0 - exp(2.0 * rev(k) * (flooredTime(k, w) - cappedTime(k + 1, t)))) / (2.0 * rev(k));
            // zeta_i (i>k)
            for (int i = k + 1; i <= upperIndex(t) - 1; i++)
            {
    ...
    Real GsrProcessCore::y(const Time t) const
    {
        ...
        for (int i = 0; i <= upperIndex(t) - 1; i++)
        {
            ...
            res2 *=
                revZero(i) ?
                    Real(vol(i) * vol(i) * (cappedTime(i + 1, t) - time2(i))) :
                    (vol(i) * vol(i) / (2.0 * rev(i)) * (1.0 - exp(-2.0 * rev(i) * (cappedTime(i + 1, t) - time2(i)))));
            res += res2;
        }
    ...
    Real GsrProcessCore::G(const Time t, const Time w) const
    {
        ...
        for (int i = lowerIndex(t); i <= upperIndex(w) - 1; i++)
        {
            ...
            res2 *= revZero(i) ? Real(cappedTime(i + 1, w) - flooredTime(i, t)) :
                                 (1.0 - exp(-rev(i) * (cappedTime(i + 1, w) - flooredTime(i, t)))) / rev(i);
            res += res2;
        }
    ...
    {
        return static_cast<int>(std::upper_bound(times_.begin(), times_.end(), t) - times_.begin());
    }
    ...
    int GsrProcessCore::upperIndex(const Time t) const
    {
        if (t < QL_MIN_POSITIVE_REAL)
            return 0;
        return static_cast<int>(std::upper_bound(times_.begin(), times_.end(), t - QL_EPSILON) - times_.begin()) + 1;
    }
    ...
    Real GsrProcessCore::flooredTime(const Size index, const Real floor) const
    {
    ...
    Real GsrProcessCore::time2(const Size index) const
    {
        ...
        if (index > times_.size())
        {
            ...
            QL_REQUIRE(times_.empty() || T_ >= times_.back(),
                       "GSR forward-measure horizon T (" << T_ << ") is less than the last step time (" << times_.back()
