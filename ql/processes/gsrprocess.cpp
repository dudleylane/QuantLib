...
namespace QuantLib
{
    ...
    GsrProcess::GsrProcess(
        Array times, Array vols, Array reversions, const Real T, const Date& referenceDate, DayCounter dc)
    : ForwardMeasureProcess1D(T), core_(std::move(times), std::move(vols), std::move(reversions), T),
    ...
    {
        checkT(w + dt);
        return core_.expectation_x0dep_part(w, xw, dt) + core_.expectation_rn_part(w, dt) +
    ...
    Real GsrProcess::sigma(Time t) const
    {
        return core_.sigma(t);
    }

    Real GsrProcess::reversion(Time t) const
    {
    ...
    {
        QL_REQUIRE(w >= t, "G(t,w) should be called with w (" << w << ") not lesser than t (" << t << ")");
        QL_REQUIRE(t >= 0.0 && w <= getForwardMeasureTime(), "G(t,w) should be called with (t,w)=("
                                                                 << t << "," << w << ") in Range [0,"
