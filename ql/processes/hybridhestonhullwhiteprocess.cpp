...
namespace QuantLib
{
    ...
    : hestonProcess_(hestonProcess), hullWhiteProcess_(hullWhiteProcess),
      hullWhiteModel_(new HullWhite(hestonProcess->riskFreeRate(), hullWhiteProcess->a(), hullWhiteProcess->sigma())),
      corrEquityShortRate_(corrEquityShortRate), discretization_(discretization),
      maxRho_(std::sqrt(1 - hestonProcess->rho() * hestonProcess->rho()) -
              std::sqrt(QL_EPSILON) /* reserve for rounding errors */),

      T_(hullWhiteProcess->getForwardMeasureTime()), endDiscount_(hestonProcess->riskFreeRate()->discount(T_))
    {

        QL_REQUIRE(corrEquityShortRate * corrEquityShortRate + hestonProcess->rho() * hestonProcess->rho() <= 1.0,
                   "correlation matrix is not positive definite");

        QL_REQUIRE(hullWhiteProcess->sigma() > 0.0, "positive vol of Hull White process is required");
    }
    ...
    {
        return {hestonProcess_->s0()->value(), hestonProcess_->v0(), hullWhiteProcess_->x0()};
    }
    ...
    Array HybridHestonHullWhiteProcess::drift(Time t, const Array& x) const
    {
        ...
        return {y0[0], y0[1], hullWhiteProcess_->drift(t, x[2])};
    }
    ...
    Array HybridHestonHullWhiteProcess::apply(const Array& x0, const Array& dx) const
    {
        ...
        return {yt[0], yt[1], hullWhiteProcess_->apply(x0[2], dx[2])};
    }
    ...
    Matrix HybridHestonHullWhiteProcess::diffusion(Time t, const Array& x) const
    {
        ...
        retVal[2][2] = std::sqrt(sigma * sigma - retVal[2][1] * retVal[2][1] - retVal[2][0] * retVal[2][0]);

        return retVal;
    }

    Array HybridHestonHullWhiteProcess::evolve(Time t0, const Array& x0, Time dt, const Array& dw) const
    {
        ...
        const Time t = t0 + dt;
        const Time T = T_;
        const Rate dy = hestonProcess_->dividendYield()->forwardRate(s, t, Continuous, NoFrequency);

        const Real df =
            std::log(hestonProcess_->riskFreeRate()->discount(t) / hestonProcess_->riskFreeRate()->discount(s));
        ...
        const Real m3 = (r - hullWhiteProcess_->alpha(s)) * hullWhiteProcess_->B(s, t);

        const Real m4 =
            sigma * sigma / (2 * a * a) * (dt + 2 / a * (eat - eas) - 1 / (2 * a) * (eat * eat - eas * eas));
        ...
        const Real nu = hestonProcess_->kappa() * (hestonProcess_->theta() - eta * eta);

        retVal[1] = x0[1] + nu * dt + eta2 * std::sqrt(dt) * (xi * dw[0] + std::sqrt(1 - xi * xi) * dw[1]);

        if (discretization_ == BSMHullWhite)
        {
            const Real v1 =
                eta * eta * dt +
                sigma * sigma / (a * a) * (dt - 2 / a * (1 - eat * ias) + 1 / (2 * a) * (1 - eat * eat * ias * ias)) +
            ...
            const Real rhoT = std::min(maxRho_, std::max(-maxRho_, v12 / std::sqrt(v1 * v2)));
            QL_REQUIRE(rhoT <= 1.0 && rhoT >= -1.0 && 1 - rhoT * rhoT / (1 - xi * xi) >= 0.0,
    ...
    DiscountFactor HybridHestonHullWhiteProcess::numeraire(Time t, const Array& x) const
    {
    ...
    const ext::shared_ptr<HestonProcess>& HybridHestonHullWhiteProcess::hestonProcess() const
    {
        return hestonProcess_;
    }

    const ext::shared_ptr<HullWhiteForwardProcess>& HybridHestonHullWhiteProcess::hullWhiteProcess() const
    {
        return hullWhiteProcess_;
    }

    HybridHestonHullWhiteProcess::Discretization HybridHestonHullWhiteProcess::discretization() const
    {
