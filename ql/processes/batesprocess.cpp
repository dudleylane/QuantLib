...
namespace QuantLib
{
    BatesProcess::BatesProcess(const Handle<YieldTermStructure>& riskFreeRate,
    ...
    : HestonProcess(riskFreeRate, dividendYield, s0, v0, kappa, theta, sigma, rho, d), lambda_(lambda), delta_(delta),
      nu_(nu), m_(std::exp(nu + 0.5 * delta * delta) - 1)
    {
    }
    ...
    Array BatesProcess::evolve(Time t0, const Array& x0, Time dt, const Array& dw) const
    {
        ...
        retVal[0] *= std::exp(-lambda_ * m_ * dt + nu_ * n + delta_ * std::sqrt(n) * dw[hestonFactors + 1]);
