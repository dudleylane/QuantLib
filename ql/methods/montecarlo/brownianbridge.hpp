      ...
      public:
        ...
        */
        ...
        void transform(RandomAccessIterator1 begin, RandomAccessIterator1 end, RandomAccessIterator2 output) const
        {
            ...
            QL_REQUIRE(Size(end - begin) == size_, "incompatible sequence size");
            ...
            for (Size i = 1; i < size_; ++i)
            {
                ...
                if (j != 0)
                {
                    output[l] = leftWeight_[i] * output[j - 1] + rightWeight_[i] * output[k] + stdDev_[i] * begin[i];
                }
                else
                {
                    output[l] = rightWeight_[i] * output[k] + stdDev_[i] * begin[i];
                }
            }
