                                            ...
                                            std::vector<std::vector<Real>>& basisCoefficients)
    {
        ...
        for (Size i = steps - 1; i != 0; --i)
        {
            ...
            for (j = 0; j < exerciseData.size(); ++j)
            {
                ...
                {
                    std::copy(exerciseData[j].values.begin(), exerciseData[j].values.end(), temp.begin());
                    temp.back() = exerciseData[j].cumulatedCashFlows - exerciseData[j].controlValue;
            ...
            std::copy(alphas.begin(), alphas.end(), basisCoefficients[i - 1].begin());
            ...
            for (j = 0; j < exerciseData.size(); ++j)
            {
                if (exerciseData[j].isValid)
                {
                    ...
                    Real estimatedContinuationValue =
                        std::inner_product(exerciseData[j].values.begin(), exerciseData[j].values.end(), alphas.begin(),
                    ...
                    // deflated cash-flows at previous time frame
                    Real value = estimatedContinuationValue <= exerciseValue ? exerciseValue : continuationValue;
