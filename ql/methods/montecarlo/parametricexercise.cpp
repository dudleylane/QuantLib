...
namespace QuantLib
{
    ...
    namespace
    {
        ...
        ValueEstimate::ValueEstimate(const std::vector<NodeData>& simulationData,
                                     const ParametricExercise& exercise,
                                     Size exerciseIndex)
        : simulationData_(simulationData), exercise_(exercise), exerciseIndex_(exerciseIndex),
                                          ...
                                          OptimizationMethod& method)
    {
        ...
        for (Size i = steps - 1; i != 0; --i)
        {
            ...
            std::copy(parameters[i - 1].begin(), parameters[i - 1].end(), guess.begin());
            ...
            std::copy(result.begin(), result.end(), parameters[i - 1].begin());
            ...
            for (Size j = 0; j < previousData.size(); ++j)
            {
                if (exerciseData[j].isValid)
                {
                    if (exercise.exercise(i - 1, parameters[i - 1], exerciseData[j].values))
                        previousData[j].cumulatedCashFlows += exerciseData[j].exerciseValue;
                    else
