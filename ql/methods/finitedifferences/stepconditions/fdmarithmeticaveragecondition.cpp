...
namespace QuantLib
{
    ...
    FdmArithmeticAverageCondition::FdmArithmeticAverageCondition(std::vector<Time> averageTimes,
                                                                 Real,
                                                                 Size pastFixings,
                                                                 const ext::shared_ptr<FdmMesher>& mesher,
                                                                 Size equityDirection)
    : x_(mesher->layout()->dim()[equityDirection]), a_(mesher->layout()->dim()[equityDirection == 0 ? 1 : 0]),
      ...
      equityDirection_(equityDirection)
    {
        ...
        QL_REQUIRE(equityDirection == 0 || equityDirection == 1, "equityDirection has to be 0 or 1");
    ...
    void FdmArithmeticAverageCondition::applyTo(Array& a, Time t) const
    {
        ...
        const auto iter = std::find(averageTimes_.begin(), averageTimes_.end(), t);
        const Size nTimes = std::count(averageTimes_.begin(), averageTimes_.end(), t);
        if (nTimes > 0)
        {
            Array aCopy(a);
            ...
            for (Size i = 0; i < x_.size(); ++i)
            {
                ...
                MonotonicCubicNaturalSpline interp(a_.begin(), a_.end(), tmp.begin());
                for (Size j = 0; j < a_.size(); ++j)
                {
                    Size index = i * xSpacing + j * aSpacing;
                    a[index] = interp((iT - nTimes) / (double)(iT)*a_[j] + nTimes / (double)(iT)*x_[i], true);
                }
            }
        }
    }
