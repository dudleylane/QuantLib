...
namespace QuantLib
{
    ...
    : process_(std::move(process)), solverDesc_(std::move(solverDesc)), schemeDesc_(schemeDesc),
      quantoHelper_(std::move(quantoHelper)), leverageFct_(std::move(leverageFct)), mixingFactor_(mixingFactor)
    {
    ...
    {
        ext::shared_ptr<FdmLinearOpComposite> op(ext::make_shared<FdmHestonOp>(
            solverDesc_.mesher, process_.currentLink(),
            (!quantoHelper_.empty()) ? quantoHelper_.currentLink() : ext::shared_ptr<FdmQuantoHelper>(), leverageFct_,
            mixingFactor_));
    ...
    Real FdmHestonSolver::meanVarianceGammaAt(Real s, Real v) const
    {
        ...
        const Real x = std::log(s);
        const Real alpha = process_->rho() * process_->sigma() / s;
        return gammaAt(s, v) + solver_->derivativeYY(x, v) * alpha * alpha +
