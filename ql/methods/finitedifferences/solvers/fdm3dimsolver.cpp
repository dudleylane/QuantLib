...
namespace QuantLib
{
      ...
      interpolation_(solverDesc.mesher->layout()->dim()[2])
    {
        ...
        {
            initialValues_[iter.index()] = solverDesc.calculator->avgInnerValue(iter, solverDesc.maturity);
    ...
    void Fdm3DimSolver::performCalculations() const
    {
        ...
        {
            std::copy(rhs.begin() + i * y_.size() * x_.size(), rhs.begin() + (i + 1) * y_.size() * x_.size(),
                      resultValues_[i].begin());

            interpolation_[i] =
                ext::make_shared<BicubicSpline>(x_.begin(), x_.end(), y_.begin(), y_.end(), resultValues_[i]);
        }
    }
    ...
    Real Fdm3DimSolver::interpolateAt(Real x, Real y, Rate z) const
    {
        ...
        return MonotonicCubicNaturalSpline(z_.begin(), z_.end(), zArray.begin())(z);
    }
    ...
    Real Fdm3DimSolver::thetaAt(Real x, Real y, Rate z) const
    {
        ...
        for (Size i = 0; i < z_.size(); ++i)
        {
            zArray[i] = BicubicSpline(x_.begin(), x_.end(), y_.begin(), y_.end(), thetaValues[i])(x, y);
        }

        return (MonotonicCubicNaturalSpline(z_.begin(), z_.end(), zArray.begin())(z) - interpolateAt(x, y, z)) /
