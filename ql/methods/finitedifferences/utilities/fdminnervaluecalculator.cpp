...
namespace QuantLib
{
    ...
    namespace
    {
        ...
        {
            explicit mapped_payoff(const Payoff& payoff, const std::function<Real(Real)>& gridMapping)
    ...
    Real FdmCellAveragingInnerValue::avgInnerValueCalc(const FdmLinearOpIterator& iter, Time t)
    {
        ...
        try
        {
            const Real acc = ((f(a) != 0.0 || f(b) != 0.0) ? Real((f(a) + f(b)) * 5e-5) : 1e-4);
    ...
    FdmLogInnerValue::FdmLogInnerValue(const ext::shared_ptr<Payoff>& payoff,
                                       const ext::shared_ptr<FdmMesher>& mesher,
                                       Size direction)
    : FdmCellAveragingInnerValue(payoff, mesher, direction, [](Real x) -> Real { return std::exp(x); })
    {
    }
    ...
    Real FdmLogBasketInnerValue::innerValue(const FdmLinearOpIterator& iter, Time)
    {
    ...
    Real FdmLogBasketInnerValue::avgInnerValue(const FdmLinearOpIterator& iter, Time t)
    {
        return innerValue(iter, t);
    }
}
