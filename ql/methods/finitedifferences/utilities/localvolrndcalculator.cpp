...
namespace QuantLib
{
    ...
    : xGrid_(xGrid), tGrid_(tGrid), x0Density_(x0Density), localVolProbEps_(eps), maxIter_(maxIter),
      gaussianStepSize_(gaussianStepSize), spot_(std::move(spot)), localVol_(localVol), rTS_(std::move(rTS)),
      qTS_(std::move(qTS)), timeGrid_(new TimeGrid(localVol->maxTime(), tGrid)), xm_(tGrid),
    ...
    LocalVolRNDCalculator::LocalVolRNDCalculator(ext::shared_ptr<Quote> spot,
                                                 ...
                                                 Size maxIter,
                                                 Time gaussianStepSize)
    : xGrid_(xGrid), tGrid_(timeGrid->size() - 1), x0Density_(x0Density), localVolProbEps_(eps), maxIter_(maxIter),
      gaussianStepSize_(gaussianStepSize), spot_(std::move(spot)), localVol_(std::move(localVol)), rTS_(std::move(rTS)),
      qTS_(std::move(qTS)), timeGrid_(timeGrid), xm_(tGrid_), pm_(new Matrix(tGrid_, xGrid_))
    {
        registerWith(spot_);
        registerWith(rTS_);
        registerWith(qTS_);
    ...
    Real LocalVolRNDCalculator::pdf(Real x, Time t) const
    {
        ...
        QL_REQUIRE(t <= timeGrid_->back(), "given time exceeds local vol time grid");
        ...
        if (t <= tMin)
        {
            ...
            const Real xm = -0.5 * stdDev * stdDev + std::log(spot_->value() * qTS_->discount(t) / rTS_->discount(t));
        ...
        else if (t <= timeGrid_->at(1))
        {
            ...
            const Real xm =
                -0.5 * stdDev * stdDev + std::log(spot_->value() * qTS_->discount(tMin) / rTS_->discount(tMin));
        ...
        {
            const auto lb = std::lower_bound(timeGrid_->begin(), timeGrid_->end(), t);
            const auto llb = lb - 1;
    ...
    Real LocalVolRNDCalculator::cdf(Real x, Time t) const
    {
        ...
        const Size idx = (tc > t) ? timeGrid_->index(tc) - 1 : std::min(xm_.size() - 1, timeGrid_->index(tc));
        ...
        if (x > 0.5 * (xr + xl))
        {
            ...
            return 1.0 -
                   GaussLobattoIntegral(maxIter_, 0.1 * localVolProbEps_)([&](Real _x) { return pdf(_x, t); }, x, xr);
        }
        else
        {
            ...
            return GaussLobattoIntegral(maxIter_, 0.1 * localVolProbEps_)([&](Real _x) { return pdf(_x, t); }, xl, x);
        }
    }
    ...
    Real LocalVolRNDCalculator::invcdf(Real p, Time t) const
    {
        ...
        {
            const Real stepSize = 0.02 * (xm_[0]->locations().back() - xm_[0]->locations().front());
            return RiskNeutralDensityCalculator::InvCDFHelper(this, std::log(spot_->value()), 0.1 * localVolProbEps_,
                                                              maxIter_, stepSize)
                .inverseCDF(p, t);
        }
        else
        {
            ...
            const Array x(xm_[idx]->locations().begin(), xm_[idx]->locations().end());
            ...
            return RiskNeutralDensityCalculator::InvCDFHelper(this, xm, 0.1 * localVolProbEps_, maxIter_, stepSize)
                .inverseCDF(p, t);
        }
    }

    ext::shared_ptr<Fdm1dMesher> LocalVolRNDCalculator::mesher(Time t) const
    {
        calculate();
        ...
        {
            return ext::make_shared<Predefined1dMesher>(std::vector<Real>(xGrid_, std::log(spot_->value())));
        }
    }
    ...
    void LocalVolRNDCalculator::performCalculations() const
    {
        ...
        Time t = std::min(sT, (gaussianStepSize_ > 0.0) ? gaussianStepSize_ : 0.5 * sT);
        const Volatility vol = localVol_->localVol(0.0, spot_->value());

        const Volatility stdDev = vol * std::sqrt(t);
        Real xm = -0.5 * stdDev * stdDev + std::log(spot_->value() * qTS_->discount(t) / rTS_->discount(t));
        ...
        ext::shared_ptr<Fdm1dMesher> mesher(
            new Concentrating1dMesher(sLowerBound, sUpperBound, xGrid_, std::make_pair(xm, x0Density_), true));
        ...
        ext::shared_ptr<DouglasScheme> evolver(
            new DouglasScheme(0.5, ext::make_shared<FdmLocalVolFwdOp>(ext::make_shared<FdmMesherComposite>(mesher),
        ...
        for (Size i = 1; i <= tGrid_; ++i)
        {
            ...
            const Real maxLeftValue = std::max(std::fabs(*std::min_element(p.begin(), p.begin() + b)),
                                               std::fabs(*std::max_element(p.begin(), p.begin() + b)));
            const Real maxRightValue = std::max(std::fabs(*std::min_element(p.end() - b, p.end())),
            ...
            if (std::max(maxLeftValue, maxRightValue) > localVolProbEps_)
            {
                ...
                const Real vm = DiscreteSimpsonIntegral()(x, vols) / (x.back() - x.front());
                ...
                mesher = ext::shared_ptr<Fdm1dMesher>(
                    new Concentrating1dMesher(sLowerBound, sUpperBound, xGrid_, std::make_pair(xm, 0.1), false));

                const CubicNaturalSpline pSpline(x.begin(), x.end(), p.begin());
                const Array xn(mesher->locations().begin(), mesher->locations().end());
                ...
                evolver = ext::make_shared<DouglasScheme>(
                    0.5, ext::make_shared<FdmLocalVolFwdOp>(ext::make_shared<FdmMesherComposite>(mesher), spot_, rTS_,
                                                            qTS_, localVol_));
            }
            evolver->setStep(dt);
            t += dt;
            ...
            pFct_[i - 1] = ext::make_shared<CubicNaturalSpline>(xm_[i - 1]->locations().begin(),
                                                                xm_[i - 1]->locations().end(), pm_->row_begin(i - 1));
        }
    }
    ...
    Real LocalVolRNDCalculator::probabilityInterpolation(Size idx, Real x) const
    {
        calculate();
