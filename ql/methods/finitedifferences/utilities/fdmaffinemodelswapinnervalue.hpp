...
namespace QuantLib
{
    ...
    : disModel_(std::move(disModel)), fwdModel_(std::move(fwdModel)), index_(swap->iborIndex()),
      swap_(
          [&]() -> ext::shared_ptr<FixedVsFloatingSwap>
          {
              ...
              if (ois)
              {
                  auto clonedIndex = ext::dynamic_pointer_cast<OvernightIndex>(ois->overnightIndex()->clone(fwdTs_));
                  QL_REQUIRE(clonedIndex, "failed to clone OvernightIndex");
                  return ext::make_shared<OvernightIndexedSwap>(
                      ois->type(), ois->nominal(), ois->fixedSchedule(), ois->fixedRate(), ois->fixedDayCount(),
                      ois->overnightSchedule(), clonedIndex, ois->spread(), ois->paymentLag(), ois->paymentConvention(),
                      ois->paymentCalendar(), ois->telescopicValueDates(), ois->averagingMethod(), ois->lookbackDays(),
                      ois->lockoutDays(), ois->applyObservationShift());
              }
              return ext::make_shared<VanillaSwap>(swap->type(), swap->nominal(), swap->fixedSchedule(),
                                                   swap->fixedRate(), swap->fixedDayCount(), swap->floatingSchedule(),
                                                   swap->iborIndex()->clone(fwdTs_), swap->spread(),
                                                   swap->floatingDayCount(), swap->paymentConvention());
          }()),
    ...
    template <class ModelType>
    inline Real FdmAffineModelSwapInnerValue<ModelType>::innerValue(const FdmLinearOpIterator& iter, Time t)
    {
        ...
        if (disTs_.empty() || iterExerciseDate != disTs_->referenceDate())
        {
            ...
            disTs_.linkTo(ext::shared_ptr<YieldTermStructure>(
                new FdmAffineModelTermStructure(disRate, discount->calendar(), discount->dayCounter(), iterExerciseDate,
                                                discount->referenceDate(), disModel_)));

            const Handle<YieldTermStructure> fwd = fwdModel_->termStructure();

            fwdTs_.linkTo(ext::shared_ptr<YieldTermStructure>(new FdmAffineModelTermStructure(
                fwdRate, fwd->calendar(), fwd->dayCounter(), iterExerciseDate, fwd->referenceDate(), fwdModel_)));
        }
        else
        {
            ext::dynamic_pointer_cast<FdmAffineModelTermStructure>(disTs_.currentLink())->setVariable(disRate);
            ext::dynamic_pointer_cast<FdmAffineModelTermStructure>(fwdTs_.currentLink())->setVariable(fwdRate);
        }
        ...
        for (Size j = 0; j < 2; j++)
        {
            ...
            {
                npv += ext::dynamic_pointer_cast<Coupon>(i)->accrualStartDate() >= iterExerciseDate ?
    ...
    template <class ModelType>
    inline Real FdmAffineModelSwapInnerValue<ModelType>::avgInnerValue(const FdmLinearOpIterator& iter, Time t)
    {
