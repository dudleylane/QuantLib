...
namespace QuantLib
{
    ...
    Array FdmHestonGreensFct::get(Time t, Algorithm algorithm) const
    {
        ...
        for (const auto& iter : *mesher_->layout())
        {
            ...
            const Real v = (trafoType_ != FdmSquareRootFwdOp::Log) ? mesher_->location(iter, 1) :
            ...
            switch (algorithm)
            {
                case ZeroCorrelation:
                {
                    ...
                    const Real p_x = M_1_SQRTPI * M_SQRT1_2 / sd_x * std::exp(-0.5 * squared((x - x0) / sd_x));
                    const Real p_v = SquareRootProcessRNDCalculator(v0, kappa, theta, sigma).pdf(v, t);
                ...
                case Gaussian:
                {
                    ...
                    retVal = 1.0 / (M_TWOPI * sd_x * sd_v * std::sqrt(1 - rho * rho)) *
                             std::exp(-(squared((x - x0) / sd_x) + squared((v - z0) / sd_v) -
