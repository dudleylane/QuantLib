...
namespace QuantLib
{
    ...
    EscrowedDividendAdjustment::EscrowedDividendAdjustment(DividendSchedule dividendSchedule,
                                                           ...
                                                           Time maturity)
    : dividendSchedule_(std::move(dividendSchedule)), rTS_(std::move(rTS)), qTS_(std::move(qTS)),
    ...
    Real EscrowedDividendAdjustment::dividendAdjustment(Time t) const
    {
        ...
        for (auto const& dividend : dividendSchedule_)
        {
            ...
            {
                divAdj -= dividend->amount() * rTS_->discount(divTime) / rTS_->discount(t) * qTS_->discount(t) /
    ...
    const Handle<YieldTermStructure>& EscrowedDividendAdjustment::riskFreeRate() const
    {
        return rTS_;
    }

    const Handle<YieldTermStructure>& EscrowedDividendAdjustment::dividendYield() const
    {
        return qTS_;
    }
}
