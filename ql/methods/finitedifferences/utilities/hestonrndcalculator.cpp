...
namespace QuantLib
{
    ...
    namespace
    {
        ...
        HestonParams getHestonParams(const ext::shared_ptr<HestonProcess>& process)
        {
            const HestonParams p = {process->v0(), process->kappa(), process->theta(), process->sigma(),
                                    process->rho()};
            return p;
        }
        ...
        std::complex<Real> omega(const HestonParams& p, Real p_x)
        {
            ...
            return std::sqrt(g * g + p.sigma * p.sigma * std::complex<Real>(p_x * p_x, -p_x));
        }
        ...
        class CpxPv_Helper
        {
          public:
            ...
            : p_(p), t_(t), x_(x),
              c_inf_(std::min(10.0, std::max(0.0001, std::sqrt(1.0 - squared(p_.rho)) / p_.sigma)) *
                     (p_.v0 + p_.kappa * p_.theta * t))
            {
            }

            Real operator()(Real x) const { return std::real(transformPhi(x)); }
            ...
            Real p0(Real p_x) const
            {
                ...
                return std::real(phi(u_x) / ((p_x * c_inf_) * std::complex<Real>(0.0, u_x)));
            }
    ...
    Real HestonRNDCalculator::pdf(Real x, Time t) const
    {
        return GaussLobattoIntegral(maxIntegrationIterations_, 0.1 * integrationEps_)(
                   CpxPv_Helper(getHestonParams(hestonProcess_), x_t(x, t), t), 0.0, 1.0) /
               M_TWOPI;
    }
    ...
    Real HestonRNDCalculator::cdf(Real x, Time t) const
    {
        ...
        return GaussLobattoIntegral(maxIntegrationIterations_,
                                    0.1 * integrationEps_)([&](Real p_x) { return helper.p0(p_x); }, 0.0, 1.0) /
                   M_TWOPI +
               0.5;
    }
    ...
    Real HestonRNDCalculator::invcdf(Real p, Time t) const
    {
        ...
        const Volatility expVol = std::sqrt(theta + (v0 - theta) * (1 - std::exp(-kappa * t)) / (t * kappa));

        const ext::shared_ptr<BlackScholesMertonProcess> bsmProcess(ext::make_shared<BlackScholesMertonProcess>(
            hestonProcess_->s0(), hestonProcess_->dividendYield(), hestonProcess_->riskFreeRate(),
            Handle<BlackVolTermStructure>(
                ext::make_shared<BlackConstantVol>(hestonProcess_->riskFreeRate()->referenceDate(), NullCalendar(),
                                                   expVol, hestonProcess_->riskFreeRate()->dayCounter()))));

        const Real guess = BSMRNDCalculator(bsmProcess).invcdf(p, t);

        return RiskNeutralDensityCalculator::InvCDFHelper(this, guess, 0.1 * integrationEps_, maxIntegrationIterations_)
            .inverseCDF(p, t);
    }
}
