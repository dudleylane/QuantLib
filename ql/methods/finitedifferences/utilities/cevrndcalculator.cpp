...
namespace QuantLib
{
    ...
    {
        return std::pow(x * squared(alpha_ * (1.0 - beta_)), 1.0 / (2.0 * (1.0 - beta_)));
    }
    ...
    Real CEVRNDCalculator::pdf(Real f, Time t) const
    {
        ...
        if (delta_ < 2.0)
        {
            return boost::math::pdf(boost::math::non_central_chi_squared_distribution<Real>(4.0 - delta_, y / t),
                                    x0_ / t) /
                   t * 2.0 * (1.0 - beta_) * y / f;
        }
        else
        {
            return boost::math::pdf(boost::math::non_central_chi_squared_distribution<Real>(delta_, x0_ / t), y / t) /
                   t * 2.0 * (beta_ - 1.0) * y / f;
        }
    }
    ...
    Real CEVRNDCalculator::cdf(Real f, Time t) const
    {
        ...
        if (delta_ < 2.0)
            return 1.0 - boost::math::cdf(boost::math::non_central_chi_squared_distribution<Real>(2.0 - delta_, y / t),
                                          x0_ / t);
        else
            return 1.0 -
                   boost::math::cdf(boost::math::non_central_chi_squared_distribution<Real>(delta_, x0_ / t), y / t);
    }
    ...
    Real CEVRNDCalculator::invcdf(Real q, Time t) const
    {
        if (delta_ < 2.0)
        {
            ...
            try
            {
                ...
                const Real guess = invX(brent.solve(cdfApprox, 1e-8, y0, 0.02 * y0) * t);
        ...
        else
        {
            const Real x = t * boost::math::quantile(
                                   boost::math::non_central_chi_squared_distribution<Real>(delta_, x0_ / t), 1 - q);
            return invX(x);
        }
    }
}
