...
namespace QuantLib
{
    ...
    SquareRootProcessRNDCalculator::SquareRootProcessRNDCalculator(Real v0, Real kappa, Real theta, Real sigma)
    : v0_(v0), kappa_(kappa), theta_(theta), d_(4 * kappa / (sigma * sigma)), df_(d_ * theta)
    {
    }
    ...
    Real SquareRootProcessRNDCalculator::pdf(Real v, Time t) const
    {
        ...
        const boost::math::non_central_chi_squared_distribution<Real> dist(df_, ncp);
    ...
    Real SquareRootProcessRNDCalculator::stationary_pdf(Real v) const
    {
        ...
        return std::pow(beta, alpha) * std::pow(v, alpha - 1) * std::exp(-beta * v - boost::math::lgamma(alpha));
    }
