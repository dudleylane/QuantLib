...
namespace QuantLib
{
    ...
    FdmSimpleProcess1dMesher::FdmSimpleProcess1dMesher(Size size,
    ...
    : Fdm1dMesher(size)
    {
        ...
        for (Size l = 1; l <= tAvgSteps; ++l)
        {
            ...
            const Real qMin =
                std::min({mp, process->x0(), process->evolve(0, process->x0(), t, InverseCumulativeNormal()(eps))});
            const Real qMax =
                std::max({mp, process->x0(), process->evolve(0, process->x0(), t, InverseCumulativeNormal()(1 - eps))});
            ...
            {
                p += dp;
                locations_[i] += process->evolve(0, process->x0(), t, InverseCumulativeNormal()(p));
            }
