...
namespace QuantLib
{
    ...
    namespace
    {
        ...
        {
            interpolated_volatility(const std::vector<Real>& pGrid, const std::vector<Real>& vGrid)
            : variance(pGrid.begin(), pGrid.end(), vGrid.begin())
            {
            }
            Real operator()(Real x) const { return std::sqrt(variance(x, true)); }
            LinearInterpolation variance;
        };
    }

    FdmHestonVarianceMesher::FdmHestonVarianceMesher(Size size,
    ...
    : Fdm1dMesher(size)
    {
        ...
        try
        {
            ...
            for (Size l = 1; l <= tAvgSteps; ++l)
            {
                ...
                const Real ncp = 4 * process->kappa() * std::exp(-process->kappa() * t) /
                                 (squared(mixedSigma) * (1 - std::exp(-process->kappa() * t))) * process->v0();
                const Real k = squared(mixedSigma) * (1 - std::exp(-process->kappa() * t)) / (4 * process->kappa());

                const Real qMin = 0.0; // v_min = 0.0;
                const Real qMax =
                    std::max(process->v0(),
                             k * InverseNonCentralCumulativeChiSquareDistribution(df, ncp, 100, 1e-8)(1 - epsilon));
                ...
                for (Size i = 1; i < size; ++i)
                {
                    ...
                    p += ps;
                    const Real tmp = k * InverseNonCentralCumulativeChiSquareDistribution(df, ncp, 100, 1e-8)(p);
            ...
            QL_REQUIRE(grid.size() == size * tAvgSteps, "something wrong with the grid size");
        ...
        {
            // use default mesh
            const Real vol = mixedSigma * std::sqrt(process->theta() / (2 * process->kappa()));

            const Real mean = process->theta();
            const Real upperBound = std::max(process->v0() + 4 * vol, mean + 4 * vol);
            const Real lowerBound = std::max(0.0, std::min(process->v0() - 4 * vol, mean - 4 * vol));
        ...
        Real skewHint = ((process->kappa() != 0.0) ? Real(std::max(1.0, mixedSigma / process->kappa())) : 1.0);

        std::sort(pGrid.begin(), pGrid.end());
        volaEstimate_ =
            GaussLobattoIntegral(100000, 1e-4)(interpolated_volatility(pGrid, vGrid), pGrid.front(), pGrid.back()) *
            std::pow(skewHint, 1.5);
    ...
    : Fdm1dMesher(size)
    {
        ...
        const FdmHestonVarianceMesher mesher(size, process, maturity, tAvgSteps, epsilon, mixingFactor);
        ...
        {
            typedef boost::accumulators::accumulator_set<Real,
            ...
            for (Size l = 1; l <= tAvgSteps; ++l)
            {
                ...
                const Real leverageAvg =
                    GaussLobattoIntegral(10000, 1e-4)(interpolated_volatility(u, sig), u.front(), u.back()) /
                    (1 - 2 * epsilon);
