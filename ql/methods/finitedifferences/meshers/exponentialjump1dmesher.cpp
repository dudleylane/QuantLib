...
namespace QuantLib
{
    ExponentialJump1dMesher::ExponentialJump1dMesher(Size steps, Real beta, Real jumpIntensity, Real eta, Real eps)
    : Fdm1dMesher(steps), beta_(beta), jumpIntensity_(jumpIntensity), eta_(eta)
    {
    ...
    Real ExponentialJump1dMesher::jumpSizeDensity(Real x, Time t) const
    {
        ...
        const Real gammaValue = std::exp(GammaFunction().logValue(1.0 - jumpIntensity_ / beta_));
        return jumpIntensity_ * gammaValue / norm *
               (incompleteGammaFunction(a, x * std::exp(beta_ * t) * eta_) - incompleteGammaFunction(a, x * eta_)) *
               std::pow(eta_, jumpIntensity_ / beta_) / (beta_ * std::pow(x, a));
    }

    Real ExponentialJump1dMesher::jumpSizeDensity(Real x) const
    {
        const Real a = 1.0 - jumpIntensity_ / beta_;
        const Real gammaValue = std::exp(GammaFunction().logValue(jumpIntensity_ / beta_));
        return std::exp(-x * eta_) * std::pow(x, -a) * std::pow(eta_, 1.0 - a) / gammaValue;
    }
    ...
    Real ExponentialJump1dMesher::jumpSizeDistribution(Real x, Time t) const
    {
        ...
        return GaussLobattoIntegral(1000000, 1e-12)([&](Real _x) { return jumpSizeDensity(_x, t); }, xmin,
                                                    std::max(x, xmin));
    }
    ...
    Real ExponentialJump1dMesher::jumpSizeDistribution(Real x) const
    {
        ...
        const Real lowerEps = (std::pow(xmin, a) / a - std::pow(xmin, a + 1) / (a + 1)) / gammaValue;

        return lowerEps + GaussLobattoIntegral(10000, 1e-12)([&](Real _x) { return jumpSizeDensity(_x); }, xmin / eta_,
                                                             std::max(x, xmin / eta_));
    }
}
