...
namespace QuantLib
{
    ...
    Concentrating1dMesher::Concentrating1dMesher(
        Real start, Real end, Size size, const std::pair<Real, Real>& cPoints, const bool requireCPoint)
    : Fdm1dMesher(size)
    {
        ...
        const Real density = cPoints.second == Null<Real>() ? Null<Real>() : cPoints.second * (end - start);

        QL_REQUIRE(cPoint == Null<Real>() || (cPoint >= start && cPoint <= end),
                   "cPoint must be between start and end");
        QL_REQUIRE(density == Null<Real>() || density > 0.0, "density > 0 required");
        QL_REQUIRE(cPoint == Null<Real>() || density != Null<Real>(), "density must be given if cPoint is given");
        QL_REQUIRE(!requireCPoint || cPoint != Null<Real>(), "cPoint is required in grid but not given");
        ...
        if (cPoint != Null<Real>())
        {
            ...
            if (requireCPoint)
            {
                ...
                if (!close(cPoint, start) && !close(cPoint, end))
                {
                    ...
                    const Real u0 = std::max(std::min(std::lround(z0 * (size - 1)), static_cast<long>(size) - 2), 1L) /
                                    ((Real)(size - 1));
                    u.push_back(u0);
                    z.push_back(z0);
                }
                u.push_back(1.0);
                z.push_back(1.0);
                transform = ext::shared_ptr<Interpolation>(new LinearInterpolation(u.begin(), u.end(), z.begin()));
            }
    ...
    namespace
    {
        ...
        {
          public:
            OdeIntegrationFct(const std::vector<Real>& points, const std::vector<Real>& betas, Real tol)
        ...
        bool equal_on_first(const std::pair<Real, Real>& p1, const std::pair<Real, Real>& p2)
        {
    ...
    Concentrating1dMesher::Concentrating1dMesher(
        Real start, Real end, Size size, const std::vector<std::tuple<Real, Real, bool>>& cPoints, Real tol)
    : Fdm1dMesher(size)
    {
        ...
        const Real a =
            Brent().solve([&](Real x) -> Real { return fct.solve(x, start, 0.0, 1.0) - end; }, tol, aInit, 0.1 * aInit);
        ...
        for (Size i = 0; i < points.size(); ++i)
        {
            if (std::get<2>(cPoints[i]) && points[i] > start && points[i] < end)
            {
                ...
                const Size j = std::distance(y.begin(), std::lower_bound(y.begin(), y.end(), points[i]));

                const Real e = Brent().solve([&](Real x) -> Real { return odeSolution(x, true) - points[i]; },
