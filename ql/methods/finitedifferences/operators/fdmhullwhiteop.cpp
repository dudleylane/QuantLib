...
namespace QuantLib
{
    ...
    FdmHullWhiteOp::FdmHullWhiteOp(const ext::shared_ptr<FdmMesher>& mesher,
                                   const ext::shared_ptr<HullWhite>& model,
                                   Size direction)
    : direction_(direction), x_(mesher->locations(direction)),
      dzMap_(FirstDerivativeOp(direction, mesher)
                 .mult(-x_ * model->a())
                 .add(SecondDerivativeOp(direction, mesher)
                          .mult(0.5 * model->sigma() * model->sigma() * Array(mesher->layout()->size(), 1.0)))),
      mapT_(direction, mesher), model_(model)
    {
    }
    ...
    void FdmHullWhiteOp::setTime(Time t1, Time t2)
    {
        ...
        const ext::shared_ptr<OneFactorModel::ShortRateDynamics> dynamics = model_->dynamics();
