...
namespace QuantLib
{
    ...
    : mesher_(mesher), rTS_(bsProcess->riskFreeRate().currentLink()), qTS_(bsProcess->dividendYield().currentLink()),
      ...
      illegalLocalVolOverwrite_(illegalLocalVolOverwrite), direction_(direction), quantoHelper_(std::move(quantoHelper))
    {
    }
    ...
    void FdmBlackScholesOp::setTime(Time t1, Time t2)
    {
        ...
        if (localVol_ != nullptr)
        {
            ...
            {
                mapT_.axpyb(r - q - 0.5 * v, dxMap_, dxxMap_.mult(0.5 * v), Array(1, -r));
            }
        }
        else
        {
            ...
            if (quantoHelper_ != nullptr)
            {
                mapT_.axpyb(Array(1, r - q - 0.5 * v) - quantoHelper_->quantoAdjustment(Array(1, std::sqrt(v)), t1, t2),
                            dxMap_, dxxMap_.mult(0.5 * Array(mesher_->layout()->size(), v)), Array(1, -r));
            }
            else
            {
