...
namespace QuantLib
{
    ...
    : varianceValues_(0.5 * mesher->locations(1)), dxMap_(FirstDerivativeOp(0, mesher)),
      dxxMap_(SecondDerivativeOp(0, mesher).mult(0.5 * mesher->locations(1))), mapT_(0, mesher), mesher_(mesher),
      rTS_(std::move(rTS)), qTS_(std::move(qTS)), quantoHelper_(std::move(quantoHelper)),
    ...
    void FdmHestonEquityPart::setTime(Time t1, Time t2)
    {
        ...
        if (quantoHelper_ != nullptr)
        {
            mapT_.axpyb(r - q - varianceValues_ * Lsquare -
                            quantoHelper_->quantoAdjustment(volatilityValues_ * L_, t1, t2),
                        dxMap_, dxxMap_.mult(Lsquare), Array(1, -0.5 * r));
        }
        else
        {
            mapT_.axpyb(r - q - varianceValues_ * Lsquare, dxMap_, dxxMap_.mult(Lsquare), Array(1, -0.5 * r));
        }
    }
    ...
    FdmHestonOp::FdmHestonOp(const ext::shared_ptr<FdmMesher>& mesher,
                             const ext::shared_ptr<HestonProcess>& hestonProcess,
                             const ext::shared_ptr<FdmQuantoHelper>& quantoHelper,
                             const ext::shared_ptr<LocalVolTermStructure>& leverageFct,
                             const Real mixingFactor)
    : correlationMap_(SecondOrderMixedDerivativeOp(0, 1, mesher)
                          .mult(hestonProcess->rho() * hestonProcess->sigma() * mixingFactor * mesher->locations(1))),
      dyMap_(mesher,
    ...
    {
        return dyMap_.getMap().apply(u) + dxMap_.getMap().apply(u) + dxMap_.getL() * correlationMap_.apply(u);
    }

    Array FdmHestonOp::apply_direction(Size direction, const Array& r) const
    {
        if (direction == 0)
    ...
    Array FdmHestonOp::solve_splitting(Size direction, const Array& r, Real a) const
    {
    ...
    {
        return {dxMap_.getMap().toMatrix(), dyMap_.getMap().toMatrix(), correlationMap_.toMatrix()};
    }
