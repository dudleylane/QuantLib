...
namespace QuantLib
{
    ...
    FdmCIREquityPart::FdmCIREquityPart(const ext::shared_ptr<FdmMesher>& mesher,
                                       const ext::shared_ptr<GeneralizedBlackScholesProcess>& bsProcess,
                                       Real strike)
    : dxMap_(FirstDerivativeOp(0, mesher)), dxxMap_(SecondDerivativeOp(0, mesher)), mapT_(0, mesher), mesher_(mesher),
      qTS_(bsProcess->dividendYield().currentLink()), strike_(strike),
    ...
    void FdmCIREquityPart::setTime(Time t1, Time t2)
    {
        ...
        mapT_.axpyb(mesher_->locations(1) - q - 0.5 * v, dxMap_, dxxMap_.mult(Array(mesher_->layout()->size(), v / 2)),
    ...
    FdmCIRRatesPart::FdmCIRRatesPart(const ext::shared_ptr<FdmMesher>& mesher, Real sigma, Real kappa, Real theta)
    ...
    FdmCIRMixedPart::FdmCIRMixedPart(const ext::shared_ptr<FdmMesher>& mesher,
    ...
    : dyMap_(SecondOrderMixedDerivativeOp(0, 1, mesher)
      ...
      mapT_(0, 1, mesher), mesher_(mesher), sigma1_(bsProcess->blackVolatility().currentLink()), strike_(strike)
    {
    }
    ...
    FdmCIROp::FdmCIROp(const ext::shared_ptr<FdmMesher>& mesher,
                       const ext::shared_ptr<CoxIngersollRossProcess>& cirProcess,
                       const ext::shared_ptr<GeneralizedBlackScholesProcess>& bsProcess,
                       const Real rho,
                       const Real strike)
    : dxMap_(mesher, bsProcess, strike),
      dyMap_(mesher, cirProcess->volatility(), cirProcess->speed(), cirProcess->level()),
      dzMap_(mesher, cirProcess, bsProcess, rho, strike)
    {
    }
    ...
    Array FdmCIROp::apply_direction(Size direction, const Array& r) const
    {
        if (direction == 0)
    ...
    Array FdmCIROp::solve_splitting(Size direction, const Array& r, Real a) const
    {
        if (direction == 0)
        {
    ...
    {
        return {dxMap_.getMap().toMatrix(), dyMap_.getMap().toMatrix(), dzMap_.getMap().toMatrix()};
    }
