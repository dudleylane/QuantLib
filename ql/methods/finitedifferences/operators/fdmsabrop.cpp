...
namespace QuantLib
{
    FdmSabrOp::FdmSabrOp(const ext::shared_ptr<FdmMesher>& mesher,
                         ...
                         Real nu,
                         Real rho)
    : rTS_(std::move(rTS)), dffMap_(SecondDerivativeOp(0, mesher).mult(0.5 * Exp(2.0 * mesher->locations(1)) *
    ...
    Array FdmSabrOp::apply_direction(Size direction, const Array& r) const
    {
        if (direction == 0)
    ...
    Array FdmSabrOp::solve_splitting(Size direction, const Array& r, Real a) const
    {
    ...
    Array FdmSabrOp::preconditioner(const Array& r, Real dt) const
    {
    ...
    {
        return {mapA_.toMatrix(), mapF_.toMatrix(), correlationMap_.toMatrix()};
    }
