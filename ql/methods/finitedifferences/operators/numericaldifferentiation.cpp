...
namespace QuantLib
{
    ...
    namespace
    {
        Array calcOffsets(Real h, Size n, NumericalDifferentiation::Scheme scheme)
        {
            ...
            switch (scheme)
            {
                case NumericalDifferentiation::Central:
                    QL_REQUIRE(n > 2 && (n % 2), "number of steps must be an odd number greater than two");
        ...
        Array calcWeights(const Array& x, Size M)
        {
            ...
            for (Size n = 1; n < N; ++n)
            {
                ...
                for (Size nu = 0; nu < n; ++nu)
                {
                    ...
                    {
                        d[m][n][nu] = (x[n] * d[m][n - 1][nu] - ((m > 0) ? Real(m * d[m - 1][n - 1][nu]) : 0.0)) / c3;
                    }
                }
                ...
                {
                    d[m][n][n] =
                        c1 / c2 * (((m > 0) ? Real(m * d[m - 1][n - 1][n - 1]) : 0.0) - x[n - 1] * d[m][n - 1][n - 1]);
                }
                c1 = c2;
            }
    ...
    NumericalDifferentiation::NumericalDifferentiation(std::function<Real(Real)> f,
                                                       ...
                                                       Array x_offsets)
    : offsets_(std::move(x_offsets)), w_(calcWeights(offsets_, orderOfDerivative)), f_(std::move(f))
    {
    }
    ...
    : offsets_(calcOffsets(stepSize, steps, scheme)), w_(calcWeights(offsets_, orderOfDerivative)), f_(std::move(f))
    {
    }
}
