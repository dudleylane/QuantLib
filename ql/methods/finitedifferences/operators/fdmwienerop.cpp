...
namespace QuantLib
{
    ...
    FdmWienerOp::FdmWienerOp(const ext::shared_ptr<FdmMesher>& mesher,
    ...
    : rTS_(std::move(rTS))
    {
        ...
        for (Size i = 0; i < lambdas.size(); ++i)
            ops_.emplace_back(ext::make_shared<TripleBandLinearOp>(
                SecondDerivativeOp(i, mesher).mult(Array(mesher->layout()->size(), 0.5 * lambdas[i]))));
    }
    ...
    Array FdmWienerOp::solve_splitting(Size direction, const Array& x, Real s) const
    {
