...
namespace QuantLib
{
    ...
    Fdm2dBlackScholesOp::Fdm2dBlackScholesOp(const ext::shared_ptr<FdmMesher>& mesher,
    ...
    : mesher_(mesher), p1_(p1), p2_(p2),

      localVol1_((localVol) ? p1->localVolatility().currentLink() : ext::shared_ptr<LocalVolTermStructure>()),
      localVol2_((localVol) ? p2->localVolatility().currentLink() : ext::shared_ptr<LocalVolTermStructure>()),
      ...
      corrMapTemplate_(SecondOrderMixedDerivativeOp(0, 1, mesher).mult(Array(mesher->layout()->size(), correlation))),
    ...
    void Fdm2dBlackScholesOp::setTime(Time t1, Time t2)
    {
        ...
        currentForwardRate_ = p1_->riskFreeRate()->forwardRate(t1, t2, Continuous).rate();
    }
    ...
    Array Fdm2dBlackScholesOp::apply_direction(Size direction, const Array& x) const
    {
        if (direction == 0)
        {
            return opX_.apply(x);
    ...
    Array Fdm2dBlackScholesOp::solve_splitting(Size direction, const Array& x, Real s) const
    {
        if (direction == 0)
        {
    ...
    Array Fdm2dBlackScholesOp::preconditioner(const Array& r, Real dt) const
    {
    ...
    {
        return {opX_.toMatrix(), opY_.toMatrix(),
                corrMapT_.toMatrix() +
