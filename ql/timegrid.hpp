    ...
    class TimeGrid
    {
      public:
        ...
        */
        ...
        TimeGrid(Iterator begin, Iterator end) : mandatoryTimes_(begin, end)
        {
        ...
        */
        template <class Iterator>
        TimeGrid(Iterator begin, Iterator end, Size steps) : mandatoryTimes_(begin, end)
        {
            QL_REQUIRE(begin != end, "empty time sequence");
            std::sort(mandatoryTimes_.begin(), mandatoryTimes_.end());
            // We seem to assume that the grid begins at 0.
            ...
            for (auto t = mandatoryTimes_.begin(); t < mandatoryTimes_.end(); ++t)
            {
                Time periodEnd = *t;
                if (periodEnd != 0.0)
                {
        ...
        TimeGrid(std::initializer_list<Time> times) : TimeGrid(times.begin(), times.end()) {}
        TimeGrid(std::initializer_list<Time> times, Size steps) : TimeGrid(times.begin(), times.end(), steps) {}
        //@}
        ...
        Time closestTime(Time t) const { return times_[closestIndex(t)]; }
