...
namespace QuantLib
{
    ...
    : GeneralizedBlackScholesProcess(x0, dividendTS, riskFreeTS, blackVolTS, d), discretization_(evolDisc)
    {
    }
    ...
    Real ExtendedBlackScholesMertonProcess::drift(Time t, Real x) const
    {
        ...
        return riskFreeRate()->forwardRate(t, t1, Continuous, NoFrequency, true).rate() -
               dividendYield()->forwardRate(t, t1, Continuous, NoFrequency, true).rate() - 0.5 * sigma * sigma;
    }
    ...
    Real ExtendedBlackScholesMertonProcess::evolve(Time t0, Real x0, Time dt, Real dw) const
    {
        ...
        switch (discretization_)
        {
            case Milstein:
                // Milstein scheme
                return apply(x0, drift(t0, x0) * dt + 0.5 * std::pow(diffusion(t0, x0), 2) * (dw * dw - 1) * dt +
            ...
            case PredictorCorrector:
                ...
                predictor = apply(expectation(t0, x0, dt), stdDeviation(t0, x0, dt) * dw);
                t1 = t0 + 0.0001;
                sigma0 = diffusion(t0, x0);
                sigma1 = diffusion(t0 + dt, predictor);
                rate0 = riskFreeRate()->forwardRate(t0, t1, Continuous, NoFrequency, true).rate() -
                        dividendYield()->forwardRate(t0, t1, Continuous, NoFrequency, true).rate() -
                        0.5 * std::pow(sigma0, 2);
                rate1 = riskFreeRate()->forwardRate(t0 + dt, t1 + dt, Continuous, NoFrequency, true).rate() -
                        dividendYield()->forwardRate(t0 + dt, t1 + dt, Continuous, NoFrequency, true).rate() -
                        0.5 * std::pow(sigma1, 2);
                driftterm = 0.5 * rate1 + 0.5 * rate0;
                diffusionterm = 0.5 * (sigma1 + sigma0);
                corrector = apply(x0, driftterm * dt + diffusionterm * std::sqrt(dt) * dw);
                return corrector;
            default:
