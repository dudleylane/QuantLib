...
namespace QuantLib
{
    ...
    ExtOUWithJumpsProcess::ExtOUWithJumpsProcess(
        ext::shared_ptr<ExtendedOrnsteinUhlenbeckProcess> process, Real Y0, Real beta, Real jumpIntensity, Real eta)
    : Y0_(Y0), beta_(beta), jumpIntensity_(jumpIntensity), eta_(eta), ouProcess_(std::move(process))
    {
    ...
    ext::shared_ptr<ExtendedOrnsteinUhlenbeckProcess> ExtOUWithJumpsProcess::getExtendedOrnsteinUhlenbeckProcess() const
    {
        return ouProcess_;
    }
    ...
    {
        return {ouProcess_->x0(), Y0_};
    }

    Array ExtOUWithJumpsProcess::drift(Time t, const Array& x) const
    {
        return {ouProcess_->drift(t, x[0]), -beta_ * x[1]};
    }
    ...
    Array ExtOUWithJumpsProcess::evolve(Time t0, const Array& x0, Time dt, const Array& dw) const
    {
        ...
        const Real u1 = std::max(QL_EPSILON, std::min(cumNormalDist_(dw[1]), 1.0 - QL_EPSILON));

        const Time interarrival = -1.0 / jumpIntensity_ * std::log(u1);
        if (interarrival < dt)
        {
            const Real u2 = std::max(QL_EPSILON, std::min(cumNormalDist_(dw[2]), 1.0 - QL_EPSILON));
