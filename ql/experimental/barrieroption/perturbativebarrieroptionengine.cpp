...
namespace QuantLib
{
    ...
    namespace
    {
            ...
            */
            ...
            for (i = 1; i <= npoint; i++)
            {
                ...
                /*
                  ...
                  !!
                */
                ccaux = v(p, tt, x, xstar, gm) + v(p, tt, x, -xstar, gm) - v(p, tt, -x, xstar, gm) -
                        v(p, tt, -x, -xstar, gm);
        ...
        */
        Real derivn3(Real limit[4], Real sigmarho[4], int idx)
        {
            ...
            if (idx == 1)
            {
                ...
                rho = (sigmarho[3] - sigmarho[1] * sigmarho[2]) /
                      std::pow((1.0 - sigmarho[1] * sigmarho[1]) * (1.0 - sigmarho[2] * sigmarho[2]), 0.5);
        ...
        Real TVTMFN(Real X, Real H1, Real H2, Real H3, Real R23, Real RUA, Real RUB, Real AR, Real RUC, int NUC);
        Real ADONET(Real ZRO,
                    Real ONE,
                    Real EPS,
                    Real (*TVTMFN)(
                        Real X, Real H1, Real H2, Real H3, Real R23, Real RUA, Real RUB, Real AR, Real RUC, int NUC));
        ...
        Real PNTGND(int, Real, Real, Real, Real, Real, Real, Real);
        ...
        Real KRNRDT(Real, Real, Real (*TVTMFN)(Real, Real, Real, Real, Real, Real, Real, Real, Real, int), Real&);
            ...
            Real& ERR)
        {
            ...
            */
            ...
            {
                T = WID * XGK[J + 1];
                FC = TVTMFN(CEN - T, H1, H2, H3, R23, RUA, RUB, AR, RUC, NUC) +
                     TVTMFN(CEN + T, H1, H2, H3, R23, RUA, RUB, AR, RUC, NUC);
        ...
        Real STUDNT(int NU, Real T)
        {
            ...
            */
            ...
            if (NU < 1)
                ...
                result = (1 + T / std::pow((2.0 + T * T), 0.5)) / 2.0;
            else
            {
                TT = T * T;
        ...
        Real ND2(Real a, Real b, Real rho)
        {
            ...
            else
            {
                ...
                if (fabs(R) < 1)
                {
                    ...
                    ASR = -(BS / AS + HK) / 2;
                    if (ASR > -100)
                        BVN = AA * exp(ASR) * (1 - C * (BS - AS) * (1 - D * BS / 5) / 3 + C * D * AS * AS / 5);
                    if (-HK < 100)
                    {
                        BB = std::pow(BS, 0.5);
                        BVN = BVN - exp(-HK / 2) * std::pow(TWOPI, 0.5) * PHID(-BB / AA) * BB *
                                        (1 - C * BS * (1 - D * BS / 5) / 3);
                    }
                    AA = AA / 2;
                    for (I = 1; I <= LG; I++)
                    {
                        for (IS = -1; IS <= 1; IS = IS + 2)
                        {
                            ...
                            if (ASR > -100)
                            {
                                ...
                                BVN =
                                    BVN + AA * WL[I][NG] * exp(ASR) *
                                              (exp(-HK * (1 - RS) / (2 * (1 + RS))) / RS - (1 + C * XS * (1 + D * XS)));
                            }
                        }
                    }
                    BVN = -BVN / TWOPI;
        ...
        struct integr_adapter
        {
            ...
            Real operator()(Real t1, Real t2) const { return r->forwardRate(t1, t2, Continuous) * (t2 - t1); }
        };
        ...
        struct integalpha_adapter
        {
            ...
            explicit integalpha_adapter(const ext::shared_ptr<GeneralizedBlackScholesProcess>& process)
            : r(*(process->riskFreeRate())), q(*(process->dividendYield()))
            {
            }
            Real operator()(Real t1, Real t2) const
            {
                Real alpha = r->forwardRate(t1, t2, Continuous).rate() - q->forwardRate(t1, t2, Continuous).rate();
                return alpha * (t2 - t1);
            }
        };
        ...
        struct alpha_adapter
        {
            ...
            {
                return r->forwardRate(t, t, Continuous).rate() - q->forwardRate(t, t, Continuous).rate();
            }
        };
        ...
        struct integs_adapter
        {
            ...
            Real operator()(Real t1, Real t2) const { return v->blackForwardVariance(t1, t2, s, true); }
        };
    ...
    void PerturbativeBarrierOptionEngine::calculate() const
    {
        ...
        QL_REQUIRE(arguments_.barrierType == Barrier::UpOut, "this engine only manages up-and-out options");

        QL_REQUIRE(arguments_.rebate == 0.0, "this engine does not manage non-null rebates");

        ext::shared_ptr<PlainVanillaPayoff> payoff = ext::dynamic_pointer_cast<PlainVanillaPayoff>(arguments_.payoff);
        QL_REQUIRE(payoff && payoff->optionType() == Option::Put, "this engine only manages put options");
        ...
        results_.value = BarrierUPD(kprice, stock, hbarr, tauMin, tauMax, order_, igm, integr_adapter(process_),
                                    integalpha_adapter(process_), integs_adapter(process_), alpha_adapter(process_),
