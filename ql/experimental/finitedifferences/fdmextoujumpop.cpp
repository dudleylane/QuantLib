...
namespace QuantLib
{
    ...
    FdmExtOUJumpOp::FdmExtOUJumpOp(const ext::shared_ptr<FdmMesher>& mesher,
                                   const ext::shared_ptr<ExtOUWithJumpsProcess>& process,
                                   const ext::shared_ptr<YieldTermStructure>& rTS,
                                   const FdmBoundaryConditionSet& bcSet,
                                   Size integroIntegrationOrder)
    : mesher_(mesher), process_(process), rTS_(rTS), bcSet_(bcSet), gaussLaguerreIntegration_(integroIntegrationOrder),
      x_(mesher->locations(0)),
      ouOp_(new FdmExtendedOrnsteinUhlenbeckOp(mesher, process->getExtendedOrnsteinUhlenbeckProcess(), rTS, bcSet)),
      dyMap_(FirstDerivativeOp(1, mesher).mult(-process->beta() * mesher->locations(1)))
    {
        ...
        integroPart_ = SparseMatrix(mesher_->layout()->size(), mesher_->layout()->size());
        ...
        for (const auto& iter : *mesher_->layout())
        {
            ...
            for (Size i = 0; i < yInt.size(); ++i)
            {
                ...
                const Integer l = (ys > yLoc.back()) ?
                                      yLoc.size() - 2 :
                                      std::upper_bound(yLoc.begin(), yLoc.end() - 1, ys) - yLoc.begin() - 1;

                const Real s = (ys - yLoc[l]) / (yLoc[l + 1] - yLoc[l]);
                integroPart_(diag, mesher_->layout()->neighbourhood(iter, 1, l - yIndex)) += weight * lambda * (1 - s);
                integroPart_(diag, mesher_->layout()->neighbourhood(iter, 1, l + 1 - yIndex)) += weight * lambda * s;
            }
        }
    }
    ...
    Array FdmExtOUJumpOp::apply_direction(Size direction, const Array& r) const
    {
        if (direction == 0)
    ...
    Array FdmExtOUJumpOp::solve_splitting(Size direction, const Array& r, Real a) const
    {
        if (direction == 0)
        {
