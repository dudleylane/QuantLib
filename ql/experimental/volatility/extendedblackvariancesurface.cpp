...
namespace QuantLib
{
    ...
    ExtendedBlackVarianceSurface::ExtendedBlackVarianceSurface(const Date& referenceDate,
    ...
    : BlackVarianceTermStructure(referenceDate, calendar), dayCounter_(std::move(dayCounter)), maxDate_(dates.back()),
      volatilities_(volatilities), strikes_(std::move(strikes)), lowerExtrapolation_(lowerEx),
      upperExtrapolation_(upperEx)
    {
        ...
        QL_REQUIRE(dates[0] > referenceDate, "cannot have dates_[0] <= referenceDate_");
    ...
    void ExtendedBlackVarianceSurface::setVariances()
    {
        ...
        for (Size j = 1; j <= times_.size(); j++)
        {
            for (Size i = 0; i < strikes_.size(); i++)
            {
                ...
                QL_REQUIRE(variances_[i][j] >= variances_[i][j - 1], "variance must be non-decreasing");
            }
        }
    }
    ...
    Real ExtendedBlackVarianceSurface::blackVarianceImpl(Time t, Real strike) const
    {
