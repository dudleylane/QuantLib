...
namespace QuantLib
{
    ...
    SviSmileSection::SviSmileSection(const Date& d, Rate forward, std::vector<Real> sviParams, const DayCounter& dc)
    ...
    void SviSmileSection::init()
    {
        ...
        QL_REQUIRE(params_.size() == 5,
                   "svi expects 5 parameters (a,b,sigma,rho,m) but (" << params_.size() << ") given");
        detail::checkSviParameters(params_[0], params_[1], params_[2], params_[3], params_[4], exerciseTime());
    }
    ...
    Volatility SviSmileSection::volatilityImpl(Rate strike) const
    {
        ...
        Real totalVariance = detail::sviTotalVariance(params_[0], params_[1], params_[2], params_[3], params_[4], k);
