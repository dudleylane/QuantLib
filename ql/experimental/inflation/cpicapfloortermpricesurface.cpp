...
namespace QuantLib
{
    ...
    CPICapFloorTermPriceSurface::CPICapFloorTermPriceSurface(
        ...
        const Matrix& cPrice,
        const Matrix& fPrice)
    : TermStructure(0, cal, dc), zii_(std::move(zii)), interpolationType_(interpolationType),
      nominalTS_(std::move(yts)), cStrikes_(cStrikes), fStrikes_(fStrikes), cfMaturities_(cfMaturities),
      cPrice_(cPrice), fPrice_(fPrice), nominal_(nominal), bdc_(bdc), observationLag_(observationLag),
      baseRate_(baseRate)
    {
            ...
            for (Size i = 0; i < cPrice_.rows(); i++)
            {
                ...
                if (i > 0)
                {
                    QL_REQUIRE(cPrice_[i][j] <= cPrice_[i - 1][j],
                               "non-decreasing cap prices: " << cPrice_[i][j] << " then " << cPrice_[i - 1][j]);
                }
            }
        }
    ...
    Rate CPICapFloorTermPriceSurface::atmRate(Date maturity) const
    {
        ...
        Time T = inflationYearFraction(zii_->frequency(), detail::CPI::isInterpolated(interpolationType_), dayCounter(),
