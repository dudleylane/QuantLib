...
namespace QuantLib
{
    ...
    class GaussianLHPLossModel : public DefaultLossModel, public LatentModel<GaussianCopulaPolicy>
    {
      ...
      public:
        ...
        { // not an overload of Deflossmodel ???<<<<<???
        ...
        */
        Real averageRecovery(const Date& d) const // no explicit time dependence in this model
        {
            ...
            Real denominator = std::inner_product(notionals.begin(), notionals.end(), probs.begin(), Real(0.));
            if (denominator == 0.)
                return 0.;

            std::transform(notionals.begin(), notionals.end(), probs.begin(), notionals.begin(), std::multiplies<>());

            return std::inner_product(recoveries.begin(), recoveries.end(), notionals.begin(), Real(0.)) / denominator;
        }
