...
namespace QuantLib
{
    ...
    Real OneFactorCopula::conditionalProbability(Real p, Real m) const
    {
        ...
        Real res = cumulativeZ((inverseCumulativeY(p) - sqrt(c) * m) / sqrt(1. - c));
    ...
    //-------------------------------------------------------------------------
    vector<Real> OneFactorCopula::conditionalProbability(const vector<Real>& prob, Real m) const
    {
        //-------------------------------------------------------------------------
        calculate();
    ...
    Real OneFactorCopula::cumulativeY(Real y) const
    {
        ...
        {
            if (y_[i] > y)
                return ((y_[i] - y) * cumulativeY_[i - 1] + (y - y_[i - 1]) * cumulativeY_[i]) / (y_[i] - y_[i - 1]);
        }
