...
namespace QuantLib
{
    ...
    class FarlieGumbelMorgensternCopulaRng
    {
      public:
        ...
        explicit FarlieGumbelMorgensternCopulaRng(const RNG& uniformGenerator, Real theta);
    ...
    template <class RNG>
    FarlieGumbelMorgensternCopulaRng<RNG>::FarlieGumbelMorgensternCopulaRng(const RNG& ug, Real th)
    : uniformGenerator_(ug), theta_(th)
    {
        QL_REQUIRE(th >= -1.0 && th <= 1.00, "theta (" << th << ") must be in [-1,1]");
    }
