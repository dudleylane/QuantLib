...
namespace QuantLib
{
    ...
    {
        QL_REQUIRE(degreesFreedom.size() == factors.size(), "Incompatible sizes in convolution.");
    ...
    std::vector<Real> CumulativeBehrensFisher::polynCharactT(Natural n) const
    {
        ...
        for (Size k = 1; k < n; k++)
        {
            ...
            std::vector<Real> lowUp = convolveVectorPolynomials(recursionFactor, low);
            // add them up:
    ...
    std::vector<Real> CumulativeBehrensFisher::convolveVectorPolynomials(const std::vector<Real>& v1,
                                                                         const std::vector<Real>& v2) const
    {
#if defined(QL_EXTRA_SAFETY_CHECKS)
        QL_REQUIRE(!v1.empty() && !v2.empty(), "Incorrect vectors in polynomial.");
#endif
    ...
    Probability CumulativeBehrensFisher::operator()(const Real x) const
    {
        ...
        {
            integral -= polyConvolved_[exponent] * Factorial::get(exponent - 1) * std::sin((exponent)*atan2xa) /
                        std::pow(rootsqr, static_cast<Real>(exponent));
        }
        return .5 + integral / M_PI;
    }

    Probability CumulativeBehrensFisher::density(const Real x) const
    {
        Real squared = a2_ + x * x;
        ...
        {
            integral += polyConvolved_[exponent] * Factorial::get(exponent) * std::cos((exponent + 1) * atan2xa) /
    ...
    InverseCumulativeBehrensFisher::InverseCumulativeBehrensFisher(const std::vector<Integer>& degreesFreedom,
                                                                   const std::vector<Real>& factors,
                                                                   Real accuracy)
    : normSqr_(std::inner_product(factors.begin(), factors.end(), factors.begin(), Real(0.))), accuracy_(accuracy),
      distrib_(degreesFreedom, factors)
    {
    }
    ...
    Real InverseCumulativeBehrensFisher::operator()(const Probability q) const
    {
        ...
        Real xMin = InverseCumulativeNormal::standard_value(effectiveq) * normSqr_;
        ...
        Real xMax = 1.e6;
        return sign * Brent().solve([&](Real x) -> Real { return distrib_(x) - effectiveq; }, accuracy_,
                                    (xMin + xMax) / 2., xMin, xMax);
    }
