    ...
    */
    class GaussianQuadMultidimIntegrator
    {
      private:
        ...
        {
          public:
            explicit VectorIntegrator(Size n, Real mu = 0.0) : GaussHermiteIntegration(n, mu) {}
            ...
            std::vector<Real> operator()(const F& f) const
            {
                ...
                std::for_each(term.begin(), term.end(), [&](Real x) -> Real { return x * w_[i]; });
                ...
                for (i--; i >= 0; --i)
                {
                    ...
                    // sum[j] += term[j] * w_[i];
                    std::transform(term.begin(), term.end(), sum.begin(), sum.begin(),
      ...
      private:
        ...
        */
        ...
        {
            integrationEntries_[levelSpawn - 1] = [&](const std::function<Real(const std::vector<Real>&)>& f, Real x)
        ...
        Real scalarIntegrator(const std::function<Real(const std::vector<Real>& arg1)>& f, const Real mFctr) const
        {
        ...
        template <int intgDepth>
        std::vector<Real> vectorIntegratorVR(const std::function<std::vector<Real>(const std::vector<Real>& arg1)>& f,
        ...
        mutable std::vector<std::function<Real(std::function<Real(const std::vector<Real>& varg2)> f1, const Real r3)>>
            integrationEntries_;
        mutable std::vector<std::function<std::vector<Real>(
            const std::function<std::vector<Real>(const std::vector<Real>& vvarg2)>& vf1, const Real vr3)>>
    ...
    template <>
    inline Real
    GaussianQuadMultidimIntegrator::operator()(const std::function<Real(const std::vector<Real>& v1)>& f) const
    {
    ...
    template <>
    inline Real
    GaussianQuadMultidimIntegrator::integrate<Real>(const std::function<Real(const std::vector<Real>& v1)>& f) const
    {
    ...
    template <>
    inline Real
    GaussianQuadMultidimIntegrator::scalarIntegrator<1>(const std::function<Real(const std::vector<Real>& arg1)>& f,
                                                        const Real mFctr) const
    {
        varBuffer_[0] = mFctr;
        return f(varBuffer_);
    ...
    template <>
    inline std::vector<Real> GaussianQuadMultidimIntegrator::vectorIntegratorVR<1>(
        const std::function<std::vector<Real>(const std::vector<Real>& arg1)>& f, const Real mFctr) const
    {
        varBuffer_[0] = mFctr;
        return f(varBuffer_);
    }
    ...
    {
        integrationEntries_[0] = [&](const std::function<Real(const std::vector<Real>&)>& f, Real x)
        { return scalarIntegrator<1>(f, x); };
        integrationEntriesVR_[0] = [&](const std::function<std::vector<Real>(const std::vector<Real>&)>& f, Real x)
