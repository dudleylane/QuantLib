...
namespace QuantLib
{
    ...
    ExtendedJarrowRudd::ExtendedJarrowRudd(const ext::shared_ptr<StochasticProcess1D>& process,
                                           Time end,
                                           Size steps,
                                           Real)
    : ExtendedEqualProbabilitiesBinomialTree<ExtendedJarrowRudd>(process, end, steps)
    {
        // drift removed
    ...
    ExtendedCoxRossRubinstein::ExtendedCoxRossRubinstein(const ext::shared_ptr<StochasticProcess1D>& process,
                                                         Time end,
                                                         Size steps,
                                                         Real)
    : ExtendedEqualJumpsBinomialTree<ExtendedCoxRossRubinstein>(process, end, steps)
    {
    ...
    ExtendedAdditiveEQPBinomialTree::ExtendedAdditiveEQPBinomialTree(
        const ext::shared_ptr<StochasticProcess1D>& process, Time end, Size steps, Real)
    : ExtendedEqualProbabilitiesBinomialTree<ExtendedAdditiveEQPBinomialTree>(process, end, steps)
    {

        up_ = -0.5 * this->driftStep(0.0) + 0.5 * std::sqrt(4.0 * process->variance(0.0, x0_, dt_) -
                                                            3.0 * this->driftStep(0.0) * this->driftStep(0.0));
    }

    Real ExtendedAdditiveEQPBinomialTree::upStep(Time stepTime) const
    {
        return (-0.5 * this->driftStep(stepTime) +
                0.5 * std::sqrt(4.0 * this->treeProcess_->variance(stepTime, x0_, dt_) -
                                3.0 * this->driftStep(stepTime) * this->driftStep(stepTime)));
    }


    ExtendedTrigeorgis::ExtendedTrigeorgis(const ext::shared_ptr<StochasticProcess1D>& process,
                                           Time end,
                                           Size steps,
                                           Real)
    : ExtendedEqualJumpsBinomialTree<ExtendedTrigeorgis>(process, end, steps)
    {
        ...
        dx_ = std::sqrt(process->variance(0.0, x0_, dt_) + this->driftStep(0.0) * this->driftStep(0.0));
    ...
    ExtendedTian::ExtendedTian(const ext::shared_ptr<StochasticProcess1D>& process, Time end, Size steps, Real)
    ...
    Real ExtendedTian::underlying(Size i, Size index) const
    {
        ...
        return x0_ * std::pow(down, Real(BigInteger(i) - BigInteger(index))) * std::pow(up, Real(index));
    }
    ...
    ExtendedLeisenReimer::ExtendedLeisenReimer(const ext::shared_ptr<StochasticProcess1D>& process,
                                               ...
                                               Size steps,
                                               Real strike)
    : ExtendedBinomialTree<ExtendedLeisenReimer>(process, end, ((steps % 2) != 0U ? steps : steps + 1)), end_(end),
      oddSteps_((steps % 2) != 0U ? steps : steps + 1), strike_(strike)
    {
        ...
        Real d2 = (std::log(x0_ / strike) + this->driftStep(0.0) * oddSteps_) / std::sqrt(variance);

        pu_ = PeizerPrattMethod2Inversion(d2, oddSteps_);
        pd_ = 1.0 - pu_;
        Real pdash = PeizerPrattMethod2Inversion(d2 + std::sqrt(variance), oddSteps_);
    ...
    Real ExtendedLeisenReimer::underlying(Size i, Size index) const
    {
        ...
        Real d2 = (std::log(x0_ / strike_) + this->driftStep(stepTime) * oddSteps_) / std::sqrt(variance);
    ...
    Real ExtendedJoshi4::computeUpProb(Real k, Real dj) const
    {
        ...
        Real gamma = (5.0 / 6.0) * alpha5 + (13.0 / 12.0) * alpha3 + (25.0 / 128.0) * alpha;
        Real delta = -0.1025 * alpha - 0.9285 * alpha3 - 1.43 * alpha5 - 0.5 * alpha7;
        Real p = 0.5;
    ...
    ExtendedJoshi4::ExtendedJoshi4(const ext::shared_ptr<StochasticProcess1D>& process,
                                   ...
                                   Size steps,
                                   Real strike)
    : ExtendedBinomialTree<ExtendedJoshi4>(process, end, ((steps % 2) != 0U ? steps : steps + 1)), end_(end),
      oddSteps_((steps % 2) != 0U ? steps : steps + 1), strike_(strike)
    {
