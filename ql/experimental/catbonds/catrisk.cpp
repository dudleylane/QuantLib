...
namespace QuantLib
{
    ...
    EventSetSimulation::EventSetSimulation(ext::shared_ptr<std::vector<std::pair<Date, Real>>> events,
                                           ...
                                           Date start,
                                           Date end)
    : CatSimulation(start, end), events_(std::move(events)), eventsStart_(eventsStart), eventsEnd_(eventsEnd)
    {
        years_ = end_.year() - start_.year();
        if (eventsStart_.month() < start_.month() ||
            (eventsStart_.month() == start_.month() && eventsStart_.dayOfMonth() <= start_.dayOfMonth()))
        {
    ...
    bool EventSetSimulation::nextPath(std::vector<std::pair<Date, Real>>& path)
    {
        ...
        {
            std::pair<Date, Real> e(events_->at(i_).first + (start_.year() - periodStart_.year()) * Years,
                                    events_->at(i_).second);
            path.push_back(e);
            ++i_; // i points to the first element after the start of the relevant period.
        }
    ...
    EventSet::EventSet(ext::shared_ptr<std::vector<std::pair<Date, Real>>> events, Date eventsStart, Date eventsEnd)
    ...
    : CatSimulation(start, end), maxLoss_(maxLoss), exponential_(lambda), gammaAlpha_(alpha), gammaBeta_(beta)
    {
    ...
    bool BetaRiskSimulation::nextPath(std::vector<std::pair<Date, Real>>& path)
    {
        ...
        {
            auto days = static_cast<Integer>(std::lround(eventFraction * dayCount_ / yearFraction_));
    ...
    BetaRisk::BetaRisk(Real maxLoss, Real years, Real mean, Real stdDev) : maxLoss_(maxLoss), lambda_(1.0 / years)
    {
        QL_REQUIRE(mean < maxLoss,
        ...
        QL_REQUIRE(normalizedVar < normalizedMean * (1.0 - normalizedMean),
