...
namespace QuantLib
{
    ...
    namespace detail
    {
        ...
        {
          public:
            typedef boost::signals2::signal_type<void(),
                                                 boost::signals2::keywords::mutex_type<std::recursive_mutex>>::type
                signal_type;

            void connect(const signal_type::slot_type& slot) { sig_.connect(slot); }
        ...
        class ProxyUpdater
        {
          ...
          public:
            explicit ProxyUpdater(const ext::shared_ptr<T>& observerProxy) : proxy_(observerProxy.get()) {}

            void operator()() const { proxy_->update(); }

            bool operator==(const ProxyUpdater<T>& other) const { return proxy_ == other.proxy_; }
...
#    endif
    ...
    void Observable::unregisterObserver(const ext::shared_ptr<Observer::Proxy>& observerProxy, bool disconnect)
    {
        {
            std::lock_guard<std::recursive_mutex> lock(mutex_);
    ...
    Observable::Observable() : sig_(new detail::Signal()) {}

    Observable::Observable(const Observable&) : sig_(new detail::Signal())
    {
