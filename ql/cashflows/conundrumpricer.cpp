...
namespace QuantLib
{
    ...
    : forwardValue_(forwardValue), expiryDate_(expiryDate), swapTenor_(swapTenor),
      volatilityStructure_(volatilityStructure), smile_(volatilityStructure_->smileSection(expiryDate_, swapTenor_))
    {
    ...
    Real MarketQuotedOptionPricer::operator()(Real strike, Option::Type optionType, Real deflator) const
    {
        const Real variance = smile_->variance(strike);
        if (volatilityStructure_->volatilityType() == ShiftedLognormal)
        {
            return deflator * blackFormula(optionType, strike, forwardValue_, std::sqrt(variance));
        }
        else
        {
            return deflator * bachelierBlackFormula(optionType, strike, forwardValue_, std::sqrt(variance));
        }
    }
    ...
    : CmsCouponPricer(swaptionVol), modelOfYieldCurve_(modelOfYieldCurve), meanReversion_(std::move(meanReversion))
    {
    ...
    void HaganPricer::initialize(const FloatingRateCoupon& coupon)
    {
        ...
        rateCurve_ = swapIndex->discountingTermStructure().empty() ? *(swapIndex->forwardingTermStructure()) :
        ...
        if (fixingDate_ > today)
        {
            ...
            Time startTime = dc.yearFraction(rateCurve_->referenceDate(), swap->startDate());
            Time swapFirstPaymentTime = dc.yearFraction(rateCurve_->referenceDate(), schedule.date(1));
            Time paymentTime = dc.yearFraction(rateCurve_->referenceDate(), paymentDate_);
            ...
            vanillaOptionPricer_ = ext::shared_ptr<VanillaOptionPricer>(
                new MarketQuotedOptionPricer(swapRateValue_, fixingDate_, swapTenor_, *swaptionVolatility()));
        }
    }
    ...
    Real HaganPricer::floorletPrice(Rate effectiveFloor) const
    {
        ...
        {
            // the fixing is determined
            const Rate Rs = std::max(effectiveFloor - coupon_->swapIndex()->fixing(fixingDate_), 0.);
            Rate price = (gearing_ * Rs) * (coupon_->accrualPeriod() * discount_);
            return price;
        }
        else
    ...
    namespace
    {
        ...
        {
          public:
            VariableChange(std::function<Real(Real)>& f, Real a, Real b, Size k) : a_(a), width_(b - a), f_(f), k_(k) {}
    ...
    NumericHaganPricer::NumericHaganPricer(const Handle<SwaptionVolatilityStructure>& swaptionVol,
                                           ...
                                           Real precision,
                                           Real hardUpperLimit)
    : HaganPricer(swaptionVol, modelOfYieldCurve, meanReversion), lowerLimit_(lowerLimit), upperLimit_(upperLimit),
    ...
    Real NumericHaganPricer::integrate(Real a, Real b, const ConundrumIntegrand& integrand) const
    {
        ...
        if (a > 0)
        {
            ...
            GaussKronrodNonAdaptive gaussKronrodNonAdaptive(precision_, 1000000, 1.0);
    ...
    Real NumericHaganPricer::optionletPrice(Option::Type optionType, Real strike) const
    {

        ext::shared_ptr<ConundrumIntegrand> integrand(
            new ConundrumIntegrand(vanillaOptionPricer_, rateCurve_, gFunction_, fixingDate_, paymentDate_, annuity_,
        ...
        Real swaptionPrice = (*vanillaOptionPricer_)(strike, optionType, annuity_);
    ...
    Real NumericHaganPricer::swapletPrice() const
    {
        ...
        else
        {
            ...
            return gearing_ *
                       (coupon_->accrualPeriod() * discount_ * swapRateValue_ + atmCapletPrice - atmFloorletPrice) +
                   spreadLegValue_;
        }
    }

    Real NumericHaganPricer::refineIntegration(Real integralValue, const ConundrumIntegrand& integrand) const
    {
        Real percDiff = 1000.;
    ...
    Real NumericHaganPricer::resetUpperLimit(Real stdDeviationsForUpperLimit) const
    {
        ...
        // return 1.0;
        Real variance = swaptionVolatility()->blackVariance(fixingDate_, swapTenor_, swapRateValue_);
    ...
    NumericHaganPricer::ConundrumIntegrand::ConundrumIntegrand(ext::shared_ptr<VanillaOptionPricer> o,
    ...
    : vanillaOptionPricer_(std::move(o)), forwardValue_(forwardValue), annuity_(annuity), fixingDate_(fixingDate),
      paymentDate_(paymentDate), strike_(strike), optionType_(optionType), gFunction_(std::move(gFunction))
    {
    }
    ...
    AnalyticHaganPricer::AnalyticHaganPricer(const Handle<SwaptionVolatilityStructure>& swaptionVol,
                                             GFunctionFactory::YieldCurveModel modelOfYieldCurve,
                                             const Handle<Quote>& meanReversion)
    ...
    // Hagan, 3.5b, 3.5c
    Real AnalyticHaganPricer::optionletPrice(Option::Type optionType, Real strike) const
    {
        Real variance = swaptionVolatility()->blackVariance(fixingDate_, swapTenor_, swapRateValue_);
        ...
        if (swaptionVolatility()->volatilityType() == ShiftedLognormal)
        {
            ...
            price += sign * firstDerivativeOfGAtForwardValue * annuity_ * swapRateValue_ *
                     (swapRateValue_ * std::exp(variance) * N32 - (swapRateValue_ + strike) * N12 + strike * Nminus12);
        }
        else
        {
            const Real sqrtSigma2T = std::sqrt(variance);
    ...
    Real GFunctionFactory::GFunctionStandard::operator()(Real x)
    {
        ...
        return x / std::pow((1.0 + x / q_), delta_) * 1.0 / (1.0 - 1.0 / std::pow((1.0 + x / q_), n));
    }
    ...
    Real GFunctionFactory::GFunctionStandard::firstDerivative(Real x)
    {
        ...
        Real secDen = q_ * std::pow(a, delta_) * (std::pow(a, n) - 1.0) * (std::pow(a, n) - 1.0);
    ...
    Real GFunctionFactory::GFunctionStandard::secondDerivative(Real x)
    {
        ...
        Real C1 = (std::pow(a, delta_) - delta_ / q_ * x * std::pow(a, (delta_ - 1.0))) / std::pow(a, 2 * delta_);

        Real D = std::pow(a, (n - 1.0)) / ((std::pow(a, n) - 1.0) * (std::pow(a, n) - 1.0));
        Real D1 = ((n - 1.0) * std::pow(a, (n - 2.0)) * (std::pow(a, n) - 1.0) - 2 * n * std::pow(a, (2 * (n - 1.0)))) /
                  (q_ * (std::pow(a, n) - 1.0) * (std::pow(a, n) - 1.0) * (std::pow(a, n) - 1.0));

        return A1 * B + AA * B1 - n / q_ * (C1 * D + C * D1);
    }

    ext::shared_ptr<GFunction> GFunctionFactory::newGFunctionStandard(Size q, Real delta, Size swapLength)
    {
    ...
    GFunctionFactory::GFunctionExactYield::GFunctionExactYield(const CmsCoupon& coupon)
    {
        ...
        Real paymentTime = dc.yearFraction(rateCurve->referenceDate(), coupon.date());
        ...
        for (Size i = 0; i < n; ++i)
        {
            ext::shared_ptr<Coupon> cpn = ext::dynamic_pointer_cast<Coupon>(fixedLeg[i]);
    ...
    Real GFunctionFactory::GFunctionExactYield::firstDerivative(Real x)
    {
        ...
        return -delta_ * accruals_[0] * std::pow(b[0], delta_ + 1.) * x * c + std::pow(b[0], delta_) * c +
               std::pow(b[0], delta_) * x * derC;
        // Real dx = 1.0e-8;
    ...
    Real GFunctionFactory::GFunctionExactYield::secondDerivative(Real x)
    {
        ...
        return (-delta_ * accruals_[0] * std::pow(b[0], delta_ + 1.) * c + std::pow(b[0], delta_) * derC) *
               ...
               std::pow(b[0], delta_) * c *
                   (delta_ * std::pow(accruals_[0] * b[0], 2.) * x - delta_ * accruals_[0] * b[0] - x * derC * sum +
                    (1. - c) * sum - x * (1. - c) * sumOfSquare);
        // Real dx = 1.0e-8;
    ...
    GFunctionFactory::GFunctionWithShifts::GFunctionWithShifts(const CmsCoupon& coupon, Handle<Quote> meanReversion)
    ...
    Real GFunctionFactory::GFunctionWithShifts::derRs_derX(Real x)
    {
        ...
        {
            sqrtDenominator += accruals_[i] * swapPaymentDiscounts_[i] * std::exp(-shapedSwapPaymentTimes_[i] * x);
    ...
    Real GFunctionFactory::GFunctionWithShifts::der2Rs_derX2(Real x)
    {
        ...
        for (Size i = 0; i < accruals_.size(); i++)
        {
            denOfRfunztion += accruals_[i] * swapPaymentDiscounts_[i] * std::exp(-shapedSwapPaymentTimes_[i] * x);
        ...
        derNumOfDerR -= (shapedSwapPaymentTimes_.back() * swapPaymentDiscounts_.back() *
                        ...
                        derDenOfRfunztion;
        derNumOfDerR -=
            (discountAtStart_ - swapPaymentDiscounts_.back() * std::exp(-shapedSwapPaymentTimes_.back() * x)) *
            der2DenOfRfunztion;
    ...
    Real GFunctionFactory::GFunctionWithShifts::der2Z_derX2(Real x)
    {
        ...
        const Real derNumOfDerZ =
            (-shapedPaymentTime_ * std::exp(-shapedPaymentTime_ * x) *
                 (-shapedPaymentTime_ +
    ...
    Real GFunctionFactory::GFunctionWithShifts::ObjectiveFunction::operator()(const Real& x) const
    {
        ...
        {
            Real temp = o_.accruals_[i] * o_.swapPaymentDiscounts_[i] * std::exp(-o_.shapedSwapPaymentTimes_[i] * x);
            result += temp;
            derivative_ -= o_.shapedSwapPaymentTimes_[i] * temp;
        }
        result *= Rs_;
        derivative_ *= Rs_;
        Real temp = o_.swapPaymentDiscounts_.back() * std::exp(-o_.shapedSwapPaymentTimes_.back() * x);
    ...
    Real GFunctionFactory::GFunctionWithShifts::calibrationOfShift(Real Rs)
    {
        ...
        if (Rs != tmpRs_)
        {
            ...
            D += accruals_.back() * swapPaymentDiscounts_.back() * shapedSwapPaymentTimes_.back();
            initialGuess = N / D;
            ...
            {
                calibratedShift_ =
                    solver.solve(*objectiveFunction_, accuracy_,
                                 std::max(std::min(initialGuess, upper * .99), lower * .99), lower, upper);
            }
            catch (std::exception& e)
            {
                QL_FAIL("meanReversion: " << meanReversion_->value() << ", swapRateValue: " << swapRateValue_
                                          << ", swapStartTime: " << swapStartTime_ << ", shapedPaymentTime: "
