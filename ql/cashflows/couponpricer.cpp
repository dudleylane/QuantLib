...
namespace QuantLib
{
    ...
    IborCouponPricer::IborCouponPricer(Handle<OptionletVolatilityStructure> v, ext::optional<bool> useIndexedCoupon)
    : capletVol_(std::move(v)),
      useIndexedCoupon_(useIndexedCoupon ? *useIndexedCoupon : !IborCoupon::Settings::instance().usingAtParCoupons())
    {
    ...
    void IborCouponPricer::initializeCachedData(const IborCoupon& coupon) const
    {
        ...
        coupon.fixingValueDate_ =
            coupon.iborIndex()->fixingCalendar().advance(coupon.fixingDate_, coupon.iborIndex()->fixingDays(), Days);
        ...
        else
        {
            ...
            { // par coupon approximation
                ...
                coupon.fixingEndDate_ = std::max(coupon.fixingEndDate_, coupon.fixingValueDate_ + 1);
            }
        }

        coupon.spanningTime_ =
            coupon.iborIndex()->dayCounter().yearFraction(coupon.fixingValueDate_, coupon.fixingEndDate_);
        ...
        coupon.spanningTimeIndexMaturity_ =
            coupon.iborIndex()->dayCounter().yearFraction(coupon.fixingValueDate_, coupon.fixingMaturityDate_);
    ...
    Real BlackIborCouponPricer::optionletRate(Option::Type optionType, Real effStrike) const
    {
        ...
        else
        {
            ...
            Real stdDev = std::sqrt(capletVolatility()->blackVariance(fixingDate_, effStrike));
            Real shift = capletVolatility()->displacement();
            bool shiftedLn = capletVolatility()->volatilityType() == ShiftedLognormal;
            Rate fixing = shiftedLn ? blackFormula(optionType, effStrike, adjustedFixing(), stdDev, 1.0, shift) :
                                      bachelierBlackFormula(optionType, effStrike, adjustedFixing(), stdDev, 1.0);
            return fixing;
        }
    }
    ...
    Rate BlackIborCouponPricer::adjustedFixing(Rate fixing) const
    {
        ...
        Spread adjustment = shiftedLn ?
                                Real((fixing + shift) * (fixing + shift) * variance * tau / (1.0 + fixing * tau)) :
        ...
        if (timingAdjustment_ == BivariateLognormal)
        {
            ...
            {
                Real fixing2 =
                    (index_->forwardingTermStructure()->discount(d5) / index_->forwardingTermStructure()->discount(d4) -
                     1.0) /
                    tau2;
                adjustment -= shiftedLn ? Real(correlation_->value() * tau2 * variance * (fixing + shift) *
                                               (fixing2 + shift) / (1.0 + fixing2 * tau2)) :
                                          Real(correlation_->value() * tau2 * variance / (1.0 + fixing2 * tau2));
            }
        }
    ...
    namespace
    {
                             ...
                             public Visitor<MultipleResetsCoupon>
        {
          ...
          public:
            explicit PricerSetter(ext::shared_ptr<FloatingRateCouponPricer> pricer) : pricer_(std::move(pricer)) {}
        ...
        void PricerSetter::visit(OvernightIndexedCoupon& c)
        {
            if (c.averagingMethod() == RateAveraging::Compound)
            {
                ...
                QL_REQUIRE(overnightCouponPricer, "pricer not compatible with overnight indexed coupon");
        ...
        void PricerSetter::visit(CappedFlooredOvernightIndexedCoupon& c)
        {
            ...
            if (c.averagingMethod() == RateAveraging::Compound)
            {
                ...
                QL_REQUIRE(p, "pricer not compatible with capped-floored overnight indexed coupon");
                c.setPricer(p);
            ...
            else
            {
                ...
                QL_REQUIRE(p, "pricer not compatible with arithmetic averaged capped-floored overnight indexed coupon");
                c.setPricer(p);
                c.underlying()->accept(*this);
            }
        }
        ...
        void PricerSetter::visit(CmsSpreadCoupon& c)
        {
            ...
            QL_REQUIRE(cmsSpreadCouponPricer, "pricer not compatible with CMS spread coupon");
        ...
        void PricerSetter::visit(RangeAccrualFloatersCoupon& c)
        {
            ...
            QL_REQUIRE(rangeAccrualPricer, "pricer not compatible with range-accrual coupon");
