...
namespace QuantLib
{
    ...
    namespace
    {
        Handle<YieldTermStructure> configureDividendHandle(const Handle<YieldTermStructure>& dividendHandle)
        {
            if (dividendHandle.empty())
            {
                ext::shared_ptr<YieldTermStructure> flatTs(ext::make_shared<FlatForward>(
                    0, NullCalendar(), Handle<Quote>(ext::make_shared<SimpleQuote>(0.0)), Actual365Fixed()));
    ...
    void setCouponPricer(const Leg& leg, const ext::shared_ptr<EquityCashFlowPricer>& p)
    {
        ...
        {
            ext::shared_ptr<EquityCashFlow> c = ext::dynamic_pointer_cast<EquityCashFlow>(i);
            if (c != nullptr)
                c->setPricer(p);
        }
    }
    ...
    EquityQuantoCashFlowPricer::EquityQuantoCashFlowPricer(Handle<YieldTermStructure> quantoCurrencyTermStructure,
    ...
    void EquityQuantoCashFlowPricer::initialize(const EquityCashFlow& cashFlow)
    {
        ...
        QL_REQUIRE(!quantoCurrencyTermStructure_.empty(), "Quanto currency term structure handle cannot be empty.");
        QL_REQUIRE(!equityVolatility_.empty(), "Equity volatility term structure handle cannot be empty.");
        QL_REQUIRE(!fxVolatility_.empty(), "FX volatility term structure handle cannot be empty.");
        QL_REQUIRE(!correlation_.empty(), "Correlation handle cannot be empty.");

        QL_REQUIRE(quantoCurrencyTermStructure_->referenceDate() == equityVolatility_->referenceDate() &&
    ...
    Real EquityQuantoCashFlowPricer::price() const
    {
        ...
        Handle<YieldTermStructure> dividendHandle = configureDividendHandle(index_->equityDividendCurve());

        Handle<YieldTermStructure> quantoTermStructure(ext::make_shared<QuantoTermStructure>(
            dividendHandle, quantoCurrencyTermStructure_, index_->equityInterestRateCurve(), equityVolatility_, strike,
            fxVolatility_, 1.0, correlation_->value()));
