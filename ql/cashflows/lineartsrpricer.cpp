...
namespace QuantLib
{
    ...
    const Real LinearTsrPricer::defaultLowerBound = 0.0001, LinearTsrPricer::defaultUpperBound = 2.0000;
    ...
    Real LinearTsrPricer::singularTerms(const Option::Type type, const Real strike) const
    {

        Real omega = (type == Option::Call ? 1.0 : -1.0);
        Real s1 = std::max(omega * (swapRateValue_ - strike), 0.0) * (a_ * swapRateValue_ + b_);
        Real s2 = (a_ * strike + b_) *
                  smileSection_->optionPrice(strike, strike < swapRateValue_ ? Option::Put : Option::Call);
        return s1 + s2;
    }

    Real LinearTsrPricer::integrand(const Real strike) const
    {
        return 2.0 * a_ * smileSection_->optionPrice(strike, strike < swapRateValue_ ? Option::Put : Option::Call);
    }
    ...
    void LinearTsrPricer::initialize(const FloatingRateCoupon& coupon)
    {
        ...
        if (fixingDate_ > today_)
        {
            ...
            ext::shared_ptr<SmileSection> sectionTmp = swaptionVolatility()->smileSection(fixingDate_, swapTenor_);
            ...
            if (sectionTmp->atmLevel() == Null<Real>())
                smileSection_ = ext::make_shared<AtmSmileSection>(sectionTmp, swapRateValue_);
            else
            ...
            a_ = discountCurve_->discount(paymentDate_) * (gamma - GsrG(paymentDate_)) /
                 (discountCurve_->discount(lastd) * GsrG(lastd) + swapRateValue_ * gy * gamma);

            b_ = discountCurve_->discount(paymentDate_) / gy - a_ * swapRateValue_;
        }
    }

    Real LinearTsrPricer::strikeFromVegaRatio(Real ratio, Option::Type optionType, Real referenceStrike) const
    {
        ...
        if (optionType == Option::Call)
        {
            ...
            b = max = k = std::min(smileSection_->maxStrike(), adjustedUpperBound_);
        }
        else
        {
            // NOLINTNEXTLINE(clang-analyzer-deadcode.DeadStores)
            a = min = k = std::max(smileSection_->minStrike(), adjustedLowerBound_);
            b = swapRateValue_;
            max = referenceStrike;
        }
    ...
    Real LinearTsrPricer::strikeFromPrice(Real price, Option::Type optionType, Real referenceStrike) const
    {
    ...
    Real LinearTsrPricer::optionletPrice(Option::Type optionType, Real strike) const
    {
        ...
        switch (settings_.strategy_)
        {
            ...
            case Settings::VegaRatio:
            {
                ...
                Real bound = strikeFromVegaRatio(settings_.vegaRatio_, optionType, strike);
                if (optionType == Option::Call)
            ...
            case Settings::BSStdDevs:
            {
                ...
                {
                    upperTmp = (atm + shift) * std::exp(settings_.stdDevs_ * atmVol -
                                                        0.5 * atmVol * atmVol * smileSection_->exerciseTime()) -
                               shift;
                    lowerTmp = (atm + shift) * std::exp(-settings_.stdDevs_ * atmVol -
                                                        0.5 * atmVol * atmVol * smileSection_->exerciseTime()) -
                               shift;
                }
                else
                {
                    Real tmp = settings_.stdDevs_ * atmVol * std::sqrt(smileSection_->exerciseTime());
                    upperTmp = atm + tmp;
                    lowerTmp = atm - tmp;
                }
        ...
        if (upper > lower)
        {
            ...
            if (tmpBound > lower)
            {
                result += (*integrator_)(integrand_f(this), lower, tmpBound);
            }
            tmpBound = std::max(lower, swapRateValue_);
            if (upper > tmpBound)
            {
                result += (*integrator_)(integrand_f(this), tmpBound, upper);
            }
    ...
    Real LinearTsrPricer::capletPrice(Rate effectiveCap) const
    {
        ...
        if (fixingDate_ <= today_)
        {
            ...
            Rate price =
                (gearing_ * Rs) * (coupon_->accrualPeriod() * discountCurvePaymentDiscount_ * couponDiscountRatio_);
            return price;
        }
        else
        {
    ...
    Real LinearTsrPricer::swapletPrice() const
    {
        if (fixingDate_ <= today_)
        {
            ...
            Rate price = (gearing_ * Rs + spread_) *
                         (coupon_->accrualPeriod() * discountCurvePaymentDiscount_ * couponDiscountRatio_);
            return price;
        }
        else
        {
            ...
            return gearing_ * (coupon_->accrualPeriod() * discountCurvePaymentDiscount_ * swapRateValue_ *
