...
namespace QuantLib
{
    ...
    Real midEquivalent(const Real bid, const Real ask, const Real last, const Real close)
    {
        ...
        else
        {
            ...
            {
                QL_REQUIRE(close != Null<Real>() && close > 0.0, "all input prices are invalid");
                return close;
            }
        }
    }
    ...
    {
        QL_REQUIRE(bid != Null<Real>() && bid > 0.0, "invalid bid price");
        QL_REQUIRE(ask != Null<Real>() && ask > 0.0, "invalid ask price");
        return (bid + ask) / 2.0;
    }


    IntervalPrice::IntervalPrice() : open_(Null<Real>()), close_(Null<Real>()), high_(Null<Real>()), low_(Null<Real>())
    {
    }
    ...
    void IntervalPrice::setValue(Real value, IntervalPrice::Type t)
    {
        switch (t)
        {
            case Open:
    ...
    TimeSeries<IntervalPrice> IntervalPrice::makeSeries(const std::vector<Date>& d,
    ...
    {
        Size dsize = d.size();
        QL_REQUIRE((open.size() == dsize && close.size() == dsize && high.size() == dsize && low.size() == dsize),
                   "size mismatch (" << dsize << ", " << open.size() << ", " << close.size() << ", " << high.size()
                                     << ", " << low.size() << ")");
    ...
    std::vector<Real> IntervalPrice::extractValues(const TimeSeries<IntervalPrice>& ts, IntervalPrice::Type t)
    {
    ...
    TimeSeries<Real> IntervalPrice::extractComponent(const TimeSeries<IntervalPrice>& ts, IntervalPrice::Type t)
    {
