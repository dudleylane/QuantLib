...
namespace QuantLib
{
    ...
    // constructors
    Date::Date() : serialNumber_(Date::serial_type(0)) {}

    Date::Date(Date::serial_type serialNumber) : serialNumber_(serialNumber)
    {
    ...
    Date Date::advance(const Date& date, Integer n, TimeUnit units)
    {
        switch (units)
        {
            ...
            case Months:
            {
                ...
                QL_ENSURE(y >= 1900 && y <= 2199, "year " << y << " out of bounds. "
    ...
    namespace detail
    {
        ...
        const Integer MonthLengthTable[12] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
        const Integer MonthLeapLengthTable[12] = {31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
    ...
    namespace
    {
        ...
        {
            static const boost::gregorian::date dateReference(1899, boost::gregorian::Dec, 30);
            return dateReference;
        }


#    define compatibleEnums                                                                                      \
        (int(boost::date_time::Monday) + 1 == Monday && int(boost::date_time::Tuesday) + 1 == Tuesday &&         \
         int(boost::date_time::Wednesday) + 1 == Wednesday && int(boost::date_time::Thursday) + 1 == Thursday && \
         int(boost::date_time::Friday) + 1 == Friday && int(boost::date_time::Saturday) + 1 == Saturday &&       \
         int(boost::date_time::Sunday) + 1 == Sunday && int(boost::date_time::Jan) == January &&                 \
         int(boost::date_time::Feb) == February && int(boost::date_time::Mar) == March &&                        \
         int(boost::date_time::Apr) == April && int(boost::date_time::May) == May &&                             \
         int(boost::date_time::Jun) == June && int(boost::date_time::Jul) == July &&                             \
         int(boost::date_time::Aug) == August && int(boost::date_time::Sep) == September &&                      \
         int(boost::date_time::Oct) == October && int(boost::date_time::Nov) == November &&                      \
        ...
        void advance(ptime& dt, Integer n, TimeUnit units)
        {
            ...
            switch (units)
            {
                ...
                case Years:
                {
                    ...
                    const Day eoM = gregorian_calendar::end_of_month_day(date.year(), date.month());
                    ...
                    if (date.day() == eoM)
                    {
                        ...
                        const Day newEoM = gregorian_calendar::end_of_month_day(dt.date().year(), dt.date().month());
        ...
        boost::gregorian::date gregorianDate(Year y, Month m, Day d)
        {
            ...
            const boost::gregorian::greg_month bM = mapQLDateType2Boost<compatibleEnums>(m);

            const Day len = boost::gregorian::gregorian_calendar::end_of_month_day(y, bM);
    ...
    Date::Date(const ptime& dateTime) : dateTime_(dateTime) {}

    Date::Date(Day d, Month m, Year y) : dateTime_(gregorianDate(y, m, d)) {}

    Date::Date(
        Day d, Month m, Year y, Hour hours, Minute minutes, Second seconds, Millisecond millisec, Microsecond microsec)
    ...
    {
        return mapBoostDateType2QL<compatibleEnums>(dateTime_.date().day_of_week());
    }
    ...
    Time Date::fractionOfDay() const
    {
        ...
        const Time seconds =
            (t.hours() * 60.0 + t.minutes()) * 60.0 + t.seconds() + Real(t.fractional_seconds()) / ticksPerSecond();
    ...
    {
        return dateTime_.time_of_day().fractional_seconds() / Real(ticksPerSecond());
    }

    Millisecond Date::milliseconds() const
    {
        return dateTime_.time_of_day().fractional_seconds() / (ticksPerSecond() / 1000);
    }
    ...
    {
        const Date::serial_type n = (dateTime_.date() - serialNumberDateReference()).days();
        if (n != 0)
            checkSerialNumber(n);
        return n;
    }

    const ptime& Date::dateTime() const
    {
        return dateTime_;
    }
    ...
    {
        return d.dayOfMonth() == boost::gregorian::gregorian_calendar::end_of_month_day(
                                     d.year(), mapQLDateType2Boost<compatibleEnums>(d.month()));
    }
    ...
    void Date::checkSerialNumber(Date::serial_type serialNumber)
    {
        QL_REQUIRE(serialNumber >= minimumSerialNumber() && serialNumber <= maximumSerialNumber(),
                   "Date's serial number (" << serialNumber
                                            << ") outside "
                                               "allowed range ["
    ...
    Date Date::nthWeekday(Size nth, Weekday dayOfWeek, Month m, Year y)
    {
        QL_REQUIRE(nth > 0, "zeroth day of week in a given (month, year) is undefined");
        QL_REQUIRE(nth < 6, "no more than 5 weekday in a given (month, year)");
    ...
    namespace detail
    {
        ...
        { // NOLINT(cppcoreguidelines-special-member-functions)
            ...
            : out_(&out), flags_(out.flags()), filler_(out.fill()), loc_(out.getloc())
            {
                std::locale loc(out.getloc(), new nopunct);
                out.imbue(loc);
                out << std::resetiosflags(std::ios_base::adjustfield | std::ios_base::basefield |
                                          std::ios_base::floatfield | std::ios_base::showbase | std::ios_base::showpos |
        ...
        std::ostream& operator<<(std::ostream& out, const short_date_holder& holder)
        {
            ...
            else
            {
                ...
                Integer dd = d.dayOfMonth(), mm = Integer(d.month()), yyyy = d.year();
        ...
        std::ostream& operator<<(std::ostream& out, const formatted_date_holder& holder)
        {
            ...
            else
            {
                ...
                out.imbue(std::locale(std::locale(), new date_facet(holder.f.c_str())));
                out << boostDate;
            }
            return out;
        }
...
#ifdef QL_HIGH_RESOLUTION_DATE
        std::ostream& operator<<(std::ostream& out, const iso_datetime_holder& holder)
        {
            const Date& d = holder.d;
            ...
            out << std::setw(2) << std::setfill('0') << hh << ":" << std::setw(2) << std::setfill('0') << mm << ":"
                << std::setw(2) << std::setfill('0') << s << "," << std::setw(3) << std::setfill('0') << millis
