      ...
      private:
        class Impl final : public DayCounter::Impl
        {
          ...
          public:
            ...
            {
                return includeLastDay_ ? std::string("Actual/366 (inc)") : std::string("Actual/366");
            }
            ...
            {
                return (daysBetween(d1, d2) + (includeLastDay_ ? 1.0 : 0.0)) / 366.0;
            }
        };

      public:
        explicit Actual366(const bool includeLastDay = false)
        : DayCounter(ext::shared_ptr<DayCounter::Impl>(new Actual366::Impl(includeLastDay)))
        {
        }
    };
