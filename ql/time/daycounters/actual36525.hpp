      ...
      private:
        class Impl final : public DayCounter::Impl
        {
          ...
          public:
            ...
            {
                return includeLastDay_ ? std::string("Actual/365.25 (inc)") : std::string("Actual/365.25");
            }
            ...
            {
                return (daysBetween(d1, d2) + (includeLastDay_ ? 1.0 : 0.0)) / 365.25;
            }
        };

      public:
        explicit Actual36525(const bool includeLastDay = false)
        : DayCounter(ext::shared_ptr<DayCounter::Impl>(new Actual36525::Impl(includeLastDay)))
        {
        }
    };
