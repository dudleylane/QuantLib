...
namespace QuantLib
{
    ...
    namespace
    {
        ...
        template <class T>
        Integer findCouponsPerYear(const T& impl, Date refStart, Date refEnd)
        {
        ...
        */
        template <class T>
        Time yearFractionGuess(const T& impl, const Date& start, const Date& end)
        {
            // asymptotically correct.
            return Real(impl.dayCount(start, end)) / 365.0;
        }

        std::vector<Date> getListOfPeriodDatesIncludingQuasiPayments(const Schedule& schedule)
        {
            ...
            if (!schedule.hasIsRegular() || !schedule.isRegular(1))
            {
                ...
                Date notionalFirstCoupon = schedule.calendar().advance(
                    firstCoupon, -schedule.tenor(), schedule.businessDayConvention(), schedule.endOfMonth());
                ...
                if (notionalFirstCoupon > issueDate)
                {
                    Date priorNotionalCoupon =
                        schedule.calendar().advance(notionalFirstCoupon, -schedule.tenor(),
            ...
            if (!schedule.hasIsRegular() || !schedule.isRegular(schedule.size() - 1))
            {
                Date notionalLastCoupon =
                    schedule.calendar().advance(schedule.date(schedule.size() - 2), schedule.tenor(),
                                                schedule.businessDayConvention(), schedule.endOfMonth());
                ...
                {
                    Date nextNotionalCoupon = schedule.calendar().advance(
                        notionalLastCoupon, schedule.tenor(), schedule.businessDayConvention(), schedule.endOfMonth());
        ...
        template <class T>
        Time
        yearFractionWithReferenceDates(const T& impl, const Date& d1, const Date& d2, const Date& d3, const Date& d4)
        {
    ...
    ext::shared_ptr<DayCounter::Impl> ActualActual::implementation(ActualActual::Convention c, Schedule schedule)
    {
        switch (c)
        {
            case ISMA:
    ...
    Time ActualActual::ISMA_Impl::yearFraction(const Date& d1, const Date& d2, const Date& d3, const Date& d4) const
    {
        if (d1 == d2)
        {
            return 0.0;
        ...
        QL_REQUIRE(d1 >= firstDate && d2 <= lastDate,
                   "Dates out of range of schedule: " << "date 1: " << d1 << ", date 2: " << d2
                                                      << ", first date: " << firstDate << ", last date: " << lastDate);
        ...
        for (Size i = 0; i < couponDates.size() - 1; i++)
        {
            ...
            {
                yearFractionSum += yearFractionWithReferenceDates(*this, std::max(d1, startReferencePeriod),
    ...
    Time ActualActual::Old_ISMA_Impl::yearFraction(const Date& d1, const Date& d2, const Date& d3, const Date& d4) const
    {
        if (d1 == d2)
            return 0.0;
        ...
        if (d2 <= refPeriodEnd)
        {
            ...
            if (d1 >= refPeriodStart)
            {
                ...
                return period * Real(daysBetween(d1, d2)) / daysBetween(refPeriodStart, refPeriodEnd);
            }
            else
            {
    ...
    Time ActualActual::ISDA_Impl::yearFraction(const Date& d1, const Date& d2, const Date&, const Date&) const
    {
        if (d1 == d2)
            return 0.0;
        ...
        Real dib1 = (Date::isLeap(y1) ? 366.0 : 365.0), dib2 = (Date::isLeap(y2) ? 366.0 : 365.0);
    ...
    Time ActualActual::AFB_Impl::yearFraction(const Date& d1, const Date& d2, const Date&, const Date&) const
    {
        if (d1 == d2)
            return 0.0;
        ...
        {
            temp = newD2 - 1 * Years;
            if (temp.dayOfMonth() == 28 && temp.month() == 2 && Date::isLeap(temp.year()))
            {
                temp += 1;
            }
            if (temp >= d1)
