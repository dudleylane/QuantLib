...
namespace QuantLib
{
      ...
      isRegular_(std::move(isRegular))
    {
        ...
        QL_REQUIRE(isRegular_.empty() || isRegular_.size() == dates.size() - 1,
                   "isRegular size (" << isRegular_.size() << ") must be zero or equal to the number of dates minus 1 ("
                                      << dates.size() - 1 << ")");
    }
    ...
    : tenor_(tenor), calendar_(std::move(cal)), convention_(convention),
      ...
      endOfMonth_(allowsEndOfMonth(tenor) ? endOfMonth : false), firstDate_(first == effectiveDate ? Date() : first),
      nextToLastDate_(nextToLast == terminationDate ? Date() : nextToLast)
    {
        ...
        if (effectiveDate == Date() && first == Date() && rule == DateGeneration::Backward)
        {
        ...
        QL_REQUIRE(effectiveDate < terminationDate, "effective date (" << effectiveDate
        ...
        if (tenor.length() == 0)
            ...
            QL_REQUIRE(tenor.length() > 0, "non positive tenor (" << tenor << ") not allowed");
        ...
        if (firstDate_ != Date())
        {
            switch (*rule_)
            {
                ...
                case DateGeneration::Forward:
                    QL_REQUIRE(firstDate_ > effectiveDate && firstDate_ <= terminationDate,
                ...
                case DateGeneration::ThirdWednesday:
                    QL_REQUIRE(IMM::isIMMdate(firstDate_, false),
                               "first date (" << firstDate_ << ") is not an IMM date");
                    break;
                ...
                case DateGeneration::CDS2015:
                    QL_FAIL("first date incompatible with " << *rule_ << " date generation rule");
                default:
        ...
        if (nextToLastDate_ != Date())
        {
            switch (*rule_)
            {
                case DateGeneration::Backward:
                case DateGeneration::Forward:
                    QL_REQUIRE(nextToLastDate_ >= effectiveDate && nextToLastDate_ < terminationDate,
                ...
                case DateGeneration::ThirdWednesday:
                    QL_REQUIRE(IMM::isIMMdate(nextToLastDate_, false),
                               "next-to-last date (" << nextToLastDate_ << ") is not an IMM date");
                    break;
                case DateGeneration::Zero:
                case DateGeneration::Twentieth:
                case DateGeneration::TwentiethIMM:
                ...
                case DateGeneration::CDS:
                case DateGeneration::CDS2015:
                    QL_FAIL("next to last date incompatible with " << *rule_ << " date generation rule");
                default:
                    QL_FAIL("unknown rule (" << Integer(*rule_) << ")");
            }
        }
        ...
        switch (*rule_)
        {
            ...
            case DateGeneration::Backward:
                ...
                if (nextToLastDate_ != Date())
                {
                    ...
                    Date temp = nullCalendar.advance(seed, -periods * (*tenor_), convention, *endOfMonth_);
                ...
                for (;;)
                {
                    ...
                    {
                        if (firstDate_ != Date() &&
                            (calendar_.adjust(dates_.back(), convention) != calendar_.adjust(firstDate_, convention)))
                        {
                            dates_.push_back(firstDate_);
                            isRegular_.push_back(nullCalendar.advance(dates_[dates_.size() - 2], -1 * (*tenor_),
                                                                      convention, *endOfMonth_) == firstDate_);
                        }
                        break;
                    }
                    else
                    {
                        ...
                        // after adjustment
                        if (calendar_.adjust(dates_.back(), convention) != calendar_.adjust(temp, convention))
                        {
                ...
                if (calendar_.adjust(dates_.back(), convention) != calendar_.adjust(effectiveDate, convention))
                {
                    dates_.push_back(effectiveDate);
                    isRegular_.push_back(nullCalendar.advance(dates_[dates_.size() - 2], -1 * (*tenor_), convention,
                                                              *endOfMonth_) == effectiveDate);
                }
            ...
            case DateGeneration::Forward:
                ...
                else if (*rule_ == DateGeneration::Twentieth || *rule_ == DateGeneration::TwentiethIMM ||
                         *rule_ == DateGeneration::OldCDS || *rule_ == DateGeneration::CDS ||
                ...
                for (;;)
                {
                    ...
                    if (temp > exitDate)
                    {
                        if (nextToLastDate_ != Date() && (calendar_.adjust(dates_.back(), convention) !=
                                                          calendar_.adjust(nextToLastDate_, convention)))
                        {
                            dates_.push_back(nextToLastDate_);
                            isRegular_.push_back(nullCalendar.advance(dates_[dates_.size() - 2], 1 * (*tenor_),
                                                                      convention, *endOfMonth_) == nextToLastDate_);
                        }
                        break;
                    }
                    else
                ...
                {
                    if (*rule_ == DateGeneration::Twentieth || *rule_ == DateGeneration::TwentiethIMM ||
                        *rule_ == DateGeneration::OldCDS || *rule_ == DateGeneration::CDS ||
                        *rule_ == DateGeneration::CDS2015)
                    {
        ...
        // schedule
        if (terminationDateConvention != Unadjusted && *rule_ != DateGeneration::CDS &&
            *rule_ != DateGeneration::CDS2015)
        {
        ...
        if (dates_.size() >= 2 && dates_[dates_.size() - 2] >= dates_.back())
        {
            ...
            {
                isRegular_[isRegular_.size() - 2] = (dates_[dates_.size() - 2] == dates_.back());
            }
        ...
        {
            isRegular_[1] = (dates_[1] == dates_.front());
            dates_[1] = dates_.front();
            dates_.erase(dates_.begin());
            isRegular_.erase(isRegular_.begin());
        }

        QL_ENSURE(dates_.size() > 1, "degenerate single date ("
                                         << dates_[0] << ") schedule" << "\n seed date: " << seed
    ...
    Schedule Schedule::until(const Date& truncationDate) const
    {
        ...
        QL_REQUIRE(truncationDate > result.dates_[0], "truncation date " << truncationDate
    ...
    std::vector<Date>::const_iterator Schedule::lower_bound(const Date& refDate) const
    {
        Date d = (refDate == Date() ? Settings::instance().evaluationDate() : refDate);
    ...
    {
        QL_REQUIRE(hasIsRegular(), "full interface (isRegular) not available");
        QL_REQUIRE(i <= isRegular_.size() && i > 0, "index (" << i << ") must be in [1, " << isRegular_.size() << "]");
    ...
    MakeSchedule& MakeSchedule::withTerminationDateConvention(BusinessDayConvention conv)
    {
    ...
    MakeSchedule::operator Schedule() const
    {
        ...
        return Schedule(effectiveDate_, terminationDate_, *tenor_, calendar, convention, terminationDateConvention,
