...
namespace QuantLib
{
    ...
    EurodollarFuturesImpliedStdDevQuote::EurodollarFuturesImpliedStdDevQuote(Handle<Quote> forward,
    ...
    : impliedStdev_(guess), strike_(100.0 - strike), accuracy_(accuracy), maxIter_(maxIter),
      forward_(std::move(forward)), callPrice_(std::move(callPrice)), putPrice_(std::move(putPrice))
    {
    ...
    void EurodollarFuturesImpliedStdDevQuote::performCalculations() const
    {
        ...
        if (strike_ > forwardValue)
        {
            impliedStdev_ = blackFormulaImpliedStdDev(Option::Call, strike_, forwardValue, putPrice_->value(), discount,
                                                      displacement, impliedStdev_, accuracy_, maxIter_);
        }
        else
        {
            impliedStdev_ = blackFormulaImpliedStdDev(Option::Put, strike_, forwardValue, callPrice_->value(), discount,
                                                      displacement, impliedStdev_, accuracy_, maxIter_);
        }
    }
}
