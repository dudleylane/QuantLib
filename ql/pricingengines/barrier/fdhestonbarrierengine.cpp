...
namespace QuantLib
{
    ...
    void FdHestonBarrierEngine::calculate() const
    {
        ...
        ext::shared_ptr<FdmDividendHandler> dividendCondition(ext::make_shared<FdmDividendHandler>(
            dividends_, mesher, process->riskFreeRate()->referenceDate(), process->riskFreeRate()->dayCounter(), 0));
        ...
        ext::shared_ptr<FdmHestonSolver> solver(
            ext::make_shared<FdmHestonSolver>(Handle<HestonProcess>(process), solverDesc, schemeDesc_,
        ...
        if (arguments_.barrierType == Barrier::DownIn || arguments_.barrierType == Barrier::UpIn)
        {
            ...
            vanillaOption.setPricingEngine(ext::shared_ptr<PricingEngine>(ext::make_shared<FdHestonVanillaEngine>(
                *model_, dividends_, tGrid_, xGrid_, vGrid_, dampingSteps_, schemeDesc_)));
            ...
            rebateOption.setPricingEngine(ext::make_shared<FdHestonRebateEngine>(
                *model_, dividends_, tGrid_, std::max(xGridMin, xGrid_ / 4), std::max(vGridMin, vGrid_ / 4),
