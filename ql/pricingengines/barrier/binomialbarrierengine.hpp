    ...
    */
      ...
      public:
        ...
        */
        ...
        : process_(std::move(process)), timeSteps_(timeSteps), maxTimeSteps_(maxTimeSteps)
        {
            ...
            QL_REQUIRE(maxTimeSteps == 0 || maxTimeSteps >= timeSteps, "maxTimeSteps must be zero or "
    ...
    void BinomialBarrierEngine<T, D>::calculate() const
    {
        ...
        Handle<YieldTermStructure> flatRiskFree(
            ext::shared_ptr<YieldTermStructure>(new FlatForward(referenceDate, r, rfdc)));
        Handle<YieldTermStructure> flatDividends(
            ext::shared_ptr<YieldTermStructure>(new FlatForward(referenceDate, q, divdc)));
        Handle<BlackVolTermStructure> flatVol(
            ext::shared_ptr<BlackVolTermStructure>(new BlackConstantVol(referenceDate, volcal, v, voldc)));
        ...
        if (std::is_base_of_v<CoxRossRubinstein, T> && maxTimeSteps_ > timeSteps_ && s0 > 0 && arguments_.barrier > 0)
        {
            Real divisor;
        ...
        ext::shared_ptr<T> tree(new T(bs, maturity, optimum_steps, payoff->strike()));

        ext::shared_ptr<BlackScholesLattice<T>> lattice(new BlackScholesLattice<T>(tree, r, maturity, optimum_steps));
