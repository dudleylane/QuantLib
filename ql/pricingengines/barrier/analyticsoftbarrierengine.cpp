...
namespace QuantLib
{
    ...
    AnalyticSoftBarrierEngine::AnalyticSoftBarrierEngine(ext::shared_ptr<GeneralizedBlackScholesProcess> process)
    ...
    void AnalyticSoftBarrierEngine::calculate() const
    {
        ...
        bool isKnockedIn = (barrierType == Barrier::DownIn && S <= L) || (barrierType == Barrier::UpIn && S >= U);
        bool isKnockedOut = (barrierType == Barrier::DownOut && S <= L) || (barrierType == Barrier::UpOut && S >= U);

        bool isSingleBarrier = (std::fabs(U - L) < 1e-4);


        // edge case 1: fully knocked in options should be priced as vanilla (there are no more barrier features to
        // consider)
        if (isKnockedIn)
        {
    ...
    Real AnalyticSoftBarrierEngine::knockInValue(
        Real S, Real X, Rate r, Volatility sigma, Time T, Real U, Real L, Real b, Option::Type optionType, Integer eta)
        const
    {
        // constant terms
        ...
        // term 1
        Real term1 =
            eta * S * std::exp((b - r) * T) * std::pow(S, -2.0 * mu) * std::pow(SX, mu + 0.5) / (2.0 * (mu + 0.5));


        term1 *=
            std::pow(U * U / SX, mu + 0.5) * Nd1 - lambda1 * Nd2 - std::pow(L * L / SX, mu + 0.5) * Ne1 + lambda1 * Ne2;


        // term 2
        Real term2 =
            eta * X * std::exp(-r * T) * std::pow(S, -2.0 * (mu - 1)) * std::pow(SX, mu - 0.5) / (2.0 * (mu - 0.5));


        term2 *=
            std::pow(U * U / SX, mu - 0.5) * Nd3 - lambda2 * Nd4 - std::pow(L * L / SX, mu - 0.5) * Ne3 + lambda2 * Ne4;
    ...
    void AnalyticSoftBarrierEngine::validateInputs(Real S,
                                                   Real X,
                                                   Rate r,
                                                   Rate q,
                                                   Time T,
                                                   ...
                                                   Real sigma) const
    {
        ...
        // Barrier type checks
        QL_REQUIRE(barrierType == Barrier::DownIn || barrierType == Barrier::DownOut || barrierType == Barrier::UpIn ||
    ...
    Real AnalyticSoftBarrierEngine::standardBarrierEquivalent() const
    {
        ...
        BarrierOption tempOption(arguments_.barrierType, arguments_.barrier_hi, 0.0, payoff, arguments_.exercise);
