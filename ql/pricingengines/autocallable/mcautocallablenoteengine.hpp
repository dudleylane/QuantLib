    ...
    */
    ...
    class MCAutocallableNoteEngine : public AutocallableNote::engine
    {
      public:
        ...
        MCAutocallableNoteEngine(ext::shared_ptr<GeneralizedBlackScholesProcess> process,
                                 Size requiredSamples,
                                 BigNatural seed = 0)
        : process_(std::move(process)), requiredSamples_(requiredSamples), requiredTolerance_(0.0), maxSamples_(0),
          seed_(seed)
        {
            QL_REQUIRE(process_, "MCAutocallableNoteEngine: null process");
            QL_REQUIRE(requiredSamples_ > 0, "MCAutocallableNoteEngine: samples must be positive");
        ...
        MCAutocallableNoteEngine(ext::shared_ptr<GeneralizedBlackScholesProcess> process,
                                 ...
                                 BigNatural seed = 0)
        : process_(std::move(process)), requiredSamples_(0), requiredTolerance_(requiredTolerance),
          maxSamples_(maxSamples), minSamples_(minSamples), seed_(seed)
        {
            QL_REQUIRE(process_, "MCAutocallableNoteEngine: null process");
            QL_REQUIRE(requiredTolerance_ > 0.0, "MCAutocallableNoteEngine: requiredTolerance must be "
                                                 "positive (got "
                                                     << requiredTolerance_ << ")");
            QL_REQUIRE(maxSamples_ >= minSamples_, "MCAutocallableNoteEngine: maxSamples ("
                                                       << maxSamples_ << ") must be >= minSamples (" << minSamples_
                                                       << ")");
            registerWith(process_);
        }
        ...
        void calculate() const override
        {
            ...
            for (const Date& d : args.observationDates)
            {
                ...
                QL_REQUIRE(t > 0.0, "MCAutocallableNoteEngine: observation date "
                                        << d << " is not strictly after today (" << today << ")");
            ...
            DiscountFactor maturityDf = process_->riskFreeRate()->discount(maturityTime);

            // Path generator
            using generator_type = PathGenerator<typename RNG::rsg_type>;
            typename RNG::rsg_type rsg = RNG::make_sequence_generator(grid.size() - 1, seed_);
            ...
            auto drawOne = [&]()
            {
                ...
                stats.add(pricePath(path, obsTimes, obsDfs, maturityDf, args));
            };
            ...
            else
            {
                ...
                for (Size n = 0; n < minSamples_; ++n)
                    drawOne();
                const Size batchSize = 1024;
                while (stats.errorEstimate() > requiredTolerance_)
                {
                    QL_REQUIRE(stats.samples() < maxSamples_, "MCAutocallableNoteEngine: hit maxSamples ("
                                                                  << maxSamples_
                                                                  << ") without reaching stderr "
                                                                     "tolerance "
                                                                  << requiredTolerance_
                                                                  << "; current stderr = " << stats.errorEstimate());
                    Size remaining = maxSamples_ - stats.samples();
                    Size thisBatch = std::min(batchSize, remaining);
                    for (Size n = 0; n < thisBatch; ++n)
                        drawOne();
                }
            }
            results.value = stats.mean();
            results.errorEstimate = stats.errorEstimate();
            results.additionalResults["samples"] = static_cast<Real>(stats.samples());
        }

      private:
        static Real pricePath(const Path& path,
                              ...
                              const AutocallableNote::arguments& args)
        {
            ...
            for (Size i = 0; i < nObs; ++i)
            {
                ...
                if (S_i >= args.autocallBarriers[i] * S0)
                {
                    ...
                    return args.notional * (1.0 + args.couponRate * periods) * obsDfs[i];
                }
            }
            ...
            if (ratio >= args.protectionBarrier)
            {
                ...
                return args.notional * (1.0 + args.couponRate * periods) * maturityDf;
            }
