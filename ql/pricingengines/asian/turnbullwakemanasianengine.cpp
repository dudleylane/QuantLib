...
void TurnbullWakemanAsianEngine::calculate() const
{
    ...
    QL_REQUIRE(arguments_.averageType == Average::Type::Arithmetic, "must be Arithmetic Average::Type");
    ...
    if (effectiveStrike <= 0.0)
    {
        ...
        if (payoff->optionType() == Option::Type::Call)
        {
            ...
            {
                S_A_hat +=
                    (spot * process_->dividendYield()->discount(fd) / process_->riskFreeRate()->discount(fd)) / m;
            }
    ...
    for (const auto& fd : arguments_.fixingDates)
    {
        ...
        spotVars.push_back(process_->blackVolatility()->blackVariance(times.back(), effectiveStrike));
