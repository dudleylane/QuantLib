...
namespace QuantLib
{
    ...
    ArithmeticASOPathPricer::ArithmeticASOPathPricer(
        Option::Type type, DiscountFactor discount, Real runningSum, Size pastFixings, Size fixingCount)
    : type_(type), discount_(discount), runningSum_(runningSum), pastFixings_(pastFixings), fixingCount_(fixingCount)
    {
    }
    ...
    Real ArithmeticASOPathPricer::operator()(const Path& path) const
    {
        ...
        QL_REQUIRE(nFixings <= n, "fixingCount (" << nFixings << ") exceeds path length (" << n << ")");
        ...
        if (path.timeGrid().mandatoryTimes()[0] == 0.0)
        {
            ...
            averageStrike =
                std::accumulate(path.begin(), path.begin() + nFixings, runningSum_) / (pastFixings_ + nFixings);
        }
        else
        {
            // first path point is T=0 (not a fixing), skip it
            averageStrike =
                std::accumulate(path.begin() + 1, path.begin() + nFixings, runningSum_) / (pastFixings_ + nFixings - 1);
        }

        return discount_ * PlainVanillaPayoff(type_, averageStrike)(path.back());
    }
