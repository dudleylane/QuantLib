...
namespace QuantLib
{
    ...
    CounterpartyAdjSwapEngine::CounterpartyAdjSwapEngine(const Handle<YieldTermStructure>& discountCurve,
    ...
    : baseSwapEngine_(Handle<PricingEngine>(ext::make_shared<DiscountingSwapEngine>(discountCurve))),
      swaptionletEngine_(swaptionEngine), discountCurve_(discountCurve), defaultTS_(ctptyDTS),
      ctptyRecoveryRate_(ctptyRecoveryRate),
      invstDTS_(invstDTS.empty() ? Handle<DefaultProbabilityTermStructure>(ext::make_shared<FlatHazardRate>(
                                       0, NullCalendar(), 1.e-12, ctptyDTS->dayCounter())) :
                                   invstDTS),
    ...
    CounterpartyAdjSwapEngine::CounterpartyAdjSwapEngine(const Handle<YieldTermStructure>& discountCurve,
                                                         ...
                                                         Real invstRecoveryRate)
    : baseSwapEngine_(Handle<PricingEngine>(ext::make_shared<DiscountingSwapEngine>(discountCurve))),
      swaptionletEngine_(Handle<PricingEngine>(ext::make_shared<BlackSwaptionEngine>(discountCurve, blackVol))),
      discountCurve_(discountCurve), defaultTS_(ctptyDTS), ctptyRecoveryRate_(ctptyRecoveryRate),
      invstDTS_(invstDTS.empty() ? Handle<DefaultProbabilityTermStructure>(ext::make_shared<FlatHazardRate>(
                                       0, NullCalendar(), 1.e-12, ctptyDTS->dayCounter())) :
                                   invstDTS),
      invstRecoveryRate_(invstRecoveryRate)
    ...
    void CounterpartyAdjSwapEngine::calculate() const
    {
        ...
        QL_REQUIRE(!defaultTS_.empty(), "no ctpty default term structure set");
        QL_REQUIRE(!swaptionletEngine_.empty(), "no swap option engine set");
        ...
        Real cumOptVal = 0., cumPutVal = 0.;
        // Vanilla swap so 0 leg is floater

        auto nextFD = arguments_.fixedPayDates.begin();
        ...
        Rate baseSwapFairRate = -baseSwapRate * vSResults->legNPV[1] / vSResults->legNPV[0];
        ...
        while (nextFD != arguments_.fixedPayDates.end())
        {
            ...
            ext::shared_ptr<FloatingRateCoupon> floatCoupon =
                ext::dynamic_pointer_cast<FloatingRateCoupon>(arguments_.legs[1][0]);
            ...
            Period baseSwapsTenor(arguments_.fixedPayDates.back().serialNumber() - swapletStart.serialNumber(), Days);
            ext::shared_ptr<VanillaSwap> swaplet = MakeVanillaSwap(baseSwapsTenor, swapIndex,
            ...
            ext::shared_ptr<VanillaSwap> revSwaplet = MakeVanillaSwap(baseSwapsTenor, swapIndex,
                                                                      baseSwapFairRate // strike
                                                                      )
            ...
            Swaption swaptionlet(swaplet, ext::make_shared<EuropeanExercise>(swapletStart));
            Swaption putSwaplet(revSwaplet, ext::make_shared<EuropeanExercise>(swapletStart));
            ...
            cumOptVal += swaptionlet.NPV() * defaultTS_->defaultProbability(swapletStart, *nextFD);
            cumPutVal += putSwaplet.NPV() * invstDTS_->defaultProbability(swapletStart, *nextFD);

            swapletStart = *nextFD;
            ++nextFD;
        }

        results_.value = baseSwapNPV - (1. - ctptyRecoveryRate_) * cumOptVal + (1. - invstRecoveryRate_) * cumPutVal;

        results_.fairRate =
            -baseSwapRate *
            (vSResults->legNPV[1] - (1. - ctptyRecoveryRate_) * cumOptVal + (1. - invstRecoveryRate_) * cumPutVal) /
            vSResults->legNPV[0];
    }
