...
namespace QuantLib
{
    ...
    void IsdaCdsEngine::calculate() const
    {
        ...
        QL_REQUIRE(discountCurve_->dayCounter() == dc, "yield term structure day counter ("
        ...
        QL_REQUIRE(discountCurve_->referenceDate() == evalDate,
                   "yield term structure reference date (" << discountCurve_->referenceDate()
        ...
        QL_REQUIRE(arguments_.paysAtDefaultTime, "ISDA engine not compatible with end period payment");
        ...
        protectionNpv *= arguments_.claim->amount(Date(), arguments_.notional, recoveryRate_);
        ...
        for (auto& i : arguments_.leg)
        {
            ...
            if (!detail::simple_event(coupon->accrualEndDate()).hasOccurred(effectiveProtectionStart, false))
            {
                ...
                {
                    auto it0 = std::upper_bound(nodes.begin(), nodes.end(), start);
                    auto it1 = std::lower_bound(nodes.begin(), nodes.end(), end);
