...
namespace QuantLib
{
    ...
    Real BaroneAdesiWhaleyApproximationEngine::criticalPrice(const ext::shared_ptr<StrikedTypePayoff>& payoff,
                                                             ...
                                                             Real tolerance)
    {
        ...
        QL_REQUIRE(riskFreeDiscount <= 1.0, "Barone-Adesi-Whaley approximation is not defined for "
        ...
        Real K = (!close(riskFreeDiscount, 1.0, 1000)) ?
                     Real(-2.0 * std::log(riskFreeDiscount) / (variance * (1.0 - riskFreeDiscount))) :
                     2.0 / variance;
        Real temp =
        ...
        switch (payoff->optionType())
        {
            case Option::Call:
                ...
                {
                    QL_REQUIRE(++iter <= maxIterations, "Barone-Adesi-Whaley critical-price iteration "
                    ...
                    LHS = Si - payoff->strike();
                    Real temp2 = blackFormula(payoff->optionType(), payoff->strike(), forwardSi, std::sqrt(variance)) *
                                 riskFreeDiscount;
    ...
    void BaroneAdesiWhaleyApproximationEngine::calculate() const
    {
        ...
        else
        {
            ...
            CumulativeNormalDistribution cumNormalDist;
            Real tolerance = 1e-6;
            Real Sk = criticalPrice(payoff, riskFreeDiscount, dividendDiscount, variance, tolerance);
