...
namespace QuantLib
{
    ...
    AnalyticGJRGARCHEngine::AnalyticGJRGARCHEngine(const ext::shared_ptr<GJRGARCHModel>& model)
    : GenericModelEngine<GJRGARCHModel, VanillaOption::arguments, VanillaOption::results>(model)
    {
        init_ = false;
    }
    ...
    void AnalyticGJRGARCHEngine::calculate() const
    {
        ...
        const Rate riskFreeDiscount = process->riskFreeRate()->discount(arguments_.exercise->lastDate());
        const Rate dividendDiscount = process->dividendYield()->discount(arguments_.exercise->lastDate());
        ...
        if (!init_ || b1 != b1_ || b2 != b2_ || b3 != b3_ || la != la_)
        {
            ...
            m2 = b1 * b1 + b2 * b2 * (pow(la, 4) + 6 * la * la + 3) +
                 (b3 * b3 + 2 * b2 * b3) * (pow(la, 4) * N + pow(la, 3) * n + 6 * la * la * N + 5 * la * n + 3 * N) +
                 2 * b1 * b2 * (1 + la * la) + 2 * b3 * b1 * (la * la * N + la * n + N); // ok
            m3 = pow(b1, 3) +
                 (3 * b3 * b3 * b1 + 6 * b1 * b2 * b3) *
                     (pow(la, 3) * n + 5 * la * n + 3 * N + pow(la, 4) * N + 6 * la * la * N) +
                 pow(b2, 3) * (15 + pow(la, 6) + 15 * pow(la, 4) + 45 * la * la) +
                 (pow(b3, 3) + 3 * b2 * b2 * b3 + 3 * b3 * b3 * b2) *
                     (pow(la, 5) * n + 14 * pow(la, 3) * n + 33 * la * n + 15 * N + 15 * pow(la, 4) * N +
                      45 * la * la * N + pow(la, 6) * N) +
            ...
            v2 = -4 * b2 * b2 * (3 * la + pow(la, 3)) -
                 (4 * b3 * b3 + 8 * b2 * b3) * (la * la * n + 2 * n + pow(la, 3) * N + 3 * la * N) - 4 * b1 * b2 * la -
                 4 * b3 * b1 * (n + la * N); // ok
            v3 = -12 * b3 * b1 * (b3 + 2 * b2) * (la * la * n + 2 * n + pow(la, 3) * N + 3 * la * N) -
                 6 * pow(b2, 3) * la * (15 + pow(la, 4) + 10 * la * la) -
                 6 * b3 * (b3 * b3 + 3 * b2 * b2 + 3 * b3 * b2) *
                     (9 * la * la * n + 8 * n + 15 * la * N + pow(la, 4) * n + pow(la, 5) * N + 10 * pow(la, 3) * N) -
            ...
            z2 = b1 * b1 + b2 * b2 * (15 + pow(la, 4) + 18 * la * la) +
                 (b3 * b3 + 2 * b2 * b3) * (pow(la, 3) * n + 17 * la * n + 15 * N + pow(la, 4) * N + 18 * la * la * N) +
        ...
        if (!init_ || !constants_match || b0 != b0_ || h1 != h1_ || T != T_)
        {
            ...
            for (i = 0; i < T; ++i)
            {
                ...
                Real Eh2 = b0 * b0 * ((1 + m1) * (1 - m2i) / (1 - m2) - 2 * m1 * m1im2i / (m1 - m2)) / (1 - m1) +
                           2 * b0 * m1 * m1im2i * h1 / (m1 - m2) + m2i * h1 * h1; // ko
                Real Eh3 = pow(b0, 3) *
                               ((1 - m3i) / (1 - m3) + 3 * m2 * ((1 - m3i) / (1 - m3) - m2im3i / (m2 - m3)) / (1 - m2) +
                                ...
                                6 * m1 * m2 *
                                    ...
                                    (1 - m1)) +
                           3 * b0 * b0 * m1 * h1 *
                               (m1im3i / (m1 - m3) + 2 * m2 * (m1im3i / (m1 - m3) - m2im3i / (m2 - m3)) / (m1 - m2)) +
                           3 * b0 * m2 * h1 * h1 * m2im3i / (m2 - m3) + m3i * h1 * h1 * h1; // ko
                ...
                for (j = 0; j < T - i - 1; ++j)
                {
                    ...
                    Real Ehh2 = b0 * b0 * Eh *
                                    ((1 + m1) * (1 - m2ai[j + 1]) / (1 - m2) -
                                     2 * m1 * (m1ai[j + 1] - m2ai[j + 1]) / (m1 - m2)) /
                                    (1 - m1) +
                                2 * b0 * m1 * Eh2 * (m1ai[j + 1] - m2ai[j + 1]) / (m1 - m2) + m2ai[j + 1] * Eh3; // ko
                    Real Eh2h = b0 * Eh2 * (1 - m1ai[j + 1]) / (1 - m1) + m1ai[j + 1] * Eh3;                     // ok
                    Real Eh1_2eh = v1 * m1ai[j] * Eh3_2;                                                         // ko
                    Real Eh1_2eh2 =
                        2 * b0 * v1 * (m1ai[j + 1] - m2ai[j + 1]) * Eh3_2 / (m1 - m2) + v2 * m2ai[j] * Eh5_2; // ko
                    Real Ehij = b0 * (1 - m1ai[i + j + 1]) / (1 - m1) + m1ai[i + j + 1] * h1;                 // ko
                    Real Ehh3_2 = 0.375 * Ehh2 / std::sqrt(Ehij) + 0.75 * std::sqrt(Ehij) * Ehh -
                                  0.125 * std::pow(Ehij, 1.5) * Eh;                                          // ko
                    Real Eh3_2eh = v1 * m1ai[j] * Eh5_2;                                                     // ko
                    Real Eh3_2e3h = x1 * m1ai[j] * Eh5_2;                                                    // ok
                    Real Eh1_2eh3_2 = 0.375 * Eh1_2eh2 / std::sqrt(Ehij) + 0.75 * std::sqrt(Ehij) * Eh1_2eh; // ko
                    sEhh += Ehh;
                    sEh1_2eh += Eh1_2eh;
                    sEhh2 += Ehh2;
                    sEh2h += Eh2h;
                    sEh1_2eh2 += Eh1_2eh2;
                    sEh3_2eh += Eh3_2eh;
                    sEhe2h += b0 * Eh * (1 - m1ai[j + 1]) / (1 - m1) + z1 * m1ai[j] * Eh2; // ko
                    sEh3_2e3h += Eh3_2e3h;                                                 // ok
                    for (k = 0; k < T - i - j - 2; ++k)
                    {
                        Real Ehhh = b0 * Ehh * (1 - m1ai[k + 1]) / (1 - m1) + m1ai[k + 1] * Ehh2;             // ko
                        Real Eh1_2ehh = b0 * Eh1_2eh * (1 - m1ai[k + 1]) / (1 - m1) + m1ai[k + 1] * Eh1_2eh2; // ko
                        sEhhh += Ehhh;
                        sEh1_2ehh += Eh1_2ehh;
            ...
            ex3 = pow(T * r, 3) - 1.5 * T * T * r * r * sEh + 3 * T * r * (SD1 / 4 + SD2 - SD3) +
                  (ST2 - ST1 / 8 + 3 * ST3 / 4 - 3 * ST4 / 2);
            SQ2 = 6 * sEhe2h + (12 * sEh1_2eh1_2eh + 3 * sEh2);
            SQ4 = 2 * sEhhh + 2 * sEhh2;
            SQ5 = 3 * sEhh1_2eh + 3 * sEh1_2ehh + 3 * sEh3_2eh + 3 * sEh1_2eh2 + sEh3_2e3h;
            ex4 = pow(T * r, 4) - 2 * pow(T * r, 3) * sEh + 6 * T * T * r * r * (SD1 / 4 + SD2 - SD3) +
                  T * r * (4 * ST2 - ST1 / 2 + 3 * ST3 - 6 * ST4) + (SQ2 + 3 * SQ4 / 2 - 2 * SQ5);
        ...
        A3 = s * std::exp(del * stdev) * stdev *
             ((2 * stdev - d_) * std::exp(-d_ * d_ / 2) / std::sqrt(2 * M_PI) +
              sigma * CumulativeNormalDistribution()(d_)) /
             6;
        A4 = s * std::exp(del * stdev) * stdev *
             ((d_ * d_ - 1 - 3 * stdev * (d_ - stdev)) * exp(-d_ * d_ / 2) / std::sqrt(2 * M_PI) -
              sigma * stdev * CumulativeNormalDistribution()(d_)) /
             24;
        ...
        switch (payoff->optionType())
        {
            case Option::Call:
                ...
                break;
            case Option::Put:
                results_.value = Capp + strikePrice * riskFreeDiscount / dividendDiscount - spotPrice;
                break;
            default:
