...
namespace QuantLib
{
    ...
    {
      public:
        QdPlusBoundaryEvaluator(Real S, Real strike, Rate rf, Rate dy, Volatility vol, Time t, Time T)
        : tau(t), K(strike), sigma(vol), sigma2(sigma * sigma), v(sigma * std::sqrt(tau)), r(rf), q(dy),
          dr(std::exp(-r * tau)), dq(std::exp(-q * tau)),
          ddr((std::abs(r * tau) > 1e-5) ? Real(r / (1 - dr)) :
                                           Real(1 / (tau * (1 - 0.5 * r * tau * (1 - r * tau / 3))))),
          omega(2 * (r - q) / sigma2), lambda(0.5 * (-(omega - 1) - std::sqrt(squared(omega - 1) + 8 * ddr / sigma2))),
          lambdaPrime(2 * ddr * ddr / (sigma2 * std::sqrt(squared(omega - 1) + 8 * ddr / sigma2))),
          alpha(2 * dr / (sigma2 * (2 * lambda + omega - 1))),
          beta(alpha * (ddr + lambdaPrime / (2 * lambda + omega - 1)) - lambda),
          xMax(QdPlusAmericanEngine::xMax(strike, r, q)), xMin(QL_EPSILON * 1e4 * std::min(0.5 * (strike + S), xMax)),
        ...
        Real derivative(Real S) const
        {
            ...
            return 1 - dq * Phi_dp + dq / v * phi_dp + beta * (1 - dq * Phi_dp) + alpha / dr * charm;
        }
    ...
    detail::QdPlusAddOnValue::QdPlusAddOnValue(
        Time T, Real S, Real K, Rate r, Rate q, Volatility vol, const Real xmax, ext::shared_ptr<Interpolation> q_z)
    : T_(T), S_(S), K_(K), xmax_(xmax), r_(r), q_(q), vol_(vol), q_z_(std::move(q_z))
    {
    }
    ...
    detail::QdPutCallParityEngine::QdPutCallParityEngine(ext::shared_ptr<GeneralizedBlackScholesProcess> process)
    ...
    void detail::QdPutCallParityEngine::calculate() const
    {
        ...
        const auto payoff = ext::dynamic_pointer_cast<StrikedTypePayoff>(arguments_.payoff);
        ...
        Real S, Real K, Rate r, Rate q, Volatility vol, Time T) const
    {
        ...
        if (r <= 0.0 && r <= q)
            return std::max(
                0.0, BlackCalculator(Option::Put, K, S * std::exp((r - q) * T), vol * std::sqrt(T), std::exp(-r * T))
                         .value());
        ...
        if (close(vol, 0.0))
        {
            ...
            const Real extremT = close_enough(r, q) ? QL_MAX_REAL : Real(std::log(r * K / (q * S)) / (r - q));
    ...
    QdPlusAmericanEngine::QdPlusAmericanEngine(ext::shared_ptr<GeneralizedBlackScholesProcess> process,
                                               Size interpolationPoints,
                                               QdPlusAmericanEngine::SolverType solverType,
                                               Real eps,
                                               Size maxIter)
    : detail::QdPutCallParityEngine(std::move(process)), interpolationPoints_(interpolationPoints),
      solverType_(solverType), eps_(eps),
      maxIter_((maxIter == Null<Size>()) ?
                   ((solverType == Newton || solverType == Brent || solverType == Ridder) ? 100 : 10) :
                   maxIter)
    {
    }
    ...
    Real QdPlusAmericanEngine::buildInSolver(
        const QdPlusBoundaryEvaluator& eval, Solver solver, Real S, Real strike, Size maxIter, Real guess) const
    {
    ...
    std::pair<Size, Real> QdPlusAmericanEngine::putExerciseBoundaryAtTau(
        Real S, Real K, Rate r, Rate q, Volatility vol, Time T, Time tau) const
    {

        if (tau < QL_EPSILON)
            return std::pair<Size, Real>(Size(0), xMax(K, r, q));
        ...
        switch (solverType_)
        {
            ...
            case SuperHalley:
            {
                ...
                do
                {
                    ...
                    const Real step = (solverType_ == Halley) ? Real(1 / (1 - 0.5 * lf) * fx / fPrime) :
    ...
    QdPlusAmericanEngine::getPutExerciseBoundary(Real S, Real K, Rate r, Rate q, Volatility vol, Time T) const
    {
        ...
        return ext::make_shared<ChebyshevInterpolation>(
            ...
            [&, this](Real z)
            {
                ...
                return squared(std::log(this->putExerciseBoundaryAtTau(S, K, r, q, vol, T, x_sq).second / xmax));
            },
            ChebyshevInterpolation::SecondKind);
    }

    Real QdPlusAmericanEngine::calculatePut(Real S, Real K, Rate r, Rate q, Volatility vol, Time T) const
    {

        if (r < 0.0 && q < r)
            QL_FAIL("double-boundary case q<r<0 for a put option is given");

        const ext::shared_ptr<Interpolation> q_z = getPutExerciseBoundary(S, K, r, q, vol, T);
...
#else
        const Real addOn = GaussLobattoIntegral(100000, QL_MAX_REAL, 0.1 * eps_)(aov, 0.0, std::sqrt(T));
#endif
