...
namespace QuantLib
{
    CashDividendEuropeanEngine::CashDividendEuropeanEngine(ext::shared_ptr<GeneralizedBlackScholesProcess> process,
                                                           DividendSchedule dividends,
                                                           CashDividendModel cashDividendModel)
    : process_(std::move(process)), dividends_(std::move(dividends)), cashDividendModel_(cashDividendModel)
    {
    ...
    void CashDividendEuropeanEngine::calculate() const
    {
        ...
        DividendSchedule dividends;
        std::copy_if(dividends_.begin(), dividends_.end(), std::back_inserter(dividends),
                     [settlementDate, maturityDate](const ext::shared_ptr<Dividend>& div) -> bool
                     { return (div->date() >= settlementDate && div->date() <= maturityDate && div->amount() > 0.0); });
        std::sort(dividends.begin(), dividends.end(),
                  [](const ext::shared_ptr<Dividend>& d1, const ext::shared_ptr<Dividend>& d2)
                  { return d1->date() < d2->date(); });

        if (cashDividendModel_ == Escrowed || (dividends.size() == 1 && dividends.back()->date() == settlementDate))
        {
            VanillaOption option(payoff, exercise);
            option.setPricingEngine(ext::make_shared<AnalyticDividendEuropeanEngine>(process_, dividends_));
        ...
        {
            underlyings.back() = ext::make_shared<FixedDividend>(underlyings.back()->amount() + strike, maturityDate);
        }
        else
        ...
        if (underlyings.size() == 1)
        {
            VanillaOption option(
                ext::make_shared<PlainVanillaPayoff>(payoff->optionType(), underlyings.back()->amount()), exercise);
        ...
        for (const auto& div : underlyings)
        {
            ...
            processes.emplace_back(ext::make_shared<GeneralizedBlackScholesProcess>(
                ...
                Handle<YieldTermStructure>(
                    ext::make_shared<FlatForward>(settlementDate, q_mod - r_mod, rTS->dayCounter())),
                zeroRateTS,
                Handle<BlackVolTermStructure>(ext::make_shared<BlackConstantVol>(
                    volRefDate, volTS->calendar(),
                    Handle<Quote>(
                        ext::make_shared<SimpleQuote>(std::sqrt(volTS->blackVariance(div->date(), strike) / maturity))),
                    volDc))));
        }
        ...
        BasketOption basketOption(
            ext::make_shared<AverageBasketPayoff>(ext::make_shared<PlainVanillaPayoff>(Option::Put, process_->x0()),
                                                  Array(underlyings.size(), 1.0)),
            ext::make_shared<EuropeanExercise>(maturityDate));

        basketOption.setPricingEngine(ext::make_shared<ChoiBasketEngine>(processes, rho, 10, 2000, false, true));
        ...
        else
        {
            ...
            const Real fwd =
                (process_->x0() - divDiscounted) * qTS->discount(maturityDate) / rTS->discount(maturityDate);
