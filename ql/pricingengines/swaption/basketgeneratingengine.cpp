...
namespace QuantLib
{
    ...
    BasketGeneratingEngine::calibrationBasket(const ext::shared_ptr<Exercise>& exercise,
                                              ...
                                              const CalibrationBasketType basketType) const
    {
        ...
        Size minIdxAlive = static_cast<Size>(
            std::upper_bound(exercise->dates().begin(), exercise->dates().end(), today) - exercise->dates().begin());

        ext::shared_ptr<RebatedExercise> rebEx = ext::dynamic_pointer_cast<RebatedExercise>(exercise);
            ...
            switch (basketType)
            {
                ...
                case Naive:
                {
                    Real swapLength = swaptionVolatility->dayCounter().yearFraction(standardSwapBase->valueDate(expiry),
                                                                                    underlyingLastDate());
                    ext::shared_ptr<SmileSection> sec = swaptionVolatility->smileSection(
                        expiry, static_cast<Size>(std::lround(swapLength * 12.0)) * Months, true);
                    ...
                    helper = ext::make_shared<SwaptionHelper>(
                        expiry, underlyingLastDate(), Handle<Quote>(ext::make_shared<SimpleQuote>(atmVol)),
                ...
                case MaturityStrikeByDeltaGamma:
                {
                    ...
                    Real zSpreadDsc =
                        oas_.empty() ?
                            Real(1.0) :
                            exp(-oas_->value() *
                                onefactormodel_->termStructure()->dayCounter().yearFraction(expiry, rebateDate));
                    ...
                    matchHelper_ = ext::make_shared<MatchHelper>(underlyingType(), npv, delta, gamma, *onefactormodel_,
                    ...
                    ext::shared_ptr<SmileSection> sec = swaptionVolatility->smileSection(expiry, matPeriod, true);
                    Real shift = sec->shift();

                    // we have to floor the strike of the calibration instrument,
                    // see warning in the header
                    solution[2] = std::max(solution[2], 0.00001 - shift); // floor at 0.1bp - shift
