...
namespace QuantLib
{
    ...
    namespace detail
    {
        ...
        struct Black76Spec
        {
            ...
            Real delta(const Option::Type type,
                       ...
                       const Real displacement)
            {
                return blackFormulaForwardDerivative(type, strike, atmForward, stdDev, annuity, displacement);
            }
        };
        ...
        struct BachelierSpec
        {
            ...
            Real delta(const Option::Type type,
                       ...
                       const Real)
            {
                return bachelierBlackFormulaForwardDerivative(type, strike, atmForward, stdDev, annuity);
            }
        };
    ...
    */
    ...
    class BlackSwaptionEngine : public detail::BlackStyleSwaptionEngine<detail::Black76Spec>
    {
      public:
    ...
    */
    ...
    class BachelierSwaptionEngine : public detail::BlackStyleSwaptionEngine<detail::BachelierSpec>
    {
      public:
    ...
    namespace detail
    {
        ...
        template <class Spec>
        BlackStyleSwaptionEngine<Spec>::BlackStyleSwaptionEngine(Handle<YieldTermStructure> discountCurve,
                                                                 Volatility vol,
                                                                 const DayCounter& dc,
                                                                 Real displacement,
        ...
        void BlackStyleSwaptionEngine<Spec>::calculate() const
        {
            ...
            ext::shared_ptr<FixedRateCoupon> firstCoupon = ext::dynamic_pointer_cast<FixedRateCoupon>(fixedLeg[0]);
            QL_REQUIRE(firstCoupon->accrualStartDate() >= exerciseDate,
                       "swap start (" << firstCoupon->accrualStartDate() << ") before exercise date (" << exerciseDate
            ...
            if (spread != 0.0)
            {
                Spread correction = spread * std::fabs(swap->floatingLegBPS() / swap->fixedLegBPS());
                strike -= correction;
            ...
            if (arguments_.settlementType == Settlement::Physical ||
                (arguments_.settlementType == Settlement::Cash &&
                 arguments_.settlementMethod == Settlement::CollateralizedCashPrice))
            {
                     ...
                     arguments_.settlementMethod == Settlement::ParYieldCurve)
            {
                ...
                Date discountDate = model_ == DiscountCurve ? firstCoupon->accrualStartDate() : valuation_date;
                ...
                Real fixedLegCashBPS =
                    CashFlows::bps(fixedLeg, InterestRate(atmForward, dayCount, Compounded, freq), false, discountDate);
                annuity = std::fabs(fixedLegCashBPS / basisPoint) * discountCurve_->discount(discountDate);
            }
            else
            {
            ...
            Time swapLength = vol_->swapLength(floatingSchedule.dates().front(), floatingSchedule.dates().back());
            ...
            Real displacement =
                vol_->volatilityType() == ShiftedLognormal ? vol_->shift(exerciseDate, swapLength) : 0.0;
            ...
            results_.additionalResults["delta"] = Spec().delta(w, strike, atmForward, stdDev, annuity, displacement);
