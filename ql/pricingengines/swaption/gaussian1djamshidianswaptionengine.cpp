...
namespace QuantLib
{
    ...
    class Gaussian1dJamshidianSwaptionEngine::rStarFinder
    {
      public:
        ...
        : strike_(nominal), maturityDate_(maturityDate), valueDate_(valueDate), startIndex_(startIndex),
    ...
    void Gaussian1dJamshidianSwaptionEngine::calculate() const
    {
        ...
        QL_REQUIRE(arguments_.swap->spread() == 0.0,
                   "non zero spread (" << arguments_.swap->spread() << ") not allowed"); // PC
        ...
        Size startIndex = std::upper_bound(arguments_.fixedResetDates.begin(), arguments_.fixedResetDates.end(),
                                           arguments_.exercise->date(0) - 1) -
                          arguments_.fixedResetDates.begin();
        // only consider coupons with start date >= exercise dates

        rStarFinder finder(*model_, arguments_.nominal, arguments_.exercise->date(0),
                           arguments_.fixedResetDates[startIndex], arguments_.fixedPayDates, amounts, startIndex);
        Brent s1d;
        Rate minStrike = -8.0;
        Rate maxStrike = 8.0;
        ...
        for (Size i = startIndex; i < size; i++)
        {
            ...
            Real dboValue =
                model_->zerobondOption(w, arguments_.exercise->date(0), arguments_.fixedResetDates[startIndex],
