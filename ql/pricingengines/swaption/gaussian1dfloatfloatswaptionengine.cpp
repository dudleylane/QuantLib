...
namespace QuantLib
{
    ...
    void Gaussian1dFloatFloatSwaptionEngine::calculate() const
    {
        ...
        rebatedExercise_ = ext::dynamic_pointer_cast<RebatedExercise>(arguments_.exercise);
    ...
    Real Gaussian1dFloatFloatSwaptionEngine::underlyingNpv(const Date& expiry, const Real y) const
    {
    ...
    const Array Gaussian1dFloatFloatSwaptionEngine::initialGuess(const Date& expiry) const
    {
        ...
        return {
            nominalAvg1, weightedMaturity1,
            0.03 // ???
        };
    }

    // calculate npv and underlying npv as of expiry date
    std::pair<Real, Real> Gaussian1dFloatFloatSwaptionEngine::npvs(const Date& expiry,
                                                                   const Real y,
                                                                   ...
                                                                   const bool considerProbabilities) const
    {
        ...
        auto filit = std::upper_bound(events.begin(), events.end(), expiry - (includeExerciseOnExpiry ? 1 : 0));
        events.erase(events.begin(), filit);

        int idx = events.size() - 1;

        FloatFloatSwap swap = *arguments_.swap;
        Option::Type type = arguments_.type == Swap::Payer ? Option::Call : Option::Put;

        Array npv0(2 * integrationPoints_ + 1, 0.0), npv1(2 * integrationPoints_ + 1, 0.0);   // arrays for npvs of the
                                                                                              // option
        Array npv0a(2 * integrationPoints_ + 1, 0.0), npv1a(2 * integrationPoints_ + 1, 0.0); // arrays for npvs of the
                                                                                              // underlying
        ...
        Size noEx = arguments_.exercise->dates().size() -
                    (std::upper_bound(arguments_.exercise->dates().begin(), arguments_.exercise->dates().end(),
        ...
        ext::shared_ptr<IborIndex> ibor1 = ext::dynamic_pointer_cast<IborIndex>(arguments_.index1);
        ext::shared_ptr<SwapIndex> cms1 = ext::dynamic_pointer_cast<SwapIndex>(arguments_.index1);
        ext::shared_ptr<SwapSpreadIndex> cmsspread1 = ext::dynamic_pointer_cast<SwapSpreadIndex>(arguments_.index1);
        ext::shared_ptr<IborIndex> ibor2 = ext::dynamic_pointer_cast<IborIndex>(arguments_.index2);
        ext::shared_ptr<SwapIndex> cms2 = ext::dynamic_pointer_cast<SwapIndex>(arguments_.index2);
        ext::shared_ptr<SwapSpreadIndex> cmsspread2 = ext::dynamic_pointer_cast<SwapSpreadIndex>(arguments_.index2);
        ...
        do
        {
            ...
            bool isExercise = std::find(arguments_.exercise->dates().begin(), arguments_.exercise->dates().end(),
                                        event0) != arguments_.exercise->dates().end();

            bool isLeg1Fixing = std::find(arguments_.leg1FixingDates.begin(), arguments_.leg1FixingDates.end(),
                                          event0) != arguments_.leg1FixingDates.end();

            bool isLeg2Fixing = std::find(arguments_.leg2FixingDates.begin(), arguments_.leg2FixingDates.end(),
            ...
            for (Size k = 0; k < (event0 > expiry ? npv0.size() : 1); k++)
            {
                ...
                {
                    Real zSpreadDf = oas_.empty() ? Real(1.0) : std::exp(-oas_->value() * (event1Time - event0Time));
                    Array yg =
                        model_->yGrid(stddevs_, integrationPoints_, event1Time, event0Time, event0 > expiry ? z[k] : y);
                    ...
                    CubicInterpolation payoff0a(z.begin(), z.end(), npv1a.begin(), CubicInterpolation::Spline, true,
                                                CubicInterpolation::Lagrange, 0.0, CubicInterpolation::Lagrange, 0.0);
                    ...
                    CubicInterpolation payoff1(z.begin(), z.end(), p.begin(), CubicInterpolation::Spline, true,
                                               CubicInterpolation::Lagrange, 0.0, CubicInterpolation::Lagrange, 0.0);
                    CubicInterpolation payoff1a(z.begin(), z.end(), pa.begin(), CubicInterpolation::Spline, true,
                                                CubicInterpolation::Lagrange, 0.0, CubicInterpolation::Lagrange, 0.0);
                    for (Size i = 0; i < z.size() - 1; i++)
                    {
                        price += Gaussian1dModel::gaussianShiftedPolynomialIntegral(
                                     0.0, payoff1.cCoefficients()[i], payoff1.bCoefficients()[i],
                                     payoff1.aCoefficients()[i], p[i], z[i], z[i], z[i + 1]) *
                                 zSpreadDf;
                        pricea += Gaussian1dModel::gaussianShiftedPolynomialIntegral(
                                      0.0, payoff1a.cCoefficients()[i], payoff1a.bCoefficients()[i],
                                      payoff1a.aCoefficients()[i], pa[i], z[i], z[i], z[i + 1]) *
                                  zSpreadDf;
                    }
                    if (extrapolatePayoff_)
                    {
                        ...
                        {
                            price += Gaussian1dModel::gaussianShiftedPolynomialIntegral(
                                         0.0, 0.0, 0.0, 0.0, p[z.size() - 2], z[z.size() - 2], z[z.size() - 1], 100.0) *
                                     zSpreadDf;
                            price += Gaussian1dModel::gaussianShiftedPolynomialIntegral(0.0, 0.0, 0.0, 0.0, p[0], z[0],
                                                                                        -100.0, z[0]) *
                                     zSpreadDf;
                            pricea +=
                                Gaussian1dModel::gaussianShiftedPolynomialIntegral(
                                    0.0, 0.0, 0.0, 0.0, pa[z.size() - 2], z[z.size() - 2], z[z.size() - 1], 100.0) *
                                zSpreadDf;
                            ...
                            if (type == Option::Call)
                                price +=
                                    Gaussian1dModel::gaussianShiftedPolynomialIntegral(
                                        ...
                                        payoff1.bCoefficients()[z.size() - 2], payoff1.aCoefficients()[z.size() - 2],
                            ...
                            if (type == Option::Call)
                                pricea +=
                                    Gaussian1dModel::gaussianShiftedPolynomialIntegral(
                                        ...
                                        payoff1a.bCoefficients()[z.size() - 2], payoff1a.aCoefficients()[z.size() - 2],
                                        pa[z.size() - 2], z[z.size() - 2], z[z.size() - 1], 100.0) *
                                    zSpreadDf;
                            if (type == Option::Put)
                                pricea += Gaussian1dModel::gaussianShiftedPolynomialIntegral(
                                              0.0, payoff1a.cCoefficients()[0], payoff1a.bCoefficients()[0],
                                              payoff1a.aCoefficients()[0], pa[0], z[0], -100.0, z[0]) *
                                          zSpreadDf;
                        }
                    }
                }
                ...
                if (considerProbabilities && probabilities_ != None)
                {
                    for (Size m = 0; m < npvp0.size(); m++)
                    {
                        ...
                        {
                            Real zSpreadDf =
                                oas_.empty() ? Real(1.0) : std::exp(-oas_->value() * (event1Time - event0Time));
                            Array yg = model_->yGrid(stddevs_, integrationPoints_, event1Time, event0Time,
                                                     event0 > expiry ? z[k] : 0.0);
                            CubicInterpolation payoff0(z.begin(), z.end(), npvp1[m].begin(), CubicInterpolation::Spline,
                                                       true, CubicInterpolation::Lagrange, 0.0,
                            ...
                            CubicInterpolation payoff1(z.begin(), z.end(), p.begin(), CubicInterpolation::Spline, true,
                                                       CubicInterpolation::Lagrange, 0.0, CubicInterpolation::Lagrange,
                                                       0.0);
                            for (Size i = 0; i < z.size() - 1; i++)
                            {
                                price += Gaussian1dModel::gaussianShiftedPolynomialIntegral(
                            ...
                            if (extrapolatePayoff_)
                            {
                                ...
                                {
                                    price += Gaussian1dModel::gaussianShiftedPolynomialIntegral(
                                                 0.0, 0.0, 0.0, 0.0, p[z.size() - 2], z[z.size() - 2], z[z.size() - 1],
                                                 100.0) *
                                             zSpreadDf;
                                    price += Gaussian1dModel::gaussianShiftedPolynomialIntegral(
                ...
                if (isEventDate)
                {
                    ...
                    if (isLeg1Fixing)
                    { // if event is a fixing date and
                      // exercise date,
                        // the coupon is part of the exercise into right (by
                        // definition)
                        Size j =
                            std::find(arguments_.leg1FixingDates.begin(), arguments_.leg1FixingDates.end(), event0) -
                            arguments_.leg1FixingDates.begin();
                        Real zSpreadDf =
                            oas_.empty() ?
                                Real(1.0) :
                                std::exp(-oas_->value() * (model_->termStructure()->dayCounter().yearFraction(
                                                              event0, arguments_.leg1PayDates[j])));
                        bool done = false;
                        do
                        {
                            Real amount;
                            ...
                            else
                            {
                                ...
                                if (cms1 != nullptr)
                                {
                                    estFixing = model_->swapRate(arguments_.leg1FixingDates[j], cms1->tenor(), event0,
                                                                 zk, cms1);
                                }
                                if (cmsspread1 != nullptr)
                                    estFixing =
                                        cmsspread1->gearing1() * model_->swapRate(arguments_.leg1FixingDates[j],
                                                                                  cmsspread1->swapIndex1()->tenor(),
                                                                                  event0, zk,
                                                                                  cmsspread1->swapIndex1()) +
                                        cmsspread1->gearing2() * model_->swapRate(arguments_.leg1FixingDates[j],
                            ...
                            npv0a[k] -= amount *
                                        model_->zerobond(arguments_.leg1PayDates[j], event0, zk, discountCurve_) /
                    ...
                    { // if event is a fixing date and
                      // exercise date,
                        ...
                        // definition)
                        Size j =
                            std::find(arguments_.leg2FixingDates.begin(), arguments_.leg2FixingDates.end(), event0) -
                        ...
                        do
                        {
                            ...
                            else
                            {
                                ...
                                if (cms2 != nullptr)
                                    estFixing = model_->swapRate(arguments_.leg2FixingDates[j], cms2->tenor(), event0,
                                                                 zk, cms2);
                                if (cmsspread2 != nullptr)
                                    estFixing =
                                        cmsspread2->gearing1() * model_->swapRate(arguments_.leg2FixingDates[j],
                                                                                  cmsspread2->swapIndex1()->tenor(),
                                                                                  event0, zk,
                                                                                  cmsspread2->swapIndex1()) +
                                        cmsspread2->gearing2() * model_->swapRate(arguments_.leg2FixingDates[j],
                            ...
                            npv0a[k] += amount *
                                        model_->zerobond(arguments_.leg2PayDates[j], event0, zk, discountCurve_) /
                                        model_->numeraire(event0Time, zk, discountCurve_) * zSpreadDf;
                    ...
                    if (isExercise)
                    {
                        Size j = std::find(arguments_.exercise->dates().begin(), arguments_.exercise->dates().end(),
                                           event0) -
                                 arguments_.exercise->dates().begin();
                        Real rebate = 0.0;
                        Real zSpreadDf = 1.0;
                        ...
                        if (rebatedExercise_ != nullptr)
                        {
                            ...
                            zSpreadDf =
                                oas_.empty() ?
                                    Real(1.0) :
                                    std::exp(-oas_->value() *
                                             (model_->termStructure()->dayCounter().yearFraction(event0, rebateDate)));
                        }
                        Real exerciseValue = (type == Option::Call ? 1.0 : -1.0) * npv0a[k] +
                                             rebate * model_->zerobond(rebateDate, event0) * zSpreadDf /
                        ...
                        if (considerProbabilities && probabilities_ != None)
                        {
                            if (exIdx == noEx)
                            {
                                ...
                                npvp0.back()[k] = probabilities_ == Naive ?
                                                      Real(1.0) :
                                                      1.0 / (model_->zerobond(event0Time, 0.0, 0.0, discountCurve_) *
                                                             model_->numeraire(event0, z[k], discountCurve_));
                            }
        ...
        std::pair<Real, Real> res(npv1[0] * model_->numeraire(event1Time, y, discountCurve_),
