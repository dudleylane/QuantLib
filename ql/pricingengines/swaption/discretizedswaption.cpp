...
namespace QuantLib
{
    ...
    : DiscretizedOption(ext::shared_ptr<DiscretizedAsset>(), args.exercise->type(), std::vector<Time>()),
      arguments_(args)
    {
        ...
        prepareSwaptionWithSnappedDates(arguments_, referenceDate, dayCounter, snappedArgs, fixedCouponAdjustments,
                                        floatingCouponAdjustments);

        exerciseTimes_.resize(snappedArgs.exercise->dates().size());
        for (Size i = 0; i < exerciseTimes_.size(); ++i)
            exerciseTimes_[i] = dayCounter.yearFraction(referenceDate, snappedArgs.exercise->date(i));

        Time lastFixedPayment = dayCounter.yearFraction(referenceDate, snappedArgs.fixedPayDates.back());
        Time lastFloatingPayment = dayCounter.yearFraction(referenceDate, snappedArgs.floatingPayDates.back());
        lastPayment_ = std::max(lastFixedPayment, lastFloatingPayment);

        underlying_ = ext::make_shared<DiscretizedSwap>(snappedArgs, referenceDate, dayCounter, fixedCouponAdjustments,
                                                        floatingCouponAdjustments);
    }
    ...
    void DiscretizedSwaption::prepareSwaptionWithSnappedDates(const Swaption::arguments& args,
                                                              const Date& referenceDate,
                                                              const DayCounter& dayCounter,
                                                              ...
                                                              std::vector<CouponAdjustment>& floatingCouponAdjustments)
    {
        ...
        auto snappedSwap = ext::make_shared<VanillaSwap>(
            ...
            args.swap->fixedDayCount(), snappedFloatSchedule, args.swap->iborIndex(), args.swap->spread(),
            args.swap->floatingDayCount(), args.swap->paymentConvention());

        Swaption snappedSwaption(snappedSwap, args.exercise, args.settlementType, args.settlementMethod);
