...
namespace QuantLib
{
    ...
    class JamshidianSwaptionEngine::rStarFinder
    {
      public:
        ...
        : strike_(nominal), maturity_(maturity), valueTime_(valueTime), times_(std::move(fixedPayTimes)),
        ...
        Real operator()(Rate x) const
        {
            ...
            {
                Real dbValue = model_->discountBond(maturity_, times_[i], x) / B;
    ...
    void JamshidianSwaptionEngine::calculate() const
    {
        ...
        QL_REQUIRE(arguments_.exercise->type() == Exercise::European, "cannot use the Jamshidian decomposition "
                                                                      "on exotic swaptions");
        ...
        Real maturity = dayCounter.yearFraction(referenceDate, arguments_.exercise->date(0));
        ...
        rStarFinder finder(*model_, arguments_.nominal, maturity, valueTime, fixedPayTimes, amounts);
        Brent s1d;
        Rate minStrike = -10.0;
        Rate maxStrike = 10.0;
        ...
        for (Size i = 0; i < size; i++)
        {
            ...
            Real strike = model_->discountBond(maturity, fixedPayTime, rStar) / B;
            // Looks like the swaption decomposed into individual options adjusted for maturity. Each individual option
            // is valued by Hull-White (or other one-factor model).
            Real dboValue = model_->discountBondOption(w, strike, maturity, valueTime, fixedPayTime);
