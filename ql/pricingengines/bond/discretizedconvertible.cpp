...
namespace QuantLib
{
    ...
    DiscretizedConvertible::DiscretizedConvertible(ConvertibleBond::arguments args,
                                                   ext::shared_ptr<GeneralizedBlackScholesProcess> process,
                                                   DividendSchedule dividends,
                                                   Handle<Quote> creditSpread,
                                                   const TimeGrid& grid)
    : arguments_(std::move(args)), process_(std::move(process)), creditSpread_(std::move(creditSpread))
    {
        ...
        for (Size i = 0; i < dividends.size(); i++)
        {
            ...
            {
                dividendValues_[i] = dividends[i]->amount() * process_->riskFreeRate()->discount(dividends[i]->date());
            }
        }
        ...
        for (Size i = 0; i < arguments_.cashflows.size() - 1; ++i)
        {
            ...
            {
                couponTimes_.push_back(dayCounter.yearFraction(bondSettlement, arguments_.cashflows[i]->date()));
        ...
        for (Size i = 0; i < dividendTimes_.size(); ++i)
            dividendTimes_[i] = dayCounter.yearFraction(bondSettlement, dividendDates_[i]);
    ...
    void DiscretizedConvertible::reset(Size size)
    {
        ...
        Rate riskFreeRate = process_->riskFreeRate()->zeroRate(exercise, rfdc, Continuous, NoFrequency);

        // Calculate blended discount rate to be used on roll back.
        for (Size j = 0; j < values_.size(); j++)
        {
            spreadAdjustedRate_[j] = conversionProbability_[j] * riskFreeRate +
    ...
    void DiscretizedConvertible::applyCallability(Size i, bool convertible)
    {
        ...
        switch (arguments_.callabilityTypes[i])
        {
            case Callability::Call:
                ...
                {
                    Real conversionValue = arguments_.redemption / arguments_.conversionRatio;
                    Real trigger = conversionValue * arguments_.callabilityTriggers[i];
                    for (j = 0; j < values_.size(); j++)
                    {
                        ...
                        if (grid[j] >= trigger)
                        {
                            ...
                            values_[j] = std::min(
                                std::max(arguments_.callabilityPrices[i], arguments_.conversionRatio * grid[j]),
                                values_[j]);
                        }
                    }
                }
                else if (convertible)
                {
                    for (j = 0; j < values_.size(); j++)
                    {
                        ...
                        values_[j] =
                            std::min(std::max(arguments_.callabilityPrices[i], arguments_.conversionRatio * grid[j]),
                                     values_[j]);
                    }
                }
                else
