...
namespace QuantLib
{
    ...
    BlackCalculator::BlackCalculator(const ext::shared_ptr<StrikedTypePayoff>& p,
                                     ...
                                     Real stdDev,
                                     Real discount)
    : strike_(p->strike()), forward_(forward), stdDev_(stdDev), discount_(discount), variance_(stdDev * stdDev)
    {
        initialize(p);
    }

    BlackCalculator::BlackCalculator(Option::Type optionType, Real strike, Real forward, Real stdDev, Real discount)
    : strike_(strike), forward_(forward), stdDev_(stdDev), discount_(discount), variance_(stdDev * stdDev)
    {
    ...
    Real BlackCalculator::gamma(Real spot) const
    {
        ...
        Real temp2 = D2alphaDs2 * forward_ + 2.0 * DalphaDs * DforwardDs + D2betaDs2 * x_ + 2.0 * DbetaDs * DxDs_;
    ...
    Real BlackCalculator::gammaForward() const
    {
        ...
        Real temp2 = D2alphaDforward2 * forward_ + 2.0 * DalphaDforward + D2betaDforward2 * x_; // DXDforward = 0.0
    ...
    Real BlackCalculator::strikeGamma() const
    {
        ...
        Real temp2 = D2alphaD2strike * forward_ + D2betaD2strike * x_ + 2.0 * DbetaDstrike * DxDstrike_;
