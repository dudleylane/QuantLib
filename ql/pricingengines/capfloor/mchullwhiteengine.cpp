...
namespace QuantLib::detail
{
    ...
    Real HullWhiteCapFloorPricer::operator()(const Path& path) const
    {
        ...
        for (Size i = 0; i < fixingTimes_.size(); i++)
        {
            ...
            Time start = startTimes_[i], end = endTimes_[i], fixing = fixingTimes_[i];
            if (end <= 0.0)
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    DiscountFactor d1 = model_->discountBond(fixing, start, ri_1);
                    DiscountFactor d2 = model_->discountBond(fixing, end, ri_1);
                ...
                Real payoff = isCap ? std::max(currentLibor - strike, 0.0) : std::max(strike - currentLibor, 0.0);
