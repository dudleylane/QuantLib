...
namespace QuantLib
{
    ...
    void Gaussian1dCapFloorEngine::calculate() const
    {
        ...
        for (Size i = 0; i < optionlets; ++i)
        {
            ...
            ext::shared_ptr<IborIndex> iborIndex = ext::dynamic_pointer_cast<IborIndex>(arguments_.indexes[i]);
                    ...
                    else
                    {
                        ...
                        for (Size j = 0; j < z.size() - 1; j++)
                        {
                            price += Gaussian1dModel::gaussianShiftedPolynomialIntegral(
                                0.0, payoff.cCoefficients()[j], payoff.bCoefficients()[j], payoff.aCoefficients()[j],
                                p[j], z[j], z[j], z[j + 1]);
                        }
