...
namespace QuantLib
{
    ...
    std::vector<Time> DiscretizedCapFloor::mandatoryTimes() const
    {
        ...
        std::copy(endTimes_.begin(), endTimes_.end(), std::back_inserter(times));
        return times;
    }
            ...
            if (isOnTime(startTimes_[i]))
            {
                ...
                if ((type == CapFloor::Cap) || (type == CapFloor::Collar))
                {
                    ...
                    for (Size j = 0; j < values_.size(); j++)
                        values_[j] += nominal * accrual * gearing * std::max<Real>(strike - bond.values()[j], 0.0);
                }
                ...
                if ((type == CapFloor::Floor) || (type == CapFloor::Collar))
                {
                    ...
                    for (Size j = 0; j < values_.size(); j++)
                        values_[j] +=
                            nominal * accrual * mult * gearing * std::max<Real>(bond.values()[j] - strike, 0.0);
                }
            }
        }
    }
