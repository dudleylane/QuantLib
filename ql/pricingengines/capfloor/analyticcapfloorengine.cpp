...
namespace QuantLib
{
    ...
    void AnalyticCapFloorEngine::calculate() const
    {
        ...
        bool includeRefDatePayments = Settings::instance().includeReferenceDateEvents();
        if (referenceDate == Settings::instance().evaluationDate())
        {
            ext::optional<bool> includeTodaysPayments = Settings::instance().includeTodaysCashFlows();
        ...
        for (Size i = 0; i < nPeriods; i++)
        {
            ...
            Time fixingTime = dayCounter.yearFraction(referenceDate, arguments_.fixingDates[i]);
            Time paymentTime = dayCounter.yearFraction(referenceDate, arguments_.endDates[i]);

            bool not_expired = includeRefDatePayments ? paymentTime >= 0.0 : paymentTime > 0.0;
            ...
            if (not_expired)
            {
                ...
                if (fixingTime <= 0.0)
                {
                    if (type == CapFloor::Cap || type == CapFloor::Collar)
                    {
                        ...
                        value += discount * arguments_.nominals[i] * tenor * arguments_.gearings[i] *
                    ...
                    if (type == CapFloor::Floor || type == CapFloor::Collar)
                    {
                        ...
                        value += discount * arguments_.nominals[i] * tenor * mult * arguments_.gearings[i] *
                                 std::max(0.0, strike - fixing);
                    }
                }
                else
                {
                    Time maturity = dayCounter.yearFraction(referenceDate, arguments_.startDates[i]);
                    if (type == CapFloor::Cap || type == CapFloor::Collar)
                    {
                        Real temp = 1.0 + arguments_.capRates[i] * tenor;
                        value += arguments_.nominals[i] * arguments_.gearings[i] * temp *
                                 model_->discountBondOption(Option::Put, 1.0 / temp, maturity, paymentTime);
                    }
                    if (type == CapFloor::Floor || type == CapFloor::Collar)
                    {
                        Real temp = 1.0 + arguments_.floorRates[i] * tenor;
                        Real mult = (type == CapFloor::Floor) ? 1.0 : -1.0;
                        value += arguments_.nominals[i] * arguments_.gearings[i] * temp * mult *
                                 model_->discountBondOption(Option::Call, 1.0 / temp, maturity, paymentTime);
                    }
                }
            }
        }
