...
namespace QuantLib
{
    ...
    namespace detail
    {
        SumExponentialsRootSolver::SumExponentialsRootSolver(Array a, Array sig, Real K)
        : a_(std::move(a)), sig_(std::move(sig)), K_(K)
        {
            QL_REQUIRE(a_.size() == sig_.size(), "Arrays must have the same size");
        }
        ...
        Real SumExponentialsRootSolver::getRoot(Real xTol, Strategy strategy) const
        {
            ...
            QL_REQUIRE(std::all_of(attr.begin(), attr.end(), [](Real x) -> bool { return x >= 0.0; }),
                       "a*sig should not be negative");

            const bool logProb = std::all_of(a_.begin(), a_.end(), [](Real x) -> bool { return x > 0; });

            QL_REQUIRE(K_ > 0 || !logProb, "non-positive strikes only allowed for spread options");

            // linear approximation
            const Real denom = std::accumulate(attr.begin(), attr.end(), Real(0.0));
            const Real xInit =
                (std::abs(denom) > 1000 * QL_EPSILON) ?
                    std::min(10.0, std::max(-10.0, (K_ - std::accumulate(a_.begin(), a_.end(), Real(0.0))) / denom)) :
                    Real(0.0);
    ...
    SingleFactorBsmBasketEngine::SingleFactorBsmBasketEngine(
        std::vector<ext::shared_ptr<GeneralizedBlackScholesProcess>> p, Real xTol)
    : xTol_(xTol), n_(p.size()), processes_(std::move(p))
    {
    ...
    void SingleFactorBsmBasketEngine::calculate() const
    {
        ...
        QL_REQUIRE(n_ == weights.size(), "wrong number of weights arguments in payoff");
        ...
        if (std::all_of(stdDev.begin(), stdDev.end(), [](Real x) -> bool { return close_enough(x, 0.0); }))
        {
            results_.value = dr0 * payoff->operator()(std::accumulate(fwdBasket.begin(), fwdBasket.end(), Real(0.0)));
        }
        else
        {
            const Real d = -detail::SumExponentialsRootSolver(fwdBasket * Exp(-0.5 * v), stdDev, strike)
            ...
            results_.value =
                cp * dr0 *
                std::inner_product(fwdBasket.begin(), fwdBasket.end(), stdDev.begin(), Real(-strike * N(cp * d)),
                                   std::plus<>(), [d, cp, &N](Real x, Real y) -> Real { return x * N(cp * (d + y)); });
