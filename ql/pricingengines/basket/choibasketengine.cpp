...
namespace QuantLib
{
    ...
    ChoiBasketEngine::ChoiBasketEngine(std::vector<ext::shared_ptr<GeneralizedBlackScholesProcess>> processes,
                                       Matrix rho,
                                       Real lambda,
                                       Size maxNrIntegrationSteps,
                                       bool calcFwdDelta,
                                       bool controlVariate)
    : n_(processes.size()), processes_(std::move(processes)), rho_(std::move(rho)), lambda_(lambda),
      maxNrIntegrationSteps_(maxNrIntegrationSteps), calcFwdDelta_(calcFwdDelta || controlVariate),
    ...
    void ChoiBasketEngine::calculate() const
    {
        ...
        if (flip)
        {
            // q1 = inverse(C)*vStar1;
            for (Size i = 0; i < n_; ++i)
                q1[i] = (vStar1[i] - std::inner_product(C.row_begin(i), C.row_begin(i) + i, q1.begin(), Real(0.0))) /
                        C[i][i];
        ...
        const Matrix R = HouseholderTransformation(HouseholderReflection(e1).reflectionVector(q1)).getMatrix();
        ...
        for (Size i = 0; i < n_ - 1; ++i)
            std::transform(U.column_begin(i), U.column_end(i), v.column_begin(i),
        ...
        do
        {
            ...
            QL_REQUIRE(lambda / lambda_ > 1e-10, "can not rescale lambda to fit max integration order");
        } while (std::accumulate(nIntOrder.begin(), nIntOrder.end(), 1.0, std::multiplies<>()) >
        ...
        for (Size i = 0; i < n_; ++i)
        {
            ...
            const Volatility vol =
                vStar1[i] / std::sqrt(bv->dayCounter().yearFraction(bv->referenceDate(), maturityDate));
            p.push_back(ext::make_shared<BlackProcess>(
                Handle<Quote>(quotes[i]), processes_[i]->riskFreeRate(),
                Handle<BlackVolTermStructure>(ext::make_shared<BlackConstantVol>(
                    bv->referenceDate(), bv->calendar(), Handle<Quote>(ext::make_shared<SimpleQuote>(vol)),
                    bv->dayCounter()))));
        }
        ...
        Array vq(n_);
        for (Size i = 0; i < n_; ++i)
            vq[i] = 0.5 * std::accumulate(v.row_begin(i), v.row_end(i), Real(0.0),
                                          [](Real acc, Real x) -> Real { return acc + x * x; });

        MultiDimGaussianIntegration ghq(nIntOrder,
        ...
        if (calcFwdDelta_)
        {
            ...
            for (Size k = 0; k < n_; ++k)
            {
                ...
                const auto deltaPricer = [&](const Array& z) -> Real
                {
                    ...
                    const Real vz = std::inner_product(v.row_begin(k), v.row_end(k), z.begin(), Real(0.0));
