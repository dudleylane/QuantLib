...
namespace QuantLib
{
    ...
    DengLiZhouBasketEngine::DengLiZhouBasketEngine(
        std::vector<ext::shared_ptr<GeneralizedBlackScholesProcess>> processes, Matrix rho)
    : n_(processes.size()), processes_(std::move(processes)), rho_(std::move(rho))
    {
        ...
        std::for_each(processes_.begin(), processes_.end(), [this](const auto& p) { registerWith(p); });
    }
    ...
    void DengLiZhouBasketEngine::calculate() const
    {
        ...
        const ext::shared_ptr<AverageBasketPayoff> avgPayoff =
            ...
            (ext::dynamic_pointer_cast<SpreadBasketPayoff>(arguments_.payoff) != nullptr) ?
                ext::make_shared<AverageBasketPayoff>(
                    ext::dynamic_pointer_cast<SpreadBasketPayoff>(arguments_.payoff)->basePayoff(),
                    Array({1.0, -1.0})) :
        ...
        QL_REQUIRE(n_ == weights.size() && n_ > 1, "wrong number of weights arguments in payoff");
        ...
        const Size M = std::distance(p.begin(), std::lower_bound(p.begin(), p.end(), Real(0),
                                                                 [](const auto& p, const Real& value) -> bool
        ...
        if (M > 1)
        {
            ...
            const Real S0 =
                std::accumulate(p.begin(), p.begin() + M, Real(0.0), [](const Real& value, const auto& p) -> Real
            ...
            for (Size i = 0; i < M; ++i)
                for (Size j = 0; j < M; ++j)
                    v_s += vol[i] * vol[j] * F[i] * F[j] * rho[std::get<1>(p[i])][std::get<1>(p[j])];
            ...
            for (Size i = 0; i < N; ++i)
            {
                ...
                nRho[i + 1][0] = nRho[0][i + 1] = std::min(1.0, std::max(-1.0, rhoHat / (std::sqrt(v_s) * F0)));
            }
        }
        else
        {
        ...
        const Real callValue = DengLiZhouBasketEngine::calculate_vanilla_call(Log(_s), dr0, _dq, _v, nRho, strike);
    ...
    {
        const Real psi = 1.0 / (1.0 + std::inner_product(D.row_begin(i), D.row_end(i), D.row_begin(i), Real(0.0)));
        ...
        const Real vFv = std::inner_product(DF.row_begin(i), DF.row_end(i), D.row_begin(i), Real(0.0));
        const Real J_1 = psi * sqrtPsi * (psi * u * u - 1.0) * vFv * n_uSqrtPsi;

        const Real vFFv = std::inner_product(DF.row_begin(i), DF.row_end(i), DF.row_begin(i), Real(0.0));
        const Real J_2 =
            u * psi * sqrtPsi * n_uSqrtPsi *
            (2 * tF2 + vFv * vFv * (squared(squared(psi * u)) - 10.0 * psi * psi * psi * u * u + 15 * psi * psi) +
    ...
    Real DengLiZhouBasketEngine::calculate_vanilla_call(
        const Array& x, DiscountFactor dr, const Array& dq, const Array& v, const Matrix& rho, Real K)
    {
        ...
        const Real R =
            std::accumulate(mu.begin() + 1, mu.end(), Real(0), [](Real a, Real b) -> Real { return a + std::exp(b); });
        ...
        {
            trF += F[i][i];
            trF2 += squared(F[i][i]) + 2.0 * std::accumulate(F.row_begin(i) + i + 1, F.row_end(i), Real(0),
        ...
        const Array d =
            (sig11Inv10 - Exp(Array(mu.begin() + 1, mu.end())) * Array(nu.begin() + 1, nu.end()) / (nu[0] * (R + K))) /
            sqSig_xy;
        ...
        Array C(N + 2);
        C[0] = c + trF + nu[0] * sqSig_xy + nu[0] * DotProduct(sig10, d) + squared(nu[0]) * DotProduct(sig10, Esig10);
        C[N + 1] = c + trF;

        for (Size k = 1; k < N + 1; ++k)
            C[k] = c + trF + nu[k] * sig11d[k - 1] +
                   squared(nu[k]) * std::inner_product(sig11.row_begin(k - 1), sig11.row_end(k - 1),
