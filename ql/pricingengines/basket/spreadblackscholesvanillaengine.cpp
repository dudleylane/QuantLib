...
namespace QuantLib
{
    ...
    void SpreadBlackScholesVanillaEngine::calculate() const
    {
        ...
        const Real f1 = process1_->stateVariable()->value() / process1_->riskFreeRate()->discount(maturityDate) *
                        process1_->dividendYield()->discount(maturityDate);

        const Real f2 = process2_->stateVariable()->value() / process2_->riskFreeRate()->discount(maturityDate) *
                        process2_->dividendYield()->discount(maturityDate);

        const Real variance1 = process1_->blackVolatility()->blackVariance(maturityDate, f1);
        const Real variance2 = process2_->blackVolatility()->blackVariance(maturityDate, f2);

        const DiscountFactor df = process1_->riskFreeRate()->discount(exercise->lastDate());
