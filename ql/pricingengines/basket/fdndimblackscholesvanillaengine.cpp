...
namespace QuantLib
{
    ...
    namespace detail
    {
        ...
        {
          public:
            FdmPCABasketInnerValue(ext::shared_ptr<BasketPayoff> payoff,
                                   ...
                                   Matrix Q,
                                   Array l)
            : n_(logS0.size()), payoff_(std::move(payoff)), mesher_(std::move(mesher)), logS0_(std::move(logS0)),
              v_(vols * vols), qTS_(std::move(qTS)), rTS_(std::move(rTS)), Q_(std::move(Q)), l_(std::move(l)),
              cachedT_(Null<Real>()), qf_(n_)
            {
            }
    ...
    : processes_(std::move(processes)), rho_(std::move(rho)), xGrids_(std::move(xGrids)), tGrid_(tGrid),
      dampingSteps_(dampingSteps), schemeDesc_(schemeDesc)
    {

        QL_REQUIRE(!processes_.empty(), "no Black-Scholes process is given.");
        QL_REQUIRE(rho_.size1() == rho_.size2() && rho_.size1() == processes_.size(),
                   "correlation matrix has the wrong size.");
        QL_REQUIRE(xGrids_.size() == 1 || xGrids_.size() == processes_.size(), "wrong number of xGrids is given.");
...
#    define PDE_MAX_SUPPORTED_DIM 4
        ...
        const SymmetricSchurDecomposition schur(getCovariance(vols.begin(), vols.end(), rho_));
        ...
        {
            const Size xGrid =
                (xGrids_.size() > 1) ? xGrids_[i] : std::max(Size(4), Size(xGrids_[0] * std::pow(l[i] / l[0], 0.1)));
            ...
            for (Size j = 0; j < xGrid; ++j)
                x[j] = 1.3 * std::sqrt(l[i]) * sqrtT * InverseCumulativeNormal()(eps + j * xStepStize);

            meshers.emplace_back(ext::make_shared<Predefined1dMesher>(x));
        }

        const ext::shared_ptr<FdmMesherComposite> mesher = ext::make_shared<FdmMesherComposite>(meshers);

        const ext::shared_ptr<BasketPayoff> payoff = ext::dynamic_pointer_cast<BasketPayoff>(arguments_.payoff);
        QL_REQUIRE(payoff, "basket payoff expected");

        const ext::shared_ptr<YieldTermStructure> rTS = processes_[0]->riskFreeRate().currentLink();
        ...
        const ext::shared_ptr<FdmInnerValueCalculator> calculator =
            ext::make_shared<detail::FdmPCABasketInnerValue>(payoff, mesher, Log(s), stdDev / sqrtT, qTS, rTS, Q, l);

        const ext::shared_ptr<FdmStepConditionComposite> conditions = FdmStepConditionComposite::vanillaComposite(
            DividendSchedule(), arguments_.exercise, mesher, calculator, rTS->referenceDate(), rTS->dayCounter());

        const FdmBoundaryConditionSet boundaries;
        const FdmSolverDesc solverDesc = {mesher, boundaries, conditions, calculator, maturity, tGrid_, dampingSteps_};

        const bool isEuropean = ext::dynamic_pointer_cast<EuropeanExercise>(arguments_.exercise) != nullptr;
        const ext::shared_ptr<FdmWienerOp> op =
            ext::make_shared<FdmWienerOp>(mesher, (isEuropean) ? ext::shared_ptr<YieldTermStructure>() : rTS, l);

        switch (processes_.size())
        {
#define BOOST_PP_LOCAL_MACRO(n)                                                          \
    case n:                                                                              \
        results_.value = ext::make_shared<FdmNdimSolver<n>>(solverDesc, schemeDesc_, op) \
                             ->interpolateAt(std::vector<Real>(processes_.size(), 0.0)); \
        break;
