...
namespace QuantLib
{
        ...
        Real f1, Real f2, Real k, Option::Type optionType, Real variance1, Real variance2, DiscountFactor df) const
    {
        ...
        const Real stdev = std::sqrt(variance1 + b * b * variance2 - 2 * rho_ * b * sigma1 * sigma2);

        const Real lfa = std::log(f1 / a);

        const Real d1 = (lfa + (0.5 * variance1 + 0.5 * b * b * variance2 - b * rho_ * sigma1 * sigma2)) / stdev;
        const Real d2 = (lfa + (-0.5 * variance1 + variance2 * b * (0.5 * b - 1) + rho_ * sigma1 * sigma2)) / stdev;
