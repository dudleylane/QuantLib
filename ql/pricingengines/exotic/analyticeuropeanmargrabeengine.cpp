...
namespace QuantLib
{
    ...
    void AnalyticEuropeanMargrabeEngine::calculate() const
    {
        ...
        ext::shared_ptr<NullPayoff> payoff = ext::dynamic_pointer_cast<NullPayoff>(arguments_.payoff);
        ...
        Real d1 = (std::log((quantity1 * forward1) / (quantity2 * forward2)) + 0.5 * variance) / stdDev;
        Real d2 = d1 - stdDev;
        ...
        results_.value = riskFreeDiscount * (quantity1 * forward1 * Nd1 - quantity2 * forward2 * Nd2);
        ...
        results_.theta = -((stdDev * vega / sqt) / (2 * t) - (q1 * quantity1 * s1 * results_.delta1) -
                           (q2 * quantity2 * s2 * results_.delta2));
        results_.rho = 0.0;
    }
