...
namespace QuantLib
{
    ...
    MakeCreditDefaultSwap::MakeCreditDefaultSwap(const Period& tenor, Rate runningSpread)
    : tenor_(tenor), runningSpread_(runningSpread)
    {
    }

    MakeCreditDefaultSwap::MakeCreditDefaultSwap(const Date& termDate, Rate runningSpread)
    : termDate_(termDate), runningSpread_(runningSpread)
    {
    }

    MakeCreditDefaultSwap::MakeCreditDefaultSwap(const Schedule& schedule, Rate runningSpread)
    ...
    MakeCreditDefaultSwap::operator ext::shared_ptr<CreditDefaultSwap>() const
    {
        ...
        Date upfrontDate =
            upfrontDate_ != Date() ? upfrontDate_ : WeekendsOnly().advance(tradeDate, cashSettlementDays_, Days);
        ...
        else
        {
            ...
            schedule =
                Schedule(protectionStart, end, couponTenor_, WeekendsOnly(), convention_, Unadjusted, rule_, false);
        }

        auto cds = ext::make_shared<CreditDefaultSwap>(side_, nominal_, upfrontRate_, runningSpread_, schedule,
                                                       convention_, dayCounter_, settlesAccrual_, paysAtDefaultTime_,
                                                       protectionStart, upfrontDate, claim_, lastPeriodDayCounter_,
    ...
    MakeCreditDefaultSwap& MakeCreditDefaultSwap::settleAccrual(bool b)
    {
        settlesAccrual_ = b;
        return *this;
    }

    MakeCreditDefaultSwap& MakeCreditDefaultSwap::payAtDefaultTime(bool b)
    {
    ...
    MakeCreditDefaultSwap& MakeCreditDefaultSwap::rebateAccrual(bool b)
    {
        rebatesAccrual_ = b;
        return *this;
    }
    ...
    MakeCreditDefaultSwap& MakeCreditDefaultSwap::withPricingEngine(const ext::shared_ptr<PricingEngine>& engine)
    {
        engine_ = engine;
        return *this;
    }
