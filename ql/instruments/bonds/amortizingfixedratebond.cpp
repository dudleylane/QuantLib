...
namespace QuantLib
{
    ...
    AmortizingFixedRateBond::AmortizingFixedRateBond(Natural settlementDays,
                                                     ...
                                                     Integer paymentLag)
    : Bond(settlementDays, schedule.calendar(), issueDate), frequency_(schedule.tenor().frequency()),
                             ...
                             const Calendar& paymentCalendar)
    {
        ...
        Schedule retVal(startDate, maturityDate, Period(frequency), paymentCalendar, Unadjusted, Unadjusted,
    ...
    namespace
    {
        ...
        bool isSubPeriod(const Period& subPeriod, const Period& superPeriod, Integer& numSubPeriods)
        {
            ...
            Real minPeriodRatio = ((Real)superDays.first) / ((Real)subDays.second);
            Real maxPeriodRatio = ((Real)superDays.second) / ((Real)subDays.first);
    ...
    std::vector<Real>
    sinkingNotionals(const Period& bondLength, const Frequency& sinkingFrequency, Rate couponRate, Real initialNotional)
    {
        Integer nPeriods;
