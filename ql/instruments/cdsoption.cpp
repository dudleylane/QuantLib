...
namespace QuantLib
{
          ...
          public:
            ...
            : targetValue_(targetValue), vol_(ext::make_shared<SimpleQuote>(0.0))
            {
                ...
                engine_ = ext::shared_ptr<PricingEngine>(
                    new BlackCdsOptionEngine(probability, recoveryRate, termStructure, h));
    ...
    CdsOption::CdsOption(const ext::shared_ptr<CreditDefaultSwap>& swap,
                         ...
                         bool knocksOut)
    : Option(ext::shared_ptr<Payoff>(new NullPayoff), exercise), swap_(swap), knocksOut_(knocksOut)
    {
        QL_REQUIRE(swap->side() == Protection::Buyer || knocksOut_, "receiver CDS options must knock out");
    ...
    Volatility CdsOption::impliedVolatility(Real targetValue,
                                            const Handle<YieldTermStructure>& termStructure,
                                            const Handle<DefaultProbabilityTermStructure>& probability,
                                            Real recoveryRate,
                                            Real accuracy,
