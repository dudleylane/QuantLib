...
namespace QuantLib
{
    ...
    : ForwardRateAgreement(index,
                           valueDate,
                           index->maturityDate(valueDate),
                           type,
                           strikeForwardRate,
                           notionalAmount,
    ...
    ForwardRateAgreement::ForwardRateAgreement(const ext::shared_ptr<IborIndex>& index,
                                               ...
                                               Real notionalAmount,
                                               Handle<YieldTermStructure> discountCurve)
    : fraType_(type), notionalAmount_(notionalAmount), index_(index), useIndexedCoupon_(false),
      dayCounter_(index->dayCounter()), calendar_(index->fixingCalendar()),
      businessDayConvention_(index->businessDayConvention()), valueDate_(valueDate), maturityDate_(maturityDate),
      discountCurve_(std::move(discountCurve))
    {
        ...
        strikeForwardRate_ = InterestRate(strikeForwardRate, index->dayCounter(), Simple, Once);
        registerWith(index_);
    }
    ...
    void ForwardRateAgreement::calculateForwardRate() const
    {
        if (useIndexedCoupon_)
            forwardRate_ = InterestRate(index_->fixing(fixingDate()), index_->dayCounter(), Simple, Once);
        else
