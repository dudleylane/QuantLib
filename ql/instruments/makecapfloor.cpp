...
namespace QuantLib
{
    ...
    MakeCapFloor::operator ext::shared_ptr<CapFloor>() const
    {
        ...
        if (strike_ == Null<Rate>())
        {
            ...
            ext::shared_ptr<BlackCapFloorEngine> temp = ext::dynamic_pointer_cast<BlackCapFloorEngine>(engine_);
            QL_REQUIRE(temp, "cannot calculate ATM without a BlackCapFloorEngine");
            Handle<YieldTermStructure> discountCurve = temp->termStructure();
            strikeVector[0] = CashFlows::atmRate(leg, **discountCurve, false, discountCurve->referenceDate());
        }

        ext::shared_ptr<CapFloor> capFloor(new CapFloor(capFloorType_, leg, strikeVector));
    ...
    MakeCapFloor& MakeCapFloor::withEffectiveDate(const Date& effectiveDate, bool firstCapletExcluded)
    {
    ...
    MakeCapFloor& MakeCapFloor::withTerminationDateConvention(BusinessDayConvention bdc)
    {
    ...
    MakeCapFloor& MakeCapFloor::withPricingEngine(const ext::shared_ptr<PricingEngine>& engine)
    {
        engine_ = engine;
        return *this;
    }
