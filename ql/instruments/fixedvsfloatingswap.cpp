...
namespace QuantLib
{
    ...
    : Swap(2), type_(type), fixedNominals_(std::move(fixedNominals)), fixedSchedule_(std::move(fixedSchedule)),
      fixedRate_(fixedRate), fixedDayCount_(std::move(fixedDayCount)), floatingNominals_(std::move(floatingNominals)),
      floatingSchedule_(std::move(floatingSchedule)), iborIndex_(std::move(iborIndex)), spread_(spread),
      floatingDayCount_(std::move(floatingDayCount))
    {
        ...
        legs_[0] = FixedRateLeg(fixedSchedule_)
                       ...
                       .withPaymentCalendar(paymentCalendar.empty() ? fixedSchedule_.calendar() : paymentCalendar);
        ...
        sameNominals_ = std::equal(fixedNominals_.begin(), fixedNominals_.end(), floatingNominals_.begin(),
                                   floatingNominals_.end());
        if (!sameNominals_)
        {
    ...
    void FixedVsFloatingSwap::arguments::validate() const
    {
        ...
        QL_REQUIRE(fixedNominals.size() == fixedPayDates.size(), "number of fixed nominals different from "
        ...
        QL_REQUIRE(floatingNominals.size() == floatingPayDates.size(), "number of floating nominals different from "
