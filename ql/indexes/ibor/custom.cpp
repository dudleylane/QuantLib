...
namespace QuantLib
{
    ...
    {
        Date fixingDate = valueCalendar_.advance(valueDate, -static_cast<Integer>(fixingDays_), Days);
    ...
    {
        return maturityCalendar_.advance(valueDate, tenor_, convention_, endOfMonth_);
    }

    ext::shared_ptr<IborIndex> CustomIborIndex::clone(const Handle<YieldTermStructure>& h) const
    {
        return ext::make_shared<CustomIborIndex>(familyName_, tenor_, fixingDays_, currency_, fixingCalendar(),
