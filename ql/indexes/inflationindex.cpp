...
namespace QuantLib
{
    ...
    InflationIndex::InflationIndex(std::string familyName,
                                   ...
                                   Currency currency)
    : familyName_(std::move(familyName)), region_(std::move(region)), revised_(revised), frequency_(frequency),
      availabilityLag_(availabilityLag), currency_(std::move(currency))
    {
    ...
    Real ZeroInflationIndex::fixing(const Date& fixingDate, bool /*forecastTodaysFixing*/) const
    {
        if (!needsForecast(fixingDate))
        {
            const Real I1 = pastFixing(fixingDate);
            QL_REQUIRE(I1 != Null<Real>(),
                       "Missing " << name() << " fixing for " << inflationPeriod(fixingDate, frequency_).first);
    ...
    bool ZeroInflationIndex::needsForecast(const Date& fixingDate) const
    {
        ...
        auto latestPossibleHistoricalFixingPeriod = inflationPeriod(today - availabilityLag_, frequency_);
    ...
    Real ZeroInflationIndex::forecastFixing(const Date& fixingDate) const
    {
        ...
        QL_REQUIRE(!needsForecast(baseDate), name()
                                                 << " index fixing at base date " << baseDate << " is not available");
        ...
        Time t1 = inflationYearFraction(frequency_, false, zeroInflation_->dayCounter(), baseDate, firstDateInPeriod);
    ...
    ext::shared_ptr<ZeroInflationIndex> ZeroInflationIndex::clone(const Handle<ZeroInflationTermStructure>& h) const
    {
        return ext::make_shared<ZeroInflationIndex>(familyName_, region_, revised_, frequency_, availabilityLag_,
                                                    currency_, h);
    }
    ...
    : InflationIndex("YYR_" + underlyingIndex->familyName(),
                     underlyingIndex->region(),
                     underlyingIndex->revised(),
                     underlyingIndex->frequency(),
                     underlyingIndex->availabilityLag(),
                     underlyingIndex->currency()),
      ratio_(true), underlyingIndex_(underlyingIndex), yoyInflation_(std::move(yoyInflation))
    {
    ...
    YoYInflationIndex::YoYInflationIndex(const std::string& familyName,
                                         ...
                                         const Currency& currency,
                                         Handle<YoYInflationTermStructure> yoyInflation)
    : InflationIndex(familyName, region, revised, frequency, availabilityLag, currency), ratio_(false),
    ...
    Real YoYInflationIndex::pastFixing(const Date& fixingDate) const
    {
        if (ratio())
        {
            ...
            Rate previousFixing =
                CPI::laggedFixing(underlyingIndex_, fixingDate - 1 * Years, Period(0, Months), interpolationType);
        ...
        { // NOT ratio
            ...
            QL_REQUIRE(YY0 != Null<Rate>(), "Missing " << name() << " fixing for " << periodStart);
            ...
            else
            {
                ...
                QL_REQUIRE(YY1 != Null<Rate>(), "Missing " << name() << " fixing for " << periodEnd + 1);
    ...
    ext::shared_ptr<YoYInflationIndex> YoYInflationIndex::clone(const Handle<YoYInflationTermStructure>& h) const
    {
        if (ratio_)
        {
        ...
        {
            return ext::make_shared<YoYInflationIndex>(familyName_, region_, revised_, frequency_, availabilityLag_,
                                                       currency_, h);
        }
    }


    CPI::InterpolationType detail::CPI::effectiveInterpolationType(const QuantLib::CPI::InterpolationType& type)
    {
        QL_DEPRECATED_DISABLE_WARNING
