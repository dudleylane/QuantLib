...
namespace QuantLib
{
    ...
    class CalibratedModel::CalibrationFunction : public CostFunction
    {
      public:
        ...
        : model_(model, null_deleter()), instruments_(h), weights_(std::move(weights)), projection_(projection)
        {
        }
    ...
    void CalibratedModel::calibrate(const vector<ext::shared_ptr<CalibrationHelper>>& instruments,
                                    ...
                                    const vector<bool>& fixParameters)
    {
        ...
        QL_REQUIRE(weights.empty() || weights.size() == instruments.size(),
                   "mismatch between number of instruments (" << instruments.size() << ") and weights ("
                                                              << weights.size() << ")");
        ...
        QL_REQUIRE(fixParameters.empty() || fixParameters.size() == prms.size(),
                   "mismatch between number of parameters (" << prms.size() << ") and fixed-parameter specs ("
                                                             << fixParameters.size() << ")");
    ...
    Real CalibratedModel::value(const Array& params, const vector<ext::shared_ptr<CalibrationHelper>>& instruments)
    {
    ...
    ShortRateModel::ShortRateModel(Size nArguments) : CalibratedModel(nArguments) {}
