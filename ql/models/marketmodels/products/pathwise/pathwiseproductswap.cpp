...
namespace QuantLib
{
    ...
    MarketModelPathwiseSwap::MarketModelPathwiseSwap(const std::vector<Time>& rateTimes,
                                                     ...
                                                     Real multiplier)
    : rateTimes_(rateTimes), accruals_(accruals), strikes_(strikes), numberRates_(rateTimes.size() - 1),
      multiplier_(multiplier)
    {
        ...
        QL_REQUIRE(accruals_.size() == numberRates_, "accruals.size() does not equal numberOfRates or 1");

        QL_REQUIRE(strikes.size() == numberRates_, "strikes.size() does not equal numberOfRates or 1");
    ...
    std::unique_ptr<MarketModelPathwiseMultiProduct> MarketModelPathwiseSwap::clone() const
    {
    ...
    {
        return rateTimes_; // note rateTimes_[0] is not used as a cash flow time but it is easier to keep track if we
                           // include it.
    }
