...
namespace QuantLib
{
    ...
    MarketModelPathwiseInverseFloater::MarketModelPathwiseInverseFloater(const std::vector<Time>& rateTimes,
                                                                         ...
                                                                         bool payer)
    : rateTimes_(rateTimes), fixedAccruals_(std::move(fixedAccruals)), floatingAccruals_(floatingAccruals),
        ...
        std::vector<std::vector<MarketModelPathwiseMultiProduct::CashFlow>>& cashFlowsGenerated)
    {
        ...
        if (inverseFloatingCoupon > 0.0)
        {
            ...
            cashFlowsGenerated[0][0].amount[currentIndex_ + 1] =
                multiplier_ *
                (-fixedMultipliers_[currentIndex_] * fixedAccruals_[currentIndex_] - floatingAccruals_[currentIndex_]);
        }
        else
        {
    ...
    std::unique_ptr<MarketModelPathwiseMultiProduct> MarketModelPathwiseInverseFloater::clone() const
    {
