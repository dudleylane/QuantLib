...
namespace QuantLib
{
    ...
    : MultiProductMultiStep(rateTimes), paymentTimes_(forwardOptionPaymentTimes),
      forwardOptionPaymentTimes_(forwardOptionPaymentTimes), swaptionPaymentTimes_(swaptionPaymentTimes),
      forwardPayOffs_(std::move(forwardPayOffs)), swapPayOffs_(std::move(swapPayOffs)), period_(period), offset_(offset)
    {
        QL_REQUIRE(rateTimes.size() >= 2, "we need at least two rate times in MultiStepPeriodCapletSwaptions ");
        ...
        QL_REQUIRE(offset_ < period_, "the offset must be less then the period in MultiStepPeriodCapletSwaptions ");
    ...
    std::unique_ptr<MarketModelMultiProduct> MultiStepPeriodCapletSwaptions::clone() const
    {
