...
namespace QuantLib
{
    ...
    : MultiProductMultiStep(rateTimes), accruals_(accruals), accrualsFloating_(accrualsFloating),
      paymentTimes_(paymentTimes), paymentTimesFloating_(paymentTimesFloating), allPaymentTimes_(paymentTimes),
      totalCoupon_(totalCoupon), strikes_(strikes), multipliers_(std::move(multipliers)),
      floatingSpreads_(floatingSpreads)
    {
        ...
        QL_REQUIRE(paymentTimesFloating.size() + 1 == rateTimes.size(),
    ...
    bool MultiStepTarn::nextTimeStep(const CurveState& currentState,
                                     ...
                                     std::vector<std::vector<MarketModelMultiProduct::CashFlow>>& genCashFlows)
    {
        ...
        Real obviousCoupon =
            std::max(strikes_[currentIndex_] - multipliers_[currentIndex_] * liborRate, 0.0) * accruals_[currentIndex_];
    ...
    std::vector<Time> MultiStepTarn::possibleCashFlowTimes() const
    {
    ...
    Size MultiStepTarn::maxNumberOfCashFlowsPerProductPerStep() const
    {
        return 2;
    }
