...
namespace QuantLib
{
    ...
    SobolBrownianGeneratorBase::SobolBrownianGeneratorBase(Size factors, Size steps, Ordering ordering)
    : factors_(factors), steps_(steps), ordering_(ordering), bridge_(steps),
      orderedIndices_(factors, std::vector<Size>(steps)), bridgedVariates_(factors, std::vector<Real>(steps))
    {
    ...
    const std::vector<std::vector<Size>>& SobolBrownianGeneratorBase::orderedIndices() const
    {
        return orderedIndices_;
    }

    std::vector<std::vector<Real>> SobolBrownianGeneratorBase::transform(const std::vector<std::vector<Real>>& variates)
    {

        QL_REQUIRE((variates.size() == factors_ * steps_), "inconsistent variate vector");

        const Size dim = factors_ * steps_;
        const Size nPaths = variates.front().size();

        std::vector<std::vector<Real>> retVal(factors_, std::vector<Real>(nPaths * steps_));
    ...
    Size SobolBrownianGeneratorBase::numberOfFactors() const
    {
        return factors_;
    }

    Size SobolBrownianGeneratorBase::numberOfSteps() const
    {
        return steps_;
    }

    SobolBrownianGenerator::SobolBrownianGenerator(
        Size factors, Size steps, Ordering ordering, unsigned long seed, SobolRsg::DirectionIntegers integers)
    ...
    SobolBrownianGeneratorFactory::SobolBrownianGeneratorFactory(SobolBrownianGenerator::Ordering ordering,
                                                                 unsigned long seed,
    ...
    ext::shared_ptr<BrownianGenerator> SobolBrownianGeneratorFactory::create(Size factors, Size steps) const
    {
        return ext::shared_ptr<BrownianGenerator>(
            new SobolBrownianGenerator(factors, steps, ordering_, seed_, integers_));
    }

    Burley2020SobolBrownianGenerator::Burley2020SobolBrownianGenerator(Size factors,
                                                                       Size steps,
                                                                       Ordering ordering,
                                                                       unsigned long seed,
                                                                       SobolRsg::DirectionIntegers integers,
    ...
    : SobolBrownianGeneratorBase(factors, steps, ordering),
      generator_(Burley2020SobolRsg(factors * steps, seed, integers, scrambleSeed), InverseCumulativeNormal())
    {
    }
    ...
    ext::shared_ptr<BrownianGenerator> Burley2020SobolBrownianGeneratorFactory::create(Size factors, Size steps) const
    {
        return ext::shared_ptr<BrownianGenerator>(
            new Burley2020SobolBrownianGenerator(factors, steps, ordering_, seed_, integers_, scrambleSeed_));
    }
}
