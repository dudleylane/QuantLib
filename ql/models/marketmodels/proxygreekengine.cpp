...
namespace QuantLib
{
    ...
    : originalEvolver_(std::move(evolver)), constrainedEvolvers_(std::move(constrainedEvolvers)),
      diffWeights_(std::move(diffWeights)), startIndexOfConstraint_(std::move(startIndexOfConstraint)),
      ...
      cashFlowsGenerated_(product->numberOfProducts())
    {
        ...
        const std::vector<Rate>& evolutionTimes = product_->evolution().evolutionTimes();
        constraints_.resize(evolutionTimes.size());
        constraintsActive_.resize(evolutionTimes.size());
    }

    void ProxyGreekEngine::singlePathValues(std::vector<Real>& values,
                                            std::vector<std::vector<std::vector<Real>>>& modifiedValues)
    {
        ...
        for (Size i = 0; i < constrainedEvolvers_.size(); ++i)
        {
            for (Size j = 0; j < constrainedEvolvers_[i].size(); ++j)
            {
                ...
                singleEvolverValues(*(constrainedEvolvers_[i][j]), modifiedValues[i][j]);
            }
        }
    }

    void ProxyGreekEngine::multiplePathValues(SequenceStatisticsInc& stats,
    ...
    void ProxyGreekEngine::singleEvolverValues(MarketModelEvolver& evolver, std::vector<Real>& values, bool storeRates)
    {
        ...
        do
        {
            ...
            done = product_->nextTimeStep(evolver.currentState(), numberCashFlowsThisStep_, cashFlowsGenerated_);
            if (storeRates)
            {
                constraints_[thisStep] =
                    evolver.currentState().swapRate(startIndexOfConstraint_[thisStep], endIndexOfConstraint_[thisStep]);
                constraintsActive_[thisStep] = true;
            }

            Size numeraire = evolver.numeraires()[thisStep];
            ...
            for (Size i = 0; i < numberProducts_; ++i)
            {
                ...
                for (Size j = 0; j < numberCashFlowsThisStep_[i]; ++j)
                {
                    ...
                    Real bonds = cashflows[j].amount * discounter.numeraireBonds(evolver.currentState(), numeraire);
            ...
            if (!done)
            {
                ...
                principalInNumerairePortfolio *= evolver.currentState().discountRatio(numeraire, nextNumeraire);
            }
