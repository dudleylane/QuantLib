...
namespace QuantLib
{
    ...
    SwaptionPseudoDerivative::SwaptionPseudoDerivative(const ext::shared_ptr<MarketModel>& inputModel,
                                                       Size startIndex,
                                                       Size endIndex)
    {
        ...
        std::vector<Real> subForwards(inputModel->initialRates().begin() + startIndex,
                                      inputModel->initialRates().begin() + endIndex);
            ...
            for (Size j = startIndex; j < endIndex; ++j)
                for (Size k = startIndex; k < endIndex; ++k)
                    for (Size f = 0; f < factors; ++f)
                        thisVariance_ +=
                            zed[0][j - startIndex] * thisPseudo[j][f] * thisPseudo[k][f] * zed[0][k - startIndex];
    ...
    namespace
    {
        ...
        QuickCap::QuickCap(Real Strike,
                           ...
                           const std::vector<Real>& expiries,
                           Real price)
        : strike_(Strike), annuities_(annuities), currentRates_(currentRates), expiries_(expiries), price_(price)
        {
        }
        ...
        Real QuickCap::operator()(Real volatility) const
        {
            ...
            {
                price += blackFormula(Option::Call, strike_, currentRates_[i], volatility * std::sqrt(expiries_[i]),
                                      annuities_[i]);
            }
            return price - price_;
        }
        ...
        Real QuickCap::vega(Real volatility) const // returns vol derivative
        {
            ...
            for (Size i = 0; i < annuities_.size(); ++i)
            {
                ...
                vega +=
                    blackFormulaVolDerivative(strike_, currentRates_[i],

                                              volatility * std::sqrt(expiries_[i]), expiries_[i], annuities_[i], 0.0);
            }
    ...
    : firstDF_(firstDF)
    {
        ...
        QL_REQUIRE(endIndex <= inputModel->numberOfRates(),
        ...
        for (Size j = startIndex; j < endIndex; ++j)
        {
            ...
            Real capletPrice = blackFormula(Option::Call, strike, forward, sd, annuity, displacement);
        ...
        for (Size step = 0; step < inputModel->evolution().numberOfSteps(); ++step)
        {
            ...
            for (Size rate = std::max(inputModel->evolution().firstAliveRate()[step], startIndex); rate < endIndex;
                 ++rate)
            {
                for (Size f = 0; f < factors; ++f)
                {
                    ...
                    Real volDerivative =
                        inputModel->pseudoRoot(step)[rate][f] / (displacedImpliedVols[rate - startIndex] * expiry);
                    Real capletVega =
                        blackFormulaVolDerivative(strike, inputModel->initialRates()[rate],
                                                  displacedImpliedVols[rate - startIndex] * std::sqrt(expiry), expiry,
