...
namespace QuantLib
{
    ...
    : pseudoRoot_(pseudoRoot), aliveIndex_(aliveIndex), taus_(taus), displacements_(displacements),
      numberBumps_(pseudoBumps.size()), factors_(pseudoRoot.columns()), drifts_(taus.size()), bumpedRates_(taus.size())
    {
        Size numberRates = taus.size();

        QL_REQUIRE(pseudoRoot_.rows() == numberRates, "pseudoRoot_.rows()<> taus.size()");

        QL_REQUIRE(displacements_.size() == numberRates, "displacements_.size()<> taus.size()");

        QL_REQUIRE(drifts_.size() == numberRates, "drifts_.size()<> taus.size()");

        for (Size i = 0; i < pseudoBumps.size(); ++i)
        {
            QL_REQUIRE(pseudoBumps[i].rows() == numberRates, "pseudoBumps[i].rows()<> taus.size() with i =" << i);

            QL_REQUIRE(pseudoBumps[i].columns() == factors_, "pseudoBumps[i].columns()<> factors with i = " << i);
                                                   ...
                                                   Matrix& B)
    {
        ...
        QL_REQUIRE(B.rows() == numberBumps_, "B.rows()<> numberBumps_");

        QL_REQUIRE(B.columns() == taus_.size(), "B.columns()<> number of rates");

        for (Size i = 0; i < numberBumps_; ++i)
        {
            const Matrix& pseudo = pseudoBumped_[i];
            driftsComputers_[i].compute(oldRates, drifts_);
    ...
    : pseudoRoot_(pseudoRoot), aliveIndex_(aliveIndex), taus_(taus), pseudoBumps_(pseudoBumps),
      displacements_(std::move(displacements)), numberBumps_(pseudoBumps.size()), factors_(pseudoRoot.columns()),
                                          ...
                                          Matrix& B)
    {
        ...
        QL_REQUIRE(B.rows() == numberBumps_,
                   "we need B.rows() which is " << B.rows() << " to equal numberBumps_ which is " << numberBumps_);
        QL_REQUIRE(B.columns() == numberRates,
                   "we need B.columns() which is " << B.columns() << " to equal numberRates which is " << numberRates);
    ...
    RatePseudoRootJacobianAllElements::RatePseudoRootJacobianAllElements(const Matrix& pseudoRoot,
                                                                         ...
                                                                         const std::vector<Time>& taus,
                                                                         std::vector<Spread> displacements)
    : pseudoRoot_(pseudoRoot), aliveIndex_(aliveIndex), taus_(taus), displacements_(std::move(displacements)),
