...
namespace QuantLib
{
    ...
    {
        QL_REQUIRE(j < swaptions_.size() + caps_.size(),
        ...
        {
            SwaptionPseudoDerivative thisPseudo(bumps_.associatedModel(), swaptions_[j].startIndex_,
                                                swaptions_[j].endIndex_);
        ...
        else // its a cap
        {
            ...
            CapPseudoDerivative thisPseudo(bumps_.associatedModel(), caps_[j].strike_, caps_[j].startIndex_,
                                           caps_[j].endIndex_, 1.0); // ifrst df shouldn't make any difference
    ...
    OrthogonalizedBumpFinder::OrthogonalizedBumpFinder(
        ...
        Real tolerance)
    : derivativesProducer_(bumps, swaptions, caps), multiplierCutOff_(multiplierCutOff), tolerance_(tolerance)
    {
    }

    void OrthogonalizedBumpFinder::GetVegaBumps(std::vector<std::vector<Matrix>>& theBumps) const
    {
        OrthogonalProjections projector(derivativesProducer_.getAllOnePercentBumps(), multiplierCutOff_, tolerance_);
            ...
            if (projector.validVectors()[instrument])
            {
                for (Size cluster = 0; cluster < bumpClusters.size(); ++cluster)
                {
                    ...
                    for (Size step = bumpClusters[cluster].stepBegin(); step < bumpClusters[cluster].stepEnd(); ++step)
                        for (Size rate = bumpClusters[cluster].rateBegin(); rate < bumpClusters[cluster].rateEnd();
                             ++rate)
