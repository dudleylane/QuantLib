...
namespace QuantLib
{
    ...
    : numberOfRates_(taus.size()), numberOfFactors_(pseudo.columns()),
      ...
      oneOverTaus_(taus.size()), pseudo_(pseudo), tmp_(taus.size(), 0.0), e_(pseudo_.columns(), pseudo_.rows(), 0.0),
    ...
    void LMMNormalDriftCalculator::compute(const LMMCurveState& cs, std::vector<Real>& drifts) const
    {
        compute(cs.forwardRates(), drifts);
    }

    void LMMNormalDriftCalculator::compute(const std::vector<Rate>& fwds, std::vector<Real>& drifts) const
    {
    ...
    void LMMNormalDriftCalculator::computePlain(const LMMCurveState& cs, std::vector<Real>& drifts) const
    {
        computePlain(cs.forwardRates(), drifts);
    }

    void LMMNormalDriftCalculator::computePlain(const std::vector<Rate>& forwards, std::vector<Real>& drifts) const
    {
    ...
    void LMMNormalDriftCalculator::computeReduced(const LMMCurveState& cs, std::vector<Real>& drifts) const
    {
        computeReduced(cs.forwardRates(), drifts);
    }

    void LMMNormalDriftCalculator::computeReduced(const std::vector<Rate>& forwards, std::vector<Real>& drifts) const
    {
