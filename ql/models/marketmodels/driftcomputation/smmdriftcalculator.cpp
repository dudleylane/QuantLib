...
namespace QuantLib
{
    ...
    : numberOfRates_(taus.size()), numberOfFactors_(pseudo.columns()), numeraire_(numeraire), alive_(alive),
      ...
      wkaj_(pseudo_.columns(), pseudo_.rows(), 0.0), wkpj_(pseudo_.columns(), pseudo_.rows() + 1, 0.0),
    ...
    void SMMDriftCalculator::compute(const CoterminalSwapCurveState& cs, std::vector<Real>& drifts) const
    {
        ...
        for (Size k = 0; k < numberOfFactors_; ++k)
        {
            ...
            for (Integer j = numberOfRates_ - 2; j >= static_cast<Integer>(alive_) - 1; --j)
            {
                ...
                wkpj_[k][j + 1] = SR[j + 1] * (pseudo_[j + 1][k] * annuity + wkaj_[k][j + 1]) +
        ...
        for (Size k = 0; k < numberOfFactors_; ++k)
        {
            ...
            {
                wkajshifted_[k][j] = -wkaj_[k][j] / annuities[j] + wkpj_[k][numeraire_] * numeraireRatio;
            }
        }
