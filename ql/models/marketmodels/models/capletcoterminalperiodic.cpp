    ...
    )
    {
        ...
        do
        {
            ...
            CTSMMCapletMaxHomogeneityCalibration unperiodicCalibrator(
                evolution, corr, displacedSwapVariances.interpolatedVariances(), capletVols, cs, displacement,
                caplet0Swaption1Priority);


            failures = static_cast<QuantLib::Integer>(unperiodicCalibrator.calibrate(
                numberOfFactors, maxUnperiodicIterations, toleranceUnperiodic, max1dIterations, tolerance1d));

            swapCovariancePseudoRoots = unperiodicCalibrator.swapPseudoRoots();

            ext::shared_ptr<MarketModel> smm(
                new PseudoRootFacade(swapCovariancePseudoRoots, evolution.rateTimes(), cs->coterminalSwapRates(),
            ...
            ext::shared_ptr<MarketModel> periodflmm(new FwdPeriodAdapter(flmm, period, offset, newDisplacements));
            ...
            for (Size i = 0; i < numberBigRates; ++i)
            {
                ...
                totalSwaptionError +=
                    (marketSwaptionVols[i] - modelSwaptionVols[i]) * (marketSwaptionVols[i] - modelSwaptionVols[i]);
            }
        ...
        } while (errorImprovement > periodTolerance / 10.0 && periodSwaptionRmsError > periodTolerance &&
