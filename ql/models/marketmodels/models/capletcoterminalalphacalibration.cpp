...
namespace QuantLib
{
    ...
    : CTSMMCapletCalibration(evolution, corr, displacedSwapVariances, mktCapletVols, cs, displacement),
      alphaInitial_(alphaInitial), alphaMax_(alphaMax), alphaMin_(alphaMin), maximizeHomogeneity_(maximizeHomogeneity),
      parametricForm_(std::move(parametricForm)), alpha_(numberOfRates_), a_(numberOfRates_), b_(numberOfRates_)
    {
        if (!parametricForm_)
            parametricForm_ = ext::shared_ptr<AlphaForm>(new AlphaFormLinearHyperbolic(evolution.rateTimes()));

        QL_REQUIRE(numberOfRates_ == alphaInitial.size(), "mismatch between number of rates ("
                                                              << numberOfRates_ << ") and alphaInitial ("
                                                              << alphaInitial.size() << ")");

        QL_REQUIRE(numberOfRates_ == alphaMax.size(), "mismatch between number of rates ("
                                                          << numberOfRates_ << ") and alphaMax (" << alphaMax.size()
                                                          << ")");

        QL_REQUIRE(numberOfRates_ == alphaMin.size(), "mismatch between number of rates ("
                                                          << numberOfRates_ << ") and alphaMin (" << alphaMin.size()
                                                          << ")");
    }
        ...
        std::vector<Matrix>& swapCovariancePseudoRoots)
    {
        ...
        for (Size i = 0; i < numberOfRates - 1; ++i)
        {
            ...
            if (maximizeHomogeneity)
                success =
                    solver.solveWithMaxHomogeneity(alphaInitial[i + 1], i, firstRateVols, secondRateVols, correlations,
                                                   w0, w1, targetVariance, tolerance, alphaMax[i + 1], alphaMin[i + 1],
                                                   maxIterations, alpha[i + 1], a[i + 1], b[i + 1], theseNewVols);
            else
                success = solver.solve(alphaInitial[i + 1], i, firstRateVols, secondRateVols, correlations, w0, w1,
                                       targetVariance, tolerance, alphaMax[i + 1], alphaMin[i + 1], maxIterations,
    ...
    Natural
    CTSMMCapletAlphaFormCalibration::calibrationImpl_(Natural numberOfFactors, Natural maxIterations, Real tolerance)
    {

        return capletAlphaFormCalibration(evolution_, *corr_, displacedSwapVariances_,
