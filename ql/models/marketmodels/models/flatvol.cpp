...
namespace QuantLib
{
    ...
    {
        QL_REQUIRE(t1 <= t2, "integrations bounds (" << t1 << "," << t2 << ") are in reverse order");
    ...
    FlatVol::FlatVol(const vector<Volatility>& vols,
    ...
    : numberOfFactors_(numberOfFactors), numberOfRates_(initialRates.size()),
      numberOfSteps_(evolution.evolutionTimes().size()), initialRates_(initialRates), displacements_(displacements),
      evolution_(evolution), pseudoRoots_(numberOfSteps_, Matrix(numberOfRates_, numberOfFactors_))
    {
        ...
        QL_REQUIRE(numberOfRates_ == displacements.size(), "mismatch between number of rates ("
                                                               << numberOfRates_ << ") and displacements ("
                                                               << displacements.size() << ")");
        QL_REQUIRE(numberOfRates_ == vols.size(),
                   "mismatch between number of rates (" << numberOfRates_ << ") and vols (" << vols.size() << ")");
        ...
        QL_REQUIRE(numberOfFactors <= numberOfRates_,
                   "number of factors (" << numberOfFactors << ") cannot be greater than numberOfRates ("
                                         << numberOfRates_ << ")");
        ...
        for (Size k = 0, kk = 0; k < numberOfSteps_; ++k)
        {
            ...
            for (; corrTimes[kk] < evolTimes[k]; ++kk)
            {
                ...
                for (Size i = 0; i < numberOfRates_; ++i)
                {
                    ...
                    {
                        Real cov =
                            flatVolCovariance(effStartTime, effStopTime, rateTimes[i], rateTimes[j], vols[i], vols[j]);
            ...
            pseudoRoots_[k] = rankReducedSqrt(covariance, numberOfFactors, 1.0, SalvagingAlgorithm::None);

            QL_ENSURE(pseudoRoots_[k].rows() == numberOfRates_,
                      "step " << k << " flat vol wrong number of rows: " << pseudoRoots_[k].rows() << " instead of "
                              << numberOfRates_);
            QL_ENSURE(pseudoRoots_[k].columns() == numberOfFactors,
                      "step " << k << " flat vol wrong number of columns: " << pseudoRoots_[k].columns()
    ...
    FlatVolFactory::FlatVolFactory(Real longTermCorrelation,
                                   ...
                                   Spread displacement)
    : longTermCorrelation_(longTermCorrelation), beta_(beta), times_(std::move(times)), vols_(std::move(vols)),
      yieldCurve_(std::move(yieldCurve)), displacement_(displacement)
    {
        volatility_ = LinearInterpolation(times_.begin(), times_.end(), vols_.begin());
        volatility_.update();
        registerWith(yieldCurve_);
    }

    ext::shared_ptr<MarketModel> FlatVolFactory::create(const EvolutionDescription& evolution,
                                                        Size numberOfFactors) const
    {
        ...
        Matrix correlations = exponentialCorrelations(evolution.rateTimes(), longTermCorrelation_, beta_);
        ...
        return ext::shared_ptr<MarketModel>(
            new FlatVol(displacedVolatilities, corr, evolution, numberOfFactors, initialRates, displacements));
    }
