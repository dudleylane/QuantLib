...
namespace QuantLib
{
    ...
    : evolution_(std::move(evolution)), corr_(std::move(corr)),
      displacedSwapVariances_(std::move(displacedSwapVariances)), mktCapletVols_(std::move(mktCapletVols)),
      mdlCapletVols_(evolution_.numberOfRates()), mktSwaptionVols_(evolution_.numberOfRates()),
      mdlSwaptionVols_(evolution_.numberOfRates()), cs_(std::move(cs)), displacement_(displacement),
    ...
    {
        QL_REQUIRE(i < numberOfRates_, "index (" << i << ") must less than number of rates (" << numberOfRates_ << ")");
        ...
        const CurveState& cs)
    {
        ...
        QL_REQUIRE(evolutionTimes == corr.times(), "evolutionTimes " << io::sequence(evolutionTimes)
                                                                     << " not equal to correlation times "
                                                                     << io::sequence(corr.times()));

        const std::vector<Time>& rateTimes = evolution.rateTimes();
        QL_REQUIRE(rateTimes == cs.rateTimes(), "mismatch between EvolutionDescription and "
        ...
        QL_REQUIRE(numberOfRates == displacedSwapVariances.size(),
                   "mismatch between EvolutionDescription number of rates ("
                       << numberOfRates << ") and displacedSwapVariances size (" << displacedSwapVariances.size()
                       << ")");
        QL_REQUIRE(numberOfRates == corr.numberOfRates(), "mismatch between EvolutionDescription number of rates ("
                                                              << numberOfRates << ") and corr number of rates ("
                                                              << corr.numberOfRates() << ")");
        QL_REQUIRE(numberOfRates == mktCapletVols.size(), "mismatch between EvolutionDescription number of rates ("
                                                              << numberOfRates << ") and mktCapletVols size ("
                                                              << mktCapletVols.size() << ")");
        QL_REQUIRE(numberOfRates == cs.numberOfRates(), "mismatch between EvolutionDescription number of rates ("
                                                            << numberOfRates << ") and CurveState  number of rates ("
                                                            << cs.numberOfRates() << ")");

        std::vector<Time> temp(rateTimes.begin(), rateTimes.end() - 1);
        QL_REQUIRE(temp == evolutionTimes, "mismatch between evolutionTimes and rateTimes");

        Volatility lastSwaptionVol = displacedSwapVariances.back()->totalVolatility(numberOfRates - 1);
        QL_REQUIRE(close(lastSwaptionVol, mktCapletVols[numberOfRates - 1]),
                   "last caplet vol (" << std::setprecision(16) << mktCapletVols[numberOfRates - 1]
                                           ...
                                           Real innerSolvingTolerance)
    {
        ...
        std::vector<Spread> displacements(numberOfRates_, displacement_);
        ...
        do
        {
            failures_ = calibrationImpl_(numberOfFactors, innerSolvingMaxIterations, innerSolvingTolerance);

            ext::shared_ptr<MarketModel> ctsmm(
                new PseudoRootFacade(swapCovariancePseudoRoots_, rateTimes, cs_->coterminalSwapRates(), displacements));
            const Matrix& swaptionTotCovariance = ctsmm->totalCovariance(numberOfRates_ - 1);

            CotSwapToFwdAdapter flmm(ctsmm);
            const Matrix& capletTotCovariance = flmm.totalCovariance(numberOfRates_ - 1);
