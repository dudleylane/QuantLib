...
namespace QuantLib
{
    ...
    CTSMMCapletOriginalCalibration::CTSMMCapletOriginalCalibration(
        ...
        bool lowestRoot,
        bool useFullApprox)
    : CTSMMCapletCalibration(evolution, corr, displacedSwapVariances, mktCapletVols, cs, displacement), alpha_(alpha),
      lowestRoot_(lowestRoot), useFullApprox_(useFullApprox)
    {

        QL_REQUIRE(numberOfRates_ == alpha.size(),
                   "mismatch between number of rates (" << numberOfRates_ << ") and alpha (" << alpha.size() << ")");
    }
        ...
        std::vector<Matrix>& swapCovariancePseudoRoots)
    {
        ...
        QL_REQUIRE(numberOfFactors <= numberOfRates, "number of factors (" << numberOfFactors
                                                                           << ") cannot be greater than numberOfRates ("
                                                                           << numberOfRates << ")");
        QL_REQUIRE(numberOfFactors > 0, "number of factors (" << numberOfFactors << ") must be greater than zero");
        ...
        for (Size i = 0; i < corrPseudo.size(); ++i)
            corrPseudo[i] = rankReducedSqrt(corr.correlation(i), numberOfFactors, 1.0, SalvagingAlgorithm::None);

        Matrix zedMatrix = SwapForwardMappings::coterminalSwapZedMatrix(cs, displacement);
        ...
        for (Size i = 0; i < numberOfSteps; ++i)
        {
            ...
            for (Size j = i; j < numberOfRates; ++j)
            {
                ...
                swapTimeInhomogeneousVariances[i][j] = var[i] / ((1.0 + alpha[j] * s) * (1.0 + alpha[j] * s));
        ...
        for (Size i = 0; i < numberOfSteps; ++i)
        {
            ...
            for (Size j = 0; j < numberOfRates; ++j)
                for (Size k = 0; k < CovarianceSwapPseudos[i].columns(); ++k)
                    CovarianceSwapPseudos[i][j][k] *= std::sqrt(swapTimeInhomogeneousVariances[i][j]);

            CovarianceSwapMarginalCovs[i] = CovarianceSwapPseudos[i] * transpose(CovarianceSwapPseudos[i]);
        ...
        for (Size i = 0; i < numberOfRates; ++i)
        {
            ...
            for (j = 0; j <= static_cast<Integer>(i) - 2; ++j)
            {
                ...
                almostTotCovariance[i] += correlation * std::sqrt(swapTimeInhomogeneousVariances[j][i] *
            ...
            if (i > 0)
            {
                ...
                for (Size k = 0; k < numberOfFactors; ++k)
                    correlation += thisPseudo[i - 1][k] * thisPseudo[i][k];
                leftCovariance[i] = correlation * std::sqrt(swapTimeInhomogeneousVariances[j][i] *
                                                            swapTimeInhomogeneousVariances[j][i - 1]);
            }
        }
        ...
        for (Size i = 1; i < numberOfSteps; ++i)
        {
            ...
            for (Size k = i + 1; k < numberOfSteps; ++k)
                for (Size l = i + 1; l < numberOfSteps; ++l)
                    extraConstantPart +=
                        invertedZedMatrix[i - 1][k] * CovarianceSwapCovs[i - 1][k][l] * invertedZedMatrix[i - 1][l];
            ...
            for (Size k = i + 1; k < numberOfSteps; ++k)
            {
                if (i > 1)
                {
                    extraConstantPart += invertedZedMatrix[i - 1][i - 1] * CovarianceSwapCovs[i - 2][i - 1][k] *
                                         invertedZedMatrix[i - 1][k] * a[i - 1];

                    extraConstantPart += invertedZedMatrix[i - 1][k] * CovarianceSwapCovs[i - 2][k][i - 1] *
                                         invertedZedMatrix[i - 1][i - 1] * a[i - 1];
                }

                extraConstantPart += invertedZedMatrix[i - 1][i - 1] * CovarianceSwapMarginalCovs[i - 1][i - 1][k] *
                                     invertedZedMatrix[i - 1][k] * b[i - 1];

                extraConstantPart += invertedZedMatrix[i - 1][k] * CovarianceSwapCovs[i - 1][k][i - 1] *
            ...
            {
                extraLinearPart +=
                    invertedZedMatrix[i - 1][k] * CovarianceSwapCovs[i - 1][k][i] * invertedZedMatrix[i - 1][i];

                extraLinearPart +=
                    invertedZedMatrix[i - 1][i] * CovarianceSwapCovs[i - 1][i][k] * invertedZedMatrix[i - 1][k];
            }


            Real constantPart = w0 * w0 * totVariance[i - 1] + extraConstantPart * extraMultiplier - v1t1;
            Real linearPart = -2 * w0 * w1 * (a[i - 1] * almostTotCovariance[i] + b[i - 1] * leftCovariance[i]) +
            ...
            QL_ENSURE(root >= 0.0, "negative root -- it should have not happened");
        }
        ...
        for (Size k = 0; k < numberOfSteps; ++k)
        {
            ...
            QL_ENSURE(swapCovariancePseudoRoots[k].rows() == numberOfRates,
                      "step " << k << " abcd vol wrong number of rows: " << swapCovariancePseudoRoots[k].rows()
                              << " instead of " << numberOfRates);
            QL_ENSURE(swapCovariancePseudoRoots[k].columns() == numberOfFactors,
                      "step " << k << " abcd vol wrong number of columns: " << swapCovariancePseudoRoots[k].columns()
    ...
    Natural CTSMMCapletOriginalCalibration::calibrationImpl_(Natural numberOfFactors, Natural, Real)
    {

        return calibrationFunction(evolution_, *corr_, displacedSwapVariances_,
                                   // not mktCapletVols_ but...
                                   usedCapletVols_, *cs_, displacement_,

                                   alpha_, lowestRoot_, useFullApprox_,
