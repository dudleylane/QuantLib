...
namespace QuantLib
{
    ...
    : variances_(rateTimes.size() - 1, 0.0), volatilities_(rateTimes.size() - 1, 0.0), rateTimes_(rateTimes), a_(a),
      b_(b), c_(c), d_(d)
    {

        checkIncreasingTimes(rateTimes);
        QL_REQUIRE(rateTimes.size() > 1, "Rate times must contain at least two values");
        QL_REQUIRE(resetIndex < rateTimes_.size() - 1, "resetIndex (" << resetIndex
    ...
    const std::vector<Real>& PiecewiseConstantAbcdVariance::volatilities() const
    {
