...
namespace QuantLib
{
      ...
      displacements_(std::move(newDisplacements))
    {
        ...
        const std::vector<Spread>& largeDisplacements_ = largeModel->displacements();
        ...
        QL_REQUIRE(displacements_.size() == numberOfRates_,
        ...
        LMMCurveState smallCS(ForwardForwardMappings::RestrictCurveState(largeCS, period, offset));
        ...
        evolution_ = EvolutionDescription(smallCS.rateTimes(), newEvolutionTimes);

        numberOfSteps_ = newEvolutionTimes.size();


        const std::vector<Time>& rateTimes = smallCS.rateTimes();
        ...
        Matrix YMatrix = ForwardForwardMappings::YMatrix(largeCS, largeDisplacements_, displacements_, period, offset);

        const std::vector<Size>& alive = evolution_.firstAliveRate();
