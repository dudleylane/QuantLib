...
namespace QuantLib
{
    ...
    namespace
    {
        ...
        template <class T>
        Real Bisection(Real target, Real low, Real high, Real tolerance, T& theObject, Real (T::*Value)(Real))
        {
        ...
        template <class T>
        Real FindHighestOK(Real low, Real high, Real tolerance, T& theObject, bool (T::*Value)(Real))
        {
        ...
        template <class T>
        Real FindLowestOK(Real low, Real high, Real tolerance, T& theObject, bool (T::*Value)(Real))
        {
    ...
    AlphaFinder::AlphaFinder(ext::shared_ptr<AlphaForm> parametricform) : parametricform_(std::move(parametricform)) {}
    ...
    Real AlphaFinder::homogeneityfailure(Real alpha)
    {
        ...
        finalPart(alpha, stepindex_, ratetwohomogeneousvols_, computeQuadraticPart(alpha), computeLinearPart(alpha),
                  constantPart_, dum1, dum2, dum3, putativevols_);
    ...
    bool AlphaFinder::testIfSolutionExists(Real alpha)
    {
        ...
        return finalPart(alpha, stepindex_, ratetwohomogeneousvols_, computeQuadraticPart(alpha),
                         computeLinearPart(alpha), constantPart_, dum1, dum2, dum3, putativevols_);
    }
                            ...
                            std::vector<Volatility>& ratetwovols)
    {
        ...
        if (bottomValue <= targetVariance)
        {
            ...
            alpha = Bisection<AlphaFinder>(targetVariance, bottomAlpha, bilimit, tolerance, *this,
                                           &AlphaFinder::valueAtTurningPoint);
        }
        else
        {
            // find root of decreasing function (or as if decreasing function)
            alpha = Bisection<AlphaFinder>(-targetVariance, bilimit, topAlpha, tolerance, *this,
                                           &AlphaFinder::minusValueAtTurningPoint);
        }
        finalPart(alpha, stepindex, ratetwohomogeneousvols, quadraticPart_, linearPart_, constantPart_, alpha, a, b,
                  ratetwovols);
        return true;
    }
                                              ...
                                              std::vector<Volatility>& ratetwovols)
    {
        ...
        if (foundOKPoint)
        {
            if (!alphaMinOK)
            {
                ...
                if (alpha0OK)
                {
                    ...
                    alpha1 = FindLowestOK<AlphaFinder>(alphaMin, alpha0, tolerance, *this,
                                                       &AlphaFinder::testIfSolutionExists);
                }
                else
                {
                    // alphaMaxOK must be true to get here
                    alpha1 = FindLowestOK<AlphaFinder>(alpha0, alphaMax, tolerance, *this,
                                                       &AlphaFinder::testIfSolutionExists);
                }
            }
            ...
            if (!alphaMaxOK)
            {
                // higher alpha is bad
                alpha2 =
                    FindHighestOK<AlphaFinder>(alpha1, alphaMax, tolerance, *this, &AlphaFinder::testIfSolutionExists);
            }
            else
                alpha2 = alphaMax;
        }
        else
        {
            ...
            else
            {
                alpha2 = alphaDown;
                alpha1 =
                    FindLowestOK<AlphaFinder>(alphaMin, alpha2, tolerance, *this, &AlphaFinder::testIfSolutionExists);
            }
        }
        ...
        bool failed;
        alpha = Minimize<AlphaFinder>(alpha1, alpha2, tolerance, *this, &AlphaFinder::homogeneityfailure,
                                      &AlphaFinder::testIfSolutionExists, failed);

        finalPart(alpha, stepindex, ratetwohomogeneousvols, computeQuadraticPart(alpha), computeLinearPart(alpha),
                  constantPart_, alpha, a, b, ratetwovols);
