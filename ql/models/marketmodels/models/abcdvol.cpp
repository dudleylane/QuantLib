...
namespace QuantLib
{
      ...
      evolution_(evolution), pseudoRoots_(numberOfSteps_, Matrix(numberOfRates_, numberOfFactors_))
    {
        ...
        QL_REQUIRE(numberOfRates_ == ks.size(),
                   "mismatch between number of rates (" << numberOfRates_ << ") and ks (" << ks.size() << ")");
        ...
        for (Size k = 0, kk = 0; k < numberOfSteps_; ++k)
        {
            ...
            for (; corrTimes[kk] < evolTimes[k]; ++kk)
            {
                ...
                for (Size i = 0; i < numberOfRates_; ++i)
                {
                    ...
                    {
                        Real cov =
                            ks[i] * ks[j] * abcd.covariance(effStartTime, effStopTime, rateTimes[i], rateTimes[j]);
            ...
            for (Size i = 0; i < numberOfRates_; ++i)
            {
                for (Size j = i; j < numberOfRates_; ++j)
                {
                    Real cov = ks[i] * ks[j] * abcd.covariance(effStartTime, effStopTime, rateTimes[i], rateTimes[j]);
                    covariance[i][j] += cov * corrMatrix[i][j];
                }
            }
            ...
            QL_ENSURE(pseudoRoots_[k].rows() == numberOfRates_,
                      "step " << k << " abcd vol wrong number of rows: " << pseudoRoots_[k].rows() << " instead of "
                              << numberOfRates_);
            QL_ENSURE(pseudoRoots_[k].columns() == numberOfFactors,
                      "step " << k << " abcd vol wrong number of columns: " << pseudoRoots_[k].columns()
