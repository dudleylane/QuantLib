...
namespace QuantLib
{
    ...
    NormalFwdRatePc::NormalFwdRatePc(const ext::shared_ptr<MarketModel>& marketModel,
    ...
    : marketModel_(marketModel), numeraires_(numeraires), initialStep_(initialStep),
      ...
      initialForwards_(marketModel->initialRates()), drifts1_(numberOfRates_), drifts2_(numberOfRates_),
      ...
      alive_(marketModel->evolution().firstAliveRate())
    {
        ...
        for (Size j = 0; j < steps; ++j)
        {
            ...
            calculators_.emplace_back(A, marketModel->evolution().rateTaus(), numeraires[j], alive_[j]);
            /*
    ...
    {
        currentStep_ = initialStep_;
        std::copy(initialForwards_.begin(), initialForwards_.end(), forwards_.begin());
    ...
    Real NormalFwdRatePc::advanceStep()
    {
        ...
        for (i = alive; i < numberOfRates_; ++i)
        {
            ...
            forwards_[i] += std::inner_product(A.row_begin(i), A.row_end(i), brownians_.begin(), Real(0.0));
        }
