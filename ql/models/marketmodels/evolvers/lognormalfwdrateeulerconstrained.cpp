...
namespace QuantLib
{
    ...
    LogNormalFwdRateEulerConstrained::LogNormalFwdRateEulerConstrained(const ext::shared_ptr<MarketModel>& marketModel,
    ...
    : marketModel_(marketModel), numeraires_(numeraires), initialStep_(initialStep),
      ...
      drifts1_(numberOfRates_), initialDrifts_(numberOfRates_), brownians_(numberOfFactors_),
    ...
    void LogNormalFwdRateEulerConstrained::setConstraintType(const std::vector<Size>& startIndexOfSwapRate,
                                                             const std::vector<Size>& endIndexOfSwapRate)
    {
        QL_REQUIRE(startIndexOfSwapRate.size() == numeraires_.size(), "Size mismatch in constraint specification.");
    ...
    void LogNormalFwdRateEulerConstrained::setThisConstraint(const std::vector<Rate>& rateConstraints,
                                                             const std::valarray<bool>& isConstraintActive)
    {
        QL_REQUIRE(rateConstraints.size() == numeraires_.size(), "wrong number of constraints specified");

        QL_REQUIRE(isConstraintActive.size() == numeraires_.size(), "wrong number of isConstraintActive specified");
