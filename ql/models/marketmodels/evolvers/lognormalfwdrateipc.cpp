...
namespace QuantLib
{
    ...
    LogNormalFwdRateIpc::LogNormalFwdRateIpc(const ext::shared_ptr<MarketModel>& marketModel,
    ...
    : marketModel_(marketModel), numeraires_(numeraires), initialStep_(initialStep),
      ...
      drifts1_(numberOfRates_), initialDrifts_(numberOfRates_), g_(numberOfRates_), brownians_(numberOfFactors_),
      correlatedBrownians_(numberOfRates_), rateTaus_(marketModel->evolution().rateTaus()),
      alive_(marketModel->evolution().firstAliveRate())
    {
        checkCompatibility(marketModel->evolution(), numeraires);
        QL_REQUIRE(isInTerminalMeasure(marketModel->evolution(), numeraires), "terminal measure required for ipc ");
