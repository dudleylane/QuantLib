...
namespace QuantLib
{
    SquareRootAndersen::SquareRootAndersen(Real meanLevel,
                                           ...
                                           Real w2,
                                           Real cutPoint)
    : theta_(meanLevel), k_(reversionSpeed), epsilon_(volVar), v0_(v0), numberSubSteps_(numberSubSteps),
      dt_(evolutionTimes.size() * numberSubSteps), eMinuskDt_(evolutionTimes.size() * numberSubSteps), w1_(w1), w2_(w2),
      PsiC_(cutPoint), vPath_(evolutionTimes.size() * numberSubSteps + 1), state_(1)
    {
        Size j = 0;
