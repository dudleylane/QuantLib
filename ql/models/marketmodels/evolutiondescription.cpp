...
namespace QuantLib
{
    ...
    EvolutionDescription::EvolutionDescription(const std::vector<Time>& rateTimes,
                                               const std::vector<Time>& evolutionTimes,
                                               const std::vector<std::pair<Size, Size>>& relevanceRates)
    : numberOfRates_(rateTimes.empty() ? 0 : rateTimes.size() - 1), rateTimes_(rateTimes),
      evolutionTimes_(evolutionTimes.empty() && !rateTimes.empty() ?
                          std::vector<Time>(rateTimes.begin(), rateTimes.end() - 1) :
                          evolutionTimes),
      relevanceRates_(relevanceRates), rateTaus_(numberOfRates_),
      ...
      firstAliveRate_(evolutionTimes_.size())
    {
        ...
        QL_REQUIRE(evolutionTimes_.back() <= rateTimes[rateTimes.size() - 2],
                   "The last evolution time (" << evolutionTimes_.back() << ") is past the last fixing time ("
                                               << rateTimes[numberOfRates_ - 2] << ")");

        if (relevanceRates.empty())
            relevanceRates_ = std::vector<std::pair<Size, Size>>(numberOfSteps, std::make_pair(0, numberOfRates_));
        else
            QL_REQUIRE(relevanceRates.size() == numberOfSteps, "relevanceRates / evolutionTimes mismatch");
    ...
    void checkCompatibility(const EvolutionDescription& evolution, const std::vector<Size>& numeraires)
    {
        ...
        QL_REQUIRE(numeraires.size() == n,
                   "Size mismatch between numeraires (" << numeraires.size() << ") and evolution times (" << n << ")");

        const std::vector<Time>& rateTimes = evolution.rateTimes();
        for (Size i = 0; i < n - 1; i++)
            QL_REQUIRE(rateTimes[numeraires[i]] >= evolutionTimes[i],
                       io::ordinal(i + 1)
                           << " step, evolution time " << evolutionTimes[i] << ": the numeraire (" << numeraires[i]
    ...
    {
        const std::vector<Time>& rateTimes = evolution.rateTimes();
        return *std::min_element(numeraires.begin(), numeraires.end()) == rateTimes.size() - 1;
    }

    bool
    isInMoneyMarketPlusMeasure(const EvolutionDescription& evolution, const std::vector<Size>& numeraires, Size offset)
    {
        bool res = true;
        const std::vector<Time>& rateTimes = evolution.rateTimes();
        Size maxNumeraire = rateTimes.size() - 1;
        QL_REQUIRE(offset <= maxNumeraire, "offset (" << offset
    ...
    {
        return std::vector<Size>(evolution.evolutionTimes().size(), evolution.rateTimes().size() - 1);
    }
