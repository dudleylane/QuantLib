...
namespace QuantLib
{
    ...
    class PathwiseAccountingEngine
    {
      ...
      private:
        ...
        //     std::vector<std::vector<std::vector<Real> > > V_; // one V for each product, with components for each
        //     time step and rate
    ...
    // So for each vega, we have a vector of matrices. So we need a vector of vectors of matrices to compute all the
    // vegas. We do the outermost vector by time step and inner one by which vega. This is tested in
    ...
    // To compute a vega means changing the pseudo-square root at each time step
    // So for each vega, we have a vector of matrices. So we need a vector of vectors of matrices to compute all the
    // vegas. We do the outermost vector by time step and inner one by which vega. This implementation is different in
    // that all the linear combinations by the bumps are done as late as possible, whereas PathwiseVegasAccountingEngine
    // does them as early as possible. This is tested in MarketModelTest::testPathwiseVegas
