...
namespace QuantLib
{
    ...
    namespace
    {
        ...
        {
          public:
            explicit DecoratedHedge(const CallSpecifiedMultiProduct& product) : CallSpecifiedMultiProduct(product)
            {
            ...
            void reset() override
            {
                ...
                for (Size i = 0; i < lastSavedStep_; ++i)
                    CallSpecifiedMultiProduct::nextTimeStep(*savedStates_[i], numberCashFlowsThisStep_,
                                                            cashFlowsGenerated_);
                enableCallability();
            }
                              ...
                              std::vector<std::vector<CashFlow>>& cashFlowsGenerated) override
            {
                ...
                return CallSpecifiedMultiProduct::nextTimeStep(currentState, numberCashFlowsThisStep,
                                                               cashFlowsGenerated);
            }
    ...
    UpperBoundEngine::UpperBoundEngine(ext::shared_ptr<MarketModelEvolver> evolver,
    ...
    : evolver_(std::move(evolver)), innerEvolvers_(std::move(innerEvolvers)), composite_(MultiProductComposite()),
      initialNumeraireValue_(initialNumeraireValue)
    {
        ...
        composite_.add(DecoratedHedge(CallSpecifiedMultiProduct(hedge, hedgeStrategy, ExerciseAdapter(hedgeRebate))));
        composite_.finalize();
        ...
        const std::vector<Time>& evolutionTimes = composite_.evolution().evolutionTimes();
        numberOfSteps_ = evolutionTimes.size();

        isExerciseTime_.resize(evolutionTimes.size());
        isExerciseTime_ = isInSubset(evolutionTimes, hedgeStrategy.exerciseTimes());
        ...
        const std::vector<Time>& cashFlowTimes = composite_.possibleCashFlowTimes();
        const std::vector<Rate>& rateTimes = composite_.evolution().rateTimes();
    ...
    void UpperBoundEngine::multiplePathValues(Statistics& stats, Size outerPaths, Size innerPaths)
    {
    ...
    std::pair<Real, Real> UpperBoundEngine::singlePathValue(Size innerPaths)
    {
        ...
        for (Size k = 0; k < numberOfSteps_; ++k)
        {
            ...
            composite_.nextTimeStep(evolver_->currentState(), numberCashFlowsThisStep_, cashFlowsGenerated_);

            // First, we accumulate cash flows from both the
            // underlying...
            Real underlyingCashFlows = collectCashFlows(k, principalInNumerairePortfolio, underlyingOffset_,
                                                        underlyingOffset_ + underlyingSize_);

            // ...and the hedge
            Real hedgeCashFlows =
                collectCashFlows(k, principalInNumerairePortfolio, hedgeOffset_, hedgeOffset_ + hedgeSize_);
            ...
            Real rebateCashFlow =
                collectCashFlows(k, principalInNumerairePortfolio, rebateOffset_, rebateOffset_ + rebateSize_);

            Real hedgeRebateCashFlow = collectCashFlows(k, principalInNumerairePortfolio, hedgeRebateOffset_,
            ...
            if (isExerciseTime_[k])
            {
                ...
                if (k != numberOfSteps_ - 1)
                {
                    ...
                    ext::shared_ptr<MarketModelEvolver> currentEvolver = innerEvolvers_[exercise++];
                    ...
                    unexercisedHedgeValue =
                        std::accumulate(values.begin(), values.end(), Real(0.0)) / principalInNumerairePortfolio;
                                            ...
                                            Size endProduct) const
    {
        ...
        for (Size i = beginProduct; i < endProduct; ++i)
        {
            ...
            for (Size j = 0; j < numberCashFlowsThisStep_[i]; ++j)
            {
                ...
                numeraireUnits += cashflows[j].amount * discounter.numeraireBonds(evolver_->currentState(), numeraire);
            }
        }
