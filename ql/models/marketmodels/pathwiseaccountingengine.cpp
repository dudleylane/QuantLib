...
namespace QuantLib
{
    ...
    Real PathwiseAccountingEngine::singlePathValues(std::vector<Real>& values)
    {
        ...
        for (Integer currentStep = numberSteps_ - 1; currentStep >= 0;
             --currentStep) // must be a signed type as we go negative
        {
                ...
                if (!noFlows)
                {
                    if (doDeflation_)
                        discounters_[cashFlowIndex].getFactors(
                            ...
                            deflatorAndDerivatives_); // get amount to discount cash flow by and amount to multiply its
                                                      // derivatives by
                        ...
                        if (numberCashFlowsThisIndex_[j][cashFlowIndex] > 0)
                        {
                            ...
                            for (Size i = 1; i <= numberRates_; ++i)
                            {
                                ...
                                if (doDeflation_)
                                {
                                    ...
                                    thisDerivative +=
                                        totalCashFlowsThisIndex_[j][cashFlowIndex][0] * deflatorAndDerivatives_[i];
                                }
                ...
                if (nextStepIndex != stepToUse) // then we need to update V
                {
                    ...
                    for (Size i = 0; i < numberProducts_; ++i)
                    {
                        ...
                        for (Size f = 0; f < factors; ++f)
                        {
                            ...
                            {
                                Real thisPartialTermr =
                                    LIBORRates_[stepToUse][r] * V_[i][stepToUse][r] * thisPseudoRoot_[r][f];
        ...
        return 1.0; // we have put the weight in already, this results in lower variance since weight changes along the
                    // path
    }

    void PathwiseAccountingEngine::multiplePathValues(SequenceStatisticsInc& stats, Size numberOfPaths)
    {
      ...
      deflatorAndDerivatives_(pseudoRootStructure_->numberOfRates() + 1)
    {
        ...
        for (Size i = 0; i < numberSteps_; ++i)
        {
            ...
            jacobianComputers_.emplace_back(pseudoRootStructure_->pseudoRoot(i), evolution.firstAliveRate()[i],
                                            numeraires_[i], evolution.rateTaus(), vegaBumps[i],
    ...
    Real PathwiseVegasAccountingEngine::singlePathValues(std::vector<Real>& values)
    {
        ...
        do
        {
            ...
            jacobianComputers_[thisStep].getBumps(lastForwards_, stepsDiscounts_, currentForwards_,
                                                  evolver_->browniansThisStep(), jacobiansThisPaths_[thisStep]);
                    ...
                    for (Size j = 0; j < numberProducts_; ++j)
                    {
                        if (numberCashFlowsThisIndex_[j][cashFlowIndex] > 0)
                        {
                            ...
                            for (Size k = 0; k < numberBumps_; ++k)
                                ...
                                {
                                    vegasThisPath_[j][k] +=
                                        fullDerivatives_[i] * jacobiansThisPaths_[stepToUse - 1][k][i];
                                }
                    ...
                    for (Size i = 0; i < numberProducts_; ++i)
                    {
                        ...
                        if (nextStepIndex > 0)
                            ...
                            for (Size l = 0; l < numberBumps_; ++l)
                                for (Size j = 0; j < numberRates_; ++j)
                                    vegasThisPath_[i][l] +=
                                        V_[i][nextStepIndex][j] * jacobiansThisPaths_[nextStepIndex - 1][l][j];
    ...
    void PathwiseVegasAccountingEngine::multiplePathValues(std::vector<Real>& means,
    ...
    PathwiseVegasOuterAccountingEngine::PathwiseVegasOuterAccountingEngine(
        ...
        Real initialNumeraireValue)
    : evolver_(std::move(evolver)), product_(product), pseudoRootStructure_(std::move(pseudoRootStructure)),
      vegaBumps_(vegaBumps), initialNumeraireValue_(initialNumeraireValue),
      numberProducts_(product->numberOfProducts()), doDeflation_(!product->alreadyDeflated()),
      numerairesHeld_(product->numberOfProducts()), numberCashFlowsThisStep_(product->numberOfProducts()),
      cashFlowsGenerated_(product->numberOfProducts()), stepsDiscounts_(pseudoRootStructure_->numberOfRates() + 1),
      ...
      deflatorAndDerivatives_(pseudoRootStructure_->numberOfRates() + 1)
    {
        ...
        {
            jacobianComputers_.emplace_back(pseudoRootStructure_->pseudoRoot(i), evolution.firstAliveRate()[i],
                                            numeraires_[i], evolution.rateTaus(),
                                            pseudoRootStructure_->displacements());
        ...
        //        std::vector<std::vector<std::vector<Matrix> >  > elementary_vegas_ThisPath_;  // dimensions are
        //        product, step, rate, rate and factor
    ...
    Real PathwiseVegasOuterAccountingEngine::singlePathValues(std::vector<Real>& values)
    {
        ...
        for (Size i = 0; i < numberProducts_; ++i)
        {
            for (Size j = 0; j < numberSteps_; ++j)
            {
                ...
                // note the simplification here arising from the fact that the elementary vega affects the evolution on
                // precisely one step
                ...
                for (Size k = 0; k < numberRates_; ++k)
                    for (Size f = 0; f < factors_; ++f)
                    {
                        ...
                        /*
                                                          if (j ==distinguishedStep_ && k ==distinguishedRate_ &&f==
                           distinguishedFactor_) std::cout << sensitivity << "," <<  jacobiansThisPaths_[j][j][k][f] <<
                           "," << gaussians_[j][f] << "," << V_[i][nextIndex][j] << "," << LIBORRates_[nextIndex][j] <<
                           "\n";
                          */
    ...
    void PathwiseVegasOuterAccountingEngine::multiplePathValuesElementary(std::vector<Real>& means,
                                                                          std::vector<Real>& errors,
                                                                          Size numberOfPaths)
    {
    ...
    void PathwiseVegasOuterAccountingEngine::multiplePathValues(std::vector<Real>& means,
                                                                std::vector<Real>& errors,
                                                                Size numberOfPaths)
    {
        ...
        errors.resize((1 + numberRates_ + numberBumps_) *
                      numberProducts_); // post linear combinations, errors are not meaningful so don't attempt to
                                        // compute s.e.s for vegas
            ...
            for (Size bump = 0; bump < numberBumps_; ++bump)
            {
                ...
                for (Size t = 0; t < numberSteps_; ++t)
                    for (Size r = 0; r < numberRates_; ++r)
                        for (Size f = 0; f < factors_; ++f)
                            thisVega +=
                                vegaBumps_[t][bump][r][f] * allMeans[p * inDataPerProduct + 1 + numberRates_ +
