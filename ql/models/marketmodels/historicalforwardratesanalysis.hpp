...
namespace QuantLib
{
                                        ...
                                        const Interpolator& i = Interpolator())
    {
        ...
        for (ibor = iborIndexes.begin(); ibor != iborIndexes.end(); ++ibor)
        {
            ...
            rateHelpers.push_back(ext::shared_ptr<RateHelper>(
                new DepositRateHelper(quoteHandle, (*ibor)->tenor(), (*ibor)->fixingDays(), (*ibor)->fixingCalendar(),
                                      (*ibor)->businessDayConvention(), (*ibor)->endOfMonth(), (*ibor)->dayCounter())));
        }
        ...
        for (swap = swapIndexes.begin(); swap != swapIndexes.end(); ++swap)
        {
            ...
            Handle<Quote> quoteHandle(quote);
            rateHelpers.push_back(ext::shared_ptr<RateHelper>(new SwapRateHelper(
                quoteHandle, (*swap)->tenor(), (*swap)->fixingCalendar(), (*swap)->fixedLegTenor().frequency(),
                (*swap)->fixedLegConvention(), (*swap)->dayCounter(), (*swap)->iborIndex())));
        }
        ...
        PiecewiseYieldCurve<Traits, Interpolator> yc(settlementDays, cal, rateHelpers, yieldCurveDayCounter,
                                                     std::vector<Handle<Quote>>(), std::vector<Date>(), i, bootstrap);
        ...
        for (; currentDate <= endDate; currentDate = cal.advance(currentDate, step, Following))
        {
            ...
            try
            {
                for (Size i = 0; i < nRates; ++i)
                {
                    ...
                    fwdRates[i] = yc.forwardRate(d, indexTenor, indexDayCounter, Simple);
                }
            }
            catch (std::exception& e)
            {
    ...
    // inline
    template <class Traits, class Interpolator>
    const std::vector<Period>& HistoricalForwardRatesAnalysisImpl<Traits, Interpolator>::fixingPeriods() const
    {
        return fixingPeriods_;
    }

    template <class Traits, class Interpolator>
    inline const std::vector<Date>& HistoricalForwardRatesAnalysisImpl<Traits, Interpolator>::skippedDates() const
    {
    ...
    template <class Traits, class Interpolator>
    inline const std::vector<Date>& HistoricalForwardRatesAnalysisImpl<Traits, Interpolator>::failedDates() const
    {
        return failedDates_;
    }
    ...
    {
        historicalForwardRatesAnalysis<Traits, Interpolator>(
            *stats_, skippedDates_, skippedDatesErrorMessage_, failedDates_, failedDatesErrorMessage_, fixingPeriods_,
            startDate, endDate, step, fwdIndex, initialGap, horizon, iborIndexes, swapIndexes, yieldCurveDayCounter,
            yieldCurveAccuracy);
    }
}
