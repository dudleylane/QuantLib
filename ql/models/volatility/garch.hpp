...
namespace QuantLib
{
    ...
    class Garch11 : public VolatilityCompositor
    {
      public:
        ...
        : alpha_(a), beta_(b), gamma_(1 - a - b), vl_(vl), logLikelihood_(0), mode_(BestOfTwo)
        {
        }
        ...
        {
            const auto values = quoteSeries.values();
            calibrate(values.cbegin(), values.cend(), method, endCriteria);
        }
        ...
        {
            const auto values = quoteSeries.values();
            calibrate(values.cbegin(), values.cend(), method, endCriteria, initialGuess);
        }
        ...
        void calibrate(ForwardIterator begin, ForwardIterator end)
        {
            ...
            ext::shared_ptr<Problem> p = calibrate_r2(mode_, r2, mean_r2, alpha_, beta_, vl_);
        ...
        void calibrate(ForwardIterator begin, ForwardIterator end, OptimizationMethod& method, EndCriteria endCriteria)
        {
            std::vector<Volatility> r2;
            Real mean_r2 = to_r2(begin, end, r2);
            ext::shared_ptr<Problem> p = calibrate_r2(mode_, r2, mean_r2, method, endCriteria, alpha_, beta_, vl_);
            gamma_ = 1 - alpha_ - beta_;
            vl_ /= gamma_;
            logLikelihood_ = p ? -p->functionValue() : -costFunction(begin, end);
        }
                       ...
                       const Array& initialGuess)
        {
            std::vector<Volatility> r2;
            to_r2(begin, end, r2);
            ext::shared_ptr<Problem> p = calibrate_r2(r2, method, endCriteria, initialGuess, alpha_, beta_, vl_);
            gamma_ = 1 - alpha_ - beta_;
            vl_ /= gamma_;
            logLikelihood_ = p ? -p->functionValue() : -costFunction(begin, end);
        }

        Real forecast(Real r, Real sigma2) const { return gamma_ * vl_ + alpha_ * r * r + beta_ * sigma2; }
        ...
        static ext::shared_ptr<Problem>
        calibrate_r2(Mode mode, const std::vector<Volatility>& r2, Real mean_r2, Real& alpha, Real& beta, Real& omega);
