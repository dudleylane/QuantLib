...
namespace QuantLib
{
    ...
    namespace
    {
        ...
        class Garch11Constraint : public Constraint
        {
            ...
            class Impl final : public Constraint::Impl
            {
              ...
              public:
                ...
                bool test(const Array& x) const override
                {
                    ...
                    return x[0] > 0 && x[1] >= 0 && x[2] >= 0 && x[1] + x[2] < gammaUpper_ &&
                           x[1] + x[2] >= gammaLower_;
                }
            };
          ...
          public:
            ...
            : Constraint(ext::shared_ptr<Constraint::Impl>(new Garch11Constraint::Impl(gammaLower, gammaUpper)))
            {
            }
        };
        ...
        void Garch11CostFunction::gradient(Array& grad, const Array& x) const
        {
            ...
            std::transform(grad.begin(), grad.end(), grad.begin(), [=](Real x) -> Real { return x / norm; });
        }
        ...
        class FitAcfProblem : public LeastSquareProblem
        {
          public:
            ...
            void targetValueAndGradient(const Array& x, Matrix& grad_fct2fit, Array& target, Array& fct2fit) override;
        ...
        class FitAcfConstraint : public Constraint
        {
          ...
          public:
            ...
            : Constraint(ext::shared_ptr<Constraint::Impl>(new FitAcfConstraint::Impl(gammaLower, gammaUpper)))
            {
            }
        };
        ...
        // acf(0), and acf(1).
        Real initialGuess1(const Array& acf, Real mean_r2, Real& alpha, Real& beta, Real& omega)
        {
            Real A21 = acf[1];
            ...
            Real gammaLower =
                A <= 1. / 3. - tol_level ? std::sqrt((1 - 3 * A) / (3 - 3 * A)) + tol_level : Real(tol_level);
        ...
        Real initialGuess2(const Array& acf, Real mean_r2, Real& alpha, Real& beta, Real& omega)
        {
            Real A21 = acf[1];
            Real A4 = acf[0] + mean_r2 * mean_r2;
            Real A = mean_r2 * mean_r2 / A4; // 1/sigma^2
    ...
    Garch11::time_series Garch11::calculate(const time_series& quoteSeries, Real alpha, Real beta, Real omega)
    {
        time_series retval;
    ...
    ext::shared_ptr<Problem> Garch11::calibrate_r2(
        Mode mode, const std::vector<Volatility>& r2, Real mean_r2, Real& alpha, Real& beta, Real& omega)
    {
        EndCriteria endCriteria(10000, 500, tol_level, tol_level, tol_level);
        Simplex method(0.001);
        return calibrate_r2(mode, r2, mean_r2, method, endCriteria, alpha, beta, omega);
    }

    ext::shared_ptr<Problem> Garch11::calibrate_r2(Mode mode,
                                                   ...
                                                   Real& omega)
    {
        ...
        std::transform(r2.begin(), r2.end(), tmp.begin(), [=](Real x) -> Real { return x - mean_r2; });
        ...
        if (mode != DoubleOptimization)
        {
            try
            {
                ret = calibrate_r2(r2, method, constraints, endCriteria, fCost1 <= fCost2 ? opt1 : opt2, alpha, beta,
                                   omega);
            }
            catch (const std::exception&)
            {
    ...
    ext::shared_ptr<Problem> Garch11::calibrate_r2(const std::vector<Volatility>& r2,
                                                   OptimizationMethod& method,
                                                   const EndCriteria& endCriteria,
                                                   const Array& initGuess,
                                                   Real& alpha,
                                                   Real& beta,
                                                   Real& omega)
    {
    ...
    {
        Garch11CostFunction cost(r2);
        ext::shared_ptr<Problem> problem(new Problem(cost, constraints, initGuess));
        EndCriteria::Type ret = method.minimize(*problem, endCriteria);
        QL_REQUIRE(ret != EndCriteria::None && ret != EndCriteria::Unknown, "GARCH(1,1) calibration failed to run "
                                                                            "(EndCriteria="
                                                                                << ret << ")");
