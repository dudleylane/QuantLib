...
namespace QuantLib
{
    ...
    HullWhite::HullWhite(const Handle<YieldTermStructure>& termStructure, Real a, Real sigma)
    : Vasicek(termStructure->forwardRate(0.0, 0.0, Continuous, NoFrequency), a, 0.0, sigma, 0.0),
    ...
    ext::shared_ptr<Lattice> HullWhite::tree(const TimeGrid& grid) const
    {
        ...
        ext::shared_ptr<ShortRateDynamics> numericDynamics(new Dynamics(phi, a(), sigma()));
    ...
    Real HullWhite::discountBondOption(Option::Type type, Real strike, Time maturity, Time bondMaturity) const
    {
        ...
        {
            v = sigma() * B(maturity, bondMaturity) * std::sqrt(0.5 * (1.0 - std::exp(-2.0 * _a * maturity)) / _a);
        }
    ...
    Real HullWhite::discountBondOption(
    ...
    Rate HullWhite::convexityBias(Real futuresPrice, Time t, Time T, Real sigma, Real a)
    {
        QL_REQUIRE(futuresPrice >= 0.0, "negative futures price (" << futuresPrice << ") not allowed");
        QL_REQUIRE(t >= 0.0, "negative t (" << t << ") not allowed");
        QL_REQUIRE(T >= t, "T (" << T << ") must not be less than t (" << t << ")");
        QL_REQUIRE(sigma >= 0.0, "negative sigma (" << sigma << ") not allowed");
        QL_REQUIRE(a >= 0.0, "negative a (" << a << ") not allowed");
