...
namespace QuantLib
{
    ...
    class CoxIngersollRoss::VolatilityConstraint : public Constraint
    {
      ...
      public:
        ...
        : Constraint(ext::shared_ptr<Constraint::Impl>(new VolatilityConstraint::Impl(k, theta)))
        {
        }
    };

    CoxIngersollRoss::CoxIngersollRoss(Rate r0, Real theta, Real k, Real sigma, bool withFellerConstraint)
    : OneFactorAffineModel(4), theta_(arguments_[0]), k_(arguments_[1]), sigma_(arguments_[2]), r0_(arguments_[3])
    {
    ...
    ext::shared_ptr<OneFactorModel::ShortRateDynamics> CoxIngersollRoss::dynamics() const
    {
        return ext::shared_ptr<ShortRateDynamics>(new Dynamics(theta(), k(), sigma(), x0()));
    }
    ...
    Real CoxIngersollRoss::A(Time t, Time T) const
    {
        ...
        Real value = std::log(numerator / denominator) * 2.0 * k() * theta() / sigma2;
    ...
    Real CoxIngersollRoss::discountBondOption(Option::Type type, Real strike, Time t, Time s) const
    {
        ...
        Real call = discountS * chis(2.0 * z * (rho + psi + b)) - strike * discountT * chit(2.0 * z * (rho + psi));
    ...
    ext::shared_ptr<Lattice> CoxIngersollRoss::tree(const TimeGrid& grid) const
    {
        ext::shared_ptr<TrinomialTree> trinomial(new TrinomialTree(dynamics()->process(), grid, true));
