...
namespace QuantLib
{
    ...
    ExtendedCoxIngersollRoss::ExtendedCoxIngersollRoss(const Handle<YieldTermStructure>& termStructure,
                                                       Real theta,
                                                       Real k,
                                                       Real sigma,
                                                       Real x0,
                                                       bool withFellerConstraint)
    : CoxIngersollRoss(x0, theta, k, sigma, withFellerConstraint), TermStructureConsistentModel(termStructure)
    {
        ExtendedCoxIngersollRoss::generateArguments();
    }

    ext::shared_ptr<Lattice> ExtendedCoxIngersollRoss::tree(const TimeGrid& grid) const
    {
        TermStructureFittingParameter phi(termStructure());
        ext::shared_ptr<Dynamics> numericDynamics(new Dynamics(phi, theta(), k(), sigma(), x0()));

        ext::shared_ptr<TrinomialTree> trinomial(new TrinomialTree(numericDynamics->process(), grid, true));

        typedef TermStructureFittingParameter::NumericalImpl NumericalImpl;
        ext::shared_ptr<NumericalImpl> impl = ext::dynamic_pointer_cast<NumericalImpl>(phi.implementation());

        return ext::shared_ptr<Lattice>(new ShortRateTree(trinomial, numericDynamics, impl, grid));
    }
    ...
    Real ExtendedCoxIngersollRoss::discountBondOption(Option::Type type, Real strike, Time t, Time s) const
    {
