...
namespace QuantLib
{
    ...
    : Gaussian1dModel(termStructure), CalibratedModel(1), modelSettings_(std::move(modelSettings)),
      capletCalibrated_(false), reversion_(ConstantParameter(reversion, NoConstraint())), sigma_(arguments_[0]),
      volstepdates_(std::move(volstepdates)), volatilities_(std::move(volatilities)), swaptionVol_(swaptionVol),
      swaptionExpiries_(swaptionExpiries), swaptionTenors_(swaptionTenors), swapIndexBase_(swapIndexBase),
      iborIndex_(swapIndexBase->iborIndex())
    {

        QL_REQUIRE(swaptionExpiries.size() == swaptionTenors.size(),
                   "number of swaption expiries (" << swaptionExpiries.size()
                                                   << ") is differnt from number of swaption tenors ("
                                                   << swaptionTenors.size() << ")");
        QL_REQUIRE(!swaptionExpiries.empty(), "need at least one swaption expiry to calibrate numeraire");
    ...
    MarkovFunctional::MarkovFunctional(const Handle<YieldTermStructure>& termStructure,
                                       ...
                                       MarkovFunctional::ModelSettings modelSettings)
    : Gaussian1dModel(termStructure), CalibratedModel(1), modelSettings_(std::move(modelSettings)),
      capletCalibrated_(true), reversion_(ConstantParameter(reversion, NoConstraint())), sigma_(arguments_[0]),
      volstepdates_(std::move(volstepdates)), volatilities_(std::move(volatilities)), capletVol_(capletVol),
      capletExpiries_(capletExpiries), iborIndex_(std::move(iborIndex))
    {

        QL_REQUIRE(!capletExpiries.empty(), "need at least one caplet expiry to calibrate numeraire");
        QL_REQUIRE(!termStructure.empty(), "yield term structure handle is empty");
    ...
    void MarkovFunctional::updateTimes1() const
    {
        ...
        for (auto i = volstepdates_.begin(); i != volstepdates_.end(); ++i, ++j)
        {
            ...
            if (j == 0)
                QL_REQUIRE(volsteptimes_[0] > 0.0, "volsteptimes must be positive (" << volsteptimes_[0] << ")");
            else
                QL_REQUIRE(volsteptimes_[j] > volsteptimes_[j - 1], "volsteptimes must be strictly increasing ("
    ...
    void MarkovFunctional::updateTimes2() const
    {
        ...
        QL_REQUIRE(volatilities_.size() == volsteptimes_.size() + 1,
                   "there must be n+1 volatilities (" << volatilities_.size() << ") for n volatility step times ("
                                                      << volsteptimes_.size() << ")");
    }
    ...
    void MarkovFunctional::initialize()
    {
        ...
        {
            normalIntegralW_[i] *= std::exp(-normalIntegralX_[i] * normalIntegralX_[i]) * M_1_SQRTPI;
        ...
        do
        {
            ...
            for (auto i = calibrationPoints_.rbegin(); i != calibrationPoints_.rend() && done; ++i)
            {
                ...
                for (auto j = i->second.paymentDates_.rbegin(); j != i->second.paymentDates_.rend() && done; ++j)
                {
                    if (*j < numeraireKnown)
                    {
                        if (capletCalibrated_)
                        ...
                        {
                            Size months = std::max(1, static_cast<Integer>(((numeraireKnown - *j) / 365.25) * 12.0));
                            while (underlyingSwap(swapIndexBase_, *j, months * Months)->maturityDate() < numeraireKnown)
                                ++months;
        ...
        stateProcess_ = ext::make_shared<MfStateProcess>(reversion_(0.0), volsteptimesArray_, sigma_.params());

        y_ = yGrid(modelSettings_.yStdDevs_, modelSettings_.yGridPoints_);

        discreteNumeraire_ = ext::make_shared<Matrix>(times_.size(), 2 * modelSettings_.yGridPoints_ + 1, 1.0);
        for (Size i = 0; i < times_.size(); i++)
        {
            auto cubicInt = ext::make_shared<CubicInterpolation>(
                y_.begin(), y_.end(), discreteNumeraire_->row_begin(i), CubicInterpolation::Spline, true,
    ...
    void MarkovFunctional::makeSwaptionCalibrationPoint(const Date& expiry, const Period& tenor)
    {
        ...
        QL_REQUIRE(calibrationPoints_.count(expiry) == 0,
                   "swaption expiry (" << expiry << ") occurs more than once in calibration set");
        ...
        for (unsigned int k = 1; k < sched.size(); k++)
        {
            p.yearFractions_.push_back(
                swapIndexBase_->dayCounter().yearFraction(k == 1 ? expiry : sched.date(k - 1), sched.date(k)));
    ...
    void MarkovFunctional::makeCapletCalibrationPoint(const Date& expiry)
    {
        ...
        QL_REQUIRE(calibrationPoints_.count(expiry) == 0,
                   "caplet expiry (" << expiry << ") occurs more than once in calibration set");
        ...
        Date endDate = iborIndex_->fixingCalendar().advance(
            valueDate, iborIndex_->tenor(), iborIndex_->businessDayConvention(), iborIndex_->endOfMonth());
    ...
    void MarkovFunctional::updateSmiles() const
    {
        ...
        for (auto i = calibrationPoints_.rbegin(); i != calibrationPoints_.rend(); ++i)
        {
            ...
            {
                i->second.annuity_ =
                    i->second.yearFractions_[0] * termStructure()->discount(i->second.paymentDates_[0], true);
            ...
            else
            {
                ...
                {
                    annuity +=
                        i->second.yearFractions_[k] * termStructure()->discount(i->second.paymentDates_[k], true);
                }
                i->second.annuity_ = annuity;
                i->second.atm_ = (termStructure()->discount(i->first, true) -
                                  termStructure()->discount(i->second.paymentDates_.back(), true)) /
                                 annuity;
                smileSection = swaptionVol_->smileSection(i->first, i->second.tenor_, true);
            }

            i->second.rawSmileSection_ =
                ext::shared_ptr<SmileSection>(new AtmSmileSection(smileSection, i->second.atm_));
            ...
            if ((modelSettings_.adjustments_ & ModelSettings::KahaleSmile) != 0)
            {
                ...
                arbitrageIndices_.push_back(
                    ext::dynamic_pointer_cast<KahaleSmileSection>(i->second.smileSection_)->coreIndices());
            }
            else
            {
                ...
                if ((modelSettings_.adjustments_ & ModelSettings::SabrSmile) != 0)
                {
                    ...
                    SmileSectionUtils ssutils(*i->second.rawSmileSection_, modelSettings_.smileMoneynessCheckpoints_);
                    std::vector<Real> k = ssutils.strikeGrid();
                    k.erase(k.begin()); // the first strike is zero which we do
                                        // not want in the sabr calibration
                    QL_REQUIRE(i->second.rawSmileSection_->volatilityType() == ShiftedLognormal,
                               "MarkovFunctional: SABR calibration to normal "
                               "input volatilities is not supported");
                    QL_REQUIRE(k.size() >= 4,
                               "for sabr calibration at least 4 points are needed (is " << k.size() << ")");
                    std::vector<Real> v;
                    v.reserve(k.size());
                    for (Real j : k)
                    {
                    ...
                    // during calibration ?
                    ext::shared_ptr<SabrInterpolatedSmileSection> sabrSection(new SabrInterpolatedSmileSection(
                        i->first, i->second.atm_, k, false, i->second.rawSmileSection_->volatility(i->second.atm_), v,
                        0.03, 0.80, 0.50, 0.00, false, false, false, false, true, ext::shared_ptr<EndCriteria>(),
                        ext::shared_ptr<OptimizationMethod>(), Actual365Fixed(), i->second.rawSmileSection_->shift()));
                ...
                else if ((modelSettings_.adjustments_ & ModelSettings::CustomSmile) != 0)
                {
                    ...
                    i->second.smileSection_ =
                        modelSettings_.customSmileFactory_->smileSection(i->second.rawSmileSection_, i->second.atm_);
                    arbitrageIndices_.emplace_back(Null<Size>(), Null<Size>());
                }
                else
                { // no smile pretreatment
            ...
            {
                i->second.minRateDigital_ = i->second.smileSection_->digitalOptionPrice(
                    modelSettings_.lowerRateBound_ - i->second.smileSection_->shift(), Option::Call, i->second.annuity_,
                    modelSettings_.digitalGap_);
                i->second.maxRateDigital_ = i->second.smileSection_->digitalOptionPrice(
                    modelSettings_.upperRateBound_ - i->second.smileSection_->shift(), Option::Call, i->second.annuity_,
                    modelSettings_.digitalGap_);
            }
    ...
    void MarkovFunctional::updateNumeraireTabulation() const
    {
        ...
        for (auto i = calibrationPoints_.rbegin(); i != calibrationPoints_.rend(); ++i, --idx)
        {
            ...
            if ((modelSettings_.adjustments_ & ModelSettings::CustomSmile) != 0)
            {
                mfSec = ext::dynamic_pointer_cast<CustomSmileSection>(i->second.smileSection_);
                QL_REQUIRE(mfSec, "no CustomSmileSection given, this is unexpected...");
            }
            ...
            for (unsigned int k = 0; k < i->second.paymentDates_.size(); k++)
            {
                deflatedFinalPayments = deflatedZerobondArray(
                    termStructure()->timeFromReference(i->second.paymentDates_[k]), times_[idx], y_);
            ...
            for (int c = 0; c == 0 || (c == 1 && ((modelSettings_.adjustments_ & ModelSettings::AdjustDigitals) != 0));
                 c++)
            {
                ...
                for (int j = y_.size() - 1; j >= 0; j--)
                {
                        ...
                        if ((modelSettings_.adjustments_ & ModelSettings::NoPayoffExtrapolation) == 0)
                        {
                            if ((modelSettings_.adjustments_ & ModelSettings::ExtrapolatePayoffFlat) != 0)
                            {
                                integral = gaussianShiftedPolynomialIntegral(
                                    0.0, 0.0, 0.0, 0.0, discreteDeflatedAnnuities[j - 1], y_[j - 1], y_[j], 100.0);
                            }
                            else
                            {
                                ...
                                integral = gaussianShiftedPolynomialIntegral(
                                    0.0, cc, cb, ca, discreteDeflatedAnnuities[j - 1], y_[j - 1], y_[j], 100.0);
                            }
                        }
                    }
                    else
                    ...
                    if (integral < 0)
                    {
                        QL_MFMESSAGE(modelOutputs_, "WARNING: integral for digitalPrice is "
                                                    "negative for j="
                    ...
                    if ((modelSettings_.adjustments_ & ModelSettings::CustomSmile) != 0)
                    {
                        swapRate = mfSec->inverseDigitalCall(digital, i->second.annuity_);
                    }
                    else if (digital >= i->second.minRateDigital_)
                    {
                        swapRate = modelSettings_.lowerRateBound_ - i->second.rawSmileSection_->shift();
                        check = false;
                    }
                    ...
                    if (check && j < (int)y_.size() - 1 && swapRate > swapRate0)
                    {
                        QL_MFMESSAGE(modelOutputs_, "WARNING: swap rate is decreasing in y for "
                                                    "t=" << times_[idx]
                                                         << ", j=" << j << " (y, swap rate) is (" << y_[j] << ","
                                                         << swapRate << ") but for j=" << j + 1 << " it is ("
                                                         << y_[j + 1] << "," << swapRate0 << ") --- reset rate to "
                                                         << swapRate0 << " in node j=" << j);
                        swapRate = swapRate0;
                    }
                    swapRate0 = swapRate;
                    Real numeraire =
                        1.0 / std::max(swapRate * discreteDeflatedAnnuities[j] + deflatedFinalPayments[j], 1E-6);
            ...
            if ((modelSettings_.adjustments_ & ModelSettings::AdjustYts) != 0)
            {
                ...
                Real marketDeflatedZerobond =
                    termStructure()->discount(times_[idx], true) / termStructure()->discount(numeraireTime_, true);
                for (int j = y_.size() - 1; j >= 0; j--)
                {
        ...
        if (modelOutputs_.dirty_)
        {
            ...
            for (Size i = 1; i < times_.size() - 1; i++)
            {
                modelOutputs_.marketZerorate_.push_back(
                    termStructure()->zeroRate(times_[i], QuantLib::Continuous, QuantLib::Annual));
                // we need to put a small positive time here since the zerobond
                // implementation optimizes the case t=0.0 then using the
                // initial yts
                modelOutputs_.modelZerorate_.push_back(-std::log(zerobond(times_[i], 1.0E-10)) / times_[i]);
            }
            ...
            for (auto& calibrationPoint : calibrationPoints_)
            {
                ...
                std::vector<Real> strikes, marketCall, marketPut, modelCall, modelPut, marketVega, marketRawCall,
                    marketRawPut;
                for (Size j = 0; j < money.size(); j++)
                {
                    strikes.push_back(sec->volatilityType() == Normal ?
                                          Real(calibrationPoint.second.atm_ + money[j]) :
                                          money[j] * (calibrationPoint.second.atm_ + shift) - shift);
                    try
                    {
                        marketRawCall.push_back(
                            rawSec->optionPrice(strikes[j], Option::Call, calibrationPoint.second.annuity_));
                        marketRawPut.push_back(
                            rawSec->optionPrice(strikes[j], Option::Put, calibrationPoint.second.annuity_));
                    }
                    catch (Error&)
                    {
                    ...
                    marketCall.push_back(sec->optionPrice(strikes[j], Option::Call, calibrationPoint.second.annuity_));
                    marketPut.push_back(sec->optionPrice(strikes[j], Option::Put, calibrationPoint.second.annuity_));
                    modelCall.push_back(
                        calibrationPoint.second.isCaplet_ ?
                            capletPriceInternal(Option::Call, calibrationPoint.first, strikes[j], Date(), 0.0, true) :
                            swaptionPriceInternal(Option::Call, calibrationPoint.first, calibrationPoint.second.tenor_,
                                                  strikes[j], Date(), 0.0, true));
                    modelPut.push_back(
                        calibrationPoint.second.isCaplet_ ?
                            capletPriceInternal(Option::Put, calibrationPoint.first, strikes[j], Date(), 0.0, true) :
                            swaptionPriceInternal(Option::Put, calibrationPoint.first, calibrationPoint.second.tenor_,
                                                  strikes[j], Date(), 0.0, true));
    ...
    Array MarkovFunctional::numeraireArray(const Time t, const Array& y) const
    {
        ...
        Real inverseNormalization =
            termStructure()->discount(numeraireTime_, true) / termStructure()->discount(t, true);

        Time tz = std::min(t, times_.back());
        Size i =
            std::min<Size>(std::upper_bound(times_.begin(), times_.end() - 1, t) - times_.begin(), times_.size() - 1);
    ...
    Real MarkovFunctional::numeraireImpl(const Time t, const Real y, const Handle<YieldTermStructure>& yts) const
    {
        ...
        return numeraireArray(t, ya)[0] *
               (yts.empty() ? Real(1.0) :
                              (yts->discount(numeraireTime()) / yts->discount(t) * termStructure()->discount(t) /
                               termStructure()->discount(numeraireTime())));
    }

    Real MarkovFunctional::zerobondImpl(const Time T,
                                        const Time t,
                                        const Real y,
                                        const Handle<YieldTermStructure>& yts) const
    {
        ...
        Array ya(1, y);
        return zerobondArray(T, t, ya)[0] * (yts.empty() ?
                                                 Real(1.0) :
                                                 (yts->discount(T) / yts->discount(t) * termStructure()->discount(t) /
                                                  termStructure()->discount(T)));
    }

    Real MarkovFunctional::deflatedZerobond(Time T, Time t, Real y) const
    {
                                          ...
                                          const Real shift) const
    {
        ...
        Brent b;
        Real solution = b.solve(z, modelSettings_.marketRateAccuracy_,
    ...
    std::ostream& operator<<(std::ostream& out, const MarkovFunctional::ModelOutputs& m)
    {
        ...
        out << "Adjustments          : "
            << ((m.settings_.adjustments_ & MarkovFunctional::ModelSettings::AdjustDigitals) != 0 ? "Digitals " : "")
            << ((m.settings_.adjustments_ & MarkovFunctional::ModelSettings::AdjustYts) != 0 ? "Yts " : "")
            << ((m.settings_.adjustments_ & MarkovFunctional::ModelSettings::ExtrapolatePayoffFlat) != 0 ?
                    "FlatPayoffExt " :
                    "")
            << ((m.settings_.adjustments_ & MarkovFunctional::ModelSettings::NoPayoffExtrapolation) != 0 ?
                    "NoPayoffExt " :
                    "")
            << ((m.settings_.adjustments_ & MarkovFunctional::ModelSettings::KahaleSmile) != 0 ? "Kahale " : "")
            << ((m.settings_.adjustments_ & MarkovFunctional::ModelSettings::SmileExponentialExtrapolation) != 0 ?
                    "SmileExp " :
                    "")
            << ((m.settings_.adjustments_ & MarkovFunctional::ModelSettings::KahaleInterpolation) != 0 ? "KahaleInt " :
                                                                                                         "")
            << ((m.settings_.adjustments_ & MarkovFunctional::ModelSettings::SmileDeleteArbitragePoints) != 0 ?
                    "SmileDelArb " :
                    "")
            << ((m.settings_.adjustments_ & MarkovFunctional::ModelSettings::SabrSmile) != 0 ? "Sabr" : "")
            << std::endl;
        ...
        out << "expiry;tenor;atm;annuity;digitalAdj;ytsAdj;marketzerorate;"
               "modelzerorate;diff(bp)"
            << std::endl;
        for (Size i = 0; i < m.expiries_.size(); i++)
        {
            out << m.expiries_[i] << ";" << m.tenors_[i] << ";" << m.atm_[i] << ";" << m.annuity_[i] << ";"
                << m.digitalsAdjustmentFactors_[i] << ";" << m.adjustmentFactors_[i] << ";" << m.marketZerorate_[i]
                << ";" << m.modelZerorate_[i] << ";" << (m.marketZerorate_[i] - m.modelZerorate_[i]) * 10000.0
                << std::endl;
        }
        out << std::endl;
        ...
        for (Size i = 0; i < m.expiries_.size(); i++)
        {
            ...
            std::string p = os.str();
            out << "strike(" << p << ");marketCallRaw(" << p << ";marketCall(" << p << ");modelCall(" << p
                << ");marketPutRaw(" << p << ");marketPut(" << p << ");modelPut(" << p << ");marketVega(" << p << ")"
    ...
    Real MarkovFunctional::forwardRateInternal(const Date& fixing,
                                               ...
                                               ext::shared_ptr<IborIndex> iborIdx) const
    {
        ...
        Date endDate = iborIdx->fixingCalendar().advance(iborIdx->valueDate(fixing), iborIdx->tenor(),
                                                         ...
                                                         iborIdx->endOfMonth()); // FIXME Here we should use the
    ...
    Real MarkovFunctional::swapRateInternal(const Date& fixing,
                                            const Period& tenor,
                                            const Date& referenceDate,
                                            const Real y,
                                            bool zeroFixingDays,
                                            ext::shared_ptr<SwapIndex> swapIdx) const
    {
        ...
        Schedule sched = underlying->fixedSchedule();
        Real annuity = swapAnnuityInternal(fixing, tenor, referenceDate, y, zeroFixingDays, swapIdx);
        Rate atm = (zerobond(zeroFixingDays ? fixing : sched.dates().front(), referenceDate, y) -
                    zerobond(sched.calendar().adjust(sched.dates().back(), underlying->paymentConvention()),
                             referenceDate, y)) /
                   annuity;
        return atm;
    }

    Real MarkovFunctional::swapAnnuityInternal(const Date& fixing,
                                               const Period& tenor,
                                               const Date& referenceDate,
                                               const Real y,
                                               const bool zeroFixingDays,
                                               ext::shared_ptr<SwapIndex> swapIdx) const
    {
        ...
        {
            annuity +=
                zerobond(sched.calendar().adjust(sched.date(j), underlying->paymentConvention()), referenceDate, y) *
                swapIdx->dayCounter().yearFraction(j == 1 && zeroFixingDays ? fixing : sched.date(j - 1),
                                                   sched.date(j));
        }
        return annuity;
    }
                                                 ...
                                                 const ext::shared_ptr<SwapIndex>& swapIdx) const
    {
        ...
        Time referenceTime = referenceDate == Date() ? 0.0 : termStructure()->timeFromReference(referenceDate);
        ...
        {
            Real annuity = swapAnnuityInternal(expiry, tenor, expiry, yg[i], zeroFixingDays, swapIdx);
            Rate atm = swapRateInternal(expiry, tenor, expiry, yg[i], zeroFixingDays, swapIdx);
            ...
            else
            {
                if (type == Option::Call)
                    price += gaussianShiftedPolynomialIntegral(
                        ...
                        payoff.aCoefficients()[z.size() - 2], p[z.size() - 2], z[z.size() - 2], z[z.size() - 1], 100.0);
                if (type == Option::Put)
                    price +=
                        gaussianShiftedPolynomialIntegral(0.0, payoff.cCoefficients()[0], payoff.bCoefficients()[0],
    ...
    Real MarkovFunctional::capletPriceInternal(const Option::Type& type,
                                               const Date& expiry,
                                               const Rate strike,
                                               const Date& referenceDate,
                                               const Real y,
                                               ...
                                               ext::shared_ptr<IborIndex> iborIdx) const
    {
        ...
        Date endDate = iborIdx->fixingCalendar().advance(valueDate, iborIdx->tenor(), iborIdx->businessDayConvention(),
                                                         iborIdx->endOfMonth()); // FIXME Here we should use the
                                                                                 // calculation date calendar ?
        ...
        for (Size i = 0; i < yg.size(); i++)
        {
            ...
            Rate atm = forwardRateInternal(expiry, expiry, yg[i], zeroFixingDays, iborIdx);
            p[i] = annuity * std::max((type == Option::Call ? 1.0 : -1.0) * (atm - strike), 0.0) /
                   numeraire(fixingTime, yg[i]);
        }
