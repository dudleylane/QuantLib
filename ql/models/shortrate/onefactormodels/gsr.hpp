...
namespace QuantLib
{
      ...
      public:
        ...
        std::vector<bool> MoveVolatility(Size i)
        {
            QL_REQUIRE(i < volatilities_.size(),
                       "volatility with index " << i << " does not exist (0..." << volatilities_.size() - 1 << ")");
        ...
        std::vector<bool> MoveReversion(Size i)
        {
            QL_REQUIRE(i < reversions_.size(),
                       "reversion with index " << i << " does not exist (0..." << reversions_.size() - 1 << ")");
            std::vector<bool> res(reversions_.size() + volatilities_.size(), true);
            res[i] = false;
            return res;
        }
    ...
    {
        return ext::dynamic_pointer_cast<GsrProcess>(stateProcess_)->getForwardMeasureTime();
    }

    inline void Gsr::numeraireTime(const Real T)
    {
        ext::dynamic_pointer_cast<GsrProcess>(stateProcess_)->setForwardMeasureTime(T);
    }
}
