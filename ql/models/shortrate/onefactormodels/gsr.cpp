...
namespace QuantLib
{
    ...
    Gsr::Gsr(const Handle<YieldTermStructure>& termStructure,
             ...
             const Real T)
    : Gaussian1dModel(termStructure), CalibratedModel(2), reversion_(arguments_[0]), sigma_(arguments_[1]),
    ...
    Gsr::Gsr(const Handle<YieldTermStructure>& termStructure,
             ...
             const Real T)
    : Gaussian1dModel(termStructure), CalibratedModel(2), reversion_(arguments_[0]), sigma_(arguments_[1]),
      volatilities_(std::move(volatilities)), reversions_(std::vector<Handle<Quote>>(1, reversion)),
      volstepdates_(std::move(volstepdates))
    {
    ...
    Gsr::Gsr(const Handle<YieldTermStructure>& termStructure,
             ...
             const Real T)
    : Gaussian1dModel(termStructure), CalibratedModel(2), reversion_(arguments_[0]), sigma_(arguments_[1]),
      volatilities_(std::move(volatilities)), reversions_(std::move(reversions)), volstepdates_(std::move(volstepdates))
    {
    ...
    void Gsr::initialize(Real T)
    {
        ...
        stateProcess_ = ext::make_shared<GsrProcess>(volsteptimesArray_, sigma_.params(), reversion_.params(), T);
    ...
    Real Gsr::zerobondImpl(const Time T, const Time t, const Real y, const Handle<YieldTermStructure>& yts) const
    {
        ...
        Real d = yts.empty() ? termStructure()->discount(T, true) / termStructure()->discount(t, true) :
                               yts->discount(T, true) / yts->discount(t, true);

        return d * exp(-x * gtT - 0.5 * p->y(t) * gtT * gtT);
    }

    Real Gsr::numeraireImpl(const Time t, const Real y, const Handle<YieldTermStructure>& yts) const
    {
