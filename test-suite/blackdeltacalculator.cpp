...
BOOST_AUTO_TEST_CASE(testDeltaValues)
{
    ...
    for (Size i = 0; i < std::size(values); i++)
    {
        ...
        BlackDeltaCalculator myCalc(currOt, currDt, currSpot, currdDf, currfDf, currStdDev);
...
BOOST_AUTO_TEST_CASE(testDeltaPriceConsistency)
{
    ...
    ext::shared_ptr<YieldTermStructure> qTS(new FlatForward(today, qHandle, dc));

    ext::shared_ptr<SimpleQuote> rQuote(new SimpleQuote(0.0));
    Handle<Quote> rHandle(qQuote);
    ext::shared_ptr<YieldTermStructure> rTS(new FlatForward(today, rHandle, dc));

    ext::shared_ptr<SimpleQuote> volQuote(new SimpleQuote(0.0));
    Handle<Quote> volHandle(volQuote);
    ext::shared_ptr<BlackVolTermStructure> volTS(new BlackConstantVol(today, calendar, volHandle, dc));
    ...
    for (auto& value : values)
    {
        ...
        payoff = ext::shared_ptr<StrikedTypePayoff>(new PlainVanillaPayoff(value.type, value.strike));
        ...
        BlackDeltaCalculator myCalc(value.type, DeltaVolQuote::PaSpot, spotQuote->value(), discDom, discFor, implVol);

        stochProcess = ext::make_shared<BlackScholesMertonProcess>(spotHandle, Handle<YieldTermStructure>(qTS),
...
BOOST_AUTO_TEST_CASE(testPutCallParity)
{
    ...
    */
    ...
    for (auto& value : values)
    {
        ...
        payoff = ext::shared_ptr<StrikedTypePayoff>(new PlainVanillaPayoff(Option::Call, value.strike));
        exDate = today + timeToDays(value.t);
        exercise = ext::shared_ptr<Exercise>(new EuropeanExercise(exDate));
        ...
        BlackDeltaCalculator myCalc(Option::Call, DeltaVolQuote::Spot, spotQuote->value(), discDom, discFor, implVol);
...
BOOST_AUTO_TEST_CASE(testAtmCalcs)
{
    ...
    for (Size i = 0; i < std::size(values); i++)
    {
        ...
        BlackDeltaCalculator myCalc(Option::Call, currDt, currSpot, currdDf, currfDf, currStdDev);
