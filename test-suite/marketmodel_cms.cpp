...
void setup()
{
    ...
    Schedule dates(todaysDate, endDate, Period(Semiannual), calendar, Following, Following, DateGeneration::Backward,
                   false);
    ...
    {
        volatilities[i] = todaysCMSwapRates[i] * mktVols[i] / (todaysCMSwapRates[i] + displacement);
...
ext::shared_ptr<MarketModel> makeMarketModel(const EvolutionDescription& evolution,
                                             ...
                                             Volatility volBump = 0.0)
{
    ...
    LMMCurveState curveState_lmm(rateTimes);
    curveState_lmm.setOnForwardRates(todaysForwards);
    std::vector<Rate> usedRates = curveState_lmm.cmSwapRates(spanningForwards);
    std::transform(usedRates.begin(), usedRates.end(), bumpedRates.begin(), [=](Rate r) { return r + rateBump; });
    ...
    {
        case ExponentialCorrelationFlatVolatility:
            return ext::shared_ptr<MarketModel>(new FlatVol(bumpedVols, corr, evolution, numberOfFactors, bumpedRates,
...
std::vector<Size> makeMeasure(const MarketModelMultiProduct& product, MeasureType measureType)
{
    ...
    switch (measureType)
    {
        ...
        case MoneyMarketPlus:
            ...
            {
                BOOST_ERROR("failure in verifying MoneyMarketPlus(" << measureOffset_ << ") measure:\n"
                                                                    << to_stream(result));
            }
            break;
        default:
    ...
    switch (evolverType)
    {
        case Pc:
            return ext::shared_ptr<MarketModelEvolver>(
                new LogNormalCmSwapRatePc(spanningForwards, marketModel, generatorFactory, numeraires, initialStep));
        default:
...
void checkCMSAndSwaptions(const SequenceStatisticsInc& stats,
                          ...
                          const std::string& config)
{
    ...
    for (Size i = 0; i < N; ++i)
    {
        Real expectedNPV = curveState_lmm.cmSwapAnnuity(i, i, spanningForwards) * (todaysCMSwapRates[i] - fixedRate) *
                           todaysDiscounts[i];
    ...
    if (maxError > errorThreshold)
    {
        ...
        for (Size i = 0; i < N; ++i)
        {
            BOOST_TEST_MESSAGE(io::ordinal(i + 1)
                               << " CMS NPV: " << io::rate(results[i]) << " +- " << io::rate(errors[i])
                               << "; expected: " << io::rate(expectedNPVs[i])
                               << "; discrepancy/error = " << discrepancies[N - 1 - i] << " standard errors");
        }
    ...
    for (Size i = 0; i < N; ++i)
    {
        Real expectedSwaption =
            BlackCalculator(displacedPayoff[i], todaysCMSwapRates[i] + displacement,
                            volatilities[i] * std::sqrt(rateTimes[i]),
                            curveState_lmm.cmSwapAnnuity(i, i, spanningForwards) * todaysDiscounts[i])
                .value();
    ...
    if (maxError > errorThreshold)
    {
        ...
        for (Size i = 1; i <= N; ++i)
        {
            BOOST_TEST_MESSAGE(io::ordinal(i)
                               << " Swaption: " << io::rate(results[2 * N - i]) << " +- " << io::rate(errors[2 * N - i])
                               << "; expected: " << io::rate(expectedSwaptions[N - i])
                               << "; discrepancy/error = " << io::percent(discrepancies[N - i]) << " standard errors");
        }
        BOOST_ERROR("test failed");
    }
}
...
BOOST_AUTO_TEST_CASE(testMultiStepCmSwapsAndSwaptions, *precondition(if_speed(Slow)))
{
    ...
    // until ConstantMaturitySwap is ready
    MultiStepCoterminalSwaps swaps(rateTimes, accruals, accruals, swapPaymentTimes, fixedRate);
    // swaptions
    std::vector<Time> swaptionPaymentTimes(rateTimes.begin(), rateTimes.end() - 1);
    std::vector<ext::shared_ptr<StrikedTypePayoff>> displacedPayoff(todaysForwards.size()),
        undisplacedPayoff(todaysForwards.size());
    for (Size i = 0; i < undisplacedPayoff.size(); ++i)
    {
        displacedPayoff[i] =
            ext::shared_ptr<StrikedTypePayoff>(new PlainVanillaPayoff(Option::Call, fixedRate + displacement));

        undisplacedPayoff[i] = ext::shared_ptr<StrikedTypePayoff>(new PlainVanillaPayoff(Option::Call, fixedRate));
    }

    // until ConstantMaturitySwap is ready
    MultiStepCoterminalSwaptions swaptions(rateTimes, swaptionPaymentTimes, undisplacedPayoff);
            ...
            for (auto& measure : measures)
            {
                ...
                for (Size i = 0; i < std::size(evolvers) - stop; i++)
                {
                    for (Size n = 0; n < 1; n++)
                    {
                        ...
                        checkCMSAndSwaptions(*stats, fixedRate, displacedPayoff, marketModel, config.str());
                    }
                }
            }
        }
