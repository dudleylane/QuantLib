...
{
  public:
    explicit AmericanMaxPathPricer(ext::shared_ptr<Payoff> payoff) : payoff_(std::move(payoff)) {}
    ...
    {
        return LsmBasisSystem::multiPathBasisSystem(2, 2, LsmBasisSystem::Monomial);
    }
...
template <class RNG>
class MCAmericanMaxEngine : public MCLongstaffSchwartzEngine<VanillaOption::engine, MultiVariate, RNG>
{
  public:
    ...
    : MCLongstaffSchwartzEngine<VanillaOption::engine, MultiVariate, RNG>(processes,
                                                                          timeSteps,
                                                                          timeStepsPerYear,
                                                                          brownianbridge,
                                                                          antitheticVariate,
  ...
  protected:
    ext::shared_ptr<LongstaffSchwartzPathPricer<MultiPath>> lsmPathPricer() const override
    {
        ...
        return ext::make_shared<LongstaffSchwartzPathPricer<MultiPath>>(
            ...
            this->timeGrid(), earlyExercisePathPricer, process->riskFreeRate().currentLink());
    }
};
...
BOOST_AUTO_TEST_CASE(testAmericanOption, *precondition(if_speed(Fast)))
{
    ...
    LsmBasisSystem::PolynomialType polynomialTypes[] = {LsmBasisSystem::Monomial, LsmBasisSystem::Laguerre,
    ...
    for (Integer i = 0; i < 2; ++i)
    {
        for (Integer j = 0; j < 3; ++j)
        {
            Handle<BlackVolTermStructure> flatVolTS(ext::shared_ptr<BlackVolTermStructure>(
                new BlackConstantVol(settlementDate, NullCalendar(), volatility + 0.1 * j, dayCounter)));

            ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(type, underlying + 4 * i));

            Handle<Quote> underlyingH(ext::shared_ptr<Quote>(new SimpleQuote(underlying)));

            ext::shared_ptr<GeneralizedBlackScholesProcess> stochasticProcess(
                new GeneralizedBlackScholesProcess(underlyingH, flatDividendTS, flatTermStructure, flatVolTS));
            ...
            const Real exerciseProbability = americanOption.result<QuantLib::Real>("exerciseProbability");

            americanOption.setPricingEngine(
                ext::shared_ptr<PricingEngine>(new FdBlackScholesVanillaEngine(stochasticProcess, 401, 200)));
            ...
            {
                BOOST_ERROR("Failed to reproduce american option " << "exercise probability"
...
BOOST_AUTO_TEST_CASE(testAmericanMaxOption)
{
    ...
    Handle<BlackVolTermStructure> flatVolTS(ext::shared_ptr<BlackVolTermStructure>(
        new BlackConstantVol(settlementDate, NullCalendar(), volatility, dayCounter)));
    ...
    ext::shared_ptr<StochasticProcessArray> process(new StochasticProcessArray(v, corr));
    VanillaOption americanMaxOption(payoff, americanExercise);

    ext::shared_ptr<PricingEngine> mcengine(new MCAmericanMaxEngine<PseudoRandom>(
        process, 25, Null<Size>(), false, true, false, 4096, Null<Real>(), Null<Size>(), 42, 1024));
    ...
    for (Size i = 0; i < 3; ++i)
    {
        ...
        underlyingH.linkTo(ext::shared_ptr<Quote>(new SimpleQuote(underlying)));
