...
BOOST_AUTO_TEST_CASE(testDirectLookup)
{
    ...
    ExchangeRate eur_usd = rateManager.lookup(EUR, USD, Date(4, August, 2004), ExchangeRate::Direct);
    Money calculated = eur_usd.exchange(m1);
    ...
    eur_usd = rateManager.lookup(EUR, USD, Date(5, August, 2004), ExchangeRate::Direct);
    ...
    ExchangeRate usd_eur = rateManager.lookup(USD, EUR, Date(4, August, 2004), ExchangeRate::Direct);
    ...
    usd_eur = rateManager.lookup(USD, EUR, Date(5, August, 2004), ExchangeRate::Direct);
...
BOOST_AUTO_TEST_CASE(testTriangulatedLookup)
{
    ...
    ExchangeRate itl_usd = rateManager.lookup(ITL, USD, Date(4, August, 2004));
    ...
    itl_usd = rateManager.lookup(ITL, USD, Date(5, August, 2004));
    ...
    ExchangeRate usd_itl = rateManager.lookup(USD, ITL, Date(4, August, 2004));
    ...
    usd_itl = rateManager.lookup(USD, ITL, Date(5, August, 2004));
...
BOOST_AUTO_TEST_CASE(testSmartLookup)
{
    ...
    Currency EUR = EURCurrency(), USD = USDCurrency(), GBP = GBPCurrency(), CHF = CHFCurrency(), SEK = SEKCurrency(),
             JPY = JPYCurrency();
    ...
    ExchangeRate usd_sek = rateManager.lookup(USD, SEK, Date(4, August, 2004));
    ...
    ExchangeRate eur_sek = rateManager.lookup(EUR, SEK, Date(4, August, 2004));
    calculated = eur_sek.exchange(m2);
    expected = Money(m2.value() * eur_usd1.rate() * usd_chf1.rate() / chf_sek1.rate(), SEK);
    ...
    expected = Money(m5.value() * eur_usd2.rate() * usd_chf2.rate() / chf_sek2.rate(), EUR);
    ...
    ExchangeRate eur_jpy = rateManager.lookup(EUR, JPY, Date(4, August, 2004));
    ...
    expected = Money(m6.value() * jpy_sek2.rate() * eur_usd2.rate() * usd_chf2.rate() / chf_sek2.rate(), EUR);
    ...
    ExchangeRate gbp_jpy = rateManager.lookup(GBP, JPY, Date(4, August, 2004));
    calculated = gbp_jpy.exchange(m3);
    expected = Money(
        m3.value() * eur_gbp1.rate() * eur_usd1.rate() * usd_chf1.rate() * jpy_sek1.rate() / chf_sek1.rate(), JPY);
    ...
    expected = Money(
        m6.value() * jpy_sek2.rate() * eur_usd2.rate() * usd_chf2.rate() * eur_gbp2.rate() / chf_sek2.rate(), GBP);
