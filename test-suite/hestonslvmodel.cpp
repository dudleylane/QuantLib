                         ...
                         Size tGrid)
{
    ...
    QL_REQUIRE(x.size() > 3 && x[1] <= x0 && x[x.size() - 2] >= x0, "insufficient mesher");

    const Array::const_iterator upperb = std::upper_bound(x.begin(), x.end(), x0);
...
ext::shared_ptr<FdmMesher> createStationaryDistributionMesher(Real kappa, Real theta, Real sigma, Size vGrid)
{
    ...
    return ext::shared_ptr<FdmMesher>(ext::make_shared<FdmMesherComposite>(ext::make_shared<Predefined1dMesher>(v)));
}
...
class q_fct
{
  public:
    ...
    Real operator()(Real v) const { return (*spline_)(v, true) * std::pow(v, -alpha_); }
...
Real fokkerPlanckPrice2D(const Array& p, const ext::shared_ptr<FdmMesherComposite>& mesher)
{
...
Real hestonPxBoundary(Time maturity, Real eps, const ext::shared_ptr<HestonModel>& model)
{

    const AnalyticPDFHestonEngine pdfEngine(model);
    const Real sInit = model->process()->s0()->value();
    const Real xMin = Brent().solve([&](Real x) -> Real { return pdfEngine.cdf(x, maturity) - eps; }, sInit * 1e-3,
                                    sInit, sInit * 0.001, 1000 * sInit);
...
void hestonFokkerPlanckFwdEquationTest(const FokkerPlanckFwdTestCase& testCase)
{
    ...
    std::vector<Period> maturities = {Period(1, Months), Period(3, Months), Period(6, Months), Period(9, Months),
    ...
    const ext::shared_ptr<PricingEngine> engine(ext::make_shared<AnalyticHestonEngine>(model));
    ...
    const ext::shared_ptr<Fdm1dMesher> varianceMesher(
        ext::make_shared<Concentrating1dMesher>(lowerBound, upperBound, vGrid, cPoints, 1e-12));

    const Real sEps = 1e-4;
    const Real sLowerBound = std::log(hestonPxBoundary(maturity, sEps, model));
    const Real sUpperBound = std::log(hestonPxBoundary(maturity, 1 - sEps, model));

    const ext::shared_ptr<Fdm1dMesher> spotMesher(
        ext::make_shared<Concentrating1dMesher>(sLowerBound, sUpperBound, xGrid, std::make_pair(x0, 0.1), true));

    const ext::shared_ptr<FdmMesherComposite> mesher(ext::make_shared<FdmMesherComposite>(spotMesher, varianceMesher));

    const ext::shared_ptr<FdmLinearOpComposite> hestonFwdOp(
        ext::make_shared<FdmHestonFwdOp>(mesher, process, transformationType));

    ModifiedCraigSneydScheme evolver(FdmSchemeDesc::ModifiedCraigSneyd().theta, FdmSchemeDesc::ModifiedCraigSneyd().mu,
                                     hestonFwdOp);

    // step one days using non-correlated process
    const Time eT = 1.0 / 365;
    Array p = FdmHestonGreensFct(mesher, process, testCase.trafoType).get(eT, testCase.greensAlgorithm);
    ...
    for (auto maturitie : maturities)
    {
        ...
        const Time nextMaturityTime = dc.yearFraction(todaysDate, nextMaturityDate);
        ...
        for (Real strike : strikes)
        {
            ...
            const Real calculated = fokkerPlanckPrice2D(pd, mesher) * rTS->discount(nextMaturityDate);

            const ext::shared_ptr<Exercise> exercise(ext::make_shared<EuropeanExercise>(nextMaturityDate));
            ...
            if (diff > testCase.eps)
            {
                BOOST_FAIL("failed to reproduce Heston SLV prices at"
                           << "\n   strike      " << strike << "\n   kappa       " << kappa << "\n   theta       "
                           << theta << "\n   rho         " << rho << "\n   sigma       " << sigma << "\n   v0          "
                           << v0 << "\n   transform   " << transformationType << std::fixed << std::setprecision(5)
        ...
        if (avg > testCase.avgEps)
        {
            BOOST_FAIL("failed to reproduce Heston SLV prices"
                       ...
                       << "\n   kappa       " << kappa << "\n   theta       " << theta << "\n   rho         " << rho
                       << "\n   sigma       " << sigma << "\n   v0          " << v0 << "\n   transform   "
                       << transformationType << std::fixed << std::setprecision(5) << "\n   average diff: " << avg
                       << "\n   tolerance:  " << testCase.avgEps);
        }
    }
}

ext::shared_ptr<Matrix> createLocalVolMatrixFromProcess(const ext::shared_ptr<BlackScholesMertonProcess>& lvProcess,
                                                        const std::vector<Real>& strikes,
                                                        const std::vector<Date>& dates,
                                                        std::vector<Time>& times)
{

    const ext::shared_ptr<LocalVolTermStructure> localVol = lvProcess->localVolatility().currentLink();
    ...
    ext::shared_ptr<Matrix> surface(ext::make_shared<Matrix>(strikes.size(), dates.size()));
...
std::tuple<std::vector<Real>, std::vector<Date>, ext::shared_ptr<BlackVarianceSurface>>
createSmoothImpliedVol(const DayCounter& dc, const Calendar& cal)
{
    ...
    const std::vector<Real> surfaceStrikes = {2.222222222, 11.11111111, 44.44444444, 75.55555556, 80,
    ...
    Volatility v[] = {
        1.015873, 1.015873, 0.915873, 0.89729,  0.796493, 0.730914, 0.631335, 0.568895, 0.851309, 0.821309, 0.781309,
        0.641309, 0.635593, 0.583653, 0.508045, 0.463182, 0.686034, 0.630534, 0.590534, 0.500534, 0.448706, 0.416661,
        0.375470, 0.353442, 0.526034, 0.482263, 0.447713, 0.387703, 0.355064, 0.337438, 0.316966, 0.306859, 0.497587,
    ...
    const ext::shared_ptr<BlackVarianceSurface> volTS(ext::make_shared<BlackVarianceSurface>(
        todaysDate, cal, dates, surfaceStrikes, blackVolMatrix, dc, BlackVarianceSurface::ConstantExtrapolation,
...
ext::shared_ptr<LocalVolTermStructure> getFixedLocalVolFromHeston(const ext::shared_ptr<HestonModel>& hestonModel,
                                                                  const ext::shared_ptr<TimeGrid>& timeGrid)
{

    const Handle<BlackVolTermStructure> trueImpliedVolSurf(ext::make_shared<HestonBlackVolSurface>(
        Handle<HestonModel>(hestonModel), AnalyticHestonEngine::AndersenPiterbarg,
        AnalyticHestonEngine::Integration::gaussLaguerre(32)));

    const ext::shared_ptr<HestonProcess> hestonProcess = hestonModel->process();

    const ext::shared_ptr<LocalVolTermStructure> localVol(ext::make_shared<NoExceptLocalVolSurface>(
        trueImpliedVolSurf, hestonProcess->riskFreeRate(), hestonProcess->dividendYield(), hestonProcess->s0(),
        std::sqrt(hestonProcess->theta())));


    const ext::shared_ptr<LocalVolRNDCalculator> localVolRND(ext::make_shared<LocalVolRNDCalculator>(
        hestonProcess->s0().currentLink(), hestonProcess->riskFreeRate().currentLink(),
        hestonProcess->dividendYield().currentLink(), localVol, timeGrid));

    std::vector<ext::shared_ptr<std::vector<Real>>> strikes;
    for (Size i = 1; i < timeGrid->size(); ++i)
    {
        const Time t = timeGrid->at(i);
        const ext::shared_ptr<Fdm1dMesher> fdm1dMesher = localVolRND->mesher(t);

        const std::vector<Real>& logStrikes = fdm1dMesher->locations();
        const ext::shared_ptr<std::vector<Real>> strikeSlice(ext::make_shared<std::vector<Real>>(logStrikes.size()));
    ...
    const ext::shared_ptr<Matrix> localVolMatrix(ext::make_shared<Matrix>(nStrikes, timeGrid->size() - 1));
    for (Size i = 1; i < timeGrid->size(); ++i)
    {
        const Time t = timeGrid->at(i);
    ...
    const Date todaysDate = hestonProcess->riskFreeRate()->referenceDate();
    const DayCounter dc = hestonProcess->riskFreeRate()->dayCounter();

    const std::vector<Time> expiries(timeGrid->begin() + 1, timeGrid->end());
...
BOOST_AUTO_TEST_CASE(testBlackScholesFokkerPlanckFwdEquation)
{
    ...
    const ext::shared_ptr<PricingEngine> engine(ext::make_shared<AnalyticEuropeanEngine>(process));

    const ext::shared_ptr<FdmMesher> uniformMesher(
        ext::make_shared<FdmMesherComposite>(ext::make_shared<FdmBlackScholesMesher>(xGrid, process, maturity, s0)));
    ...
    const ext::shared_ptr<FdmMesher> concentratedMesher(
        ext::make_shared<FdmMesherComposite>(ext::make_shared<FdmBlackScholesMesher>(
            xGrid, process, maturity, s0, Null<Real>(), Null<Real>(), 0.0001, 1.5, std::pair<Real, Real>(s0, 0.1))));
    ...
    const ext::shared_ptr<FdmMesher> shiftedMesher(ext::make_shared<FdmMesherComposite>(
        ext::make_shared<FdmBlackScholesMesher>(xGrid, process, maturity, s0, Null<Real>(), Null<Real>(), 0.0001, 1.5,
    ...
    for (Real strike : strikes)
    {
        ...
        const Real calcUniform = fokkerPlanckPrice1D(uniformMesher, uniformBSFwdOp, payoff, x0, maturity, tGrid);
        const Real calcConcentrated =
            fokkerPlanckPrice1D(concentratedMesher, concentratedBSFwdOp, payoff, x0, maturity, tGrid);
        const Real calcShifted = fokkerPlanckPrice1D(shiftedMesher, shiftedBSFwdOp, payoff, x0, maturity, tGrid);
        const Real tol = 0.02;
...
BOOST_AUTO_TEST_CASE(testSquareRootZeroFlowBC)
{
    ...
    const Real expected[5][5] = {{0.000548, -0.000245, -0.005657, -0.001167, -0.000024},
    ...
    for (Size i = 0; i < 5; ++i)
    {
        ...
        const SquareRootProcessRNDCalculator rndCalculator(v_0, kappa, theta, sigma);
        ...
        // test derivatives
        const Real flowSym2Order =
            sigma * sigma * v0 / (4 * h) * (p1 - pm1) + (kappa * (v0 - theta) + sigma * sigma / 2) * p0;

        const Real flowSym4Order = sigma * sigma * v0 / (24 * h) * (-p2 + 8 * p1 - 8 * pm1 + pm2) +
                                   (kappa * (v0 - theta) + sigma * sigma / 2) * p0;

        const Real fwd1Order =
            sigma * sigma * v0 / (2 * h) * (p1 - p0) + (kappa * (v0 - theta) + sigma * sigma / 2) * p0;

        const Real fwd2Order =
            sigma * sigma * v0 / (4 * h) * (4 * p1 - 3 * p0 - p2) + (kappa * (v0 - theta) + sigma * sigma / 2) * p0;

        const Real fwd3Order = sigma * sigma * v0 / (12 * h) * (-p2 + 6 * p1 - 3 * p0 - 2 * pm1) +
                               (kappa * (v0 - theta) + sigma * sigma / 2) * p0;

        const Real tol = 0.000002;
        if (std::fabs(expected[i][0] - flowSym2Order) > tol || std::fabs(expected[i][1] - flowSym4Order) > tol ||
            std::fabs(expected[i][2] - fwd1Order) > tol || std::fabs(expected[i][3] - fwd2Order) > tol ||
...
BOOST_AUTO_TEST_CASE(testTransformedZeroFlowBC)
{
    ...
    const ext::shared_ptr<FdmMesher> mesher = createStationaryDistributionMesher(kappa, theta, sigma, vGrid);
...
BOOST_AUTO_TEST_CASE(testSquareRootEvolveWithStationaryDensity)
{
    ...
    for (Real sigma = 0.2; sigma < 2.01; sigma += 0.1)
    {
        ...
        const ext::shared_ptr<FdmMesher> mesher(
            ext::make_shared<FdmMesherComposite>(ext::make_shared<Uniform1dMesher>(vMin, vMax, vGrid)));

        const Array v = mesher->locations(0);
        const FdmSquareRootFwdOp::TransformationType transform =
            (sigma < 0.75) ? FdmSquareRootFwdOp::Plain : FdmSquareRootFwdOp::Power;
        ...
        const ext::shared_ptr<FdmSquareRootFwdOp> op(
            ext::make_shared<FdmSquareRootFwdOp>(mesher, kappa, theta, sigma, 0, transform));
        ...
        const Real calculated = GaussLobattoIntegral(1000000, 1e-6)(f, v.front(), v.back());
...
BOOST_AUTO_TEST_CASE(testSquareRootLogEvolveWithStationaryDensity)
{
    ...
    for (Real sigma = 0.2; sigma < 2.01; sigma += 0.1)
    {
        ...
        const ext::shared_ptr<FdmSquareRootFwdOp> op(
            ext::make_shared<FdmSquareRootFwdOp>(mesher, kappa, theta, sigma, 0, FdmSquareRootFwdOp::Log));
        ...
        const Real calculated = FdmMesherIntegral(mesher, DiscreteSimpsonIntegral()).integrate(p);
...
BOOST_AUTO_TEST_CASE(testSquareRootFokkerPlanckFwdEquation)
{
    ...
    const ext::shared_ptr<FdmMesher> mesher(
        ext::make_shared<FdmMesherComposite>(ext::make_shared<Uniform1dMesher>(lowerBound, upperBound, xGrid)));
    ...
    for (Size i = 0; i < x.size(); ++i)
    {
        ...
        {
            BOOST_FAIL("failed to reproduce pdf at" << std::fixed << std::setprecision(5) << "\n   x:          " << x[i]
                                                    << "\n   calculated: " << calculated
                                                    << "\n   expected:   " << expected << "\n   tolerance:  " << tol);
        }
    }
...
BOOST_AUTO_TEST_CASE(testHestonFokkerPlanckFwdEquation, *precondition(if_speed(Slow)))
{
    ...
    FokkerPlanckFwdTestCase testCases[] = {
        {100.0, 0.01, 0.02,
         ...
         0.05, 1.0, 0.05, -0.75, std::sqrt(0.2), 101, 401, 25, 25, 0.02, 0.05, FdmSquareRootFwdOp::Power,
         FdmHestonGreensFct::Gaussian, FdmSchemeDesc::DouglasType},
        {100.0, 0.01, 0.02,
         // Feller constraint violated, high vol-of-vol case
         // \frac{2\kappa\theta}{\sigma^2} = 2.0 * 1.0 * 0.05 / 0.2 = 0.5 < 1
         0.05, 1.0, 0.05, -0.75, std::sqrt(0.2), 201, 501, 10, 10, 0.005, 0.02, FdmSquareRootFwdOp::Log,
         FdmHestonGreensFct::Gaussian, FdmSchemeDesc::HundsdorferType},
        {100.0, 0.01, 0.02,
         // Feller constraint violated, high vol-of-vol case
         // \frac{2\kappa\theta}{\sigma^2} = 2.0 * 1.0 * 0.05 / 0.2 = 0.5 < 1
         0.05, 1.0, 0.05, -0.75, std::sqrt(0.2), 201, 501, 25, 25, 0.01, 0.03, FdmSquareRootFwdOp::Log,
         FdmHestonGreensFct::ZeroCorrelation, FdmSchemeDesc::HundsdorferType},
        {100.0, 0.01, 0.02,
         // Feller constraint fulfilled, low vol-of-vol case
         // \frac{2\kappa\theta}{\sigma^2} = 2.0 * 1.0 * 0.05 / 0.05 = 2.0 > 1
         0.05, 1.0, 0.05, -0.75, std::sqrt(0.05), 201, 401, 5, 5, 0.01, 0.02, FdmSquareRootFwdOp::Plain,
         FdmHestonGreensFct::Gaussian, FdmSchemeDesc::HundsdorferType}};
...
BOOST_AUTO_TEST_CASE(testHestonFokkerPlanckFwdEquationLogLVLeverage, *precondition(if_speed(Fast)))
{
    ...
    const FdmSquareRootFwdOp::TransformationType transform = FdmSquareRootFwdOp::Plain;
    ...
    std::vector<std::tuple<Real, Real, bool>> critPoints = {std::make_tuple(lowerBound, beta, true),
    ...
    const ext::shared_ptr<Fdm1dMesher> equityMesher(ext::make_shared<Concentrating1dMesher>(
        std::log(2.0), std::log(600.0), xGrid, std::make_pair(x0 + 0.005, 0.1), true));

    const ext::shared_ptr<FdmMesherComposite> mesher(
        ext::make_shared<FdmMesherComposite>(equityMesher, varianceMesher));

    const std::tuple<std::vector<Real>, std::vector<Date>, ext::shared_ptr<BlackVarianceSurface>> smoothSurface =
        createSmoothImpliedVol(dayCounter, calendar);
    const ext::shared_ptr<BlackScholesMertonProcess> lvProcess(ext::make_shared<BlackScholesMertonProcess>(
        spot, qTS, rTS, Handle<BlackVolTermStructure>(std::get<2>(smoothSurface))));
    ...
    for (const auto& iter : *mesher->layout())
    {
        ...
        const Real p_x = 1.0 / (std::sqrt(M_TWOPI * bsV0 * eT)) * std::exp(-0.5 * squared(x - x0) / (bsV0 * eT));
    ...
    const std::vector<Real> denseStrikes = {
        2.222222222, 11.11111111, 20,          25,          30,          35,          40,          44.44444444,
        50,          55,          60,          65,          70,          75.55555556, 80,          84.44444444,
        88.88888889, 93.33333333, 97.77777778, 100,         102.2222222, 106.6666667, 111.1111111, 115.5555556,
        120,         124.4444444, 166.6666667, 222.2222222, 444.4444444, 666.6666667};
    ...
    ext::shared_ptr<Matrix> m = createLocalVolMatrixFromProcess(lvProcess, denseStrikes, dates, times);
    ...
    const ext::shared_ptr<PricingEngine> lvEngine(ext::make_shared<AnalyticEuropeanEngine>(lvProcess));

    const ext::shared_ptr<FdmLinearOpComposite> hestonFwdOp(
        ext::make_shared<FdmHestonFwdOp>(mesher, hestonProcess, transform, leverage));

    HundsdorferScheme evolver(FdmSchemeDesc::Hundsdorfer().theta, FdmSchemeDesc::Hundsdorfer().mu, hestonFwdOp);
    ...
    const ext::shared_ptr<PricingEngine> fdmEngine(
        ext::make_shared<FdBlackScholesVanillaEngine>(lvProcess, 50, 201, 0, FdmSchemeDesc::Douglas(), true, 0.2));
    ...
    for (Size strike = 5; strike < 200; strike += 10)
    {
        ...
        const Real calculated = fokkerPlanckPrice2D(pd, mesher) * rTS->discount(maturityDate);
        ...
        if (std::fabs(expected - calculated) > tol)
        {
            BOOST_FAIL("failed to reproduce Heston prices at"
                       << "\n   strike      " << strike << std::fixed << std::setprecision(5) << "\n   calculated: "
                       << calculated << "\n   expected:   " << expected << "\n   tolerance:  " << tol);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testBlackScholesFokkerPlanckFwdEquationLocalVol, *precondition(if_speed(Fast)))
{
    ...
    std::tuple<std::vector<Real>, std::vector<Date>, ext::shared_ptr<BlackVarianceSurface>> smoothImpliedVol =
        createSmoothImpliedVol(dayCounter, calendar);

    const std::vector<Real>& strikes = std::get<0>(smoothImpliedVol);
    const std::vector<Date>& dates = std::get<1>(smoothImpliedVol);
    const Handle<BlackVolTermStructure> vTS = Handle<BlackVolTermStructure>(std::get<2>(smoothImpliedVol));
    ...
    for (Size i = 1; i < dates.size(); i += 2)
    {
        for (Size j = 3; j < strikes.size() - 3; j += 2)
        {
            ...
            const ext::shared_ptr<FdmMesher> uniformMesher(ext::make_shared<FdmMesherComposite>(
                ext::make_shared<FdmBlackScholesMesher>(xGrid, process, maturity, s0)));

            //-- operator --
            const ext::shared_ptr<FdmLinearOpComposite> uniformBSFwdOp(
                ext::make_shared<FdmLocalVolFwdOp>(uniformMesher, *spot, *rTS, *qTS, localVol));

            const ext::shared_ptr<FdmMesher> concentratedMesher(ext::make_shared<FdmMesherComposite>(
                ext::make_shared<FdmBlackScholesMesher>(xGrid, process, maturity, s0, Null<Real>(), Null<Real>(),
                                                        0.0001, 1.5, std::pair<Real, Real>(s0, 0.1))));

            //-- operator --
            const ext::shared_ptr<FdmLinearOpComposite> concentratedBSFwdOp(
                ext::make_shared<FdmLocalVolFwdOp>(concentratedMesher, *spot, *rTS, *qTS, localVol));

            const ext::shared_ptr<FdmMesher> shiftedMesher(ext::make_shared<FdmMesherComposite>(
                ext::make_shared<FdmBlackScholesMesher>(xGrid, process, maturity, s0, Null<Real>(), Null<Real>(),
                                                        0.0001, 1.5, std::pair<Real, Real>(s0 * 1.1, 0.2))));

            //-- operator --
            const ext::shared_ptr<FdmLinearOpComposite> shiftedBSFwdOp(
                ext::make_shared<FdmLocalVolFwdOp>(shiftedMesher, *spot, *rTS, *qTS, localVol));
            ...
            if (std::fabs(expected - calcUniform) > tol)
            {
                BOOST_FAIL("failed to reproduce european option price "
                           << "with an uniform mesher"
                           << "\n   strike:     " << strikes[i] << std::fixed << std::setprecision(8)
...
BOOST_AUTO_TEST_CASE(testLocalVolsvSLVPropDensity)
{
    ...
    const Handle<BlackVolTermStructure> vTS =
        Handle<BlackVolTermStructure>(std::get<2>(createSmoothImpliedVol(dayCounter, calendar)));
    ...
    const Handle<HestonModel> hestonModel(ext::make_shared<HestonModel>(hestonProcess));

    const Handle<LocalVolTermStructure> localVol(ext::make_shared<NoExceptLocalVolSurface>(vTS, rTS, qTS, spot, 0.3));
    localVol->enableExtrapolation(true);

    const Size vGrid = 151;
    const Size xGrid = 51;

    const HestonSLVFokkerPlanckFdmParams fdmParams = {xGrid,
                                                      vGrid,
                                                      500,
                                                      50,
                                                      100.0,
    ...
    const HestonSLVFDMModel slvModel(localVol, hestonModel, finalDate, fdmParams, true);

    const std::list<HestonSLVFDMModel::LogEntry>& logEntries = slvModel.logEntries();

    const SquareRootProcessRNDCalculator squareRootRndCalculator(v0, kappa, theta, sigma);
    ...
    for (const auto& logEntrie : logEntries)
    {
        ...
        if (t > 0.2)
        {
            ...
            {
                const Real pCalc =
                    DiscreteSimpsonIntegral()(x, Array(prob->begin() + i * xGrid, prob->begin() + (i + 1) * xGrid));

                const Real expected = squareRootRndCalculator.pdf(std::exp(z[i]), t);
                const Real calculated = pCalc / std::exp(z[i]);

                if (std::fabs(expected - calculated) > 0.01 && std::fabs((expected - calculated) / expected) > 0.04)
                {
...
BOOST_AUTO_TEST_CASE(testBarrierPricingViaHestonLocalVol)
{
    ...
    const Handle<BlackVolTermStructure> surf(ext::make_shared<HestonBlackVolSurface>(hestonModel));

    const Real strikeValues[] = {50, 75, 100, 125, 150, 200, 400};
    const Period maturities[] = {Period(1, Months),  Period(2, Months), Period(3, Months), Period(4, Months),
                                 Period(5, Months),  Period(6, Months), Period(9, Months), Period(1, Years),
                                 Period(18, Months), Period(2, Years),  Period(3, Years),  Period(5, Years)};

    const ext::shared_ptr<LocalVolSurface> localVolSurface(ext::make_shared<LocalVolSurface>(surf, rTS, qTS, spot));
        ...
        for (auto maturitie : maturities)
        {
            ...
            const ext::shared_ptr<GeneralizedBlackScholesProcess> bsProcess(
                ext::make_shared<GeneralizedBlackScholesProcess>(
                    spot, qTS, rTS, Handle<BlackVolTermStructure>(flatVol(impliedVol, dc))));

            const ext::shared_ptr<PricingEngine> analyticEngine(ext::make_shared<AnalyticEuropeanEngine>(bsProcess));

            const ext::shared_ptr<Exercise> exercise(ext::make_shared<EuropeanExercise>(exerciseDate));
            const ext::shared_ptr<StrikedTypePayoff> payoff(
                ext::make_shared<PlainVanillaPayoff>(spot->value() < strike ? Option::Call : Option::Put, strike));

            const ext::shared_ptr<PricingEngine> localVolEngine(
                ext::make_shared<FdBlackScholesVanillaEngine>(bsProcess, 201, 801, 0, FdmSchemeDesc::Douglas(), true));
...
{
    BOOST_TEST_MESSAGE("Testing Monte-Carlo vs FDM Pricing for "
                       "Heston SLV models...");
    ...
    const ext::shared_ptr<LocalVolTermStructure> leverageFct = ext::make_shared<LocalConstantVol>(todaysDate, 0.25, dc);

    const ext::shared_ptr<HestonSLVProcess> slvProcess = ext::make_shared<HestonSLVProcess>(hestonProcess, leverageFct);

    const ext::shared_ptr<PricingEngine> mcEngine =
        MakeMCEuropeanHestonEngine<PseudoRandom, GeneralStatistics, HestonSLVProcess>(slvProcess)
            .withStepsPerYear(100)
            .withAntitheticVariate()
            .withSamples(10000)
            .withSeed(1234);

    const ext::shared_ptr<PricingEngine> fdEngine = ext::make_shared<FdHestonVanillaEngine>(
        hestonModel, 51, 401, 101, 0, FdmSchemeDesc::ModifiedCraigSneyd(), leverageFct);

    const ext::shared_ptr<Exercise> exercise = ext::make_shared<EuropeanExercise>(exerciseDate);

    const ext::shared_ptr<HestonProcess> mixingProcess = ext::make_shared<HestonProcess>(
        rTS, qTS, spot, v0, kappa, theta, sigma * 10, rho, HestonProcess::QuadraticExponentialMartingale);
    const ext::shared_ptr<HestonModel> mixingModel = ext::make_shared<HestonModel>(mixingProcess);

    const ext::shared_ptr<PricingEngine> fdEngineWithMixingFactor = ext::make_shared<FdHestonVanillaEngine>(
        mixingModel, 51, 401, 101, 0, FdmSchemeDesc::ModifiedCraigSneyd(), leverageFct, 0.1);
...
{
    BOOST_TEST_MESSAGE("Testing Monte-Carlo Calibration...");
    ...
    const ext::shared_ptr<LocalVolTermStructure> localVol = ext::make_shared<LocalConstantVol>(todaysDate, 0.3, dc);
    ...
    for (unsigned long nSim : nSims)
    {
        ...
        const ext::shared_ptr<PricingEngine> bsEngine(
            ext::make_shared<AnalyticEuropeanEngine>(ext::make_shared<GeneralizedBlackScholesProcess>(
                spot, qTS, rTS, Handle<BlackVolTermStructure>(flatVol(0.3, dc)))));

        const Real strikes[] = {50, 80, 100, 120, 150, 200};
        const Date maturities[] = {todaysDate + Period(3, Months), todaysDate + Period(6, Months),
        ...
        for (auto maturity : maturities)
        {
            ...
            const ext::shared_ptr<PricingEngine> fdEngine =
                ext::make_shared<FdHestonVanillaEngine>(hestonModel, std::max(Size(26), Size(maturityTime * 51)), 201,
                                                        51, 0, FdmSchemeDesc::ModifiedCraigSneyd(), leverageFct);
            ...
            {
                const ext::shared_ptr<StrikedTypePayoff> payoff =
                    ext::make_shared<PlainVanillaPayoff>(strike < s0 ? Option::Put : Option::Call, strike);
        ...
        {
            BOOST_ERROR("Failed to reproduce average calibration quality"
                        << "\n average calibration quality : " << qualityFactor / nValues << "bp"
                        << "\n tolerance                  :  5.0bp");
        }

        if (qualityFactor / nValues > 15.0)
        {
            BOOST_ERROR("Failed to reproduce maximum calibration error"
...
{
    BOOST_TEST_MESSAGE("Testing double no touch pricing with SLV and mixing...");
    ...
    */
    ...
    const ext::shared_ptr<PricingEngine> analyticEngine(
        ext::make_shared<AnalyticDoubleBarrierBinaryEngine>(ext::make_shared<GeneralizedBlackScholesProcess>(
            spot, qTS, rTS, Handle<BlackVolTermStructure>(flatVol(implVol, dc)))));

    std::vector<Time> expiries;
    const Period timeStepPeriod = Period(1, Weeks);
    for (Date expiry = todaysDate + timeStepPeriod; expiry <= maturityDate; expiry += timeStepPeriod)
    {
        expiries.push_back(dc.yearFraction(todaysDate, expiry));
    }

    const ext::shared_ptr<TimeGrid> timeGrid(ext::make_shared<TimeGrid>(expiries.begin(), expiries.end()));

    // first build the true local vol surface from another Heston model
    const Handle<LocalVolTermStructure> localVol(getFixedLocalVolFromHeston(hestonModel, timeGrid));

    const ext::shared_ptr<BrownianGeneratorFactory> sobolGeneratorFactory(
        ext::make_shared<SobolBrownianGeneratorFactory>(SobolBrownianGenerator::Diagonal, 1234UL, SobolRsg::JoeKuoD7));
    ...
    const Handle<HestonModel> modHestonModel(ext::make_shared<HestonModel>(
        ext::make_shared<HestonProcess>(rTS, qTS, spot, v0, kappa, theta, eta * sigma, rho)));

    const ext::shared_ptr<LocalVolTermStructure> leverageFct =
        HestonSLVMCModel(localVol, modHestonModel, sobolGeneratorFactory, maturityDate, 182, xGrid, nSim)
            .leverageFunction();

    const ext::shared_ptr<PricingEngine> fdEngine(ext::make_shared<FdHestonDoubleBarrierEngine>(
        modHestonModel.currentLink(), 51, 101, 31, 0, FdmSchemeDesc::Hundsdorfer(), leverageFct));

    const Real expected[] = {0.0334,  0.1141,  0.1319,  0.0957,  0.0464,  0.0058,  -0.0192, -0.0293, -0.0297,
                             -0.0251, -0.0192, -0.0134, -0.0084, -0.0045, -0.0015, 0.0005,  0.0017,  0.0020};
    const Real tol = 1e-2;
    ...
    for (Size i = 0; i < 18; ++i)
    {
        ...
        DoubleBarrierOption doubleBarrier(DoubleBarrier::KnockOut, barrier_lo, barrier_hi, 0.0,
        ...
        {
            BOOST_ERROR("Failed to reproduce price difference for a Double-No-Touch "
...
{
    BOOST_TEST_MESSAGE("Testing diffusion and drift of the SLV process...");
    ...
    const ext::shared_ptr<LocalVolTermStructure> localVol = getFixedLocalVolFromHeston(
        ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(rTS, qTS, spot, 0.1, 1.0, 0.13, 0.8, 0.4)),
    ...
    const ext::shared_ptr<HestonSLVProcess> slvProcess = ext::make_shared<HestonSLVProcess>(hestonProcess, localVol);

    VanillaOption option(ext::make_shared<PlainVanillaPayoff>(Option::Call, s0),
                         ext::make_shared<EuropeanExercise>(maturityDate));

    option.setPricingEngine(ext::make_shared<FdHestonVanillaEngine>(hestonModel.currentLink(), 26, 201, 101, 0,
                                                                    FdmSchemeDesc::ModifiedCraigSneyd(), localVol));
    ...
    SobolBrownianBridgeRsg rsg(2, nTimeSteps, SobolBrownianGenerator::Diagonal, 12345U, SobolRsg::JoeKuoD7);
    ...
    for (Size i = 0; i < nSims; ++i)
    {
        ...
        for (Size j = 0; j < nTimeSteps; ++j, t += dt)
        {
            ...
            x = slvProcess->apply(x, slvProcess->diffusion(t, xt) * sqrtDt * dw + slvProcess->drift(t, xt) * dt);
        }
    ...
    {
        BOOST_ERROR("Failed to reproduce call option price with HestonSLVProcess "
