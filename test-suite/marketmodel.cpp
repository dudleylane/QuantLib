...
void setup()
{
    ...
    dates = Schedule(todaysDate, endDate, Period(Semiannual), calendar, Following, Following, DateGeneration::Backward,
                     false);
    ...
    for (Size i = 1; i <= N; ++i)
    {
        ...
        {
            coterminalAnnuity[N - i] = coterminalAnnuity[N - i + 1] + accruals[N - i] * todaysDiscounts[N - i + 1];
        }
    ...
    Volatility mktVols[] = {0.15541283, 0.18719678, 0.20890740, 0.22318179, 0.23212717, 0.23731450, 0.23988649,
                            0.24066384, 0.24023111, 0.23900189, 0.23726699, 0.23522952, 0.23303022, 0.23076564,
                            0.22850101, 0.22627951, 0.22412881, 0.22206569, 0.22009939};
    ...
    {
        volatilities[i] = todaysForwards[i] * mktVols[i] / (todaysForwards[i] + displacement);
...
ext::shared_ptr<SequenceStatisticsInc> simulate(const ext::shared_ptr<MarketModelEvolver>& evolver,
                                                const MarketModelMultiProduct& product)
{
    ...
    ext::shared_ptr<SequenceStatisticsInc> stats(new SequenceStatisticsInc(product.numberOfProducts()));
                                             ...
                                             Volatility volBump = 0.0)
{
    ...
    ext::shared_ptr<LmVolatilityModel> volModel(new LmExtLinearExponentialVolModel(fixingTimes, 0.5, 0.6, 0.1, 0.1));
    ext::shared_ptr<LmCorrelationModel> corrModel(
        new LmLinearExponentialCorrelationModel(evolution.numberOfRates(), longTermCorrelation, beta));

    std::vector<Rate> bumpedForwards(todaysForwards.size());
    std::transform(todaysForwards.begin(), todaysForwards.end(), bumpedForwards.begin(),
                   [=](Rate r) { return r + forwardBump; });

    std::vector<Volatility> bumpedVols(volatilities.size());
    if (logNormal)
        std::transform(volatilities.begin(), volatilities.end(), bumpedVols.begin(),
                       [=](Volatility v) { return v + volBump; });
    else
        std::transform(normalVols.begin(), normalVols.end(), bumpedVols.begin(),
                       [=](Volatility v) { return v + volBump; });

    Matrix correlations = exponentialCorrelations(evolution.rateTimes(), longTermCorrelation, beta);
    ext::shared_ptr<PiecewiseConstantCorrelation> corr(
        new TimeHomogeneousForwardCorrelation(correlations, evolution.rateTimes()));
    switch (marketModelType)
    {
        case ExponentialCorrelationFlatVolatility:
            return ext::shared_ptr<MarketModel>(new FlatVol(bumpedVols, corr, evolution, numberOfFactors,
                                                            bumpedForwards,
                                                            std::vector<Spread>(bumpedForwards.size(), displacement)));
        case ExponentialCorrelationAbcdVolatility:
            return ext::shared_ptr<MarketModel>(new AbcdVol(0.0, 0.0, 1.0, 1.0, bumpedVols, corr, evolution,
                                                            numberOfFactors, bumpedForwards,
                                                            std::vector<Spread>(bumpedForwards.size(), displacement)));
            // case CalibratedMM:
...
std::vector<Size> makeMeasure(const MarketModelMultiProduct& product, MeasureType measureType)
{
    ...
    switch (measureType)
    {
        ...
        case MoneyMarketPlus:
            ...
            {
                BOOST_ERROR("\nfailure in verifying MoneyMarketPlus(" << measureOffset_ << ") measure:\n"
                                                                      << to_stream(result));
            }
            break;
        default:
    ...
    switch (evolverType)
    {
        case Ipc:
            return ext::shared_ptr<MarketModelEvolver>(
                new LogNormalFwdRateIpc(marketModel, generatorFactory, numeraires, initialStep));
        case Balland:
            return ext::shared_ptr<MarketModelEvolver>(
                new LogNormalFwdRateBalland(marketModel, generatorFactory, numeraires, initialStep));
        case Pc:
            return ext::shared_ptr<MarketModelEvolver>(
                new LogNormalFwdRatePc(marketModel, generatorFactory, numeraires, initialStep));
        case NormalPc:
            return ext::shared_ptr<MarketModelEvolver>(
                new NormalFwdRatePc(marketModel, generatorFactory, numeraires, initialStep));
        default:
            QL_FAIL("unknown MarketModelEvolver type");
    }
}
                                       ...
                                       const std::string& config)
{
    ...
    for (subProductExpectedValue = subProductExpectedValues.begin();
         subProductExpectedValue != subProductExpectedValues.end(); ++subProductExpectedValue)
    {
        ...
        {
            Real stdDev = (results[currentResultIndex] - value) / errors[currentResultIndex];
        ...
        if (printReport_ || (subProductExpectedValue->testBias && isBiased) ||
            std::max(-minError, maxError) > errorThreshold)
        {
            ...
            for (Size j = 0; j < subProductExpectedValue->values.size(); ++j)
            {
                BOOST_TEST_MESSAGE(io::ordinal(j + 1) << " " << subProductExpectedValue->description << ": "
                                                      << io::rate(results[currentResultIndex]) << "\t"
                                                      << io::rate(subProductExpectedValue->values[j]) << "\t"
                                                      << io::rate(errors[currentResultIndex])
                                                      << "; discrepancy = " << stdDevs[currentResultIndex] << "\n");
                ++currentResultIndex;
            }
                                ...
                                const std::string& config)
{
    ...
    for (Size i = 0; i < N; ++i)
    {
        expectedForwards[i] = (todaysForwards[i] - forwardStrikes[i]) * accruals[i] * todaysDiscounts[i + 1];
        ...
        expectedCaplets[i] = BlackCalculator(displacedPayoffs[i], todaysForwards[i] + displacement,
                                             volatilities[i] * std::sqrt(expiry), todaysDiscounts[i + 1] * accruals[i])
                                 .value();
    ...
    if (printReport_ || minError > 0.0 || maxError < 0.0 || minError < -errorThreshold || maxError > errorThreshold)
    {
        BOOST_TEST_MESSAGE(config);
        Size i;
        for (i = 0; i < N; ++i)
        {
            BOOST_TEST_MESSAGE(io::ordinal(i + 1)
                               << " forward: " << io::rate(results[i]) << "\t" << io::rate(expectedForwards[i]) << "\t"
                               << io::rate(errors[i]) << "; discrepancy = " << forwardStdDevs[i] << "\n");
        }
        for (i = 0; i < N; ++i)
        {
            BOOST_TEST_MESSAGE(io::ordinal(i + 1)
                               << "\t" << io::rate(results[i + N]) << " +- " << io::rate(errors[i + N]) << "\t"
                               << io::rate(expectedCaplets[i]) << "\t" << io::rate(errors[i + N]) << "; discrepancy = "
                                      ...
                                      const std::string& config)
{
    ...
    for (Size i = 0; i < N; ++i)
    {
        ...
        Time expiry = rateTimes[i];
        expectedCaplets[i] =
            bachelierBlackFormula(displacedPayoffs[i], todaysForwards[i] + displacement,
                                  normalVols[i] * std::sqrt(expiry), todaysDiscounts[i + 1] * accruals[i]);
        capletStdDev[i] = (results[i + N] - expectedCaplets[i]) / errors[i + N];
        if (capletStdDev[i] > maxError)
            maxError = capletStdDev[i];
        else if (capletStdDev[i] < minError)
    ...
    Real errorThreshold = 2.50;
    if (minError > 0.0 || maxError < 0.0 || minError < -errorThreshold || maxError > errorThreshold)
    {
        BOOST_TEST_MESSAGE(config);
        Size i;
        for (i = 0; i < N; ++i)
        {
            BOOST_TEST_MESSAGE(io::ordinal(i + 1)
                               << " forward: " << io::rate(results[i]) << " +- " << io::rate(errors[i])
                               << "; expected: " << io::rate(expectedForwards[i])
                               << "; discrepancy = " << forwardStdDevs[i] << " standard errors");
        }
        for (i = 0; i < N; ++i)
        {
            BOOST_TEST_MESSAGE(io::ordinal(i + 1)
                               << " caplet: " << io::rate(results[i + N]) << " +- " << io::rate(errors[i + N])
                               << "; expected: " << io::rate(expectedCaplets[i]) << "; discrepancy = "
                               << (results[i + N] - expectedCaplets[i]) / (errors[i + N] == 0.0 ? 1.0 : errors[i + N])
                               << " standard errors");
        }
        BOOST_ERROR("test failed");
...
void checkCallableSwap(const SequenceStatisticsInc& stats, const std::string& config)
{
    ...
    if (swapError > tolerance || bermudanNPV < 0.0 || callableNPV < receiverNPV || callableError > tolerance)
    ...
    if (callableError > tolerance)
        BOOST_ERROR("agreement between receiver+bermudan and callable failed:"
                    ...
                    << receiverNPV + bermudanNPV << "\n    callable:          " << callableNPV
                    << "\n    error:             " << callableError << "\n    tolerance:         " << tolerance);
    if (printReport_)
    {
        BOOST_TEST_MESSAGE(std::setprecision(2)
                           << "    payer swap:        " << io::rate(payerNPV) << " +/- "
                           << io::rate(stats.errorEstimate()[0]) << "\n    receiver swap:     " << io::rate(receiverNPV)
                           << " +/- " << io::rate(stats.errorEstimate()[1]) << "\n    bermudan:          "
                           << io::rate(bermudanNPV) << " +/- " << io::rate(stats.errorEstimate()[2])
                           << "\n    receiver+bermudan: " << io::rate(receiverNPV + bermudanNPV)
                           << "\n    callable:          " << io::rate(callableNPV) << " +/- "
                           << io::rate(stats.errorEstimate()[3]));
    }
}
...
BOOST_AUTO_TEST_CASE(testOneStepForwardsAndOptionlets)
{
    ...
    std::vector<ext::shared_ptr<StrikedTypePayoff>> displacedPayoffs(todaysForwards.size());
    for (Size i = 0; i < todaysForwards.size(); ++i)
    {
        forwardStrikes[i] = todaysForwards[i] + 0.01;
        optionletPayoffs[i] = ext::shared_ptr<Payoff>(new PlainVanillaPayoff(Option::Call, todaysForwards[i]));
        displacedPayoffs[i] =
            ext::shared_ptr<StrikedTypePayoff>(new PlainVanillaPayoff(Option::Call, todaysForwards[i] + displacement));
    }

    OneStepForwards forwards(rateTimes, accruals, paymentTimes, forwardStrikes);
    OneStepOptionlets optionlets(rateTimes, accruals, paymentTimes, optionletPayoffs);
    ...
    for (auto& j : marketModels)
    {
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            for (auto& measure : measures)
            {
                ...
                bool logNormal = true;
                ext::shared_ptr<MarketModel> marketModel = makeMarketModel(logNormal, evolution, factors, j);
                ...
                for (Size i = 0; i < std::size(evolvers) - stop; i++)
                {
                    ...
                    for (Size n = 0; n < 1; n++)
                    {
                        ...
                        config << marketModelTypeToString(j) << ", " << factors
                               << (factors > 1 ?
                                       (factors == todaysForwards.size() ? " (full) factors, " : " factors, ") :
                                       " factor,")
                        ...
                        checkForwardsAndOptionlets(*stats, forwardStrikes, displacedPayoffs, config.str());
                    }
                }
            }
        }
...
BOOST_AUTO_TEST_CASE(testOneStepNormalForwardsAndOptionlets)
{
    ...
    std::vector<Rate> forwardStrikes(todaysForwards.size());
    std::vector<ext::shared_ptr<Payoff>> optionletPayoffs(todaysForwards.size());
    std::vector<ext::shared_ptr<PlainVanillaPayoff>> displacedPayoffs(todaysForwards.size());
    for (Size i = 0; i < todaysForwards.size(); ++i)
    {
        forwardStrikes[i] = todaysForwards[i] + 0.01;
        optionletPayoffs[i] = ext::shared_ptr<Payoff>(new PlainVanillaPayoff(Option::Call, todaysForwards[i]));
            ...
            for (auto& measure : measures)
            {
                ...
                for (Size i = 0; i < std::size(evolvers) - stop; i++)
                {
                    ...
                    for (Size n = 0; n < 1; n++)
                    {
                        ...
                        ext::shared_ptr<SequenceStatisticsInc> stats = simulate(evolver, product);
                        checkNormalForwardsAndOptionlets(*stats, forwardStrikes, displacedPayoffs, config.str());
                    }
                }
            }
        }
...
BOOST_AUTO_TEST_CASE(testInverseFloater)
{
    ...
    MultiStepInverseFloater product(rateTimes, fixedAccruals, floatingAccruals, fixedStrikes, fixedMultipliers,
                                    floatingSpreads, paymentTimes, payer);

    MarketModelPathwiseInverseFloater productPathwise(rateTimes, fixedAccruals, floatingAccruals, fixedStrikes,
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            for (auto& measure : measures)
            {
                ...
                for (auto& i : evolvers)
                {
                    ...
                    std::ostringstream config;
                    config << marketModelTypeToString(j) << ", " << factors
                           << (factors > 1 ? (factors == todaysForwards.size() ? " (full) factors, " : " factors, ") :
                                             " factor,")
                    ...
                    ext::shared_ptr<SequenceStatisticsInc> stats = simulate(evolver, productComposite);
                    ...
                    for (Size i = 0; i < accruals.size(); ++i)
                    {
                        ...
                        Real inverseCouponPV =
                            ...
                            blackFormula(Option::Put, fixedStrikes[i] / 2.0, todaysForwards[i], modelVolatilities[i]);
                    ...
                    if (fabs(errorInSds) > 4.0)
                        BOOST_FAIL("Inverse floater product has price error equal to "
                                   << errorInSds << " sds . Price " << truePrice << " MC price " << stats->mean()[0]
                                   << " \n");

                    Real numericalTolerance = 1E-12;

                    if (fabs(stats->mean()[0] - stats->mean()[1]) > numericalTolerance)
                        BOOST_FAIL("Inverse floater and wrapper pathwise inverse floater do not agree:"
                               ...
                               const std::string& testDescription)
{
    ...
    BOOST_TEST_MESSAGE("Testing exact repricing of " << testDescription
                                                     << "in a lognormal forward rate market model...");
    ...
    for (auto& j : marketModels)
    {
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            MeasureType measures[] = {// ProductSuggested,
                                      Terminal, MoneyMarketPlus, MoneyMarket};
            for (auto& measure : measures)
            {
                std::vector<Size> numeraires = makeMeasure(product, measure);
                ...
                for (Size i = 0; i < std::size(evolvers) - stop; i++)
                {
                    ...
                    for (Size n = 0; n < 1; n++)
                    {
                        ...
                        SobolBrownianGeneratorFactory generatorFactory(SobolBrownianGenerator::Diagonal, seed_);
                        ...
                        ext::shared_ptr<SequenceStatisticsInc> stats = simulate(evolver, product);
                        checkMultiProductCompositeResults(*stats, subProductExpectedValues, config.str());
                    }
                }
            }
        }
...
void addForwards(MultiProductComposite& product, std::vector<SubProductExpectedValues>& subProductExpectedValues)
{
    ...
    MultiStepForwards forwards(rateTimes, accruals, paymentTimes, forwardStrikes);
    product.add(forwards);
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i)
    {
        subProductExpectedValues.back().values.push_back((todaysForwards[i] - forwardStrikes[i]) * accruals[i] *
                                                         todaysDiscounts[i + 1]);
    }
}

void addOptionLets(MultiProductComposite& product, std::vector<SubProductExpectedValues>& subProductExpectedValues)
{
    ...
    MultiStepOptionlets optionlets(rateTimes, accruals, paymentTimes, optionletPayoffs);
    product.add(optionlets);
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i)
    {
        subProductExpectedValues.back().values.push_back(
            BlackCalculator(displacedPayoffs[i], todaysForwards[i] + displacement,
                            volatilities[i] * std::sqrt(rateTimes[i]), todaysDiscounts[i + 1] * accruals[i])
                .value());
    }
}


void addCoinitialSwaps(MultiProductComposite& product, std::vector<SubProductExpectedValues>& subProductExpectedValues)
{

    // create the products...
    Real fixedRate = 0.04;
    MultiStepCoinitialSwaps multiStepCoinitialSwaps(rateTimes, accruals, accruals, paymentTimes, fixedRate);
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i)
    {
        coinitialSwapValue += (todaysForwards[i] - fixedRate) * accruals[i] * todaysDiscounts[i + 1];
                                    ...
                                    std::vector<SubProductExpectedValues>& subProductExpectedValues)
{
    ...
    for (Size i = 0; i < payoffs.size(); ++i)
        payoffs[i] = ext::shared_ptr<StrikedTypePayoff>(new PlainVanillaPayoff(Option::Call, todaysForwards[i]));

    MultiStepCoterminalSwaptions swaptions(rateTimes, rateTimes, payoffs);
    product.add(swaps);
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i)
    {
        Real expectedNPV = curveState.coterminalSwapAnnuity(0, i) * (atmRates[i] - fixedRate) * todaysDiscounts.front();
    ...
    subProductExpectedValues.emplace_back("coterminal swaption");
    subProductExpectedValues.back().testBias = false;
    subProductExpectedValues.back().errorThreshold = 2.32;
    const Spread displacement = 0;
    Matrix jacobian = SwapForwardMappings::coterminalSwapZedMatrix(curveState, displacement);
    bool logNormal = true;
    ...
    ext::shared_ptr<MarketModel> marketModel = makeMarketModel(logNormal, evolution, factors, marketModelType);
    for (Size i = 0; i < todaysForwards.size(); ++i)
    {
        ...
        Real expectedSwaption =
            BlackCalculator(payoff, todaysCoterminalSwapRates[i] + displacement, std::sqrt(cotSwapsCovariance[i][i]),
                            curveState.coterminalSwapAnnuity(0, i) * todaysDiscounts[0])
                .value();
...
BOOST_AUTO_TEST_CASE(testAllMultiStepProducts, *precondition(if_speed(Slow)))
{
    ...
    product.finalize();
    testMultiProductComposite(product, subProductExpectedValues, testDescription);
}
...
BOOST_AUTO_TEST_CASE(testPeriodAdapter)
{
    ...
    LMMCurveState bigRateCS(ForwardForwardMappings::RestrictCurveState(cs, period, offset));
    ...
    {
        optionletPayoffs[i] =
            ext::shared_ptr<StrikedTypePayoff>(new PlainVanillaPayoff(Option::Call, bigRateCS.forwardRate(i)));
        swaptionPayoffs[i] =
            ext::shared_ptr<StrikedTypePayoff>(new PlainVanillaPayoff(Option::Call, bigRateCS.coterminalSwapRate(i)));
        displacedOptionletPayoffs[i] = ext::shared_ptr<StrikedTypePayoff>(
            new PlainVanillaPayoff(Option::Call, bigRateCS.forwardRate(i) + displacement));
        displacedSwaptionPayoffs[i] = ext::shared_ptr<StrikedTypePayoff>(
            new PlainVanillaPayoff(Option::Call, bigRateCS.coterminalSwapRate(i) + displacement));
    }

    MultiStepPeriodCapletSwaptions theProduct(rateTimes, capletPaymentTimes, swaptionPaymentTimes, optionletPayoffs,
                                              swaptionPayoffs, period, offset);
    ...
    ext::shared_ptr<MarketModel> originalModel =
        makeMarketModel(logNormal, evolution, factors, ExponentialCorrelationAbcdVolatility);

    std::vector<Spread> newDisplacements;

    ext::shared_ptr<MarketModel> adaptedforwardModel(
        new FwdPeriodAdapter(originalModel, period, offset, newDisplacements));

    ext::shared_ptr<MarketModel> adaptedSwapModel(new FwdToCotSwapAdapter(adaptedforwardModel));
    ...
    for (Size j = 0; j < adaptedSwapModel->numberOfRates(); ++j)
    {
        ...
        Real capletAnnuity = todaysDiscounts[0] * bigRateCS.discountRatio(j + 1, 0) * bigRateCS.rateTaus()[j];

        approxCapletPrices[j] = BlackCalculator(displacedOptionletPayoffs[j], bigRateCS.forwardRate(j) + displacement,
                                                adaptedForwardSds[j], capletAnnuity)
                                    .value();

        Real swaptionAnnuity = todaysDiscounts[0] * bigRateCS.coterminalSwapAnnuity(0, j);

        approxSwaptionPrices[j] =
            BlackCalculator(displacedSwaptionPayoffs[j], bigRateCS.coterminalSwapRate(j) + displacement,
                            adaptedSwapSds[j], swaptionAnnuity)
                .value();
    }
    SobolBrownianGeneratorFactory generatorFactory(SobolBrownianGenerator::Diagonal, seed_);
    ...
    ext::shared_ptr<MarketModelEvolver> evolver =
        makeMarketModelEvolver(originalModel, theProduct.suggestedNumeraires(), generatorFactory, Pc);

    ext::shared_ptr<SequenceStatisticsInc> stats = simulate(evolver, theProduct);
    ...
    for (Size i = 0; i < numberBigRates; ++i)
    {
        ...
        {
            BOOST_FAIL(io::ordinal(i + 1)
                       << "caplet , approx price " << approxCapletPrices[i] << ", \t simulation price " << results[i]
    ...
    for (Size i = 0; i < numberBigRates; ++i)
    {
        ...
        {
            BOOST_FAIL(io::ordinal(i + 1)
                       << "swaption, approx price " << approxSwaptionPrices[i] << ", \t simulation price "
                       << results[i + numberBigRates] << ", \t error in sds " << swaptionErrorsInSds[i]);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testCallableSwapNaif, *precondition(if_speed(Slow)))
{
    ...
    // 0. a payer swap
    MultiStepSwap payerSwap(rateTimes, accruals, accruals, paymentTimes, fixedRate, true);

    // 1. the equivalent receiver swap
    MultiStepSwap receiverSwap(rateTimes, accruals, accruals, paymentTimes, fixedRate, false);
    ...
    CallSpecifiedMultiProduct dummyProduct =
        CallSpecifiedMultiProduct(receiverSwap, naifStrategy, ExerciseAdapter(nullRebate));
                ...
                for (Size i = 0; i < std::size(evolvers) - stop; i++)
                {
                    ...
                    for (Size n = 0; n < 1; n++)
                    {
                        ...
                        CallSpecifiedMultiProduct bermudanProduct =
                            CallSpecifiedMultiProduct(MultiStepNothing(evolution), naifStrategy, payerSwap);
                        ...
                        ext::shared_ptr<SequenceStatisticsInc> stats = simulate(evolver, allProducts);
                        ...
                        SobolBrownianGeneratorFactory uFactory(SobolBrownianGenerator::Diagonal, seed_ + 142);
                        ...
                        for (Size s = 0; s < isExerciseTime.size(); ++s)
                        {
                            if (isExerciseTime[s])
                            {
                                ...
                                ext::shared_ptr<MarketModelEvolver> e =
                                    makeMarketModelEvolver(marketModel, numeraires, iFactory, evolvers[i], s);
                        ...
                        UpperBoundEngine uEngine(evolver, innerEvolvers, receiverSwap, nullRebate, receiverSwap,
                                                 nullRebate, naifStrategy, initialNumeraireValue);
                        Statistics uStats;
            ...
            for (auto& measure : measures)
            {
                ...
                for (Size i = 0; i < std::size(evolvers) - stop; i++)
                {
                    ...
                    for (Size n = 0; n < 1; n++)
                    {
                        ...
                        collectNodeData(*evolver, receiverSwap, basisSystem, nullRebate, control, trainingPaths_,
                                        collectedData);
                        genericLongstaffSchwartzRegression(collectedData, basisCoefficients);
                        LongstaffSchwartzExerciseStrategy exerciseStrategy(basisSystem, basisCoefficients, evolution,
                                                                           numeraires, nullRebate, control);

                        // 2. bermudan swaption to enter into the payer swap
                        CallSpecifiedMultiProduct bermudanProduct =
                            CallSpecifiedMultiProduct(MultiStepNothing(evolution), exerciseStrategy, payerSwap);
                        ...
                        UpperBoundEngine uEngine(evolver, innerEvolvers, receiverSwap, nullRebate, receiverSwap,
                                                 nullRebate, exerciseStrategy, initialNumeraireValue);
                        Statistics uStats;
                        uEngine.multiplePathValues(uStats, 255, 256);
                        Real delta = uStats.mean();
                        Real deltaError = uStats.errorEstimate();
...
using slices = boost::mpl::vector<slice<ExponentialCorrelationFlatVolatility, 4>,
...
BOOST_AUTO_TEST_CASE_TEMPLATE(testCallableSwapAnderson, T, slices)
{
    ...
    BOOST_TEST_MESSAGE("Pricing callable swap with Anderson exercise "
                       ...
                       << testedFactor << " and model type " << marketModelTypeToString(marketModelType) << "...");
    ...
    NothingExerciseValue control(rateTimes);
    NothingExerciseValue nullRebate(rateTimes);
    TriggeredSwapExercise parametricForm(rateTimes, exerciseTimes, std::vector<Time>(exerciseTimes.size(), fixedRate));
    ...
    for (auto& measure : measures)
    {
        ...
        for (Size i = 0; i < std::size(evolvers) - stop; i++)
        {
            for (Size n = 0; n < 1; n++)
            {
                ...
                collectNodeData(*evolver, receiverSwap, parametricForm, nullRebate, control, trainingPaths_,
                                collectedData);
                Simplex om(0.01);
                ...
                Real initialNumeraireValue = todaysDiscounts[initialNumeraire];
                Real firstPassValue =
                    genericEarlyExerciseOptimization(collectedData, parametricForm, parameters, ec, om) *
                ...
                UpperBoundEngine uEngine(evolver, innerEvolvers, receiverSwap, nullRebate, receiverSwap, nullRebate,
...
BOOST_AUTO_TEST_CASE(testGreeks, *precondition(if_speed(Fast)))
{
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i)
    {
        payoffs[i] = ext::shared_ptr<Payoff>(new
                                             ...
                                             CashOrNothingPayoff(Option::Call, todaysForwards[i], 0.01));
        displacedPayoffs[i] = ext::shared_ptr<StrikedTypePayoff>(
            new
            // PlainVanillaPayoff(Option::Call, todaysForwards[i]+displacement));
            CashOrNothingPayoff(Option::Call, todaysForwards[i] + displacement, 0.01));
    }

    MultiStepOptionlets product(rateTimes, accruals, paymentTimes, payoffs);
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            for (auto& measure : measures)
            {
                ...
                for (Size n = 0; n < 1; n++)
                {
                    ...
                    deltaGammaEvolvers.push_back(ext::shared_ptr<ConstrainedEvolver>(
                        new LogNormalFwdRateEulerConstrained(marketModel, generatorFactory, numeraires)));
                    deltaGammaEvolvers.back()->setConstraintType(startIndexOfConstraint, endIndexOfConstraint);
                    marketModel = makeMarketModel(logNormal, evolution, factors, j, forwardBump);
                    deltaGammaEvolvers.push_back(ext::shared_ptr<ConstrainedEvolver>(
                        new LogNormalFwdRateEulerConstrained(marketModel, generatorFactory, numeraires)));
                    deltaGammaEvolvers.back()->setConstraintType(startIndexOfConstraint, endIndexOfConstraint);
                    ...
                    vegaEvolvers.push_back(ext::shared_ptr<ConstrainedEvolver>(
                        new LogNormalFwdRateEulerConstrained(marketModel, generatorFactory, numeraires)));
                    vegaEvolvers.back()->setConstraintType(startIndexOfConstraint, endIndexOfConstraint);
                    marketModel = makeMarketModel(logNormal, evolution, factors, j, 0.0, volBump);
                    vegaEvolvers.push_back(ext::shared_ptr<ConstrainedEvolver>(
                        new LogNormalFwdRateEulerConstrained(marketModel, generatorFactory, numeraires)));
                    vegaEvolvers.back()->setConstraintType(startIndexOfConstraint, endIndexOfConstraint);
                    ...
                    ProxyGreekEngine engine(evolver, constrainedEvolvers, diffWeights, startIndexOfConstraint,
                                            endIndexOfConstraint, product, initialNumeraireValue);
                    ...
                    for (Size i = 0; i < todaysForwards.size(); ++i)
                    {
                        ...
                        pricePlus[i] = BlackCalculator(displacedPayoffs[i], fwdPlus[i],
                                                       volatilities[i] * sqrt(rateTimes[i]), discPlus[i + 1] * tau)
                                           .value();
                        price0[i] = BlackCalculator(displacedPayoffs[i], todaysForwards[i],
                                                    volatilities[i] * sqrt(rateTimes[i]), todaysDiscounts[i + 1] * tau)
                                        .value();
                        priceMinus[i] = BlackCalculator(displacedPayoffs[i], fwdMinus[i],
                                                        volatilities[i] * sqrt(rateTimes[i]), discMinus[i + 1] * tau)
                                            .value();
                    }
...
BOOST_AUTO_TEST_CASE(testPathwiseGreeks)
{
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i)
    {
        payoffs[i] = ext::shared_ptr<Payoff>(new PlainVanillaPayoff(Option::Call, todaysForwards[i]));
        // CashOrNothingPayoff(Option::Call, todaysForwards[i], 0.01));
        displacedPayoffs[i] =
            ext::shared_ptr<StrikedTypePayoff>(new PlainVanillaPayoff(Option::Call, todaysForwards[i] + displacement));
        // CashOrNothingPayoff(Option::Call, todaysForwards[i]+displacement, 0.01));
    ...
    {
        MarketModelPathwiseMultiDeflatedCaplet product1(rateTimes, accruals, paymentTimes, todaysForwards);

        MarketModelPathwiseMultiCaplet product2(rateTimes, accruals, paymentTimes, todaysForwards);
        ...
        MultiStepOptionlets productDummy(rateTimes, accruals, paymentTimes, payoffs);
                    ...
                    for (Size n = 0; n < 1; n++)
                    {
...

                        {
                            ...
                            PathwiseAccountingEngine accountingengine(
                                ext::make_shared<LogNormalFwdRateEuler>(evolver), // method relies heavily on LMM Euler
                                *product,
                        ...
                        for (Size i = 0; i < product->numberOfProducts(); ++i)
                        {
                            ...
                            modelPrices[i] = BlackCalculator(displacedPayoffs[i], todaysForwards[i],
                                                             ...
                                                             todaysDiscounts[i + 1] * (rateTimes[i + 1] - rateTimes[i]))
                                                 .value();
                        ...
                        for (Size i = 0; i < todaysForwards.size(); ++i)
                        {
                            ...
                            for (Size k = 0; k < product->numberOfProducts(); ++k)
                            {
                                ...
                                Real priceUp =
                                    BlackCalculator(displacedPayoffs[k], fwdPlus[k],
                                                    volatilities[k] * sqrt(rateTimes[k]), discPlus[k + 1] * tau)
                                        .value();
                                Real priceDown =
                                    BlackCalculator(displacedPayoffs[k], fwdMinus[k],
                                                    volatilities[k] * sqrt(rateTimes[k]), discMinus[k + 1] * tau)
                                        .value();
                        ...
                        for (Size i = 0; i < product->numberOfProducts(); ++i)
                        {
                            ...
                            {
                                BOOST_TEST_MESSAGE("Caplet " << i << " price " << prices[i] << " model price "
                                                             << modelPrices[i]
                            ...
                            for (Size j = 0; j < todaysForwards.size(); ++j)
                            {
                                ...
                                if (fabs(deltaErrorInSds) > errorTheshold)
                                {
                                    ...
                                    BOOST_TEST_MESSAGE("Caplet " << i << " delta " << j << "has value " << deltas[i][j]
                                                                 << " model value " << modelDeltas[i][j]
                                                                 << "   Standard error: " << deltasErrors[i][j]
...
BOOST_AUTO_TEST_CASE(testPathwiseVegas, *precondition(if_speed(Fast)))
{
    ...
    BOOST_TEST_MESSAGE("Testing pathwise vegas in a lognormal forward rate market model...");
    ...
    MarketModelPathwiseMultiCaplet caplets(rateTimes, accruals, paymentTimes, todaysForwards);


    MarketModelPathwiseMultiDeflatedCaplet capletsDeflated(rateTimes, accruals, paymentTimes, todaysForwards);
    ...
    Size pathsToDo = 10; // for the numerical differentiation test we are requiring equality on each path so this is
    ...
    for (auto& j : marketModels)
    {
        ...
        Size testedFactors[] = {
            std::min<Size>(3UL, todaysForwards.size())
            //    todaysForwards.size()
            //, 4, 8,
        };
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            SwaptionPseudoDerivative derivative(marketModel, startIndex, endIndex);
            ...
            // test that the derivative of swaption implied vols to the pseudo-root elements are correct, finite
            // differencing versus analytic value
            ...
            for (Size step = 0; step < evolution.numberOfSteps(); ++step)
            {
                for (Size l = 0; l < evolution.numberOfRates(); ++l)
                    for (Size f = 0; f < factors; ++f)
                    {
                        ...
                        PseudoRootFacade bumpedUp(pseudoRoots, rateTimes, marketModel->initialRates(),
                                                  marketModel->displacements());


                        // compute the implied vol of the swaption with the bumped pseudo roots

                        Real upImpVol = SwapForwardMappings::swaptionImpliedVolatility(bumpedUp, startIndex, endIndex);
                        ...
                        PseudoRootFacade bumpedDown(pseudoRoots, rateTimes, marketModel->initialRates(),
                                                    marketModel->displacements());

                        // compute the implied vol of the swaption with the bumped down pseudo roots

                        Real downImpVol =
                            SwapForwardMappings::swaptionImpliedVolatility(bumpedDown, startIndex, endIndex);

                        // undo bumping

                        pseudoRoots[step][l][f] += numericalBumpSizeForSwaptionPseudo;

                        // use symmetric finite differencing to compute the change in the swaptions implied vol for
                        // changes in this pseudo-root element
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            for (Size startIndex = 1; startIndex < evolution.numberOfRates() - 1; ++startIndex)
                for (Size endIndex = startIndex + 1; endIndex < evolution.numberOfRates(); ++endIndex)
                {
                    ...
                    CapPseudoDerivative derivative(marketModel, capStrike, startIndex, endIndex, initialNumeraireValue);
                    ...
                    for (Size step = 0; step < evolution.numberOfSteps(); ++step)
                    {
                        for (Size l = 0; l < evolution.numberOfRates(); ++l)
                            for (Size f = 0; f < factors; ++f)
                            {
                                ...
                                // get total covariances of rates with bumped up pseudo-roots , we really only need the
                                // variances
                                ...
                                // get total covariances of rates with bumped down pseudo-roots , we really only need
                                // the variances
                                ...
                                for (Size k = startIndex; k < endIndex; ++k)
                                {
                                    ...
                                    Real upPrice = blackFormula(Option::Call, capStrike, forward, upSd, annuity,
                                                                marketModel->displacements()[k]);


                                    Real downPrice = blackFormula(Option::Call, capStrike, forward, downSd, annuity,
                                                                  marketModel->displacements()[k]);
                    ...
                    // test the implied vol of the cap, each underlying caplet has a different implied vol and the cap's
                    // is different again
                    ...
                    for (Size m = startIndex; m < endIndex; ++m)
                    {
                        ...
                        priceConstVol += blackFormula(Option::Call, capStrike, forward, impVol * sqrt(expiry), annuity,
                                                      marketModel->displacements()[m]);

                        priceVarVol += blackFormula(Option::Call, capStrike, forward, sqrt(totalCov[m][m]), annuity,
                                                    marketModel->displacements()[m]);
                    }
                    ...
                    for (Size step = 0; step < evolution.numberOfSteps(); ++step)
                    {
                        for (Size l = 0; l < evolution.numberOfRates(); ++l)
                            for (Size f = 0; f < factors; ++f)
                            {
                                ...
                                CapPseudoDerivative upDerivative(
                                    ext::shared_ptr<MarketModel>(new PseudoRootFacade(bumpedUp)), capStrike, startIndex,
                                ...
                                CapPseudoDerivative downDerivative(
                                    ext::shared_ptr<MarketModel>(new PseudoRootFacade(bumpedDown)), capStrike,
                                    startIndex, endIndex, initialNumeraireValue);
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            for (auto& measure : measures)
            {
                ...
                for (Size l = 0; l < evolution.numberOfSteps(); ++l)
                {
                    ...
                    testees.emplace_back(pseudoRoot, evolution.firstAliveRate()[l], numeraires[l], evolution.rateTaus(),
                                         pseudoBumps, marketModel->displacements());

                    testees2.emplace_back(pseudoRoot, evolution.firstAliveRate()[l], numeraires[l],
                                          evolution.rateTaus(), marketModel->displacements());


                    testers.emplace_back(pseudoRoot, evolution.firstAliveRate()[l], numeraires[l], evolution.rateTaus(),
                                         pseudoBumps, marketModel->displacements());
                    testersDown.emplace_back(pseudoRoot, evolution.firstAliveRate()[l], numeraires[l],
                                             evolution.rateTaus(), pseudoBumpsDown, marketModel->displacements());
                }


                ext::shared_ptr<BrownianGenerator> generator(generatorFactory.create(factors, steps));
                LogNormalFwdRateEuler evolver(marketModel, generatorFactory, numeraires);
                ...
                std::vector<std::vector<MarketModelMultiProduct::CashFlow>> cashFlowsGenerated(
                ...
                for (Size l = 0; l < pathsToDo; ++l)
                {
                    ...
                    do
                    {
                        ...
                        evolver.advanceStep();
                            ...
                            product.nextTimeStep(evolver.currentState(), numberCashFlowsThisStep, cashFlowsGenerated);
                        ...
                        for (Size j = 0; j < B.rows(); ++j)
                            for (Size k = 0; k < B.columns(); ++k)
                            {
                                ...
                                if (fabs(errorSize) > multiplier)
                                {
                                    ++numberFailures;
                                    if (printReport_)
                                        BOOST_TEST_MESSAGE("path " << l << " step " << currentStep << " j " << j
                                                                   << " k " << k << " B " << B[j][k] << "  B2 "
                                                                   << B2[j][k]);
                                }
            ...
            for (Size deflate = 0; deflate < 2; ++deflate)
            {
                ...
                for (auto& measure : measures)
                {
                    ...
                    //      SequenceStatistics
                    //      stats(product.numberOfProducts()*(todaysForwards.size()+1+vegaBumps[0].size()));
...

                    {
                        ...
                        PathwiseVegasAccountingEngine accountingengine(
                            ext::make_shared<LogNormalFwdRateEuler>(evolver), // method relies heavily on LMM Euler
                            productToUse,
                            marketModel, // we need pseudo-roots and displacements
                            vegaBumps, initialNumeraireValue);
                    ...
                    {
                        truePrices[r] =
                            BlackCalculator(displacedPayoffs[r], todaysForwards[r], sqrt(totalCovariance[r][r]),
                                            todaysDiscounts[r + 1] * (rateTimes[r + 1] - rateTimes[r]))
                                .value();
                    }
                    ...
                    for (Size b = 0; b < vegaBumps[0].size(); ++b)
                    {
                        ...
                        for (Size r = 0; r < truePrices.size(); ++r)
                        {
                            ...
                            bumpedPrices[r] =
                                BlackCalculator(displacedPayoffs[r], todaysForwards[r], sqrt(variances[r]),
                                                todaysDiscounts[r + 1] * (rateTimes[r + 1] - rateTimes[r]))
                                    .value();
                    ...
                    // for deltas and prices the pathwise vega engine should agree precisely with the pathwiseaccounting
                    // engine so lets see if it does
                    ...
                    for (Size i = 0; i < productToUse2->numberOfProducts(); ++i)
                    {
                        ...
                        if (fabs(priceDiff) > 5 * priceErrors2[i]) // two sets of standard error
                            BOOST_FAIL("pathwise accounting engine and pathwise vegas accounting engine not in perfect "
                                       "agreement for price.\n product "
                        ...
                        for (Size j = 0; j < todaysForwards.size(); ++j)
                        {
                            ...
                            if (fabs(error) > 5 * deltasErrors2[i][j]) // two sets of standard error
                                BOOST_FAIL("pathwise accounting engine and pathwise vegas accounting engine not in "
                                           "perfect agreement for dealts.\n product "
            ...
            if (numberDeflatedErrors + numberUndeflatedErrors > 0)
                BOOST_FAIL("Model pathwise vega test for caplets fails : "
                           << numberDeflatedErrors << " deflated errors and " << numberUndeflatedErrors
...

            {
                ...
                MarketModelPathwiseMultiDeflatedCap capsDeflated(rateTimes, accruals, paymentTimes, capStrike,
                                                                 startsAndEnds);
                ...
                for (auto& measure : measures)
                {
                    ...
                    LogNormalFwdRateEuler evolver2(marketModel, generatorFactory2, numeraires);
...

                    {
                        ...
                        PathwiseVegasOuterAccountingEngine accountingengine(
                            ext::make_shared<LogNormalFwdRateEuler>(evolver2), // method relies heavily on LMM Euler
                            capsDeflated,
                            marketModel, // we need pseudo-roots and displacements
                            vegaBumps, initialNumeraireValue);
...

                    {
                        ...
                        if (numberMeanFailures > 0)
                            BOOST_FAIL("Comparison of Pathwise vegas accounting engine and "
                                       "PathwiseVegasOuterAccountingEngine yields discrepancies:"
                                       << numberMeanFailures << "  out of " << values.size());
                    }
                    ...
                    ext::shared_ptr<StrikedTypePayoff> dispayoff(
                        new PlainVanillaPayoff(Option::Call, capStrike + displacement));

                    for (Size r = 0; r < trueCapletPrices.size(); ++r)
                        trueCapletPrices[r] =
                            BlackCalculator(dispayoff, todaysForwards[r], sqrt(totalCovariance[r][r]),
                                            todaysDiscounts[r + 1] * (rateTimes[r + 1] - rateTimes[r]))
                                .value();
                    ...
                    for (Size b = 0; b < vegaBumps[0].size(); ++b)
                    {
                        ...
                        for (Size r = 0; r < trueCapletPrices.size(); ++r)
                        {
                            bumpedCapletPrices[r] =
                                BlackCalculator(dispayoff, todaysForwards[r], sqrt(variances[r]),
                                                todaysDiscounts[r + 1] * (rateTimes[r + 1] - rateTimes[r]))
                                    .value();
...
BOOST_AUTO_TEST_CASE(testPathwiseMarketVegas)
{
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i)
    {
        payoffs[i] = ext::shared_ptr<Payoff>(new PlainVanillaPayoff(Option::Call, cs.coterminalSwapRate(i)));

        displacedPayoffs[i] = ext::shared_ptr<StrikedTypePayoff>(
            new PlainVanillaPayoff(Option::Call, cs.coterminalSwapRate(i) + displacement));
    }


    MultiStepOptionlets dummyProduct(rateTimes, accruals, paymentTimes, payoffs);

    Real bumpSizeNumericalDifferentiation = 1E-6;

    MarketModelPathwiseCoterminalSwaptionsDeflated swaptionsDeflated(rateTimes, cs.coterminalSwapRates());
    MarketModelPathwiseCoterminalSwaptionsNumericalDeflated swaptionsDeflated2(rateTimes, cs.coterminalSwapRates(),
                                                                               bumpSizeNumericalDifferentiation);
    ...
    MarketModelPathwiseMultiDeflatedCap capsDeflated(rateTimes, accruals, paymentTimes, capStrike, startsAndEnds);
    ...
    for (auto& j : marketModels)
    {
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            VegaBumpCollection possibleBumps(marketModel, allowFactorwiseBumping);

            OrthogonalizedBumpFinder bumpFinder(
                possibleBumps, swaptions, caps,
            ...
            // and zero off it
            {
                ...
                {
                    SwaptionPseudoDerivative thisPseudoDerivative(marketModel, swaptions[i].startIndex_,
                ...
                if (numberDiagonalFailures + offDiagonalFailures > 0)
                    BOOST_FAIL("Pathwise market vega analytic test fails for  swaptions : "
                               << offDiagonalFailures << " off diagonal failures \n "
            ...
            {
                CapPseudoDerivative thisPseudoDerivative(marketModel, caps[i].strike_, caps[i].startIndex_,
            ...
            if (numberDiagonalFailures + offDiagonalFailures > 0)
                BOOST_FAIL("Pathwise market vega analytic test fails for caps : "
                           << offDiagonalFailures << " off diagonal failures \n "
                           << " and " << numberDiagonalFailures << " on the diagonal.");
    ...
    std::vector<std::vector<MarketModelPathwiseMultiProduct::CashFlow>> cashFlowsGenerated1(
    ...
    std::vector<std::vector<MarketModelPathwiseMultiProduct::CashFlow>> cashFlowsGenerated2(cashFlowsGenerated1);
    ...
    for (auto& j : marketModels)
    {
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            LogNormalFwdRateEuler evolver1(marketModel, generatorFactory, swaptionsDeflated.suggestedNumeraires());

            LogNormalFwdRateEuler evolver2(marketModel, generatorFactory, swaptionsDeflated.suggestedNumeraires());
            ...
            for (Size p = 0; p < pathsToDo; ++p)
            {
                ...
                do
                {
                    ...
                    done = swaptionsDeflated.nextTimeStep(evolver1.currentState(), numberCashFlowsThisStep1,
                                                          cashFlowsGenerated1);

                    evolver2.advanceStep();
                    bool done2 = swaptionsDeflated2.nextTimeStep(evolver2.currentState(), numberCashFlowsThisStep2,
                                                                 cashFlowsGenerated2);
                    ...
                    for (Size prod = 0; prod < swaptionsDeflated.numberOfProducts(); ++prod)
                    {
                        ...
                        for (Size cf = 0; cf < numberCashFlowsThisStep1[prod]; ++cf)
                            for (Size rate = 0; rate <= numberRates; ++rate)
                                if (fabs(cashFlowsGenerated1[prod][cf].amount[rate] -
                                         cashFlowsGenerated2[prod][cf].amount[rate]) > tolerance)
                                    BOOST_FAIL(
                                        "numerical swaptions derivative and swaptions disagree on cash flow size. cf = "
                                        << cf << "step " << step << ", rate " << rate << ", amount1 "
                                        << cashFlowsGenerated1[prod][cf].amount[rate] << " ,amount2 "
                                        << cashFlowsGenerated2[prod][cf].amount[rate] << "\n");
                    }
    ...
    for (auto& j : marketModels)
    {
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            LogNormalFwdRateEuler evolver(marketModel, generatorFactory, swaptionsDeflated.suggestedNumeraires());
...

            {
                ...
                PathwiseVegasAccountingEngine accountingEngine(ext::make_shared<LogNormalFwdRateEuler>(evolver),
                                                               swaptionsDeflated, marketModel, theBumps,
                                                               initialNumeraireValue);
            ...
            for (Size i = 0; i < swaptions.size(); ++i)
                impliedVols_[i] = SwapForwardMappings::swaptionImpliedVolatility(*marketModel, swaptions[i].startIndex_,
                                                                                 swaptions[i].endIndex_);
            ...
            for (Size i = 0; i < swaptions.size(); ++i)
            {
                ...
                Real vega = blackFormulaVolDerivative(swapRate, swapRate, sd, expiry, annuity, swapDisplacement);
            ...
            for (Size i = 0; i < swaptions.size(); ++i)
            {
                ...
                Real thisErrorInSds =
                    thisError / (standardErrors[i][i] + 1E-6); // silly to penalize for tiny standard error
            ...
            if (offDiagonalFailures + numberDiagonalFailures > 0)
                BOOST_FAIL("Pathwise market vega test fails for coterminal swaptions : "
                           << offDiagonalFailures << " off diagonal failures \n "
                           << " and " << numberDiagonalFailures << " on the diagonal.");
    ...
    for (auto& j : marketModels)
    {
        ...
        Size testedFactors[] = {
            std::min<Size>(2UL, todaysForwards.size())
            //    todaysForwards.size()
            //, 4, 8,
        };
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            LogNormalFwdRateEuler evolver(marketModel, generatorFactory, capsDeflated.suggestedNumeraires());
...

            {
                ...
                PathwiseVegasAccountingEngine accountingEngine(ext::make_shared<LogNormalFwdRateEuler>(evolver),
                                                               capsDeflated, marketModel, theBumps,
                                                               initialNumeraireValue);
            ...
            for (Size i = 0; i < caps.size(); ++i)
            {
                ...
                CapPseudoDerivative capPseudo(marketModel, caps[i].strike_, caps[i].startIndex_, caps[i].endIndex_,
                                              initialNumeraireValue);
                ...
                for (Size j = caps[i].startIndex_; j < caps[i].endIndex_; ++j)
                {
                    ...
                    Real capletVega =
                        blackFormulaVolDerivative(caps[i].strike_, forward, sd, expiry, annuity, displacement);
            ...
            for (Size i = 0; i < caps.size(); ++i)
            {
                ...
                Real thisErrorInSds = thisError / (standardErrors[i][i + swaptions.size()] +
                                                   1E-6); // silly to penalize for tiny standard error

                if (fabs(thisErrorInSds) > 4)
                {
                    BOOST_TEST_MESSAGE(" MC cap vega: " << vegasMatrix[i][i + swaptions.size()]
                                                        << " Analytic cap vega:" << analyticVegas[i]
                                                        << " Error in sds:" << thisErrorInSds << "\n");
                    ++numberDiagonalFailures;
                }
            }
            ...
            if (offDiagonalFailures + numberDiagonalFailures > 0)
                BOOST_FAIL("Pathwise market vega test fails for caps: "
                           << offDiagonalFailures << " off diagonal failures \n "
                           << " and " << numberDiagonalFailures << " on the diagonal.");
        ...
        for (Size k = 0; k < N - i; k++)
        {
            ...
            for (Size j = 0; j < N; j++)
            {
                ...
                for (Size l = 0; l < N - j; l++)
                {
                    ...
                    {
                        BOOST_ERROR("     T1=" << T1 << "," << "T2=" << T2 << ",\t\t" << "xMin=" << xMin << ","
                                               << "xMax=" << xMax << ",\t\t" << "analytical: " << analytical << ",\t"
                                               << "numerical:   " << numerical);
                    }
                    if (T1 == T2)
                    {
...
BOOST_AUTO_TEST_CASE(testAbcdVolatilityCompare)
{
    ...
    */
    ...
    ext::shared_ptr<LmVolatilityModel> lmAbcd(new LmExtLinearExponentialVolModel(rateTimes, b, c, d, a));
    ...
    for (i1 = 0; i1 < rateTimes.size(); i1++)
    {
        for (i2 = 0; i2 < rateTimes.size(); i2++)
        {
            ...
            do
            {
                ...
                {
                    BOOST_FAIL(" T1=" << rateTimes[i1] << "," << "T2=" << rateTimes[i2] << ",\t\t" << "xMin=" << 0
                                      << "," << "xMax=" << T << ",\t\t" << "abcd: " << abcdCovariance << ",\t"
                                      << "lm: " << lmCovariance);
                }
                T += 0.5;
...
BOOST_AUTO_TEST_CASE(testAbcdVolatilityFit)
{
    ...
    for (Size i = 0; i < blackVols.size(); i++)
    {
        ...
        {
            Real modelVol = abcd.volatility(0.0, rateTimes[i], rateTimes[i]);
...
BOOST_AUTO_TEST_CASE(testStochVolForwardsAndOptionlets)
{
    ...
    ext::shared_ptr<MarketModelVolProcess> volProcess(new SquareRootAndersen(
        meanLevel, reversionSpeed, volVar, v0, evolution.evolutionTimes(), numberSubSteps, w1, w2, cutPoint));
        ...
        for (unsigned long factors : testedFactors)
        {
            ...
            for (auto& measure : measures)
            {
                ...
                for (Size n = 0; n < 1; n++)
                {
                    ...
                    ext::shared_ptr<MarketModelEvolver> evolver(new SVDDFwdRatePc(marketModel, generatorFactory,
                                                                                  volProcess, firstVolatilityFactor,
                                                                                  volatilityFactorStep, numeraires));
                    ...
                    {
                        Real trueValue =
                            todaysDiscounts[i] - todaysDiscounts[i + 1] * (1 + forwardStrikes[i] * accruals[i]);
                        Real error = results[i] - trueValue;
                        Real errorSds = error / errors[i];

                        if (fabs(errorSds) > 3.5)
                            BOOST_FAIL("error in sds: " << errorSds << " for forward " << i
                                                        << " in SV LMM test. True value:" << trueValue
                                                        << ", actual value: " << results[i] << " , standard error "
                                                        << errors[i]);
                    }
                    ...
                    for (Size i = 0; i < accruals.size(); ++i)
                    {
                        ...
                        Real trueValue =
                            AnalyticHestonEngine(
                                ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(
                                    ...
                                    Handle<YieldTermStructure>(flatRate(0.0, dayCounter)),
                                    Handle<Quote>(ext::make_shared<SimpleQuote>(todaysForwards[i] + displacement)), v1,
                                    kappa, theta, sigma, rho)),
                                AnalyticHestonEngine::Gatheral, AnalyticHestonEngine::Integration::gaussLaguerre(128))
                                .priceVanillaPayoff(ext::make_shared<PlainVanillaPayoff>(
                                                        payoff->optionType(), todaysForwards[i] + displacement),
                                                    rateTimes[i]);
                        ...
                        if (fabs(errorSds) > 4)
                            BOOST_FAIL("error in sds: " << errorSds << " for caplet " << i
                                                        << " in SV LMM test. True value:" << trueValue
...
BOOST_AUTO_TEST_CASE(testDriftCalculator)
{
    ...
    MarketModelType marketModels[] = {ExponentialCorrelationFlatVolatility, ExponentialCorrelationAbcdVolatility};
    for (auto& k : marketModels)
    { // loop over market models
        bool logNormal = true;
        ext::shared_ptr<MarketModel> marketModel = makeMarketModel(logNormal, evolution, factors, k);
        ...
        { // loop over steps
            ...
            for (Size h = inf; h < numeraires.size(); ++h)
            { // loop over numeraires
                LMMDriftCalculator driftcalculator(A, displacements, rateTaus, numeraires[h], alive[j]);
                driftcalculator.computePlain(todaysForwards, drifts);
                driftcalculator.computeReduced(todaysForwards, driftsReduced);
                for (Size i = 0; i < drifts.size(); ++i)
                {
                    Real error = std::abs(driftsReduced[i] - drifts[i]);
                    if (error > tolerance)
                        BOOST_ERROR("MarketModel: "
                                    << marketModelTypeToString(k) << ", " << io::ordinal(j + 1) << " step, "
...
BOOST_AUTO_TEST_CASE(testIsInSubset)
{
    ...
    for (Size i = 0; i < dim; i++)
        set.push_back(i * 1.0);
    for (Size i = 0; i < dim; i++)
        subset.push_back(dim + i * 1.0);
    std::valarray<bool> result = isInSubset(set, subset);
    if (printReport_)
    {
        for (Size i = 0; i < dim; i++)
        {
            BOOST_TEST_MESSAGE(io::ordinal(i + 1) << ":" << " set[" << i << "] =  " << set[i] << ", subset[" << i
                                                  << "] =  " << subset[i] << ", result[" << i << "] =  " << result[i]);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testAbcdDegenerateCases)
{
    ...
    if (std::fabs(cov1 - 1.0) > 1E-14 || std::isnan(cov1) || std::isinf(cov1))
        BOOST_FAIL("(a,b,c,d)=(0,0,0,1): true covariance should be 1.0, " << "error is " << std::fabs(cov1 - 1.0));

    Real cov2 = f2.covariance(0.0, 1.0, 1.0, 1.0);
    if (std::fabs(cov2 - 1.0) > 1E-14 || std::isnan(cov2) || std::isinf(cov2))
        BOOST_FAIL("(a,b,c,d)=(1,0,0,0): true covariance should be 1.0, " << "error is " << std::fabs(cov2 - 1.0));
}
...
BOOST_AUTO_TEST_CASE(testCovariance)
{
    ...
    for (Size i = 1; i <= n; i++)
        rateTimes.push_back(static_cast<Time>(i));
    evolTimes1.push_back(n - 1);
    for (Size i = 1; i <= n - 1; i++)
        evolTimes2.push_back(static_cast<Time>(i));
    for (Size i = 1; i <= 2 * n - 2; i++)
        evolTimes3.push_back(0.5 * i);
    ...
    ext::shared_ptr<PiecewiseConstantCorrelation> corr(new TimeHomogeneousForwardCorrelation(c, rateTimes));
    ...
    for (Size k = 0; k < modelNames.size(); k++)
    {
        for (Size l = 0; l < evolNames.size(); l++)
        {
            ...
            {
                case 0:
                    model = ext::shared_ptr<MarketModel>(new FlatVol(vols, corr, evolution, n - 1, rates, displ));
                    break;
                case 1:
                    model = ext::shared_ptr<MarketModel>(
                        new AbcdVol(1.0, 0.0, 1.0E-50, 0.0, ks, corr, evolution, n - 1, rates, displ));
                    break;
                default:
                    ...
                    for (Size x = 0; x < n - 1; x++)
                    {
                        for (Size y = 0; y < n - 1; y++)
                        {
                            if (std::min(rateTimes[x], rateTimes[y]) >= evolTimes[l][i] &&
                                ...
                                BOOST_FAIL("Model " << modelNames[k] << " with " << evolNames[l]
                                                    << ": covariance matrix in step " << i << ": true value at (" << x
                                                    << "," << y << ") is " << c[x][y] * dt << " actual value is "
                                                    << cov[x][y]);
                        }
                    }
                }
