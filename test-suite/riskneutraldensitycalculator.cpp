...
BOOST_AUTO_TEST_CASE(testDensityAgainstOptionPrices)
{
    ...
    const ext::shared_ptr<BlackScholesMertonProcess> bsmProcess(
        new BlackScholesMertonProcess(spot, qTS, rTS, Handle<BlackVolTermStructure>(flatVol(v, dayCounter))));
    ...
    for (Real t : times)
    {
        ...
        for (Real strike : strikes)
        {
            ...
            const BlackCalculator blackCalc(Option::Put, strike, fwd, stdDev, df);

            const Real tol = 10 * std::sqrt(QL_EPSILON);
            const Real calculatedCDF = bsm.cdf(xs, t);
            const Real expectedCDF = blackCalc.strikeSensitivity() / df;
            ...
            const Real expectedPDF =
                strike / df *
                (BlackCalculator(Option::Put, strike + deltaStrike, fwd, stdDev, df).strikeSensitivity() -
                 BlackCalculator(Option::Put, strike - deltaStrike, fwd, stdDev, df).strikeSensitivity()) /
                (2 * deltaStrike);
...
BOOST_AUTO_TEST_CASE(testBSMagainstHestonRND)
{
    ...
    const BSMRNDCalculator bsm(bsmProcess);
    const HestonRNDCalculator heston(ext::make_shared<HestonProcess>(rTS, qTS, spot, v0, kappa, theta, sigma, rho),
                                     1e-8);
...
class DumasParametricVolSurface : public BlackVolatilityTermStructure
{
  public:
    ...
    : BlackVolatilityTermStructure(0, NullCalendar(), Following, rTS->dayCounter()), b1_(b1), b2_(b2), b3_(b3), b4_(b4),
      b5_(b5), spot_(std::move(spot)), rTS_(rTS), qTS_(std::move(qTS))
    {
    }
...
{
  public:
    ProbWeightedPayoff(Time t, ext::shared_ptr<Payoff> payoff, ext::shared_ptr<RiskNeutralDensityCalculator> calc)
    : t_(t), payoff_(std::move(payoff)), calc_(std::move(calc))
    {
    }

    Real operator()(Real x) const { return calc_->pdf(x, t_) * (*payoff_)(std::exp(x)); }
...
std::vector<Time> adaptiveTimeGrid(Size maxStepsPerYear, Size minStepsPerYear, Real decay, Time endTime)
{
    ...
    while (t < endTime)
    {
        const Time dt = maxDt * std::exp(-decay * t) + minDt * (1.0 - std::exp(-decay * t));
        t += dt;
...
BOOST_AUTO_TEST_CASE(testLocalVolatilityRND)
{
    ...
    const ext::shared_ptr<Quote> spot(ext::make_shared<SimpleQuote>(s0));
    const ext::shared_ptr<YieldTermStructure> rTS(flatRate(todaysDate, r, dayCounter));
    const ext::shared_ptr<YieldTermStructure> qTS(flatRate(todaysDate, q, dayCounter));

    const ext::shared_ptr<TimeGrid> timeGrid(new TimeGrid(1.0, 101));

    const ext::shared_ptr<LocalVolRNDCalculator> constVolCalc(new LocalVolRNDCalculator(
        spot, rTS, qTS, ext::make_shared<LocalConstantVol>(todaysDate, v, dayCounter), timeGrid, 201));

    const Real rTol = 0.01, atol = 0.005;
    for (Time t = 0.1; t < 0.99; t += 0.015)
    {
        const Volatility stdDev = v * std::sqrt(t);
        const Real xm = -0.5 * stdDev * stdDev + std::log(s0 * qTS->discount(t) / rTS->discount(t));
    ...
    if (!(constVolCalc->pdf(xl + 0.0001, tl) > 0.0 && constVolCalc->pdf(xl - 0.0001, tl) == 0.0))
    {
    ...
    const ext::shared_ptr<BlackScholesMertonProcess> bsmProcess(
        new BlackScholesMertonProcess(Handle<Quote>(spot), Handle<YieldTermStructure>(qTS),
                                      Handle<YieldTermStructure>(rTS), Handle<BlackVolTermStructure>(dumasVolSurface)));

    const ext::shared_ptr<LocalVolTermStructure> localVolSurface = ext::make_shared<NoExceptLocalVolSurface>(
        Handle<BlackVolTermStructure>(dumasVolSurface), Handle<YieldTermStructure>(rTS),
        Handle<YieldTermStructure>(qTS), Handle<Quote>(spot), b1);

    const std::vector<Time> adaptiveGrid = adaptiveTimeGrid(400, 50, 5.0, 3.0);

    const ext::shared_ptr<TimeGrid> dumasTimeGrid(new TimeGrid(adaptiveGrid.begin(), adaptiveGrid.end()));

    const ext::shared_ptr<LocalVolRNDCalculator> dumasVolCalc(
        new LocalVolRNDCalculator(spot, rTS, qTS, localVolSurface, dumasTimeGrid, 401, 0.1, 1e-8));

    const Real strikes[] = {25, 50, 95, 100, 105, 150, 200, 400};
    const std::vector<Date> maturities = {todaysDate + Period(1, Weeks),   todaysDate + Period(1, Months),
    ...
    {
        const Time expiry = rTS->dayCounter().yearFraction(todaysDate, maturity);

        const ext::shared_ptr<PricingEngine> engine(new FdBlackScholesVanillaEngine(
            bsmProcess, std::max(Size(51), Size(expiry * 101)), 201, 0, FdmSchemeDesc::Douglas(), true, b1));
        ...
        for (Real strike : strikes)
        {
            const ext::shared_ptr<StrikedTypePayoff> payoff(
                new PlainVanillaPayoff((strike > spot->value()) ? Option::Call : Option::Put, strike));

            VanillaOption option(payoff, exercise);
            option.setPricingEngine(engine);
            const Real expected = option.NPV();

            const Time tx = std::max(dumasTimeGrid->at(1), dumasTimeGrid->closestTime(expiry));
            const std::vector<Real> x = dumasVolCalc->mesher(tx)->locations();

            const ProbWeightedPayoff probWeightedPayoff(expiry, payoff, dumasVolCalc);

            const DiscountFactor df = rTS->discount(expiry);
            const Real calculated = GaussLobattoIntegral(10000, 1e-10)(probWeightedPayoff, x.front(), x.back()) * df;
...
BOOST_AUTO_TEST_CASE(testSquareRootProcessRND)
{
    ...
    const SquareRootProcessParams params[] = {{0.17, 1.0, 0.09, 0.5}, {1.0, 0.6, 0.1, 0.75}, {0.005, 0.6, 0.1, 0.05}};

    for (const auto& param : params)
    {
        const SquareRootProcessRNDCalculator rndCalculator(param.v0, param.kappa, param.theta, param.sigma);
        ...
        for (Real v = 1e-5; v < 1.0; v += (v < param.theta) ? 0.005 : 0.1)
        {
            ...
            const Real cdfExpected =
                GaussLobattoIntegral(10000, 0.01 * tol)([&](Real _x) { return rndCalculator.pdf(_x, t); }, 0, v);
        ...
        for (Real q = 1e-5; q < 1.0; q += 0.001)
        {
            ...
            if (std::fabs(statInvCdfCalculated - statInvCdfExpected) > tol)
            {
                BOOST_FAIL("failed to calculate stationary inverse of cdf"
                           ...
                           << statInvCdfCalculated - statInvCdfExpected << "\n   tolerance:  " << tol);
            }
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testBlackScholesWithSkew, *precondition(if_speed(Fast)))
{
    ...
    const Handle<BlackVolTermStructure> hestonSurface(ext::make_shared<HestonBlackVolSurface>(
        Handle<HestonModel>(ext::make_shared<HestonModel>(hestonProcess)), AnalyticHestonEngine::AndersenPiterbarg,
    ...
    const ext::shared_ptr<LocalVolTermStructure> localVol(
        ext::make_shared<NoExceptLocalVolSurface>(hestonSurface, rTS, qTS, spot, std::sqrt(theta)));

    const LocalVolRNDCalculator localVolCalc(spot.currentLink(), rTS.currentLink(), qTS.currentLink(), localVol,
                                             timeGrid, 151, 0.25);

    const HestonRNDCalculator hestonCalc(hestonProcess);

    const GBSMRNDCalculator gbsmCalc(ext::make_shared<BlackScholesMertonProcess>(spot, qTS, rTS, hestonSurface));
    ...
    for (Real strike : strikes)
    {
        ...
        if (std::fabs(expected - calculatedGBSM) > gbsmTol)
        {
            BOOST_FAIL("failed to match Heston and GBSM cdf"
                       ...
                       << "\n   diff:       " << std::fabs(calculatedGBSM - expected)
                       << "\n   tolerance:  " << gbsmTol);
        }
        ...
        if (std::fabs(expected - calculatedLocalVol) > localVolTol)
        {
            BOOST_FAIL("failed to match Heston and local Volatility cdf"
                       ...
                       << "\n   diff:       " << std::fabs(calculatedLocalVol - expected)
    ...
    for (Real strike : strikes)
    {
        ...
        const Real calculatedLocalVol = localVolCalc.pdf(logStrike, maturity) / strike;
    ...
    for (Real quantile : quantiles)
    {
        ...
        const Real calculatedLocalVol = std::exp(localVolCalc.invcdf(quantile, maturity));
...
BOOST_AUTO_TEST_CASE(testMassAtZeroCEVProcessRND)
{
    ...
    const std::pair<Real, Real> params[] = {{0.1, 1.6}, {0.01, 2.0}, {10.0, 0.35}, {50.0, 0.1}};
    ...
    for (const auto& param : params)
    {
        ...
        const ext::shared_ptr<CEVRNDCalculator> calculator = ext::make_shared<CEVRNDCalculator>(f0, alpha, beta);

        const Real ax = 15.0 * std::sqrt(t) * alpha * std::pow(f0, beta);

        const Real calculated =
            GaussLobattoIntegral(1000, 1e-8)([&](Real _x) -> Real { return calculator->pdf(_x, t); },
...
BOOST_AUTO_TEST_CASE(testCEVCDF)
{
    ...
    for (Size i = 1; i < std::size(betas); ++i)
    {
        ...
        for (Real x = 1.3; x < 3.1; x += 0.1)
        {
            ...
            {
                BOOST_FAIL("failed to reproduce the inverse cumulative probability"
                           << "\n   alpha:     " << alpha << "\n   beta:      " << beta << "\n   x:         " << x
