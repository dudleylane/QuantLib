    ...
    switch (evolverType)
    {
        case Pc:
            return ext::shared_ptr<MarketModelEvolver>(
                new LogNormalCotSwapRatePc(marketModel, generatorFactory, numeraires, initialStep));
        default:
                                      ...
                                      const std::string& config)
{
    ...
    {
        Real expectedNPV =
            curveState_lmm.coterminalSwapAnnuity(i, i) * (todaysSwaps[i] - fixedRate) * todaysDiscounts[i];
    ...
    if (maxError > errorThreshold)
    {
        ...
        for (Size i = 0; i < N; ++i)
        {
            BOOST_TEST_MESSAGE(io::ordinal(i + 1)
                               << " coterminal swap NPV: " << io::rate(results[i]) << " +- " << io::rate(errors[i])
    ...
    for (Size i = 0; i < N; ++i)
    {
        Real expectedSwaption = BlackCalculator(displacedPayoff[i], todaysSwaps[i] + displacement,
                                                volatilities[i] * std::sqrt(rateTimes[i]),
                                                curveState_lmm.coterminalSwapAnnuity(i, i) * todaysDiscounts[i])
                                    .value();
            ...
            for (auto& measure : measures)
            {
                ...
                for (Size i = 0; i < std::size(evolvers) - stop; i++)
                {
                    for (Size n = 0; n < 1; n++)
                    {
                        ...
                        checkCoterminalSwapsAndSwaptions(*stats, fixedRate, displacedPayoff, marketModel, config.str());
                    }
                }
            }
        }
