...
class OneDimensionalPolynomialDegreeN : public CostFunction
{
  public:
    ...
    : coefficients_(coefficients), polynomialDegree_(coefficients.size() - 1)
    {
    }
...
class OptimizationBasedCostFunction : public CostFunction
{
  public:
    ...
    Array values(const Array&) const override
    {
        ...
        Problem problem(oneDimensionalPolynomialDegreeN, constraint, initialValues);
...
ext::shared_ptr<OptimizationMethod> makeOptimizationMethod(OptimizationMethodType optimizationMethodType,
                                                           Real simplexLambda,
                                                           ...
                                                           Real levenbergMarquardtGtol)
{
    ...
    {
        case simplex:
            return ext::shared_ptr<OptimizationMethod>(new Simplex(simplexLambda));
        case levenbergMarquardt:
            return ext::shared_ptr<OptimizationMethod>(
                new LevenbergMarquardt(levenbergMarquardtEpsfcn, levenbergMarquardtXtol, levenbergMarquardtGtol));
        case levenbergMarquardt2:
            return ext::shared_ptr<OptimizationMethod>(
                new LevenbergMarquardt(levenbergMarquardtEpsfcn, levenbergMarquardtXtol, levenbergMarquardtGtol, true));
        case conjugateGradient:
...
std::vector<NamedOptimizationMethod>
makeOptimizationMethods(const std::vector<OptimizationMethodType>& optimizationMethodTypes,
                        Real simplexLambda,
                        Real levenbergMarquardtEpsfcn,
                        Real levenbergMarquardtXtol,
                        Real levenbergMarquardtGtol)
{
    ...
    for (auto optimizationMethodType : optimizationMethodTypes)
    {
        ...
        namedOptimizationMethod.optimizationMethod =
            makeOptimizationMethod(optimizationMethodType, simplexLambda, levenbergMarquardtEpsfcn,
...
void setup()
{
    ...
    coefficients[1] = b;
    coefficients[2] = a;
    costFunctions_.push_back(ext::shared_ptr<CostFunction>(new OneDimensionalPolynomialDegreeN(coefficients)));
    ...
    endCriterias_.push_back(ext::make_shared<EndCriteria>(maxIterations_.back(), maxStationaryStateIterations_.back(),
    ...
    optimizationMethods_.push_back(makeOptimizationMethods(optimizationMethodTypes, simplexLambda,
                                                           levenbergMarquardtEpsfcn, levenbergMarquardtXtol,
    ...
    for (Size i = 0; i < costFunctions_.size(); ++i)
    {
        ...
        for (Size j = 0; j < (optimizationMethods_[i]).size(); ++j)
        {
            ...
            for (Size k = 0; k < endCriteriaTests; ++k)
            {
                ...
                EndCriteria endCriteria(endCriterias_[i]->maxIterations(),
                                        endCriterias_[i]->maxStationaryStateIterations(), rootEpsilon,
                                        endCriterias_[i]->functionEpsilon(), endCriterias_[i]->gradientNormEpsilon());
                rootEpsilon *= .1;
                ...
                bool correct = (xError <= endCriteria.rootEpsilon() || yError <= endCriteria.functionEpsilon());

                if ((!completed) || (!correct))
                    BOOST_ERROR("costFunction # = "
...
BOOST_AUTO_TEST_CASE(nestedOptimizationTest)
{
    ...
    Problem problem(optimizationBasedCostFunction, constraint, initialValues);
    LevenbergMarquardt optimizationMethod;
    // Simplex optimizationMethod(0.1);
    // ConjugateGradient optimizationMethod;
    // SteepestDescent optimizationMethod;
...
{
  public:
    ModFourthDeJong() : uniformRng_(MersenneTwisterUniformRng(4711)) {}
    Array values(const Array& x) const override { return Array(x.size(), value(x)); }
    Real value(const Array& x) const override
    {
        Real fx = 0.0;
...
BOOST_AUTO_TEST_CASE(testDifferentialEvolution)
{
    ...
    DifferentialEvolution::Configuration conf = DifferentialEvolution::Configuration()
    ...
    DifferentialEvolution::Configuration conf2 = DifferentialEvolution::Configuration()
    ...
    std::vector<DifferentialEvolution> diffEvolOptimisers = {deOptim, deOptim, deOptim, deOptim, deOptim2};

    std::vector<ext::shared_ptr<CostFunction>> costFunctions = {
        ext::shared_ptr<CostFunction>(new FirstDeJong), ext::shared_ptr<CostFunction>(new SecondDeJong),
        ext::shared_ptr<CostFunction>(new ModThirdDeJong), ext::shared_ptr<CostFunction>(new ModFourthDeJong),
        ext::shared_ptr<CostFunction>(new Griewangk)};

    std::vector<BoundaryConstraint> constraints = {
        {-10.0, 10.0}, {-10.0, 10.0}, {-10.0, 10.0}, {-10.0, 10.0}, {-600.0, 600.0}};

    std::vector<Array> initialValues = {Array(3, 5.0), Array(2, 5.0), Array(5, 5.0), Array(30, 5.0), Array(10, 100.0)};

    std::vector<EndCriteria> endCriteria = {{100, 10, 1e-10, 1e-8, Null<Real>()},
                                            {100, 10, 1e-10, 1e-8, Null<Real>()},
                                            {100, 10, 1e-10, 1e-8, Null<Real>()},
                                            {500, 100, 1e-10, 1e-8, Null<Real>()},
                                            {1000, 800, 1e-12, 1e-10, Null<Real>()}};

    std::vector<Real> minima = {0.0, 0.0, 0.0, 10.9639796558, 0.0};
