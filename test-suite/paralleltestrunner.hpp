      ...
      public:
        ...
        bool visit(test_unit const& tu)
        {
            if (tu.p_parent_id == framework::master_test_suite().p_id)
            {
                ...
                QL_REQUIRE(!tu.p_name.get().compare("QuantLibTests"), "could not find QuantLib test suite");
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (!clientMode)
        {
            ...
            for (TestCaseCollector::id_map_t::const_iterator p_it = tcc.map().begin(); p_it != tcc.map().end(); ++p_it)
            {
            ...
            QL_REQUIRE(ids.size() + qlRoot.size() == tcc.numberOfTests(), "missing test case in distrubtion list");
