...
struct CommonVars
{
    ...
    CommonVars()
    {
        ...
        termStructure.linkTo(flatRate(referenceDate, 0.02, Actual365Fixed()));
        ...
        atmVol = Handle<SwaptionVolatilityStructure>(ext::shared_ptr<SwaptionVolatilityStructure>(
            new SwaptionVolatilityMatrix(calendar, Following, atmOptionTenors, atmSwapTenors, m, Actual365Fixed(),
                                         false, QuantLib::VolatilityType::Normal)));
        ...
        */
        ...
        ext::shared_ptr<SwapIndex> swapIndexBase(new SwapIndex("swapIndexBase", 2 * Years, iborIndex->fixingDays(),
                                                               iborIndex->currency(), iborIndex->fixingCalendar(),
                                                               1 * Years, ModifiedFollowing,
                                                               Thirty360(Thirty360::EurobondBasis), // EUR
                                                               iborIndex, termStructure));
        ext::shared_ptr<SwapIndex> shortSwapIndexBase(
            new SwapIndex("shortSwapIndexBase", 1 * Years, iborIndex->fixingDays(), iborIndex->currency(),
                          iborIndex->fixingCalendar(), 1 * Years, ModifiedFollowing,
                          Thirty360(Thirty360::EurobondBasis), // EUR
                          iborIndex, termStructure));
        ...
        for (Size i = 0; i < nRows; ++i)
        {
            ...
            guess[i][0] = Handle<Quote>(ext::shared_ptr<Quote>(new SimpleQuote(0.01)));
            guess[i][1] = Handle<Quote>(ext::shared_ptr<Quote>(new SimpleQuote(0.0)));
            guess[i][2] = Handle<Quote>(ext::shared_ptr<Quote>(new SimpleQuote(0.3)));
            guess[i][3] = Handle<Quote>(ext::shared_ptr<Quote>(new SimpleQuote(0.5)));
        }
        ...
        for (auto& yieldCurveModel : yieldCurveModels)
        {
            numericalPricers.push_back(
                ext::shared_ptr<CmsCouponPricer>(new NumericHaganPricer(atmVol, yieldCurveModel, zeroMeanRev)));


            analyticPricers.push_back(
                ext::shared_ptr<CmsCouponPricer>(new AnalyticHaganPricer(atmVol, yieldCurveModel, zeroMeanRev)));
        }
    }
};
...
BOOST_AUTO_TEST_CASE(testFairRate)
{
    ...
    ext::shared_ptr<SwapIndex> swapIndex(new SwapIndex("CMS10Y", 10 * Years, vars.iborIndex->fixingDays(),
                                                       vars.iborIndex->currency(), vars.iborIndex->fixingCalendar(),
                                                       1 * Years, ModifiedFollowing,
                                                       Thirty360(Thirty360::EurobondBasis), // EUR
                                                       vars.iborIndex, vars.termStructure));
    // FIXME
    ...
    for (Size j = 0; j < vars.yieldCurveModels.size(); ++j)
    {
        ...
        if (std::round(10.0 * (difference - tol)) / 10.0 >
            0.0) // seems a more appropriate comparison to me instead of  difference > tol
            BOOST_FAIL(
                "\nCoupon payment date: "
...
BOOST_AUTO_TEST_CASE(testCmsSwap)
{
    ...
    for (Size i = 0; i < n; ++i)
        // no cap, floor
        // no gearing, spread
        cms[i] = MakeCms(Period(swapLengths[i], Years), swapIndex, vars.iborIndex, spread);
...
BOOST_AUTO_TEST_CASE(testParity)
{
    ...
    CappedFlooredCmsCoupon cpn_plain(paymentDate, nominal, startDate, endDate, swapIndex->fixingDays(), swapIndex,
                                     gearing, spread, infiniteCap, infiniteFloor, startDate, endDate,
                                     vars.iborIndex->dayCounter());
    for (Rate strike = -0.005; strike <= 0.035; strike += 0.01)
    {
        CappedFlooredCmsCoupon cpn_capped(paymentDate, nominal, startDate, endDate, swapIndex->fixingDays(), swapIndex,
                                          gearing, spread, strike, infiniteFloor, startDate, endDate,
                                          vars.iborIndex->dayCounter());
        CappedFlooredCmsCoupon cpn_floored(paymentDate, nominal, startDate, endDate, swapIndex->fixingDays(), swapIndex,
