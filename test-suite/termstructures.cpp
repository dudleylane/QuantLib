...
struct CommonVars
{
    ...
    CommonVars()
    {
        ...
        Size deposits = std::size(depositData), swaps = std::size(swapData);

        std::vector<ext::shared_ptr<RateHelper>> instruments(deposits + swaps);
        for (Size i = 0; i < deposits; i++)
        {
            instruments[i] = ext::shared_ptr<RateHelper>(
                new DepositRateHelper(depositData[i].rate / 100, depositData[i].n * depositData[i].units,
                                      settlementDays, calendar, ModifiedFollowing, true, Actual360()));
        }
        ext::shared_ptr<IborIndex> index(new IborIndex("dummy", 6 * Months, settlementDays, Currency(), calendar,
                                                       ModifiedFollowing, false, Actual360()));
        for (Size i = 0; i < swaps; ++i)
        {
            instruments[i + deposits] = ext::shared_ptr<RateHelper>(
                new SwapRateHelper(swapData[i].rate / 100, swapData[i].n * swapData[i].units, calendar, Annual,
                                   Unadjusted, Thirty360(Thirty360::BondBasis), index));
        }
        termStructure = ext::shared_ptr<YieldTermStructure>(
            new PiecewiseYieldCurve<Discount, LogLinear>(settlement, instruments, Actual360()));
        dummyTermStructure = ext::shared_ptr<YieldTermStructure>(
            new PiecewiseYieldCurve<Discount, LogLinear>(settlement, instruments, Actual360()));
    }
};
...
BOOST_AUTO_TEST_CASE(testReferenceChange)
{
    ...
    vars.termStructure = ext::shared_ptr<YieldTermStructure>(
        new FlatForward(vars.settlementDays, NullCalendar(), flatRateHandle, Actual360()));
    ...
    for (i = 0; i < std::size(days); i++)
    {
        if (!close(expected[i], calculated[i]))
            BOOST_ERROR("\n  Discount at " << days[i] << " days:\n"
                                           << std::setprecision(12) << "    before date change: " << expected[i] << "\n"
...
BOOST_AUTO_TEST_CASE(testImplied)
{
    ...
    vars.termStructure =
        ext::make_shared<FlatForward>(vars.settlementDays, NullCalendar(), Handle<Quote>(flatRate), Actual360());
    Real tolerance = 1.0e-10;
    Date today = Settings::instance().evaluationDate();
    Date newToday = today + 3 * Years;
    Date newSettlement = vars.calendar.advance(newToday, vars.settlementDays, Days);
    Date testDate = newSettlement + 5 * Years;
    ext::shared_ptr<YieldTermStructure> implied(
        new ImpliedTermStructure(Handle<YieldTermStructure>(vars.termStructure), newSettlement));
    for (int i = 1; i < 4; i++)
    {
        ...
        if (std::fabs(discount - baseDiscount * impliedDiscount) > tolerance)
            BOOST_ERROR("unable to reproduce discount from implied curve\n"
                        << std::fixed << std::setprecision(10) << "    calculated: " << baseDiscount * impliedDiscount
                        << "\n"
                        << "    expected:   " << discount);
    }
}
...
BOOST_AUTO_TEST_CASE(testImpliedObs)
{
    ...
    ext::shared_ptr<YieldTermStructure> implied(new ImpliedTermStructure(h, newSettlement));
    Flag flag;
...
BOOST_AUTO_TEST_CASE(testFSpreaded)
{
    ...
    ext::shared_ptr<YieldTermStructure> spreaded(
        new ForwardSpreadedTermStructure(Handle<YieldTermStructure>(vars.termStructure), mh));
    ...
    Rate forward = vars.termStructure->forwardRate(testDate, testDate, tsdc, Continuous, NoFrequency);
    Rate spreadedForward = spreaded->forwardRate(testDate, testDate, sprdc, Continuous, NoFrequency);
    if (std::fabs(forward - (spreadedForward - me->value())) > tolerance)
        BOOST_ERROR("unable to reproduce forward from spreaded curve\n"
                    << std::setprecision(10) << "    calculated: " << io::rate(spreadedForward - me->value()) << "\n"
...
BOOST_AUTO_TEST_CASE(testFSpreadedObs)
{
    ...
    ext::shared_ptr<YieldTermStructure> spreaded(new ForwardSpreadedTermStructure(h, mh));
    Flag flag;
...
BOOST_AUTO_TEST_CASE(testZSpreaded)
{
    ...
    Handle<Quote> mh(me);
    ext::shared_ptr<YieldTermStructure> spreaded(
        new ZeroSpreadedTermStructure(Handle<YieldTermStructure>(vars.termStructure), mh));
    Date testDate = vars.termStructure->referenceDate() + 5 * Years;
    DayCounter rfdc = vars.termStructure->dayCounter();
    Rate zero = vars.termStructure->zeroRate(testDate, rfdc, Continuous, NoFrequency);
    ...
    if (std::fabs(zero - (spreadedZero - me->value())) > tolerance)
        BOOST_ERROR("unable to reproduce zero yield from spreaded curve\n"
                    << std::setprecision(10) << "    calculated: " << io::rate(spreadedZero - me->value()) << "\n"
...
BOOST_AUTO_TEST_CASE(testZSpreadedObs)
{
    ...
    ext::shared_ptr<YieldTermStructure> spreaded(new ZeroSpreadedTermStructure(h, mh));
    Flag flag;
    flag.registerWith(spreaded);
    h.linkTo(vars.termStructure);
    if (!flag.isUp())
...
{
    BOOST_TEST_MESSAGE("Testing that a zero-spreaded curve can be created with "
    ...
    ext::shared_ptr<YieldTermStructure> spreaded(new ZeroSpreadedTermStructure(underlying, spread));
...
BOOST_AUTO_TEST_CASE(testLinearInterpolationSpreadedForwardRate)
{
    ...
    std::vector<Date> spreadDates = {vars.calendar.advance(today, 100, Days), vars.calendar.advance(today, 150, Days)};
    ...
    ext::shared_ptr<YieldTermStructure> spreadedTermStructure =
        ext::make_shared<InterpolatedPiecewiseForwardSpreadedTermStructure<Linear>>(
            Handle<YieldTermStructure>(vars.termStructure), spreads, spreadDates);
    ...
    Rate expectedForwardRate =
        vars.termStructure->forwardRate(t, t, Continuous, NoFrequency, true) + (m * (time2 - time0) + 0.02);
    ...
    if (std::fabs(interpolatedForwardRate - expectedForwardRate) > tolerance)
        BOOST_ERROR("unable to reproduce interpolated forward rate\n"
                    << std::setprecision(10) << "    calculated: " << io::rate(interpolatedForwardRate) << "\n"
...
BOOST_AUTO_TEST_CASE(testForwardFlatInterpolationSpreadedForwardRate)
{
    ...
    std::vector<Date> spreadDates = {vars.calendar.advance(today, 75, Days), vars.calendar.advance(today, 260, Days)};
    ...
    Rate expectedForwardRate = vars.termStructure->forwardRate(t, t, Continuous, NoFrequency, true) + spread1->value();
...
BOOST_AUTO_TEST_CASE(testBackwardFlatInterpolationSpreadedForwardRate)
{
    ...
    std::vector<Handle<Quote>> spreads = {Handle<Quote>(spread1), Handle<Quote>(spread2), Handle<Quote>(spread3)};

    // Define the spread dates
    std::vector<Date> spreadDates = {vars.calendar.advance(today, 100, Days), vars.calendar.advance(today, 200, Days),
    ...
    Time t = dc.yearFraction(today, interpolationDate);
    Rate expectedForwardRate = vars.termStructure->forwardRate(t, t, Continuous, NoFrequency, true) + spread2->value();
...
BOOST_AUTO_TEST_CASE(testBackwardFlatInterpolationZeroRate)
{
    ...
    BackwardFlatInterpolation bckFlatInterpolation(times.begin(), times.end(), spreadValues.begin());
    ...
    if (std::fabs(interpolatedZeroRate - expectedZeroRate) > tolerance)
        BOOST_ERROR("unable to reproduce interpolated zero rate\n"
...
{
    BOOST_TEST_MESSAGE("Testing that an underlying curve can be relinked to "
                       "a null underlying curve...");
...
{
    BOOST_TEST_MESSAGE("Testing composite zero yield structures...");

    Settings::instance().evaluationDate() = Date(10, Nov, 2017);

    // First curve
    std::vector<Date> dates = {Date(10, Nov, 2017), Date(13, Nov, 2017), Date(12, Feb, 2018), Date(10, May, 2018),
                               Date(10, Aug, 2018), Date(12, Nov, 2018), Date(21, Dec, 2018), Date(15, Jan, 2020),
                               Date(31, Mar, 2021), Date(28, Feb, 2023), Date(21, Dec, 2026), Date(31, Jan, 2030),
                               Date(28, Feb, 2031), Date(31, Mar, 2036), Date(28, Feb, 2041), Date(28, Feb, 2048),
                               Date(31, Dec, 2141)};

    std::vector<Rate> rates = {0.0655823213132524, 0.0655823213132524, 0.0699455024156877, 0.0799107139233497,
                               0.0813931951022577, 0.0841615820666691, 0.0501297919004145, 0.0823483583439658,
                               0.0860720030924466, 0.0922887604375688, 0.10588902278996,   0.117021968693922,
                               0.109824660896137,  0.109231572878364,  0.119218123236241,  0.128647300167664,
                               0.0506086995288751};

    ext::shared_ptr<YieldTermStructure> termStructure1 =
        ext::shared_ptr<YieldTermStructure>(new ForwardCurve(dates, rates, Actual365Fixed(), NullCalendar()));

    // Second curve
    dates = {Date(10, Nov, 2017), Date(13, Nov, 2017), Date(11, Dec, 2017), Date(12, Feb, 2018), Date(10, May, 2018),
             Date(31, Jan, 2022), Date(7, Dec, 2023),  Date(31, Jan, 2025), Date(31, Mar, 2028), Date(7, Dec, 2033),
             Date(1, Feb, 2038),  Date(2, Apr, 2046),  Date(2, Jan, 2051),  Date(31, Dec, 2141)};

    rates = {0.056656806197189,  0.056656806197189,  0.0419541633454473, 0.0286681050019797, 0.0148840226959593,
             0.0246680238374363, 0.0255349067810599, 0.0298907184711927, 0.0263943927922053, 0.0291924526539802,
             0.0270049276163556, 0.028775807327614,  0.0293567711641792, 0.010518655099659};

    ext::shared_ptr<YieldTermStructure> termStructure2 =
        ext::shared_ptr<YieldTermStructure>(new ForwardCurve(dates, rates, Actual365Fixed(), NullCalendar()));

    typedef Real (*binary_f)(Real, Real);

    ext::shared_ptr<YieldTermStructure> compoundCurve =
        ext::shared_ptr<YieldTermStructure>(new CompositeZeroYieldStructure<binary_f>(
            Handle<YieldTermStructure>(termStructure1), Handle<YieldTermStructure>(termStructure2), sub));
