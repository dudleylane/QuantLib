            ...
            const Handle<YieldTermStructure>& discountCurve)
{
    ...
    for (Datum datum : iiData)
    {
        ...
        auto anInstrument = ext::make_shared<YearOnYearInflationSwapHelper>(quote, observationLag, maturity, calendar,
                                                                            bdc, dc, ii, interpolation, discountCurve);
...
struct CommonVars
{
    ...
    CommonVars() : nominals(1, 1000000)
    {
        ...
        Real fixData[] = {189.9, 189.9, 189.6, 190.5, 191.6, 192.0, 192.2, 192.2, 192.6, 193.1,  193.3,
                          ...
                          200.4, 201.1, 202.7, 201.6, 203.1, 204.4, 205.4, 206.2, 207.3, -999.0, -999};
        ...
        std::vector<Datum> yyData = {{Date(13, August, 2008), 2.95},  {Date(13, August, 2009), 2.95},
                                     {Date(13, August, 2010), 2.93},  {Date(15, August, 2011), 2.955},
                                     {Date(13, August, 2012), 2.945}, {Date(13, August, 2013), 2.985},
                                     {Date(13, August, 2014), 3.01},  {Date(13, August, 2015), 3.035},
                                     ...
                                     {Date(13, August, 2027), 3.155}, {Date(13, August, 2032), 3.145},
                                     {Date(13, August, 2037), 3.145}};

        // now build the helpers ...
        std::vector<ext::shared_ptr<BootstrapHelper<YoYInflationTermStructure>>> helpers = makeHelpers(
            yyData, iir, observationLag, CPI::Flat, calendar, convention, dc, Handle<YieldTermStructure>(nominalTS));
    ...
    // utilities
    Leg makeYoYLeg(const Date& startDate, Integer length, const Rate gearing = 1.0, const Rate spread = 0.0) const
    {
        ext::shared_ptr<YoYInflationIndex> ii = ext::dynamic_pointer_cast<YoYInflationIndex>(iir);
        Date endDate = calendar.advance(startDate, length * Years, Unadjusted);
        Schedule schedule(startDate, endDate, Period(frequency), calendar, Unadjusted,
    ...
    Leg makeFixedLeg(const Date& startDate, Integer length) const
    {
        ...
        return FixedRateLeg(schedule).withNotionals(nominals).withCouponRates(coupons, dc);
    }
                             ...
                             const Rate spread = 0.0) const
    {
        ...
        Handle<YoYOptionletVolatilitySurface> vol(ext::make_shared<ConstantYoYOptionletVolatility>(
            volatility, settlementDays, calendar, convention, dc, observationLag, frequency, false));

        ext::shared_ptr<YoYInflationCouponPricer> pricer;
        switch (which)
        {
            case 0:
                pricer = ext::shared_ptr<YoYInflationCouponPricer>(new BlackYoYInflationCouponPricer(vol, nominalTS));
                break;
            case 1:
            ...
            case 2:
                pricer =
                    ext::shared_ptr<YoYInflationCouponPricer>(new BachelierYoYInflationCouponPricer(vol, nominalTS));
                break;
            default:
                BOOST_FAIL("unknown coupon pricer request: which = " << which << "should be 0=Black,1=DD,2=Bachelier");
                break;
        }
    ...
    ext::shared_ptr<PricingEngine> makeEngine(Volatility volatility, Size which) const
    {
        ...
        ext::shared_ptr<YoYInflationIndex> yyii = ext::dynamic_pointer_cast<YoYInflationIndex>(iir);
        ...
        {
            case 0:
                return ext::shared_ptr<PricingEngine>(new YoYInflationBlackCapFloorEngine(iir, vol, nominalTS));
                break;
            case 1:
                ...
                break;
            case 2:
                return ext::shared_ptr<PricingEngine>(new YoYInflationBachelierCapFloorEngine(iir, vol, nominalTS));
                break;
            default:
                BOOST_FAIL("unknown engine request: which = " << which << "should be 0=Black,1=DD,2=Bachelier");
                break;
        }
        // make compiler happy
    ...
    ext::shared_ptr<YoYInflationCapFloor> makeYoYCapFloor(
        YoYInflationCapFloor::Type type, const Leg& leg, Rate strike, Volatility volatility, Size which) const
    {
        ext::shared_ptr<YoYInflationCapFloor> result;
        switch (type)
        {
            case YoYInflationCapFloor::Cap:
                result = ext::shared_ptr<YoYInflationCapFloor>(new YoYInflationCap(leg, std::vector<Rate>(1, strike)));
                break;
            case YoYInflationCapFloor::Floor:
                result =
                    ext::shared_ptr<YoYInflationCapFloor>(new YoYInflationFloor(leg, std::vector<Rate>(1, strike)));
                break;
            default:
...
BOOST_AUTO_TEST_CASE(testDecomposition)
{
    ...
    ext::shared_ptr<PricingEngine> engine(new DiscountingSwapEngine(vars.nominalTS));
    ...
    Leg cappedLeg = vars.makeYoYCapFlooredLeg(whichPricer, vars.startDate, vars.length, caps, floors0, vars.volatility);
    ...
    if (error > tolerance)
    {
        BOOST_ERROR("\nYoY Capped Leg: gearing=1, spread=0%, strike="
                    << capstrike * 100 << "%\n"
    ...
    Leg flooredLeg =
        vars.makeYoYCapFlooredLeg(whichPricer, vars.startDate, vars.length, caps0, floors, vars.volatility);
    ...
    if (error > tolerance)
    {
        BOOST_ERROR("YoY Floored Leg: gearing=1, spread=0%, strike="
                    << floorstrike * 100 << "%\n"
    ...
    Leg collaredLeg =
        vars.makeYoYCapFlooredLeg(whichPricer, vars.startDate, vars.length, caps, floors, vars.volatility);
    Swap collarLeg(fixedLeg, collaredLeg);
    collarLeg.setPricingEngine(engine);
    YoYInflationCollar collar(floatLeg, std::vector<Rate>(1, capstrike), std::vector<Rate>(1, floorstrike));
    ...
    if (error > tolerance)
    {
        BOOST_ERROR("\nYoY Capped Leg: gearing=" << gearing_p << ", " << "spread= " << spread_p * 100
    ...
    YoYInflationCollar collar_p(floatLeg_p, std::vector<Rate>(1, capstrike), std::vector<Rate>(1, floorstrike));
    ...
    YoYInflationCollar collar_n(floatLeg, std::vector<Rate>(1, (floorstrike - spread_n) / gearing_n),
            ...
            for (Real& strike : strikes)
            {
                for (Real vol : vols)
                {
                    ...
                    ext::shared_ptr<Instrument> cap =
                        vars.makeYoYCapFloor(YoYInflationCapFloor::Cap, leg, strike, vol, whichPricer);

                    ext::shared_ptr<Instrument> floor =
                        vars.makeYoYCapFloor(YoYInflationCapFloor::Floor, leg, strike, vol, whichPricer);
                    ...
                    YearOnYearInflationSwap swap(Swap::Payer, 1000000.0,
                                                 ...
                                                 0.0,         // strikes[j],
                                                 vars.dc, yoySchedule, vars.iir, vars.observationLag, CPI::Flat,
                                                 0.0, // spread on index
                    ...
                    Leg leg2 =
                        vars.makeYoYCapFlooredLeg(whichPricer, from, length, std::vector<Rate>(length, strike), // cap
