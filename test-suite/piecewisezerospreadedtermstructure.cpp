...
BOOST_AUTO_TEST_CASE(testFlatInterpolationLeft)
{
    ...
    ext::shared_ptr<ZeroYieldStructure> spreadedTermStructure = ext::make_shared<PiecewiseZeroSpreadedTermStructure>(
    ...
    Real tolerance = 1e-9;
    Real expectedRate = vars.termStructure->zeroRate(t, vars.compounding) + spread1->value();
...
BOOST_AUTO_TEST_CASE(testFlatInterpolationRight)
{
    ...
    Real tolerance = 1e-9;
    Real expectedRate = vars.termStructure->zeroRate(t, vars.compounding) + spread2->value();
...
BOOST_AUTO_TEST_CASE(testLinearInterpolationMultipleSpreads)
{
    ...
    std::vector<Handle<Quote>> spreads = {
        Handle<Quote>(spread1),
        Handle<Quote>(spread2),
        Handle<Quote>(spread3),
        Handle<Quote>(spread4),
    };

    std::vector<Date> spreadDates = {
        vars.calendar.advance(vars.today, 90, Days), vars.calendar.advance(vars.today, 150, Days),
        vars.calendar.advance(vars.today, 30, Months), vars.calendar.advance(vars.today, 40, Months)};
...
BOOST_AUTO_TEST_CASE(testSetInterpolationFactory)
{
    ...
    spreadedTermStructure = ext::make_shared<InterpolatedPiecewiseZeroSpreadedTermStructure<Cubic>>(
        Handle<YieldTermStructure>(vars.termStructure), spreads, spreadDates, vars.compounding, freq,
    ...
    Real tolerance = 1e-9;
    Real expectedRate = vars.termStructure->zeroRate(t, vars.compounding) + Real(0.026065770863);
...
BOOST_AUTO_TEST_CASE(testMaxDate)
{
    ...
    Date expectedDate = std::min(vars.termStructure->maxDate(), spreadDates.back());
...
BOOST_AUTO_TEST_CASE(testQuoteChanging)
{
    ...
    Rate interpolatedZeroRate = spreadedTermStructure->zeroRate(t, vars.compounding);
    Real tolerance = 1e-9;
    Real expectedRate = vars.termStructure->zeroRate(t, vars.compounding) + Real(0.03);
    ...
    expectedRate = vars.termStructure->zeroRate(t, vars.compounding) + Real(0.025);
