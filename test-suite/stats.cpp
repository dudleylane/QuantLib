...
void checkSequence(const std::string& name, Size dimension)
{
    ...
    for (i = 0; i < dimension; i++)
    {
        if (calculated[i] != expected)
            BOOST_FAIL("SequenceStatistics<" << name << ">: " << io::ordinal(i + 1) << " dimension: "
                                             << "wrong minimum value\n"
...
BOOST_AUTO_TEST_CASE(testStatistics)
{
    ...
    check<IncrementalStatistics>(std::string("IncrementalStatistics"));
...
BOOST_AUTO_TEST_CASE(testSequenceStatistics)
{
    ...
    checkSequence<IncrementalStatistics>(std::string("IncrementalStatistics"), 5);
...
BOOST_AUTO_TEST_CASE(testConvergenceStatistics)
{
    ...
    checkConvergence<IncrementalStatistics>(std::string("IncrementalStatistics"));
    checkConvergence<Statistics>(std::string("Statistics"));
}

#define TEST_INC_STAT(expr, expected)                                                 \
    if (!close_enough(expr, expected))                                                \
        BOOST_ERROR(std::setprecision(16) << std::scientific << #expr << " (" << expr \
                                          << ") can not be reproduced against cached result (" << expected << ")");
...
BOOST_AUTO_TEST_CASE(testIncrementalStatistics)
{
    ...
    if (stat.samples() != 500000)
        BOOST_ERROR("stat.samples()  (" << stat.samples() << ") can not be reproduced against cached result (" << 500000
                                        << ")");
    ...
    if (std::fabs(stat2.variance() - 1E-2) > tol)
        BOOST_ERROR("variance (" << stat2.variance() << ") out of expected range " << 1E-2 << " +- " << tol);
}
