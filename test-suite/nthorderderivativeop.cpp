...
BOOST_AUTO_TEST_CASE(testFirstOrder2PointsApply)
{
    ...
    const NthOrderDerivativeOp op(0, 1, 3,
                                  ext::make_shared<FdmMesherComposite>(ext::make_shared<Uniform1dMesher>(0.0, 1.0, 6)));
...
BOOST_AUTO_TEST_CASE(testFirstOrder5PointsOnUniformGrid)
{
    ...
    const SparseMatrix m =
        NthOrderDerivativeOp(0, 1, 5,
                             ext::make_shared<FdmMesherComposite>(ext::make_shared<Uniform1dMesher>(0.0, 2.0, 6)))
            .toMatrix();
...
BOOST_AUTO_TEST_CASE(testFirstOrder2PointsOnUniformGrid)
{
    ...
    const SparseMatrix m =
        NthOrderDerivativeOp(0, 1, 2,
                             ext::make_shared<FdmMesherComposite>(ext::make_shared<Uniform1dMesher>(0.0, 0.6, 4)))
            .toMatrix();
...
BOOST_AUTO_TEST_CASE(testFirstOrder2PointsOn2DimUniformGrid)
{
    ...
    const ext::shared_ptr<FdmMesher> mesher = ext::make_shared<FdmMesherComposite>(
        ext::make_shared<Uniform1dMesher>(0.0, 1, xGrid), ext::make_shared<Uniform1dMesher>(0.0, 0.4, 3));
...
BOOST_AUTO_TEST_CASE(testSecondOrder3PointsNonUniformGrid)
{
    ...
    const SparseMatrix m =
        NthOrderDerivativeOp(0, 2, 3,
                             ext::make_shared<FdmMesherComposite>(ext::make_shared<Predefined1dMesher>(xValues)))
            .toMatrix();
...
class FdmHestonNthOrderOp : public FdmLinearOpComposite
{
  public:
      ...
      preconditioner_(SecondDerivativeOp(direction, mesher).mult(Array(mesher->layout()->size(), vol2_)))
    {
        ...
        map_ = sparse_prod<SparseMatrix>(-rV, dx) + sparse_prod<SparseMatrix>(v, dxx) +
               (0.5 * rho * sigma) *
                   sparse_prod<SparseMatrix>(u, sparse_prod<SparseMatrix>(dx, dv) + sparse_prod<SparseMatrix>(dv, dx)) +
               (0.5 * sigma * sigma) * sparse_prod<SparseMatrix>(u, dvv) +
               kappa * sparse_prod<SparseMatrix>(theta * identity_matrix<Real>(n) - u, dv);
    }

    SparseMatrix toMatrix() const override { return map_; }

    Size size() const override { return 2; }
    void setTime(Time t1, Time t2) override {}

    Array apply(const Array& r) const override { return prod(map_, r); }

    Array apply_mixed(const Array& r) const override { QL_FAIL("operator splitting is not supported"); }
    ...
    Array preconditioner(const Array& r, Real dt) const override { return preconditioner_.solve_splitting(r, dt, 1.0); }

  private:
    Array solve_apply(const Array& r, Real dt) const { return r - dt * apply(r); }
...
{
  public:
    AvgPayoffFct(ext::shared_ptr<PlainVanillaPayoff> payoff, Volatility vol, Time T, Real growthFactor)
    : payoff_(std::move(payoff)), vol2_(0.5 * vol * vol * T), growthFactor_(growthFactor)
    {
    }

    Real operator()(Real x) const { return (*payoff_)(std::exp(x - vol2_) * growthFactor_); }
...
Array priceReport(const GridSetup& setup, const Array& strikes)
{
    ...
    const ext::shared_ptr<YieldTermStructure> rTS = flatRate(today, 0.05, dc);
    const ext::shared_ptr<YieldTermStructure> qTS = flatRate(today, 0.0, dc);
    ...
    const ext::shared_ptr<HestonProcess> hestonProcess =
        ext::make_shared<HestonProcess>(Handle<YieldTermStructure>(rTS), Handle<YieldTermStructure>(qTS),
                                        Handle<Quote>(ext::make_shared<SimpleQuote>(S)), v0, kappa, theta, sig, rho);

    const Real stdDev = vol * std::sqrt(T);
    const DiscountFactor df = qTS->discount(maturity) / rTS->discount(maturity);
    ...
    for (Size k = 0; k < strikes.size(); ++k)
    {
        ...
        const ext::shared_ptr<Fdm1dMesher> mesher1d = ext::make_shared<Concentrating1dMesher>(
            ymin, ymax, yGrid, std::pair<Real, Real>(specialPoint, setup.density));
        ...
        const ext::shared_ptr<FdmMesherComposite> mesher =
            ext::make_shared<FdmMesherComposite>(ext::make_shared<Predefined1dMesher>(loc),
                                                 ext::make_shared<FdmHestonVarianceMesher>(vGrid, hestonProcess, 1.0));
        ...
        const ext::shared_ptr<PlainVanillaPayoff> payoff = ext::make_shared<PlainVanillaPayoff>(Option::Put, strike);
        ...
        for (const auto& iter : *mesher->layout())
        {
            ...
            if (nx != 0 && nx != yGrid - 1 && setup.cellAvg &&
                ((sT[idx] < strike && sT[idxp1] >= strike) || (sT[idxm1] < strike && sT[idx] >= strike)))
            {
        ...
        const ext::shared_ptr<FdmHestonNthOrderOp> heatEqn =
            ext::make_shared<FdmHestonNthOrderOp>(setup.nPoints, hestonProcess, mesher);

        FdmBackwardSolver solver(heatEqn, FdmBoundaryConditionSet(), ext::shared_ptr<FdmStepConditionComposite>(),
                                 setup.scheme);
        ...
        const std::vector<Real>& x = mesher->getFdm1dMeshers()[0]->locations();
        const std::vector<Real>& v = mesher->getFdm1dMeshers()[1]->locations();
        ...
        const ext::shared_ptr<BicubicSpline> interpolation =
            ext::make_shared<BicubicSpline>(x.begin(), x.end(), v.begin(), v.end(), resultValues_);
        ...
        option.setPricingEngine(
            ext::make_shared<AnalyticHestonEngine>(ext::make_shared<HestonModel>(hestonProcess), 192));
...
{
  public:
    FdmMispricingCostFunction(const GridSetup& setup, Array strikes) : setup_(setup), strikes_(std::move(strikes)) {}

    Array values(const Array& x) const override
    {
        const GridSetup g = {x[0],         x[1],         setup_.cellAvg, setup_.midPoint, setup_.nPoints,
                             setup_.tGrid, setup_.yGrid, setup_.vGrid,   setup_.scheme};
...
BOOST_AUTO_TEST_CASE(testHigherOrderHestonOptionPricing, *precondition(if_speed(Fast)))
{
    ...
    const GridSetup initSetup = {3.87773, 0.043847, true, false, 5, 21, 20, 11, FdmSchemeDesc::CrankNicolson()};
    ...
    LevenbergMarquardt().minimize(prob, EndCriteria(400, 40, 1.0e-4, 1.0e-4, 1.0e-4));

    const GridSetup optimalSetup = {prob.currentValue()[0], prob.currentValue()[1], initSetup.cellAvg,
                                    initSetup.midPoint,     initSetup.nPoints,      initSetup.tGrid,
                                    initSetup.yGrid / 2,    initSetup.vGrid,        initSetup.scheme};
...
Real priceQuality(Real h)
{
    ...
    const GridSetup setup = {5.50966, 0.0130581, true, false, 5, 401, yGrid, 21, FdmSchemeDesc::CrankNicolson()};
...
{
    BOOST_TEST_MESSAGE("Testing Heston option pricing convergence with "
                       "higher order FDM operators and Richardson Extrapolation...");

    const Real n1 = priceQuality(1.0 / 25);
    const Real n3 = std::fabs(RichardsonExtrapolation(priceQuality, 1.0 / 25, 4.0)(2.0));
...
{
    BOOST_TEST_MESSAGE("Testing with FirstDerivativeOp on a non-uniform grid...");

    Array xValues = Exp(Array(7, 0, 0.1));

    const ext::shared_ptr<Fdm1dMesher> m =
        ext::make_shared<Predefined1dMesher>(std::vector<Real>(xValues.begin(), xValues.end()));

    const ext::shared_ptr<FdmMesher> m1d(ext::make_shared<FdmMesherComposite>(m));

    const ext::shared_ptr<FirstDerivativeOp> fx = ext::make_shared<FirstDerivativeOp>(0, m1d);

    const ext::shared_ptr<NthOrderDerivativeOp> dx = ext::make_shared<NthOrderDerivativeOp>(0, 1, 3, m1d);
...
{
    BOOST_TEST_MESSAGE("Testing with FirstDerivativeOp on a 2d uniform grid...");

    const ext::shared_ptr<Fdm1dMesher> m1 = ext::make_shared<Uniform1dMesher>(0.0, 0.6, 5);
    const ext::shared_ptr<Fdm1dMesher> m2 = ext::make_shared<Uniform1dMesher>(0.0, 1.6, 6);

    const ext::shared_ptr<FdmMesher> mc(ext::make_shared<FdmMesherComposite>(m1, m2));
...
{
    BOOST_TEST_MESSAGE("Testing nine points mixed second order "
                       "derivative operator on a uniform grid...");

    const ext::shared_ptr<Fdm1dMesher> m = ext::make_shared<Uniform1dMesher>(0.0, 0.6, 5);

    const ext::shared_ptr<FdmMesher> mc(ext::make_shared<FdmMesherComposite>(m, m));

    const SparseMatrix cc =
        prod(NthOrderDerivativeOp(0, 1, 3, mc).toMatrix(), NthOrderDerivativeOp(1, 1, 3, mc).toMatrix());
