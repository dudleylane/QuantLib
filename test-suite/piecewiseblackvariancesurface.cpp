...
{
    BOOST_TEST_MESSAGE("Testing exact repricing at input tenors...");
    ...
    std::vector<ext::shared_ptr<SmileSection>> sections = {ext::make_shared<FlatSmileSection>(d1, vol1, dc, today),
    ...
    for (Size i = 0; i < dates.size(); ++i)
    {
        ...
        if (diff > tol)
            BOOST_FAIL("failed to reprice at tenor " << i << "\n    date:       " << dates[i]
                                                     << "\n    calculated: " << std::setprecision(16) << std::scientific
                                                     << calculated << "\n    expected:   " << expected
                                                     << "\n    difference: " << diff << "\n    tolerance:  " << tol);
    }
}
...
{
    BOOST_TEST_MESSAGE("Testing linear variance interpolation between tenors...");
    ...
    if (diff > tol)
        BOOST_FAIL("failed to interpolate at midpoint"
                   << "\n    date:       " << dMid << "\n    time:       " << tMidActual << "\n    calculated: "
                   << std::setprecision(16) << std::scientific << calculated << "\n    expected:   " << expectedVar
                   << "\n    difference: " << diff << "\n    tolerance:  " << tol);
...
{
    BOOST_TEST_MESSAGE("Testing blackVol derivation from blackVariance...");
...
{
    BOOST_TEST_MESSAGE("Testing flat-vol extrapolation beyond last tenor...");
    ...
    if (diff > tol)
        BOOST_FAIL("flat-vol extrapolation: vol not constant"
                   ...
                   << std::scientific << calculatedVol << "\n    expected:   " << expectedVol
                   << "\n    difference: " << diff << "\n    tolerance:  " << tol);
}

BOOST_AUTO_TEST_CASE(testObserver)
{
    BOOST_TEST_MESSAGE("Testing observer notification from SmileSections...");
    ...
    std::vector<ext::shared_ptr<SimpleQuote>> quotes = {ext::make_shared<SimpleQuote>(vol * sqrtT),
                                                        ext::make_shared<SimpleQuote>(vol * sqrtT),
                                                        ext::make_shared<SimpleQuote>(vol * sqrtT)};
    std::vector<Handle<Quote>> handles = {Handle<Quote>(quotes[0]), Handle<Quote>(quotes[1]), Handle<Quote>(quotes[2])};

    auto section = ext::make_shared<InterpolatedSmileSection<Linear>>(
        t1, strikes, handles, Handle<Quote>(ext::make_shared<SimpleQuote>(100.0)), Linear(), dc);

    std::vector<Date> dates = {d1};
    std::vector<ext::shared_ptr<SmileSection>> sections = {section};

    auto surface = ext::make_shared<PiecewiseBlackVarianceSurface>(today, dates, sections, dc);
...
{
    BOOST_TEST_MESSAGE("Testing strike-dependent smile interpolation...");
    ...
    Real sqrtT = std::sqrt(t1);
    std::vector<Real> stdDevs = {vol80 * sqrtT, vol100 * sqrtT, vol120 * sqrtT};

    auto section = ext::make_shared<InterpolatedSmileSection<Linear>>(d1, strikes, stdDevs, 100.0, dc, Linear(), today);
    ...
    for (Real strike : strikes)
    {
        ...
        if (diff > tol)
            BOOST_FAIL("strike-dependent repricing failed at strike "
    ...
    if (var80 <= var120)
        BOOST_FAIL("expected higher variance at lower strike (skew)" << "\n    var(80):  " << std::setprecision(16)
                                                                     << std::scientific << var80
                                                                     << "\n    var(120): " << var120);
}

BOOST_AUTO_TEST_CASE(testMultiTenorSmileInterpolation)
{
    BOOST_TEST_MESSAGE("Testing interpolation between tenors with different smiles...");
    ...
    std::vector<Real> stdDevs1 = {0.25 * sqrtT1, 0.20 * sqrtT1, 0.18 * sqrtT1};
    auto section1 =
        ext::make_shared<InterpolatedSmileSection<Linear>>(d1, strikes, stdDevs1, 100.0, dc, Linear(), today);

    // tenor 2: steeper skew, higher overall level
    Real sqrtT2 = std::sqrt(t2);
    std::vector<Real> stdDevs2 = {0.35 * sqrtT2, 0.25 * sqrtT2, 0.20 * sqrtT2};
    auto section2 =
        ext::make_shared<InterpolatedSmileSection<Linear>>(d2, strikes, stdDevs2, 100.0, dc, Linear(), today);
    ...
    for (Real strike : strikes)
    {
        ...
        if (diff > tol)
            BOOST_FAIL("multi-tenor smile interpolation failed at strike "
                       << strike << "\n    time:       " << tMid << "\n    calculated: " << std::setprecision(16)
    ...
    for (Real strike : strikes)
    {
        ...
        if (var_d1 > var_dMid + tol)
            BOOST_FAIL("calendar arbitrage: variance decreased from d1 to dMid"
                       << " at strike " << strike << "\n    var(d1):   " << std::setprecision(16) << std::scientific
                       << var_d1 << "\n    var(dMid): " << var_dMid);

        if (var_dMid > var_d2 + tol)
            BOOST_FAIL("calendar arbitrage: variance decreased from dMid to d2"
                       << " at strike " << strike << "\n    var(dMid): " << std::setprecision(16) << std::scientific
                       << var_dMid << "\n    var(d2):   " << var_d2);
    }

    // butterfly arbitrage check: d^2w/dK^2 >= 0 at the interpolated tenor
    Real dK = 1.0;
    std::vector<Real> butterflyStrikes = {85.0, 90.0, 95.0, 100.0, 105.0, 110.0, 115.0};
    ...
    for (Real K : butterflyStrikes)
    {
        ...
        if (d2wdK2 < -1.0e-10)
            BOOST_FAIL("butterfly arbitrage: d^2w/dK^2 < 0 at midpoint"
                       << " strike " << K << "\n    w(K-dK): " << std::setprecision(16) << std::scientific << w_m
                       << "\n    w(K):    " << w << "\n    w(K+dK): " << w_p << "\n    d2w/dK2: " << d2wdK2);
    }
}
...
{
    BOOST_TEST_MESSAGE("Testing makeFromGrid factory method...");
    ...
    blackVols[0][0] = 0.25;
    blackVols[0][1] = 0.30; // K=80
    blackVols[1][0] = 0.20;
    blackVols[1][1] = 0.25; // K=100
    blackVols[2][0] = 0.18;
    blackVols[2][1] = 0.20; // K=120

    std::vector<Date> dates = {d1, d2};

    auto surface = PiecewiseBlackVarianceSurface::makeFromGrid(today, dates, strikes, blackVols, dc);
...
{
    BOOST_TEST_MESSAGE("Testing constructor validation checks...");
    ...
    BOOST_CHECK_EXCEPTION(
        PiecewiseBlackVarianceSurface(today, std::vector<Date>{}, std::vector<ext::shared_ptr<SmileSection>>{}, dc),
        Error, ExpectedErrorMessage("at least one date"));

    // mismatched sizes (2 dates, 1 section)
    BOOST_CHECK_EXCEPTION(PiecewiseBlackVarianceSurface(today, {d1, d2}, {section1}, dc), Error,
                          ExpectedErrorMessage("mismatch"));

    // first date on reference date
    BOOST_CHECK_EXCEPTION(
        PiecewiseBlackVarianceSurface(today, {today}, {ext::make_shared<FlatSmileSection>(today, vol, dc, today)}, dc),
        Error, ExpectedErrorMessage("must be after reference"));

    // unsorted dates
    BOOST_CHECK_EXCEPTION(PiecewiseBlackVarianceSurface(today, {d2, d1}, {section2, section1}, dc), Error,
                          ExpectedErrorMessage("sorted and unique"));

    // duplicate dates
    BOOST_CHECK_EXCEPTION(PiecewiseBlackVarianceSurface(today, {d1, d1}, {section1, section1}, dc), Error,
                          ExpectedErrorMessage("sorted and unique"));

    // null smile section
    BOOST_CHECK_EXCEPTION(PiecewiseBlackVarianceSurface(today, {d1}, {ext::shared_ptr<SmileSection>()}, dc), Error,
                          ExpectedErrorMessage("null smile section"));
}

BOOST_AUTO_TEST_CASE(testMakeFromGridValidation)
{
    BOOST_TEST_MESSAGE("Testing makeFromGrid validation checks...");
    ...
    BOOST_CHECK_EXCEPTION(PiecewiseBlackVarianceSurface::makeFromGrid(today, dates, strikes, wrongRows, dc), Error,
    ...
    BOOST_CHECK_EXCEPTION(PiecewiseBlackVarianceSurface::makeFromGrid(today, dates, strikes, wrongCols, dc), Error,
                          ExpectedErrorMessage("dates"));
}

BOOST_AUTO_TEST_CASE(testAccessors)
{
    BOOST_TEST_MESSAGE("Testing dayCounter, maxDate, minStrike, maxStrike...");
    ...
    std::vector<Date> dates = {d1, d2};
    std::vector<ext::shared_ptr<SmileSection>> sections = {ext::make_shared<FlatSmileSection>(d1, vol, dc, today),
...
{
    BOOST_TEST_MESSAGE("Testing blackVariance at reference date (t=0)...");
    ...
    std::vector<Date> dates = {d1};
    std::vector<ext::shared_ptr<SmileSection>> sections = {ext::make_shared<FlatSmileSection>(d1, 0.25, dc, today)};
...
{
    BOOST_TEST_MESSAGE("Testing surface with a single tenor...");
...
{
    BOOST_TEST_MESSAGE("Testing surface with ragged strike grids across tenors...");
    ...
    Real sqrtT1 = std::sqrt(t1);
    std::vector<Real> stdDevs1 = {0.30 * sqrtT1, 0.20 * sqrtT1, 0.18 * sqrtT1};
    auto section1 =
        ext::make_shared<InterpolatedSmileSection<Linear>>(d1, strikes1, stdDevs1, 100.0, dc, Linear(), today);

    // tenor 2: strikes [70, 90, 110, 130] — different grid, wider
    std::vector<Rate> strikes2 = {70.0, 90.0, 110.0, 130.0};
    Real sqrtT2 = std::sqrt(t2);
    std::vector<Real> stdDevs2 = {0.35 * sqrtT2, 0.25 * sqrtT2, 0.22 * sqrtT2, 0.20 * sqrtT2};
    auto section2 =
        ext::make_shared<InterpolatedSmileSection<Linear>>(d2, strikes2, stdDevs2, 100.0, dc, Linear(), today);
    ...
    Real strike = 75.0;
    Date dMid = today + 9 * Months;
    BOOST_CHECK_EXCEPTION(surface.blackVariance(dMid, strike), Error, ExpectedErrorMessage("outside the range"));
    ...
    Real diff = std::fabs(calculated - expected);
    if (diff > tol)
        BOOST_FAIL("ragged grid: interpolation at off-grid strike "
                   << strike << "\n    calculated: " << std::setprecision(16) << std::scientific << calculated
                   << "\n    expected:   " << expected);
    ...
    for (Real testStrike : testStrikes)
    {
        ...
        if (var_d1 > var_dMid + tol)
            BOOST_FAIL("ragged grid: calendar arbitrage d1->dMid at strike "
                       << testStrike << "\n    var(d1):   " << std::setprecision(16) << std::scientific << var_d1
                       << "\n    var(dMid): " << var_dMid);

        if (var_dMid > var_d2 + tol)
            BOOST_FAIL("ragged grid: calendar arbitrage dMid->d2 at strike "
                       << testStrike << "\n    var(dMid): " << std::setprecision(16) << std::scientific << var_dMid
                       << "\n    var(d2):   " << var_d2);
    }
}
...
{
    BOOST_TEST_MESSAGE("Testing single-section constructor...");
    ...
    for (const auto& tenor : tenors)
    {
        ...
        {
            BOOST_FAIL("single-section constructor vol mismatch at "
                       << tenor << std::fixed << std::setprecision(12) << "\n    calculated: " << calculated
                       << "\n    expected:   " << vol << "\n    tolerance:  " << tol);
        }
    }
}
...
{
    BOOST_TEST_MESSAGE("Testing equivalence with SABRVolTermStructure...");
    ...
    for (const auto& tenor : tenors)
    {
        ...
        auto smile = ext::make_shared<SabrSmileSection>(t, fwd, sabrParams);
...
{
    BOOST_TEST_MESSAGE("Testing with SVI smile section...");
    ...
    for (Real strike : strikes)
    {
        ...
        if (std::fabs(fromSmile - fromSurface) > tol)
        {
            BOOST_FAIL("SVI smile/surface mismatch"
                       << "\n    strike:     " << strike << std::fixed << std::setprecision(12)
                       << "\n    smile:      " << fromSmile << "\n    surface:    " << fromSurface
...
{
    BOOST_TEST_MESSAGE("Testing FD local-vol pricing from SABR smile surface...");
    ...
    auto rTS = Handle<YieldTermStructure>(ext::make_shared<FlatForward>(today, r, dc));
    auto qTS = Handle<YieldTermStructure>(ext::make_shared<FlatForward>(today, q, dc));
    ...
    for (const auto& tenor : tenors)
    {
        ...
        smiles.push_back(ext::make_shared<SabrSmileSection>(t, fwd, sabrParams));
    }

    auto volSurface = ext::make_shared<PiecewiseBlackVarianceSurface>(today, dates, std::move(smiles), dc);
    ...
    auto process = ext::make_shared<BlackScholesMertonProcess>(Handle<Quote>(spot), qTS, rTS, volHandle);

    auto fdEngine = ext::make_shared<FdBlackScholesVanillaEngine>(process, 100, 200, 0, FdmSchemeDesc::Douglas(),
                                                                  true); // localVol = true
