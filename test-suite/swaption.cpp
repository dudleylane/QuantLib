...
struct CommonVars
{
    ...
    makeSwaption(const ext::shared_ptr<VanillaSwap>& swap,
                 const Date& exercise,
                 ...
                 BlackSwaptionEngine::CashAnnuityModel model = BlackSwaptionEngine::SwapRate) const
    {
        ...
        ext::shared_ptr<PricingEngine> engine(
            new BlackSwaptionEngine(termStructure, vol, Actual365Fixed(), 0.0, model));

        ext::shared_ptr<Swaption> result(new Swaption(swap, ext::shared_ptr<Exercise>(new EuropeanExercise(exercise)),
                                                      settlementType, settlementMethod));
        result->setPricingEngine(engine);
        return result;
    }
    ext::shared_ptr<Swaption>
    makeOISwaption(const ext::shared_ptr<OvernightIndexedSwap>& swap,
                   const Date& exercise,
                   Volatility volatility,
                   Settlement::Type settlementType = Settlement::Physical,
                   Settlement::Method settlementMethod = Settlement::PhysicalOTC,
                   BlackSwaptionEngine::CashAnnuityModel model = BlackSwaptionEngine::SwapRate) const
    {
        ...
        auto result = ext::make_shared<Swaption>(swap, ext::make_shared<EuropeanExercise>(exercise), settlementType,
                                                 settlementMethod);
        result->setPricingEngine(engine);
        return result;
    }
    ...
    makeEngine(Volatility volatility, BlackSwaptionEngine::CashAnnuityModel model = BlackSwaptionEngine::SwapRate) const
    {
    ...
    CommonVars()
    {
        ...
        oisIndex = ext::make_shared<Eonia>(Handle<YieldTermStructure>(ext::make_shared<ZeroSpreadedTermStructure>(
            termStructure, Handle<Quote>(ext::make_shared<SimpleQuote>(-0.01)))));
    ...
    for (auto& exercise : exercises)
    {
        for (auto& length : lengths)
        {
            for (auto& k : type)
            {
                ...
                {
                    ext::shared_ptr<VanillaSwap> swap = MakeVanillaSwap(length, vars.index, strike)
                                                            .withEffectiveDate(startDate)
                                                            .withFixedLegTenor(1 * Years)
                                                            .withFixedLegDayCount(vars.fixedDayCount)
                                                            .withFloatingLegSpread(0.0)
                                                            .withType(k);
                    ext::shared_ptr<Swaption> swaption = vars.makeSwaption(swap, exerciseDate, vol);
                    values.push_back(swaption->NPV());
                    ext::shared_ptr<Swaption> swaption_cash =
                        vars.makeSwaption(swap, exerciseDate, vol, Settlement::Cash, Settlement::ParYieldCurve);
                ...
                {
                    auto it = std::adjacent_find(values.begin(), values.end(), std::greater<>());
                    if (it != values.end())
                    {
                        Size n = it - values.begin();
    ...
    for (auto exercise : exercises)
    {
        for (auto& length : lengths)
        {
            for (auto& k : type)
            {
                ...
                {
                    ext::shared_ptr<VanillaSwap> swap = MakeVanillaSwap(length, vars.index, 0.06)
                                                            .withFixedLegTenor(1 * Years)
                                                            .withFixedLegDayCount(vars.fixedDayCount)
                                                            .withEffectiveDate(startDate)
                                                            .withFloatingLegSpread(spread)
                                                            .withType(k);
                    ext::shared_ptr<Swaption> swaption = vars.makeSwaption(swap, exerciseDate, 0.20);
                    values.push_back(swaption->NPV());
                    ext::shared_ptr<Swaption> swaption_cash =
                        vars.makeSwaption(swap, exerciseDate, 0.20, Settlement::Cash, Settlement::ParYieldCurve);
                    values_cash.push_back(swaption_cash->NPV());
                }
                // and check that they go the right way
                if (k == Swap::Payer)
        ...
        for (auto& length : lengths)
        {
            for (auto& k : type)
            {
                ...
                for (Real spread : spreads)
                {
                    ...
                    ext::shared_ptr<VanillaSwap> equivalentSwap = MakeVanillaSwap(length, vars.index, 0.06 + correction)
                                                                      .withFixedLegTenor(1 * Years)
                                                                      .withFixedLegDayCount(vars.fixedDayCount)
                                                                      .withEffectiveDate(startDate)
                                                                      .withFloatingLegSpread(0.0)
                                                                      .withType(k);
                    ext::shared_ptr<Swaption> swaption1 = vars.makeSwaption(swap, exerciseDate, 0.20);
                    ext::shared_ptr<Swaption> swaption2 = vars.makeSwaption(equivalentSwap, exerciseDate, 0.20);
                    ext::shared_ptr<Swaption> swaption1_cash =
                        vars.makeSwaption(swap, exerciseDate, 0.20, Settlement::Cash, Settlement::ParYieldCurve);
                    ext::shared_ptr<Swaption> swaption2_cash = vars.makeSwaption(
                        equivalentSwap, exerciseDate, 0.20, Settlement::Cash, Settlement::ParYieldCurve);
...
BOOST_AUTO_TEST_CASE(testCachedValue)
{
    ...
    Date startDate = vars.calendar.advance(exerciseDate, vars.settlementDays, Days);
    ext::shared_ptr<VanillaSwap> swap = MakeVanillaSwap(10 * Years, vars.index, 0.06)
                                            .withEffectiveDate(startDate)
                                            .withFixedLegTenor(1 * Years)
                                            .withFixedLegDayCount(vars.fixedDayCount);
    ...
    ext::shared_ptr<OvernightIndexedSwap> oiswap = MakeOIS(10 * Years, vars.oisIndex, 0.06)
                                                       .withEffectiveDate(startDate)
                                                       .withPaymentFrequency(Annual)
                                                       .withFixedLegDayCount(vars.fixedDayCount);

    ext::shared_ptr<Swaption> oiswaption = vars.makeOISwaption(oiswap, exerciseDate, 0.20);
                ...
                for (Size h = 0; h < std::size(type); h++)
                {
                    ...
                    for (Real vol : vols)
                    {
                        ...
                        ext::shared_ptr<Swaption> swaption1 =
                            vars.makeSwaption(swap, exerciseDate, vol - shift, types[h], methods[h]);
                        ext::shared_ptr<Swaption> swaption2 =
                            vars.makeSwaption(swap, exerciseDate, vol + shift, types[h], methods[h]);

                        Real swaptionNPV = swaption->NPV();
                        Real numericalVegaPerPoint = (swaption2->NPV() - swaption1->NPV()) / (200.0 * shift);
                        // check only relevant vega
                        if (numericalVegaPerPoint / swaptionNPV > 1.0e-7)
                        {
                            Real analyticalVegaPerPoint = swaption->result<Real>("vega") / 100.0;
                            Real discrepancy = std::fabs(analyticalVegaPerPoint - numericalVegaPerPoint);
...
BOOST_AUTO_TEST_CASE(testCashSettledSwaptions)
{
    ...
    for (auto exercise : exercises)
    {
        for (auto length : lengths)
        {
            ...
            Schedule floatSchedule(startDate, maturity, vars.floatingTenor, vars.calendar, vars.floatingConvention,
                                   vars.floatingConvention, DateGeneration::Forward, false);
            // Swap with fixed leg conventions: Business Days = Unadjusted, DayCount = 30/360
            Schedule fixedSchedule_u(startDate, maturity, Period(vars.fixedFrequency), vars.calendar, Unadjusted,
                                     Unadjusted, DateGeneration::Forward, true);
            ext::shared_ptr<VanillaSwap> swap_u360(new VanillaSwap(type[0], vars.nominal, fixedSchedule_u, strike,
                                                                   Thirty360(Thirty360::BondBasis), floatSchedule,
                                                                   vars.index, 0.0, vars.index->dayCounter()));

            // Swap with fixed leg conventions: Business Days = Unadjusted, DayCount = Act/365
            ext::shared_ptr<VanillaSwap> swap_u365(new VanillaSwap(type[0], vars.nominal, fixedSchedule_u, strike,
                                                                   Actual365Fixed(), floatSchedule, vars.index, 0.0,
                                                                   vars.index->dayCounter()));

            // Swap with fixed leg conventions: Business Days = Modified Following, DayCount = 30/360
            Schedule fixedSchedule_a(startDate, maturity, Period(vars.fixedFrequency), vars.calendar, ModifiedFollowing,
                                     ModifiedFollowing, DateGeneration::Forward, true);
            ext::shared_ptr<VanillaSwap> swap_a360(new VanillaSwap(type[0], vars.nominal, fixedSchedule_a, strike,
                                                                   Thirty360(Thirty360::BondBasis), floatSchedule,
                                                                   vars.index, 0.0, vars.index->dayCounter()));

            // Swap with fixed leg conventions: Business Days = Modified Following, DayCount = Act/365
            ext::shared_ptr<VanillaSwap> swap_a365(new VanillaSwap(type[0], vars.nominal, fixedSchedule_a, strike,
                                                                   Actual365Fixed(), floatSchedule, vars.index, 0.0,
                                                                   vars.index->dayCounter()));
            ...
            // FlatForward curves
            Handle<YieldTermStructure> termStructure_u360(ext::shared_ptr<YieldTermStructure>(
                new FlatForward(vars.settlement, swap_u360->fairRate(), Thirty360(Thirty360::BondBasis), Compounded,
                                vars.fixedFrequency)));
            Handle<YieldTermStructure> termStructure_a360(ext::shared_ptr<YieldTermStructure>(
                new FlatForward(vars.settlement, swap_a360->fairRate(), Thirty360(Thirty360::BondBasis), Compounded,
                                vars.fixedFrequency)));
            Handle<YieldTermStructure> termStructure_u365(ext::shared_ptr<YieldTermStructure>(new FlatForward(
                vars.settlement, swap_u365->fairRate(), Actual365Fixed(), Compounded, vars.fixedFrequency)));
            Handle<YieldTermStructure> termStructure_a365(ext::shared_ptr<YieldTermStructure>(new FlatForward(
                vars.settlement, swap_a365->fairRate(), Actual365Fixed(), Compounded, vars.fixedFrequency)));
            ...
            ext::shared_ptr<Swaption> swaption_p_u360 = vars.makeSwaption(swap_u360, exerciseDate, 0.20);
            Real value_p_u360 = swaption_p_u360->NPV();
            // Cash settled swaption
            ext::shared_ptr<Swaption> swaption_c_u360 =
                vars.makeSwaption(swap_u360, exerciseDate, 0.20, Settlement::Cash, Settlement::ParYieldCurve);
            ...
            // Physical settled swaption
            ext::shared_ptr<Swaption> swaption_p_a365 = vars.makeSwaption(swap_a365, exerciseDate, 0.20);
            Real value_p_a365 = swaption_p_a365->NPV();
            // Cash settled swaption
            ext::shared_ptr<Swaption> swaption_c_a365 =
                vars.makeSwaption(swap_a365, exerciseDate, 0.20, Settlement::Cash, Settlement::ParYieldCurve);
            ...
            // Physical settled swaption
            ext::shared_ptr<Swaption> swaption_p_a360 = vars.makeSwaption(swap_a360, exerciseDate, 0.20);
            Real value_p_a360 = swaption_p_a360->NPV();
            // Cash settled swaption
            ext::shared_ptr<Swaption> swaption_c_a360 =
                vars.makeSwaption(swap_a360, exerciseDate, 0.20, Settlement::Cash, Settlement::ParYieldCurve);
            ...
            // Physical settled swaption
            ext::shared_ptr<Swaption> swaption_p_u365 = vars.makeSwaption(swap_u365, exerciseDate, 0.20);
            Real value_p_u365 = swaption_p_u365->NPV();
            // Cash settled swaption
            ext::shared_ptr<Swaption> swaption_c_u365 =
                vars.makeSwaption(swap_u365, exerciseDate, 0.20, Settlement::Cash, Settlement::ParYieldCurve);
                ...
                for (auto& k : type)
                {
                    ...
                    for (Size h = 0; h < std::size(types); h++)
                    {
                        for (auto priceType : priceTypes)
                        {
                            for (Real vol : vols)
                            {
                                ext::shared_ptr<Swaption> swaption = vars.makeSwaption(
                                    swap, exerciseDate, vol, types[h], methods[h], BlackSwaptionEngine::DiscountCurve);
                                // Black price
                                Real value = priceType == Swaption::Spot ? swaption->NPV() :
                                                                           swaption->result<Real>("forwardPrice");
                                Volatility implVol = 0.0;
                                try
                                {
                                    implVol = swaption->impliedVolatility(value, vars.termStructure, 0.10, tolerance,
                                                                          maxEvaluations, 1.0e-7, 4.0, ShiftedLognormal,
                                                                          0.0, priceType);
                                }
                                catch (std::exception& e)
                                {
                                    ...
                                    swaption->setPricingEngine(
                                        vars.makeEngine(0.0, BlackSwaptionEngine::DiscountCurve));
                                ...
                                if (std::fabs(implVol - vol) > tolerance)
                                {
                                    ...
                                    if (std::fabs(value - value2) > tolerance)
                                    {
                                        BOOST_ERROR("implied vol failure: "
                                                    ...
                                                    << "\nvol:           " << io::volatility(vol)
                                                    << "\nprice:         " << value << "\ntype:          "
                                                    << (priceType == Swaption::Spot ? "spot" : "forward") << "\n"
    ...
    for (auto& exercise : exercises)
    {
        for (auto& length : lengths)
        {
            ...
            for (Real& strike : strikes)
            {
                for (auto& k : type)
                {
                    ext::shared_ptr<OvernightIndexedSwap> swap = MakeOIS(length, vars.oisIndex, strike)
                                                                     .withEffectiveDate(startDate)
                                                                     .withPaymentFrequency(Annual)
                                                                     .withFixedLegDayCount(vars.fixedDayCount)
                                                                     .withType(k);
                    for (Size h = 0; h < std::size(types); h++)
                    {
                        for (auto priceType : priceTypes)
                        {
                            for (Real vol : vols)
                            {
                                ext::shared_ptr<Swaption> swaption = vars.makeOISwaption(
                                    swap, exerciseDate, vol, types[h], methods[h], BlackSwaptionEngine::DiscountCurve);
                                // Black price
                                Real value = priceType == Swaption::Spot ? swaption->NPV() :
                                                                           swaption->result<Real>("forwardPrice");
...
ext::shared_ptr<Engine> makeConstVolEngine(const Handle<YieldTermStructure>& discountCurve, Volatility volatility)
{
    Handle<Quote> h(ext::make_shared<SimpleQuote>(volatility));
...
void checkSwaptionDelta(bool useBachelierVol)
{
    ...
    ext::shared_ptr<YieldTermStructure> projectionCurve =
        ext::make_shared<FlatForward>(today, projectionQuoteHandle, Actual365Fixed());
    projectionCurveHandle.linkTo(projectionCurve);

    Handle<YieldTermStructure> discountHandle(
        ext::make_shared<FlatForward>(today, Handle<Quote>(ext::make_shared<SimpleQuote>(0.0085)), Actual365Fixed()));
    ext::shared_ptr<DiscountingSwapEngine> swapEngine = ext::make_shared<DiscountingSwapEngine>(discountHandle);
            ...
            for (auto& length : lengths)
            {
                for (Real& strike : strikes)
                {
                    for (Size h = 0; h < std::size(type); h++)
                    {
                        ...
                        ext::shared_ptr<Engine> swaptionEngine = makeConstVolEngine<Engine>(discountHandle, volatility);
                        ...
                        ext::shared_ptr<Swaption> swaption = ext::make_shared<Swaption>(
                            underlying, ext::make_shared<EuropeanExercise>(exerciseDate), types[h], methods[h]);
                        swaption->setPricingEngine(swaptionEngine);

                        Real value = swaption->NPV();
                        Real delta = swaption->result<Real>("delta") * bump;

                        projectionQuoteHandle.linkTo(ext::make_shared<SimpleQuote>(projectionRate + bump));
...
BOOST_AUTO_TEST_CASE(testMakeSwaptionWithExerciseCalendar)
{
    ...
    Swaption defaultSwaption = MakeSwaption(swapIndex, 1 * Years, 0.05);
    Date defaultExercise = defaultSwaption.exercise()->dates().front();

    Date expected = targetCalendar.advance(targetCalendar.adjust(today), 1 * Years, ModifiedFollowing);
    BOOST_CHECK_EQUAL(defaultExercise, expected);

    // With custom calendar, exercise date differs
    Swaption customSwaption = MakeSwaption(swapIndex, 1 * Years, 0.05).withExerciseCalendar(usCalendar);
    Date customExercise = customSwaption.exercise()->dates().front();

    Date expectedCustom = usCalendar.advance(usCalendar.adjust(today), 1 * Years, ModifiedFollowing);
    ...
    Swaption explicitSwaption =
        MakeSwaption(swapIndex, fixingDate, 0.05).withExerciseCalendar(usCalendar).withExerciseDate(explicitDate);
