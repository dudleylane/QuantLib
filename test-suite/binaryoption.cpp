...
#define REPORT_FAILURE(greekName, payoff, exercise, barrierType, barrier, s, q, r, today, v, expected, calculated, \
                       error, tolerance)                                                                           \
    BOOST_FAIL(payoff->optionType() << " option with " << barrierTypeToString(barrierType) << " barrier type:\n"   \
                                    << "    barrier:          " << barrier << "\n"                                 \
...
BOOST_AUTO_TEST_CASE(testCashOrNothingHaugValues)
{
    ...
    for (auto& value : values)
    {
        ...
        ext::shared_ptr<StrikedTypePayoff> payoff(new CashOrNothingPayoff(value.type, value.strike, value.cash));
        ...
        ext::shared_ptr<PricingEngine> engine(new AnalyticBinaryBarrierEngine(stochProcess));
        ...
        if (error > value.tol)
        {
            REPORT_FAILURE("value", payoff, amExercise, value.barrierType, value.barrier, value.s, value.q, value.r,
                           today, value.v, value.result, calculated, error, value.tol);
        }
    }
}
