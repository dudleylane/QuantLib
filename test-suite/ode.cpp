...
struct ode2
{
    std::complex<Real> operator()(Real x, const std::complex<Real>& y) { return std::complex<Real>(0.0, 1.0) * y; }
};
...
struct ode4
{
    std::vector<std::complex<Real>> operator()(const std::complex<Real>& x, const std::vector<std::complex<Real>>& y)
    {
...
BOOST_AUTO_TEST_CASE(testAdaptiveRungeKutta)
{
    ...
    {
        Real exact1 = exp(x);
        std::complex<Real> exact2 = std::exp(std::complex<Real>(0.0, x)) * std::complex<Real>(0.0, 1.0);
        Real exact3 = sin(x);
        std::complex<Real> exact4 = std::exp(std::complex<Real>(0.0, x));

        if (std::fabs(exact1 - y1) > tol1)
            BOOST_FAIL("Error in ode #1: exact solution at x=" << x << " is " << exact1 << ", numerical solution is "
                                                               << y1 << " difference " << std::fabs(exact1 - y1)
                                                               << " outside tolerance " << tol1);
        if (abs(exact2 - y2) > tol2)
            BOOST_FAIL("Error in ode #2: exact solution at x=" << x << " is " << exact2 << ", numerical solution is "
                                                               << y2 << " difference " << abs(exact2 - y2)
                                                               << " outside tolerance " << tol2);
        if (std::fabs(exact3 - y3[0]) > tol3)
            BOOST_FAIL("Error in ode #3: exact solution at x=" << x << " is " << exact3 << ", numerical solution is "
                                                               << y3[0] << " difference " << std::fabs(exact3 - y3[0])
                                                               << " outside tolerance " << tol3);
        if (abs(exact4 - y4[0]) > tol4)
            BOOST_FAIL("Error in ode #4: exact solution at x=" << x << " is " << exact4 << ", numerical solution is "
                                                               << y4[0] << " difference " << abs(exact4 - y4[0])
...
{
    return std::sqrt(DotProduct((m * transpose(m)).diagonal(), Array(m.rows(), 1.0)));
}
