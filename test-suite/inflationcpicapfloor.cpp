                                                             ...
                                                             const DayCounter& dc)
{
    ...
    for (Size i = 0; i < N; i++)
    {
        ...
        Handle<Quote> quote(ext::shared_ptr<Quote>(new SimpleQuote(iiData[i].rate / 100.0)));
...
struct CommonVars
{
    ...
    CommonVars() : nominals(1, 1000000)
    {
        ...
        Schedule rpiSchedule = MakeSchedule().from(Date(1, July, 2007)).to(Date(1, April, 2010)).withFrequency(Monthly);
        Real fixData[] = {
            206.1, 207.3, 208.0, 208.9, 209.7, 210.9, 209.8, 211.4, 212.1, 214.0, 215.1, 216.8, //  2008
            216.5, 217.2, 218.4, 217.7, 216.0, 212.9, 210.1, 211.4, 211.3, 211.5, 212.8, 213.4, //  2009
            213.4, 214.4, 215.3, 216.0, 216.6, 218.0, 217.9, 219.2, 220.7, 222.8                //  2010
        };
...
BOOST_AUTO_TEST_CASE(cpicapfloorpricesurface)
{
    ...
    Real nominal = 1.0;
    InterpolatedCPICapFloorTermPriceSurface<Bilinear> cpiSurf(
        nominal, common.baseZeroRate, common.observationLag, common.calendar, common.convention, common.dcZCIIS,
        common.ii, CPI::Flat, common.nominalUK, common.cStrikesUK, common.fStrikesUK, common.cfMaturitiesUK,
        *(common.cPriceUK), *(common.fPriceUK));
    ...
    for (Size i = 0; i < common.fStrikesUK.size(); i++)
    {
        ...
        for (Size j = 0; j < nMat; j++)
        {
            ...
            QL_REQUIRE(fabs(a - b) < 1e-7,
                       "cannot reproduce cpi floor data from surface: " << a << " vs constructed = " << b);
        }
    }
    ...
    for (Size i = 0; i < common.cStrikesUK.size(); i++)
    {
        ...
        for (Size j = 0; j < nMat; j++)
        {
            ...
            QL_REQUIRE(fabs(a - b) < 1e-7,
                       "cannot reproduce cpi cap data from surface: " << a << " vs constructed = " << b);
        }
    }
    ...
    {
        BOOST_ERROR("The requested premium, " << premium << ", does not equal the expected premium, " << expPremium
                                              << ".");
    }
...
BOOST_AUTO_TEST_CASE(cpicapfloorpricer)
{
    ...
    CommonVars common;
    Real nominal = 1.0;
    ext::shared_ptr<CPICapFloorTermPriceSurface> cpiCFpriceSurf(new InterpolatedCPICapFloorTermPriceSurface<Bilinear>(
        nominal, common.baseZeroRate, common.observationLag, common.calendar, common.convention, common.dcZCIIS,
        common.ii, CPI::Flat, common.nominalUK, common.cStrikesUK, common.fStrikesUK, common.cfMaturitiesUK,
        *(common.cPriceUK), *(common.fPriceUK)));
    ...
    CPICapFloor aCap(Option::Call, nominal,
                     startDate, // start date of contract (only)
                     baseCPI,
                     maturity, // this is pre-adjustment!
                     fixCalendar, fixConvention, payCalendar, payConvention, strike, common.ii, common.observationLag,
    ...
    QL_REQUIRE(fabs(cached - aCap.NPV()) < 1e-10,
               "InterpolatingCPICapFloorEngine does not reproduce cached price: " << cached << " vs " << aCap.NPV());
