...
class ExpressPayoff : public Payoff
{
  public:
    ...
    {
        return ((s >= 100.0) ? 108.0 : 100.0) - ((s <= 75.0) ? Real(100.0 - s) : 0.0);
    }
};
...
ext::shared_ptr<HybridHestonHullWhiteProcess> createHestonHullWhite(Time maturity)
{
    ...
    const Real v0 = 0.04;
    ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(rTS, qTS, s0, v0, 1.0, v0 * 0.75, 0.4, -0.7));

    ext::shared_ptr<HullWhiteForwardProcess> hwFwdProcess(new HullWhiteForwardProcess(rTS, 0.00883, 0.01));
    hwFwdProcess->setForwardMeasureTime(maturity);

    const Real equityShortRateCorr = -0.7;

    return ext::make_shared<HybridHestonHullWhiteProcess>(hestonProcess, hwFwdProcess, equityShortRateCorr);
}

FdmSolverDesc createSolverDesc(const std::vector<Size>& dim,
                               const ext::shared_ptr<HybridHestonHullWhiteProcess>& process)
{

    const Time maturity = process->hullWhiteProcess()->getForwardMeasureTime();

    ext::shared_ptr<FdmLinearOpLayout> layout(new FdmLinearOpLayout(dim));

    std::vector<ext::shared_ptr<Fdm1dMesher>> mesher1d = {
        ext::shared_ptr<Fdm1dMesher>(new Uniform1dMesher(std::log(22.0), std::log(440.0), dim[0])),
        ext::shared_ptr<Fdm1dMesher>(new FdmHestonVarianceMesher(dim[1], process->hestonProcess(), maturity)),
        ext::shared_ptr<Fdm1dMesher>(new Uniform1dMesher(-0.15, 0.15, dim[2]))};

    const ext::shared_ptr<FdmMesher> mesher(new FdmMesherComposite(mesher1d));
    ...
    ext::shared_ptr<FdmStepConditionComposite> conditions(
        new FdmStepConditionComposite(std::list<std::vector<Time>>(), FdmStepConditionComposite::Conditions()));

    ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Call, 160.0));

    ext::shared_ptr<FdmInnerValueCalculator> calculator(new FdmLogInnerValue(payoff, mesher, 0));

    const Size tGrid = 100;
    const Size dampingSteps = 0;

    FdmSolverDesc desc = {mesher, boundaries, conditions, calculator, maturity, tGrid, dampingSteps};

    return desc;
}

Array axpy(const boost::numeric::ublas::compressed_matrix<Real>& A, const Array& x)
{
...
{
    Size retVal = 0;
    for (SparseMatrix::const_iterator1 i1 = m.begin1(); i1 != m.end1(); ++i1)
    {
...
BOOST_AUTO_TEST_CASE(testFdmLinearOpLayout)
{
    ...
    {
        BOOST_ERROR("index.dimensions() should be " << expectedDim << ", but is " << calculatedDim);
    }
    ...
    {
        BOOST_FAIL("index.size() should be " << expectedSize << ", but is " << calculatedSize);
    }
    ...
    for (Size k = 0; k < dim[0]; ++k)
    {
        for (Size l = 0; l < dim[1]; ++l)
        {
            for (Size m = 0; m < dim[2]; ++m)
            {
                ...
                {
                    BOOST_FAIL("index.size() should be " << expectedIndex << ", but is " << calculatedIndex);
                }
            }
        }
    }
            ...
            for (Size k = 0; k < dim[0]; ++k, ++iter)
            {
                for (Size n = 1; n < 4; ++n)
                {
                    ...
                    Size calculatedIndex = k + m * dim[0] * dim[1] +
                                           ((l < dim[1] - n) ? l + n : dim[1] - 1 - (l + n - (dim[1] - 1))) * dim[0];

                    if (nn != calculatedIndex)
                    {
                        BOOST_FAIL("next neighbourhood index is " << nn << " but should be " << calculatedIndex);
                    }
                }
                ...
                for (Size n = 1; n < 7; ++n)
                {
                    ...
                    Size calculatedIndex = k + l * dim[0] + ((m < n) ? n - m : m - n) * dim[0] * dim[1];
                    if (nn != calculatedIndex)
                    {
                        BOOST_FAIL("next neighbourhood index is " << nn << " but should be " << calculatedIndex);
                    }
...
BOOST_AUTO_TEST_CASE(testFirstDerivativesMapApply)
{
    ...
    ext::shared_ptr<FdmMesher> mesher(new UniformGridMesher(index, boundaries));
    ...
    for (const auto& iter : *index)
    {
        ...
        if (z == 0)
        {
            expected = (std::cos(boundaries[2].first + dz) - std::cos(boundaries[2].first)) / dz;
        }
        else if (z == dim[2] - 1)
        {
            expected = (std::cos(boundaries[2].second) - std::cos(boundaries[2].second - dz)) / dz;
        }
        else
        {
...
BOOST_AUTO_TEST_CASE(testDerivativeWeightsOnNonUniformGrids)
{
    ...
    const ext::shared_ptr<Fdm1dMesher> mesherX(new Concentrating1dMesher(-2.0, 3.0, 50, std::make_pair(0.5, 0.01)));
    const ext::shared_ptr<Fdm1dMesher> mesherY(new Concentrating1dMesher(0.5, 5.0, 25, std::make_pair(0.5, 0.1)));
    const ext::shared_ptr<Fdm1dMesher> mesherZ(new Concentrating1dMesher(-1.0, 2.0, 31, std::make_pair(1.5, 0.01)));

    const ext::shared_ptr<FdmMesher> meshers(new FdmMesherComposite(mesherX, mesherY, mesherZ));

    const Real tol = 1e-13;
    for (Size direction = 0; direction < 3; ++direction)
    {

        const SparseMatrix dfdx = FirstDerivativeOp(direction, meshers).toMatrix();
        const SparseMatrix d2fdx2 = SecondDerivativeOp(direction, meshers).toMatrix();
        ...
        for (const auto& iter : *meshers->layout())
        {
            ...
            if (c == 0)
            {
                ...
                const Array ndWeights1st = NumericalDifferentiation({}, 1, twoPoints).weights();
                    ...
                    std::fabs((gamma1 - ndWeights1st.at(1)) / gamma1) > tol)
                {
                    BOOST_FAIL("can not reproduce the weights of the "
                               ...
                               << "\n difference beta:  " << beta1 - ndWeights1st.at(0)
                               << "\n expected gamma:   " << ndWeights1st.at(1) << "\n calculated gamma: " << gamma1
                               << "\n difference gamma: " << gamma1 - ndWeights1st.at(1));
                }
                ...
                if (std::fabs(beta2) > QL_EPSILON || std::fabs(gamma2) > QL_EPSILON)
                {
                    BOOST_FAIL("can not reproduce the weights of the "
            ...
            else if (c == meshers->layout()->dim()[direction] - 1)
            {
                    ...
                    std::fabs((beta1 - ndWeights1st.at(1)) / beta1) > tol)
                {
                    BOOST_FAIL("can not reproduce the weights of the "
                               ...
                               << "\n difference alpha: " << alpha1 - ndWeights1st.at(0)
                               << "\n expected beta:    " << ndWeights1st.at(1) << "\n calculated beta:  " << beta1
                               << "\n difference beta:  " << beta1 - ndWeights1st.at(1));
                }
                ...
                if (std::fabs(alpha2) > QL_EPSILON || std::fabs(beta2) > QL_EPSILON)
                {
                    BOOST_FAIL("can not reproduce the weights of the "
                               "second order derivative operator "
                               "on the upper boundary"
            ...
            else
            {
                ...
                const Array ndWeights1st = NumericalDifferentiation({}, 1, threePoints).weights();
                    ...
                    std::fabs((gamma1 - ndWeights1st.at(2)) / gamma1) > tol)
                {
                    BOOST_FAIL("can not reproduce the weights of the "
                               ...
                               << "\n difference gamma: " << gamma1 - ndWeights1st.at(2));
                }

                const Array ndWeights2nd = NumericalDifferentiation({}, 2, threePoints).weights();
                    ...
                    std::fabs((gamma2 - ndWeights2nd.at(2)) / gamma2) > tol)
                {
                    BOOST_FAIL("can not reproduce the weights of the "
                               ...
                               << "\n difference alpha: " << alpha2 - ndWeights2nd.at(0)
                               << "\n expected beta:    " << ndWeights2nd.at(1) << "\n calculated beta:  " << beta2
                               << "\n difference beta:  " << beta2 - ndWeights2nd.at(1)
                               << "\n expected gamma:   " << ndWeights2nd.at(2) << "\n calculated gamma: " << gamma2
                               << "\n difference gamma: " << gamma2 - ndWeights2nd.at(2));
                }
            }
        }
    }
...
BOOST_AUTO_TEST_CASE(testSecondOrderMixedDerivativesMapApply)
{
    ...
    BOOST_TEST_MESSAGE("Testing application of second-order mixed-derivatives map...");
...
BOOST_AUTO_TEST_CASE(testTripleBandMapSolve)
{
    ...
    ext::shared_ptr<FdmMesher> mesher(new UniformGridMesher(layout, boundaries));
...
BOOST_AUTO_TEST_CASE(testFdmHestonBarrier)
{
    ...
    ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(rTS, qTS, s0, 0.04, 2.5, 0.04, 0.66, -0.8));

    Settings::instance().evaluationDate() = Date(28, March, 2004);
    Date exerciseDate(28, March, 2005);

    ext::shared_ptr<FdmLinearOpComposite> hestonOp(new FdmHestonOp(mesher, hestonProcess));
    ...
    BilinearInterpolation interpolate(ty.begin(), ty.end(), tx.begin(), tx.end(), ret);

    const Real x = 100;
    const Real v0 = 0.04;

    const Real npv = interpolate(v0, std::log(x));
    const Real delta = 0.5 * (interpolate(v0, std::log(x + 1)) - interpolate(v0, std::log(x - 1)));
    const Real gamma = interpolate(v0, std::log(x + 1)) + interpolate(v0, std::log(x - 1)) - 2 * npv;
...
BOOST_AUTO_TEST_CASE(testFdmHestonAmerican)
{
    ...
    ext::shared_ptr<FdmLinearOpComposite> LinearOp(new FdmHestonOp(mesher, hestonProcess));

    ext::shared_ptr<Payoff> payoff(new PlainVanillaPayoff(Option::Put, 100.0));
    Array rhs(mesher->layout()->size());
    for (const auto& iter : *mesher->layout())
    {
        rhs[iter.index()] = payoff->operator()(std::exp(mesher->location(iter, 0)));
    }

    FdmAmericanStepCondition condition(
        mesher, ext::shared_ptr<FdmInnerValueCalculator>(new FdmLogInnerValue(payoff, mesher, 0)));
    const Real theta = 0.5 + std::sqrt(3.0) / 6.;
...
BOOST_AUTO_TEST_CASE(testFdmHestonExpress)
{
    ...
    Handle<HestonProcess> hestonProcess(ext::make_shared<HestonProcess>(rTS, qTS, s0, 0.04, 2.5, 0.04, 0.66, -0.8));
    ...
    DividendSchedule dividendSchedule(
        1, ext::shared_ptr<Dividend>(new FixedDividend(2.5, evaluationDate + Period(6, Months))));
    ext::shared_ptr<FdmDividendHandler> dividendCondition(
        new FdmDividendHandler(dividendSchedule, mesher, rTS->referenceDate(), rTS->dayCounter(), 0));

    ext::shared_ptr<StepCondition<Array>> expressCondition(
        new FdmHestonExpressCondition(redemptions, triggerLevels, exerciseTimes, mesher));

    std::list<std::vector<Time>> stoppingTimes = {exerciseTimes, dividendCondition->dividendTimes()};

    std::list<ext::shared_ptr<StepCondition<Array>>> conditions = {expressCondition, dividendCondition};

    ext::shared_ptr<FdmStepConditionComposite> condition(new FdmStepConditionComposite(stoppingTimes, conditions));
    ...
    const FdmSolverDesc solverDesc = {mesher, bcSet, condition, calculator, 1.0, 50, 0};
...
BOOST_AUTO_TEST_CASE(testFdmHestonHullWhiteOp, *precondition(if_speed(Fast)))
{
    ...
    ext::shared_ptr<HybridHestonHullWhiteProcess> jointProcess = createHestonHullWhite(maturity);
    FdmSolverDesc desc = createSolverDesc(dim, jointProcess);
    ext::shared_ptr<FdmMesher> mesher = desc.mesher;

    ext::shared_ptr<HullWhiteForwardProcess> hwFwdProcess = jointProcess->hullWhiteProcess();

    ext::shared_ptr<HullWhiteProcess> hwProcess(
        new HullWhiteProcess(jointProcess->hestonProcess()->riskFreeRate(), hwFwdProcess->a(), hwFwdProcess->sigma()));

    ext::shared_ptr<FdmLinearOpComposite> linearOp(
        new FdmHestonHullWhiteOp(mesher, jointProcess->hestonProcess(), hwProcess, jointProcess->eta()));
    ...
    for (Size k = 0; k < dim[2]; ++k)
    {
        ...
        y.push_back(BicubicSpline(ty.begin(), ty.end(), tx.begin(), tx.end(), ret)(v0, std::log(x0)));
    }

    const Real directCalc = MonotonicCubicNaturalSpline(tr.begin(), tr.end(), y.begin())(r0);
    ...
    VanillaOption option(ext::shared_ptr<StrikedTypePayoff>(new PlainVanillaPayoff(Option::Call, 160.0)),
                         ext::shared_ptr<Exercise>(new EuropeanExercise(exerciseDate)));

    const Real tol = 0.025;
    option.setPricingEngine(MakeMCHestonHullWhiteEngine<PseudoRandom>(jointProcess)
                                .withSteps(200)
...
BOOST_AUTO_TEST_CASE(testBiCGstab)
{
    ...
    const boost::numeric::ublas::compressed_matrix<Real> a = createTestMatrix(n, m, theta);

    const std::function<Array(const Array&)> matmult = [&](const Array& _x) { return axpy(a, _x); };

    SparseILUPreconditioner ilu(a, 4);
    std::function<Array(const Array&)> precond = [&](const Array& _x) { return ilu.apply(_x); };
    ...
    const Real error = std::sqrt(DotProduct(b - axpy(a, x), b - axpy(a, x)) / DotProduct(b, b));
...
BOOST_AUTO_TEST_CASE(testCrankNicolsonWithDamping)
{
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(new CashOrNothingPayoff(Option::Put, 100, 10.0));
    ...
    const ext::shared_ptr<Fdm1dMesher> equityMesher(
        new FdmBlackScholesMesher(dim[0], process, maturity, payoff->strike(), Null<Real>(), Null<Real>(), 0.0001, 1.5,
                                  std::pair<Real, Real>(payoff->strike(), 0.01)));

    const ext::shared_ptr<FdmMesher> mesher(new FdmMesherComposite(equityMesher));

    ext::shared_ptr<FdmBlackScholesOp> map(new FdmBlackScholesOp(mesher, process, payoff->strike()));
    ...
    FdmBackwardSolver solver(map, FdmBoundaryConditionSet(), ext::shared_ptr<FdmStepConditionComposite>(),
    ...
    Real calculatedGamma = (spline.secondDerivative(std::log(s)) - spline.derivative(std::log(s))) / (s * s);
...
BOOST_AUTO_TEST_CASE(testSpareMatrixReference)
{
    ...
    SparseMatrix calculated = std::accumulate(refs.begin() + 1, refs.end(), SparseMatrix(refs.front()));
    ...
    for (Size i = 0; i < rows; ++i)
    {
        for (Size j = 0; j < columns; ++j)
        {
            ...
            {
                BOOST_FAIL("Error using sparse matrix references in " << "Element (" << i << ", " << j << ")"
...
BOOST_AUTO_TEST_CASE(testFdmMesherIntegral)
{
    ...
    const ext::shared_ptr<FdmMesherComposite> mesher(new FdmMesherComposite(
        ext::shared_ptr<Fdm1dMesher>(new Concentrating1dMesher(-1, 1.6, 21, std::pair<Real, Real>(0, 0.1))),
        ext::shared_ptr<Fdm1dMesher>(new Concentrating1dMesher(-3, 4, 11, std::pair<Real, Real>(1, 0.01))),
        ext::shared_ptr<Fdm1dMesher>(new Concentrating1dMesher(-2, 1, 5, std::pair<Real, Real>(0.5, 0.1)))));
    ...
    for (const auto& iter : *mesher->layout())
    {
        ...
        f[iter.index()] = x * x + 3 * y * y - 3 * z * z + 2 * x * y - x * z - 3 * y * z + 4 * x - y - 3 * z + 2;
    }
    ...
    const Real calculatedSimpson = FdmMesherIntegral(mesher, DiscreteSimpsonIntegral()).integrate(f);

    if (std::fabs(calculatedSimpson - expectedSimpson) > tol * expectedSimpson)
    {
        BOOST_FAIL(std::setprecision(16) << "discrete mesher integration using Simpson's rule failed: "
                                         << "\n    calculated: " << calculatedSimpson
                                         << "\n    expected:   " << expectedSimpson);
    }

    const Real expectedTrapezoid = 917.0148209153263;
    const Real calculatedTrapezoid = FdmMesherIntegral(mesher, DiscreteTrapezoidIntegral()).integrate(f);

    if (std::fabs(calculatedTrapezoid - expectedTrapezoid) > tol * expectedTrapezoid)
    {
        BOOST_FAIL(std::setprecision(16) << "discrete mesher integration using Trapezoid rule failed: "
...
BOOST_AUTO_TEST_CASE(testHighInterestRateBlackScholesMesher)
{
    ...
    const ext::shared_ptr<GeneralizedBlackScholesProcess> process = ext::make_shared<GeneralizedBlackScholesProcess>(
        Handle<Quote>(ext::make_shared<SimpleQuote>(spot)), Handle<YieldTermStructure>(flatRate(today, q, dc)),
        Handle<YieldTermStructure>(flatRate(today, r, dc)), Handle<BlackVolTermStructure>(flatVol(today, v, dc)));
    ...
    const std::vector<Real> loc =
        FdmBlackScholesMesher(size, process, maturity, strike, Null<Real>(), Null<Real>(), eps, scaleFactor)
            .locations();

    const Real calculatedMin = std::exp(loc.front());
    const Real calculatedMax = std::exp(loc.back());

    const Real minimum = spot * std::exp(-normInvEps * scaleFactor * v * std::sqrt(maturity));
    const Real maximum = spot / process->riskFreeRate()->discount(maturity) *
    ...
    if (maxDiff > relTol * maximum)
    {
        BOOST_FAIL("Upper bound for Black-Scholes mesher failed: "
                   << "\n    calculated: " << calculatedMax << "\n    expected:   " << maximum << std::scientific
    ...
    if (minDiff > relTol * minimum)
    {
        BOOST_FAIL("Lower bound for Black-Scholes mesher failed: "
                   << "\n    calculated: " << calculatedMin << "\n    expected:   " << minimum << std::scientific
...
BOOST_AUTO_TEST_CASE(testLowVolatilityHighDiscreteDividendBlackScholesMesher)
{
    ...
    DividendSchedule divSchedule = {ext::make_shared<FixedDividend>(firstDivAmount, firstDivDate),
    ...
    const std::vector<Real> loc =
        FdmBlackScholesMesher(size, process, maturity, strike, Null<Real>(), Null<Real>(), eps, scaleFactor,
                              std::make_pair(Null<Real>(), Null<Real>()), divSchedule)
            .locations();

    const Real maximum = spot->value() * qTS->discount(firstDivDate) / rTS->discount(firstDivDate);

    const Real minimum =
        (1 - firstDivAmount / (spot->value() * qTS->discount(firstDivDate) / rTS->discount(firstDivDate))) *
