...
namespace {
    ...
    SwapOnCurve makeSwapOnCurve(Date today,
        ...
        for (Size i = 0; i < swapRates.size(); ++i) {
            ...
            helpers.push_back(
                ext::make_shared<SwapRateHelper>(
                    Handle<Quote>(q), swapTenors[i], cal, Annual, Unadjusted,
                    Thirty360(Thirty360::BondBasis), euribor, Handle<Quote>(),
                    0 * Days, disc));
        }
        ...
        out.swap = MakeVanillaSwap(5 * Years, euriborForecast, 0.03)
                       ...
                       .withType(Swap::Payer);
        out.swap->setPricingEngine(
            ext::make_shared<DiscountingSwapEngine>(disc));
        return out;
    }
...
BOOST_AUTO_TEST_CASE(testBucketedDeltaSign) {
    ...
    SwapOnCurve fixture = makeSwapOnCurve(
        Date(20, April, 2026),
...
BOOST_AUTO_TEST_CASE(testBucketedSumMatchesParallel) {
    ...
    BOOST_CHECK_MESSAGE(std::fabs(sumOfBuckets - parallel) < tol,
        "Sum of buckets " << sumOfBuckets << " does not match parallel "
        "delta " << parallel << " within tolerance " << tol);
}
...
BOOST_AUTO_TEST_CASE(testBucketerRestoresQuotesOnException) {
    ...
    for (const auto& q : fixture.quotes) originals.push_back(q->value());
    ...
    for (Size i = 0; i < fixture.quotes.size(); ++i) {
        BOOST_CHECK_MESSAGE(close(fixture.quotes[i]->value(), originals[i]),
            "Quote " << i << " not restored: got " << fixture.quotes[i]->value()
...
BOOST_AUTO_TEST_CASE(testBucketerRejectsInvalidInputs) {
    ...
    std::vector<ext::shared_ptr<SimpleQuote> > oneQuote{
        ext::make_shared<SimpleQuote>(0.02)};
    auto zeroBump = [&]{ CurveBucketer cb(oneQuote, 0.0); (void)cb; };
    auto negBump  = [&]{ CurveBucketer cb(oneQuote, -1e-4); (void)cb; };
    BOOST_CHECK_THROW(zeroBump(), Error);
    BOOST_CHECK_THROW(negBump(), Error);

    // Null quote rejected
    std::vector<ext::shared_ptr<SimpleQuote> > withNull{
        ext::make_shared<SimpleQuote>(0.02),
        ext::shared_ptr<SimpleQuote>()};
    auto nullQuote = [&]{ CurveBucketer cb(withNull); (void)cb; };
    ...
    auto emptyQuotes = [&]{ CurveBucketer cb(empty); (void)cb; };
