...
struct CommonVars
{
    ...
    ext::shared_ptr<VanillaSwap> makeSwap(Rate fixedRate) const
    {
        ...
        Schedule fixedSchedule(start, maturity, Period(fixedFrequency), calendar, fixedConvention, fixedConvention,
                               DateGeneration::Forward, false);
        Schedule floatSchedule(start, maturity, Period(floatingFrequency), calendar, floatingConvention,
                               floatingConvention, DateGeneration::Forward, false);
        ext::shared_ptr<VanillaSwap> swap(new VanillaSwap(type, nominal, fixedSchedule, fixedRate, fixedDayCount,
                                                          floatSchedule, index, 0.0, index->dayCounter()));
        swap->setPricingEngine(ext::shared_ptr<PricingEngine>(new DiscountingSwapEngine(termStructure)));
        return swap;
    }
};
...
BOOST_AUTO_TEST_CASE(testCachedValues)
{
    ...
    if (std::fabs(swaption.NPV() - otmValue) > tolerance)
        BOOST_ERROR("failed to reproduce cached out-of-the-money " << "swaption value:\n"
                                                                   << "calculated: " << swaption.NPV() << "\n"
                                                                   << "expected:   " << otmValue);
...
BOOST_AUTO_TEST_CASE(testCachedG2Values, *precondition(if_speed(Fast)))
{
    ...
    for (Real s = 0.5; s < 1.51; s += 0.25)
    {
        ...
        swaptions.push_back(ext::make_shared<Swaption>(swap, ext::make_shared<BermudanExercise>(exerciseDates)));
    }

    const Real a = 0.1, sigma = 0.01, b = 0.2, eta = 0.013, rho = -0.5;

    const ext::shared_ptr<G2> g2Model(ext::make_shared<G2>(vars.termStructure, a, sigma, b, eta, rho));
    const ext::shared_ptr<PricingEngine> fdmEngine(ext::make_shared<FdG2SwaptionEngine>(g2Model, 50, 75, 75, 0, 1e-3));
    const ext::shared_ptr<PricingEngine> treeEngine(ext::make_shared<TreeSwaptionEngine>(g2Model, 50));
...
BOOST_AUTO_TEST_CASE(testTreeEngineTimeSnapping)
{
    ...
    for (int i = -intervalOfDaysToTest; i < intervalOfDaysToTest + 1; i++)
    {
        ...
        if (calendar.isBusinessDay(callDate))
        {
            ...
            bermudanSwaption->setPricingEngine(ext::make_shared<TreeSwaptionEngine>(model, timesteps));
            ...
            {
                BOOST_ERROR(std::fixed << std::setprecision(2) << std::setw(5) << "At " << io::iso_date(callDate)
...
BOOST_AUTO_TEST_CASE(testBermudanOISSwaptionWithHW)
{
    ...
    BOOST_TEST_MESSAGE("Testing Bermudan swaption with OIS underlying and HW model...");
    ...
    Schedule fixedSchedule(start, maturity, Period(vars.fixedFrequency), vars.calendar, vars.fixedConvention,
                           vars.fixedConvention, DateGeneration::Forward, false);
    Schedule overnightSchedule(start, maturity, Period(vars.floatingFrequency), vars.calendar, vars.floatingConvention,
    ...
    {
        auto ois = ext::make_shared<OvernightIndexedSwap>(vars.type, vars.nominal, fixedSchedule, fixedRate,
                                                          vars.fixedDayCount, overnightSchedule, overnightIndex, 0.0);
    ...
    if (itmValue <= 0.0)
        BOOST_ERROR("ITM OIS Bermudan swaption has non-positive value: " << itmValue);
    if (atmValue <= 0.0)
        BOOST_ERROR("ATM OIS Bermudan swaption has non-positive value: " << atmValue);
    if (otmValue <= 0.0)
        BOOST_ERROR("OTM OIS Bermudan swaption has non-positive value: " << otmValue);

    // ITM > ATM > OTM monotonicity
    if (itmValue <= atmValue)
        BOOST_ERROR("ITM OIS Bermudan (" << itmValue << ") should exceed ATM (" << atmValue << ")");
    if (atmValue <= otmValue)
        BOOST_ERROR("ATM OIS Bermudan (" << atmValue << ") should exceed OTM (" << otmValue << ")");
    ...
    auto relDiff = [](Real a, Real b) { return std::fabs(a - b) / std::max(std::fabs(b), 1e-10); };
    Real itmDiff = relDiff(itmValue, itmVS.NPV());
    Real atmDiff = relDiff(atmValue, atmVS.NPV());
    Real otmDiff = relDiff(otmValue, otmVS.NPV());

    if (itmDiff > relTol)
        BOOST_ERROR("ITM OIS vs VanillaSwap Bermudan difference too large: " << std::setprecision(16) << std::scientific
                                                                             << itmValue << " vs " << itmVS.NPV()
                                                                             << " (rel diff " << itmDiff << ")");
    if (atmDiff > relTol)
        BOOST_ERROR("ATM OIS vs VanillaSwap Bermudan difference too large: " << std::setprecision(16) << std::scientific
                                                                             << atmValue << " vs " << atmVS.NPV()
                                                                             << " (rel diff " << atmDiff << ")");
    if (otmDiff > relTol)
        BOOST_ERROR("OTM OIS vs VanillaSwap Bermudan difference too large: " << std::setprecision(16) << std::scientific
...
BOOST_AUTO_TEST_CASE(testBermudanOISSwaptionWithG2)
{
    ...
    BOOST_TEST_MESSAGE("Testing Bermudan swaption with OIS underlying and G2 model...");
    ...
    auto atmOIS = ext::make_shared<OvernightIndexedSwap>(vars.type, vars.nominal, fixedSchedule, atmRate,
                                                         vars.fixedDayCount, overnightSchedule, overnightIndex, 0.0);
    ...
    if (oisValue <= 0.0)
        BOOST_ERROR("ATM OIS Bermudan swaption (G2) has non-positive value: " << oisValue);
    ...
    if (relDiff > relTol)
        BOOST_ERROR("ATM OIS vs VanillaSwap Bermudan (G2) difference too large: "
                    << std::setprecision(16) << std::scientific << oisValue << " vs " << vsValue << " (rel diff "
                    << relDiff << ")");
}
...
BOOST_AUTO_TEST_CASE(testBermudanOISSwaptionPreservesFeatures)
{
    ...
    BOOST_TEST_MESSAGE("Testing that Bermudan OIS swaption preserves averaging and lockout...");
    ...
    {
        auto ois = ext::make_shared<OvernightIndexedSwap>(vars.type, vars.nominal, fixedSchedule, atmRate,
                                                          vars.fixedDayCount, overnightSchedule, overnightIndex, 0.0,
                                                          0, // paymentLag
    ...
    Real avgDiff = std::fabs(compoundValue - simpleValue) / std::max(compoundValue, 1e-10);
    if (avgDiff < 0.001)
        BOOST_ERROR("Simple vs compound OIS Bermudan should differ,"
                    << " got " << std::setprecision(16) << std::scientific << avgDiff * 100
                    << "% (compound=" << compoundValue << ", simple=" << simpleValue << ")");
    ...
    Real lockDiff = std::fabs(lockoutValue - compoundValue) / std::max(compoundValue, 1e-10);
    if (lockDiff < 1e-8)
        BOOST_ERROR("5-day lockout OIS Bermudan should differ from plain,"
                    << " rel diff = " << std::setprecision(16) << std::scientific << lockDiff
                    << " (lockout=" << lockoutValue << ", plain=" << compoundValue << ")");
}
