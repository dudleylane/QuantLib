...
struct CommonVars
{
    ...
    makeZabrParametersGuess(Real alpha, Real beta, Real nu, Real rho, Real gamma) const
    {
        ...
        {
            guess[i] = {
                Handle<Quote>(ext::make_shared<SimpleQuote>(alpha)), Handle<Quote>(ext::make_shared<SimpleQuote>(beta)),
                Handle<Quote>(ext::make_shared<SimpleQuote>(nu)), Handle<Quote>(ext::make_shared<SimpleQuote>(rho)),
                Handle<Quote>(ext::make_shared<SimpleQuote>(gamma))};
        }
        return guess;
    }

    void makeAtmVolTest(const SwaptionVolatilityCube& volCube, Real tolerance)
    {
    ...
    void makeVolSpreadsTest(const SwaptionVolatilityCube& volCube, Real tolerance)
    {
        ...
        for (Size i = 0; i < cube.tenors.options.size(); i++)
        {
            for (Size j = 0; j < cube.tenors.swaps.size(); j++)
            {
                ...
                {
                    Rate atmStrike = volCube.atmStrike(cube.tenors.options[i], cube.tenors.swaps[j]);
                    Volatility atmVol =
                        atmVolMatrix->volatility(cube.tenors.options[i], cube.tenors.swaps[j], atmStrike, true);
                    Volatility vol = volCube.volatility(cube.tenors.options[i], cube.tenors.swaps[j],
                                                        atmStrike + cube.strikeSpreads[k], true);
                    Volatility spread = vol - atmVol;
                    Volatility expVolSpread = cube.volSpreads[i * cube.tenors.swaps.size() + j][k];
    ...
    CommonVars()
    {
        ...
        atmVolMatrix = RelinkableHandle<SwaptionVolatilityStructure>(ext::shared_ptr<SwaptionVolatilityStructure>(
        ...
        normalVolMatrix = RelinkableHandle<SwaptionVolatilityStructure>(ext::shared_ptr<SwaptionVolatilityStructure>(
            new SwaptionVolatilityMatrix(conventions.calendar, conventions.optionBdc, atm.tenors.options,
                                         atm.tenors.swaps, atm.volsHandle, conventions.dayCounter, false,
        ...
        swapIndexBase = ext::shared_ptr<SwapIndex>(new EuriborSwapIsdaFixA(2 * Years, termStructure));
        shortSwapIndexBase = ext::shared_ptr<SwapIndex>(new EuriborSwapIsdaFixA(1 * Years, termStructure));
...
BOOST_AUTO_TEST_CASE(testSabrNormalVolatility)
{
    ...
    SabrSwaptionVolatilityCube volCube(vars.normalVolMatrix, vars.cube.tenors.options, vars.cube.tenors.swaps,
                                       vars.cube.strikeSpreads, vars.cube.volSpreadsHandle, vars.swapIndexBase,
                                       vars.shortSwapIndexBase, vars.vegaWeighedSmileFit, parametersGuess,
...
BOOST_AUTO_TEST_CASE(testAtmVols)
{
    ...
    InterpolatedSwaptionVolatilityCube volCube(vars.atmVolMatrix, vars.cube.tenors.options, vars.cube.tenors.swaps,
                                               vars.cube.strikeSpreads, vars.cube.volSpreadsHandle, vars.swapIndexBase,
                                               vars.shortSwapIndexBase, vars.vegaWeighedSmileFit);
...
BOOST_AUTO_TEST_CASE(testSabrVols)
{
    ...
    SabrSwaptionVolatilityCube volCube(vars.atmVolMatrix, vars.cube.tenors.options, vars.cube.tenors.swaps,
                                       vars.cube.strikeSpreads, vars.cube.volSpreadsHandle, vars.swapIndexBase,
                                       vars.shortSwapIndexBase, vars.vegaWeighedSmileFit, parametersGuess,
                                       isParameterFixed, true);
...
BOOST_AUTO_TEST_CASE(testSpreadedCube)
{
    ...
    Handle<SwaptionVolatilityStructure> volCube(ext::shared_ptr<SwaptionVolatilityStructure>(
        new SabrSwaptionVolatilityCube(vars.atmVolMatrix, vars.cube.tenors.options, vars.cube.tenors.swaps,
                                       vars.cube.strikeSpreads, vars.cube.volSpreadsHandle, vars.swapIndexBase,
                                       vars.shortSwapIndexBase, vars.vegaWeighedSmileFit, parametersGuess,
                                       isParameterFixed, true)));
    ...
    ext::shared_ptr<SwaptionVolatilityStructure> spreadedVolCube(new SpreadedSwaptionVolatility(volCube, spreadHandle));
    ...
    for (Size i = 0; i < vars.cube.tenors.options.size(); i++)
    {
        for (Size j = 0; j < vars.cube.tenors.swaps.size(); j++)
        {
            ...
            {
                Real diff =
                    spreadedVolCube->volatility(vars.cube.tenors.options[i], vars.cube.tenors.swaps[j], strike) -
    ...
    volCube->update();
    if (!f.isUp())
        BOOST_ERROR("SpreadedSwaptionVolatilityStructure " << "does not propagate notifications");
    f.lower();
    spread->setValue(.001);
    if (!f.isUp())
        BOOST_ERROR("SpreadedSwaptionVolatilityStructure " << "does not propagate notifications");
...
BOOST_AUTO_TEST_CASE(testObservability)
{
    ...
    volCube1_0 = ext::make_shared<SabrSwaptionVolatilityCube>(
    ...
    Settings::instance().evaluationDate() =
        vars.conventions.calendar.advance(referenceDate, Period(1, Days), vars.conventions.optionBdc);

    // VolCube created after change of reference date
    volCube1_1 = ext::make_shared<SabrSwaptionVolatilityCube>(
        vars.atmVolMatrix, vars.cube.tenors.options, vars.cube.tenors.swaps, vars.cube.strikeSpreads,
        vars.cube.volSpreadsHandle, vars.swapIndexBase, vars.shortSwapIndexBase, vars.vegaWeighedSmileFit,
        parametersGuess, isParameterFixed, true);
    ...
    for (Size i = 0; i < vars.cube.tenors.options.size(); i++)
    {
        for (Size j = 0; j < vars.cube.tenors.swaps.size(); j++)
        {
            for (Size k = 0; k < vars.cube.strikeSpreads.size(); k++)
            {
                ...
                Volatility v0 = volCube1_0->volatility(vars.cube.tenors.options[i], vars.cube.tenors.swaps[j],
                                                       dummyStrike + vars.cube.strikeSpreads[k], false);
                Volatility v1 = volCube1_1->volatility(vars.cube.tenors.options[i], vars.cube.tenors.swaps[j],
                                                       dummyStrike + vars.cube.strikeSpreads[k], false);
                if (std::fabs(v0 - v1) > 1e-14)
                    BOOST_ERROR(description << " option tenor = " << vars.cube.tenors.options[i]
                                            << " swap tenor = " << vars.cube.tenors.swaps[j]
                                            << " strike = " << io::rate(dummyStrike + vars.cube.strikeSpreads[k])
                                            << "  v0 = " << io::volatility(v0) << "  v1 = " << io::volatility(v1)
    ...
    // VolCube created before change of reference date
    volCube2_0 = ext::make_shared<InterpolatedSwaptionVolatilityCube>(
        vars.atmVolMatrix, vars.cube.tenors.options, vars.cube.tenors.swaps, vars.cube.strikeSpreads,
        vars.cube.volSpreadsHandle, vars.swapIndexBase, vars.shortSwapIndexBase, vars.vegaWeighedSmileFit);
    Settings::instance().evaluationDate() =
        vars.conventions.calendar.advance(referenceDate, Period(1, Days), vars.conventions.optionBdc);
    ...
    // VolCube created after change of reference date
    volCube2_1 = ext::make_shared<InterpolatedSwaptionVolatilityCube>(
        vars.atmVolMatrix, vars.cube.tenors.options, vars.cube.tenors.swaps, vars.cube.strikeSpreads,
        vars.cube.volSpreadsHandle, vars.swapIndexBase, vars.shortSwapIndexBase, vars.vegaWeighedSmileFit);
    ...
    for (Size i = 0; i < vars.cube.tenors.options.size(); i++)
    {
        for (Size j = 0; j < vars.cube.tenors.swaps.size(); j++)
        {
            for (Size k = 0; k < vars.cube.strikeSpreads.size(); k++)
            {
                ...
                Volatility v0 = volCube2_0->volatility(vars.cube.tenors.options[i], vars.cube.tenors.swaps[j],
                                                       dummyStrike + vars.cube.strikeSpreads[k], false);
                Volatility v1 = volCube2_1->volatility(vars.cube.tenors.options[i], vars.cube.tenors.swaps[j],
                                                       dummyStrike + vars.cube.strikeSpreads[k], false);
                if (std::fabs(v0 - v1) > 1e-14)
                    BOOST_ERROR(description << " option tenor = " << vars.cube.tenors.options[i]
                                            << " swap tenor = " << vars.cube.tenors.swaps[j]
...
BOOST_AUTO_TEST_CASE(testZabrVols)
{
    ...
    ZabrSwaptionVolatilityCube volCube(vars.atmVolMatrix, vars.cube.tenors.options, vars.cube.tenors.swaps,
                                       vars.cube.strikeSpreads, vars.cube.volSpreadsHandle, vars.swapIndexBase,
                                       vars.shortSwapIndexBase, vars.vegaWeighedSmileFit, parametersGuess,
                                       isParameterFixed, true);
    ...
    ext::shared_ptr<SmileSection> smileSection = volStructure->smileSection(Period(10, Years), Period(2, Years));
...
BOOST_AUTO_TEST_CASE(testZabrSmileSection)
{
    ...
    ext::shared_ptr<SmileSection> smileSection = volStructure->smileSection(Period(5, Years), Period(5, Years));
    ...
    for (Real strike : strikes)
    {
        ...
        BOOST_CHECK_MESSAGE(vol > 0.0, "ZABR smile section volatility at strike " << strike << " should be positive");
    }
}
...
BOOST_AUTO_TEST_CASE(testZabrWithFreeGamma)
{
    ...
    ZabrSwaptionVolatilityCube cube(vars.atmVolMatrix, vars.cube.tenors.options, vars.cube.tenors.swaps,
                                    vars.cube.strikeSpreads, vars.cube.volSpreadsHandle, vars.swapIndexBase,
                                    vars.shortSwapIndexBase, vars.vegaWeighedSmileFit, parametersGuess,
                                    isParameterFixed, true);
    ...
    ext::shared_ptr<SmileSection> section = cube.smileSection(optionTenor, swapTenor);
    BOOST_REQUIRE(section != nullptr);

    // Downcast to ZabrSmileSection to access calibrated gamma
    auto zabrSection = ext::dynamic_pointer_cast<ZabrSmileSection<ZabrShortMaturityLognormal>>(section);
...
BOOST_AUTO_TEST_CASE(testZabrShiftedVolThrows)
{
    ...
    Handle<SwaptionVolatilityStructure> shiftedAtmVol(ext::make_shared<SwaptionVolatilityMatrix>(
        vars.conventions.calendar, vars.conventions.optionBdc, vars.atm.tenors.options, vars.atm.tenors.swaps,
        vars.atm.volsHandle, vars.conventions.dayCounter, false, ShiftedLognormal, shifts));
    ...
    ZabrSwaptionVolatilityCube volCube(shiftedAtmVol, vars.cube.tenors.options, vars.cube.tenors.swaps,
                                       vars.cube.strikeSpreads, vars.cube.volSpreadsHandle, vars.swapIndexBase,
                                       vars.shortSwapIndexBase, vars.vegaWeighedSmileFit, parametersGuess,
                                       isParameterFixed, true);
    ...
    Rate strike = volCube.atmStrike(vars.cube.tenors.options[0], vars.cube.tenors.swaps[0]);
    BOOST_CHECK_THROW(volCube.volatility(vars.cube.tenors.options[0], vars.cube.tenors.swaps[0], strike, true),
                      QuantLib::Error);
}
...
BOOST_AUTO_TEST_CASE(testZabrAlternativeKernel)
{
    ...
    typedef XabrSwaptionVolatilityCube<SwaptionVolCubeZabrModel<ZabrShortMaturityNormal>> ZabrNormalKernelCube;

    ZabrNormalKernelCube volCube(vars.atmVolMatrix, vars.cube.tenors.options, vars.cube.tenors.swaps,
                                 vars.cube.strikeSpreads, vars.cube.volSpreadsHandle, vars.swapIndexBase,
                                 vars.shortSwapIndexBase, vars.vegaWeighedSmileFit, parametersGuess, isParameterFixed,
                                 true);

    // Smoke test: cube constructs and returns positive vol at ATM
    ext::shared_ptr<SmileSection> section = volCube.smileSection(Period(5, Years), Period(5, Years));
    BOOST_REQUIRE(section != nullptr);
...
BOOST_AUTO_TEST_CASE(testZabrObservability)
{
    ...
    for (Size i = 0; i < vars.cube.tenors.options.size(); ++i)
    {
        for (Size j = 0; j < vars.cube.tenors.swaps.size(); ++j)
        {
            for (Size k = 0; k < vars.cube.strikeSpreads.size(); ++k)
            {
                Volatility v0 = volCube0->volatility(vars.cube.tenors.options[i], vars.cube.tenors.swaps[j],
                                                     dummyStrike + vars.cube.strikeSpreads[k], false);
                Volatility v1 = volCube1->volatility(vars.cube.tenors.options[i], vars.cube.tenors.swaps[j],
                                                     dummyStrike + vars.cube.strikeSpreads[k], false);
                if (std::fabs(v0 - v1) > 1e-14)
                    BOOST_ERROR("ZABR cube observability failed:"
                                " option tenor = "
                                << vars.cube.tenors.options[i] << " swap tenor = " << vars.cube.tenors.swaps[j]
                                << " strike = " << io::rate(dummyStrike + vars.cube.strikeSpreads[k])
                                << "  v0 = " << io::volatility(v0) << "  v1 = " << io::volatility(v1)
