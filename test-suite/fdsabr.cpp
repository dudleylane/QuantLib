...
class SabrMonteCarloPricer
{
  public:
    SabrMonteCarloPricer(
        Real f0, Time maturity, ext::shared_ptr<Payoff> payoff, Real alpha, Real beta, Real nu, Real rho)
    : f0_(f0), maturity_(maturity), payoff_(std::move(payoff)), alpha_(alpha), beta_(beta), nu_(nu), rho_(rho)
    {
    }
...
Real OsterleeReferenceResults::data_[9][3] = {{0.0610, 0.0604}, {0.0468, 0.0463}, {0.0347, 0.0343},
...
BOOST_AUTO_TEST_CASE(testFdmSabrOp, *precondition(if_speed(Fast)))
{
    ...
    const ext::shared_ptr<PlainVanillaPayoff> putPayoff = ext::make_shared<PlainVanillaPayoff>(Option::Put, strike);
    const ext::shared_ptr<PlainVanillaPayoff> callPayoff = ext::make_shared<PlainVanillaPayoff>(Option::Call, strike);

    VanillaOption optionPut(putPayoff, exercise);
    VanillaOption optionCall(callPayoff, exercise);

    const Handle<YieldTermStructure> rTS = Handle<YieldTermStructure>(flatRate(today, 0.0, dc));
    ...
    const ext::shared_ptr<GeneralizedBlackScholesProcess> bsProcess = ext::make_shared<GeneralizedBlackScholesProcess>(
        Handle<Quote>(ext::make_shared<SimpleQuote>(f0)), rTS, rTS, Handle<BlackVolTermStructure>(flatVol(0.2, dc)));
    ...
    for (Real beta : betas)
    {
        ...
        {
            BOOST_ERROR("failed to validate the call/put parity"
                        << "\n    beta           : " << beta << "\n    strike         : " << strike
                        << "\n    fwd (call/put) : " << pdeFwd << "\n    fwd (f0-strike): " << f0 - strike
                        << "\n    diff           : " << parityDiff << "\n    tol            : " << parityTol);
        }

        const Real putPdeImplVol = optionPut.impliedVolatility(optionPut.NPV(), bsProcess, 1e-6);

        const std::function<Real(Real)> mcSabr(SabrMonteCarloPricer(f0, maturityTime, putPayoff, alpha, beta, nu, rho));

        const Real mcNPV = RichardsonExtrapolation(mcSabr, 1 / 4.0)(4.0, 2.0);

        const Real putMcImplVol = optionPut.impliedVolatility(mcNPV, bsProcess, 1e-6);
        ...
        if (volDiff > volTol)
        {
            BOOST_ERROR("failed to validate PDE against MC implied volatility"
                        << "\n    beta         : " << beta << "\n    strike       : " << strike
        ...
        for (Real strike : strikes)
        {
            ...
            for (Real beta : betas)
            {
                option.setPricingEngine(
                    ext::make_shared<FdSabrVanillaEngine>(f0, alpha, beta, nu, rho, rTS, 100, 400, 3));

                const Real calculated = option.NPV();

                option.setPricingEngine(ext::make_shared<AnalyticCEVEngine>(f0, alpha, beta, rTS));

                const Real expected = option.NPV();

                if (std::fabs(expected - calculated) > tol)
                {
                    BOOST_ERROR("failed to calculate vanilla CEV option prices"
                                << "\n    beta            : " << beta << "\n    strike          : " << strike
                                << "\n    option type     : "
                                << ((payoff->optionType() == Option::Call) ? "Call" : "Put")
                                << "\n    analytic npv    : " << expected << "\n    pde npv         : " << calculated
                                << "\n    npv difference  : " << std::fabs(expected - calculated)
                                << "\n    tolerance       : " << tol);
                }
            }
        }
...
BOOST_AUTO_TEST_CASE(testFdmSabrVsVolApproximation)
{
    ...
    for (auto optionType : optionTypes)
    {
        for (Real strike : strikes)
        {
            ...
            option.setPricingEngine(ext::make_shared<FdSabrVanillaEngine>(f0, alpha, beta, nu, rho, rTS, 25, 100, 50));

            const Volatility fdmVol = option.impliedVolatility(option.NPV(), bsProcess);

            const Real hagenVol = sabrVolatility(strike, f0, maturityTime, alpha, beta, nu, rho);

            const Real diff = std::fabs(fdmVol - hagenVol);

            if (std::fabs(fdmVol - hagenVol) > tol)
            {
                BOOST_ERROR("large difference between Hagen formula and FDM"
                            << "\n    strike          : " << strike
...
BOOST_AUTO_TEST_CASE(testOosterleeTestCaseIV)
{
    ...
    const Period maturities[] = {Period(2, Years), Period(5, Years), Period(10, Years)};
    ...
    for (Size i = 0; i < std::size(maturities); ++i)
    {
        ...
        const ext::shared_ptr<PricingEngine> engine =
            ext::make_shared<FdSabrVanillaEngine>(f0, alpha, beta, nu, rho, rTS, timeSteps, 200, 21);
        ...
        for (Size j = 0; j < std::size(strikes); ++j)
        {
            ...
            const Real expected = RichardsonExtrapolation(std::function<Real(Real)>(referenceResuts), 1 / 16., 1)(2.);

            const Real diff = std::fabs(calculated - expected);
            if (diff > tol)
            {
                BOOST_ERROR("can not reproduce reference values from Monte-Carlo"
...
BOOST_AUTO_TEST_CASE(testBenchOpSabrCase)
{
    ...
    const Real expected[2][3] = {{0.221383196830866, 0.193836689413803, 0.166240814653231},
    ...
    for (Size i = 0; i < std::size(f0s); ++i)
    {
        ...
        const Real strikes[] = {f0 * std::exp(-0.1 * std::sqrt(T)), f0, f0 * std::exp(0.1 * std::sqrt(T))};
        ...
        for (Size j = 0; j < std::size(strikes); ++j)
        {
            ...
            option.setPricingEngine(ext::make_shared<FdSabrVanillaEngine>(f0, alpha, beta, nu, rho, rTS,
                                                                          Size(gridT * factor), Size(gridX * factor),
