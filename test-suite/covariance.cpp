...
BOOST_AUTO_TEST_CASE(testRankReduction)
{
    ...
    if (error > 4.0e-4)
        BOOST_ERROR(std::scientific << error
                                    << " error while salvaging covariance matrix with spectral alg "
                                       "through rankReducedSqrt\n"
                                    << std::fixed << "input matrix:\n"
                                    << badCov << "salvaged matrix:\n"
                                    << goodCov);
}
...
BOOST_AUTO_TEST_CASE(testSalvagingMatrix)
{
    ...
    for (Size i = 0; i < n; i++)
    {
        for (Size j = 0; j < n; j++)
        {
            ...
            calculated = calcCorr[i][j];
            if (std::fabs(calculated - expected) > 1.0e-10)
                BOOST_ERROR("SalvagingCorrelation with spectral alg " << "cor[" << i << "][" << j << "]:\n"
                                                                      << std::setprecision(10)
    ...
    Real error = norm(goodCov - badCov);
    if (error > 4.0e-4)
        BOOST_ERROR(std::scientific << error << " error while salvaging covariance matrix with spectral alg\n"
                                    << std::fixed << "input matrix:\n"
                                    << badCov << "salvaged matrix:\n"
                                    << goodCov);
}
...
BOOST_AUTO_TEST_CASE(testCovariance)
{
    ...
    std::vector<std::vector<Real>> data = {{3.0, 9.0}, {2.0, 7.0}, {4.0, 12.0}, {5.0, 15.0}, {6.0, 17.0}};
    ...
    for (i = 0; i < n; i++)
    {
        for (j = 0; j < n; j++)
        {
            ...
            if (std::fabs(calculated - expected) > 1.0e-10)
                BOOST_ERROR("SequenceStatistics " << "cor[" << i << "][" << j << "]:\n"
                                                  << std::setprecision(10) << "    calculated: " << calculated << "\n"
                                                  << "    expected:   " << expected);
            ...
            if (std::fabs(calculated - expected) > 1.0e-10)
                BOOST_ERROR("SequenceStatistics " << "cov[" << i << "][" << j << "]:\n"
                                                  << std::setprecision(10) << "    calculated: " << calculated << "\n"
                                                  << "    expected:   " << expected);
        }
    }
    ...
    for (i = 0; i < n; i++)
    {
        for (j = 0; j < n; j++)
        {
            Real calculated = calcCov[i][j], expected = expCov[i][j];
            if (std::fabs(calculated - expected) > 1.0e-10)
            {
                BOOST_ERROR("getCovariance " << "cov[" << i << "][" << j << "]:\n"
                                             << std::setprecision(10) << "    calculated: " << calculated << "\n"
                                             << "    expected:   " << expected);
            }
        }
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            BOOST_ERROR("CovarianceDecomposition " << "standardDev[" << i << "]:\n"
