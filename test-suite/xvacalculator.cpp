...
namespace
{
    ...
    struct Fixture
    {
        ...
        Fixture(Real flatEpe = 100.0, Real flatEne = 0.0, Rate cptyHaz = 0.02, Rate ownHaz = 0.01, Rate r = 0.03)
        {
            ...
            cptyPd = Handle<DefaultProbabilityTermStructure>(ext::make_shared<FlatHazardRate>(today, cptyHaz, dc));
            ownPd = Handle<DefaultProbabilityTermStructure>(ext::make_shared<FlatHazardRate>(today, ownHaz, dc));
            discount = Handle<YieldTermStructure>(ext::make_shared<FlatForward>(today, r, dc));
        }
    };
...
BOOST_AUTO_TEST_CASE(testCvaIsPositiveForPositiveExposure)
{
    ...
    XvaCalculator x(f.dates, f.epe, f.ene, f.cptyPd, f.ownPd,
                    ...
                    /*fund*/ 0.005, /*cap*/ 0.01, /*margin*/ 0.003, f.discount);
...
BOOST_AUTO_TEST_CASE(testCvaScalesWithLGD)
{
    ...
    XvaCalculator lowRec(f.dates, f.epe, f.ene, f.cptyPd, f.ownPd,
                         /*cptyRec*/ 0.0, /*ownRec*/ 0.4, 0.0, 0.0, 0.0, f.discount);
    XvaCalculator highRec(f.dates, f.epe, f.ene, f.cptyPd, f.ownPd,
                          /*cptyRec*/ 0.8, /*ownRec*/ 0.4, 0.0, 0.0, 0.0, f.discount);
    // LGD = 1.0 vs 0.2 -> CVA ratio should be 5.0 exactly.
    Real ratio = lowRec.cva() / highRec.cva();
    BOOST_CHECK_MESSAGE(std::fabs(ratio - 5.0) < 1e-12, "CVA LGD scaling ratio " << ratio << " != 5.0");
}
...
BOOST_AUTO_TEST_CASE(testDvaSymmetricToCva)
{
    ...
    auto pd = Handle<DefaultProbabilityTermStructure>(ext::make_shared<FlatHazardRate>(today, 0.02, dc));
    auto discount = Handle<YieldTermStructure>(ext::make_shared<FlatForward>(today, 0.03, dc));

    XvaCalculator x(dates, exposures, exposures, pd, pd, 0.4, 0.4, 0.0, 0.0, 0.0, discount);
...
BOOST_AUTO_TEST_CASE(testMvaZeroWithoutInitialMargin)
{
    ...
    Fixture f;
    XvaCalculator noIm(f.dates, f.epe, f.ene, f.cptyPd, f.ownPd, 0.4, 0.4, 0.0, 0.0, 0.01, f.discount);
    BOOST_CHECK_EQUAL(noIm.mva(), 0.0);

    std::vector<Real> im(f.dates.size(), 50.0);
    XvaCalculator withIm(f.dates, f.epe, f.ene, f.cptyPd, f.ownPd, 0.4, 0.4, 0.0, 0.0, 0.01, f.discount, im);
...
BOOST_AUTO_TEST_CASE(testTrapezoidalOnLinearlyRisingEpe)
{
    ...
    Settings::instance().evaluationDate() = today;
    DayCounter dc = Actual365Fixed();
    std::vector<Date> dates{today, today + Period(1, Years), today + Period(2, Years)};
    std::vector<Real> epe{0.0, 100.0, 200.0};
    std::vector<Real> ene(dates.size(), 0.0);

    Rate haz = 0.02, r = 0.0;
    auto pd = Handle<DefaultProbabilityTermStructure>(ext::make_shared<FlatHazardRate>(today, haz, dc));
    auto ownPd = Handle<DefaultProbabilityTermStructure>(ext::make_shared<FlatHazardRate>(today, 0.0, dc));
    auto disc = Handle<YieldTermStructure>(ext::make_shared<FlatForward>(today, r, dc));

    const Real recovery = 0.4;
    XvaCalculator x(dates, epe, ene, pd, ownPd, recovery, 0.0, 0.0, 0.0, 0.0, disc);
    ...
    Real ref = lgd * (0.5 * (epe[0] + epe[1]) * (pd1 - 0.0) * df1 + 0.5 * (epe[1] + epe[2]) * (pd2 - pd1) * df2);

    Real cva = x.cva();
    BOOST_CHECK_MESSAGE(std::fabs(cva - ref) < 1e-10, "CVA with linear EPE: got " << cva << ", expected " << ref);
}
...
BOOST_AUTO_TEST_CASE(testMvaWithInitialMarginGrid)
{
    ...
    XvaCalculator a(f.dates, f.epe, f.ene, f.cptyPd, f.ownPd, 0.4, 0.4, 0.0, 0.0, ms, f.discount, im1);
    XvaCalculator b(f.dates, f.epe, f.ene, f.cptyPd, f.ownPd, 0.4, 0.4, 0.0, 0.0, ms, f.discount, im2);
...
BOOST_AUTO_TEST_CASE(testConstructorRejectsInvalidInputs)
{
    ...
    auto badRecovery = [&]
    {
        XvaCalculator x(f.dates, f.epe, f.ene, f.cptyPd, f.ownPd, 1.5, 0.4, 0.0, 0.0, 0.0, f.discount);
        (void)x;
    };
    ...
    auto badSize = [&]
    {
        XvaCalculator x(f.dates, shortEpe, f.ene, f.cptyPd, f.ownPd, 0.4, 0.4, 0.0, 0.0, 0.0, f.discount);
        (void)x;
    };
