...
BOOST_AUTO_TEST_CASE(testAnalyticVsBlack)
{
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Put, 30));
    ...
    ext::shared_ptr<BatesProcess> process(
        new BatesProcess(riskFreeTS, dividendTS, s0, v0, kappa, theta, sigma, rho, lambda, nu, delta));

    ext::shared_ptr<PricingEngine> engine(new BatesEngine(ext::make_shared<BatesModel>(process), 64));
    ...
    engine = ext::shared_ptr<PricingEngine>(
        new BatesDetJumpEngine(ext::make_shared<BatesDetJumpModel>(process, 1.0, 0.0001), 64));
    ...
    engine = ext::shared_ptr<PricingEngine>(
        new BatesDoubleExpEngine(ext::make_shared<BatesDoubleExpModel>(process, 0.0001, 0.0001, 0.0001), 64));
    ...
    engine = ext::shared_ptr<PricingEngine>(
        new BatesDoubleExpDetJumpEngine(ext::make_shared<BatesDoubleExpDetJumpModel>(
...
BOOST_AUTO_TEST_CASE(testAnalyticAndMcVsJumpDiffusion)
{
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Put, 95));
    ...
    ext::shared_ptr<BatesProcess> batesProcess(new BatesProcess(riskFreeTS, dividendTS, s0, v0, kappa, theta, sigma,
                                                                rho, jumpIntensity->value(), meanLogJump->value(),
                                                                jumpVol->value()));

    ext::shared_ptr<Merton76Process> mertonProcess(
        new Merton76Process(s0, dividendTS, riskFreeTS, Handle<BlackVolTermStructure>(volTS),
                            Handle<Quote>(jumpIntensity), Handle<Quote>(meanLogJump), Handle<Quote>(jumpVol)));

    ext::shared_ptr<PricingEngine> batesEngine(new BatesEngine(ext::make_shared<BatesModel>(batesProcess), 160));

    const Real mcTol = 0.1;
    ext::shared_ptr<PricingEngine> mcBatesEngine = MakeMCEuropeanHestonEngine<PseudoRandom>(batesProcess)
                                                       .withStepsPerYear(2)
                                                       .withAntitheticVariate()
                                                       .withAbsoluteTolerance(mcTol)
                                                       .withSeed(1234);

    ext::shared_ptr<PricingEngine> mertonEngine(new JumpDiffusionEngine(mertonProcess, 1e-10, 1000));
...
BOOST_AUTO_TEST_CASE(testAnalyticVsMCPricing)
{
    ...
    for (auto& hestonModel : hestonModels)
    {
        ...
        ext::shared_ptr<BatesProcess> batesProcess(
            new BatesProcess(riskFreeTS, dividendTS, s0, hestonModel.v0, hestonModel.kappa, hestonModel.theta,
                             hestonModel.sigma, hestonModel.rho, 2.0, -0.2, 0.1));

        const Real mcTolerance = 0.5;
        ext::shared_ptr<PricingEngine> mcEngine = MakeMCEuropeanHestonEngine<PseudoRandom>(batesProcess)
                                                      .withStepsPerYear(20)
                                                      .withAntitheticVariate()
        ...
        ext::shared_ptr<PricingEngine> fdEngine(new FdBatesVanillaEngine(batesModel, 50, 100, 30));

        ext::shared_ptr<PricingEngine> analyticEngine(new BatesEngine(batesModel, 160));
        ...
        if (mcError > 3 * mcTolerance)
        {
            BOOST_FAIL("failed to reproduce Monte-Carlo price for BatesEngine"
                       << "\n    parameter:  " << hestonModel.name << std::fixed << std::setprecision(8)
                       << "\n    calculated: " << calculated << "\n    expected:   " << expected
...
BOOST_AUTO_TEST_CASE(testDAXCalibration)
{
    ...
    */
    ...
    BOOST_TEST_MESSAGE("Testing Bates model calibration using DAX volatility data...");
    ...
    Handle<YieldTermStructure> riskFreeTS(ext::shared_ptr<YieldTermStructure>(new ZeroCurve(dates, rates, dayCounter)));

    Handle<YieldTermStructure> dividendTS(flatRate(settlementDate, 0.0, dayCounter));
    ...
    ext::shared_ptr<PricingEngine> batesEngine(new BatesEngine(batesModel, 64));
    ...
    for (Size s = 0; s < 13; ++s)
    {
        for (Size m = 0; m < 8; ++m)
        {
            Handle<Quote> vol(ext::shared_ptr<Quote>(new SimpleQuote(v[s * 8 + m])));
            ...
            options.push_back(ext::shared_ptr<BlackCalibrationHelper>(
                new HestonModelHelper(maturity, calendar, s0->value(), strike[s], vol, riskFreeTS, dividendTS,
    ...
    batesModel->calibrate(std::vector<ext::shared_ptr<CalibrationHelper>>(options.begin(), options.end()), om,
    ...
    process = ext::make_shared<BatesProcess>(riskFreeTS, dividendTS, s0, v0, kappa, theta, sigma, rho, 1.0, -0.1, 0.1);

    pricingEngines.push_back(
        ext::shared_ptr<PricingEngine>(new BatesDetJumpEngine(ext::make_shared<BatesDetJumpModel>(process), 64)));
    ...
    pricingEngines.push_back(ext::shared_ptr<PricingEngine>(
        new BatesDoubleExpEngine(ext::make_shared<BatesDoubleExpModel>(hestonProcess, 1.0), 64)));

    pricingEngines.push_back(ext::shared_ptr<PricingEngine>(
        new BatesDoubleExpDetJumpEngine(ext::make_shared<BatesDoubleExpDetJumpModel>(hestonProcess, 1.0), 64)));

    Real expectedValues[] = {5896.37, 5499.29, 6497.89};
