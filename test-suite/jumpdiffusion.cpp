...
#define REPORT_FAILURE_1(greekName, payoff, exercise, s, q, r, today, v, intensity, meanLogJump, jumpVol, expected, \
...
#define REPORT_FAILURE_2(greekName, payoff, exercise, s, q, r, today, v, intensity, gamma, expected, calculated, \
                         error, tolerance)                                                                       \
    BOOST_FAIL(exerciseTypeToString(exercise)                                                                    \
               << " " << payoff->optionType() << " option with " << payoffTypeToString(payoff) << " payoff:\n"   \
               << "    underlying value: " << s << "\n"                                                          \
...
BOOST_AUTO_TEST_CASE(testMerton76)
{
    ...
    */
    ...
    ext::shared_ptr<Merton76Process> stochProcess(
        new Merton76Process(Handle<Quote>(spot), Handle<YieldTermStructure>(qTS), Handle<YieldTermStructure>(rTS),
                            Handle<BlackVolTermStructure>(volTS), Handle<Quote>(jumpIntensity),
                            Handle<Quote>(meanLogJump), Handle<Quote>(jumpVol)));
    ext::shared_ptr<PricingEngine> engine(new JumpDiffusionEngine(stochProcess));
    ...
    for (auto& value : values)
    {
        ...
        Volatility totalVol = std::sqrt(value.jumpIntensity * jVol * jVol + diffusionVol * diffusionVol);
        Volatility volError = std::fabs(totalVol - value.v);
        QL_REQUIRE(volError < 1e-13, volError << " mismatch");
        ...
        if (error > value.tol)
        {
            REPORT_FAILURE_2("value", payoff, exercise, value.s, value.q, value.r, today, value.v, value.jumpIntensity,
                             value.gamma, value.result, calculated, error, value.tol);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testGreeks)
{
    ...
    ext::shared_ptr<Merton76Process> stochProcess(
        new Merton76Process(Handle<Quote>(spot), qTS, rTS, volTS, Handle<Quote>(jumpIntensity),
                            Handle<Quote>(meanLogJump), Handle<Quote>(jumpVol)));
    ...
    ext::shared_ptr<PricingEngine> engine(new JumpDiffusionEngine(stochProcess, 1e-08));
                                            ...
                                            for (Real v : vols)
                                            {
                                                ...
                                                if (value > spot->value() * 1.0e-5)
                                                {
                                                    ...
                                                    for (it = expected.begin(); it != expected.end(); ++it)
                                                    {
                                                        ...
                                                        if (error > tol)
                                                        {
                                                            REPORT_FAILURE_1(greek, payoff, exercise, u, q, r, today, v,
                                                                             jj1, jj2, jj3, expct, calcl, error, tol);
                                                        }
                                                    }
                                                }
                                            }
