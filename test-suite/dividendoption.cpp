...
BOOST_AUTO_TEST_CASE(testEuropeanValues)
{
    ...
    BOOST_TEST_MESSAGE("Testing dividend European option values with no dividends...");
    ...
    for (auto& type : types)
    {
        for (Real strike : strikes)
        {
            for (int length : lengths)
            {
                ...
                auto engine1 = ext::make_shared<AnalyticDividendEuropeanEngine>(
                ...
                auto engine2 = ext::make_shared<AnalyticDividendEuropeanEngine>(
                    stochProcess, DividendVector({exDate + 6 * Months}, {1.0}));
                    ...
                    for (Real m : qRates)
                    {
                        for (Real n : rRates)
                        {
                            for (Real v : vols)
                            {
                                ...
                                if (error1 > tolerance)
                                {
                                    REPORT_FAILURE("value start limit", payoff, exercise, u, q, r, today, v, expected,
                                                   calculated1, error1, tolerance);
                                }
                                if (error2 > tolerance)
                                {
                                    REPORT_FAILURE("value start limit", payoff, exercise, u, q, r, today, v, expected,
                                                   calculated2, error2, tolerance);
                                }
                            }
                        }
                    }
...
BOOST_AUTO_TEST_CASE(testEuropeanKnownValue)
{
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Call, 40.0));
    ...
    if (error > tolerance)
    {
        REPORT_FAILURE("value start limit", payoff, exercise, u, q, r, today, v, expected, calculated, error,
                       tolerance);
    }
}
...
BOOST_AUTO_TEST_CASE(testEuropeanStartLimit)
{
    ...
    BOOST_TEST_MESSAGE("Testing dividend European option with a dividend on today's date...");
                    ...
                    for (Real m : qRates)
                    {
                        for (Real n : rRates)
                        {
                            for (Real v : vols)
                            {
                                ...
                                if (error > tolerance)
                                {
                                    REPORT_FAILURE("value", payoff, exercise, u, q, r, today, v, expected, calculated,
                                                   error, tolerance);
                                }
                            }
                        }
...
BOOST_AUTO_TEST_CASE(testFdEuropeanValues)
{
    ...
    BOOST_TEST_MESSAGE("Testing finite-difference dividend European option values...");
                        ...
                        for (Real n : rRates)
                        {
                            for (Real v : vols)
                            {
                                ...
                                if (calculated > spot->value() * 1.0e-5)
                                {
                                    ...
                                    if (error > tolerance)
                                    {
                                        REPORT_FAILURE("value", payoff, exercise, u, q, r, today, v, expected,
                  ...
                  FdBlackScholesVanillaEngine::CashDividendModel model)
{
    ...
    for (auto& type : types)
    {
        for (Real strike : strikes)
        {
            ...
            ext::shared_ptr<PricingEngine> engine = MakeFdBlackScholesVanillaEngine(stochProcess)
                                                        .withCashDividends(dividendDates, dividends)
...
{
    BOOST_TEST_MESSAGE("Testing finite-differences dividend American option greeks...");
                      ...
                      FdBlackScholesVanillaEngine::CashDividendModel model)
{
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Call, 55.0));

    Real tolerance = 1.0e-6;

    VanillaOption option(payoff, exercise);
    ext::shared_ptr<PricingEngine> engine = MakeFdBlackScholesVanillaEngine(process)
                                                .withTGrid(timeSteps)
                                                .withXGrid(gridPoints)
                                                .withCashDividendModel(model);
    option.setPricingEngine(engine);
...
BOOST_AUTO_TEST_CASE(testFdEuropeanDegenerate)
{
    ...
    BOOST_TEST_MESSAGE("Testing degenerate finite-differences dividend European option...");
...
BOOST_AUTO_TEST_CASE(testFdAmericanDegenerate)
{
    ...
    BOOST_TEST_MESSAGE("Testing degenerate finite-differences dividend American option...");
                           ...
                           FdBlackScholesVanillaEngine::CashDividendModel model)
{
    ...
    ext::shared_ptr<BlackScholesMertonProcess> process(
        new BlackScholesMertonProcess(Handle<Quote>(spot), rTS, rTS, volTS));
    ...
    ext::shared_ptr<Exercise> europeanExercise = ext::make_shared<EuropeanExercise>(exercise->lastDate());
...
BOOST_AUTO_TEST_CASE(testFdEuropeanWithDividendToday)
{
    ...
    BOOST_TEST_MESSAGE("Testing finite-differences dividend European option with dividend on today's date...");
...
BOOST_AUTO_TEST_CASE(testFdAmericanWithDividendToday)
{
    ...
    BOOST_TEST_MESSAGE("Testing finite-differences dividend American option with dividend on today's date...");
...
BOOST_AUTO_TEST_CASE(testCashDividendEuropeanEngine)
{
    ...
    const Handle<YieldTermStructure> rTS(ext::make_shared<ZeroCurve>(
        std::vector<Date>{today, Date(1, May, 2024), Date(1, November, 2024), Date(1, January, 2027)},
        std::vector<Real>{0.3, 0.15, 0.1, 0.15}, dc));
    const Handle<YieldTermStructure> qTS(ext::make_shared<ZeroCurve>(
        std::vector<Date>{today, Date(1, May, 2024), Date(1, November, 2025), Date(1, January, 2027)},
        std::vector<Real>{0.05, 0.03, 0.1, 0.05}, dc));
    const Handle<BlackVolTermStructure> vTS(ext::make_shared<BlackVarianceCurve>(
        today,
        std::vector<Date>{Date(2, January, 2024), Date(1, July, 2024), Date(1, August, 2024), Date(1, January, 2027)},
        std::vector<Volatility>{0.3, 0.4, 0.42, 0.5}, dc));

    const ext::shared_ptr<BlackScholesMertonProcess> process =
        ext::make_shared<BlackScholesMertonProcess>(spot, qTS, rTS, vTS);

    const std::vector<Date> dividendDates{Date(1, April, 2024), Date(1, November, 2024), Date(1, October, 2024),
    ...
    for (const auto cashDivModel : {CashDividendEuropeanEngine::Spot, CashDividendEuropeanEngine::Escrowed})
    {
        const ext::shared_ptr<PricingEngine> fdEngine =
            MakeFdBlackScholesVanillaEngine(process)
                ...
                .withXGrid(800)
                .withCashDividends(dividendDates, dividendAmounts)
                .withCashDividendModel(FdBlackScholesVanillaEngine::CashDividendModel(cashDivModel));

        const ext::shared_ptr<PricingEngine> cashDivEngine =
            ext::make_shared<CashDividendEuropeanEngine>(process, dividendSchedule, cashDivModel);
                ...
                for (const Real strike : {50, 100, 125, 175})
                {
                    ...
                    if (diff > tol)
                    {
                        BOOST_FAIL("Failed to compare European option prices "
                                   ...
                                   << "\n    Strike         : " << strike
                                   << "\n    Option Type    : " << ((optionType == Option::Call) ? "Call" : "Put")
...
BOOST_AUTO_TEST_CASE(testCashDividendEuropeanEngineWithManyDividends)
{
    ...
    const Handle<YieldTermStructure> rTS(ext::make_shared<ZeroCurve>(
        std::vector<Date>{today, Date(1, May, 2026), Date(1, November, 2027), Date(1, January, 2032)},
        std::vector<Real>{0.05, 0.075, 0.04, 0.06}, dc));

    const Handle<BlackVolTermStructure> vTS(ext::make_shared<BlackVarianceCurve>(
        today,
        std::vector<Date>{Date(2, January, 2026), Date(1, July, 2026), Date(1, August, 2027), Date(1, January, 2032)},
        std::vector<Volatility>{0.3, 0.4, 0.42, 0.5}, dc));

    const ext::shared_ptr<BlackScholesMertonProcess> process =
        ext::make_shared<BlackScholesMertonProcess>(Handle<Quote>(ext::make_shared<SimpleQuote>(100)), qTS, rTS, vTS);

    VanillaOption option(ext::make_shared<PlainVanillaPayoff>(Option::Call, 110.0),
                         ext::make_shared<EuropeanExercise>(maturityDate));
    ...
    {
        dividendDates.emplace_back(dividendDates.back() + Period(1 + rng.nextInt32() % 5, Days));
    ...
    option.setPricingEngine(ext::make_shared<CashDividendEuropeanEngine>(
        process, DividendVector(dividendDates, dividendAmounts), CashDividendEuropeanEngine::Spot));
...
BOOST_AUTO_TEST_CASE(testCashDividendEuropeanEngineWithSingleDividends)
{
    ...
    const ext::shared_ptr<BlackScholesMertonProcess> process = ext::make_shared<BlackScholesMertonProcess>(
        Handle<Quote>(ext::make_shared<SimpleQuote>(100)), Handle<YieldTermStructure>(flatRate(today, 0.05, dc)),
        Handle<YieldTermStructure>(flatRate(today, 0.025, dc)), Handle<BlackVolTermStructure>(flatVol(today, 0.3, dc)));
    ...
    for (Date divDate : {today - Period(1, Days), today, today + Period(1, Days), today + Period(6, Months),
                         maturityDate, maturityDate + Period(1, Days)})
        for (const auto cashDivModel : {CashDividendEuropeanEngine::Spot, CashDividendEuropeanEngine::Escrowed})
        {
            ...
            const ext::shared_ptr<PricingEngine> cashDivEngine = ext::make_shared<CashDividendEuropeanEngine>(
                process, DividendVector(dividendDates, dividendAmounts), cashDivModel);

            for (Option::Type optionType : {Option::Call, Option::Put})
            {
                VanillaOption option(ext::make_shared<PlainVanillaPayoff>(optionType, 95.0), exercise);
...
BOOST_AUTO_TEST_CASE(testZeroStrikeCallWithCashDividends)
{
    ...
    const ext::shared_ptr<BlackScholesMertonProcess> process =
        ext::make_shared<BlackScholesMertonProcess>(Handle<Quote>(ext::make_shared<SimpleQuote>(100)), qTS, rTS,
                                                    Handle<BlackVolTermStructure>(flatVol(today, 0.3, dc)));

    VanillaOption europeanOption(ext::make_shared<PlainVanillaPayoff>(Option::Call, 0.0),
                                 ext::make_shared<EuropeanExercise>(maturityDate));
    ...
    for (const Date dividendDate : {today, Date(1, January, 2026), maturityDate})
    {
        ...
        const Real europeanExpected =
            process->x0() * qTS->discount(maturityDate) -
            dividend * rTS->discount(dividendDate) / qTS->discount(dividendDate) * qTS->discount(maturityDate);
        ...
        europeanOption.setPricingEngine(
            ext::make_shared<CashDividendEuropeanEngine>(process, DividendVector({dividendDate}, {dividend})));
        ...
        VanillaOption americanOption(ext::make_shared<PlainVanillaPayoff>(Option::Call, 0.0),
...
BOOST_AUTO_TEST_CASE(testAmericanOptionsWithEscrowedDividends)
{
    ...
    const ext::shared_ptr<BlackScholesMertonProcess> process =
        ext::make_shared<BlackScholesMertonProcess>(Handle<Quote>(ext::make_shared<SimpleQuote>(100)), qTS, rTS, volTS);

    const Real dividend = 5.0;

    for (const Option::Type optionType : {Option::Call, Option::Put})
        for (const Date dividendDate : {today, Date(1, January, 2026), Date(1, January, 2027), maturityDate})
        {
            ...
            const ext::shared_ptr<PricingEngine> escrowedEngine = builder(FdBlackScholesVanillaEngine::Escrowed);

            const ext::shared_ptr<PricingEngine> spotEngine = builder(FdBlackScholesVanillaEngine::Spot);

            VanillaOption option(ext::make_shared<PlainVanillaPayoff>(optionType, 95),
                                 ext::make_shared<AmericanExercise>(maturityDate));
            ...
            vol->setValue(std::sqrt(
                (v * v * dc.yearFraction(today, dividendDate) * squared((spot->value() - dividend) / spot->value()) +
                 v * v * dc.yearFraction(dividendDate, maturityDate)) /
                maturityTime));
