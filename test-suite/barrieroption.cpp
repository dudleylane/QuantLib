...
#define REPORT_FX_FAILURE(greekName, barrierType, barrier, rebate, payoff, exercise, s, q, r, today, vol25Put, atmVol, \
                          vol25Call, v, expected, calculated, error, tolerance)                                        \
    BOOST_ERROR("\n"                                                                                                   \
                << barrierTypeToString(barrierType) << " " << exerciseTypeToString(exercise) << " "                    \
                << payoff->optionType() << " FX option with " << payoffTypeToString(payoff) << " payoff:\n"            \
                << "    underlying value: " << s << "\n"                                                               \
                << "    strike:           " << payoff->strike() << "\n"                                                \
                << "    barrier:          " << barrier << "\n"                                                         \
                << "    rebate:           " << rebate << "\n"                                                          \
...
BOOST_AUTO_TEST_CASE(testParity)
{
    ...
    ext::shared_ptr<BlackScholesProcess> stochProcess =
        ext::make_shared<BlackScholesProcess>(Handle<Quote>(spot), Handle<YieldTermStructure>(rTS), volHandle);
    ...
    BarrierOption knockIn(Barrier::DownIn, 90.0, 0.0, payoff, exercise);
    BarrierOption knockOut(Barrier::DownOut, 90.0, 0.0, payoff, exercise);
    EuropeanOption european(payoff, exercise);

    ext::shared_ptr<PricingEngine> barrierEngine = ext::make_shared<AnalyticBarrierEngine>(stochProcess);

    ext::shared_ptr<PricingEngine> europeanEngine = ext::make_shared<AnalyticEuropeanEngine>(stochProcess);
...
{
    BOOST_TEST_MESSAGE("Testing put-call symmetry for barrier options...");
    ...
    PutCallSymmetryTestCase cases[] = {{90, 95, Barrier::DownOut, 111.11111, 105.26315, Barrier::UpOut},
    ...
    ext::shared_ptr<PricingEngine> callEngine = ext::make_shared<AnalyticBarrierEngine>(processCall);
    ext::shared_ptr<PricingEngine> putEngine = ext::make_shared<AnalyticBarrierEngine>(processPut);

    for (auto& i : cases)
    {
        ext::shared_ptr<StrikedTypePayoff> putPayoff = ext::make_shared<PlainVanillaPayoff>(Option::Put, i.putStrike);
        ext::shared_ptr<StrikedTypePayoff> callPayoff =
            ext::make_shared<PlainVanillaPayoff>(Option::Call, i.callStrike);
        BarrierOption putOption(i.putType, i.putBarrier, rebate, putPayoff, exercise);
        putOption.setPricingEngine(putEngine);
        BarrierOption callOption(i.callType, i.callBarrier, rebate, callPayoff, exercise);
...
BOOST_AUTO_TEST_CASE(testHaugValues)
{
    ...
    auto runTest = [&](const NewBarrierOptionData& value, const bool useZeroSpot, const bool useTriggeredBarrier)
    {
        ...
        const auto barrier = useTriggeredBarrier ? (value.barrierType == Barrier::Type::DownIn ||
                                                            value.barrierType == Barrier::Type::DownOut ?
                                                        value.s * 1.01 :
                                                        value.s * 0.99) :
                                                   value.barrier;
        ...
        if (!useZeroSpot && !useTriggeredBarrier && value.exType == Exercise::European)
        {
            ...
            if (error > value.tol)
            {
                REPORT_FAILURE("value", value.barrierType, value.barrier, value.rebate, payoff, exercise, value.s,
                               value.q, value.r, today, value.v, expected, calculated, error, value.tol);
            }
        }
        else
        {
        ...
        if (!useZeroSpot && !useTriggeredBarrier && value.exType == Exercise::European)
        {
            ...
            if (error > 5.0e-3)
            {
                REPORT_FAILURE("fd value", value.barrierType, value.barrier, value.rebate, payoff, exercise, value.s,
                               value.q, value.r, today, value.v, expected, calculated, error, value.tol);
            }
        }
        else
        ...
        engine =
            ext::make_shared<BinomialBarrierEngine<CoxRossRubinstein, DiscretizedBarrierOption>>(stochProcess, 400);
        barrierOption.setPricingEngine(engine);
        ...
        if (!useZeroSpot && !useTriggeredBarrier)
        {
            ...
            if (error > tol)
            {
                REPORT_FAILURE("Binomial (Boyle-lau) value", value.barrierType, value.barrier, value.rebate, payoff,
                               exercise, value.s, value.q, value.r, today, value.v, expected, calculated, error, tol);
            }
        }
        else
        {
        ...
        engine = ext::make_shared<BinomialBarrierEngine<CoxRossRubinstein, DiscretizedDermanKaniBarrierOption>>(
            stochProcess, 400);
        barrierOption.setPricingEngine(engine);
        ...
        if (!useZeroSpot && !useTriggeredBarrier)
        {
            ...
            if (error > tol)
            {
                REPORT_FAILURE("Binomial (Derman) value", value.barrierType, value.barrier, value.rebate, payoff,
                               exercise, value.s, value.q, value.r, today, value.v, expected, calculated, error, tol);
            }
        }
        else
...
BOOST_AUTO_TEST_CASE(testBabsiriValues)
{
    ...
    */
    BarrierOptionData values[] = {
        {Barrier::DownIn, 0.10, 100, 90, 0.07187, 0.0}, {Barrier::DownIn, 0.15, 100, 90, 0.60638, 0.0},
        {Barrier::DownIn, 0.20, 100, 90, 1.64005, 0.0}, {Barrier::DownIn, 0.25, 100, 90, 2.98495, 0.0},
        {Barrier::DownIn, 0.30, 100, 90, 4.50952, 0.0}, {Barrier::UpIn, 0.10, 100, 110, 4.79148, 0.0},
        {Barrier::UpIn, 0.15, 100, 110, 7.08268, 0.0},  {Barrier::UpIn, 0.20, 100, 110, 9.11008, 0.0},
        {Barrier::UpIn, 0.25, 100, 110, 11.06148, 0.0}, {Barrier::UpIn, 0.30, 100, 110, 12.98351, 0.0}};
    ...
    DayCounter dc = Actual360();
    Date today = Date::todaysDate();
    ext::shared_ptr<SimpleQuote> underlying = ext::make_shared<SimpleQuote>(underlyingPrice);
    ...
    ext::shared_ptr<SimpleQuote> volatility = ext::make_shared<SimpleQuote>(0.10);
    ext::shared_ptr<BlackVolTermStructure> volTS = flatVol(today, volatility, dc);
    ...
    for (auto& value : values)
    {
        ...
        ext::shared_ptr<BlackScholesMertonProcess> stochProcess = ext::make_shared<BlackScholesMertonProcess>(
            Handle<Quote>(underlying), Handle<YieldTermStructure>(qTS), Handle<YieldTermStructure>(rTS),
            Handle<BlackVolTermStructure>(volTS));


        ext::shared_ptr<PricingEngine> engine = ext::make_shared<AnalyticBarrierEngine>(stochProcess);
        ...
        {
            REPORT_FAILURE("value", value.type, value.barrier, rebate, callPayoff, exercise, underlyingPrice, q, r,
                           today, value.volatility, expected, calculated, error, maxErrorAllowed);
        }

        Real maxMcRelativeErrorAllowed = 2.0e-2;

        ext::shared_ptr<PricingEngine> mcEngine = MakeMCBarrierEngine<LowDiscrepancy>(stochProcess)
                                                      .withStepsPerYear(1)
                                                      .withBrownianBridge()
        ...
        {
            REPORT_FAILURE("value", value.type, value.barrier, rebate, callPayoff, exercise, underlyingPrice, q, r,
                           today, value.volatility, expected, calculated, error, maxMcRelativeErrorAllowed);
        }
    }
}
        ...
        "Going to Extreme: Correcting Simulation Bias in Exotic
         Option Valuation"
          - D.R. Beaglehole, P.H. Dybvig and G. Zhou
            Financial Analysts Journal; Jan / Feb 1997; 53, 1
    */
    BarrierOptionData values[] = {{Barrier::DownOut, 0.50, 50, 45, 5.477, 0.0}};
...
BOOST_AUTO_TEST_CASE(testLocalVolAndHestonComparison)
{
    ...
    const Handle<YieldTermStructure> rTS(ext::make_shared<ZeroCurve>(dates, rates, dayCounter));
    const Handle<YieldTermStructure> qTS(flatRate(settlementDate, 0.0, dayCounter));
    ...
    const std::vector<Volatility> v = {
        1.015873, 1.015873, 1.015873, 0.89729,  0.796493, 0.730914, 0.631335, 0.568895, 0.711309, 0.711309, 0.711309,
        0.641309, 0.635593, 0.583653, 0.508045, 0.463182, 0.516034, 0.500534, 0.500534, 0.500534, 0.448706, 0.416661,
        0.375470, 0.353442, 0.516034, 0.482263, 0.447713, 0.387703, 0.355064, 0.337438, 0.316966, 0.306859, 0.497587,
        0.464373, 0.430764, 0.374052, 0.344336, 0.328607, 0.310619, 0.301865, 0.479511, 0.446815, 0.414194, 0.361010,
        0.334204, 0.320301, 0.304664, 0.297180, 0.461866, 0.429645, 0.398092, 0.348638, 0.324680, 0.312512, 0.299082,
        0.292785, 0.444801, 0.413014, 0.382634, 0.337026, 0.315788, 0.305239, 0.293855, 0.288660, 0.428604, 0.397219,
        0.368109, 0.326282, 0.307555, 0.298483, 0.288972, 0.284791, 0.420971, 0.389782, 0.361317, 0.321274, 0.303697,
        0.295302, 0.286655, 0.282948, 0.413749, 0.382754, 0.354917, 0.316532, 0.300016, 0.292251, 0.284420, 0.281164,
        0.400889, 0.370272, 0.343525, 0.307904, 0.293204, 0.286549, 0.280189, 0.277767, 0.390685, 0.360399, 0.334344,
        0.300507, 0.287149, 0.281380, 0.276271, 0.274588, 0.383477, 0.353434, 0.327580, 0.294408, 0.281867, 0.276746,
        0.272655, 0.271617, 0.379106, 0.349214, 0.323160, 0.289618, 0.277362, 0.272641, 0.269332, 0.268846, 0.377073,
        0.347258, 0.320776, 0.286077, 0.273617, 0.269057, 0.266293, 0.266265, 0.399925, 0.369232, 0.338895, 0.289042,
        0.265509, 0.255589, 0.249308, 0.249665, 0.423432, 0.406891, 0.373720, 0.314667, 0.281009, 0.263281, 0.246451,
        0.242166, 0.453704, 0.453704, 0.453704, 0.381255, 0.334578, 0.305527, 0.268909, 0.251367, 0.517748, 0.517748,
    ...
    const ext::shared_ptr<BlackVarianceSurface> volTS = ext::make_shared<BlackVarianceSurface>(
        settlementDate, calendar, std::vector<Date>(dates.begin() + 1, dates.end()), strikes, blackVolMatrix,
        dayCounter);
    volTS->setInterpolation<Bicubic>();
    const ext::shared_ptr<GeneralizedBlackScholesProcess> localVolProcess =
        ext::make_shared<BlackScholesMertonProcess>(s0, qTS, rTS, Handle<BlackVolTermStructure>(volTS));
    ...
    ext::shared_ptr<PricingEngine> fdHestonEngine = ext::make_shared<FdHestonBarrierEngine>(hestonModel, 100, 400, 50);

    ext::shared_ptr<PricingEngine> fdLocalVolEngine = ext::make_shared<FdBlackScholesBarrierEngine>(
        localVolProcess, 100, 400, 0, FdmSchemeDesc::Douglas(), true, 0.35);
    ...
    const ext::shared_ptr<StrikedTypePayoff> payoff = ext::make_shared<PlainVanillaPayoff>(Option::Put, strike);
    ...
    if (std::fabs(expectedHestonNPV - calculatedHestonNPV) > tol * expectedHestonNPV)
    {
    ...
    if (std::fabs(expectedLocalVolNPV - calculatedLocalVolNPV) > tol * expectedLocalVolNPV)
    {
        BOOST_FAIL("Failed to reproduce Heston barrier price for "
                   << "\n    strike:     " << payoff->strike() << "\n    barrier:    " << barrier
...
BOOST_AUTO_TEST_CASE(testDividendBarrierOption)
{
    ...
    auto douglas =
        ext::make_shared<FdBlackScholesBarrierEngine>(bsProcess, dividends, 100, 100, 0, FdmSchemeDesc::Douglas());

    auto crankNicolson = ext::make_shared<FdBlackScholesBarrierEngine>(bsProcess, dividends, 100, 100, 0,
                                                                       FdmSchemeDesc::CrankNicolson());

    auto craigSnyed =
        ext::make_shared<FdBlackScholesBarrierEngine>(bsProcess, dividends, 100, 100, 0, FdmSchemeDesc::CraigSneyd());

    auto hundsdorfer =
        ext::make_shared<FdBlackScholesBarrierEngine>(bsProcess, dividends, 100, 100, 0, FdmSchemeDesc::Hundsdorfer());

    auto mol = ext::make_shared<FdBlackScholesBarrierEngine>(bsProcess, dividends, 100, 100, 0,
                                                             FdmSchemeDesc::MethodOfLines());

    auto trPDF2 =
        ext::make_shared<FdBlackScholesBarrierEngine>(bsProcess, dividends, 100, 100, 0, FdmSchemeDesc::TrBDF2());

    auto hestonEngine = ext::make_shared<FdHestonBarrierEngine>(
        ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(rTS, qTS, s0, v * v, 1.0, v * v, 0.005, 0.0)),
        dividends, 50, 101, 3);

    ext::shared_ptr<PricingEngine> engines[] = {douglas,     crankNicolson, trPDF2,      craigSnyed,
                                                hundsdorfer, mol,           hestonEngine};

    auto payoff = ext::make_shared<PlainVanillaPayoff>(Option::Put, strike);
    auto exercise = ext::make_shared<EuropeanExercise>(maturity);

    Real expected[] = {rTS->discount(divDate) * rebate,
                       (*payoff)((spot - divAmount * rTS->discount(divDate)) / rTS->discount(maturity)) *
        ...
        for (Size j = 0; j < std::size(engines); ++j)
        {
            ...
            if (diff > relTol * expected[i])
            {
                BOOST_ERROR("Failed to reproduce barrier price with discrete dividends:"
                            ...
                            << "\n    expected:   " << expected[i] << std::scientific << "\n    difference  " << diff
...
BOOST_AUTO_TEST_CASE(testDividendBarrierOptionWithDividendsPastMaturity)
{
    ...
    ext::shared_ptr<PricingEngine> engines[] = {
        ext::make_shared<FdBlackScholesBarrierEngine>(bsProcess, 100, 100, 0, FdmSchemeDesc::Douglas()),
    ...
    for (Size i = 0; i < std::size(barriers); ++i)
    {
        ...
        if (diff > tolerance)
        {
            BOOST_ERROR("Failed to reproduce target option price:"
                        ...
                        << "\n    calculated:   " << barrierOption.NPV() << std::scientific << "\n    difference    "
                        << diff);
        }
...
BOOST_AUTO_TEST_CASE(testLowVolatility)
{
    ...
    auto check = [&](Real strike, Option::Type optionType, Real barrier, Barrier::Type barrierType, Real rebate, Rate r,
                     Rate q, Real expected)
    {
        R->setValue(r);
        Q->setValue(q);
...
BOOST_AUTO_TEST_CASE(testPerturbative)
{
    ...
    ext::shared_ptr<SimpleQuote> underlying = ext::make_shared<SimpleQuote>(S);
    ...
    ext::shared_ptr<BlackVolTermStructure> volTS = ext::make_shared<BlackVarianceCurve>(today, dates, vols, dc);
    ...
    ext::shared_ptr<PricingEngine> engine =
        ext::make_shared<PerturbativeBarrierOptionEngine>(stochProcess, order, zeroGamma);
    ...
    order = 1;
    engine = ext::make_shared<PerturbativeBarrierOptionEngine>(stochProcess, order, zeroGamma);
...
BOOST_AUTO_TEST_CASE(testVannaVolgaSimpleBarrierValues)
{
    ...
    BarrierFxOptionData values[] = {
        ...
        {Barrier::UpOut, 1.5, 0, Option::Call, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0.148127, 1.0e-4},
        {Barrier::UpOut, 1.5, 0, Option::Call, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0.075943, 1.0e-4},
        {Barrier::UpOut, 1.5, 0, Option::Call, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0.0274771, 1.0e-4},
        {Barrier::UpOut, 1.5, 0, Option::Call, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0.00573, 1.0e-4},
        {Barrier::UpOut, 1.5, 0, Option::Call, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0.00012, 1.0e-4},

        {Barrier::UpOut, 1.5, 0, Option::Put, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0.00697606, 1.0e-4},
        {Barrier::UpOut, 1.5, 0, Option::Put, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0.020078, 1.0e-4},
        {Barrier::UpOut, 1.5, 0, Option::Put, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0.0489395, 1.0e-4},
        {Barrier::UpOut, 1.5, 0, Option::Put, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0.0969877, 1.0e-4},
        {Barrier::UpOut, 1.5, 0, Option::Put, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0.157, 1.0e-4},

        {Barrier::UpIn, 1.5, 0, Option::Call, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0.0322202, 1.0e-4},
        {Barrier::UpIn, 1.5, 0, Option::Call, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0.0241491, 1.0e-4},
        {Barrier::UpIn, 1.5, 0, Option::Call, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0.0164275, 1.0e-4},
        {Barrier::UpIn, 1.5, 0, Option::Call, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0.01, 1.0e-4},
        {Barrier::UpIn, 1.5, 0, Option::Call, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0.00489, 1.0e-4},

        {Barrier::UpIn, 1.5, 0, Option::Put, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0.000560713, 1.0e-4},
        {Barrier::UpIn, 1.5, 0, Option::Put, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0.000546804, 1.0e-4},
        {Barrier::UpIn, 1.5, 0, Option::Put, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0.000130649, 1.0e-4},
        {Barrier::UpIn, 1.5, 0, Option::Put, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0.000300828, 1.0e-4},
        {Barrier::UpIn, 1.5, 0, Option::Put, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0.00135, 1.0e-4},

        {Barrier::DownOut, 1.1, 0, Option::Call, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0.17746, 1.0e-4},
        {Barrier::DownOut, 1.1, 0, Option::Call, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0.0994142, 1.0e-4},
        {Barrier::DownOut, 1.1, 0, Option::Call, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0.0439, 1.0e-4},
        {Barrier::DownOut, 1.1, 0, Option::Call, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0.01574, 1.0e-4},
        {Barrier::DownOut, 1.1, 0, Option::Call, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0.00501, 1.0e-4},

        {Barrier::DownOut, 1.3, 0, Option::Call, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0.00612, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Call, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0.00426, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Call, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0.00257, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Call, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0.00122, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Call, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0.00045, 1.0e-4},

        {Barrier::DownOut, 1.1, 0, Option::Put, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0.00022, 1.0e-4},
        {Barrier::DownOut, 1.1, 0, Option::Put, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0.00284, 1.0e-4},
        {Barrier::DownOut, 1.1, 0, Option::Put, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0.02032, 1.0e-4},
        {Barrier::DownOut, 1.1, 0, Option::Put, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0.058235, 1.0e-4},
        {Barrier::DownOut, 1.1, 0, Option::Put, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0.109432, 1.0e-4},

        {Barrier::DownOut, 1.3, 0, Option::Put, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Put, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Put, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Put, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0.00017, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Put, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0.00083, 1.0e-4},

        {Barrier::DownIn, 1.1, 0, Option::Call, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0.00289, 1.0e-4},
        {Barrier::DownIn, 1.1, 0, Option::Call, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0.00067784, 1.0e-4},
        {Barrier::DownIn, 1.1, 0, Option::Call, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0, 1.0e-4},
        {Barrier::DownIn, 1.1, 0, Option::Call, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0, 1.0e-4},
        {Barrier::DownIn, 1.1, 0, Option::Call, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0, 1.0e-4},

        {Barrier::DownIn, 1.3, 0, Option::Call, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0.17423, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Call, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0.09584, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Call, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0.04133, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Call, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0.01452, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Call, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0.00456, 1.0e-4},

        {Barrier::DownIn, 1.1, 0, Option::Put, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0.00732, 1.0e-4},
        {Barrier::DownIn, 1.1, 0, Option::Put, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0.01778, 1.0e-4},
        {Barrier::DownIn, 1.1, 0, Option::Put, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0.02875, 1.0e-4},
        {Barrier::DownIn, 1.1, 0, Option::Put, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0.0390535, 1.0e-4},
        {Barrier::DownIn, 1.1, 0, Option::Put, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0.0489236, 1.0e-4},

        {Barrier::DownIn, 1.3, 0, Option::Put, 1.13321, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.11638, 0.00753, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Put, 1.22687, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.10088, 0.02062, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Put, 1.31179, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08925, 0.04907, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Put, 1.38843, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08463, 0.09711, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Put, 1.46047, 1.30265, 0.0003541, 0.0033871, 1, 0.10087, 0.08925, 0.08463,
         0.08412, 0.15752, 1.0e-4},

        {Barrier::UpOut, 1.6, 0, Option::Call, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0.20493, 1.0e-4},
        {Barrier::UpOut, 1.6, 0, Option::Call, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.105577, 1.0e-4},
        {Barrier::UpOut, 1.6, 0, Option::Call, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0.0358872, 1.0e-4},
        {Barrier::UpOut, 1.6, 0, Option::Call, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.00634958, 1.0e-4},
        {Barrier::UpOut, 1.6, 0, Option::Call, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0, 1.0e-4},

        {Barrier::UpOut, 1.6, 0, Option::Put, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0.0108218, 1.0e-4},
        {Barrier::UpOut, 1.6, 0, Option::Put, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.0313339, 1.0e-4},
        {Barrier::UpOut, 1.6, 0, Option::Put, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0.0751237, 1.0e-4},
        {Barrier::UpOut, 1.6, 0, Option::Put, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.153407, 1.0e-4},
        {Barrier::UpOut, 1.6, 0, Option::Put, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0.253767, 1.0e-4},

        {Barrier::UpIn, 1.6, 0, Option::Call, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0.05402, 1.0e-4},
        {Barrier::UpIn, 1.6, 0, Option::Call, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.0410069, 1.0e-4},
        {Barrier::UpIn, 1.6, 0, Option::Call, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0.0279562, 1.0e-4},
        {Barrier::UpIn, 1.6, 0, Option::Call, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.0173055, 1.0e-4},
        {Barrier::UpIn, 1.6, 0, Option::Call, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0.00764, 1.0e-4},

        {Barrier::UpIn, 1.6, 0, Option::Put, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0.000962737, 1.0e-4},
        {Barrier::UpIn, 1.6, 0, Option::Put, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.00102637, 1.0e-4},
        {Barrier::UpIn, 1.6, 0, Option::Put, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0.000419834, 1.0e-4},
        {Barrier::UpIn, 1.6, 0, Option::Put, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.00159277, 1.0e-4},
        {Barrier::UpIn, 1.6, 0, Option::Put, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0.00473629, 1.0e-4},

        {Barrier::DownOut, 1, 0, Option::Call, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0.255098, 1.0e-4},
        {Barrier::DownOut, 1, 0, Option::Call, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.145701, 1.0e-4},
        {Barrier::DownOut, 1, 0, Option::Call, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0.06384, 1.0e-4},
        {Barrier::DownOut, 1, 0, Option::Call, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.02366, 1.0e-4},
        {Barrier::DownOut, 1, 0, Option::Call, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0.00764, 1.0e-4},

        {Barrier::DownOut, 1.3, 0, Option::Call, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0.00592, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Call, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.00421, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Call, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0.00256, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Call, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.0012, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Call, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0.0004, 1.0e-4},

        {Barrier::DownOut, 1, 0, Option::Put, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0, 1.0e-4},
        {Barrier::DownOut, 1, 0, Option::Put, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.00280549, 1.0e-4},
        {Barrier::DownOut, 1, 0, Option::Put, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0.0279945, 1.0e-4},
        {Barrier::DownOut, 1, 0, Option::Put, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.0896352, 1.0e-4},
        {Barrier::DownOut, 1, 0, Option::Put, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0.175182, 1.0e-4},

        {Barrier::DownOut, 1.3, 0, Option::Put, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0.00000, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Put, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.00000, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Put, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0.00000, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Put, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.0002, 1.0e-4},
        {Barrier::DownOut, 1.3, 0, Option::Put, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0.00096, 1.0e-4},

        {Barrier::DownIn, 1, 0, Option::Call, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0.00384783, 1.0e-4},
        {Barrier::DownIn, 1, 0, Option::Call, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.000883232, 1.0e-4},
        {Barrier::DownIn, 1, 0, Option::Call, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0, 1.0e-4},
        {Barrier::DownIn, 1, 0, Option::Call, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.00000, 1.0e-4},
        {Barrier::DownIn, 1, 0, Option::Call, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0.00000, 1.0e-4},

        {Barrier::DownIn, 1.3, 0, Option::Call, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0.25302, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Call, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.14238, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Call, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0.06128, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Call, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.02245, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Call, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0.00725, 1.0e-4},

        {Barrier::DownIn, 1, 0, Option::Put, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0.01178, 1.0e-4},
        {Barrier::DownIn, 1, 0, Option::Put, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.0295548, 1.0e-4},
        {Barrier::DownIn, 1, 0, Option::Put, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0.047549, 1.0e-4},
        {Barrier::DownIn, 1, 0, Option::Put, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.0653642, 1.0e-4},
        {Barrier::DownIn, 1, 0, Option::Put, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0.0833221, 1.0e-4},

        {Barrier::DownIn, 1.3, 0, Option::Put, 1.06145, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.12511, 0.01178, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Put, 1.19545, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.1089, 0.03236, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Put, 1.32238, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09444, 0.07554, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Put, 1.44298, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09197, 0.15479, 1.0e-4},
        {Barrier::DownIn, 1.3, 0, Option::Put, 1.56345, 1.30265, 0.0009418, 0.0039788, 2, 0.10891, 0.09525, 0.09197,
         0.09261, 0.25754, 1.0e-4},
    ...
    for (auto& value : values)
    {
        ...
        Handle<DeltaVolQuote> vol25PutQuote(Handle<DeltaVolQuote>(
            ext::make_shared<DeltaVolQuote>(-0.25, Handle<Quote>(vol25Put), value.t, DeltaVolQuote::Fwd)));

        Handle<DeltaVolQuote> vol25CallQuote(Handle<DeltaVolQuote>(
            ext::make_shared<DeltaVolQuote>(0.25, Handle<Quote>(vol25Call), value.t, DeltaVolQuote::Fwd)));

        BarrierOption barrierOption(value.barrierType, value.barrier, value.rebate, payoff, exercise);

        Real bsVanillaPrice =
            blackFormula(value.type, value.strike, spot->value() * qTS->discount(value.t) / rTS->discount(value.t),
                         value.v * std::sqrt(value.t), rTS->discount(value.t));
        ext::shared_ptr<PricingEngine> vannaVolgaEngine = ext::make_shared<VannaVolgaBarrierEngine>(
            volAtmQuote, vol25PutQuote, vol25CallQuote, Handle<Quote>(spot), Handle<YieldTermStructure>(rTS),
            Handle<YieldTermStructure>(qTS), true, bsVanillaPrice);
        ...
        if (error > value.tol)
        {
            REPORT_FX_FAILURE("value", value.barrierType, value.barrier, value.rebate, payoff, exercise, value.s,
                              value.q, value.r, today, value.vol25Put, value.volAtm, value.vol25Call, value.v, expected,
