...
CalibrationMarketData getDAXCalibrationMarketData()
{
    ...
    */
    ...
    Handle<YieldTermStructure> dividendYield(flatRate(settlementDate, 0.0, dayCounter));

    Volatility v[] = {0.6625, 0.4875, 0.4204, 0.3667, 0.3431, 0.3267, 0.3121, 0.3121, 0.6007, 0.4543, 0.3967, 0.3511,
                      0.3279, 0.3154, 0.2984, 0.2921, 0.5084, 0.4221, 0.3718, 0.3327, 0.3155, 0.3027, 0.2919, 0.2889,
                      0.4541, 0.3869, 0.3492, 0.3149, 0.2963, 0.2926, 0.2819, 0.2800, 0.4060, 0.3607, 0.3330, 0.2999,
                      0.2887, 0.2811, 0.2751, 0.2775, 0.3726, 0.3396, 0.3108, 0.2781, 0.2788, 0.2722, 0.2661, 0.2686,
                      0.3550, 0.3277, 0.3012, 0.2781, 0.2781, 0.2661, 0.2661, 0.2681, 0.3428, 0.3209, 0.2958, 0.2740,
                      0.2688, 0.2627, 0.2580, 0.2620, 0.3302, 0.3062, 0.2799, 0.2631, 0.2573, 0.2533, 0.2504, 0.2544,
                      0.3343, 0.2959, 0.2705, 0.2540, 0.2504, 0.2464, 0.2448, 0.2462, 0.3460, 0.2845, 0.2624, 0.2463,
                      0.2425, 0.2385, 0.2373, 0.2422, 0.3857, 0.2860, 0.2578, 0.2399, 0.2357, 0.2327, 0.2312, 0.2351,
                      0.3976, 0.2860, 0.2607, 0.2356, 0.2297, 0.2268, 0.2241, 0.2320};

    Handle<Quote> s0(ext::make_shared<SimpleQuote>(4468.17));
    Real strike[] = {3400, 3600, 3800, 4000, 4200, 4400, 4500, 4600, 4800, 5000, 5200, 5400, 5600};
    ...
    for (Size s = 0; s < 13; ++s)
    {
        for (Size m = 0; m < 8; ++m)
        {
            ...
            options.push_back(ext::make_shared<HestonModelHelper>(maturity, calendar, s0, strike[s], vol, riskFreeTS,
                                                                  dividendYield,
                                   ...
                                   const std::string& method)
{
    ...
    const ext::shared_ptr<AnalyticHestonEngine> engine =
        ext::make_shared<AnalyticHestonEngine>(model, formula, integration, 1e-9);
    ...
    if (std::isnan(error) || error > tol)
    {
        BOOST_ERROR("failed to reproduce simple Heston Pricing with "
                    << "\n    integration method: " << method << std::setprecision(12) << "\n    expected          : "
                    << expected << "\n    calculated        : " << calculated << "\n    error             : " << error);
    }

    if (valuations != Null<Size>() && valuations != engine->numberOfEvaluations())
    {
        BOOST_ERROR("nubmer of function evaluations does not match "
                    << "\n    integration method      : " << method << "\n    expected function calls : " << valuations
                    << "\n    number of function calls: " << engine->numberOfEvaluations());
    }
}
...
class LogCharacteristicFunction
{
  public:
    ...
    Real operator()(Real u) const { return (std::log(engine_->chF(u, t_)) / alpha_).real(); }
...
class HestonIntegrationMaxBoundTestFct
{
  public:
    ...
    : maxBound_(maxBound), callCounter_(ext::make_shared<Size>(Size(0)))
    {
    }
    ...
    Size getCallCounter() const { return *callCounter_; }
...
{
    BOOST_TEST_MESSAGE("Testing Heston model calibration using a flat volatility surface...");
    ...
    for (auto& optionMaturitie : optionMaturities)
    {
        ...
        {
            const Time tau = dayCounter.yearFraction(riskFreeTS->referenceDate(),
                                                     calendar.advance(riskFreeTS->referenceDate(), optionMaturitie));
            const Real fwdPrice = s0->value() * dividendTS->discount(tau) / riskFreeTS->discount(tau);
            const Real strikePrice = fwdPrice * std::exp(-moneyness * volatility * std::sqrt(tau));

            options.push_back(ext::make_shared<HestonModelHelper>(optionMaturitie, calendar, s0, strikePrice, vol,
    ...
    for (Real sigma = 0.1; sigma < 0.7; sigma += 0.2)
    {
        ...
        ext::shared_ptr<PricingEngine> engine(ext::make_shared<AnalyticHestonEngine>(model, 96));
        ...
        if (std::fabs(model->kappa() * (model->theta() - volatility * volatility)) > tolerance)
        {
...
BOOST_AUTO_TEST_CASE(testDAXCalibration)
{
    ...
    BOOST_TEST_MESSAGE("Testing Heston model calibration using DAX volatility data...");
    ...
    const ext::shared_ptr<PricingEngine> engines[] = {ext::make_shared<AnalyticHestonEngine>(model, 64),
...
BOOST_AUTO_TEST_CASE(testAnalyticVsBlack)
{
    ...
    ext::shared_ptr<PricingEngine> engine(
        ext::make_shared<AnalyticHestonEngine>(ext::make_shared<HestonModel>(process), 144));
    ...
    Real expected = blackFormula(payoff->optionType(), payoff->strike(), forwardPrice, std::sqrt(0.05 * yearFraction)) *
    ...
    engine = ext::make_shared<FdHestonVanillaEngine>(ext::make_shared<HestonModel>(process), 200, 200, 100);
    option.setPricingEngine(engine);
...
BOOST_AUTO_TEST_CASE(testAnalyticVsCached)
{
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(ext::make_shared<PlainVanillaPayoff>(Option::Call, 1.05));
    ext::shared_ptr<Exercise> exercise(ext::make_shared<EuropeanExercise>(exerciseDate));
    ...
    ext::shared_ptr<AnalyticHestonEngine> engine(
        ext::make_shared<AnalyticHestonEngine>(ext::make_shared<HestonModel>(process), 64));
    ...
    for (i = 0; i < 6; ++i)
    {
        ...
        ext::shared_ptr<StrikedTypePayoff> payoff(ext::make_shared<PlainVanillaPayoff>(Option::Call, K[i % 3]));
        ext::shared_ptr<Exercise> exercise(ext::make_shared<EuropeanExercise>(exerciseDate));
        ...
        ext::shared_ptr<HestonProcess> process(
            ext::make_shared<HestonProcess>(riskFreeTS, dividendTS, s0, 0.09, 1.2, 0.08, 1.8, -0.45));

        VanillaOption option(payoff, exercise);

        ext::shared_ptr<PricingEngine> engine(
            ext::make_shared<AnalyticHestonEngine>(ext::make_shared<HestonModel>(process)));
    ...
    for (i = 0; i < 3; ++i)
    {
        const Real interpolated = calculated2[i] + (calculated2[i + 3] - calculated2[i]) / (t2 - t1) * (0.7 - t1);
...
{
    BOOST_TEST_MESSAGE("Testing Monte Carlo Heston engine against cached values...");
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(ext::make_shared<PlainVanillaPayoff>(Option::Put, 1.05));
    ext::shared_ptr<Exercise> exercise(ext::make_shared<EuropeanExercise>(exerciseDate));
    ...
    ext::shared_ptr<HestonProcess> process(ext::make_shared<HestonProcess>(
        riskFreeTS, dividendTS, s0, 0.3, 1.16, 0.2, 0.8, 0.8, HestonProcess::QuadraticExponentialMartingale));
...
BOOST_AUTO_TEST_CASE(testFdBarrierVsCached, *precondition(if_speed(Fast)))
{
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(ext::make_shared<PlainVanillaPayoff>(Option::Call, 90.0));

    ext::shared_ptr<HestonProcess> process(
        ext::make_shared<HestonProcess>(rTS, qTS, s0, 0.25 * 0.25, 1.0, 0.25 * 0.25, 0.001, 0.0));

    ext::shared_ptr<PricingEngine> engine;
    engine = ext::make_shared<FdHestonBarrierEngine>(ext::make_shared<HestonModel>(process), 200, 400, 100);
...
BOOST_AUTO_TEST_CASE(testFdVanillaVsCached)
{
    ...
    ext::shared_ptr<HestonProcess> process(
        ext::make_shared<HestonProcess>(riskFreeTS, dividendTS, s0, 0.3, 1.16, 0.2, 0.8, 0.8));

    option.setPricingEngine(
        MakeFdHestonVanillaEngine(ext::make_shared<HestonModel>(process)).withTGrid(100).withXGrid(200).withVGrid(100));
...
BOOST_AUTO_TEST_CASE(testFdVanillaWithDividendsVsCached)
{
    ...
    for (Date d = settlementDate + 3 * Months; d < exercise->lastDate(); d += 6 * Months)
    {
        dividendDates.push_back(d);
        dividends.push_back(1.0);
    }

    auto process = ext::make_shared<HestonProcess>(riskFreeTS, dividendTS, s0, 0.04, 1.0, 0.04, 0.001, 0.0);
...
BOOST_AUTO_TEST_CASE(testFdAmerican)
{
    ...
    auto exerciseDate = Date(28, March, 2006);
    auto exercise = ext::make_shared<AmericanExercise>(settlementDate, exerciseDate);
    auto option = VanillaOption(payoff, exercise);
    option.setPricingEngine(
        MakeFdHestonVanillaEngine(ext::make_shared<HestonModel>(process)).withTGrid(200).withXGrid(400).withVGrid(100));
    Real calculated = option.NPV();

    Handle<BlackVolTermStructure> volTS(flatVol(settlementDate, 0.2, dayCounter));
    ...
    ext::shared_ptr<PricingEngine> ref_engine(ext::make_shared<FdBlackScholesVanillaEngine>(ref_process, 200, 400));
...
{
    BOOST_TEST_MESSAGE("Testing MC and FD Heston engines for the Kahl-Jaeckel example...");
    ...
    */
    ...
    const ext::shared_ptr<StrikedTypePayoff> payoff(ext::make_shared<PlainVanillaPayoff>(Option::Call, 200));
    ...
    for (const auto& description : descriptions)
    {
        ...
        const ext::shared_ptr<PricingEngine> engine = MakeMCEuropeanHestonEngine<PseudoRandom>(process)
    ...
    option.setPricingEngine(MakeMCEuropeanHestonEngine<LowDiscrepancy>(
                                ext::make_shared<HestonProcess>(riskFreeTS, dividendTS, s0, v0, kappa, theta, sigma,
                                                                rho, HestonProcess::BroadieKayaExactSchemeLaguerre))
                                .withSteps(1)
                                .withSamples(1023));
    ...
    option.setPricingEngine(ext::make_shared<FdHestonVanillaEngine>(hestonModel, 200, 401, 101));
    ...
    option.setPricingEngine(ext::make_shared<AnalyticHestonEngine>(hestonModel, 1e-6, 1000));
    ...
    option.setPricingEngine(ext::make_shared<COSHestonEngine>(hestonModel, 16, 400));
    calculated = option.NPV();
    error = std::fabs(calculated - expected);
    ...
    option.setPricingEngine(ext::make_shared<ExponentialFittingHestonEngine>(hestonModel));
    calculated = option.NPV();
    error = std::fabs(calculated - expected);
...
{
    BOOST_TEST_MESSAGE("Testing different numerical Heston integration algorithms...");
    ...
    std::vector<HestonParameter> params = {equityfx, highCorr, lowVolOfVol, highVolOfVol, kappaEqSigRho};
    ...
    for (auto iter = params.begin(); iter != params.end(); ++iter)
    {
        ...
        Handle<Quote> s0(ext::make_shared<SimpleQuote>(1.0));
        ext::shared_ptr<HestonProcess> process(ext::make_shared<HestonProcess>(
            riskFreeTS, dividendTS, s0, iter->v0, iter->kappa, iter->theta, iter->sigma, iter->rho));

        ext::shared_ptr<HestonModel> model(ext::make_shared<HestonModel>(process));

        ext::shared_ptr<AnalyticHestonEngine> lobattoEngine(
            ext::make_shared<AnalyticHestonEngine>(model, 1e-10, 1000000));
        ext::shared_ptr<AnalyticHestonEngine> laguerreEngine(ext::make_shared<AnalyticHestonEngine>(model, 128));
        ext::shared_ptr<AnalyticHestonEngine> legendreEngine(ext::make_shared<AnalyticHestonEngine>(
            model, AnalyticHestonEngine::Gatheral, AnalyticHestonEngine::Integration::gaussLegendre(512)));
        ext::shared_ptr<AnalyticHestonEngine> chebyshevEngine(ext::make_shared<AnalyticHestonEngine>(
            model, AnalyticHestonEngine::Gatheral, AnalyticHestonEngine::Integration::gaussChebyshev(512)));
        ext::shared_ptr<AnalyticHestonEngine> chebyshev2ndEngine(ext::make_shared<AnalyticHestonEngine>(
            model, AnalyticHestonEngine::Gatheral, AnalyticHestonEngine::Integration::gaussChebyshev2nd(512)));
        ...
        for (int maturitie : maturities)
        {
            ...
            for (Real strike : strikes)
            {
                for (auto type : types)
                {
                    ...
                    maxLaguerreDiff = std::max(maxLaguerreDiff, std::fabs(lobattoNPV - laguerre));
                    maxLegendreDiff = std::max(maxLegendreDiff, std::fabs(lobattoNPV - legendre));
                    maxChebyshevDiff = std::max(maxChebyshevDiff, std::fabs(lobattoNPV - chebyshev));
                    maxChebyshev2ndDiff = std::max(maxChebyshev2ndDiff, std::fabs(lobattoNPV - chebyshev2nd));
                }
            }
        }
        const Real maxDiff = std::max({maxLaguerreDiff, maxLegendreDiff, maxChebyshevDiff, maxChebyshev2ndDiff});
...
BOOST_AUTO_TEST_CASE(testMultipleStrikesEngine)
{
    ...
    ext::shared_ptr<HestonProcess> process(
        ext::make_shared<HestonProcess>(riskFreeTS, dividendTS, s0, 0.16, 2.5, 0.09, 0.8, -0.8));
    ext::shared_ptr<HestonModel> model(ext::make_shared<HestonModel>(process));
...
BOOST_AUTO_TEST_CASE(testAnalyticPiecewiseTimeDependent)
{
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(ext::make_shared<PlainVanillaPayoff>(Option::Call, 1.0));
    ext::shared_ptr<Exercise> exercise(ext::make_shared<EuropeanExercise>(exerciseDate));

    std::vector<Date> dates = {settlementDate, {1, January, 2007}};
    std::vector<Rate> irates = {0.0, 0.2};
    Handle<YieldTermStructure> riskFreeTS(ext::make_shared<ZeroCurve>(dates, irates, dayCounter));

    std::vector<Rate> qrates = {0.0, 0.3};
    Handle<YieldTermStructure> dividendTS(ext::make_shared<ZeroCurve>(dates, qrates, dayCounter));
    ...
    ext::shared_ptr<PiecewiseTimeDependentHestonModel> model = ext::make_shared<PiecewiseTimeDependentHestonModel>(
        riskFreeTS, dividendTS, s0, v0, theta, kappa, sigma, rho, TimeGrid(20.0, 2));

    VanillaOption option(payoff, exercise);

    ext::shared_ptr<HestonProcess> hestonProcess(
        ext::make_shared<HestonProcess>(riskFreeTS, dividendTS, s0, v0, kappa(0.0), theta(0.0), sigma(0.0), rho(0.0)));
    ext::shared_ptr<HestonModel> hestonModel = ext::make_shared<HestonModel>(hestonProcess);
    option.setPricingEngine(ext::make_shared<AnalyticHestonEngine>(hestonModel));

    const Real expected = option.NPV();

    option.setPricingEngine(ext::shared_ptr<PricingEngine>(new AnalyticPTDHestonEngine(model, 192)));
    ...
    option.setPricingEngine(ext::shared_ptr<PricingEngine>(
        new AnalyticPTDHestonEngine(model, AnalyticPTDHestonEngine::AndersenPiterbarg,
...
{
    BOOST_TEST_MESSAGE("Testing time-dependent Heston model calibration...");
    ...
    ext::shared_ptr<PiecewiseTimeDependentHestonModel> model = ext::make_shared<PiecewiseTimeDependentHestonModel>(
        riskFreeTS, dividendTS, s0, v0, theta, kappa, sigma, rho, modelGrid);

    const ext::shared_ptr<PricingEngine> engines[] = {
        ext::make_shared<AnalyticPTDHestonEngine>(model),
        ext::make_shared<AnalyticPTDHestonEngine>(model, AnalyticPTDHestonEngine::AndersenPiterbarg,
...
BOOST_AUTO_TEST_CASE(testAlanLewisReferencePrices)
{
    ...
    const ext::shared_ptr<PricingEngine> laguerreEngine(ext::make_shared<AnalyticHestonEngine>(model, 128U));

    const ext::shared_ptr<PricingEngine> gaussLobattoEngine(
        ext::make_shared<AnalyticHestonEngine>(model, QL_EPSILON, 100000U));

    const ext::shared_ptr<PricingEngine> cosEngine(ext::make_shared<COSHestonEngine>(model, 20, 400));

    const ext::shared_ptr<PricingEngine> exponentialFittingEngine(
        ext::make_shared<ExponentialFittingHestonEngine>(model));

    const ext::shared_ptr<PricingEngine> andersenPiterbargEngine(new AnalyticHestonEngine(
        model, AnalyticHestonEngine::AndersenPiterbarg,
        AnalyticHestonEngine::Integration::gaussLobatto(Null<Real>(), 1e-14, 1000000), QL_EPSILON));

    const ext::shared_ptr<PricingEngine> angledContourEngine(new AnalyticHestonEngine(
        model, AnalyticHestonEngine::AngledContour,
        AnalyticHestonEngine::Integration::gaussLobatto(Null<Real>(), 1e-14, 1000000), QL_EPSILON));

    const ext::shared_ptr<PricingEngine> optimalCvEngine(new AnalyticHestonEngine(
    ...
    const ext::shared_ptr<PricingEngine> engines[] = {
        ...
        exponentialFittingEngine, angledContourEngine, optimalCvEngine};
    ...
    for (Size i = 0; i < std::size(strikes); ++i)
    {
        ...
        for (Size j = 0; j < std::size(types); ++j)
        {
            ...
            for (Size k = 0; k < std::size(engines); ++k)
            {
                ...
                {
                    BOOST_ERROR("failed to reproduce Alan Lewis Reference prices "
                                << "\n    strike     : " << strike << "\n    option type: " << type
                                << "\n    engine type: " << k << "\n    rel. error : " << relError);
                }
            }
...
BOOST_AUTO_TEST_CASE(testExpansionOnAlanLewisReference)
{
    ...
    const ext::shared_ptr<PricingEngine> lpp2Engine =
        ext::make_shared<HestonExpansionEngine>(model, HestonExpansionEngine::LPP2);
    // don't test Forde as it does not behave well on this example
    const ext::shared_ptr<PricingEngine> lpp3Engine =
        ext::make_shared<HestonExpansionEngine>(model, HestonExpansionEngine::LPP3);

    const Real strikes[] = {80, 90, 100, 110, 120};
    const Option::Type types[] = {Option::Put, Option::Call};
    const ext::shared_ptr<PricingEngine> engines[] = {lpp2Engine, lpp3Engine};
...
BOOST_AUTO_TEST_CASE(testExpansionOnFordeReference)
{
    ...
    const Real referenceVols[][7] = {{0.27284673574924445, 0.22360758200372477, 0.21023988547031242, 0.1990674789471587,
    ...
    const Real tol[][4] = {
        {0.06, 0.03, 0.03, 0.02}, {0.15, 0.08, 0.04, 0.02}, {0.06, 0.08, 1.0, 1.0}
        // forde breaks down for long maturities
    };
    const Real tolAtm[][4] = {{4e-6, 7e-4, 2e-3, 9e-4}, {7e-6, 4e-4, 9e-4, 4e-4}, {4e-4, 3e-2, 0.28, 1.0}};
    for (Size j = 0; j < std::size(terms); ++j)
    {
        const Real term = terms[j];
        const ext::shared_ptr<HestonExpansion> lpp2 =
            ext::make_shared<LPP2HestonExpansion>(kappa, theta, sigma, v0, rho, term);
        const ext::shared_ptr<HestonExpansion> lpp3 =
            ext::make_shared<LPP3HestonExpansion>(kappa, theta, sigma, v0, rho, term);
        const ext::shared_ptr<HestonExpansion> forde =
            ext::make_shared<FordeHestonExpansion>(kappa, theta, sigma, v0, rho, term);
        ...
        for (Size i = 0; i < std::size(strikes); ++i)
        {
            ...
            for (Size k = 0; k < std::size(expansions); ++k)
            {
                ...
                if (relError > refTol)
                {
                    BOOST_ERROR("failed to reproduce Forde reference vols " << "\n    strike        : " << strike
                                                                            << "\n    expansion type: " << k
                                                                            << "\n    rel. error    : " << relError);
                }
            }
...
BOOST_AUTO_TEST_CASE(testAllIntegrationMethods)
{
    ...
    const ext::shared_ptr<StrikedTypePayoff> payoff = ext::make_shared<PlainVanillaPayoff>(Option::Put, s0->value());
    ...
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussLaguerre(),
                                  AnalyticHestonEngine::Gatheral, false, expected, tol, 256,
                                  "Gauss-Laguerre with Gatheral logarithm");

    // Gauss-Laguerre with branch correction integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussLaguerre(),
                                  AnalyticHestonEngine::BranchCorrection, false, expected, tol, 256,
                                  "Gauss-Laguerre with branch correction");

    // Gauss-Laguerre with Andersen-Piterbarg integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussLaguerre(),
                                  AnalyticHestonEngine::AndersenPiterbarg, false, expected, tol, 128,
                                  "Gauss-Laguerre with Andersen Piterbarg control variate");

    // Gauss-Legendre with Gatheral logarithm integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussLegendre(),
                                  AnalyticHestonEngine::Gatheral, false, expected, tol, 256,
    ...
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussLegendre(256),
                                  AnalyticHestonEngine::AndersenPiterbarg, false, expected, 1e-4, 256,
                                  "Gauss-Legendre with Andersen Piterbarg control variate");

    // Gauss-Chebyshev with Gatheral logarithm integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussChebyshev(512),
                                  AnalyticHestonEngine::Gatheral, false, expected, 1e-4, 1024,
                                  "Gauss-Chebyshev with Gatheral logarithm");

    // Gauss-Chebyshev with branch correction integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussChebyshev(512),
                                  AnalyticHestonEngine::BranchCorrection, false, expected, 1e-4, 1024,
                                  "Gauss-Chebyshev with branch correction");

    // Gauss-Chebyshev with Andersen-Piterbarg integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussChebyshev(512),
                                  AnalyticHestonEngine::AndersenPiterbarg, false, expected, 1e-4, 512,
                                  "Gauss-Laguerre with Andersen Piterbarg control variate");

    // Gauss-Chebyshev2nd with Gatheral logarithm integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussChebyshev2nd(512),
                                  AnalyticHestonEngine::Gatheral, false, expected, 2e-4, 1024,
                                  "Gauss-Chebyshev2nd with Gatheral logarithm");

    // Gauss-Chebyshev with branch correction integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussChebyshev2nd(512),
                                  AnalyticHestonEngine::BranchCorrection, false, expected, 2e-4, 1024,
                                  "Gauss-Chebyshev2nd with branch correction");

    // Gauss-Chebyshev with Andersen-Piterbarg integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussChebyshev2nd(512),
                                  AnalyticHestonEngine::AndersenPiterbarg, false, expected, 2e-4, 512,
                                  "Gauss-Chebyshev2nd with Andersen Piterbarg control variate");

    // Discrete Simpson rule with Gatheral logarithm
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::discreteSimpson(512),
                                  AnalyticHestonEngine::Gatheral, false, expected, tol, 1024,
                                  "Discrete Simpson rule with Gatheral logarithm");

    // Discrete Simpson rule with Andersen-Piterbarg integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::discreteSimpson(64),
                                  AnalyticHestonEngine::AndersenPiterbarg, false, expected, tol, 64,
                                  "Discrete Simpson rule with Andersen Piterbarg control variate");

    // Discrete Trapezoid rule with Gatheral logarithm
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::discreteTrapezoid(512),
                                  AnalyticHestonEngine::Gatheral, false, expected, 2e-4, 1024,
                                  "Discrete Trapezoid rule with Gatheral logarithm");

    // Discrete Trapezoid rule with Andersen-Piterbarg integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::discreteTrapezoid(64),
                                  AnalyticHestonEngine::AndersenPiterbarg, false, expected, tol, 64,
                                  "Discrete Trapezoid rule with Andersen Piterbarg control variate");

    // Gauss-Lobatto with Gatheral logarithm
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussLobatto(tol, Null<Real>()),
                                  AnalyticHestonEngine::Gatheral, true, expected, tol, Null<Size>(),
                                  "Gauss-Lobatto with Gatheral logarithm");

    // Gauss-Lobatto with Andersen-Piterbarg integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussLobatto(tol, Null<Real>()),
                                  AnalyticHestonEngine::AndersenPiterbarg, true, expected, tol, Null<Size>(),
                                  "Gauss-Lobatto with Andersen Piterbarg control variate");

    // Gauss-Konrod with Gatheral logarithm
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussKronrod(tol),
                                  AnalyticHestonEngine::Gatheral, true, expected, tol, Null<Size>(),
                                  "Gauss-Konrod with Gatheral logarithm");
    ...
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::simpson(tol),
                                  AnalyticHestonEngine::Gatheral, true, expected, 1e-6, Null<Size>(),
                                  "Simpson with Gatheral logarithm");

    // Simpson with Andersen-Piterbarg integration method
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::simpson(tol),
                                  AnalyticHestonEngine::AndersenPiterbarg, true, expected, 1e-6, Null<Size>(),
                                  "Simpson with Andersen Piterbarg control variate");

    // Trapezoid with Gatheral logarithm
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::trapezoid(tol),
                                  AnalyticHestonEngine::Gatheral, true, expected, 1e-6, Null<Size>(),
                                  "Trapezoid with Gatheral logarithm");
    ...
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussLaguerre(),
                                  AnalyticHestonEngine::AngledContour, false, expected, tol, 128,
                                  "Angled contour shift integral");

    // Angled contour shift integral w/o control variate
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::gaussLaguerre(192),
                                  AnalyticHestonEngine::AngledContourNoCV, false, expected, tol, 192,
    ...
    reportOnIntegrationMethodTest(option, model, AnalyticHestonEngine::Integration::expSinh(),
                                  AnalyticHestonEngine::AngledContour, true, expected, 1e-8, Null<Size>(),
...
BOOST_AUTO_TEST_CASE(testCosHestonCumulants)
{
    ...
    const ext::shared_ptr<COSHestonEngine> cosEngine = ext::make_shared<COSHestonEngine>(model);

    const Real tol = 1e-7;
    const NumericalDifferentiation::Scheme central(NumericalDifferentiation::Central);

    for (Time t = 0.01; t < 41.0; t += t)
    {
        const Real nc1 = NumericalDifferentiation(std::function<Real(Real)>(LogCharacteristicFunction(1, t, cosEngine)),
        ...
        const Real nc2 = NumericalDifferentiation(std::function<Real(Real)>(LogCharacteristicFunction(2, t, cosEngine)),
        ...
        const Real nc3 = NumericalDifferentiation(std::function<Real(Real)>(LogCharacteristicFunction(3, t, cosEngine)),
        ...
        const Real nc4 = NumericalDifferentiation(std::function<Real(Real)>(LogCharacteristicFunction(4, t, cosEngine)),
...
BOOST_AUTO_TEST_CASE(testCosHestonEngine)
{
    ...
    const ext::shared_ptr<PricingEngine> cosEngine(ext::make_shared<COSHestonEngine>(model, 25, 600));
    ...
    const Real expected[] = {9.364410588426075, 0.01036797658132471, 5.319092971836708, 0.01032681906278383};
...
BOOST_AUTO_TEST_CASE(testCosHestonEngineTruncation)
{
    ...
    Handle<YieldTermStructure> riskFreeTS(
        ext::shared_ptr<YieldTermStructure>(new FlatForward(todaysDate, riskFreeRate, dayCounter)));
    Handle<YieldTermStructure> dividendTS(
        ext::shared_ptr<YieldTermStructure>(new FlatForward(todaysDate, dividendYield, dayCounter)));
    ...
    europeanOption.setPricingEngine(ext::shared_ptr<PricingEngine>(new COSHestonEngine(hestonModel)));
...
BOOST_AUTO_TEST_CASE(testAndersenPiterbargPricing)
{
    ...
    const ext::shared_ptr<AnalyticHestonEngine> andersenPiterbargLaguerreEngine(ext::make_shared<AnalyticHestonEngine>(
        model, AnalyticHestonEngine::AndersenPiterbarg, AnalyticHestonEngine::Integration::gaussLaguerre()));

    const ext::shared_ptr<AnalyticHestonEngine> andersenPiterbargLobattoEngine(ext::make_shared<AnalyticHestonEngine>(
        model, AnalyticHestonEngine::AndersenPiterbarg,
        AnalyticHestonEngine::Integration::gaussLobatto(Null<Real>(), 1e-9, 10000), 1e-9));

    const ext::shared_ptr<AnalyticHestonEngine> andersenPiterbargSimpsonEngine(ext::make_shared<AnalyticHestonEngine>(
        model, AnalyticHestonEngine::AndersenPiterbarg, AnalyticHestonEngine::Integration::discreteSimpson(256), 1e-8));

    const ext::shared_ptr<AnalyticHestonEngine> andersenPiterbargTrapezoidEngine(
        ext::make_shared<AnalyticHestonEngine>(model, AnalyticHestonEngine::AndersenPiterbarg,
                                               AnalyticHestonEngine::Integration::discreteTrapezoid(164), 1e-8));

    const ext::shared_ptr<AnalyticHestonEngine> andersenPiterbargTrapezoidEngine2(
        ext::make_shared<AnalyticHestonEngine>(model, AnalyticHestonEngine::AndersenPiterbarg,
                                               AnalyticHestonEngine::Integration::trapezoid(1e-8, 256), 1e-8));

    const ext::shared_ptr<ExponentialFittingHestonEngine> andersenPiterbargExponentialFittingEngine(
        ext::make_shared<ExponentialFittingHestonEngine>(model));

    const ext::shared_ptr<PricingEngine> engines[] = {
        andersenPiterbargLaguerreEngine,  andersenPiterbargLobattoEngine,    andersenPiterbargSimpsonEngine,
        andersenPiterbargTrapezoidEngine, andersenPiterbargTrapezoidEngine2, andersenPiterbargExponentialFittingEngine};

    const std::string algos[] = {"Gauss-Laguerre",     "Gauss-Lobatto", "Discrete Simpson",
                                 "Discrete Trapezoid", "Trapezoid",     "Exponential Fitting"};

    const ext::shared_ptr<PricingEngine> analyticEngine(ext::make_shared<AnalyticHestonEngine>(model, 192));

    const Date maturityDates[] = {settlementDate + Period(1, Days), settlementDate + Period(1, Weeks),
                                  settlementDate + Period(1, Years), settlementDate + Period(10, Years)};
    ...
    for (auto maturityDate : maturityDates)
    {
        ...
        for (auto optionType : optionTypes)
        {
            for (Real strike : strikes)
            {
                VanillaOption option(ext::make_shared<PlainVanillaPayoff>(optionType, strike), exercise);
...
BOOST_AUTO_TEST_CASE(testAndersenPiterbargControlVariateIntegrand)
{
    ...
    const ext::shared_ptr<HestonModel> hestonModel(
        ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(rTS, qTS, s0, v0, kappa, theta, sigma, rho)));

    const ext::shared_ptr<COSHestonEngine> cosEngine(ext::make_shared<COSHestonEngine>(hestonModel));

    const ext::shared_ptr<AnalyticHestonEngine> engine(ext::make_shared<AnalyticHestonEngine>(
        hestonModel, AnalyticHestonEngine::AndersenPiterbarg, AnalyticHestonEngine::Integration::gaussLaguerre()));
    ...
    const Volatility implStdDev = blackFormulaImpliedStdDev(Option::Call, strike, fwd, refNPV, df);

    const Real var = cosEngine->var(maturity);
    const Real stdDev = std::sqrt(var);

    const Real d = (std::log(s0->value() / strike) + (r - q) * maturity + 0.5 * var) / stdDev;
    ...
    const Real variances[] = {
        v0 * maturity, ((1 - std::exp(-kappa * maturity)) * (v0 - theta) / (kappa * maturity) + theta) * maturity,
        ...
        squared(blackFormulaImpliedStdDev(Option::Call, strike, fwd, bsNPV + skew * q3, df)),
        squared(blackFormulaImpliedStdDev(Option::Call, strike, fwd, bsNPV + skew * q3 + kurt * q4, df)),
        // Moment matching based on
        // Rubinstein M., (1998), “Edgeworth Binomial Trees”,
        // Journal of Derivatives 5 (3), 20-27.
        squared(
            blackFormulaImpliedStdDev(Option::Call, strike, fwd, bsNPV + skew * q3 + kurt * q4 + skew * skew * q5, df)),
    ...
    for (Size i = 0; i < std::size(variances); ++i)
    {
        ...
        for (Real u = 0.001; u < 15; u *= 1.05)
        {
            ...
            const std::complex<Real> phiBS =
                std::exp(-0.5 * sigmaBS * sigmaBS * maturity * (z * z + std::complex<Real>(-z.imag(), z.real())));

            const std::complex<Real> ex = std::exp(std::complex<Real>(0.0, u * (dd - sx)));
...
BOOST_AUTO_TEST_CASE(testAndersenPiterbargConvergence)
{
    ...
    VanillaOption option(ext::make_shared<PlainVanillaPayoff>(Option::Call, s0->value()),
                         ext::make_shared<EuropeanExercise>(maturityDate));
    ...
    const Real diffs[] = {0.0892433814611486298, 0.00013096156482816923, 1.34107015270501506e-07,
                          1.22913235145460931e-10, 1.24344978758017533e-13};

    for (Size n = 10; n <= 50; n += 10)
    {
        option.setPricingEngine(
            ext::make_shared<AnalyticHestonEngine>(hestonModel, AnalyticHestonEngine::AndersenPiterbarg,
...
BOOST_AUTO_TEST_CASE(testPiecewiseTimeDependentChFvsHestonChF)
{
    ...
    const ext::shared_ptr<AnalyticHestonEngine> analyticEngine(ext::make_shared<AnalyticHestonEngine>(
        ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(rTS, qTS, s0, v0, kappa, theta, sigma, rho))));

    const ext::shared_ptr<AnalyticPTDHestonEngine> ptdHestonEngine(
        ext::make_shared<AnalyticPTDHestonEngine>(ext::make_shared<PiecewiseTimeDependentHestonModel>(
            rTS, qTS, s0, v0, thetaP, kappaP, sigmaP, rhoP,
            TimeGrid(dayCounter.yearFraction(settlementDate, maturityDate), 10))));
    ...
    for (Real r = 0.1; r < 4; r += 0.25)
    {
        for (Real phi = 0; phi < 360; phi += 60)
        {
            ...
            {
                const std::complex<Real> z = r * std::exp(std::complex<Real>(0, phi));
...
BOOST_AUTO_TEST_CASE(testPiecewiseTimeDependentComparison)
{
    ...
    const ext::shared_ptr<PiecewiseTimeDependentHestonModel> ptdModel(
        ext::make_shared<PiecewiseTimeDependentHestonModel>(rTS, qTS, s0, v0, theta, kappa, sigma, rho, modelGrid));

    const ext::shared_ptr<AnalyticPTDHestonEngine> ptdHestonEngine(ext::make_shared<AnalyticPTDHestonEngine>(ptdModel));

    option.setPricingEngine(ptdHestonEngine);
    const Real calculatedGatheral = option.NPV();

    const ext::shared_ptr<AnalyticPTDHestonEngine> ptdAPEngine(
        ext::make_shared<AnalyticPTDHestonEngine>(ptdModel, AnalyticPTDHestonEngine::AndersenPiterbarg,
                                                  AnalyticPTDHestonEngine::Integration::discreteTrapezoid(128), 1e-12));
    ...
    if (std::fabs(calculatedGatheral - calculatedAndersenPiterbarg) > 1e-10)
        BOOST_ERROR("failed to reproduce npv for time dependent Heston model "
                    ...
                    << "\n  Difference          : " << std::fabs(calculatedGatheral - calculatedAndersenPiterbarg));

    const ext::shared_ptr<HestonProcess> firstPartProcess(ext::make_shared<HestonProcess>(
        rTS, qTS, s0, v0, 1.0, 0.1, 0.30, -0.75, HestonProcess::QuadraticExponentialMartingale));

    typedef PseudoRandom::rsg_type rsg_type;
    typedef PseudoRandom::urng_type urng_type;
    typedef MultiPathGenerator<rsg_type>::sample_type sample_type;

    const MultiPathGenerator<rsg_type> firstPathGen(firstPartProcess, TimeGrid(pTimes.front(), 6),
    ...
    for (Size i = 0; i < nSims; ++i)
    {
        ...
        for (Size j = 0; j < 2; ++j)
        {
            const sample_type& path1 = (j & 1) != 0U ? firstPathGen.antithetic() : firstPathGen.next();
            const Real spot1 = path1.value[0].back();
            const Real v1 = path1.value[1].back();

            const MultiPathGenerator<rsg_type> secondPathGen(
                ext::make_shared<HestonProcess>(rTS, qTS, Handle<Quote>(ext::make_shared<SimpleQuote>(spot1)), v1, 1.0,
                                                0.1, 0.15, -0.75, HestonProcess::QuadraticExponentialMartingale),
                TimeGrid(pTimes[1] - pTimes[0], 12), PseudoRandom::make_sequence_generator(24, urng.nextInt32()));
            ...
            const MultiPathGenerator<rsg_type> thirdPathGen(
                ext::make_shared<HestonProcess>(rTS, qTS, Handle<Quote>(ext::make_shared<SimpleQuote>(spot2)), v2, 1.0,
                                                0.1, 1.25, -0.75, HestonProcess::QuadraticExponentialMartingale),
                TimeGrid(maturity - pTimes[1], 6), PseudoRandom::make_sequence_generator(12, urng.nextInt32()));
...
BOOST_AUTO_TEST_CASE(testPiecewiseTimeDependentChFAsymtotic)
{
    ...
    const Handle<Quote> s0(ext::make_shared<SimpleQuote>(100.0));
    const ext::shared_ptr<PiecewiseTimeDependentHestonModel> ptdModel(
        ext::make_shared<PiecewiseTimeDependentHestonModel>(rTS, rTS, s0, v0, theta, kappa, sigma, rho, modelGrid));

    const Real eps = 1e-8;

    const ext::shared_ptr<AnalyticPTDHestonEngine> ptdHestonEngine(
        ext::make_shared<AnalyticPTDHestonEngine>(ptdModel, AnalyticPTDHestonEngine::AndersenPiterbarg,
                                                  AnalyticPTDHestonEngine::Integration::discreteTrapezoid(128), eps));

    const std::complex<Real> D_u_inf = -std::complex<Real>(std::sqrt(1 - rhos[0] * rhos[0]), rhos[0]) / sigmas[0];

    const std::complex<Real> dd =
        std::complex<Real>(kappas[0], (2 * kappas[0] * rhos[0] - sigmas[0]) / (2 * std::sqrt(1 - rhos[0] * rhos[0]))) /
        (sigmas[0] * sigmas[0]);
    ...
    for (Size i = 0; i < 3; ++i)
    {
        ...
        C_u_inf += -kappa * theta * tau / sigma * std::complex<Real>(std::sqrt(1 - rho * rho), rho);

        cc += kappa * std::complex<Real>(2 * kappa, (2 * kappa * rho - sigma) / sqrt(1 - rho * rho)) * tau * theta /
              (2 * sigma * sigma);

        const std::complex<Real> Di =
            (i < 2) ?
                sigma / sigmas[i + 1] * std::complex<Real>(std::sqrt(1 - rhos[i + 1] * rhos[i + 1]), rhos[i + 1]) :
                std::complex<Real>(0.0, 0.0);

        clog += 2 * kappa * theta / (sigma * sigma) *
                std::log(1.0 - (Di - std::complex<Real>(std::sqrt(1 - rho * rho), rho)) /
                                   (Di + std::complex<Real>(std::sqrt(1 - rho * rho), -rho)));
    }

    const Real epsilon = eps * M_PI / s0->value();

    const Real uM = AnalyticHestonEngine::Integration::andersenPiterbargIntegrationLimit(
    ...
    const std::complex<Real> calculatedAsympotic = (D_u_inf * u + dd) * v0 + C_u_inf * u + cc + clog;
    ...
    if (std::abs(expectedlnChF - calculatedAsympotic) > 0.01)
    {
        BOOST_ERROR("failed to reproduce asymptotic of characteristic function"
                    ...
                    << "\n  diff      : " << std::abs(expectedlnChF - calculatedAsympotic)
                    << "\n  tolerance : " << 0.01);
    }
...
BOOST_AUTO_TEST_CASE(testSmallSigmaExpansion)
{
    ...
    const ext::shared_ptr<HestonModel> hestonModel =
        ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(rTS, rTS, spot, v0, kappa, theta, sigma, rho));

    const ext::shared_ptr<AnalyticHestonEngine> engine = ext::make_shared<AnalyticHestonEngine>(hestonModel);

    const std::complex<Real> expectedChF(0.990463578538352651, 2.60693475987521132e-12);

    const std::complex<Real> calculatedChF = engine->chF(std::complex<Real>(0.55, -0.5), t);
    ...
    VanillaOption option(ext::make_shared<PlainVanillaPayoff>(Option::Call, 120.0),
                         ext::make_shared<EuropeanExercise>(maturityDate));

    option.setPricingEngine(ext::make_shared<AnalyticHestonEngine>(
        hestonModel, AnalyticHestonEngine::AndersenPiterbarg, AnalyticHestonEngine::Integration::gaussLaguerre(192)));

    const Real calculatedNPV = option.NPV();

    const Real stdDev = std::sqrt(((1 - std::exp(-kappa * t)) * (v0 - theta) / (kappa * t) + theta) * t);

    const Real expectedNPV = blackFormula(Option::Call, 120.0, 100.0, stdDev);
...
BOOST_AUTO_TEST_CASE(testSmallSigmaExpansion4ExpFitting)
{
    ...
    const Real fwd = spot->value() * qTS->discount(maturity) / rTS->discount(maturity);
    ...
    const Real expected = blackFormula(Option::Call, strike, fwd, std::sqrt(v0 * maturity), rTS->discount(maturity));
    ...
    {
        option.setPricingEngine(ext::make_shared<ExponentialFittingHestonEngine>(ext::make_shared<HestonModel>(
            ...
            for (Real theta : thetas)
            {
                for (Real v0 : v0s)
                {
                    const ext::shared_ptr<PricingEngine> engine =
                        ext::make_shared<ExponentialFittingHestonEngine>(ext::make_shared<HestonModel>(
                            ext::make_shared<HestonProcess>(rTS, qTS, spot, v0, kappa, theta, 1e-13, -0.8)));

                    const Real stdDev =
                        std::sqrt(((1 - std::exp(-kappa * t)) * (v0 - theta) / (kappa * t) + theta) * t);

                    for (Real strike = spot->value() * exp(-10 * stdDev); strike < spot->value() * exp(10 * stdDev);
                         strike *= 1.2)
                    {
                        ...
                        const Real expected = blackFormula(optionType, strike, fwd, stdDev, df);
                        ...
                        optionType = (optionType == Option::Call) ? Option::Put : Option::Call;
                    }
                }
            }
        }
...
BOOST_AUTO_TEST_CASE(testExponentialFitting4StrikesAndMaturities)
{
    ...
    const Period maturities[] = {Period(1, Days), Period(1, Months), Period(1, Years), Period(10, Years)};
    ...
    const Real referenceValues[] = {
        1.1631865252540813e-58,  1.06426822273258466e-49, 6.92896489110422086e-16, 8.19515526286263236e-06,
        0.000625608178476390504, 0.00417261379371945684,  0.000625608178476390504, 8.19515526286263236e-06,
        1.92308901296741414e-10, 1.57327901822368115e-23, 5.7830515043285098e-58,  3.56081886910098813e-48,
        2.9489071194212509e-23,  1.54181757781090727e-11, 0.000367960011879847279, 0.00493886106106039818,
        0.0227152343265593776,   0.00493886106106039818,  0.000367960011879847279, 3.06653474407784574e-06,
        8.86665241279348934e-11, 1.51206812371708868e-20, 4.18506719865401643e-29, 2.46637786897559908e-15,
        1.75338784910563671e-08, 0.00284789176080218294,  0.0199133097064688458,   0.0776848755698912041,
        0.0199133097064688458,   0.00284789176080218294,  0.00012462190796343504,  2.59755319566692257e-07,
        1.13853114743124721e-12, 4.27612073892114211e-39, 1.08387452075906664e-25, 4.15179522944463802e-11,
        0.00134157732880653131,  0.029018582813884912,    0.176405213088554197,    0.029018582813884912,
        0.00134157732880653131,  5.43674074281991917e-06, 6.51443921040230507e-11, 9.25756999394709285e-21};

    const ext::shared_ptr<HestonModel> model =
        ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(rTS, qTS, s0, v0, kappa, theta, sigma, rho));

    const ext::shared_ptr<PricingEngine> engine = ext::make_shared<ExponentialFittingHestonEngine>(model);
    ...
    for (auto maturitie : maturities)
    {
        ...
        {
            const Real strike = std::exp(-moneyness[j] * std::sqrt(theta * t)) * fwd;

            for (Size k = 0; k < 2; ++k)
            {
                const ext::shared_ptr<PlainVanillaPayoff> payoff =
                    ext::make_shared<PlainVanillaPayoff>((k) != 0U ? Option::Put : Option::Call, strike);
...
BOOST_AUTO_TEST_CASE(testHestonEngineIntegration)
{
    ...
    const AnalyticHestonEngine::Integration integration = AnalyticHestonEngine::Integration::gaussLobatto(1e-12, 1e-12);
    ...
    if (testFct.getCallCounter() == 0 || std::fabs(c1 - 1 / 3.) > 1e-10 || std::fabs(c2 - 1 / 3.) > 1e-10)
    {
...
{
    BOOST_TEST_MESSAGE("Testing optimal control variate choice for the Heston model...");
...
BOOST_AUTO_TEST_CASE(testAsymptoticControlVariate)
{
    ...
    const ext::shared_ptr<HestonModel> model =
        ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(rTS, rTS, s0, v0, kappa, theta, sigma, rho));
    ...
    const Real expected[] = {0.0074676425640918,    0.008680823863233695,  0.010479611906112223, 0.023590088942038945,
                             0.0019575784806211706, 0.0005490310253748906, 0.0001657118753134695};

    const ext::shared_ptr<PricingEngine> engines[] = {
        ext::make_shared<AnalyticHestonEngine>(model, AnalyticHestonEngine::OptimalCV,
    ...
    for (Size j = 0; j < std::size(engines); ++j)
    {
        for (Size i = 0; i < std::size(moneynesses); ++i)
        {
            ...
            const ext::shared_ptr<PlainVanillaPayoff> payoff =
                ext::make_shared<PlainVanillaPayoff>(strike > 1.0 ? Option::Call : Option::Put, strike);
            ...
            if ((analyticHestonEngine != nullptr) && analyticHestonEngine->numberOfEvaluations() > 5000)
            {
...
BOOST_AUTO_TEST_CASE(testLocalVolFromHestonModel)
{
    ...
    const Handle<YieldTermStructure> rTS(
        ext::make_shared<ZeroCurve>(std::vector<Date>{todaysDate, todaysDate + Period(90, Days),
                                                      todaysDate + Period(180, Days), todaysDate + Period(1, Years)},
                                    std::vector<Rate>{0.075, 0.05, 0.075, 0.1}, dc));

    const Handle<YieldTermStructure> qTS(ext::make_shared<ZeroCurve>(
        std::vector<Date>{todaysDate, todaysDate + Period(90, Days), todaysDate + Period(1, Years)},
        std::vector<Rate>{0.06, 0.04, 0.12}, dc));
    ...
    option.setPricingEngine(ext::make_shared<AnalyticHestonEngine>(
        hestonModel, AnalyticHestonEngine::OptimalCV, AnalyticHestonEngine::Integration::gaussLaguerre(192)));
    ...
    option.setPricingEngine(ext::make_shared<FdBlackScholesVanillaEngine>(
        ext::make_shared<BlackScholesMertonProcess>(
            s0, qTS, rTS,
            Handle<BlackVolTermStructure>(ext::make_shared<HestonBlackVolSurface>(
                Handle<HestonModel>(hestonModel), AnalyticHestonEngine::OptimalCV,
...
BOOST_AUTO_TEST_CASE(testOptimalAlphaKmin)
{
    ...
    const DayCounter dc = Actual365Fixed();
    const auto model = ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(
        Handle<YieldTermStructure>(flatRate(0.0, dc)), Handle<YieldTermStructure>(flatRate(0.0, dc)),
        Handle<Quote>(ext::make_shared<SimpleQuote>(150)), 0.01, 0.1, 0.01, 2.0, 0.8));

    const auto engine = ext::make_shared<AnalyticHestonEngine>(
        model, AnalyticHestonEngine::Gatheral,
        AnalyticHestonEngine::Integration::gaussLobatto(Null<Real>(), 1e-12, 100000));

    const Real strike = 100;

    const Real alphaStar = AnalyticHestonEngine::OptimalAlpha(1.0, engine.get()).alphaSmallerMinusOne(strike).first;
    ...
    option.setPricingEngine(ext::make_shared<AnalyticHestonEngine>(
        model, AnalyticHestonEngine::AngledContour,
        AnalyticHestonEngine::Integration::gaussLobatto(Null<Real>(), 1e-12, 5000)));
...
BOOST_AUTO_TEST_CASE(testOptimalAlphaKmax)
{
    ...
    const DayCounter dc = Actual365Fixed();
    const Handle<YieldTermStructure> yTS = Handle<YieldTermStructure>(flatRate(0.0, dc));
    const Handle<Quote> spot = Handle<Quote>(ext::make_shared<SimpleQuote>(75));
    ...
    auto model =
        ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(yTS, yTS, spot, .1, 1.2, 0.2, 0.2, -0.8));
    auto engine = ext::make_shared<AnalyticHestonEngine>(model);
    Real alphaStar = AnalyticHestonEngine::OptimalAlpha(T, engine.get()).alphaGreaterZero(strike).first;
    QL_CHECK_SMALL(alphaStar - 3.22615, 1e-4);

    // case 2: kappa - sigma*rho < 0, T < t_cut
    model = ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(yTS, yTS, spot, 0.1, 1.2, 0.2, 1.5, 0.9));
    engine = ext::make_shared<AnalyticHestonEngine>(model);
    alphaStar = AnalyticHestonEngine::OptimalAlpha(T, engine.get()).alphaGreaterZero(strike).first;
    QL_CHECK_SMALL(alphaStar - 0.31137, 1e-4);

    // case 3: kappa - sigma*rho < 0, T >= t_cut
    model = ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(yTS, yTS, spot, 0.1, 1.2, 0.2, 2.25, 0.9));
    engine = ext::make_shared<AnalyticHestonEngine>(model);
    alphaStar = AnalyticHestonEngine::OptimalAlpha(T, engine.get()).alphaGreaterZero(strike).first;
    QL_CHECK_SMALL(alphaStar - 0.11940, 1e-4);

    // case 4: kappa - sigma*rho == 0
    model = ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(yTS, yTS, spot, 0.1, 1.0, 0.2, 2.0, 0.5));
    engine = ext::make_shared<AnalyticHestonEngine>(model);
    alphaStar = AnalyticHestonEngine::OptimalAlpha(T, engine.get()).alphaGreaterZero(strike).first;
...
BOOST_AUTO_TEST_CASE(testAnalyticPDFHestonEngine)
{
    ...
    const Real tol = 1e-6;
    const ext::shared_ptr<AnalyticPDFHestonEngine> pdfEngine(ext::make_shared<AnalyticPDFHestonEngine>(model, tol));

    const ext::shared_ptr<PricingEngine> analyticEngine(ext::make_shared<AnalyticHestonEngine>(model, 178));
    ...
    for (Real strike = 40; strike < 190; strike += 20)
    {
        ...
        {
            BOOST_FAIL("failed to reproduce plain vanilla european prices with"
    ...
    {
        VanillaOption digitalOption(ext::make_shared<CashOrNothingPayoff>(Option::Call, strike, 1.0), exercise);
        digitalOption.setPricingEngine(pdfEngine);
        const Real calculated = digitalOption.NPV();

        const Real eps = 0.01;
        VanillaOption longCall(ext::make_shared<PlainVanillaPayoff>(Option::Call, strike - eps), exercise);
        longCall.setPricingEngine(analyticEngine);

        VanillaOption shortCall(ext::make_shared<PlainVanillaPayoff>(Option::Call, strike + eps), exercise);
        shortCall.setPricingEngine(analyticEngine);

        const Real expected = (longCall.NPV() - shortCall.NPV()) / (2 * eps);
        if (std::fabs(calculated - expected) > tol)
        {
            BOOST_FAIL("failed to reproduce european digital prices with"
                       " the analytic probability density engine"
                       << "\n    strike     : " << strike << "\n    expected   : " << expected
                       << "\n    calculated : " << calculated
                       << "\n    diff       : " << std::fabs(calculated - expected) << "\n    tol        : " << tol);
        ...
        {
            BOOST_FAIL("failed to reproduce cumulative distribution function"
