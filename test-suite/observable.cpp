...
{ // NOLINT(cppcoreguidelines-special-member-functions)
  public:
    ~RestoreUpdates() { ObservableSettings::instance().enableUpdates(); }
};
...
{
  public:
    MTUpdateCounter() : counter_(0) { ++instanceCounter_; }
    ~MTUpdateCounter() { --instanceCounter_; }
...
BOOST_AUTO_TEST_CASE(testMultiThreadingGlobalSettings)
{
    ...
    for (local_list_type::iterator iter = localList.begin(); iter != localList.end(); ++iter)
    {
        if ((*iter)->counter() != 0)
        {
...
BOOST_AUTO_TEST_CASE(testDeepUpdate)
{
    ...
    Handle<YieldTermStructure> yts(ext::make_shared<FlatForward>(0, NullCalendar(), 0.02, Actual365Fixed()));
    ...
    std::vector<std::vector<Handle<Quote>>> quotes = {{Handle<Quote>(q), Handle<Quote>(q)},
...
BOOST_AUTO_TEST_CASE(testAddAndDeleteObserverDuringNotifyObservers)
{
    ...
    const ext::shared_ptr<MersenneTwisterUniformRng> rng = ext::make_shared<MersenneTwisterUniformRng>();
      ...
      public:
        ...
        void update() override
        {
            ...
            if (setup_ != nullptr)
            {
                ...
                {
                    const ext::shared_ptr<Observer> obs = ext::make_shared<TestObserver>();
                ...
                {
                    const unsigned int j = setup_->rng->nextInt32() % setup_->expected.size();
    ...
    for (Size t = 0; t < testRuns; ++t)
    {
        ...
        {
            const ext::shared_ptr<Observer> obs = (i == nrInitialObserver / 3 || i == nrInitialObserver / 2) ?
...
BOOST_AUTO_TEST_CASE(testDeferredObserverLifetime)
{
    ...
    auto zciHelper = ext::make_shared<ZeroCouponInflationSwapHelper>(
        quote, 3 * Months, Date(29, Dec, 2026), TARGET(), ModifiedFollowing, ActualActual(ActualActual::ISDA),
        ext::make_shared<EUHICPXT>(), CPI::Flat);
