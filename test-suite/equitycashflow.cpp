...
struct CommonVars
{
    ...
    ext::shared_ptr<EquityCashFlow> createEquityQuantoCashFlow(const ext::shared_ptr<EquityIndex>& index,
                                                               const Date& start,
                                                               const Date& end,
                                                               bool useQuantoPricer = true)
    {
        ...
        if (useQuantoPricer)
        {
            auto pricer = ext::make_shared<EquityQuantoCashFlowPricer>(quantoCcyInterestHandle, equityVolHandle,
                                                                       fxVolHandle, correlationHandle);
            cf->setPricer(pricer);
        }
        return cf;
    }
...
void checkQuantoCorrection(bool includeDividend, bool bumpData = false)
{
    ...
    ext::shared_ptr<EquityIndex> equityIndex =
        includeDividend ?
            vars.equityIndex :
            vars.equityIndex->clone(vars.localCcyInterestHandle, Handle<YieldTermStructure>(), vars.spotHandle);
...
BOOST_AUTO_TEST_CASE(testErrorWhenInconsistentMarketDataReferenceDate)
{
    ...
    checkRaisedError(cf, "Quanto currency term structure, equity and FX volatility need to have the same "
                         "reference date.");
}
