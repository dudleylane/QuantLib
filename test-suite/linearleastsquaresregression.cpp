...
BOOST_AUTO_TEST_CASE(testRegression)
{
    ...
    std::vector<std::function<Real(Real)>> v = {[](Real x) -> Real { return 1.0; }, [](Real x) -> Real { return x; },
    ...
    {
        Size i;
        const Real a[] = {rng.next().value, rng.next().value, rng.next().value, rng.next().value};
        ...
        for (i = 0; i < nr; ++i)
        {
            ...
            y[i] = a[0] * v[0](x[i]) + a[1] * v[1](x[i]) + a[2] * v[2](x[i]) + a[3] * v[3](x[i]) + rng.next().value;
        }
        ...
        const Real ma[] = {m.coefficients()[0], m.coefficients()[1], m.coefficients()[2] + m.coefficients()[4],
                           m.coefficients()[3]};
        const Real err[] = {
            m.standardErrors()[0], m.standardErrors()[1],
            std::sqrt(m.standardErrors()[2] * m.standardErrors()[2] + m.standardErrors()[4] * m.standardErrors()[4]),
            m.standardErrors()[3]};
        for (i = 0; i < v.size(); ++i)
        {
...
struct get_item
{
    ...
    Real operator()(const Array& a) const { return a[i]; }
};
...
BOOST_AUTO_TEST_CASE(testMultiDimRegression)
{
    ...
    BOOST_TEST_MESSAGE("Testing multi-dimensional linear least-squares regression...");
...
BOOST_AUTO_TEST_CASE(test1dLinearRegression)
{
    ...
    std::vector<std::function<Real(Real)>> v = {[](Real x) { return 1.0; }, [](Real x) { return x; }};
