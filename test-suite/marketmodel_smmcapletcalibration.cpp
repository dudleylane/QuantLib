...
BOOST_AUTO_TEST_CASE(testFunction)
{
    ...
    CTSMMCapletOriginalCalibration calibrator(evolution, corr, swapVariances, capletVols_, cs, displacement_, alpha,
                                              lowestRoot, useFullApprox);
    // calibrate
    ...
    bool result =
        calibrator.calibrate(numberOfFactors_, maxIterations, capletTolerance / 10, innerMaxIterations, innerTolerance);
    if (!result)
