...
BOOST_AUTO_TEST_CASE(testIntervalPrice)
{
    ...
    std::vector<Real> open = {1.3, 2.3}, close = {2.3, 3.4}, high = {3.4, 3.5}, low = {3.4, 3.2};

    TimeSeries<IntervalPrice> tsiq = IntervalPrice::makeSeries(date, open, close, high, low);
}
...
BOOST_AUTO_TEST_CASE(testIteratingDefaultContainer)
{
    ...
    const std::vector<Date> dates = {Date(25, March, 2005), Date(29, March, 2005), Date(15, March, 2005)};
...
BOOST_AUTO_TEST_CASE(testCustomContainer)
{
    ...
    // unordered container
    typedef TimeSeries<int, boost::unordered_map<Date, int>> TimeSeriesUnordered;
