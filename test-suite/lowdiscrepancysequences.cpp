...
BOOST_AUTO_TEST_CASE(testPolynomialsModuloTwo)
{
    ...
    BOOST_TEST_MESSAGE("Testing " << PPMT_MAX_DIM << " primitive polynomials modulo two...");

    const Size jj[] = {1,     1,     2,     2,      6,      6,      18,      16,      48,
    ...
    while (n < PPMT_MAX_DIM || polynomial != -1)
    {
        ...
        if (polynomial == -1)
        {
            ...
            if (j != jj[i])
            {
                BOOST_ERROR("Only " << j << " polynomials in degree " << i + 1 << " instead of " << jj[i]);
            }
        }
...
namespace
{
    void testRandomizedLatticeRule(LatticeRule::type name, const std::string& nameString)
    {
        Size maxDim = 30;
        Size N = 1024;
        Size numberBatches = 32;

        BOOST_TEST_MESSAGE("Testing randomized lattice sequences (" << nameString << ") up to dimension " << maxDim
                                                                    << "...");

        std::vector<Real> z;

        LatticeRule::getRule(name, z, N);
        LatticeRsg latticeGenerator(maxDim, z, N);

        unsigned long seed = 12345678UL;
        MersenneTwisterUniformRng rng(seed);

        RandomSequenceGenerator<MersenneTwisterUniformRng> rsg(maxDim, rng);
...
BOOST_AUTO_TEST_CASE(testSobol)
{
    ...
    BOOST_TEST_MESSAGE("Testing Sobol sequences up to dimension " << PPMT_MAX_DIM << "...");
    ...
    for (i = 0; i < points; i++)
    {
        ...
        if (point.size() != dimensionality)
        {
            BOOST_ERROR("Sobol sequence generator returns "
                        ...
                        << point.size() << " instead of  " << dimensionality);
        }
    }
        ...
        for (i = 0; i < dimensionality; i++)
        {
            ...
            if (error > tolerance)
            {
                BOOST_ERROR(io::ordinal(i + 1)
                            << " dimension: " << std::fixed << "mean (" << mean[i] << ") at the end of the "
                            << io::ordinal(j + 1) << " cycle in Sobol sequence is not " << 0.5 << std::scientific
                            << " (error = " << error << ")");
            }
        }
    }
    ...
    const Real vanderCorputSequenceModuloTwo[] = {
        ...
        // fifth cycle
        0.09375, 0.59375, 0.84375, 0.34375, 0.46875, 0.96875, 0.71875, 0.21875, 0.15625, 0.65625, 0.90625, 0.40625,
    ...
    for (i = 0; i < points; i++)
    {
        ...
        {
            BOOST_ERROR(io::ordinal(i + 1)
                        << " draw (" << std::fixed << point[0] << ") in 1-D Sobol sequence is not in the "
                        << "van der Corput sequence modulo two: "
                        << "it should have been " << vanderCorputSequenceModuloTwo[i] << std::scientific
                        << " (error = " << error << ")");
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testFaure)
{
    ...
    for (i = 0; i < points; i++)
    {
        ...
        {
            BOOST_ERROR(io::ordinal(i + 1)
                        << " draw, dimension 1 (" << std::fixed << point[0]
                        << ") in 3-D Faure sequence should have been " << vanderCorputSequenceModuloTwo[i]
    ...
    const Real FaureDimensionTwoOfTwo[] = {// first cycle (zero excluded)
                                           0.50000,
                                           // second cycle
                                           0.25000, 0.75000,
                                           // third cycle
    ...
    for (i = 0; i < points; i++)
    {
        ...
        if (error > tolerance)
        {
            BOOST_ERROR(io::ordinal(i + 1) << " draw, dimension 2 (" << std::fixed << point[1]
                                           << ") in 3-D Faure sequence should have been " << FaureDimensionTwoOfTwo[i]
                                           << std::scientific << " (error = " << error << ")");
        }
    }
    ...
    const Real FaureDimensionOneOfThree[] = {// first cycle (zero excluded)
                                             1.0 / 3, 2.0 / 3,
                                             // second cycle
                                             7.0 / 9, 1.0 / 9, 4.0 / 9, 5.0 / 9, 8.0 / 9, 2.0 / 9};
    const Real FaureDimensionTwoOfThree[] = {// first cycle (zero excluded)
                                             1.0 / 3, 2.0 / 3,
                                             // second cycle
                                             1.0 / 9, 4.0 / 9, 7.0 / 9, 2.0 / 9, 5.0 / 9, 8.0 / 9};
    const Real FaureDimensionThreeOfThree[] = {// first cycle (zero excluded)
                                               1.0 / 3, 2.0 / 3,
                                               // second cycle
    ...
    for (i = 0; i < points; i++)
    {
        ...
        if (error > tolerance)
        {
            BOOST_ERROR(io::ordinal(i + 1) << " draw, dimension 1 (" << std::fixed << point[0]
                                           << ") in 3-D Faure sequence should have been " << FaureDimensionOneOfThree[i]
                                           << std::scientific << " (error = " << error << ")");
        }
        ...
        {
            BOOST_ERROR(io::ordinal(i + 1) << " draw, dimension 2 (" << std::fixed << point[1]
                                           << ") in 3-D Faure sequence should have been " << FaureDimensionTwoOfThree[i]
                                           << std::scientific << " (error = " << error << ")");
        }
        ...
        {
            BOOST_ERROR(io::ordinal(i + 1)
                        << " draw, dimension 3 (" << std::fixed << point[2]
                        << ") in 3-D Faure sequence should have been " << FaureDimensionThreeOfThree[i]
                        << std::scientific << " (error = " << error << ")");
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testHalton)
{
    ...
    for (i = 0; i < points; i++)
    {
        ...
        {
            BOOST_ERROR(io::ordinal(i + 1)
                        << " draw (" << std::fixed << point[0] << ") in 1-D Halton sequence is not in the "
                        << "van der Corput sequence modulo two: "
                        << "it should have been " << vanderCorputSequenceModuloTwo[i] << std::scientific
                        << " (error = " << error << ")");
        }
    ...
    const Real vanderCorputSequenceModuloThree[] = {// first cycle (zero excluded)
                                                    1.0 / 3, 2.0 / 3,
                                                    // second cycle
                                                    1.0 / 9, 4.0 / 9, 7.0 / 9, 2.0 / 9, 5.0 / 9, 8.0 / 9,
                                                    // third cycle
    ...
    for (i = 0; i < points; i++)
    {
        ...
        if (error > tolerance)
        {
            BOOST_ERROR("First component of " << io::ordinal(i + 1) << " draw (" << std::fixed << point[0]
                                              << ") in 2-D Halton sequence is not in the "
                                              << "van der Corput sequence modulo two: "
                                              << "it should have been " << vanderCorputSequenceModuloTwo[i]
                                              << std::scientific << " (error = " << error << ")");
        }
        error = std::fabs(point[1] - vanderCorputSequenceModuloThree[i]);
        if (error > tolerance)
        {
            BOOST_ERROR("Second component of " << io::ordinal(i + 1) << " draw (" << std::fixed << point[1]
                                               << ") in 2-D Halton sequence is not in the "
                                               << "van der Corput sequence modulo three: "
                                               << "it should have been " << vanderCorputSequenceModuloThree[i]
                                               << std::scientific << " (error = " << error << ")");
        }
    }
    ...
    {                                        // five cycle
        ...
        if (error > tolerance)
        {
            BOOST_ERROR("First dimension mean (" << std::fixed << mean[0] << ") at the end of the "
                                                 << io::ordinal(j + 1) << " cycle in Halton sequence is not " << 0.5
                                                 << std::scientific << " (error = " << error << ")");
        }
    }
    ...
    {                                        // three cycle
        ...
        if (error > tolerance)
        {
            BOOST_ERROR("Second dimension mean (" << std::fixed << mean[1] << ") at the end of the "
                                                  << io::ordinal(j + 1) << " cycle in Halton sequence is not " << 0.5
                                                  << std::scientific << " (error = " << error << ")");
        }
    }
...
const Real dim002DiscrMersenneTwis[] = {8.84e-003, 5.42e-003, 5.23e-003, 4.47e-003, 4.75e-003, 3.11e-003, 2.97e-003};
const Real dim002DiscrPlain_Halton[] = {1.26e-003, 6.73e-004, 3.35e-004, 1.91e-004, 1.11e-004, 5.05e-005, 2.42e-005};
...
const Real dim003Discr_Sobol[] = {1.21e-003, 6.37e-004, 3.40e-004, 1.75e-004, 9.21e-005, 4.79e-005, 2.56e-005};
const Real dim003DiscrMersenneTwis[] = {7.02e-003, 4.94e-003, 4.82e-003, 4.91e-003, 3.33e-003, 2.80e-003, 2.62e-003};
const Real dim003DiscrPlain_Halton[] = {1.63e-003, 9.62e-004, 4.83e-004, 2.67e-004, 1.41e-004, 7.64e-005, 3.93e-005};
...
const Real dim003Discr_Unit_Sobol[] = {1.21e-003, 6.37e-004, 3.40e-004, 1.75e-004, 9.21e-005, 4.79e-005, 2.56e-005};

const Real dim005Discr_Sobol[] = {1.59e-003, 9.55e-004, 5.33e-004, 3.22e-004, 1.63e-004, 9.41e-005, 5.19e-005};
const Real dim005DiscrMersenneTwis[] = {4.28e-003, 3.48e-003, 2.48e-003, 1.98e-003, 1.57e-003, 1.39e-003, 6.33e-004};
const Real dim005DiscrPlain_Halton[] = {1.93e-003, 1.23e-003, 6.89e-004, 4.22e-004, 2.13e-004, 1.25e-004, 7.17e-005};
...
const Real dim005Discr_Unit_Sobol[] = {1.85e-003, 9.39e-004, 5.19e-004, 2.99e-004, 1.75e-004, 9.51e-005, 5.55e-005};

const Real dim010DiscrJackel_Sobol[] = {7.08e-004, 5.31e-004, 3.60e-004, 2.18e-004, 1.57e-004, 1.12e-004, 6.39e-005};
const Real dim010DiscrSobLev_Sobol[] = {7.01e-004, 5.10e-004, 3.28e-004, 2.21e-004, 1.57e-004, 1.08e-004, 6.38e-005};
const Real dim010DiscrMersenneTwis[] = {8.83e-004, 6.56e-004, 4.87e-004, 3.37e-004, 3.06e-004, 1.73e-004, 1.43e-004};
const Real dim010DiscrPlain_Halton[] = {1.23e-003, 6.89e-004, 4.03e-004, 2.83e-004, 1.61e-004, 1.08e-004, 6.69e-005};
...
const Real dim010Discr_Unit_Sobol[] = {7.67e-004, 4.92e-004, 3.47e-004, 2.34e-004, 1.39e-004, 9.47e-005, 5.72e-005};

const Real dim015DiscrJackel_Sobol[] = {1.59e-004, 1.23e-004, 7.73e-005, 5.51e-005, 3.91e-005, 2.73e-005, 1.96e-005};
const Real dim015DiscrSobLev_Sobol[] = {1.48e-004, 1.06e-004, 8.19e-005, 6.29e-005, 4.16e-005, 2.54e-005, 1.73e-005};
const Real dim015DiscrMersenneTwis[] = {1.63e-004, 1.12e-004, 8.36e-005, 6.09e-005, 4.34e-005, 2.95e-005, 2.10e-005};
const Real dim015DiscrPlain_Halton[] = {5.75e-004, 3.12e-004, 1.70e-004, 9.89e-005, 5.33e-005, 3.45e-005, 2.11e-005};
...
const Real dim015Discr_Unit_Sobol[] = {2.24e-004, 1.39e-004, 9.86e-005, 6.02e-005, 4.39e-005, 3.06e-005, 2.32e-005};

const Real dim030DiscrJackel_Sobol[] = {6.43e-007, 5.28e-007, 3.88e-007, 2.49e-007, 2.09e-007, 1.55e-007, 1.07e-007};
const Real dim030DiscrSobLev_Sobol[] = {1.03e-006, 6.06e-007, 3.81e-007, 2.71e-007, 2.68e-007, 1.73e-007, 1.21e-007};
const Real dim030DiscrMersenneTwis[] = {4.38e-007, 3.25e-007, 4.47e-007, 2.85e-007, 2.03e-007, 1.50e-007, 1.17e-007};
const Real dim030DiscrPlain_Halton[] = {4.45e-004, 2.23e-004, 1.11e-004, 5.56e-005, 2.78e-005, 1.39e-005, 6.95e-006};
...
const Real dim030Discr_Unit_Sobol[] = {4.35e-005, 2.17e-005, 1.09e-005, 5.43e-006, 2.73e-006, 1.37e-006, 6.90e-007};

const Real dim050DiscrJackel_Sobol[] = {2.98e-010, 2.91e-010, 2.62e-010, 1.53e-010, 1.48e-010, 1.15e-010, 8.41e-011};
const Real dim050DiscrSobLev_Sobol[] = {3.11e-010, 2.52e-010, 1.61e-010, 1.54e-010, 1.11e-010, 8.60e-011, 1.17e-010};
const Real dim050DiscrSobLem_Sobol[] = {4.57e-010, 6.84e-010, 3.68e-010, 2.20e-010, 1.81e-010, 1.14e-010, 8.31e-011};
const Real dim050DiscrMersenneTwis[] = {3.27e-010, 2.42e-010, 1.47e-010, 1.98e-010, 2.31e-010, 1.30e-010, 8.09e-011};
const Real dim050DiscrPlain_Halton[] = {4.04e-004, 2.02e-004, 1.01e-004, 5.05e-005, 2.52e-005, 1.26e-005, 6.31e-006};
...
const Real dim050Discr_Unit_Sobol[] = {1.63e-005, 8.14e-006, 4.07e-006, 2.04e-006, 1.02e-006, 5.09e-007, 2.54e-007};

const Real dim100DiscrJackel_Sobol[] = {1.26e-018, 1.55e-018, 8.46e-019, 4.43e-019, 4.04e-019, 2.44e-019, 4.86e-019};
const Real dim100DiscrSobLev_Sobol[] = {1.17e-018, 2.65e-018, 1.45e-018, 7.28e-019, 6.33e-019, 3.36e-019, 3.43e-019};
const Real dim100DiscrSobLem_Sobol[] = {8.79e-019, 4.60e-019, 6.69e-019, 7.17e-019, 5.81e-019, 2.97e-019, 2.64e-019};
const Real dim100DiscrMersenneTwis[] = {5.30e-019, 7.29e-019, 3.71e-019, 3.33e-019, 1.33e-017, 6.70e-018, 3.36e-018};
const Real dim100DiscrPlain_Halton[] = {3.63e-004, 1.81e-004, 9.07e-005, 4.53e-005, 2.27e-005, 1.13e-005, 5.66e-006};
...
const Real dim100Discr_Unit_Sobol[] = {4.97e-006, 2.48e-006, 1.24e-006, 6.20e-007, 3.10e-007, 1.55e-007, 7.76e-008};
...
class MersenneFactory
{
  public:
    ...
    MersenneTwisterUniformRsg make(Size dim, BigNatural seed) const { return MersenneTwisterUniformRsg(dim, seed); }
...
class SobolFactory
{
  public:
    ...
    SobolRsg make(Size dim, BigNatural seed) const { return SobolRsg(dim, seed, unit_); }
...
class HaltonFactory
{
  public:
    ...
    HaltonFactory(bool randomStart, bool randomShift) : start_(randomStart), shift_(randomShift) {}
    HaltonRsg make(Size dim, BigNatural seed) const { return HaltonRsg(dim, seed, start_, shift_); }
    std::string name() const
    {
        std::string prefix = start_ ? "random-start " : "";
        if (shift_)
            prefix += "random-shift ";
...
)
{
    ...
    for (Integer i = 0; i < 8; i++)
    {
        ...
        stat.reset();
#ifdef PRINT_ONLY
        outStream << "const Real dim" << dim << arrayName << "[] = {";
#endif
        for (j = jMin; j < jMin + sampleLoops; j++)
        {
            ...
            if (std::fabs(discr - discrepancy[i][j - jMin]) > tolerance * discr)
            {
                BOOST_ERROR(generatorFactory.name()
                            << "discrepancy dimension " << dimensionality[i] << " at " << points << " samples is "
                            << discr << " instead of " << discrepancy[i][j - jMin]);
            }
#endif
        }
#ifdef PRINT_ONLY
...
BOOST_AUTO_TEST_CASE(testMersenneTwisterDiscrepancy)
{
    ...
    const Real* const discrepancy[8] = {dim002DiscrMersenneTwis, dim003DiscrMersenneTwis, dim005DiscrMersenneTwis,
                                        dim010DiscrMersenneTwis, dim015DiscrMersenneTwis, dim030DiscrMersenneTwis,
                                        dim050DiscrMersenneTwis, dim100DiscrMersenneTwis};

    testGeneratorDiscrepancy(MersenneFactory(), discrepancy, "MersenneDiscrepancy.txt", "DiscrMersenneTwis");
}
...
BOOST_AUTO_TEST_CASE(testPlainHaltonDiscrepancy)
{
    ...
    const Real* const discrepancy[8] = {dim002DiscrPlain_Halton, dim003DiscrPlain_Halton, dim005DiscrPlain_Halton,
                                        dim010DiscrPlain_Halton, dim015DiscrPlain_Halton, dim030DiscrPlain_Halton,
                                        dim050DiscrPlain_Halton, dim100DiscrPlain_Halton};

    testGeneratorDiscrepancy(HaltonFactory(false, false), discrepancy, "PlainHaltonDiscrepancy.txt",
                             "DiscrPlain_Halton");
}
...
BOOST_AUTO_TEST_CASE(testRandomStartHaltonDiscrepancy)
{
    ...
    const Real* const discrepancy[8] = {dim002DiscrRStartHalton, dim003DiscrRStartHalton, dim005DiscrRStartHalton,
                                        dim010DiscrRStartHalton, dim015DiscrRStartHalton, dim030DiscrRStartHalton,
                                        dim050DiscrRStartHalton, dim100DiscrRStartHalton};

    testGeneratorDiscrepancy(HaltonFactory(true, false), discrepancy, "RandomStartHaltonDiscrepancy.txt",
                             "DiscrRStartHalton");
}
...
BOOST_AUTO_TEST_CASE(testRandomShiftHaltonDiscrepancy)
{
    ...
    const Real* const discrepancy[8] = {dim002DiscrRShiftHalton, dim003DiscrRShiftHalton, dim005DiscrRShiftHalton,
                                        dim010DiscrRShiftHalton, dim015DiscrRShiftHalton, dim030DiscrRShiftHalton,
                                        dim050DiscrRShiftHalton, dim100DiscrRShiftHalton};

    testGeneratorDiscrepancy(HaltonFactory(false, true), discrepancy, "RandomShiftHaltonDiscrepancy.txt",
                             "DiscrRShiftHalton");
}
...
BOOST_AUTO_TEST_CASE(testRandomStartRandomShiftHaltonDiscrepancy)
{
    ...
    const Real* const discrepancy[8] = {dim002DiscrRStRShHalton, dim003DiscrRStRShHalton, dim005DiscrRStRShHalton,
                                        dim010DiscrRStRShHalton, dim015DiscrRStRShHalton, dim030DiscrRStRShHalton,
                                        dim050DiscrRStRShHalton, dim100DiscrRStRShHalton};

    testGeneratorDiscrepancy(HaltonFactory(true, true), discrepancy, "RandomStartRandomShiftHaltonDiscrepancy.txt",
                             "DiscrRStRShHalton");
}
...
BOOST_AUTO_TEST_CASE(testJackelSobolDiscrepancy)
{
    ...
    const Real* const discrepancy[8] = {dim002Discr_Sobol,       dim003Discr_Sobol,       dim005Discr_Sobol,
                                        dim010DiscrJackel_Sobol, dim015DiscrJackel_Sobol, dim030DiscrJackel_Sobol,
                                        dim050DiscrJackel_Sobol, dim100DiscrJackel_Sobol};

    testGeneratorDiscrepancy(SobolFactory(SobolRsg::Jaeckel), discrepancy, "JackelSobolDiscrepancy.txt",
                             "DiscrJackel_Sobol");
}
...
BOOST_AUTO_TEST_CASE(testSobolLevitanSobolDiscrepancy)
{
    ...
    testGeneratorDiscrepancy(SobolFactory(SobolRsg::SobolLevitan), discrepancy, "SobolLevitanSobolDiscrepancy.txt",
                             "DiscrSobLev_Sobol");
}
...
BOOST_AUTO_TEST_CASE(testSobolLevitanLemieuxSobolDiscrepancy)
{
    ...
    testGeneratorDiscrepancy(SobolFactory(SobolRsg::SobolLevitanLemieux), discrepancy,
                             "SobolLevitanLemieuxSobolDiscrepancy.txt", "DiscrSobLevLem_Sobol");
}
...
BOOST_AUTO_TEST_CASE(testUnitSobolDiscrepancy)
{
    ...
    const Real* const discrepancy[8] = {dim002Discr_Unit_Sobol, dim003Discr_Unit_Sobol, dim005Discr_Unit_Sobol,
                                        dim010Discr_Unit_Sobol, dim015Discr_Unit_Sobol, dim030Discr_Unit_Sobol,
...
BOOST_AUTO_TEST_CASE(testSobolSkipping)
{
    ...
    SobolRsg::DirectionIntegers integers[] = {SobolRsg::Unit, SobolRsg::Jaeckel, SobolRsg::SobolLevitan,
...
BOOST_AUTO_TEST_CASE(testSobolBurleySkipping)
{
    ...
    SobolRsg::DirectionIntegers integers[] = {SobolRsg::Jaeckel, SobolRsg::SobolLevitan, SobolRsg::SobolLevitanLemieux};
...
BOOST_AUTO_TEST_CASE(testHighDimensionalIntegrals, *precondition(if_speed(Slow)))
{
    ...
    std::vector<std::vector<Real>> expectedOrderOfError = {{-3.0, -3.0, -4.5}, {-2.5, -2.5, -4.0}, {-2.0, -2.0, -4.0}};
    ...
    for (Size d = 0; d < dimension.size(); ++d)
    {
        ...
        BOOST_CHECK_MESSAGE(errOrder1 < expectedOrderOfError[d][0],
                            "order of error for dimension " + std::to_string(dimension[d]) + " is" +
                                std::to_string(errOrder1) + " expected " + std::to_string(expectedOrderOfError[d][0]));
        BOOST_CHECK_MESSAGE(errOrder2 < expectedOrderOfError[d][1],
                            "order of error for dimension " + std::to_string(dimension[d]) + " is" +
                                std::to_string(errOrder2) + " expected " + std::to_string(expectedOrderOfError[d][1]));
        BOOST_CHECK_MESSAGE(errOrder3 < expectedOrderOfError[d][2],
                            "order of error for dimension " + std::to_string(dimension[d]) + " is" +
                                std::to_string(errOrder3) + " expected " + std::to_string(expectedOrderOfError[d][2]));
    }
}
...
{
    BOOST_TEST_MESSAGE("Testing that the output of Burley 2020 is strictly in (0,1)...");
    ...
    for (Size i = 0; i < 100000; ++i)
    {
        ...
        for (Size j = 0; j < seq.value.size(); ++j)
        {
            if (seq.value[j] <= 0.0 || seq.value[j] >= 1.0)
                BOOST_ERROR("output " << seq.value[j] << " at sample " << i << ", dim " << j);
        }
    }
}
