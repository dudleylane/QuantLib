...
{
  public:
    ParableLocalVolatility(const Date& referenceDate, Real s0, Real alpha, const DayCounter& dayCounter)
    : LocalVolTermStructure(referenceDate, NullCalendar(), Following, dayCounter), referenceDate_(referenceDate),
      s0_(s0), alpha_(alpha)
    {
    }
  ...
  protected:
    Volatility localVolImpl(Time, Real s) const override { return alpha_ * (squared(s0_ - s) + 25.0); }
...
BOOST_AUTO_TEST_CASE(testFdmHestonVarianceMesher)
{
    ...
    const ext::shared_ptr<HestonProcess> process(ext::make_shared<HestonProcess>(
        Handle<YieldTermStructure>(flatRate(0.02, dc)), Handle<YieldTermStructure>(flatRate(0.02, dc)),
        Handle<Quote>(ext::make_shared<SimpleQuote>(100.0)), 0.09, 1.0, 0.09, 0.2, -0.5));

    const ext::shared_ptr<FdmHestonVarianceMesher> mesher = ext::make_shared<FdmHestonVarianceMesher>(5, process, 1.0);

    const std::vector<Real> locations = mesher->locations();

    const Real expected[] = {0.0, 6.652314e-02, 9.000000e-02, 1.095781e-01, 2.563610e-01};
    ...
    const ext::shared_ptr<LocalVolTermStructure> lVol = ext::make_shared<LocalConstantVol>(today, 2.5, dc);

    const ext::shared_ptr<FdmHestonLocalVolatilityVarianceMesher> constSlvMesher =
        ext::make_shared<FdmHestonLocalVolatilityVarianceMesher>(5, process, lVol, 1.0);
    ...
    if (diff > tol)
    {
        BOOST_ERROR("Failed to reproduce Heston local volatility "
                    "variance estimate"
                    << "\n    calculated: " << calculatedVol << "\n    expected:   " << expectedVol << std::scientific
                    << "\n    difference  " << diff << "\n    tolerance:  " << tol);
    }
    ...
    const ext::shared_ptr<FdmHestonLocalVolatilityVarianceMesher> slvMesher =
        ext::make_shared<FdmHestonLocalVolatilityVarianceMesher>(5, process, leverageFct, 0.5, 1, 0.01);

    const Real initialVolEstimate = ext::make_shared<FdmHestonVarianceMesher>(5, process, 0.5, 1, 0.01)->volaEstimate();
    ...
    const Real volaEstExpected = 0.5 * (leverageAvg + leverageFct->localVol(0, 100)) * initialVolEstimate;
...
BOOST_AUTO_TEST_CASE(testFdmHestonBarrierVsBlackScholes, *precondition(if_speed(Fast)))
{
    ...
    Handle<Quote> spot(ext::shared_ptr<Quote>(new SimpleQuote(0.0)));
    ...
    ext::shared_ptr<BlackScholesMertonProcess> bsProcess(new BlackScholesMertonProcess(spot, qTS, rTS, volTS));

    ext::shared_ptr<PricingEngine> analyticEngine(new AnalyticBarrierEngine(bsProcess));
    ...
    for (auto& value : values)
    {
        ...
        ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(rTS, qTS, spot, v0, 1.0, v0, 0.005, 0.0));

        barrierOption.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new FdHestonBarrierEngine(ext::make_shared<HestonModel>(hestonProcess), 200, 101, 3)));
...
BOOST_AUTO_TEST_CASE(testFdmHestonBarrier)
{
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Call, 100));

    BarrierOption barrierOption(Barrier::UpOut, 135, 0.0, payoff, exercise);

    barrierOption.setPricingEngine(ext::shared_ptr<PricingEngine>(
        new FdHestonBarrierEngine(ext::make_shared<HestonModel>(hestonProcess), 50, 400, 100)));
...
BOOST_AUTO_TEST_CASE(testFdmHestonAmerican)
{
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Put, 100));

    VanillaOption option(payoff, exercise);
    ext::shared_ptr<PricingEngine> engine(
        new FdHestonVanillaEngine(ext::make_shared<HestonModel>(hestonProcess), 200, 100, 50));
...
BOOST_AUTO_TEST_CASE(testFdmHestonIkonenToivanen)
{
    ...
    */
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Put, 10));
    ...
    for (Size i = 0; i < std::size(strikes); ++i)
    {
        ...
        ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(rTS, qTS, s0, 0.0625, 5, 0.16, 0.9, 0.1));

        ext::shared_ptr<PricingEngine> engine(
            new FdHestonVanillaEngine(ext::make_shared<HestonModel>(hestonProcess), 100, 400));
        option.setPricingEngine(engine);
...
BOOST_AUTO_TEST_CASE(testFdmHestonBlackScholes)
{
    ...
    Handle<YieldTermStructure> rTS(flatRate(0.10, Actual360()));
    Handle<YieldTermStructure> qTS(flatRate(0.0, Actual360()));
    Handle<BlackVolTermStructure> volTS(flatVol(rTS->referenceDate(), 0.25, rTS->dayCounter()));
    ...
    for (Real& strike : strikes)
    {
        ...
        option.setPricingEngine(ext::shared_ptr<PricingEngine>(new AnalyticEuropeanEngine(bsProcess)));

        const Real expected = option.NPV();

        ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(rTS, qTS, s0, 0.0625, 1, 0.0625, 0.0001, 0.0));

        // Hundsdorfer scheme
        option.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new FdHestonVanillaEngine(ext::make_shared<HestonModel>(hestonProcess), 100, 400, 3)));
        ...
        // Explicit scheme
        option.setPricingEngine(ext::shared_ptr<PricingEngine>(new FdHestonVanillaEngine(
            ext::make_shared<HestonModel>(hestonProcess), 4000, 400, 3, 0, FdmSchemeDesc::ExplicitEuler())));
...
BOOST_AUTO_TEST_CASE(testFdmHestonEuropeanWithDividends)
{
    ...
    VanillaOption option(payoff, exercise);
    auto engine = ext::make_shared<FdHestonVanillaEngine>(ext::make_shared<HestonModel>(hestonProcess),
                                                          DividendVector(dividendDates, dividends), 50, 100, 50);
    option.setPricingEngine(engine);
...
BOOST_AUTO_TEST_CASE(testFdmHestonConvergence, *precondition(if_speed(Fast)))
{
    ...
    */
    ...
    HestonTestData values[] = {{1.5, 0.04, 0.3, -0.9, 0.025, 0.0, 1.0, 100},
            ...
            for (unsigned long j : tn)
            {
                for (Real k : v0)
                {
                    ...
                    ext::shared_ptr<HestonProcess> hestonProcess(
                        new HestonProcess(rTS, qTS, s0, k, value.kappa, value.theta, value.sigma, value.rho));

                    Date exerciseDate = todaysDate + Period(static_cast<Integer>(value.T * 365), Days);
                    ext::shared_ptr<Exercise> exercise(new EuropeanExercise(exerciseDate));

                    ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Call, value.K));

                    VanillaOption option(payoff, exercise);
                    ext::shared_ptr<PricingEngine> engine(
                        new FdHestonVanillaEngine(ext::make_shared<HestonModel>(hestonProcess), j, 101, 51, 0, scheme));
                    option.setPricingEngine(engine);

                    const Real calculated = option.NPV();

                    ext::shared_ptr<PricingEngine> analyticEngine(
                        new AnalyticHestonEngine(ext::make_shared<HestonModel>(hestonProcess), 144));

                    option.setPricingEngine(analyticEngine);
                    const Real expected = option.NPV();
                    if (std::fabs(expected - calculated) / expected > 0.02 && std::fabs(expected - calculated) > 0.002)
                    {
                        BOOST_ERROR("Failed to reproduce expected npv" << "\n    calculated: " << calculated
                                                                       << "\n    expected:   " << expected
                                                                       << "\n    tolerance:  " << 0.01);
...
BOOST_AUTO_TEST_CASE(testFdmHestonIntradayPricing)
{
    ...
    const ext::shared_ptr<Exercise> europeanExercise(new EuropeanExercise(maturity));
    const ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(type, strike));
    VanillaOption option(payoff, europeanExercise);

    const Handle<Quote> s0(ext::shared_ptr<Quote>(new SimpleQuote(underlying)));
    ...
    const ext::shared_ptr<HestonProcess> process(
        new HestonProcess(flatTermStructure, flatDividendTS, s0, v0, kappa, theta, sigma, rho));
    const ext::shared_ptr<HestonModel> model(new HestonModel(process));
    const ext::shared_ptr<PricingEngine> fdm(new FdHestonVanillaEngine(model, 20, 100, 26, 0));
    option.setPricingEngine(fdm);

    const Real gammaExpected[] = {1.46757, 1.54696, 1.6408,  1.75409, 1.89464,
    ...
    for (Size i = 0; i < 10; ++i)
    {
        ...
        flatTermStructure.linkTo(ext::shared_ptr<YieldTermStructure>(new FlatForward(now, riskFreeRate, dayCounter)));
        flatDividendTS.linkTo(ext::shared_ptr<YieldTermStructure>(new FlatForward(now, dividendYield, dayCounter)));
...
BOOST_AUTO_TEST_CASE(testMethodOfLinesAndCN)
{
    ...
    const ext::shared_ptr<PricingEngine> fdmMol(
        ext::make_shared<FdHestonVanillaEngine>(model, 10, xGrid, vGrid, 0, FdmSchemeDesc::MethodOfLines()));

    const ext::shared_ptr<PlainVanillaPayoff> payoff = ext::make_shared<PlainVanillaPayoff>(Option::Put, spot->value());

    VanillaOption option(payoff, ext::make_shared<AmericanExercise>(maturity));
    ...
    const ext::shared_ptr<PricingEngine> fdmCN(
        ext::make_shared<FdHestonVanillaEngine>(model, 10, xGrid, vGrid, 0, FdmSchemeDesc::CrankNicolson()));
    ...
    BarrierOption barrierOption(Barrier::DownOut, 85.0, 10.0, payoff, ext::make_shared<EuropeanExercise>(maturity));

    barrierOption.setPricingEngine(ext::make_shared<FdHestonBarrierEngine>(model, 100, 31, 11));

    const Real expectedBarrier = barrierOption.NPV();

    barrierOption.setPricingEngine(
        ext::make_shared<FdHestonBarrierEngine>(model, 100, 31, 11, 0, FdmSchemeDesc::MethodOfLines()));
    ...
    barrierOption.setPricingEngine(
        ext::make_shared<FdHestonBarrierEngine>(model, 100, 31, 11, 0, FdmSchemeDesc::CrankNicolson()));
...
BOOST_AUTO_TEST_CASE(testSpuriousOscillations)
{
    ...
    const ext::shared_ptr<FdHestonVanillaEngine> hestonEngine(
        ext::make_shared<FdHestonVanillaEngine>(model, 6, 200, 13, 0, FdmSchemeDesc::TrBDF2()));

    VanillaOption option(ext::make_shared<PlainVanillaPayoff>(Option::Call, spot->value()),
    ...
    for (const auto& [desc, name, spurious] : descs)
    {
        const ext::shared_ptr<FdmHestonSolver> solver =
            ext::make_shared<FdmHestonSolver>(Handle<HestonProcess>(process), hestonEngine->getSolverDesc(1.0), desc);
...
BOOST_AUTO_TEST_CASE(testAmericanCallPutParity)
{
    ...
    {
        return ext::make_shared<HestonProcess>(Handle<YieldTermStructure>(flatRate(testCase.r, dc)),
                                               Handle<YieldTermStructure>(flatRate(testCase.q, dc)),
                                               Handle<Quote>(ext::make_shared<SimpleQuote>(testCase.spot)), testCase.v0,
                                               testCase.kappa, testCase.theta, testCase.sig, testCase.rho);
    };

    const OptionSpec testCaseSpecs[] = {{100.0, 90.0, 365, 0.02, 0.15, 0.25, 1.0, 0.09, 0.5, -0.75},
    ...
    for (const auto& testCaseSpec : testCaseSpecs)
    {
        ...
        VanillaOption callOption(ext::make_shared<PlainVanillaPayoff>(Option::Call, testCaseSpec.strike), exercise);

        callOption.setPricingEngine(ext::make_shared<FdHestonVanillaEngine>(
            ext::make_shared<HestonModel>(buildStochProcess(testCaseSpec)), tGrid, xGrid, vGrid));

        const Real callNpv = callOption.NPV();

        OptionSpec putOptionSpec = {testCaseSpec.strike,
                                    testCaseSpec.spot,
        ...
        VanillaOption putOption(ext::make_shared<PlainVanillaPayoff>(Option::Put, putOptionSpec.strike), exercise);

        putOption.setPricingEngine(ext::make_shared<FdHestonVanillaEngine>(
            ext::make_shared<HestonModel>(buildStochProcess(putOptionSpec)), tGrid, xGrid, vGrid));
