...
#define REPORT_FAILURE(d, res, periodName)                                                                         \
    BOOST_ERROR("wrong " << periodName << " inflation period for Date (1 " << d << "), Start Date ( " << res.first \
                         << "), End Date (" << res.second << ")");
...
ext::shared_ptr<YieldTermStructure> nominalTermStructure()
{
    ...
    return ext::shared_ptr<YieldTermStructure>(new FlatForward(evaluationDate, 0.05, Actual360()));
}

template <class T>
std::vector<ext::shared_ptr<BootstrapHelper<T>>>
makeHelpers(const std::vector<Datum>& iiData,
            const std::function<ext::shared_ptr<BootstrapHelper<T>>(const Handle<Quote>&, const Date&)>& makeHelper)
{
...
void checkSeasonality(const Handle<ZeroInflationTermStructure>& hz, const ext::shared_ptr<ZeroInflationIndex>& ii)
{

    QL_REQUIRE(!hz->hasSeasonality(),
               "We require that the initially passed in term structure " << "does not have seasonality");
    ...
    for (Size i = 0; i < fixingDates.size(); i++)
    {
        if (fabs(noSeasonalityFixings[i] - unitSeasonalityFixings[i]) > tolerance)
        {
            BOOST_ERROR("Seasonality doesn't work correctly when seasonality factors are set = 1"
                        ...
                        << " but unit seasonality fixing is: " << unitSeasonalityFixings[i] << " for fixing date "
                        << io::iso_date(fixingDates[i]));
        }
    }
    ...
    for (Size i = 0; i < fixingDates.size(); i++)
    {
        if (fabs(expectedSeasonalityFixings[i] - nonUnitSeasonalityFixings[i]) > tolerance)
        {
            BOOST_ERROR("Seasonality doesn't work correctly for non-unit seasonality factors."
                        << " Expected fixing is: " << expectedSeasonalityFixings[i] << " but calculated fixing is: "
                        << nonUnitSeasonalityFixings[i] << " for fixing date " << io::iso_date(fixingDates[i]));
        }
    }
...
BOOST_AUTO_TEST_CASE(testZeroIndex)
{
    ...
    {
        BOOST_ERROR("wrong EU HICP data (" << euhicp.name() << ", " << euhicp.frequency() << ", " << euhicp.revised()
    ...
    {
        BOOST_ERROR("wrong UK RPI data (" << ukrpi.name() << ", " << ukrpi.frequency() << ", " << ukrpi.revised()
                                          << ", " << ukrpi.availabilityLag() << ")");
    }

    UKHICP ukhicp;
    if (ukhicp.name() != "UK HICP" || ukhicp.frequency() != Monthly || ukhicp.revised() ||
        ukhicp.availabilityLag() != 1 * Months)
    {
        BOOST_ERROR("wrong UK HICP data (" << ukhicp.name() << ", " << ukhicp.frequency() << ", " << ukhicp.revised()
                                           << ", "
    ...
    {
        std::pair<Date, Date> lim = inflationPeriod(rpiSchedule[i], iir->frequency());
        for (Date d = lim.first; d <= lim.second; d++)
        {
            if (d < inflationPeriod(todayMinusLag, iir->frequency()).first)
            {
                if (std::fabs(iir->fixing(d) - fixData[i]) > eps)
                    BOOST_ERROR("Fixings not constant within a period: " << iir->fixing(d) << ", should be "
                                                                         << fixData[i]);
            }
        }
    }
...
BOOST_AUTO_TEST_CASE(testZeroTermStructure)
{
    ...
    {
        return ext::make_shared<ZeroCouponInflationSwapHelper>(quote, observationLag, maturity, calendar, bdc, dc, ii,
                                                               interpolation);
    };
    ...
    auto firstCashFlow = ext::dynamic_pointer_cast<ZeroInflationCashFlow>(firstHelper->swap()->inflationLeg().front());
    ...
    ext::shared_ptr<PiecewiseZeroInflationCurve<Linear>> pZITS =
        ext::make_shared<PiecewiseZeroInflationCurve<Linear>>(evaluationDate, baseDate, frequency, dc, helpers);
    hz.linkTo(pZITS);
    ...
    {
        ZeroCouponInflationSwap nzcis(Swap::Payer, 1000000.0, evaluationDate, datum.date, calendar, bdc, dc,
                                      datum.rate / 100.0, ii, observationLag, interpolation);
        ...
        ZeroCouponInflationSwap nzcisBumped(Swap::Payer, 1000000.0, evaluationDate, datum.date, calendar, bdc, dc,
                                            datum.rate / 100.0 + basisPoint, ii, observationLag, interpolation);
...
BOOST_AUTO_TEST_CASE(testZeroTermStructureLazyBaseDate)
{
    ...
    for (auto datum : zcData)
    {
        ...
        helpers.push_back(ext::make_shared<ZeroCouponInflationSwapHelper>(
            Handle<Quote>(quotes.back()), observationLag, datum.date, calendar, bdc, dc, ii, interpolation));
    }
    ...
    auto curve = ext::make_shared<PiecewiseZeroInflationCurve<Linear>>(evaluationDate, ii->lastFixingDate(), frequency,
                                                                       dc, helpers);
...
BOOST_AUTO_TEST_CASE(testZeroTermStructureWithNominalCurve)
{
    ...
    auto makeHelper = [&](const Handle<Quote>& quote, const Date& maturity)
    {
        return ext::shared_ptr<ZeroCouponInflationSwapHelper>( // NOLINT(modernize-make-shared)
            new ZeroCouponInflationSwapHelper(quote, observationLag, maturity, calendar, bdc, dc, ii, CPI::AsIndex,
                                              nominalTS));
    };
    auto helpers = makeHelpers<ZeroInflationTermStructure>(zcData, makeHelper);
    ...
    {
        ZeroCouponInflationSwap nzcis(Swap::Payer, 1000000.0, evaluationDate, datum.date, calendar, bdc, dc,
                                      datum.rate / 100.0, ii, observationLag, CPI::AsIndex);
        nzcis.setPricingEngine(engine);
...
BOOST_AUTO_TEST_CASE(testSeasonalityCorrection)
{
    ...
    std::vector<Date> nodes = {Date(1, June, 2007), Date(1, June, 2008), Date(1, June, 2009), Date(1, June, 2010),
                               Date(1, June, 2011), Date(1, June, 2012), Date(1, June, 2014), Date(1, June, 2017),
                               Date(1, June, 2019), Date(1, June, 2022), Date(1, June, 2027), Date(1, June, 2032),
                               Date(1, June, 2037), Date(1, June, 2047), Date(1, June, 2057)};

    std::vector<Rate> rates = {0.0293,  0.0293,  0.0295,  0.02965, 0.0298,  0.03,    0.0306, 0.03175,
    ...
    auto zeroCurve =
        ext::make_shared<InterpolatedZeroInflationCurve<Linear>>(evaluationDate, nodes, rates, frequency, dc);
    hz.linkTo(zeroCurve);
...
BOOST_AUTO_TEST_CASE(testZeroIndexFutureFixing)
{
    ...
    BOOST_CHECK_EXCEPTION(euhicp.fixing({1, March, 2024}), Error, ExpectedErrorMessage("empty Handle"));
    ...
    BOOST_CHECK_EXCEPTION(euhicp.fixing({1, April, 2024}), Error, ExpectedErrorMessage("empty Handle"));
...
BOOST_AUTO_TEST_CASE(testInterpolatedZeroTermStructure)
{
    ...
    std::vector<Date> dates = {baseDate,           today + 7 * Days,   today + 14 * Days,  today + 1 * Months,
                               today + 2 * Months, today + 3 * Months, today + 6 * Months, today + 1 * Years,
                               today + 2 * Years,  today + 5 * Years,  today + 10 * Years};
    std::vector<Rate> rates = {0.01, 0.01, 0.011, 0.012, 0.013, 0.015, 0.018, 0.02, 0.025, 0.03, 0.03};

    auto curve = ext::make_shared<InterpolatedZeroInflationCurve<Linear>>(today, dates, rates, Monthly, Actual360());
...
BOOST_AUTO_TEST_CASE(testQuotedYYIndex)
{
    ...
    YYUKRPI yyukrpi;
    QL_DEPRECATED_DISABLE_WARNING
    if (yyukrpi.name() != "UK YY_RPI" || yyukrpi.frequency() != Monthly || yyukrpi.revised() ||
        yyukrpi.interpolated() || yyukrpi.ratio() || yyukrpi.availabilityLag() != 1 * Months)
    {
        BOOST_ERROR("wrong year-on-year UK RPI data (" << yyukrpi.name() << ", " << yyukrpi.frequency() << ", "
                                                       << yyukrpi.revised() << ", " << yyukrpi.interpolated() << ", "
                                                       << yyukrpi.ratio() << ", " << yyukrpi.availabilityLag() << ")");
    }
    QL_DEPRECATED_ENABLE_WARNING
}
...
BOOST_AUTO_TEST_CASE(testQuotedYYIndexFutureFixing)
{
    ...
    BOOST_CHECK_EXCEPTION(quoted.fixing({1, April, 2024}), Error, ExpectedErrorMessage("empty Handle"));
}
...
BOOST_AUTO_TEST_CASE(testRatioYYIndex)
{
    ...
    QL_DEPRECATED_DISABLE_WARNING
    if (yyukrpir.name() != "UK YYR_RPI" || yyukrpir.frequency() != Monthly || yyukrpir.revised() ||
        yyukrpir.interpolated() || !yyukrpir.ratio() || yyukrpir.availabilityLag() != 1 * Months)
    {
        BOOST_ERROR("wrong year-on-year UK RPIr data (" << yyukrpir.name() << ", " << yyukrpir.frequency() << ", "
                                                        << yyukrpir.revised() << ", " << yyukrpir.interpolated() << ", "
                                                        << yyukrpir.ratio() << ", " << yyukrpir.availabilityLag()
                                                        << ")");
    }
    QL_DEPRECATED_ENABLE_WARNING
...
BOOST_AUTO_TEST_CASE(testRatioYYIndexFutureFixing)
{
    ...
    // April can't be available now, fail even if it's stored:
    euhicp->addFixing({1, April, 2024}, 100.4);
    BOOST_CHECK_EXCEPTION(ratio.fixing({1, April, 2024}), Error, ExpectedErrorMessage("empty Handle"));
}
...
BOOST_AUTO_TEST_CASE(testYYTermStructure)
{
    ...
    std::vector<Datum> yyData = {
        {Date(13, August, 2008), 2.95},  {Date(13, August, 2009), 2.95},  {Date(13, August, 2010), 2.93},
        {Date(15, August, 2011), 2.955}, {Date(13, August, 2012), 2.945}, {Date(13, August, 2013), 2.985},
        {Date(13, August, 2014), 3.01},  {Date(13, August, 2015), 3.035}, {Date(13, August, 2016), 3.055}, // note that
        ...
        {Date(13, August, 2027), 3.155}, {Date(13, August, 2032), 3.145}, {Date(13, August, 2037), 3.145}};
    ...
    auto makeHelper = [&](const Handle<Quote>& quote, const Date& maturity)
    {
        return ext::make_shared<YearOnYearInflationSwapHelper>(quote, observationLag, maturity, calendar, bdc, dc, iir,
                                                               interpolation, Handle<YieldTermStructure>(nominalTS));
    };
    ...
    auto firstCashFlow = ext::dynamic_pointer_cast<YoYInflationCoupon>(firstHelper->swap()->yoyLeg().front());
    ...
    auto pYYTS = ext::make_shared<PiecewiseYoYInflationCurve<Linear>>(evaluationDate, baseDate, baseYYRate,
    ...
    for (Size j = 1; j < yyData.size(); j++)
    {
        ...
        YearOnYearInflationSwap yyS2(Swap::Payer, 1000000.0,
                                     yoySchedule, // fixed schedule, but same as yoy
                                     yyData[j].rate / 100.0, dc, yoySchedule, iir, observationLag, interpolation,
                                     0.0, // spread on index
                                     dc, UnitedKingdom());

        yyS2.setPricingEngine(sppe);

        BOOST_CHECK_MESSAGE(fabs(yyS2.NPV()) < eps,
                            "fresh yoy swap NPV!=0 from TS "
                                << "swap quote for pt " << j << ", is " << yyData[j].rate / 100.0 << " vs YoY rate "
                                << pYYTS->yoyRate(yyData[j].date - observationLag) << " at quote date "
                                << (yyData[j].date - observationLag) << ", NPV of a fresh yoy swap is " << yyS2.NPV()
                                << "\n      fair rate " << yyS2.fairRate() << " payment " << yyS2.paymentConvention());
    }
    ...
    for (Size k = 0; k < 14; k++)
    {
        ...
        YearOnYearInflationSwap yyS3(Swap::Payer, 1000000.0,
                                     yoySchedule, // fixed schedule, but same as yoy
                                     yyData[jj].rate / 100.0, dc, yoySchedule, iir, observationLag, interpolation,
                                     0.0, // spread on index
                                     dc, UnitedKingdom());

        yyS3.setPricingEngine(sppe);

        BOOST_CHECK_MESSAGE(fabs(yyS3.NPV()) < 20000.0, "unexpected size of aged YoY swap, aged "
                                                            << k << " months: YY aged NPV = " << yyS3.NPV() << ", legs "
                                                            << yyS3.legNPV(0) << " and " << yyS3.legNPV(1));
    }
    // remove circular refernce
    hy.reset();
}
...
BOOST_AUTO_TEST_CASE(testPeriod)
{
    ...
    for (int year = 1950; year < 2051; ++year)
    {
        ...
        for (Size i = 1; i <= 12; ++i)
        {
            ...
            if (res.first != Date(1, Month(i), year) || res.second != Date(days[i], Month(i), year))
            {
            ...
            if ((i == 1 || i == 2 || i == 3) &&
                (res.first != Date(1, Month(1), year) || res.second != Date(31, Month(3), year)))
            {
                REPORT_FAILURE(d, res, "Quarterly");
            }
            else if ((i == 4 || i == 5 || i == 6) &&
                     (res.first != Date(1, Month(4), year) || res.second != Date(30, Month(6), year)))
            {
                REPORT_FAILURE(d, res, "Quarterly");
            }
            else if ((i == 7 || i == 8 || i == 9) &&
                     (res.first != Date(1, Month(7), year) || res.second != Date(30, Month(9), year)))
            {
                REPORT_FAILURE(d, res, "Quarterly");
            }
            else if ((i == 10 || i == 11 || i == 12) &&
                     (res.first != Date(1, Month(10), year) || res.second != Date(31, Month(12), year)))
            {
                REPORT_FAILURE(d, res, "Quarterly");
            }
            ...
            if ((i > 0 && i < 7) && (res.first != Date(1, Month(1), year) || res.second != Date(30, Month(6), year)))
            {
                REPORT_FAILURE(d, res, "Semiannual");
            }
            else if ((i > 6 && i < 13) &&
                     (res.first != Date(1, Month(7), year) || res.second != Date(31, Month(12), year)))
            {
                REPORT_FAILURE(d, res, "Semiannual");
            }
            ...
            if (res.first != Date(1, Month(1), year) || res.second != Date(31, Month(12), year))
            {
...
BOOST_AUTO_TEST_CASE(testCpiLinearInterpolation)
{
    ...
    BOOST_CHECK_THROW(calculated = CPI::laggedFixing(testIndex, Date(25, June, 2021), 3 * Months, CPI::Linear), Error);
...
BOOST_AUTO_TEST_CASE(testCpiYoYQuotedLinearInterpolation)
{
    ...
    // this would require April's fixing
    BOOST_CHECK_EXCEPTION(CPI::laggedYoYRate(testIndex, Date(25, June, 2021), 3 * Months, CPI::Linear), Error,
                          ExpectedErrorMessage("Missing UK YY_RPI fixing"));
...
BOOST_AUTO_TEST_CASE(testCpiYoYRatioLinearInterpolation)
{
    ...
    // this would require April's fixing
    BOOST_CHECK_EXCEPTION(CPI::laggedYoYRate(testIndex1, Date(25, June, 2021), 3 * Months, CPI::Linear), Error,
                          ExpectedErrorMessage("Missing UK RPI fixing"));
...
BOOST_AUTO_TEST_CASE(testNotifications)
{
    ...
    inflation_handle.linkTo(ext::make_shared<ZeroInflationCurve>(today, dates, rates, Monthly, Actual360()));
    ...
    auto cashflow = ext::make_shared<ZeroInflationCashFlow>(nominal, index, CPI::Flat, today, today + 1 * Years,
                                                            3 * Months, today + 1 * Years);
    cashflow->amount();
...
BOOST_AUTO_TEST_CASE(testExtrapolationRegression)
{
    ...
    auto makeHelper = [&](const Handle<Quote>& quote, const Date& maturity)
    {
        return ext::make_shared<ZeroCouponInflationSwapHelper>(quote, observationLag, maturity, calendar, bdc, dc, rpi,
                                                               interpolation);
    };
    auto helpers = makeHelpers<ZeroInflationTermStructure>(zcData, makeHelper);
    ...
    {
        return ext::make_shared<YearOnYearInflationSwapHelper>(quote, observationLag, maturity, calendar, bdc, dc, yoy,
                                                               interpolation, Handle<YieldTermStructure>(nominalTS));
    };
...
BOOST_AUTO_TEST_CASE(testUsCpiLinearBootstrapAtMonthStart)
{
    ...
    BOOST_TEST_MESSAGE("Testing US CPI bootstrap with CPI::Linear across evaluation dates...");
    ...
    std::vector<SwapData> swapData = {
        {3 * Months, 0.0285},  {4 * Months, 0.0268}, {5 * Months, 0.0252},  {6 * Months, 0.0241},  {7 * Months, 0.0237},
        {8 * Months, 0.0232},  {9 * Months, 0.0229}, {10 * Months, 0.0225}, {11 * Months, 0.0223}, {1 * Years, 0.0221},
        {18 * Months, 0.0230}, {2 * Years, 0.0238},  {5 * Years, 0.0245},   {10 * Years, 0.0252},  {30 * Years, 0.0260},
    };
    ...
    std::vector<std::pair<Date, Real>> fixings = {
        {Date(1, January, 2025), 309.685}, {Date(1, February, 2025), 310.326}, {Date(1, March, 2025), 311.054},
        {Date(1, April, 2025), 311.538},   {Date(1, May, 2025), 311.862},      {Date(1, June, 2025), 312.104},
        {Date(1, July, 2025), 312.332},    {Date(1, August, 2025), 312.558},   {Date(1, September, 2025), 312.816},
        {Date(1, October, 2025), 313.025}, {Date(1, November, 2025), 313.314}, {Date(1, December, 2025), 313.580}};

    Size failureCount = 0;

    for (Date evalDate(1, February, 2026); evalDate <= Date(28, February, 2026); evalDate++)
    {
        ...
        for (auto& s : swapData)
        {
            ...
            helpers.push_back(ext::make_shared<ZeroCouponInflationSwapHelper>(
                Handle<Quote>(ext::make_shared<SimpleQuote>(s.rate)), observationLag, startDate, endDate, calendar,
                ModifiedFollowing, dc, index, CPI::Linear));
        }

        try
        {
            auto curve =
                ext::make_shared<PiecewiseZeroInflationCurve<Linear>>(evalDate, baseDate, Monthly, dc, helpers);
            hz.linkTo(curve);
...
BOOST_AUTO_TEST_CASE(testEuHicpFlatBootstrapAtMonthStart)
{
    ...
    BOOST_TEST_MESSAGE("Testing EU HICP bootstrap with CPI::Flat across evaluation dates...");
    ...
    std::vector<SwapData> swapData = {
        {1 * Years, 0.0182}, {2 * Years, 0.0178},  {3 * Years, 0.0185},  {4 * Years, 0.0188},  {5 * Years, 0.0190},
        {7 * Years, 0.0195}, {10 * Years, 0.0201}, {15 * Years, 0.0210}, {20 * Years, 0.0218}, {30 * Years, 0.0229},
    };
    ...
    std::vector<std::pair<Date, Real>> fixings = {
        {Date(1, January, 2025), 126.42}, {Date(1, February, 2025), 126.81}, {Date(1, March, 2025), 127.19},
        {Date(1, April, 2025), 127.51},   {Date(1, May, 2025), 127.62},      {Date(1, June, 2025), 127.85},
        {Date(1, July, 2025), 127.23},    {Date(1, August, 2025), 127.58},   {Date(1, September, 2025), 128.07},
        {Date(1, October, 2025), 128.41}, {Date(1, November, 2025), 128.62}, {Date(1, December, 2025), 128.89}};
    ...
    for (Date evalDate(1, February, 2026); evalDate <= Date(28, February, 2026); evalDate++)
    {
        ...
        try
        {
            auto curve = ext::make_shared<PiecewiseZeroInflationCurve<Linear, GlobalBootstrap>>(evalDate, baseDate,
                                                                                                Monthly, dc, helpers);
            hz.linkTo(curve);
            curve->zeroRate(evalDate + 1 * Years);
        }
...
BOOST_AUTO_TEST_CASE(testUkRpiFlatBootstrapAtMonthStart)
{
    ...
    BOOST_TEST_MESSAGE("Testing UK RPI bootstrap with CPI::Flat across evaluation dates...");
    ...
    std::vector<SwapData> swapData = {
        {1 * Years, 0.0335},  {2 * Years, 0.0328},  {3 * Years, 0.0322},  {5 * Years, 0.0318},  {7 * Years, 0.0316},
        {10 * Years, 0.0315}, {15 * Years, 0.0320}, {20 * Years, 0.0325}, {30 * Years, 0.0330}, {50 * Years, 0.0332},
    };
    ...
    std::vector<std::pair<Date, Real>> fixings = {
        {Date(1, January, 2025), 378.2}, {Date(1, February, 2025), 379.1}, {Date(1, March, 2025), 380.3},
        {Date(1, April, 2025), 381.5},   {Date(1, May, 2025), 382.0},      {Date(1, June, 2025), 382.4},
        {Date(1, July, 2025), 381.8},    {Date(1, August, 2025), 382.1},   {Date(1, September, 2025), 383.0},
        {Date(1, October, 2025), 383.5}, {Date(1, November, 2025), 383.9}, {Date(1, December, 2025), 384.2}};
...
BOOST_AUTO_TEST_CASE(testUsCpiLinearGlobalBootstrapAtMonthStart)
{
    ...
    BOOST_TEST_MESSAGE("Testing US CPI bootstrap with GlobalBootstrap across evaluation dates...");
...
BOOST_AUTO_TEST_CASE(testPillarCollisionWithDifferentMonthLengths)
{
    ...
    BOOST_TEST_MESSAGE("Testing CPI::Linear pillar assignment across months of different length...");
    ...
    std::vector<SwapData> swapData = {
        {3 * Months, 0.0285},  {4 * Months, 0.0268}, {5 * Months, 0.0252},  {6 * Months, 0.0241},
        {7 * Months, 0.0237},  {8 * Months, 0.0232}, {9 * Months, 0.0229},  {10 * Months, 0.0225},
        {11 * Months, 0.0223}, {1 * Years, 0.0221},  {13 * Months, 0.0220}, {18 * Months, 0.0230},
        {2 * Years, 0.0238},   {5 * Years, 0.0245},  {10 * Years, 0.0252},  {30 * Years, 0.0260},
    };
    ...
    std::vector<std::pair<Date, Real>> fixings = {
        ...
        {Date(1, July, 2025), 312.332},    {Date(1, August, 2025), 312.558},   {Date(1, September, 2025), 312.816},
        {Date(1, October, 2025), 313.025}, {Date(1, November, 2025), 313.314}, {Date(1, December, 2025), 313.580},
        {Date(1, January, 2026), 314.012}, {Date(1, February, 2026), 314.382}, {Date(1, March, 2026), 314.715},
    };

    Size failureCount = 0;

    // Loop February and March with T+0 settlement
    for (Date evalDate(1, February, 2026); evalDate <= Date(31, March, 2026); evalDate++)
    {
        ...
        for (auto& s : swapData)
        {
            ...
            helpers.push_back(ext::make_shared<ZeroCouponInflationSwapHelper>(
                Handle<Quote>(ext::make_shared<SimpleQuote>(s.rate)), observationLag, startDate, endDate, calendar,
                Unadjusted, dc, index, CPI::Linear));
        }
