...
CalibrationData AndreasenHugeExampleData()
{
    ...
    const Time maturityTimes[] = {0.025, 0.101, 0.197, 0.274, 0.523, 0.772, 1.769, 2.267, 2.784, 3.781, 4.778, 5.774};
    ...
    calibrationSet.reserve(
        std::count_if(&raw[0][0], &raw[nStrikes - 1][nMaturities] + 1, [](Real x) { return x != 0.0; }) - nStrikes);
    ...
    for (const auto& i : raw)
    {
        ...
        for (Size j = 1; j < std::size(i); ++j)
        {
            if (i[j] > QL_EPSILON)
            {
                const Date maturity = today + Period(Size(365 * maturityTimes[j - 1]), Days);

                const Volatility impliedVol = i[j];

                calibrationSet.emplace_back(
                    ext::make_shared<VanillaOption>(ext::make_shared<PlainVanillaPayoff>(
                                                        (strike < spot->value()) ? Option::Put : Option::Call, strike),
...
void testAndreasenHugeVolatilityInterpolation(const CalibrationData& data, const CalibrationResults& expected)
{
    ...
    AndreasenHugeVolatilityInterpl::CalibrationSet calibrationSet = data.calibrationSet;

    const ext::shared_ptr<AndreasenHugeVolatilityInterpl> andreasenHugeVolInterplation(
        ext::make_shared<AndreasenHugeVolatilityInterpl>(calibrationSet, spot, rTS, qTS, expected.interpolationType,
                                                         expected.calibrationType));

    const std::tuple<Real, Real, Real> error = andreasenHugeVolInterplation->calibrationError();
    ...
    const ext::shared_ptr<AndreasenHugeVolatilityAdapter> volatilityAdapter(
        ext::make_shared<AndreasenHugeVolatilityAdapter>(andreasenHugeVolInterplation, 1e-12));

    const ext::shared_ptr<AndreasenHugeLocalVolAdapter> localVolAdapter(
        ext::make_shared<AndreasenHugeLocalVolAdapter>(andreasenHugeVolInterplation));
    ...
    for (Size i = 0, n = 0; i < calibrationSet.size(); ++i)
    {
        ...
        const ext::shared_ptr<VanillaOption> option = calibrationSet[i].first;
        ...
        const Volatility calculatedVol = volatilityAdapter->blackVol(t, strike, true);
        ...
        const ext::shared_ptr<PricingEngine> fdEngine(ext::make_shared<FdBlackScholesVanillaEngine>(
            localVolProcess, std::max<Size>(30, Size(100 * t)), 200, 0, FdmSchemeDesc::Douglas(), true));
        ...
        const Volatility lvImpliedVol = blackFormulaImpliedStdDevLiRS(optionType, strike, fwd, option->NPV(), discount,
                                                                      0.0, Null<Real>(), 1.0, 1e-12) /
                                        std::sqrt(t);
    ...
    for (Real strike : strikes)
    {
        for (unsigned long maturityMonth : maturityMonths)
        {
            ...
            if (std::fabs(mn) < 3.71 * vol)
            {
                ...
                calibrationSet.emplace_back(
                    ext::make_shared<VanillaOption>(ext::make_shared<PlainVanillaPayoff>(Option::Call, strike),
...
BOOST_AUTO_TEST_CASE(testAndreasenHugePut)
{
    ...
    BOOST_TEST_MESSAGE("Testing Andreasen-Huge example with Put calibration...");

    const CalibrationData data = AndreasenHugeExampleData();

    const CalibrationResults expected = {AndreasenHugeVolatilityInterpl::Put,
...
BOOST_AUTO_TEST_CASE(testAndreasenHugeCall)
{
    ...
    const CalibrationResults expected = {AndreasenHugeVolatilityInterpl::Call,
                                         AndreasenHugeVolatilityInterpl::CubicSpline,
                                         0.0015,
                                         0.00035,
                                         0.0015,
...
BOOST_AUTO_TEST_CASE(testAndreasenHugeCallPut, *precondition(if_speed(Fast)))
{
    ...
    BOOST_TEST_MESSAGE("Testing Andreasen-Huge example with instantaneous "
                       "Call and Put calibration...");

    const CalibrationData data = AndreasenHugeExampleData();

    const CalibrationResults expected = {AndreasenHugeVolatilityInterpl::CallPut,
                                         AndreasenHugeVolatilityInterpl::CubicSpline,
                                         0.0015,
                                         0.00035,
                                         0.0015,
...
{
    BOOST_TEST_MESSAGE("Testing Andreasen-Huge example with linear interpolation...");
...
{
    BOOST_TEST_MESSAGE("Testing Andreasen-Huge example with piecewise constant interpolation...");
...
BOOST_AUTO_TEST_CASE(testTimeDependentInterestRates, *precondition(if_speed(Fast)))
{
    ...
    BOOST_TEST_MESSAGE("Testing Andreasen-Huge volatility interpolation with "
    ...
    const Handle<YieldTermStructure> rTS(ext::make_shared<ZeroCurve>(dates, r, dc));
    const Handle<YieldTermStructure> qTS(ext::make_shared<ZeroCurve>(dates, q, dc));

    const CalibrationData origData = AndreasenHugeExampleData();
    AndreasenHugeVolatilityInterpl::CalibrationSet calibrationSet = origData.calibrationSet;

    const Handle<Quote> spot = origData.spot;

    const ext::shared_ptr<HestonModel> hestonModel(
        ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(rTS, qTS, spot, 0.09, 2.0, 0.09, 0.4, -0.75)));
    ...
    for (auto& i : calibrationSet)
    {
        ...
        const Volatility impliedVol =
            blackFormulaImpliedStdDevLiRS(optionType, strike, fwd, npv, discount, 0.0, Null<Real>(), 1.0, 1e-12) /
            std::sqrt(t);
...
BOOST_AUTO_TEST_CASE(testSingleOptionCalibration)
{
    ...
    const AndreasenHugeVolatilityInterpl::InterpolationType interpl[] = {
        AndreasenHugeVolatilityInterpl::Linear, AndreasenHugeVolatilityInterpl::CubicSpline,
        AndreasenHugeVolatilityInterpl::PiecewiseConstant};

    const AndreasenHugeVolatilityInterpl::CalibrationType calibrationType[] = {AndreasenHugeVolatilityInterpl::Put,
    ...
    for (auto i : interpl)
        ...
        {
            const ext::shared_ptr<AndreasenHugeVolatilityInterpl> andreasenHugeVolInterplation(
                ext::make_shared<AndreasenHugeVolatilityInterpl>(calibrationSet, spot, rTS, qTS, i, j, 50));

            const ext::shared_ptr<AndreasenHugeVolatilityAdapter> volatilityAdapter =
                ext::make_shared<AndreasenHugeVolatilityAdapter>(andreasenHugeVolInterplation);

            const Volatility calculated = volatilityAdapter->blackVol(maturity, strike);
...
{
    BOOST_TEST_MESSAGE("Testing Andreasen-Huge volatility interpolation gives "
    ...
    for (auto& i : data)
    {
        ...
        const ext::shared_ptr<AndreasenHugeVolatilityInterpl> andreasenHugeVolInterplation(
            ext::make_shared<AndreasenHugeVolatilityInterpl>(calibrationSet, spot, rTS, qTS,
                                                             AndreasenHugeVolatilityInterpl::CubicSpline,
        ...
        for (Real m = -0.7; m < 0.7; m += 0.05)
        {
            ...
            for (Size weeks = 6; weeks < 52; ++weeks)
            {
                ...
                const Real w = volatilityAdapter->blackVariance(t, k, true);
                const Real w_p = volatilityAdapter->blackVariance(t, kp, true);
                const Real w_m = volatilityAdapter->blackVariance(t, km, true);

                const Real w1 = (w_p - w_m) / (2 * eps);
                const Real w2 = (w_p + w_m - 2 * w) / (eps * eps);

                const Real g_k = squared(1 - m * w1 / (2 * w)) - w1 * w1 / 4 * (1 / w + 0.25) + 0.5 * w2;
                ...
                const Real fwdpt = spot->value() * qTS->discount(t + deltaT) / rTS->discount(t + deltaT);

                const Real kpt = fwdpt * std::exp(m);
                const Real w_pt = volatilityAdapter->blackVariance(t + deltaT, kpt, true);
...
{
    BOOST_TEST_MESSAGE("Testing Barrier option pricing with Andreasen-Huge "
    ...
    Handle<Quote> spot(ext::make_shared<SimpleQuote>(100));
    const ext::shared_ptr<HestonModel> hestonModel(
        ext::make_shared<HestonModel>(ext::make_shared<HestonProcess>(rTS, qTS, spot, 0.04, 2.0, 0.04, 0.4, -0.75)));

    const ext::shared_ptr<BlackVolTermStructure> hestonVol =
        ext::make_shared<HestonBlackVolSurface>(Handle<HestonModel>(hestonModel));

    const ext::shared_ptr<GeneralizedBlackScholesProcess> dupireLocalVolProcess =
        ext::make_shared<GeneralizedBlackScholesProcess>(spot, qTS, rTS, Handle<BlackVolTermStructure>(hestonVol));
    ...
    const ext::shared_ptr<GeneralizedBlackScholesProcess> andreasenHugeLocalVolProcess =
        ext::make_shared<GeneralizedBlackScholesProcess>(spot, qTS, rTS, Handle<BlackVolTermStructure>(hestonVol),
                                                         Handle<LocalVolTermStructure>(localVolAdapter));
    ...
    BarrierOption barrierOption(barrierType, barrier, rebate, ext::make_shared<PlainVanillaPayoff>(Option::Put, strike),
                                ext::make_shared<EuropeanExercise>(maturity));

    barrierOption.setPricingEngine(ext::make_shared<FdBlackScholesBarrierEngine>(dupireLocalVolProcess, 50, 100, 0,
                                                                                 FdmSchemeDesc::Douglas(), true, 0.2));

    const Real dupireNPV = barrierOption.NPV();

    barrierOption.setPricingEngine(ext::make_shared<FdBlackScholesBarrierEngine>(
        andreasenHugeLocalVolProcess, 200, 400, 0, FdmSchemeDesc::Douglas(), true, 0.25));
...
{
    BOOST_TEST_MESSAGE("Testing Peter's and Fabien's SABR example...");
    ...
    const ext::shared_ptr<AndreasenHugeVolatilityInterpl> andreasenHugeVolInterplation(
        ext::make_shared<AndreasenHugeVolatilityInterpl>(data.calibrationSet, data.spot, data.rTS, data.qTS));
    ...
    {
        const Volatility sabrVol = sabrVolatility(strike, forward, maturity, alpha, beta, nu, rho);
...
{
    BOOST_TEST_MESSAGE("Testing different optimizer for Andreasen-Huge "
                       "volatility interpolation...");

    const CalibrationData& data = sabrData().first;

    const ext::shared_ptr<OptimizationMethod> optimizationMethods[] = {
        ext::make_shared<LevenbergMarquardt>(), ext::make_shared<BFGS>(), ext::make_shared<Simplex>(0.2)};

    for (const auto& optimizationMethod : optimizationMethods)
    {
        const Real avgError =
            std::get<2>(AndreasenHugeVolatilityInterpl(data.calibrationSet, data.spot, data.rTS, data.qTS,
                                                       AndreasenHugeVolatilityInterpl::CubicSpline,
...
{
    BOOST_TEST_MESSAGE("Testing that reference date of adapter surface moves along with "
                       "evaluation date...");
    ...
    AndreasenHugeVolatilityInterpl::CalibrationSet calibrationSet(
        1, std::make_pair(ext::make_shared<VanillaOption>(ext::make_shared<PlainVanillaPayoff>(Option::Call, s0),
                                                          ext::make_shared<EuropeanExercise>(maturity)),
                          ext::make_shared<SimpleQuote>(impliedVol)));

    const ext::shared_ptr<AndreasenHugeVolatilityInterpl> andreasenHugeVolInterplation(
        ext::make_shared<AndreasenHugeVolatilityInterpl>(calibrationSet, spot, ts, ts));


    const Real tol = 1e-8;
    const ext::shared_ptr<AndreasenHugeVolatilityAdapter> volatilityAdapter(
        ext::make_shared<AndreasenHugeVolatilityAdapter>(andreasenHugeVolInterplation, tol));
    ...
    if (modVolRefDate != modToday || modLocalRefDate != modToday)
        BOOST_FAIL("reference dates should match modified today's date"
                   << "\n    today                     : " << modToday << "\n    local vol reference date  : "
                   << modLocalRefDate << "\n    implied vol reference date: " << modVolRefDate);

    // test update method
    const Volatility modImpliedVol = volatilityAdapter->blackVol(maturity, s0, true);
...
{
    BOOST_TEST_MESSAGE("Testing Andreasen-Huge example with flat volatility surface...");

    const Date ref(1, November, 2019);
    const DayCounter dc = Actual365Fixed();
    Settings::instance().evaluationDate() = ref;

    const Date expiries[] = {ref + 1 * Months, ref + 3 * Months, ref + 6 * Months, ref + 9 * Months,
                             ref + 1 * Years,  ref + 2 * Years,  ref + 3 * Years,  ref + 4 * Years,
                             ref + 5 * Years,  ref + 7 * Years,  ref + 10 * Years};

    const Real moneyness[] = {0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5};
    ...
    for (auto expiry : expiries)
    {
        ...
        for (Real m : moneyness)
        {
            ...
            {
                const ext::shared_ptr<VanillaOption> option = ext::make_shared<VanillaOption>(
                    ext::make_shared<PlainVanillaPayoff>((strike > fwd) ? Option::Call : Option::Put, strike),
                    exercise);
    ...
    const CalibrationResults expected = {
        AndreasenHugeVolatilityInterpl::Put, AndreasenHugeVolatilityInterpl::CubicSpline, 1e-10, 1e-10, 0.0006, 0.0002};
