...
#define REPORT_FAILURE(greekName, payoff, exercise, s, q, r, today, sigma, nu, theta, expected, calculated, error, \
                       tolerance)                                                                                  \
...
BOOST_AUTO_TEST_CASE(testVarianceGamma)
{
    ...
    VarianceGammaProcessData processes[] = {//    spot,    q,    r,sigma,   nu, theta
                                            {6000, 0.00, 0.05, 0.20, 0.05, -0.50},
                                            {6000, 0.02, 0.05, 0.15, 0.01, -0.50}};

    VarianceGammaOptionData options[] = {
        //            type,strike,  t
        {Option::Call, 5550, 1.0}, {Option::Call, 5600, 1.0}, {Option::Call, 5650, 1.0}, {Option::Call, 5700, 1.0},
        {Option::Call, 5750, 1.0}, {Option::Call, 5800, 1.0}, {Option::Call, 5850, 1.0}, {Option::Call, 5900, 1.0},
        {Option::Call, 5950, 1.0}, {Option::Call, 6000, 1.0}, {Option::Call, 6050, 1.0}, {Option::Call, 6100, 1.0},
        {Option::Call, 6150, 1.0}, {Option::Call, 6200, 1.0}, {Option::Call, 6250, 1.0}, {Option::Call, 6300, 1.0},
        {Option::Call, 6350, 1.0}, {Option::Call, 6400, 1.0}, {Option::Call, 6450, 1.0}, {Option::Call, 6500, 1.0},
        {Option::Call, 6550, 1.0}, {Option::Put, 5550, 1.0}};

    Real results[std::size(processes)][std::size(options)] = {
        {955.1637, 922.7529, 890.9872, 859.8739, 829.4197, 799.6303, 770.5104, 742.0640, 714.2943, 687.2032, 660.7921,
         635.0613, 610.0103, 585.6379, 561.9416, 538.9186, 516.5649, 494.8760, 473.8464, 453.4700, 433.7400, 234.4870},
        {732.8705, 698.5542, 665.1404, 632.6498, 601.1002, 570.5068, 540.8824, 512.2367, 484.5766, 457.9064, 432.2273,
         407.5381, 383.8346, 361.1102, 339.3559, 318.5599, 298.7087, 279.7864, 261.7751, 244.6552, 228.4057, 130.9974}};
    ...
    for (Size i = 0; i < std::size(processes); i++)
    {
        ...
        ext::shared_ptr<VarianceGammaProcess> stochProcess(new VarianceGammaProcess(
            Handle<Quote>(spot), Handle<YieldTermStructure>(qTS), Handle<YieldTermStructure>(rTS), processes[i].sigma,
            processes[i].nu, processes[i].theta));

        // Analytic engine
        ext::shared_ptr<PricingEngine> analyticEngine(new VarianceGammaEngine(stochProcess));

        // FFT engine
        ext::shared_ptr<FFTVarianceGammaEngine> fftEngine(new FFTVarianceGammaEngine(stochProcess));
        ...
        for (Size j = 0; j < std::size(options); j++)
        {
            ...
            ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(options[j].type, options[j].strike));
            ...
            if (error > tol)
            {
                REPORT_FAILURE("analytic value", payoff, exercise, processes[i].s, processes[i].q, processes[i].r,
                               today, processes[i].sigma, processes[i].nu, processes[i].theta, expected, calculated,
                               error, tol);
            }
        ...
        for (Size j = 0; j < std::size(options); j++)
        {
            ...
            if (error > tol)
            {
                ...
                REPORT_FAILURE("fft value", payoff, option->exercise(), processes[i].s, processes[i].q, processes[i].r,
                               today, processes[i].sigma, processes[i].nu, processes[i].theta, expected, calculated,
                               error, tol);
            }
        }
...
BOOST_AUTO_TEST_CASE(testSingularityAtZero)
{
    ...
    BOOST_TEST_MESSAGE("Testing variance-gamma model integration around zero...");
    ...
    Handle<YieldTermStructure> dividend(ext::make_shared<FlatForward>(valuation, 0.0, discountCounter));
    Handle<YieldTermStructure> disc(ext::make_shared<FlatForward>(valuation, 0.05, discountCounter));
    Handle<Quote> S0(ext::make_shared<SimpleQuote>(stock));
    ext::shared_ptr<QuantLib::VarianceGammaProcess> process =
        ext::make_shared<VarianceGammaProcess>(S0, dividend, disc, sigma, kappa, mu);
