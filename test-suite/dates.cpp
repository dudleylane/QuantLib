...
BOOST_AUTO_TEST_CASE(ecbDates)
{
    ...
    BOOST_TEST(!knownDates.empty(), "empty ECB date vector");

    const Size n = ECB::nextDates(Date::minDate()).size();
    BOOST_TEST(n == knownDates.size(),
               "nextDates(minDate) returns " << n << " instead of " << knownDates.size() << " dates");
    ...
    for (const Date& currentEcbDate : knownDates)
    {
        ...
        if (ECB::nextDate(ecbDateMinusOne) != currentEcbDate)
            BOOST_FAIL("next ECB date following " << ecbDateMinusOne << " must be " << currentEcbDate);

        if (ECB::nextDate(previousEcbDate) != currentEcbDate)
            BOOST_FAIL("next ECB date following " << previousEcbDate << " must be " << currentEcbDate);
    ...
    BOOST_TEST(!ECB::isECBdate(knownDate), "unable to remove an ECB date");
    ECB::addDate(knownDate);
    BOOST_TEST(ECB::isECBdate(knownDate), "unable to add an ECB date");
}
...
BOOST_AUTO_TEST_CASE(immDates)
{
    ...
    const std::string IMMcodes[] = {
        "F0", "G0", "H0", "J0", "K0", "M0", "N0", "Q0", "U0", "V0", "X0", "Z0", "F1", "G1", "H1", "J1", "K1", "M1",
        "N1", "Q1", "U1", "V1", "X1", "Z1", "F2", "G2", "H2", "J2", "K2", "M2", "N2", "Q2", "U2", "V2", "X2", "Z2",
        "F3", "G3", "H3", "J3", "K3", "M3", "N3", "Q3", "U3", "V3", "X3", "Z3", "F4", "G4", "H4", "J4", "K4", "M4",
        "N4", "Q4", "U4", "V4", "X4", "Z4", "F5", "G5", "H5", "J5", "K5", "M5", "N5", "Q5", "U5", "V5", "X5", "Z5",
        "F6", "G6", "H6", "J6", "K6", "M6", "N6", "Q6", "U6", "V6", "X6", "Z6", "F7", "G7", "H7", "J7", "K7", "M7",
        "N7", "Q7", "U7", "V7", "X7", "Z7", "F8", "G8", "H8", "J8", "K8", "M8", "N8", "Q8", "U8", "V8", "X8", "Z8",
    ...
    while (counter <= last)
    {
        ...
        if (imm <= counter)
            BOOST_FAIL(imm.weekday() << " " << imm << " is not greater than " << counter.weekday() << " " << counter);

        // check that imm is an IMM date
        if (!IMM::isIMMdate(imm, false))
            BOOST_FAIL(imm.weekday() << " " << imm << " is not an IMM date (calculated from " << counter.weekday()
                                     << " " << counter << ")");

        // check that imm is <= to the next IMM date in the main cycle
        if (imm > IMM::nextDate(counter, true))
            BOOST_FAIL(imm.weekday() << " " << imm << " is not less than or equal to the next future in the main cycle "
                                     << IMM::nextDate(counter, true));

        // check that for every date IMMdate is the inverse of IMMcode
        if (IMM::date(IMM::code(imm), counter) != imm)
            BOOST_FAIL(IMM::code(imm) << " at calendar day " << counter << " is not the IMM code matching " << imm);
        ...
        for (int i = 0; i < 40; ++i)
        {
            if (IMM::date(IMMcodes[i], counter) < counter)
                BOOST_FAIL(IMM::date(IMMcodes[i], counter)
                           << " is wrong for " << IMMcodes[i] << " at reference date " << counter);
        }
...
BOOST_AUTO_TEST_CASE(asxDates)
{
    ...
    for (Date counter(1, January, 2000); counter <= Date(1, January, 2040); ++counter)
    {
        ...
        if (asx <= counter)
            BOOST_FAIL(asx.weekday() << " " << asx << " is not greater than " << counter.weekday() << " " << counter);

        // check that asx is an ASX date
        if (!ASX::isASXdate(asx, false))
            BOOST_FAIL(asx.weekday() << " " << asx << " is not an ASX date (calculated from " << counter.weekday()
                                     << " " << counter << ")");

        // check that asx is <= to the next ASX date in the main cycle
        if (asx > ASX::nextDate(counter, true))
            BOOST_FAIL(asx.weekday() << " " << asx << " is not less than or equal to the next future in the main cycle "
                                     << ASX::nextDate(counter, true));

        // check that for every date ASXdate is the inverse of ASXcode
        if (ASX::date(ASX::code(asx), counter) != asx)
            BOOST_FAIL(ASX::code(asx) << " at calendar day " << counter << " is not the ASX code matching " << asx);
        ...
        for (const auto& ASXcode : ASXcodes)
        {
            if (ASX::date(ASXcode, counter) < counter)
                BOOST_FAIL(ASX::date(ASXcode, counter)
                           << " is wrong for " << ASXcode << " at reference date " << counter);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testConsistency)
{
    ...
    Date::serial_type minDate = Date::minDate().serialNumber() + 1, maxDate = Date::maxDate().serialNumber();

    Date::serial_type dyold = Date(minDate - 1).dayOfYear(), dold = Date(minDate - 1).dayOfMonth(),
                      mold = Date(minDate - 1).month(), yold = Date(minDate - 1).year(),
    ...
    for (Date::serial_type i = minDate; i <= maxDate; i++)
    {
        ...
        Integer dy = t.dayOfYear(), d = t.dayOfMonth(), m = t.month(), y = t.year(), wd = t.weekday();
        ...
        if (d < 1)
            BOOST_FAIL("invalid day of month: \n"
                       << "    date:  " << t << "\n"
                       << "    day: " << d);
        if (!((m == 1 && d <= 31) || (m == 2 && d <= 28) || (m == 2 && d == 29 && Date::isLeap(y)) ||
...
BOOST_AUTO_TEST_CASE(isoDates)
{
    ...
    if (d.dayOfMonth() != 15 || d.month() != January || d.year() != 2006)
    {
...
BOOST_AUTO_TEST_CASE(intraday)
{
    ...
    BOOST_CHECK_MESSAGE(d1.minutes() == 45, "failed to reproduce minute of hour");
    BOOST_CHECK_MESSAGE(d1.seconds() == 13, "failed to reproduce second of minute");

    if (Date::ticksPerSecond() == 1000)
        BOOST_CHECK_MESSAGE(d1.fractionOfSecond() == 0.234, "failed to reproduce fraction of second");
    ...
    if (Date::ticksPerSecond() >= 1000)
        BOOST_CHECK_MESSAGE(d1.milliseconds() == 234 + 76, "failed to reproduce number of milliseconds");

    if (Date::ticksPerSecond() >= 1000000)
        BOOST_CHECK_MESSAGE(d1.microseconds() == 253, "failed to reproduce number of microseconds");
    ...
    BOOST_CHECK_MESSAGE(d2.minutes() == 52, "failed to reproduce minute of hour");
    BOOST_CHECK_MESSAGE(d2.seconds() == 57, "failed to reproduce second of minute");

    if (Date::ticksPerSecond() >= 1000)
        BOOST_CHECK_MESSAGE(d2.milliseconds() == 234, "failed to reproduce number of milliseconds");
    if (Date::ticksPerSecond() >= 1000000)
        BOOST_CHECK_MESSAGE(d2.microseconds() == 253, "failed to reproduce number of microseconds");
    ...
    BOOST_CHECK_MESSAGE(d3 + Period(23, Hours) == Date(11, April, 2023, 10, 43, 13, 234, 253), "failed to add hours");

    BOOST_CHECK_MESSAGE(d3 + Period(2, Minutes) == Date(10, April, 2023, 11, 45, 13, 234, 253),
                        "failed to add minutes");

    BOOST_CHECK_MESSAGE(d3 + Period(-2, Seconds) == Date(10, April, 2023, 11, 43, 11, 234, 253),
                        "failed to add seconds");

    BOOST_CHECK_MESSAGE(d3 + Period(-20, Milliseconds) == Date(10, April, 2023, 11, 43, 13, 214, 253),
                        "failed to add milliseconds");

    BOOST_CHECK_MESSAGE(d3 + Period(20, Microseconds) == Date(10, April, 2023, 11, 43, 13, 234, 273),
                        "failed to add microseconds");
}
#endif
