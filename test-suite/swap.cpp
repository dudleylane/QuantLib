...
struct CommonVars
{
    ...
    // utilities
    ext::shared_ptr<VanillaSwap> makeSwap(Integer length,
                                          Rate fixedRate,
                                          Spread floatingSpread,
                                          DateGeneration::Rule rule = DateGeneration::Forward) const
    {
        Date maturity = calendar.advance(settlement, length, Years, floatingConvention);
        Schedule fixedSchedule(settlement, maturity, Period(fixedFrequency), calendar, fixedConvention, fixedConvention,
                               rule, false);
        Schedule floatSchedule(settlement, maturity, Period(floatingFrequency), calendar, floatingConvention,
    ...
    CommonVars()
    {
        ...
        index = ext::shared_ptr<IborIndex>(new Euribor(Period(floatingFrequency), termStructure));
        ...
        for (Real spread : spreads)
        {
            ...
            if (it != swap_values.end())
            {
                ...
                BOOST_ERROR("NPV is increasing with the fixed rate in a swap: \n"
                            << "    length: " << length << " years\n"
                            << "    value:  " << swap_values[n] << " paying fixed rate: " << io::rate(rates[n]) << "\n"
...
BOOST_AUTO_TEST_CASE(testSpreadDependency)
{
    ...
    for (int& length : lengths)
    {
        for (Real j : rates)
        {
            ...
            // and check that they go the right way
            auto it = std::adjacent_find(swap_values.begin(), swap_values.end(), std::greater<>());
            if (it != swap_values.end())
            {
                Size n = it - swap_values.begin();
...
BOOST_AUTO_TEST_CASE(testInArrears)
{
    ...
    */
    ...
    Schedule schedule(vars.today, maturity, Period(Annual), calendar, Following, Following, DateGeneration::Forward,
                      false);
    DayCounter dayCounter = SimpleDayCounter();
    std::vector<Real> nominals(1, 100000000.0);
    ext::shared_ptr<IborIndex> index(new IborIndex("dummy", 1 * Years, 0, EURCurrency(), calendar, Following, false,
                                                   dayCounter, vars.termStructure));
    Rate oneYear = 0.05;
    ...
    Leg fixedLeg = FixedRateLeg(schedule).withNotionals(nominals).withCouponRates(coupons, dayCounter);
    ...
    Handle<OptionletVolatilityStructure> vol(ext::shared_ptr<OptionletVolatilityStructure>(
        new ConstantOptionletVolatility(vars.today, NullCalendar(), Following, capletVolatility, dayCounter)));
...
BOOST_AUTO_TEST_CASE(testCachedValue)
{
    ...
    if (swap->numberOfLegs() != 2)
        BOOST_ERROR("failed to return correct number of legs:\n"
                    << std::fixed << std::setprecision(12) << "    calculated: " << swap->numberOfLegs() << "\n"
                    << "    expected:   " << 2);
...
BOOST_AUTO_TEST_CASE(testNotifications)
{
    ...
    auto swap = ext::make_shared<VanillaSwap>(Swap::Payer, nominal, schedule, 0.03, Actual365Fixed(), schedule, index,
                                              0.0, Actual365Fixed());
...
BOOST_AUTO_TEST_CASE(testFixedTenorInferenceWithTerminationDate)
{
    ...
    ext::shared_ptr<VanillaSwap> aud3Y =
        MakeVanillaSwap(3 * Years, audIndex, 0.03).withEffectiveDate(startDate).withTerminationDate(endDate3Y);
    ...
    ext::shared_ptr<VanillaSwap> gbpNoEffDate =
        MakeVanillaSwap(10 * Years, gbpIndex, 0.03).withTerminationDate(endDateSettlement);
    ...
    ext::shared_ptr<VanillaSwap> gbpMismatch =
        MakeVanillaSwap(6 * Months, gbpIndex, 0.03).withEffectiveDate(startDate).withTerminationDate(endDate10Y);
    ...
    ext::shared_ptr<VanillaSwap> gbpOverride = MakeVanillaSwap(10 * Years, gbpIndex, 0.03)
...
BOOST_AUTO_TEST_CASE(testSettlementDaysEffectiveDateConflict)
{
    ...
    BOOST_CHECK_EXCEPTION(
        ext::shared_ptr<VanillaSwap> swap =
            MakeVanillaSwap(5 * Years, index, 0.03).withSettlementDays(2).withEffectiveDate(effectiveDate),
        Error, ExpectedErrorMessage("cannot set both"));

    // effectiveDate first, then settlementDays
    BOOST_CHECK_EXCEPTION(
        ext::shared_ptr<VanillaSwap> swap =
            MakeVanillaSwap(5 * Years, index, 0.03).withEffectiveDate(effectiveDate).withSettlementDays(2),
        Error, ExpectedErrorMessage("cannot set both"));

    // withSettlementDays alone works
    ext::shared_ptr<VanillaSwap> swap1 = MakeVanillaSwap(5 * Years, index, 0.03).withSettlementDays(2);
    BOOST_CHECK(swap1->startDate() != Date());

    // withEffectiveDate alone works
    ext::shared_ptr<VanillaSwap> swap2 = MakeVanillaSwap(5 * Years, index, 0.03).withEffectiveDate(effectiveDate);
    BOOST_CHECK_EQUAL(swap2->startDate(), effectiveDate);

    // neither set (constructor defaults) works
    ext::shared_ptr<VanillaSwap> swap3 = MakeVanillaSwap(5 * Years, index, 0.03);
