...
BOOST_AUTO_TEST_CASE(testFunction)
{
    ...
    CTSMMCapletMaxHomogeneityCalibration calibrator(evolution, corr, swapVariances, capletVols_, cs, displacement_,
...
BOOST_AUTO_TEST_CASE(testPeriodFunction)
{
    ...
    VolatilityInterpolationSpecifierabcd varianceInterpolator(
        period, offset, swapVariances, // these should be associated with the long rates
    ...
    /*Integer failures =*/capletSwaptionPeriodicCalibration(
        evolution, corr, varianceInterpolator, capletVols_, cs, displacement_, caplet0Swaption1Priority,
        numberOfFactors_, period, max1dIterations, tolerance1d, maxUnperiodicIterations, toleranceUnperiodic,
        maxPeriodIterations, periodTolerance, deformationSize,
        totalSwaptionError,     // ?
    ...
    for (Size i = 0; i < numberBigRates; ++i)
    {
        ...
        error = std::fabs(swaptionVol - expSwaptionVol);
        if (error > swapTolerance)
            BOOST_ERROR("failed to reproduce " << io::ordinal(i)
                                               << " swaption vol:"
                                                  "\n expected:  "
                                               << io::rate(expSwaptionVol) << "\n realized:  " << io::rate(swaptionVol)
                                               << "\n error:     " << error << "\n tolerance: " << swapTolerance);
...
BOOST_AUTO_TEST_CASE(testSphereCylinder)
{
...

    {
        ...
        if (fabs(y1 - 1.0) > errorTol)
            BOOST_ERROR("\n failed to reproduce y1=1: " << y1 << ", " << y2 << ", " << y3);

        if (fabs(y2 - 0.0) > errorTol)
            BOOST_ERROR("\n failed to reproduce y2=0: " << y1 << ", " << y2 << ", " << y3);

        if (fabs(y3 - 0.0) > errorTol)
            BOOST_ERROR("\n failed to reproduce y3=0: " << y1 << ", " << y2 << ", " << y3);


        optimizer.findByProjection(y1, y2, y3);

        if (fabs(y1 - 1.0) > errorTol)
            BOOST_ERROR("\nfindByProjection failed to reproduce y1=1: " << y1 << ", " << y2 << ", " << y3);

        if (fabs(y2 - 0.0) > errorTol)
            BOOST_ERROR("\n findByProjection failed to reproduce y2=0: " << y1 << ", " << y2 << ", " << y3);

        if (fabs(y3 - 0.0) > errorTol)
            BOOST_ERROR("\n findByProjection failed to reproduce y3=0: " << y1 << ", " << y2 << ", " << y3);
    }

    {
        ...
        if (fabs(y1 - 1.03306) > errorTol)
            BOOST_ERROR("\n failed to reproduce y1=1.03306: " << y1 << ", " << y2 << ", " << y3);

        if (fabs(y2 - 0.999453) > errorTol)
            BOOST_ERROR("\n failed to reproduce y2=0.999453: " << y1 << ", " << y2 << ", " << y3);

        if (fabs(y3 - 4.78893) > errorTol)
            BOOST_ERROR("\n failed to reproduce y3=4.78893: " << y1 << ", " << y2 << ", " << y3);
        ...
        if (fabs(y2 - 1.0) > errorTol)
            BOOST_ERROR("\n findByProjection failed to reproduce y2 =1: " << y1 << " " << y2 << " " << y3);

        if (fabs(y3 - sqrt(23.0)) > errorTol)
            BOOST_ERROR("\n findByProjection failed to reproduce y3 =sqrt(23): " << y1 << " " << y2 << " " << y3);
    }
}
