...
BOOST_AUTO_TEST_CASE(testMalformedJsonRejected)
{
    ...
    auto open1 = [&]
    {
        JsonQuoteLoader l(p1);
        (void)l;
    };
    BOOST_CHECK_EXCEPTION(open1(), Error, ExpectedErrorMessage("parse error"));
    std::remove(p1.c_str());

    std::string p2 = writeTemp(R"({"USD3M": "not a number"})");
    auto open2 = [&]
    {
        JsonQuoteLoader l(p2);
        (void)l;
    };
    BOOST_CHECK_EXCEPTION(open2(), Error, ExpectedErrorMessage("not a number"));
    std::remove(p2.c_str());

    std::string p3 = writeTemp("[]"); // top-level array rejected
    auto open3 = [&]
    {
        JsonQuoteLoader l(p3);
        (void)l;
    };
    BOOST_CHECK_EXCEPTION(open3(), Error, ExpectedErrorMessage("top-level JSON must be an object"));
    std::remove(p3.c_str());
}
...
BOOST_AUTO_TEST_CASE(testAmbiguousSchemaRejected)
{
    ...
    BOOST_CHECK_EXCEPTION(open(), Error, ExpectedErrorMessage("ambiguous schema"));
    std::remove(p.c_str());
}
...
BOOST_AUTO_TEST_CASE(testEmptyObjectRejected)
{
    ...
    BOOST_CHECK_EXCEPTION(open(), Error, ExpectedErrorMessage("no quote rows loaded"));
    std::remove(p.c_str());
}
...
BOOST_AUTO_TEST_CASE(testDisabledBuildThrows)
{
    ...
    BOOST_CHECK_EXCEPTION(open(), Error, ExpectedErrorMessage("JSON market-data support is disabled"));
    std::remove(p.c_str());
}
