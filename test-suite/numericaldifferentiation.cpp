...
void checkTwoArraysAreTheSame(const Array& calculated, const Array& expected)
{
    bool correct = (calculated.size() == expected.size()) &&
                   std::equal(calculated.begin(), calculated.end(), expected.begin(), isTheSame);
...
void singleValueTest(const std::string& comment, Real calculated, Real expected, Real tol)
{
...
BOOST_AUTO_TEST_CASE(testTabulatedCentralScheme)
{
    ...
    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 1, 1.0, 3, central).weights(), {-0.5, 0.0, 0.5});

    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 1, 0.5, 3, central).weights(), {-1.0, 0.0, 1.0});

    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 1, 0.25, 7, central).weights(),
    ...
    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 1, 0.5, 7, central).offsets(),
...
BOOST_AUTO_TEST_CASE(testTabulatedBackwardScheme)
{
    ...
    const NumericalDifferentiation::Scheme backward = NumericalDifferentiation::Backward;

    // see http://en.wikipedia.org/wiki/Finite_difference_coefficient
    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 1, 1.0, 2, backward).weights(), {1.0, -1.0});

    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 2, 2.0, 4, backward).weights(),
                             {2 / 4.0, -5 / 4.0, 4 / 4.0, -1.0 / 4.0});

    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 4, 1.0, 6, backward).weights(),
                             {3.0, -14.0, 26.0, -24.0, 11.0, -2.0});

    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 2, 0.5, 4, backward).offsets(), {0.0, -0.5, -1.0, -1.5});
}
...
BOOST_AUTO_TEST_CASE(testTabulatedForwardScheme)
{
    ...
    const NumericalDifferentiation::Scheme forward = NumericalDifferentiation::Forward;

    // see http://en.wikipedia.org/wiki/Finite_difference_coefficient
    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 1, 1.0, 2, forward).weights(), {-1.0, 1.0});

    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 1, 0.5, 3, forward).weights(), {-6 / 2.0, 4.0, -2 / 2.0});

    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 1, 0.5, 7, forward).weights(),
                             {-98 / 20.0, 12.0, -30 / 2.0, 40 / 3.0, -30 / 4.0, 12 / 5.0, -2 / 6.0});

    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 2, 0.5, 4, forward).offsets(), {0.0, 0.5, 1.0, 1.5});
}
...
BOOST_AUTO_TEST_CASE(testIrregularSchemeFirstOrder)
{
    ...
    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 1, offsets).weights(), {alpha, beta, gamma});
}
...
BOOST_AUTO_TEST_CASE(testIrregularSchemeSecondOrder)
{
    ...
    checkTwoArraysAreTheSame(NumericalDifferentiation(f, 2, offsets).weights(), {alpha, beta, gamma});
}
...
BOOST_AUTO_TEST_CASE(testDerivativesOfSineFunction)
{
    ...
    const std::function<Real(Real)> df_central =
        NumericalDifferentiation(f, 1, std::sqrt(QL_EPSILON), 3, NumericalDifferentiation::Central);

    const std::function<Real(Real)> df_backward =
        NumericalDifferentiation(f, 1, std::sqrt(QL_EPSILON), 3, NumericalDifferentiation::Backward);

    const std::function<Real(Real)> df_forward =
        NumericalDifferentiation(f, 1, std::sqrt(QL_EPSILON), 3, NumericalDifferentiation::Forward);
    ...
    const std::function<Real(Real)> df4_central =
        NumericalDifferentiation(f, 4, 1e-2, 7, NumericalDifferentiation::Central);
    const std::function<Real(Real)> df4_backward =
        NumericalDifferentiation(f, 4, 1e-2, 7, NumericalDifferentiation::Backward);
    const std::function<Real(Real)> df4_forward =
        NumericalDifferentiation(f, 4, 1e-2, 7, NumericalDifferentiation::Forward);
...
Array vandermondeCoefficients(Size order, Real x, const Array& gridPoints)
{
...
BOOST_AUTO_TEST_CASE(testCoefficientBasedOnVandermonde)
{
    ...
    {
        for (Natural nGridPoints = order + 1; nGridPoints < order + 3; ++nGridPoints)
        {
            ...
            const Array weightsVandermonde = vandermondeCoefficients(order, x, gridPoints);
