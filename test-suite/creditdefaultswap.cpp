...
BOOST_AUTO_TEST_CASE(testCachedValue)
{
    ...
    Handle<Quote> hazardRate = Handle<Quote>(ext::shared_ptr<Quote>(new SimpleQuote(0.01234)));
    RelinkableHandle<DefaultProbabilityTermStructure> probabilityCurve;
    probabilityCurve.linkTo(
        ext::shared_ptr<DefaultProbabilityTermStructure>(new FlatHazardRate(0, calendar, hazardRate, Actual360())));

    RelinkableHandle<YieldTermStructure> discountCurve;

    discountCurve.linkTo(ext::shared_ptr<YieldTermStructure>(new FlatForward(today, 0.06, Actual360())));
    ...
    Schedule schedule(issueDate, maturity, Period(frequency), calendar, convention, convention, DateGeneration::Forward,
                      false);
    ...
    cds.setPricingEngine(
        ext::shared_ptr<PricingEngine>(new MidPointCdsEngine(probabilityCurve, recoveryRate, discountCurve)));
    ...
    if (std::fabs(calculatedNpv - npv) > tolerance)
        BOOST_ERROR("Failed to reproduce NPV with mid-point engine\n"
                    << std::setprecision(10) << "    calculated NPV: " << calculatedNpv << "\n"
                    << "    expected NPV:   " << npv);

    if (std::fabs(calculatedFairRate - fairRate) > tolerance)
        BOOST_ERROR("Failed to reproduce fair rate with mid-point engine\n"
                    << std::setprecision(10) << "    calculated fair rate: " << calculatedFairRate << "\n"
                    << "    expected fair rate:   " << fairRate);

    cds.setPricingEngine(
        ext::shared_ptr<PricingEngine>(new IntegralCdsEngine(1 * Days, probabilityCurve, recoveryRate, discountCurve)));
    ...
    if (std::fabs(calculatedNpv - npv) > notional * tolerance * 10)
        BOOST_ERROR("Failed to reproduce NPV with integral engine "
                    "(step = 1 day)\n"
                    << std::setprecision(10) << "    calculated NPV: " << calculatedNpv << "\n"
                    << "    expected NPV:   " << npv);

    if (std::fabs(calculatedFairRate - fairRate) > tolerance)
        BOOST_ERROR("Failed to reproduce fair rate with integral engine "
                    "(step = 1 day)\n"
                    << std::setprecision(10) << "    calculated fair rate: " << calculatedFairRate << "\n"
                    << "    expected fair rate:   " << fairRate);

    cds.setPricingEngine(ext::shared_ptr<PricingEngine>(
        new IntegralCdsEngine(1 * Weeks, probabilityCurve, recoveryRate, discountCurve)));
...
BOOST_AUTO_TEST_CASE(testCachedMarketValue)
{
    ...
    BOOST_TEST_MESSAGE("Testing credit-default swap against cached market values...");

    Settings::instance().evaluationDate() = Date(9, June, 2006);
    Date evalDate = Settings::instance().evaluationDate();
    Calendar calendar = UnitedStates(UnitedStates::GovernmentBond);

    std::vector<Date> discountDates = {evalDate,
    ...
    RelinkableHandle<YieldTermStructure> discountCurve;
    discountCurve.linkTo(ext::shared_ptr<YieldTermStructure>(new DiscountCurve(discountDates, dfs, curveDayCounter)));

    DayCounter dayCounter = Thirty360(Thirty360::BondBasis);
    std::vector<Date> dates = {evalDate,
                               calendar.advance(evalDate, 6, Months, ModifiedFollowing),
    ...
    std::vector<Probability> defaultProbabilities = {0.0000, 0.0047, 0.0093, 0.0286, 0.0619,
    ...
    piecewiseFlatHazardRate.linkTo(ext::shared_ptr<DefaultProbabilityTermStructure>(
        new InterpolatedHazardRateCurve<BackwardFlat>(dates, hazardRates, Thirty360(Thirty360::BondBasis))));
    ...
    Schedule schedule(issueDate, maturity, Period(cdsFrequency), calendar, cdsConvention, cdsConvention,
    ...
    CreditDefaultSwap cds(Protection::Seller, cdsNotional, fixedRate, schedule, cdsConvention, dayCount, true, true);
    cds.setPricingEngine(
        ext::shared_ptr<PricingEngine>(new MidPointCdsEngine(piecewiseFlatHazardRate, recoveryRate, discountCurve)));
    ...
    if (std::fabs(npv - calculatedNpv) > tolerance)
        BOOST_ERROR("Failed to reproduce the npv for the given credit-default swap\n"
                    << std::setprecision(10) << "    computed NPV:  " << calculatedNpv << "\n"
                    << "    Given NPV:     " << npv);

    if (std::fabs(fairRate - calculatedFairRate) > tolerance)
        BOOST_ERROR("Failed to reproduce the fair rate for the given credit-default swap\n"
...
BOOST_AUTO_TEST_CASE(testImpliedHazardRate)
{
    ...
    RelinkableHandle<YieldTermStructure> discountCurve;
    discountCurve.linkTo(ext::shared_ptr<YieldTermStructure>(new FlatForward(today, 0.03, Actual360())));
    ...
    for (Integer n = 6; n <= 10; ++n)
    {
        ...
        CreditDefaultSwap cds(Protection::Seller, notional, fixedRate, schedule, convention, cdsDayCount, true, true);
        cds.setPricingEngine(
            ext::shared_ptr<PricingEngine>(new MidPointCdsEngine(probabilityCurve, recoveryRate, discountCurve)));

        Real NPV = cds.NPV();
        Rate flatRate = cds.impliedHazardRate(NPV, discountCurve, dayCounter, recoveryRate);
        ...
        probability.linkTo(ext::shared_ptr<DefaultProbabilityTermStructure>(
            new FlatHazardRate(today, Handle<Quote>(ext::shared_ptr<Quote>(new SimpleQuote(flatRate))), dayCounter)));

        CreditDefaultSwap cds2(Protection::Seller, notional, fixedRate, schedule, convention, cdsDayCount, true, true);
        cds2.setPricingEngine(
            ext::shared_ptr<PricingEngine>(new MidPointCdsEngine(probability, recoveryRate, discountCurve)));
...
BOOST_AUTO_TEST_CASE(testFairSpread)
{
    ...
    BOOST_TEST_MESSAGE("Testing fair-spread calculation for credit-default swaps...");
    ...
    ext::shared_ptr<PricingEngine> engine(new MidPointCdsEngine(probabilityCurve, recoveryRate, discountCurve));
    ...
    CreditDefaultSwap fairCds(Protection::Seller, notional, fairRate, schedule, convention, dayCount, true, true);
    ...
    if (std::fabs(fairNPV) > tolerance)
        BOOST_ERROR("Failed to reproduce null NPV with calculated fair spread\n"
...
BOOST_AUTO_TEST_CASE(testFairUpfront)
{
    ...
    BOOST_TEST_MESSAGE("Testing fair-upfront calculation for credit-default swaps...");
    ...
    ext::shared_ptr<PricingEngine> engine(new MidPointCdsEngine(probabilityCurve, recoveryRate, discountCurve, true));

    CreditDefaultSwap cds(Protection::Seller, notional, upfront, fixedRate, schedule, convention, dayCount, true, true);
    cds.setPricingEngine(engine);

    Rate fairUpfront = cds.fairUpfront();

    CreditDefaultSwap fairCds(Protection::Seller, notional, fairUpfront, fixedRate, schedule, convention, dayCount,
                              true, true);
    fairCds.setPricingEngine(engine);
    ...
    if (std::fabs(fairNPV) > tolerance)
        BOOST_ERROR("Failed to reproduce null NPV with calculated fair upfront\n"
                    << "    calculated upfront: " << io::rate(fairUpfront) << "\n"
                    << "    calculated NPV:     " << fairNPV);

    // same with null upfront to begin with
    upfront = 0.0;
    CreditDefaultSwap cds2(Protection::Seller, notional, upfront, fixedRate, schedule, convention, dayCount, true,
                           true);
    cds2.setPricingEngine(engine);

    fairUpfront = cds2.fairUpfront();

    CreditDefaultSwap fairCds2(Protection::Seller, notional, fairUpfront, fixedRate, schedule, convention, dayCount,
                               true, true);
...
BOOST_AUTO_TEST_CASE(testIsdaEngine)
{
    ...
    BOOST_TEST_MESSAGE("Testing ISDA engine calculations for credit-default swaps...");
    ...
    double dep_quotes[] = {0.003081, 0.005525, 0.007163, 0.012413, 0.014, 0.015488};
    ...
    for (size_t i = 0; i < sizeof(dep_tenors) / sizeof(int); i++)
    {
        isdaRateHelpers.push_back(ext::make_shared<DepositRateHelper>(
            dep_quotes[i], dep_tenors[i] * Months, 2, WeekendsOnly(), ModifiedFollowing, false, Actual360()));
    }
    int swap_tenors[] = {2, 3, 4, 5, 6, 7, 8, 9, 10, 12, 15, 20, 25, 30};
    double swap_quotes[] = {0.011907, 0.01699, 0.021198, 0.02444,  0.026937, 0.028967, 0.030504,
    ...
    ext::shared_ptr<IborIndex> isda_ibor = ext::make_shared<IborIndex>(
        "IsdaIbor", 3 * Months, 2, USDCurrency(), WeekendsOnly(), ModifiedFollowing, false, Actual360());
    for (size_t i = 0; i < sizeof(swap_tenors) / sizeof(int); i++)
    {
        isdaRateHelpers.push_back(ext::make_shared<SwapRateHelper>(swap_quotes[i], swap_tenors[i] * Years,
                                                                   WeekendsOnly(), Semiannual, ModifiedFollowing,
                                                                   Thirty360(Thirty360::BondBasis), isda_ibor));
    }

    RelinkableHandle<YieldTermStructure> discountCurve;
    discountCurve.linkTo(ext::make_shared<PiecewiseYieldCurve<Discount, LogLinear>>(0, WeekendsOnly(), isdaRateHelpers,
                                                                                    Actual365Fixed()));


    RelinkableHandle<DefaultProbabilityTermStructure> probabilityCurve;
    Date termDates[] = {Date(20, June, 2010), Date(20, June, 2011), Date(20, June, 2012), Date(20, June, 2016),
                        Date(20, June, 2019)};
    ...
    double markitValues[] = {-97798.29358, // 0.001
                             ...
                             1646623.672,  // 0.1
                             1579803.626,  // 0.1
                             -274298.9203, -274122.4725, 2279730.93,   2147972.527,  -592420.2297, -591571.2294,
                             3993550.206,  3545843.418,  -797501.1422, -795915.9787, 4702034.688,  4042340.999};
    ...
    for (auto termDate : termDates)
    {
        for (Real spread : spreads)
        {
            for (Real& recovery : recoveries)
            {
                ...
                Rate h = quotedTrade->impliedHazardRate(0., discountCurve, Actual365Fixed(), recovery, 1e-10,
                                                        CreditDefaultSwap::ISDA);

                probabilityCurve.linkTo(ext::make_shared<FlatHazardRate>(0, WeekendsOnly(), h, Actual365Fixed()));
                ...
                ext::shared_ptr<CreditDefaultSwap> conventionalTrade =
                    MakeCreditDefaultSwap(termDate, 0.01).withNominal(10000000.).withPricingEngine(engine);

                QL_CHECK_CLOSE(conventionalTrade->notional() * conventionalTrade->fairUpfront(), markitValues[l],
                               tolerance);
...
BOOST_AUTO_TEST_CASE(testAccrualRebateAmounts)
{
    ...
    InputData inputs = {{Date(18, Mar, 2009), 24166.67}, {Date(19, Mar, 2009), 0.00},
    ...
    for (auto& input : inputs)
    {
        ...
        CreditDefaultSwap cds = MakeCreditDefaultSwap(maturity, spread).withNominal(notional);
...
{
    BOOST_TEST_MESSAGE("Testing ISDA engine calculations for a single credit-default swap record (reconciliation)...");
    ...
    double swap_quotes[] = {-0.004820, -0.004420, -0.003990, -0.003520, -0.002970, -0.002370, -0.001760,
                            -0.001140, -0.000540, 0.000570,  0.001880,  0.002940,  0.002820};

    ext::shared_ptr<IborIndex> isda_ibor = ext::make_shared<IborIndex>(
        "IsdaIbor", 6 * Months, 2, EURCurrency(), WeekendsOnly(), ModifiedFollowing, false, Actual360());
    for (size_t i = 0; i < sizeof(swap_tenors) / sizeof(int); i++)
    {
        isdaRateHelpers.push_back(ext::make_shared<SwapRateHelper>(swap_quotes[i], swap_tenors[i] * Years,
                                                                   WeekendsOnly(), Annual, ModifiedFollowing,
                                                                   Thirty360(Thirty360::BondBasis), isda_ibor));
    }
    ...
    Rate h =
        quotedTrade->impliedHazardRate(0., discountCurve, Actual365Fixed(), recovery, 1e-10, CreditDefaultSwap::ISDA);

    probabilityCurve.linkTo(ext::make_shared<FlatHazardRate>(0, WeekendsOnly(), h, Actual365Fixed()));

    ext::shared_ptr<IsdaCdsEngine> engine =
        ext::make_shared<IsdaCdsEngine>(probabilityCurve, recovery, discountCurve, ext::nullopt, IsdaCdsEngine::Taylor,
                                        IsdaCdsEngine::HalfDayBias, IsdaCdsEngine::Piecewise);

    ext::shared_ptr<CreditDefaultSwap> conventionalTrade =
        MakeCreditDefaultSwap(instrumentMaturity, coupon).withNominal(nominal).withPricingEngine(engine);
    ...
    Real derived_accrual = df * (npv - conventionalTrade->defaultLegNPV() - conventionalTrade->couponLegNPV());
...
BOOST_AUTO_TEST_CASE(testIsdaCalculatorReconcileSingleWithIssueDateInThePast)
{
    ...
    ext::shared_ptr<CreditDefaultSwap> conventionalTrade = MakeCreditDefaultSwap(instrumentMaturity, coupon)
                                                               .withNominal(nominal)
                                                               .withPricingEngine(engine)
    ...
    Real npv = conventionalTrade->NPV();
    Real calculated_accrual = npv - conventionalTrade->defaultLegNPV() - conventionalTrade->couponLegNPV();
...
BOOST_AUTO_TEST_CASE(testDefaultConventions)
{
    ...
    ext::shared_ptr<CreditDefaultSwap> cds = MakeCreditDefaultSwap(5 * Years, 0.01);
    ...
    Schedule schedule(today - 4, termDate, 3 * Months, WeekendsOnly(), Following, Unadjusted, DateGeneration::CDS2015,
                      false);
    ...
    cds = MakeCreditDefaultSwap(5 * Years, 0.01).withNominal(10000.0).withUpfrontRate(0.02);
    ...
    cds = MakeCreditDefaultSwap(5 * Years, 0.01).withCashSettlementDays(2);
    BOOST_CHECK_EQUAL(cds->cashSettlementDays(), 2U);
    BOOST_CHECK_EQUAL(cds->upfrontPayment()->date(), today + 4);

    cds = MakeCreditDefaultSwap(5 * Years, 0.01).withCashSettlementDays(2).withUpfrontDate(today + 7);
    BOOST_CHECK_EQUAL(cds->cashSettlementDays(), 2U);
    BOOST_CHECK_EQUAL(cds->upfrontPayment()->date(), today + 7);

    cds = MakeCreditDefaultSwap(5 * Years, 0.01).withProtectionStart(today + 2);
    BOOST_CHECK_EQUAL(cds->protectionStartDate(), today + 2);

    cds = MakeCreditDefaultSwap(5 * Years, 0.01).withCouponTenor(6 * Months);
    BOOST_CHECK_EQUAL(cds->coupons().size(), 11U); // 5Y semiannually, modulo CDS conventions

    cds = MakeCreditDefaultSwap(5 * Years, 0.01).withTradeDate(today + 3);
    ...
    cds = MakeCreditDefaultSwap(5 * Years, 0.01).settleAccrual(false);
    BOOST_CHECK_EQUAL(cds->settlesAccrual(), false);

    cds = MakeCreditDefaultSwap(5 * Years, 0.01).payAtDefaultTime(false);
    BOOST_CHECK_EQUAL(cds->paysAtDefaultTime(), false);

    cds = MakeCreditDefaultSwap(5 * Years, 0.01).rebateAccrual(false);
    BOOST_CHECK_EQUAL(cds->rebatesAccrual(), false);

    cds = MakeCreditDefaultSwap(5 * Years, 0.01).withDayCounter(Actual365Fixed());
    ...
    cds = MakeCreditDefaultSwap(5 * Years, 0.01).withLastPeriodDayCounter(Actual365Fixed());
