...
std::vector<ext::shared_ptr<Helper>> makeHelpers(const std::vector<Datum>& iiData,
                                                 ...
                                                 const DayCounter& dc)
{
    ...
    for (Datum datum : iiData)
    {
        ...
        auto h = ext::make_shared<ZeroCouponInflationSwapHelper>(quote, observationLag, maturity, calendar, bdc, dc, ii,
                                                                 CPI::Flat);
...
{ // NOLINT(cppcoreguidelines-special-member-functions)
    ...
    CommonVars()
    {
        ...
        yTS.linkTo(ext::shared_ptr<YieldTermStructure>(new FlatForward(evaluationDate, 0.05, dayCounter)));
        ...
        std::vector<ext::shared_ptr<Helper>> helpers =
            makeHelpers(zciisData, ii, observationLag, calendar, convention, dayCounter);

        Date baseDate = ii->lastFixingDate();

        cpiTS.linkTo(ext::make_shared<PiecewiseZeroInflationCurve<Linear>>(evaluationDate, baseDate, ii->frequency(),
                                                                           dayCounter, helpers));
    }
...
BOOST_AUTO_TEST_CASE(testCleanPrice)
{
    ...
    CPIBond bond(settlementDays, notional, baseCPI, contractObservationLag, fixedIndex, observationInterpolation,
    ...
    {
        BOOST_FAIL("failed to reproduce expected CPI-bond dirty price" << std::fixed << std::setprecision(12)
    ...
    if (std::fabs(calculated - storedPrice) > tolerance)
    {
        BOOST_FAIL("failed to reproduce expected CPI-bond clean price" << std::fixed << std::setprecision(12)
                                                                       << "\n  expected:   " << storedPrice
                                                                       << "\n  calculated: " << calculated);
    }
}
...
BOOST_AUTO_TEST_CASE(testCPILegWithoutBaseCPI)
{
    ...
    Date settlementDate =
        fixedPaymentCalendar.advance(common.evaluationDate, settlementDays * Days, fixedPaymentConvention);

    Real npvWithBaseDate = CashFlows::npv(legWithBaseDate, **common.yTS, false, settlementDate, settlementDate);
    Real accruedsBaseDate = CashFlows::accruedAmount(legWithBaseDate, false, settlementDate);

    Real npvWithBaseCPI = CashFlows::npv(legWithBaseCPI, **common.yTS, false, settlementDate, settlementDate);
