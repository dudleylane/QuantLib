...
struct CommonVars
{
    ...
    CommonVars()
    {
        ...
        iborIndex = ext::shared_ptr<IborIndex>(new Euribor(Period(floatingFrequency), termStructure));
        Calendar calendar = iborIndex->fixingCalendar();
        swapIndex = ext::make_shared<SwapIndex>("EuriborSwapIsdaFixA", 10 * Years, swapSettlementDays,
                                                iborIndex->currency(), calendar, Period(fixedFrequency),
                                                fixedConvention, iborIndex->dayCounter(), iborIndex);
        spread = 0.0;
        nonnullspread = 0.003;
        ...
        Handle<SwaptionVolatilityStructure> swaptionVolatilityStructure(ext::shared_ptr<SwaptionVolatilityStructure>(
            new ConstantSwaptionVolatility(today, NullCalendar(), Following, 0.2, Actual365Fixed())));
        Handle<Quote> meanReversionQuote(ext::shared_ptr<Quote>(new SimpleQuote(0.01)));
        cmspricer = ext::shared_ptr<CmsCouponPricer>(
            new AnalyticHaganPricer(swaptionVolatilityStructure, GFunctionFactory::Standard, meanReversionQuote));
    }
};
...
BOOST_AUTO_TEST_CASE(testConsistency)
{
    ...
    Schedule bondSchedule(Date(4, January, 2005), Date(4, January, 2037), Period(Annual), bondCalendar, Unadjusted,
                          Unadjusted, DateGeneration::Backward, false);
    ext::shared_ptr<Bond> bond(new FixedRateBond(settlementDays, vars.faceAmount, bondSchedule,
                                                 std::vector<Rate>(1, 0.04), ActualActual(ActualActual::ISDA),
                                                 Following, 100.0, Date(4, January, 2005)));
    ...
    AssetSwap assetSwap2(payFixedRate, bond, fairCleanPrice, vars.iborIndex, vars.spread, Schedule(),
                         vars.iborIndex->dayCounter(), isPar);
    ...
    swapEngine = ext::shared_ptr<PricingEngine>(
        new DiscountingSwapEngine(vars.termStructure, true, bond->settlementDate(), bond->settlementDate()));
    ...
    assetSwap2 = AssetSwap(payFixedRate, bond, fairCleanPrice, vars.iborIndex, vars.spread, Schedule(),
                           vars.iborIndex->dayCounter(), isPar);
    assetSwap2.setPricingEngine(swapEngine);
    if (std::fabs(assetSwap2.NPV()) > tolerance)
    {
    ...
    Schedule overnightSchedule(bond->settlementDate(), bond->maturityDate(), 6 * Months, overnight->fixingCalendar(),
                               overnight->businessDayConvention(), overnight->businessDayConvention(),
                               DateGeneration::Backward, false);

    parAssetSwap = AssetSwap(payFixedRate, bond, bondPrice, overnight, vars.spread, overnightSchedule,
                             overnight->dayCounter(), isPar);

    swapEngine = ext::make_shared<DiscountingSwapEngine>(vars.termStructure, true, bond->settlementDate(),
    ...
    assetSwap2 = AssetSwap(payFixedRate, bond, fairCleanPrice, overnight, vars.spread, overnightSchedule,
                           overnight->dayCounter(), isPar);
    assetSwap2.setPricingEngine(swapEngine);
    if (std::fabs(assetSwap2.NPV()) > tolerance)
    {
    ...
    AssetSwap assetSwap4(payFixedRate, bond, fairCleanPrice, vars.iborIndex, vars.spread, Schedule(),
                         vars.iborIndex->dayCounter(), isPar);
    ...
    assetSwap4 = AssetSwap(payFixedRate, bond, fairCleanPrice, vars.iborIndex, vars.spread, Schedule(),
                           vars.iborIndex->dayCounter(), isPar);
    assetSwap4.setPricingEngine(swapEngine);
    if (std::fabs(assetSwap4.NPV()) > tolerance)
    {
    ...
    mktAssetSwap = AssetSwap(payFixedRate, bond, bondPrice, overnight, vars.spread, overnightSchedule,
                             overnight->dayCounter(), isPar);
    ...
    assetSwap4 = AssetSwap(payFixedRate, bond, fairCleanPrice, overnight, vars.spread, overnightSchedule,
                           overnight->dayCounter(), isPar);
    assetSwap4.setPricingEngine(swapEngine);
    if (std::fabs(assetSwap4.NPV()) > tolerance)
    {
...
BOOST_AUTO_TEST_CASE(testImpliedValue)
{
    ...
    Schedule fixedBondSchedule1(Date(4, January, 2005), Date(4, January, 2037), Period(Annual), bondCalendar,
                                Unadjusted, Unadjusted, DateGeneration::Backward, false);
    ext::shared_ptr<Bond> fixedBond1(new FixedRateBond(settlementDays, vars.faceAmount, fixedBondSchedule1,
                                                       std::vector<Rate>(1, 0.04), ActualActual(ActualActual::ISDA),
                                                       Following, 100.0, Date(4, January, 2005)));
    ...
    AssetSwap fixedBondAssetSwap1(payFixedRate, fixedBond1, fixedBondPrice1, vars.iborIndex, vars.spread, Schedule(),
    ...
    Schedule fixedBondSchedule2(Date(5, February, 2005), Date(5, February, 2019), Period(Annual), bondCalendar,
                                Unadjusted, Unadjusted, DateGeneration::Backward, false);
    ext::shared_ptr<Bond> fixedBond2(new FixedRateBond(settlementDays, vars.faceAmount, fixedBondSchedule2,
                                                       std::vector<Rate>(1, 0.05), Thirty360(Thirty360::BondBasis),
                                                       Following, 100.0, Date(5, February, 2005)));

    fixedBond2->setPricingEngine(bondEngine);

    Real fixedBondPrice2 = fixedBond2->cleanPrice();
    AssetSwap fixedBondAssetSwap2(payFixedRate, fixedBond2, fixedBondPrice2, vars.iborIndex, vars.spread, Schedule(),
                                  vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    Schedule floatingBondSchedule1(Date(29, September, 2003), Date(29, September, 2013), Period(Semiannual),
                                   bondCalendar, Unadjusted, Unadjusted, DateGeneration::Backward, false);

    ext::shared_ptr<Bond> floatingBond1(
        new FloatingRateBond(settlementDays, vars.faceAmount, floatingBondSchedule1, vars.iborIndex, Actual360(),
    ...
    AssetSwap floatingBondAssetSwap1(payFixedRate, floatingBond1, floatingBondPrice1, vars.iborIndex, vars.spread,
    ...
    if (error3 > tolerance2)
    {
        BOOST_FAIL("wrong zero spread asset swap price for floater:"
                   ...
                   << "\n  asset swap fair price: " << floatingBondAssetSwapPrice1 << std::scientific
                   << std::setprecision(2) << "\n  error:                 " << error3
                   << "\n  tolerance:             " << tolerance2);
    }
    ...
    Schedule floatingBondSchedule2(Date(24, September, 2004), Date(24, September, 2018), Period(Semiannual),
                                   bondCalendar, ModifiedFollowing, ModifiedFollowing, DateGeneration::Backward, false);
    ext::shared_ptr<Bond> floatingBond2(
        new FloatingRateBond(settlementDays, vars.faceAmount, floatingBondSchedule2, vars.iborIndex, Actual360(),
    ...
    AssetSwap floatingBondAssetSwap2(payFixedRate, floatingBond2, floatingBondPrice2, vars.iborIndex, vars.spread,
                                     Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    if (error5 > tolerance2)
    {
        BOOST_FAIL("wrong zero spread asset swap price for floater:"
                   ...
                   << "\n  asset swap fair price: " << floatingBondAssetSwapPrice2 << std::scientific
                   << std::setprecision(2) << "\n  error:                 " << error5
                   << "\n  tolerance:             " << tolerance2);
    }
    ...
    Schedule cmsBondSchedule1(Date(22, August, 2005), Date(22, August, 2020), Period(Annual), bondCalendar, Unadjusted,
                              Unadjusted, DateGeneration::Backward, false);
    ext::shared_ptr<Bond> cmsBond1(new CmsRateBond(
        settlementDays, vars.faceAmount, cmsBondSchedule1, vars.swapIndex, Thirty360(Thirty360::BondBasis), Following,
        fixingDays, std::vector<Real>(1, 1.0), std::vector<Spread>(1, 0.0), std::vector<Rate>(1, 0.055),
        std::vector<Rate>(1, 0.025), inArrears, 100.0, Date(22, August, 2005)));
    ...
    AssetSwap cmsBondAssetSwap1(payFixedRate, cmsBond1, cmsBondPrice1, vars.iborIndex, vars.spread, Schedule(),
                                vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    if (error6 > tolerance2)
    {
        BOOST_FAIL("wrong zero spread asset swap price for cms bond:"
                   ...
                   << "\n  asset swap fair price: " << cmsBondAssetSwapPrice1 << std::scientific << std::setprecision(2)
                   << "\n  error:                 " << error6 << "\n  tolerance:             " << tolerance2);
    }
    ...
    Schedule cmsBondSchedule2(Date(06, May, 2005), Date(06, May, 2015), Period(Annual), bondCalendar, Unadjusted,
                              Unadjusted, DateGeneration::Backward, false);
    ext::shared_ptr<Bond> cmsBond2(new CmsRateBond(
        settlementDays, vars.faceAmount, cmsBondSchedule2, vars.swapIndex, Thirty360(Thirty360::BondBasis), Following,
        fixingDays, std::vector<Real>(1, 0.84), std::vector<Spread>(1, 0.0), std::vector<Rate>(), std::vector<Rate>(),
        inArrears, 100.0, Date(06, May, 2005)));
    ...
    AssetSwap cmsBondAssetSwap2(payFixedRate, cmsBond2, cmsBondPrice2, vars.iborIndex, vars.spread, Schedule(),
                                vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    if (error7 > tolerance2)
    {
        BOOST_FAIL("wrong zero spread asset swap price for cms bond:"
                   ...
                   << "\n  asset swap fair price: " << cmsBondAssetSwapPrice2 << std::scientific << std::setprecision(2)
                   << "\n  error:                 " << error7 << "\n  tolerance:             " << tolerance2);
    }
    ...
    ext::shared_ptr<Bond> zeroCpnBond1(new ZeroCouponBond(settlementDays, bondCalendar, vars.faceAmount,
                                                          Date(20, December, 2015), Following, 100.0,
                                                          Date(19, December, 1985)));

    zeroCpnBond1->setPricingEngine(bondEngine);

    Real zeroCpnBondPrice1 = zeroCpnBond1->cleanPrice();
    AssetSwap zeroCpnAssetSwap1(payFixedRate, zeroCpnBond1, zeroCpnBondPrice1, vars.iborIndex, vars.spread, Schedule(),
                                vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    ext::shared_ptr<Bond> zeroCpnBond2(new ZeroCouponBond(settlementDays, bondCalendar, vars.faceAmount,
                                                          Date(17, February, 2028), Following, 100.0,
                                                          Date(17, February, 1998)));

    zeroCpnBond2->setPricingEngine(bondEngine);

    Real zeroCpnBondPrice2 = zeroCpnBond2->cleanPrice();
    AssetSwap zeroCpnAssetSwap2(payFixedRate, zeroCpnBond2, zeroCpnBondPrice2, vars.iborIndex, vars.spread, Schedule(),
                                vars.iborIndex->dayCounter(), parAssetSwap);
...
BOOST_AUTO_TEST_CASE(testMarketASWSpread)
{
    ...
    AssetSwap fixedBondParAssetSwap1(payFixedRate, fixedBond1, fixedBondMktPrice1, vars.iborIndex, vars.spread,
                                     Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    fixedBondParAssetSwap1.setPricingEngine(swapEngine);
    Real fixedBondParAssetSwapSpread1 = fixedBondParAssetSwap1.fairSpread();
    AssetSwap fixedBondMktAssetSwap1(payFixedRate, fixedBond1, fixedBondMktPrice1, vars.iborIndex, vars.spread,
                                     Schedule(), vars.iborIndex->dayCounter(), mktAssetSwap);
    ...
    Real error1 = std::fabs(fixedBondMktAssetSwapSpread1 - 100 * fixedBondParAssetSwapSpread1 / fixedBondMktFullPrice1);
    ...
    if (error1 > tolerance2)
    {
        BOOST_FAIL("wrong asset swap spreads for fixed bond:"
                   ...
                   << io::rate(fixedBondParAssetSwapSpread1) << std::scientific << std::setprecision(2)
                   << "\n  error:             " << error1 << "\n  tolerance:         " << tolerance2);
    }
    ...
    AssetSwap fixedBondParAssetSwap2(payFixedRate, fixedBond2, fixedBondMktPrice2, vars.iborIndex, vars.spread,
                                     Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    fixedBondParAssetSwap2.setPricingEngine(swapEngine);
    Real fixedBondParAssetSwapSpread2 = fixedBondParAssetSwap2.fairSpread();
    AssetSwap fixedBondMktAssetSwap2(payFixedRate, fixedBond2, fixedBondMktPrice2, vars.iborIndex, vars.spread,
                                     Schedule(), vars.iborIndex->dayCounter(), mktAssetSwap);
    fixedBondMktAssetSwap2.setPricingEngine(swapEngine);
    Real fixedBondMktAssetSwapSpread2 = fixedBondMktAssetSwap2.fairSpread();
    Real error2 = std::fabs(fixedBondMktAssetSwapSpread2 - 100 * fixedBondParAssetSwapSpread2 / fixedBondMktFullPrice2);
    ...
    if (error2 > tolerance2)
    {
        BOOST_FAIL("wrong asset swap spreads for fixed bond:"
                   ...
                   << io::rate(fixedBondParAssetSwapSpread2) << std::scientific << std::setprecision(2)
                   << "\n  error:             " << error2 << "\n  tolerance:         " << tolerance2);
    }
    ...
    AssetSwap floatingBondParAssetSwap1(payFixedRate, floatingBond1, floatingBondMktPrice1, vars.iborIndex, vars.spread,
                                        Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap floatingBondMktAssetSwap1(payFixedRate, floatingBond1, floatingBondMktPrice1, vars.iborIndex, vars.spread,
                                        Schedule(), vars.iborIndex->dayCounter(), mktAssetSwap);
    ...
    Real error3 =
        std::fabs(floatingBondMktAssetSwapSpread1 - 100 * floatingBondParAssetSwapSpread1 / floatingBondMktFullPrice1);
    ...
    if (error3 > tolerance2)
    {
        BOOST_FAIL("wrong asset swap spreads for floating bond:"
                   ...
                   << "\n  par ASW spread:    " << io::rate(floatingBondParAssetSwapSpread1) << std::scientific
                   << std::setprecision(2) << "\n  error:             " << error3
                   << "\n  tolerance:         " << tolerance2);
    }
    ...
    AssetSwap floatingBondParAssetSwap2(payFixedRate, floatingBond2, floatingBondMktPrice2, vars.iborIndex, vars.spread,
                                        Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap floatingBondMktAssetSwap2(payFixedRate, floatingBond2, floatingBondMktPrice2, vars.iborIndex, vars.spread,
                                        Schedule(), vars.iborIndex->dayCounter(), mktAssetSwap);
    ...
    Real error4 =
        std::fabs(floatingBondMktAssetSwapSpread2 - 100 * floatingBondParAssetSwapSpread2 / floatingBondMktFullPrice2);
    ...
    if (error4 > tolerance2)
    {
        BOOST_FAIL("wrong asset swap spreads for floating bond:"
                   ...
                   << "\n  par ASW spread:    " << io::rate(floatingBondParAssetSwapSpread2) << std::scientific
                   << std::setprecision(2) << "\n  error:             " << error4
                   << "\n  tolerance:         " << tolerance2);
    }
    ...
    AssetSwap cmsBondParAssetSwap1(payFixedRate, cmsBond1, cmsBondMktPrice1, vars.iborIndex, vars.spread, Schedule(),
                                   vars.iborIndex->dayCounter(), parAssetSwap);
    cmsBondParAssetSwap1.setPricingEngine(swapEngine);
    Real cmsBondParAssetSwapSpread1 = cmsBondParAssetSwap1.fairSpread();
    AssetSwap cmsBondMktAssetSwap1(payFixedRate, cmsBond1, cmsBondMktPrice1, vars.iborIndex, vars.spread, Schedule(),
    ...
    Real error5 = std::fabs(cmsBondMktAssetSwapSpread1 - 100 * cmsBondParAssetSwapSpread1 / cmsBondMktFullPrice1);
    ...
    if (error5 > tolerance2)
    {
        BOOST_FAIL("wrong asset swap spreads for cms bond:"
                   ...
                   << io::rate(cmsBondParAssetSwapSpread1) << std::scientific << std::setprecision(2)
                   << "\n  error:             " << error5 << "\n  tolerance:         " << tolerance2);
    }
    ...
    AssetSwap cmsBondParAssetSwap2(payFixedRate, cmsBond2, cmsBondMktPrice2, vars.iborIndex, vars.spread, Schedule(),
                                   vars.iborIndex->dayCounter(), parAssetSwap);
    cmsBondParAssetSwap2.setPricingEngine(swapEngine);
    Spread cmsBondParAssetSwapSpread2 = cmsBondParAssetSwap2.fairSpread();
    AssetSwap cmsBondMktAssetSwap2(payFixedRate, cmsBond2, cmsBondMktPrice2, vars.iborIndex, vars.spread, Schedule(),
                                   vars.iborIndex->dayCounter(), mktAssetSwap);
    cmsBondMktAssetSwap2.setPricingEngine(swapEngine);
    Real cmsBondMktAssetSwapSpread2 = cmsBondMktAssetSwap2.fairSpread();
    Real error6 = std::fabs(cmsBondMktAssetSwapSpread2 - 100 * cmsBondParAssetSwapSpread2 / cmsBondMktFullPrice2);
    ...
    if (error6 > tolerance2)
    {
        BOOST_FAIL("wrong asset swap spreads for cms bond:"
                   ...
                   << io::rate(cmsBondParAssetSwapSpread2) << std::scientific << std::setprecision(2)
                   << "\n  error:             " << error6 << "\n  tolerance:         " << tolerance2);
    }
    ...
    AssetSwap zeroCpnBondParAssetSwap1(payFixedRate, zeroCpnBond1, zeroCpnBondMktPrice1, vars.iborIndex, vars.spread,
                                       Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap zeroCpnBondMktAssetSwap1(payFixedRate, zeroCpnBond1, zeroCpnBondMktPrice1, vars.iborIndex, vars.spread,
                                       Schedule(), vars.iborIndex->dayCounter(), mktAssetSwap);
    ...
    Real error7 =
        std::fabs(zeroCpnBondMktAssetSwapSpread1 - 100 * zeroCpnBondParAssetSwapSpread1 / zeroCpnBondMktFullPrice1);
    ...
    if (error7 > tolerance2)
    {
        BOOST_FAIL("wrong asset swap spreads for zero cpn bond:"
                   ...
                   << io::rate(zeroCpnBondParAssetSwapSpread1) << std::scientific << std::setprecision(2)
                   << "\n  error:             " << error7 << "\n  tolerance:         " << tolerance2);
    }
    ...
    AssetSwap zeroCpnBondParAssetSwap2(payFixedRate, zeroCpnBond2, zeroCpnBondMktPrice2, vars.iborIndex, vars.spread,
                                       Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap zeroCpnBondMktAssetSwap2(payFixedRate, zeroCpnBond2, zeroCpnBondMktPrice2, vars.iborIndex, vars.spread,
                                       Schedule(), vars.iborIndex->dayCounter(), mktAssetSwap);
    ...
    Real error8 =
        std::fabs(zeroCpnBondMktAssetSwapSpread2 - 100 * zeroCpnBondParAssetSwapSpread2 / zeroCpnBondMktFullPrice2);
    ...
    if (error8 > tolerance2)
    {
        BOOST_FAIL("wrong asset swap spreads for zero cpn bond:"
                   ...
                   << io::rate(zeroCpnBondParAssetSwapSpread2) << std::scientific << std::setprecision(2)
                   << "\n  error:             " << error8 << "\n  tolerance:         " << tolerance2);
    }
}
...
BOOST_AUTO_TEST_CASE(testZSpread)
{
    ...
    Real fixedBondCleanPrice1 = BondFunctions::cleanPrice(*fixedBond1, *vars.termStructure, vars.spread,
                                                          vars.compounding, Annual, fixedBondSettlementDate1);
    Real tolerance = 1.0e-13;
    ...
    if (error1 > tolerance)
    {
        BOOST_FAIL("wrong clean price for fixed bond:"
                   ...
                   << "\n  par asset swap spread: " << fixedBondCleanPrice1 << std::scientific << std::setprecision(2)
                   << "\n  error:                 " << error1 << "\n  tolerance:             " << tolerance);
    }
    ...
    Real fixedBondCleanPrice2 = BondFunctions::cleanPrice(*fixedBond2, *vars.termStructure, vars.spread,
                                                          vars.compounding, Annual, fixedBondSettlementDate2);
    Real error3 = std::fabs(fixedBondImpliedValue2 - fixedBondCleanPrice2);
    if (error3 > tolerance)
    {
        BOOST_FAIL("wrong clean price for fixed bond:"
                   << std::fixed << std::setprecision(4) << "\n  market asset swap spread: " << fixedBondImpliedValue2
                   << "\n  par asset swap spread: " << fixedBondCleanPrice2 << std::scientific << std::setprecision(2)
                   << "\n  error:                 " << error3 << "\n  tolerance:             " << tolerance);
    }
    ...
    // standard market conventions:
    // bond's frequency + coumpounding and daycounter of the YieldCurve
    Real floatingBondCleanPrice1 = BondFunctions::cleanPrice(*floatingBond1, *vars.termStructure, vars.spread,
    ...
    if (error5 > tolerance)
    {
        BOOST_FAIL("wrong clean price for fixed bond:" << std::fixed << std::setprecision(4)
    ...
    // standard market conventions:
    // bond's frequency + coumpounding and daycounter of the YieldCurve
    Real floatingBondCleanPrice2 = BondFunctions::cleanPrice(*floatingBond2, *vars.termStructure, vars.spread,
                                                             vars.compounding, Semiannual, fixedBondSettlementDate1);
    ...
    // standard market conventions:
    // bond's frequency + coumpounding and daycounter of the YieldCurve
    Real cmsBondCleanPrice1 = BondFunctions::cleanPrice(*cmsBond1, *vars.termStructure, vars.spread, vars.compounding,
    ...
    if (error9 > tolerance)
    {
        BOOST_FAIL("wrong clean price for fixed bond:"
                   ...
                   << "\n  par asset swap spread: " << cmsBondCleanPrice1 << std::scientific << std::setprecision(2)
                   << "\n  error:                 " << error9 << "\n  tolerance:             " << tolerance);
    }
    ...
    // standard market conventions:
    // bond's frequency + coumpounding and daycounter of the YieldCurve
    Real cmsBondCleanPrice2 = BondFunctions::cleanPrice(*cmsBond2, *vars.termStructure, vars.spread, vars.compounding,
    ...
    if (error11 > tolerance)
    {
        BOOST_FAIL("wrong clean price for fixed bond:"
                   ...
                   << "\n  par asset swap spread: " << cmsBondCleanPrice2 << std::scientific << std::setprecision(2)
                   << "\n  error:                 " << error11 << "\n  tolerance:             " << tolerance);
    }
    ...
    // standard market conventions:
    // bond's frequency + coumpounding and daycounter of the YieldCurve
    Real zeroCpnBondCleanPrice1 = BondFunctions::cleanPrice(*zeroCpnBond1, *vars.termStructure, vars.spread,
                                                            vars.compounding, Annual, zeroCpnBondSettlementDate1);
    ...
    if (error13 > tolerance)
    {
        BOOST_FAIL("wrong clean price for zero coupon bond:"
                   ...
                   << "\n  zero cpn price: " << zeroCpnBondCleanPrice1 << std::scientific << std::setprecision(2)
                   << "\n  error:                 " << error13 << "\n  tolerance:             " << tolerance);
    }
    ...
    // standard market conventions:
    // bond's frequency + coumpounding and daycounter of the YieldCurve
    Real zeroCpnBondCleanPrice2 = BondFunctions::cleanPrice(*zeroCpnBond2, *vars.termStructure, vars.spread,
                                                            vars.compounding, Annual, zeroCpnBondSettlementDate2);
    ...
    if (error15 > tolerance)
    {
        BOOST_FAIL("wrong clean price for zero coupon bond:"
                   ...
                   << "\n  zero cpn price: " << zeroCpnBondCleanPrice2 << std::scientific << std::setprecision(2)
                   << "\n  error:                 " << error15 << "\n  tolerance:             " << tolerance);
    }
}
...
BOOST_AUTO_TEST_CASE(testGenericBondImplied)
{
    ...
    Schedule fixedBondSchedule1(fixedBondStartDate1, fixedBondMaturityDate1, Period(Annual), bondCalendar, Unadjusted,
                                Unadjusted, DateGeneration::Backward, false);
    ...
    fixedBondLeg1.push_back(ext::shared_ptr<CashFlow>(new SimpleCashFlow(100.0, fixedbondRedemption1)));
    ext::shared_ptr<Bond> fixedBond1(new Bond(settlementDays, bondCalendar, vars.faceAmount, fixedBondMaturityDate1,
    ...
    Schedule fixedBondSchedule2(fixedBondStartDate2, fixedBondMaturityDate2, Period(Annual), bondCalendar, Unadjusted,
                                Unadjusted, DateGeneration::Backward, false);
    ...
    fixedBondLeg2.push_back(ext::shared_ptr<CashFlow>(new SimpleCashFlow(100.0, fixedbondRedemption2)));
    ext::shared_ptr<Bond> fixedBond2(new Bond(settlementDays, bondCalendar, vars.faceAmount, fixedBondMaturityDate2,
                                              fixedBondStartDate2, fixedBondLeg2));
    fixedBond2->setPricingEngine(bondEngine);
    ...
    Schedule floatingBondSchedule1(floatingBondStartDate1, floatingBondMaturityDate1, Period(Semiannual), bondCalendar,
                                   Unadjusted, Unadjusted, DateGeneration::Backward, false);
    ...
    floatingBondLeg1.push_back(ext::shared_ptr<CashFlow>(new SimpleCashFlow(100.0, floatingbondRedemption1)));
    ...
    Schedule floatingBondSchedule2(floatingBondStartDate2, floatingBondMaturityDate2, Period(Semiannual), bondCalendar,
    ...
    Leg floatingBondLeg2 = IborLeg(floatingBondSchedule2, vars.iborIndex)
                               ...
                               .withSpreads(0.0025)
                               .inArrears(inArrears);
    Date floatingbondRedemption2 = bondCalendar.adjust(floatingBondMaturityDate2, ModifiedFollowing);
    floatingBondLeg2.push_back(ext::shared_ptr<CashFlow>(new SimpleCashFlow(100.0, floatingbondRedemption2)));
    ...
    Schedule cmsBondSchedule1(cmsBondStartDate1, cmsBondMaturityDate1, Period(Annual), bondCalendar, Unadjusted,
                              Unadjusted, DateGeneration::Backward, false);
    ...
    ext::shared_ptr<Bond> cmsBond1(
        new Bond(settlementDays, bondCalendar, vars.faceAmount, cmsBondMaturityDate1, cmsBondStartDate1, cmsBondLeg1));
    cmsBond1->setPricingEngine(bondEngine);
    ...
    Schedule cmsBondSchedule2(cmsBondStartDate2, cmsBondMaturityDate2, Period(Annual), bondCalendar, Unadjusted,
                              Unadjusted, DateGeneration::Backward, false);
    ...
    ext::shared_ptr<Bond> cmsBond2(
        new Bond(settlementDays, bondCalendar, vars.faceAmount, cmsBondMaturityDate2, cmsBondStartDate2, cmsBondLeg2));
    cmsBond2->setPricingEngine(bondEngine);
    ...
    Leg zeroCpnBondLeg1 = Leg(1, ext::shared_ptr<CashFlow>(new SimpleCashFlow(100.0, zeroCpnBondRedemption1)));
    ext::shared_ptr<Bond> zeroCpnBond1(new Bond(settlementDays, bondCalendar, vars.faceAmount, zeroCpnBondMaturityDate1,
    ...
    Leg zeroCpnBondLeg2 = Leg(1, ext::shared_ptr<CashFlow>(new SimpleCashFlow(100.0, zerocpbondRedemption2)));
    ext::shared_ptr<Bond> zeroCpnBond2(new Bond(settlementDays, bondCalendar, vars.faceAmount, zeroCpnBondMaturityDate2,
                                                zeroCpnBondStartDate2, zeroCpnBondLeg2));
    zeroCpnBond2->setPricingEngine(bondEngine);
...
BOOST_AUTO_TEST_CASE(testMASWWithGenericBond)
{
    ...
    if (error1 > tolerance2)
        BOOST_FAIL("wrong asset swap spreads for fixed bond:"
                   ...
                   << "\n  par asset swap spread:    " << io::rate(fixedBondParAssetSwapSpread1) << std::scientific
                   << std::setprecision(2) << "\n  error:                    " << error1
                   << "\n  tolerance:                " << tolerance2);
    ...
    if (error2 > tolerance2)
        BOOST_FAIL("wrong asset swap spreads for fixed bond:"
                   ...
                   << "\n  par asset swap spread:    " << io::rate(fixedBondParAssetSwapSpread2) << std::scientific
                   << std::setprecision(2) << "\n  error:                    " << error2
                   << "\n  tolerance:                " << tolerance2);
    ...
    if (error3 > tolerance2)
        BOOST_FAIL("wrong asset swap spreads for floating bond:"
                   ...
                   << "\n  par asset swap spread:    " << io::rate(floatingBondParAssetSwapSpread1) << std::scientific
                   << std::setprecision(2) << "\n  error:                    " << error3
                   << "\n  tolerance:                " << tolerance2);
    ...
    if (error4 > tolerance2)
        BOOST_FAIL("wrong asset swap spreads for floating bond:"
                   ...
                   << "\n  par asset swap spread:    " << io::rate(floatingBondParAssetSwapSpread2) << std::scientific
                   << std::setprecision(2) << "\n  error:                    " << error4
                   << "\n  tolerance:                " << tolerance2);
    ...
    if (error5 > tolerance2)
        BOOST_FAIL("wrong asset swap spreads for cms bond:"
                   ...
                   << "\n  par asset swap spread:    " << io::rate(cmsBondParAssetSwapSpread1) << std::scientific
                   << std::setprecision(2) << "\n  error:                    " << error5
                   << "\n  tolerance:                " << tolerance2);
    ...
    if (error6 > tolerance2)
        BOOST_FAIL("wrong asset swap spreads for cms bond:"
                   ...
                   << "\n  par asset swap spread:    " << io::rate(cmsBondParAssetSwapSpread2) << std::scientific
                   << std::setprecision(2) << "\n  error:                    " << error6
                   << "\n  tolerance:                " << tolerance2);
    ...
    if (error7 > tolerance2)
        BOOST_FAIL("wrong asset swap spreads for zero cpn bond:"
                   ...
                   << "\n  par asset swap spread:    " << io::rate(zeroCpnBondParAssetSwapSpread1) << std::scientific
                   << std::setprecision(2) << "\n  error:                    " << error7
                   << "\n  tolerance:                " << tolerance2);
    ...
    if (error8 > tolerance2)
        BOOST_FAIL("wrong asset swap spreads for zero cpn bond:"
                   ...
                   << "\n  par asset swap spread:    " << io::rate(zeroCpnBondParAssetSwapSpread2) << std::scientific
                   << std::setprecision(2) << "\n  error:                    " << error8
                   << "\n  tolerance:                " << tolerance2);
}
...
BOOST_AUTO_TEST_CASE(testSpecializedBondVsGenericBond)
{
    ...
    ext::shared_ptr<Bond> fixedSpecializedBond1(
        new FixedRateBond(settlementDays, vars.faceAmount, fixedBondSchedule1, std::vector<Rate>(1, 0.04),
    ...
    Real fixedSpecializedTheoDirty1 = fixedSpecializedBondTheoValue1 + fixedSpecializedBond1->accruedAmount();
    ...
    // equivalent specialized fixed rate bond
    ext::shared_ptr<Bond> fixedSpecializedBond2(
        new FixedRateBond(settlementDays, vars.faceAmount, fixedBondSchedule2, std::vector<Rate>(1, 0.05),
    ...
    Real fixedSpecializedBondTheoDirty2 = fixedSpecializedBondTheoValue2 + fixedSpecializedBond2->accruedAmount();
    ...
    ext::shared_ptr<Bond> floatingSpecializedBond1(
        new FloatingRateBond(settlementDays, vars.faceAmount, floatingBondSchedule1, vars.iborIndex, Actual360(),
                             Following, fixingDays, std::vector<Real>(1, 1), std::vector<Spread>(1, 0.0056),
                             std::vector<Rate>(), std::vector<Rate>(), inArrears, 100.0, Date(29, September, 2003)));
    ...
    // equivalent specialized floater
    ext::shared_ptr<Bond> floatingSpecializedBond2(
        new FloatingRateBond(settlementDays, vars.faceAmount, floatingBondSchedule2, vars.iborIndex, Actual360(),
                             ModifiedFollowing, fixingDays, std::vector<Real>(1, 1), std::vector<Spread>(1, 0.0025),
                             std::vector<Rate>(), std::vector<Rate>(), inArrears, 100.0, Date(24, September, 2004)));
    ...
    Real floatingSpecializedTheoDirty2 = floatingSpecializedBondTheoValue2 + floatingSpecializedBond2->accruedAmount();
    ...
    if (error12 > tolerance)
    {
        BOOST_FAIL("wrong dirty price for cms bond:"
                   ...
                   << "\n  specialized cms bond's theo dirty price: " << cmsSpecializedBondTheoDirty2 << std::scientific
                   << std::setprecision(2) << "\n  error:                 " << error12
                   << "\n  tolerance:             " << tolerance);
    }
    ...
    ext::shared_ptr<Bond> zeroCpnSpecializedBond1(new ZeroCouponBond(settlementDays, bondCalendar, vars.faceAmount,
                                                                     Date(20, December, 2015), Following, 100.0,
                                                                     Date(19, December, 1985)));
    ...
    if (error13 > tolerance)
    {
        BOOST_FAIL("wrong clean price for zero coupon bond:"
                   ...
                   << "\n  specialized zero bond's clean price: " << zeroCpnSpecializedBondTheoValue1 << std::scientific
                   << std::setprecision(2) << "\n  error:                 " << error13
                   << "\n  tolerance:             " << tolerance);
    }
    Real zeroCpnBondTheoDirty1 = zeroCpnBondTheoValue1 + zeroCpnBond1->accruedAmount();
    Real zeroCpnSpecializedBondTheoDirty1 = zeroCpnSpecializedBondTheoValue1 + zeroCpnSpecializedBond1->accruedAmount();
    ...
    // specialized zerocpn bond
    ext::shared_ptr<Bond> zeroCpnSpecializedBond2(new ZeroCouponBond(settlementDays, bondCalendar, vars.faceAmount,
                                                                     Date(17, February, 2028), Following, 100.0,
                                                                     Date(17, February, 1998)));
    ...
    Real zeroCpnSpecializedBondTheoDirty2 = zeroCpnSpecializedBondTheoValue2 + zeroCpnSpecializedBond2->accruedAmount();
...
BOOST_AUTO_TEST_CASE(testSpecializedBondVsGenericBondUsingAsw)
{
    ...
    AssetSwap fixedBondAssetSwap1(payFixedRate, fixedBond1, fixedBondPrice1, vars.iborIndex, vars.nonnullspread,
                                  Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    fixedBondAssetSwap1.setPricingEngine(swapEngine);
    AssetSwap fixedSpecializedBondAssetSwap1(payFixedRate, fixedSpecializedBond1, fixedSpecializedBondPrice1,
    ...
    AssetSwap fixedBondASW1(payFixedRate, fixedBond1, fixedBondMktPrice1, vars.iborIndex, vars.spread, Schedule(),
                            vars.iborIndex->dayCounter(), parAssetSwap);
    fixedBondASW1.setPricingEngine(swapEngine);
    AssetSwap fixedSpecializedBondASW1(payFixedRate, fixedSpecializedBond1, fixedBondMktPrice1, vars.iborIndex,
    ...
    AssetSwap fixedBondAssetSwap2(payFixedRate, fixedBond2, fixedBondPrice2, vars.iborIndex, vars.nonnullspread,
                                  Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    fixedBondAssetSwap2.setPricingEngine(swapEngine);
    AssetSwap fixedSpecializedBondAssetSwap2(payFixedRate, fixedSpecializedBond2, fixedSpecializedBondPrice2,
                                             vars.iborIndex, vars.nonnullspread, Schedule(),
                                             vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap fixedBondASW2(payFixedRate, fixedBond2, fixedBondMktPrice2, vars.iborIndex, vars.spread, Schedule(),
                            vars.iborIndex->dayCounter(), parAssetSwap);
    fixedBondASW2.setPricingEngine(swapEngine);
    AssetSwap fixedSpecializedBondASW2(payFixedRate, fixedSpecializedBond2, fixedBondMktPrice2, vars.iborIndex,
                                       vars.spread, Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap floatingBondAssetSwap1(payFixedRate, floatingBond1, floatingBondPrice1, vars.iborIndex,
                                     vars.nonnullspread, Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    floatingBondAssetSwap1.setPricingEngine(swapEngine);
    AssetSwap floatingSpecializedBondAssetSwap1(payFixedRate, floatingSpecializedBond1, floatingSpecializedBondPrice1,
                                                vars.iborIndex, vars.nonnullspread, Schedule(),
                                                vars.iborIndex->dayCounter(), parAssetSwap);
    floatingSpecializedBondAssetSwap1.setPricingEngine(swapEngine);
    Real floatingBondAssetSwapPrice1 = floatingBondAssetSwap1.fairCleanPrice();
    Real floatingSpecializedBondAssetSwapPrice1 = floatingSpecializedBondAssetSwap1.fairCleanPrice();
    ...
    AssetSwap floatingBondASW1(payFixedRate, floatingBond1, floatingBondMktPrice1, vars.iborIndex, vars.spread,
                               Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    floatingBondASW1.setPricingEngine(swapEngine);
    AssetSwap floatingSpecializedBondASW1(payFixedRate, floatingSpecializedBond1, floatingBondMktPrice1, vars.iborIndex,
                                          vars.spread, Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap floatingBondAssetSwap2(payFixedRate, floatingBond2, floatingBondPrice2, vars.iborIndex,
                                     vars.nonnullspread, Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    floatingBondAssetSwap2.setPricingEngine(swapEngine);
    AssetSwap floatingSpecializedBondAssetSwap2(payFixedRate, floatingSpecializedBond2, floatingSpecializedBondPrice2,
                                                vars.iborIndex, vars.nonnullspread, Schedule(),
                                                vars.iborIndex->dayCounter(), parAssetSwap);
    floatingSpecializedBondAssetSwap2.setPricingEngine(swapEngine);
    Real floatingBondAssetSwapPrice2 = floatingBondAssetSwap2.fairCleanPrice();
    Real floatingSpecializedBondAssetSwapPrice2 = floatingSpecializedBondAssetSwap2.fairCleanPrice();
    ...
    AssetSwap floatingBondASW2(payFixedRate, floatingBond2, floatingBondMktPrice2, vars.iborIndex, vars.spread,
                               Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    floatingBondASW2.setPricingEngine(swapEngine);
    AssetSwap floatingSpecializedBondASW2(payFixedRate, floatingSpecializedBond2, floatingBondMktPrice2, vars.iborIndex,
                                          vars.spread, Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap cmsBondAssetSwap1(payFixedRate, cmsBond1, cmsBondPrice1, vars.iborIndex, vars.nonnullspread, Schedule(),
                                vars.iborIndex->dayCounter(), parAssetSwap);
    cmsBondAssetSwap1.setPricingEngine(swapEngine);
    AssetSwap cmsSpecializedBondAssetSwap1(payFixedRate, cmsSpecializedBond1, cmsSpecializedBondPrice1, vars.iborIndex,
                                           vars.nonnullspread, Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap cmsBondASW1(payFixedRate, cmsBond1, cmsBondMktPrice1, vars.iborIndex, vars.spread, Schedule(),
                          vars.iborIndex->dayCounter(), parAssetSwap);
    cmsBondASW1.setPricingEngine(swapEngine);
    AssetSwap cmsSpecializedBondASW1(payFixedRate, cmsSpecializedBond1, cmsBondMktPrice1, vars.iborIndex, vars.spread,
                                     Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    if (error10 > tolerance)
    {
        BOOST_FAIL("wrong asw spread for cm bond:" << std::fixed << std::setprecision(4)
                                                   << "\n  generic cms rate bond's  asw spread: " << cmsBondASWSpread1
                                                   << "\n  equivalent specialized bond's asw spread: "
                                                   << cmsSpecializedBondASWSpread1 << std::scientific
                                                   << std::setprecision(2) << "\n  error:                 " << error10
                                                   << "\n  tolerance:             " << tolerance);
    }
    ...
    AssetSwap cmsBondAssetSwap2(payFixedRate, cmsBond2, cmsBondPrice2, vars.iborIndex, vars.nonnullspread, Schedule(),
                                vars.iborIndex->dayCounter(), parAssetSwap);
    cmsBondAssetSwap2.setPricingEngine(swapEngine);
    AssetSwap cmsSpecializedBondAssetSwap2(payFixedRate, cmsSpecializedBond2, cmsSpecializedBondPrice2, vars.iborIndex,
                                           vars.nonnullspread, Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    if (error11 > tolerance)
    {
        BOOST_FAIL("wrong clean price for cmsbond:" << std::fixed << std::setprecision(4)
                                                    << "\n  generic  bond's clean price: " << cmsBondAssetSwapPrice2
                                                    << "\n  equivalent specialized cms rate bond's price: "
                                                    << cmsSpecializedBondAssetSwapPrice2 << std::scientific
                                                    << std::setprecision(2) << "\n  error:                 " << error11
                                                    << "\n  tolerance:             " << tolerance);
    }
    Real cmsBondMktPrice2 = 94.35; // market executable price as of 4th sept 2007
    AssetSwap cmsBondASW2(payFixedRate, cmsBond2, cmsBondMktPrice2, vars.iborIndex, vars.spread, Schedule(),
                          vars.iborIndex->dayCounter(), parAssetSwap);
    cmsBondASW2.setPricingEngine(swapEngine);
    AssetSwap cmsSpecializedBondASW2(payFixedRate, cmsSpecializedBond2, cmsBondMktPrice2, vars.iborIndex, vars.spread,
                                     Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    if (error12 > tolerance)
    {
        BOOST_FAIL("wrong asw spread for cm bond:" << std::fixed << std::setprecision(4)
                                                   ...
                                                   << "\n  equivalent specialized bond's asw spread: "
                                                   << cmsSpecializedBondASWSpread2 << std::scientific
                                                   << std::setprecision(2) << "\n  error:                 " << error12
                                                   << "\n  tolerance:             " << tolerance);
    }
    ...
    AssetSwap zeroCpnBondAssetSwap1(payFixedRate, zeroCpnBond1, zeroCpnBondPrice1, vars.iborIndex, vars.nonnullspread,
                                    Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    zeroCpnBondAssetSwap1.setPricingEngine(swapEngine);
    AssetSwap zeroCpnSpecializedBondAssetSwap1(payFixedRate, zeroCpnSpecializedBond1, zeroCpnSpecializedBondPrice1,
                                               vars.iborIndex, vars.nonnullspread, Schedule(),
                                               vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap zeroCpnBondASW1(payFixedRate, zeroCpnBond1, zeroCpnBondMktPrice1, vars.iborIndex, vars.spread, Schedule(),
                              vars.iborIndex->dayCounter(), parAssetSwap);
    zeroCpnBondASW1.setPricingEngine(swapEngine);
    AssetSwap zeroCpnSpecializedBondASW1(payFixedRate, zeroCpnSpecializedBond1, zeroCpnBondMktPrice1, vars.iborIndex,
                                         vars.spread, Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap zeroCpnBondAssetSwap2(payFixedRate, zeroCpnBond2, zeroCpnBondPrice2, vars.iborIndex, vars.nonnullspread,
                                    Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
    zeroCpnBondAssetSwap2.setPricingEngine(swapEngine);
    AssetSwap zeroCpnSpecializedBondAssetSwap2(payFixedRate, zeroCpnSpecializedBond2, zeroCpnSpecializedBondPrice2,
                                               vars.iborIndex, vars.nonnullspread, Schedule(),
                                               vars.iborIndex->dayCounter(), parAssetSwap);
    ...
    AssetSwap zeroCpnBondASW2(payFixedRate, zeroCpnBond2, zeroCpnBondMktPrice2, vars.iborIndex, vars.spread, Schedule(),
                              vars.iborIndex->dayCounter(), parAssetSwap);
    zeroCpnBondASW2.setPricingEngine(swapEngine);
    AssetSwap zeroCpnSpecializedBondASW2(payFixedRate, zeroCpnSpecializedBond2, zeroCpnBondMktPrice2, vars.iborIndex,
                                         vars.spread, Schedule(), vars.iborIndex->dayCounter(), parAssetSwap);
