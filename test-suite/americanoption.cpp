...
#define REPORT_FAILURE(greekName, payoff, exercise, s, q, r, today, v, expected, calculated, error, tolerance)      \
    BOOST_ERROR(exerciseTypeToString(exercise)                                                                      \
                ...
                << std::fixed << std::setprecision(4) << "    expected   " << greekName << ": " << expected << "\n" \
                << "    calculated " << greekName << ": " << calculated << "\n"                                     \
                << std::scientific << "    error:            " << error << "\n"                                     \
                << "    tolerance:        " << tolerance);
...
BOOST_AUTO_TEST_CASE(testBaroneAdesiWhaleyValues)
{
    ...
    */
    AmericanOptionData values[] = {//        type, strike,   spot,    q,    r,    t,  vol,   value
...
BOOST_AUTO_TEST_CASE(testBjerksundStenslandValues)
{
    ...
    AmericanOptionData values[] = {//      type, strike,   spot,    q,    r,    t,  vol,   value, tol
    ...
    for (auto& value : values)
    {
        ...
        ext::shared_ptr<PricingEngine> engine(new BjerksundStenslandApproximationEngine(stochProcess));
...
BOOST_AUTO_TEST_CASE(testJuValues)
{
    ...
    for (auto& juValue : juValues)
    {
        ...
        ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(juValue.type, juValue.strike));
        ...
        if (error > tolerance)
        {
            REPORT_FAILURE("value", payoff, exercise, juValue.s, juValue.q, juValue.r, today, juValue.v, juValue.result,
                           calculated, error, tolerance);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testFdValues)
{
    ...
    ext::shared_ptr<PricingEngine> pdeEngine = ext::make_shared<FdBlackScholesVanillaEngine>(stochProcess, 100, 400);

    ext::shared_ptr<PricingEngine> qrPlusEngine = ext::make_shared<FdBlackScholesVanillaEngine>(stochProcess);
    ...
    for (auto& juValue : juValues)
    {
        ...
        if (std::abs(pdeCalculated - qrPlusCalculated) > 2e-2)
            BOOST_FAIL("QR+ boundary approximation failed to "
                       ...
                       << "\n    OptionType: " << ((juValue.type == Option::Call) ? "Call" : "Put")
                       << std::setprecision(16) << "\n    spot:       " << spot->value()
                                ...
                                if (value > spot->value() * 1.0e-5)
                                {
                                    ...
                                    */
                                    ...
                                    for (it = calculated.begin(); it != calculated.end(); ++it)
                                    {
                                        ...
                                        if (error > tol)
                                        {
                                            REPORT_FAILURE(greek, payoff, exercise, u, q, r, today, v, expct, calcl,
                                                           error, tol);
                                        }
                                    }
                                }
...
BOOST_AUTO_TEST_CASE(testFDShoutNPV)
{
    ...
    const auto process = ext::make_shared<BlackScholesMertonProcess>(spot, q, r, volTS);
    ...
    const TestDescription testDescriptions[] = {
        {105, Option::Put, 19.136}, {105, Option::Call, 28.211}, {120, Option::Put, 28.02}, {80, Option::Call, 40.785}};

    const auto engine = ext::make_shared<FdBlackScholesShoutEngine>(process, 400, 200);
    ...
    for (const TestDescription& desc : testDescriptions)
    {
        ...
        auto option = VanillaOption(ext::make_shared<PlainVanillaPayoff>(type, strike),
        ...
        if (diff > tol)
        {
            BOOST_FAIL("failed to reproduce known shout option price for "
                       << "\n    strike:     " << strike
                       << "\n    option type:" << ((type == Option::Call) ? "Call" : "Put")
...
BOOST_AUTO_TEST_CASE(testZeroVolFDShoutNPV)
{
    ...
    VanillaOption option(ext::make_shared<PlainVanillaPayoff>(Option::Put, 100.0),
                         ext::make_shared<AmericanExercise>(today, maturityDate));

    option.setPricingEngine(ext::make_shared<FdBlackScholesVanillaEngine>(process, dividends, 50, 50));

    const Real americanNPV = option.NPV();

    VanillaOption option2(ext::make_shared<PlainVanillaPayoff>(Option::Put, 100.0),
                          ext::make_shared<AmericanExercise>(today, maturityDate));

    option2.setPricingEngine(ext::make_shared<FdBlackScholesShoutEngine>(process, dividends, 50, 50));
...
BOOST_AUTO_TEST_CASE(testLargeDividendShoutNPV)
{
    ...
    const auto process =
        ext::make_shared<BlackScholesMertonProcess>(Handle<Quote>(ext::make_shared<SimpleQuote>(s0)), q, r, vTS);
    ...
    option.setPricingEngine(ext::make_shared<FdBlackScholesShoutEngine>(process, dividends, 100, 400));

    Real calculated = option.NPV();

    VanillaOption ref_option(ext::make_shared<PlainVanillaPayoff>(Option::Call, strike),
                             ext::make_shared<AmericanExercise>(today, dividendDate));

    ref_option.setPricingEngine(ext::make_shared<FdBlackScholesShoutEngine>(process, 100, 400));

    const Real expected = ref_option.NPV() * r->discount(maturityDate) / r->discount(dividendDate);
...
BOOST_AUTO_TEST_CASE(testEscrowedVsSpotAmericanOption)
{
    ...
    const auto process = ext::make_shared<BlackScholesMertonProcess>(
        Handle<Quote>(ext::make_shared<SimpleQuote>(100)), Handle<YieldTermStructure>(flatRate(0.08, dc)),
        Handle<YieldTermStructure>(flatRate(0.04, dc)), Handle<BlackVolTermStructure>(flatVol(vol, dc)));
    ...
    option.setPricingEngine(ext::make_shared<FdBlackScholesVanillaEngine>(process, dividends, 100, 400));
...
BOOST_AUTO_TEST_CASE(testTodayIsDividendDate)
{
    ...
    const auto process = ext::make_shared<BlackScholesMertonProcess>(
        Handle<Quote>(ext::make_shared<SimpleQuote>(100)), Handle<YieldTermStructure>(flatRate(0.05, dc)),
        Handle<YieldTermStructure>(flatRate(0.07, dc)), Handle<BlackVolTermStructure>(flatVol(vol, dc)));

    const auto maturityDate = today + Period(12, Months);
    std::vector<Date> dividendDates = {today, today + Period(11, Months)};
    std::vector<Real> dividendAmounts = {5.0, 5.0};

    ext::shared_ptr<PricingEngine> spotEngine = MakeFdBlackScholesVanillaEngine(process)
                                                    .withTGrid(100)
                                                    .withXGrid(400)
                                                    .withCashDividends(dividendDates, dividendAmounts)
                                                    .withCashDividendModel(FdBlackScholesVanillaEngine::Spot);

    ext::shared_ptr<PricingEngine> escrowedEngine = MakeFdBlackScholesVanillaEngine(process)
                                                        .withTGrid(100)
                                                        .withXGrid(400)
                                                        .withCashDividends(dividendDates, dividendAmounts)
                                                        .withCashDividendModel(FdBlackScholesVanillaEngine::Escrowed);
    ...
    VanillaOption option(ext::make_shared<PlainVanillaPayoff>(Option::Put, strike),
                         ext::make_shared<AmericanExercise>(today, maturityDate));
    ...
    spotEngine = MakeFdBlackScholesVanillaEngine(process)
                     .withTGrid(100)
                     .withXGrid(400)
                     .withCashDividends(dividendDates, dividendAmounts)
                     .withCashDividendModel(FdBlackScholesVanillaEngine::Spot);
...
BOOST_AUTO_TEST_CASE(testCallPutParity)
{
    ...
    auto buildStochProcess = [&dc](const OptionSpec& testCase)
    {
        return ext::make_shared<BlackScholesMertonProcess>(
            ...
            Handle<YieldTermStructure>(flatRate(testCase.q, dc)), Handle<YieldTermStructure>(flatRate(testCase.r, dc)),
            Handle<BlackVolTermStructure>(flatVol(testCase.volatility, dc)));
    };
    const OptionSpec testCaseSpecs[] = {{100.0, 100.0, 365, 0.5, 0.15, 0.02},
    ...
    {
        const auto maturityDate = today + Period(testCaseSpec.maturityInDays, Days);
        const Time maturityTime = dc.yearFraction(today, maturityDate);
        const Size tGrid = Size(maturityTime * timeStepsPerYear);

        const auto exercise = ext::make_shared<AmericanExercise>(today, maturityDate);

        VanillaOption putOption(ext::make_shared<PlainVanillaPayoff>(Option::Put, testCaseSpec.strike), exercise);
        putOption.setPricingEngine(
            ext::make_shared<FdBlackScholesVanillaEngine>(buildStochProcess(testCaseSpec), tGrid, xGrid));
        const Real putNpv = putOption.NPV();

        OptionSpec callOptionSpec = {testCaseSpec.strike,     testCaseSpec.spot, testCaseSpec.maturityInDays,
                                     testCaseSpec.volatility, testCaseSpec.q,    testCaseSpec.r};
        VanillaOption callOption(ext::make_shared<PlainVanillaPayoff>(Option::Call, callOptionSpec.strike), exercise);
        callOption.setPricingEngine(
            ext::make_shared<FdBlackScholesVanillaEngine>(buildStochProcess(callOptionSpec), tGrid, xGrid));
...
BOOST_AUTO_TEST_CASE(testQdPlusBoundaryValues)
{
    ...
    const QdPlusAmericanEngine qrPlusEngine(ext::shared_ptr<GeneralizedBlackScholesProcess>(), 10);

    std::vector<std::pair<Real, Real>> testCaseSpecs = {{4.9, 87.76960949965387},
    ...
    for (const auto& testCaseSpec : testCaseSpecs)
    {
        const auto calculated = qrPlusEngine.putExerciseBoundaryAtTau(S, K, r, q, sigma, maturity, testCaseSpec.first);
...
BOOST_AUTO_TEST_CASE(testQdPlusBoundaryConvergence)
{
    ...
    TestCaseSpec testCases[] = {{0.10, 0.03, 120, 2000}, {0.0001, 0.03, 120, 2000}, {0.0001, 0.000002, 120, 2000},
                                {0.01, 0.75, 120, 2000}, {0.03, 0.0, 30, 2000},     {0.03, 0.0, 1e7, 2500},
                                {0.075, 0.0, 1e-8, 2000}};

    const std::vector<std::pair<QdPlusAmericanEngine::SolverType, std::string>> solverTypes{
    ...
    for (const auto& testCase : testCases)
    {
        ...
        {
            const QdPlusAmericanEngine qrPlusEngine(ext::shared_ptr<GeneralizedBlackScholesProcess>(), Null<Size>(),
                                                    solverType.first, 1e-8);

            Size nrEvaluations = 0;

            for (Real t = 0.0; t < maturity; t += 0.1)
            {
                const auto calculated = qrPlusEngine.putExerciseBoundaryAtTau(S, testCase.strike, testCase.r,
                                                                              testCase.q, sigma, maturity, t);
                nrEvaluations += calculated.first;
            }

            const Size maxEvaluations = (solverType.first == QdPlusAmericanEngine::Halley ||
                                         solverType.first == QdPlusAmericanEngine::SuperHalley) ?
                                            750 :
                                            testCase.maxEvaluations;
...
BOOST_AUTO_TEST_CASE(testQdAmericanEngines)
{
    ...
    const double pde_values[] = {
        581.46895,  113.78442,  581.44547,  1408.579,   49.19448,   1060.27367, 834.83366,  176.48305,   120.38008,
        307.11264,  602.7006,   233.80171,  204.74596,  0.30987,    0,          0,          5.36215,     0.01711,
        0,          84.51193,   0.67131,    0.06414,    152.67188,  54.75257,   90.31861,   168.50289,   18.38926,
        0,          282.4995,   0,          0.08428,    12.30929,   42.26359,   139.87748,  0.28724,     0.00421,
        0,          0.00206,    0,          658.60427,  140.51139,  23.17387,   0.35612,    0,           909.14828,
        0,          0.11549,    5.46749,    144.25428,  2576.6754,  562.16484,  0,          122.725,     383.48463,
        278.7447,   3.52566,    82.34348,   81.06139,   0,          10.42824,   4.95917,    25.28602,    31.38869,
        3.53697,    0,          0.012,      0,          0.4263,     162.16184,  0.4618,     97.714,      283.03442,
        0.38176,    70.25367,   134.94142,  2.19293,    226.4746,   76.74309,   46.03123,   15.76214,    0.01666,
        1806.26208, 0,          103.93726,  6.82956,    337.81301,  0.64236,    677.63248,  25.01763,    443.79052,
        1793.78327, 118.6293,   185.79849,  11.59313,   679.01736,  17.99005,   403.57554,  1.67418,     0,
        0.03795,    3326.09089, 71.1996,    0,          485.10353,  0,          1681.25166, 0,           43.15432,
        0.75825,    0.05895,    34.71493,   0.00015,    5.58671,    115.98793,  37.7713,    399.24494,   0.00766,
        445.42207,  152.65397,  0,          47.05874,   0.96921,    14.21875,   257.84754,  109.62533,   2553.99295,
        138.46663,  192.33614,  81.41877,   18.21403,   113.926,    27.28409,   174.77093,  42.70527,    0.90326,
        0,          967.9901,   616.0143,   253.56442,  0.00397,    2493.82098, 9.29406,    11.00023,    0,
        0,          234.12481,  0,          72.46356,   0,          9.00932,    48.67934,   29.42756,    13.4271,
        0,          0,          0,          0,          20.71417,   48.57474,   2.26452,    0,           109.0243,
        0,          21.26801,   1.21164,    0,          86.25232,   36.00437,   4.53844,    7.40503,     313.53602,
        379.76105,  165.84347,  77.19665,   9.02466,    0.10634,    214.84982,  6.13387,    133.44645,   303.25953,
        0,          134.26724,  246.89804,  0,          123.32975,  32.83429,   9.56819,    7.42582,     0,
        73.82832,   196.84831,  0.00001,    72.70391,   2173.8649,  123.00513,  153.83539,  21.63003,    209.84752,
        30.12425,   0,          197.6502,   0,          164.02863,  7.65143,    56.57631,   2392.70018,  0,
        0,          34.23457,   171.08459,  0.49387,    31.13395,   237.68801,  0.01262,    0,           0,
        0,          0,          41.56635,   0,          8.41535,    55.01775,   310.50094,  0,           14.85456,
        174.34018,  7.19772,    0.00001,    0,          91.70874,   0.00001,    17.51724,   0.00587,     0,
        532.24902,  2.05553,    36.80843,   0,          33.39288,   0.00006,    0.04439,    1.3434,      0,
        0.41816,    926.37642,  0,          247.61559,  151.98965,  0.35243,    4.33198,    23294.47744, 0.00791,
        12.51996,   53.47727,   167.95572,  0.0062,     6.8482,     0,          347.83408,  852.85742,   558.21422,
        0,          53.89293,   78.61011,   187.3978,   9.18927,    0.00553,    113.48101,  1467.30556,  74.82251,
        94.84476,   0,          101.3649,   59.27007,   0,          773.81251,  0,          542.7889,    0,
        68.96209,   96.0435,    0.00004,    0.10738,    0.00187,    324.97758,  245.68455,  30.52818,    129.84472,
        0,          46.86288,   368.41675,  139.29763,  4.4393,     16.29594,   25.7554,    64.02621,    89.41363,
        0.62751,    219.65237,  0.26039,    0,          12.02172,   101.97733,  69.37456,   45.81122,    1263.33603,
        164.31607,  15.88788,   0,          48.77797,   0.13233,    147.16808,  10.31217,   7.50634,     7.48611,
        177.95409,  225.77562,  3.56947,    0.02531,    4.88869,    8.76632,    0,          0,           0.02214,
        305.08468,  44.52185,   182.17332,  538.31458,  0,          46.97229,   0,          31.94202,    410.43038,
        0,          70.35432,   15.58346,   74.14177,   953.67663,  11.79128,   59.83061,   0,           37.86557,
        1184.22731, 2411.37823, 0,          0,          0,          0,          49.3179,    236.38654,   21.36225,
        218.048,    517.57006,  0,          0,          12.52933,   256.71967,  0.00025,    1.47981,     158.19166,
        0,          1923.70709, 4.94441,    1199.81196, 45.92353,   85.73255,   14.91338,   88.81459,    21.42459,
        3456.9466,  31.97838,   233.26863,  49.34801,   2684.07758, 0,          0,          32.24149,    0,
        111.79552,  0.00506,    8.77602,    0,          406.54213,  0.32974,    365.53998,  1.49714,     19.65603,
        37.33877,   205.06928,  0.01805,    589.23478,  9.58273,    0.02946,    286.48706,  463.34512,   528.21392,
        0,          47.71294,   21.0864,    114.81771,  80.489,     21.30905,   41.95873,   19.03598,    156.09295,
        0,          73.6509,    0,          0,          168.17576,  0,          32.71243,   36.75044,    177.64583,
        0.05618,    156.38616,  1370.4754,  24.5976,    59.83173,   0,          354.93074,  34.96889,    0.00532,
        16.95287,   1259.72993, 241.05777,  18.9778,    0.57635,    43.98093,   25.2678,    369.39896,   0.31549,
        0,          31.95512,   101.60559,  11.22079,   970.16273,  0,          0,          1.55445,     0,
        18.6067,    0,          1124.20117, 52.67762,   10.38273,   0,          10.22588,   251.27813,   0,
        431.82244,  0,          1.31252,    0,          84.72154,   100.98411,  160.95557,  129.51372,   0.00026,
        103.81663,  421.64767,  0.00031,    0,          104.48529,  162.59225,  0,          1504.0869,   88.11253,
        4.14052,    0.07195,    203.78754,  0.00002,    42.5395,    0,          17.05087,   26.89157,    64.64923,
        0,          390.87453,  124.55406,  0.01018,    94.23963};
    ...
    {
        const Option::Type optionType = (rng.next().value > 0) ? Option::Call : Option::Put;
        ...
        const OptionSpec spec = {optionType, spot, strike, maturityInDays, vol, r, q, pde_value, -1};

        testCaseSpecs.push_back(spec);
    }

    testCaseSpecs.insert(testCaseSpecs.end(), std::begin(edgeTestCases), std::end(edgeTestCases));
    ...
    const auto bsProcess = ext::make_shared<BlackScholesMertonProcess>(
        ...
        Handle<YieldTermStructure>(flatRate(today, rRate, dc)), Handle<BlackVolTermStructure>(flatVol(today, vol, dc)));
    ...
    for (const auto& testCaseSpec : testCaseSpecs)
    {
        const Date maturityDate = today + Period(testCaseSpec.maturityInDays, Days);
        ...
        VanillaOption option(ext::make_shared<PlainVanillaPayoff>(testCaseSpec.optionType, testCaseSpec.strike),
                             ext::make_shared<AmericanExercise>(today, maturityDate));
        ...
        if ((testCaseSpec.precision > 0 && std::abs(expected - calculated) > testCaseSpec.precision) ||
            (testCaseSpec.precision < 0 && expected > 0.1 && std::abs(calculated - expected) / expected > 0.005) ||
            (testCaseSpec.precision < 0 && expected <= 0.1 && std::abs(expected - calculated) > 5e-4))
        {
            BOOST_ERROR("QR+ boundary approximation failed to "
                        "reproduce cached edge and PDE values for "
                        << "\n    OptionType: " << ((testCaseSpec.optionType == Option::Call) ? "Call" : "Put")
                        << std::setprecision(16) << "\n    spot:       " << spot->value()
...
BOOST_AUTO_TEST_CASE(testQdFpIterationScheme)
{
    ...
    const ext::shared_ptr<QdFpIterationScheme> schemes[] = {ext::make_shared<QdFpLegendreScheme>(l, m, n, p),
    ...
    for (const auto& scheme : schemes)
    {
        ...
        QL_CHECK_SMALL(scheme->getFixedPointIntegrator()->operator()(nd, -10.0, 10.0) - 1.0, tol);
        QL_CHECK_SMALL(scheme->getExerciseBoundaryToPriceIntegrator()->operator()(nd, -10.0, 10.0) - 1.0, tol);
    }
}
...
BOOST_AUTO_TEST_CASE(testAndersenLakeHighPrecisionExample)
{
    ...
    const SchemeSpec testCases[] = {{24, 3, 9, 0.05, {0.1069528125898476, 0.1069524359360852}, 1e-6},
    ...
    for (const auto& testCase : testCases)
    {
        ...
        const auto bsProcess = ext::make_shared<BlackScholesMertonProcess>(
            ...
            Handle<YieldTermStructure>(flatRate(today, r, dc)), Handle<BlackVolTermStructure>(flatVol(today, vol, dc)));

        VanillaOption americanOption(payoff, ext::make_shared<AmericanExercise>(today, maturityDate));

        VanillaOption europeanOption(payoff, ext::make_shared<EuropeanExercise>(maturityDate));

        europeanOption.setPricingEngine(ext::make_shared<AnalyticEuropeanEngine>(bsProcess));

        const Real europeanNPV = europeanOption.NPV();

        const QdFpAmericanEngine::FixedPointEquation schemes[] = {QdFpAmericanEngine::FP_A, QdFpAmericanEngine::FP_B};

        for (Size i = 0; i < std::size(schemes); ++i)
        {

            americanOption.setPricingEngine(ext::make_shared<QdFpAmericanEngine>(
                bsProcess, ext::make_shared<QdFpLegendreTanhSinhScheme>(l, m, n, tol), schemes[i]));
            ...
            if (diff > tol)
            {
                BOOST_ERROR("failed to reproduce high precision literature values"
                            << "\n    FP-Scheme: " << ((schemes[i] == QdFpAmericanEngine::FP_A) ? "FP-A" : "FP-B")
                            << "\n    r        : " << r << "\n    (l,m,n)  : (" << l << "," << m << "," << n << ")"
                            << "\n    diff     : " << diff << "\n    tol      : " << tol);
            }
        }
...
BOOST_AUTO_TEST_CASE(testQdEngineStandardExample)
{
    ...
    const auto bsProcess = ext::make_shared<BlackScholesMertonProcess>(
        Handle<Quote>(ext::make_shared<SimpleQuote>(S)), Handle<YieldTermStructure>(flatRate(today, q, dc)),
        Handle<YieldTermStructure>(flatRate(today, r, dc)), Handle<BlackVolTermStructure>(flatVol(today, sigma, dc)));
    ...
    {
        americanOption.setPricingEngine(ext::make_shared<QdFpAmericanEngine>(
            bsProcess, ext::make_shared<QdFpLegendreScheme>(32, 2, 15, 48), schemes[i]));
...
class QdFpGaussLobattoScheme : public QdFpIterationScheme
{
  public:
    ...
    : m_(m), n_(n), integrator_(ext::make_shared<GaussLobattoIntegral>(100000, QL_MAX_REAL, 0.1 * eps))
    {
    }
    Size getNumberOfChebyshevInterpolationNodes() const override { return n_; }
    Size getNumberOfNaiveFixedPointSteps() const override { return m_ - 1; }
    Size getNumberOfJacobiNewtonFixedPointSteps() const override { return Size(1); }
    ext::shared_ptr<Integrator> getFixedPointIntegrator() const override { return integrator_; }
    ext::shared_ptr<Integrator> getExerciseBoundaryToPriceIntegrator() const override { return integrator_; }
...
BOOST_AUTO_TEST_CASE(testBulkQdFpAmericanEngine)
{
    ...
    const auto qdFpFastAmericanEngine =
        ext::make_shared<QdFpAmericanEngine>(bsProcess, QdFpAmericanEngine::fastScheme());
    ...
    const auto qdFpGaussLobattoAmericanEngine =
        ext::make_shared<QdFpAmericanEngine>(bsProcess, ext::make_shared<QdFpGaussLobattoScheme>(3, 7, 1e-5));

    IncrementalStatistics statsAccurate, statsLobatto;
    for (auto t : T)
    {
        const Date maturityDate = today + Period(t, Days);
        VanillaOption option(payoff, ext::make_shared<AmericanExercise>(today, maturityDate));
...
BOOST_AUTO_TEST_CASE(testQdEngineWithLobattoIntegral)
{
    ...
    VanillaOption option(ext::make_shared<PlainVanillaPayoff>(Option::Put, K),
                         ext::make_shared<AmericanExercise>(today, maturityDate));

    const QdFpAmericanEngine::FixedPointEquation schemes[] = {QdFpAmericanEngine::FP_A, QdFpAmericanEngine::FP_B};

    const auto gaussLobattoScheme = ext::make_shared<QdFpGaussLobattoScheme>(10, 30, 1e-10);

    for (auto scheme : schemes)
    {
        const auto highPrecisionEngine =
            ext::make_shared<QdFpAmericanEngine>(bsProcess, QdFpAmericanEngine::highPrecisionScheme(), scheme);
        const auto lobattoEngine = ext::make_shared<QdFpAmericanEngine>(bsProcess, gaussLobattoScheme, scheme);
        ...
        for (Real s : std::list<Real>{36, 40 - 1e-8, 40, 40 + 1e-8, 50})
        {
            ...
            if (diff > tol || std::isnan(lobattoNPV))
            {
                BOOST_ERROR("failed to reproduce high precision American "
                            ...
                            << "\n    FP-Scheme: " << ((scheme == QdFpAmericanEngine::FP_A) ? "FP-A" : "FP-B")
                            << "\n    spot     : " << s << "\n    diff     : " << diff << "\n    tol      : " << tol);
            }
        }
    }
...
BOOST_AUTO_TEST_CASE(testQdNegativeDividendYield)
{
    ...
    const auto process = ext::make_shared<BlackScholesMertonProcess>(
        Handle<Quote>(spot), Handle<YieldTermStructure>(flatRate(qRate, dc)),
        Handle<YieldTermStructure>(flatRate(rRate, dc)), Handle<BlackVolTermStructure>(flatVol(0.4, dc)));

    VanillaOption option(ext::make_shared<PlainVanillaPayoff>(Option::Put, K),
                         ext::make_shared<AmericanExercise>(today, maturityDate));
    const auto qdPlusEngine = ext::make_shared<QdPlusAmericanEngine>(process);
    const auto qdFpEngine = ext::make_shared<QdFpAmericanEngine>(process);
    const auto fdmEngine = ext::make_shared<FdBlackScholesVanillaEngine>(process, 800, 200);
...
BOOST_AUTO_TEST_CASE(testBjerksundStenslandEuropeanGreeks)
{
    ...
    const auto bsProcess = ext::make_shared<BlackScholesMertonProcess>(
        Handle<Quote>(spot), Handle<YieldTermStructure>(flatRate(qTS, Actual365Fixed())),
        Handle<YieldTermStructure>(flatRate(rTS, Actual360())),
        Handle<BlackVolTermStructure>(flatVol(today, sigma, Thirty360(Thirty360::European))));
    ...
    const OptionSpec testCaseSpecs[] = {{Option::Put, -0.05, 0.02}, {Option::Call, 0.05, -0.025}};

    const auto europeanExercise = ext::make_shared<EuropeanExercise>(maturityDate);
    const auto americanExercise = ext::make_shared<AmericanExercise>(today, maturityDate);

    const auto europeanEngine = ext::make_shared<AnalyticEuropeanEngine>(bsProcess);

    const auto bjerksundStenslandEngine = ext::make_shared<BjerksundStenslandApproximationEngine>(bsProcess);
    ...
    for (const auto& testCaseSpec : testCaseSpecs)
    {
        ...
        VanillaOption americanOption(ext::make_shared<PlainVanillaPayoff>(testCaseSpec.type, K), americanExercise);
        americanOption.setPricingEngine(bjerksundStenslandEngine);

        VanillaOption europeanOption(ext::make_shared<PlainVanillaPayoff>(testCaseSpec.type, K), europeanExercise);
...
BOOST_AUTO_TEST_CASE(testBjerksundStenslandAmericanGreeks)
{
    ...
    const auto bsProcess = ext::make_shared<BlackScholesMertonProcess>(
        Handle<Quote>(spot), Handle<YieldTermStructure>(flatRate(qRate, Actual360())),
        Handle<YieldTermStructure>(flatRate(rRate, Actual365Fixed())),
        Handle<BlackVolTermStructure>(flatVol(today, vol, Thirty360(Thirty360::ISDA))));
    ...
    for (auto type : types)
    {
        const auto payoff = [type](Real strike) { return ext::make_shared<PlainVanillaPayoff>(type, strike); };
        ...
        for (auto t : T)
        {
            ...
            { return ext::make_shared<AmericanExercise>(today, maturityDate + offset); };
                ...
                for (auto q : qy)
                {
                    ...
                    for (auto v : sig)
                    {
                        ...
                        for (auto s : S)
                        {
                            ...
                            const auto exerciseType =
                                ext::any_cast<std::string>(option.additionalResults().find("exerciseType")->second);
                            ...
                            if (error > 5e-6)
                                REPORT_FAILURE("delta", stdPayoff, stdExercise, s, q, r, today, v,
                            ...
                            if (error > 1e-4 && t < 1000)
                                REPORT_FAILURE("gamma", stdPayoff, stdExercise, s, q, r, today, v,
                            ...
                            if (error > 5e-6)
                                REPORT_FAILURE("strikeSensitivity", stdPayoff, stdExercise, s, q, r, today, v,
                            ...
                            if (q != 0.0)
                            {
                                ...
                                if (error > 3e-2)
                                    REPORT_FAILURE("dividendRho", stdPayoff, stdExercise, s, q, r, today, v,
                                ...
                                if (error > 3e-2)
                                    REPORT_FAILURE("rho", stdPayoff, stdExercise, s, q, r, today, v,
                            ...
                            if (error > 5e-4)
                                REPORT_FAILURE("vega", stdPayoff, stdExercise, s, q, r, today, v, numericalResults.vega,
                                               vega, error, 5e-4);
                            ...
                            if (exerciseType == "American")
                            {
                                ...
                                if (error > thetaTol)
                                {
                                    REPORT_FAILURE("theta", stdPayoff, stdExercise, s, q, r, today, v,
...
BOOST_AUTO_TEST_CASE(testSingleBjerksundStenslandGreeks)
{
    ...
    const auto bsProcess = ext::make_shared<BlackScholesMertonProcess>(
        Handle<Quote>(spot), Handle<YieldTermStructure>(flatRate(qRate, Actual365Fixed())),
        Handle<YieldTermStructure>(flatRate(rRate, Actual365Fixed())),
        Handle<BlackVolTermStructure>(flatVol(today, vol, Actual365Fixed())));

    const Date maturityDate = today + Period(2, Years);

    const auto exercise = ext::make_shared<AmericanExercise>(today, maturityDate);
    const auto payoff = ext::make_shared<PlainVanillaPayoff>(Option::Call, 100);

    VanillaOption option(payoff, exercise);

    option.setPricingEngine(ext::make_shared<BjerksundStenslandApproximationEngine>(bsProcess));
    ...
    const auto exerciseType = ext::any_cast<std::string>(option.additionalResults().find("exerciseType")->second);
    ...
    const auto report = [=](Real value, Real expectedValue, const std::string& name)
    {
        ...
        if (error > tol)
            REPORT_FAILURE(name, payoff, exercise, s, q, r, today, v, value, expectedValue, error, tol);
    };

    report(npv, expectedNpv, "npv");
    report(delta, expectedDelta, "delta");
    report(gamma, expectedGamma, "gamma");
    report(strikeSensitivity, expectedStrikeSensitivity, "strikeSensitivity");
...
{
    BOOST_TEST_MESSAGE("Testing that the FD engine respects the earliest date for American exercise...");
    ...
    auto bsmProcess = ext::make_shared<BlackScholesMertonProcess>(spot, qTS, rTS, volTS);
    ...
    auto fdEngine = ext::make_shared<FdBlackScholesVanillaEngine>(bsmProcess, 200, 200, 0);
    ...
    BOOST_CHECK_MESSAGE(fullPrice - latePrice > 0.01, "Restricting exercise window should reduce price: "
                                                      "full="
                                                          << fullPrice << " late=" << latePrice);

    // The restricted option should be worth more than European
    // (it still has some early exercise value in the last 3 months)
    BOOST_CHECK_MESSAGE(latePrice > euroPrice + 0.01, "Restricted American should exceed European: "
                                                      "late="
                                                          << latePrice << " euro=" << euroPrice);
    ...
    BOOST_CHECK_MESSAGE(midPrice >= latePrice - 1e-8, "Wider window should give higher price: "
                                                      "6M="
                                                          << midPrice << " 3M=" << latePrice);
    BOOST_CHECK_MESSAGE(fullPrice >= midPrice - 1e-8, "Full window should give highest price: "
                                                      "full="
                                                          << fullPrice << " 6M=" << midPrice);
}
...
BOOST_AUTO_TEST_CASE(testBaroneAdesiWhaleyNegativeRates)
{
    ...
    VanillaOption option(payoff, exercise);
    option.setPricingEngine(ext::make_shared<BaroneAdesiWhaleyApproximationEngine>(stochProcess));

    // BAW should reject with a message pointing users to Bjerksund-Stensland.
    BOOST_CHECK_EXCEPTION(option.NPV(), Error, ExpectedErrorMessage("BjerksundStensland"));
    ...
    callOption.setPricingEngine(ext::make_shared<BaroneAdesiWhaleyApproximationEngine>(stochProcess));

    BOOST_CHECK_EXCEPTION(callOption.NPV(), Error, ExpectedErrorMessage("BjerksundStensland"));

    // Verify Bjerksund-Stensland actually works for the same negative-rate
    // input, so the error message's guidance is actionable.
    option.setPricingEngine(ext::make_shared<BjerksundStenslandApproximationEngine>(stochProcess));
    BOOST_CHECK_NO_THROW(option.NPV());
    BOOST_CHECK_MESSAGE(option.NPV() >= 4.0, "BS American put NPV " << option.NPV() << " below intrinsic 4.0");
}
