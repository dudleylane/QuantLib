...
struct CommonVars
{
    ...
    const std::vector<Real> expectedDrifts = {-0.0825792, -0.0787625, -0.0748546,  -0.0708555, -0.0667655,
                                              -0.0625846, -0.0583128, -0.0539504,  -0.0494972, -0.0449536,
                                              -0.0403194, -0.0355949, -0.0307801,  -0.025875,  -0.0208799,
                                              -0.0157948, -0.0106197, -0.00535471, 0};

    const std::vector<Real> expectedDiscountRatios = {1.58379, 1.55274, 1.52154, 1.49025, 1.45888, 1.42748, 1.39607,
                                                      1.36468, 1.33335, 1.3021,  1.27096, 1.23996, 1.20913, 1.17848,
                                                      1.14806, 1.11788, 1.08796, 1.05833, 1.029};

    const std::vector<Real> expectedForwardRates = {0.04, 0.041, 0.042, 0.043, 0.044, 0.045, 0.046, 0.047, 0.048, 0.049,
                                                    0.05, 0.051, 0.052, 0.053, 0.054, 0.055, 0.056, 0.057, 0.058};

    const std::vector<Real> expectedSwapAnnuity = {0.776368, 0.760772, 0.745125, 0.729442, 0.713739, 0.698034, 0.68234,
                                                   0.666673, 0.651048, 0.635479, 0.619979, 0.604563, 0.589242, 0.574031,
                                                   0.558939, 0.54398,  0.529163, 0.5145,   0.5};

    const std::vector<Real> expectedCotDrifts = {-0.0472372,  -0.0447452,  -0.042233,   -0.0397016, -0.0371516,
    ...
    const std::vector<Real> expectedCotDiscountRatios = {1.58379, 1.55274, 1.52154, 1.49025, 1.45888, 1.42748, 1.39607,
                                                         1.36468, 1.33335, 1.3021,  1.27096, 1.23996, 1.20913, 1.17848,
                                                         1.14806, 1.11788, 1.08796, 1.05833, 1.029};

    const std::vector<Real> expectedCotSwapAnnuity = {12.0934, 11.317,  10.5563, 9.81115, 9.08171, 8.36797, 7.66994,
    ...
    CommonVars()
    {
        ...
        Schedule dates(todaysDate, endDate, Period(Semiannual), calendar, Following, Following,
...
BOOST_AUTO_TEST_CASE(testLMMCurveState)
{
    ...
    LMMDriftCalculator lmmDriftcalculator(vars.pseudo, vars.displacements, vars.taus, vars.numeraire,
    ...
    for (Size i = 0; i < vars.N; ++i)
    {
        ...
        {
            BOOST_FAIL("LMM drifts mismatched: " << lmmDrifts[i] << "\t\t" << vars.expectedDrifts[i]);
        }

        if (std::fabs(lmmCs.discountRatio(i, vars.N) - vars.expectedDiscountRatios[i]) > vars.tol)
        {
            BOOST_FAIL("LMM discount ratio mismatch: " << lmmCs.discountRatio(i, vars.N) << "\t\t"
                                                       << vars.expectedDiscountRatios[i]);
        }

        if (std::fabs(lmmCs.forwardRate(i) - vars.expectedForwardRates[i]) > vars.tol)
        {
            BOOST_FAIL("LMM forward rate mismatch: " << lmmCs.forwardRate(i) << "\t\t" << vars.expectedForwardRates[i]);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testCoterminalSwapCurveState)
{
    ...
    for (Size i = 1; i <= vars.N; ++i)
    {
        ...
        else
        {
            coterminalAnnuity[vars.N - i] =
                coterminalAnnuity[vars.N - i + 1] + vars.accruals[vars.N - i] * vars.todaysDiscounts[vars.N - i + 1];
        }
    ...
    std::copy(vars.rateTimes.begin(), vars.rateTimes.end() - 1, evolutionTimes.begin());
    ...
    for (Size i = 0; i < vars.N; ++i)
    {
        ...
        {
            BOOST_FAIL("COT drifts mismatched: " << cotDrifts[i] << "\t\t" << vars.expectedCotDrifts[i]);
        }

        if (std::fabs(cotCs.discountRatio(i, vars.N) - vars.expectedCotDiscountRatios[i]) > vars.tol)
        {
        ...
        {
            BOOST_FAIL("COT forward rate mismatch: " << cotCs.forwardRate(i) << "\t\t" << vars.expectedForwardRates[i]);
        }

        if (std::fabs(cotCs.coterminalSwapRate(i) - todaysCoterminalSwapRates[i]) > vars.tol)
        {
        ...
        if (std::fabs(cotCs.coterminalSwapAnnuity(vars.numeraire, i) - vars.expectedCotSwapAnnuity[i]) > vars.tol)
        {
            BOOST_FAIL("COT swap annunity mismatch: " << cotCs.coterminalSwapAnnuity(vars.numeraire, i) << "\t\t"
...
BOOST_AUTO_TEST_CASE(testCMSwapCurveState)
{
    ...
    CMSMMDriftCalculator cmsDriftcalculator(vars.pseudo, vars.displacements, vars.taus, vars.numeraire,
                                            vars.firstAliveRates[0], vars.spanningFwds);
    ...
    for (Size i = 0; i < vars.N; ++i)
    {
        ...
        {
            BOOST_FAIL("CMS drifts mismatched: " << cmsDrifts[i] << "\t\t" << vars.expectedDrifts[i]);
        }

        if (std::fabs(cmsCs.discountRatio(i, vars.N) - vars.expectedDiscountRatios[i]) > vars.tol)
        {
        ...
        {
            BOOST_FAIL("CMS forward rate mismatch: " << cmsCs.forwardRate(i) << "\t\t" << vars.expectedForwardRates[i]);
        }

        if (std::fabs(cmsCs.cmSwapRate(i, vars.spanningFwds) - vars.expectedForwardRates[i]) > vars.tol)
        {
            // Swap rate should be the same as Forward Rates
        ...
        if (std::fabs(cmsCs.cmSwapAnnuity(vars.numeraire, i, vars.spanningFwds) - vars.expectedSwapAnnuity[i]) >
            vars.tol)
        {
            BOOST_FAIL("CMS swap annunity mismatch: " << cmsCs.cmSwapAnnuity(vars.numeraire, i, vars.spanningFwds)
