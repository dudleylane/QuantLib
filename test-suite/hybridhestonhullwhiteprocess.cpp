...
BOOST_AUTO_TEST_CASE(testBsmHullWhiteEngine)
{
    ...
    const Handle<Quote> spot(ext::shared_ptr<Quote>(new SimpleQuote(100.0)));
    ...
    ext::shared_ptr<HullWhite> hullWhiteModel(new HullWhite(Handle<YieldTermStructure>(rTS), 0.00883, 0.00526));

    ext::shared_ptr<BlackScholesMertonProcess> stochProcess(new BlackScholesMertonProcess(spot, qTS, rTS, volTS));
    ...
    ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Call, fwd));

    EuropeanOption option(payoff, exercise);

    const Real tol = 1e-8;
    const Real corr[] = {-0.75, -0.25, 0.0, 0.25, 0.75};
    const Volatility expectedVol[] = {0.217064577, 0.243995801, 0.256402830, 0.268236596, 0.290461343};
    ...
    for (Size i = 0; i < std::size(corr); ++i)
    {
        ...
        const Handle<BlackVolTermStructure> compVolTS(flatVol(today, expectedVol[i], dc));

        ext::shared_ptr<BlackScholesMertonProcess> bsProcess(new BlackScholesMertonProcess(spot, qTS, rTS, compVolTS));
        ext::shared_ptr<PricingEngine> bsEngine(new AnalyticEuropeanEngine(bsProcess));

        EuropeanOption comp(payoff, exercise);
        comp.setPricingEngine(bsEngine);

        Volatility impliedVol = comp.impliedVolatility(npv, bsProcess, 1e-10, 100);
...
BOOST_AUTO_TEST_CASE(testCompareBsmHWandHestonHW)
{
    ...
    const Handle<YieldTermStructure> rTS(ext::shared_ptr<YieldTermStructure>(new ZeroCurve(dates, rates, dc)));
    const Handle<YieldTermStructure> qTS(ext::shared_ptr<YieldTermStructure>(new ZeroCurve(dates, divRates, dc)));

    ext::shared_ptr<SimpleQuote> vol(new SimpleQuote(0.25));
    const Handle<BlackVolTermStructure> volTS(flatVol(today, vol, dc));

    ext::shared_ptr<BlackScholesMertonProcess> bsmProcess(new BlackScholesMertonProcess(spot, qTS, rTS, volTS));

    ext::shared_ptr<HestonProcess> hestonProcess(
        new HestonProcess(rTS, qTS, spot, vol->value() * vol->value(), 1.0, vol->value() * vol->value(), 1e-4, 0.0));

    ext::shared_ptr<HestonModel> hestonModel(new HestonModel(hestonProcess));

    ext::shared_ptr<HullWhite> hullWhiteModel(new HullWhite(Handle<YieldTermStructure>(rTS), 0.01, 0.01));

    ext::shared_ptr<PricingEngine> bsmhwEngine(new AnalyticBSMHullWhiteEngine(0.0, bsmProcess, hullWhiteModel));

    ext::shared_ptr<PricingEngine> hestonHwEngine(new AnalyticHestonHullWhiteEngine(hestonModel, hullWhiteModel, 128));


    const Real tol = 1e-5;
    const Real strike[] = {0.25, 0.5, 0.75, 0.8, 0.9, 1.0, 1.1, 1.2, 1.5, 2.0, 4.0};
    ...
    for (auto type : types)
    {
        for (Real j : strike)
        {
            for (unsigned long l : maturity)
            {
                ...
                Real fwd = j * spot->value() * qTS->discount(maturityDate) / rTS->discount(maturityDate);
                ...
                if (std::fabs(calculated - expected) > calculated * tol && std::fabs(calculated - expected) > tol)
                {
                    BOOST_ERROR("Failed to reproduce npvs"
                                << "\n    calculated: " << calculated << "\n    expected  : " << expected
                                << "\n    strike    : " << j << "\n    maturity  : " << l
                                << "\n    type      : " << ((type == Option::Put) ? "Put" : "Call"));
                }
            }
        }
    }
...
BOOST_AUTO_TEST_CASE(testZeroBondPricing)
{
    ...
    const Handle<YieldTermStructure> ts(ext::shared_ptr<YieldTermStructure>(new ZeroCurve(dates, rates, dc)));
    const Handle<YieldTermStructure> ds(flatRate(today, 0.0, dc));

    const ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(ts, ds, s0, 0.02, 1.0, 0.2, 0.5, -0.8));
    const ext::shared_ptr<HullWhiteForwardProcess> hwProcess(new HullWhiteForwardProcess(ts, 0.05, 0.05));
    ...
    for (Size i = 0; i < nrTrails; ++i)
    {
        ...
        for (Size j = 1; j < m; ++j)
        {
            ...
            const DiscountFactor zeroBond = 1.0 / jointProcess->numeraire(t, states);
            const DiscountFactor zeroOption = zeroBond * std::max(0.0, hwModel->discountBond(t, T, states[2]) - strike);
...
BOOST_AUTO_TEST_CASE(testMcVanillaPricing)
{
    ...
    const ext::shared_ptr<BlackScholesMertonProcess> bsmProcess(new BlackScholesMertonProcess(s0, qTS, rTS, volTS));
    const ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(rTS, qTS, s0, 0.0625, 0.5, 0.0625, 1e-5, 0.3));
    const ext::shared_ptr<HullWhiteForwardProcess> hwProcess(new HullWhiteForwardProcess(rTS, 0.01, 0.01));
    hwProcess->setForwardMeasureTime(dc.yearFraction(today, maturity));
    ...
    for (Real i : corr)
    {
        for (Real j : strike)
        {
            ...
            ext::shared_ptr<PricingEngine> engine = MakeMCHestonHullWhiteEngine<PseudoRandom>(jointProcess)
                                                        .withSteps(1)
            ...
            const ext::shared_ptr<HullWhite> hwModel(
                new HullWhite(Handle<YieldTermStructure>(rTS), hwProcess->a(), hwProcess->sigma()));

            VanillaOption optionBsmHW(payoff, exercise);
            optionBsmHW.setPricingEngine(
                ext::shared_ptr<PricingEngine>(new AnalyticBSMHullWhiteEngine(i, bsmProcess, hwModel)));
...
BOOST_AUTO_TEST_CASE(testMcPureHestonPricing)
{
    ...
    const ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(rTS, qTS, s0, 0.08, 1.5, 0.0625, 0.5, -0.8));
    const ext::shared_ptr<HullWhiteForwardProcess> hwProcess(new HullWhiteForwardProcess(rTS, 0.1, 1e-8));
    hwProcess->setForwardMeasureTime(dc.yearFraction(today, maturity + Period(1, Years)));
    ...
    for (Real i : corr)
    {
        ...
        {
            ext::shared_ptr<HybridHestonHullWhiteProcess> jointProcess(
                new HybridHestonHullWhiteProcess(hestonProcess, hwProcess, i, HybridHestonHullWhiteProcess::Euler));
            ...
            optionPureHeston.setPricingEngine(
                ext::shared_ptr<PricingEngine>(new AnalyticHestonEngine(ext::make_shared<HestonModel>(hestonProcess))));

            Real expected = optionPureHeston.NPV();

            optionHestonHW.setPricingEngine(MakeMCHestonHullWhiteEngine<PseudoRandom>(jointProcess)
                                                .withSteps(2)
                                                .withAntitheticVariate()
                                                .withControlVariate()
                                                .withAbsoluteTolerance(tol)
            ...
            if (std::fabs(calculated - expected) > 3 * error && std::fabs(calculated - expected) > tol)
            {
...
BOOST_AUTO_TEST_CASE(testAnalyticHestonHullWhitePricing)
{
    ...
    const ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(rTS, qTS, s0, 0.08, 1.5, 0.0625, 0.5, -0.8));
    const ext::shared_ptr<HestonModel> hestonModel(new HestonModel(hestonProcess));

    const ext::shared_ptr<HullWhiteForwardProcess> hwFwdProcess(new HullWhiteForwardProcess(rTS, 0.01, 0.01));
    hwFwdProcess->setForwardMeasureTime(dc.yearFraction(today, maturity));
    const ext::shared_ptr<HullWhite> hullWhiteModel(new HullWhite(rTS, hwFwdProcess->a(), hwFwdProcess->sigma()));
    ...
    for (auto type : types)
    {
        for (Real j : strike)
        {
            ext::shared_ptr<HybridHestonHullWhiteProcess> jointProcess(new HybridHestonHullWhiteProcess(
                hestonProcess, hwFwdProcess, 0.0, HybridHestonHullWhiteProcess::Euler));
            ...
            VanillaOption optionPureHeston(payoff, exercise);
            optionPureHeston.setPricingEngine(
                ext::shared_ptr<PricingEngine>(new AnalyticHestonHullWhiteEngine(hestonModel, hullWhiteModel, 128)));
...
BOOST_AUTO_TEST_CASE(testCallableEquityPricing)
{
    ...
    */
    ...
    const ext::shared_ptr<HestonProcess> hestonProcess(
        new HestonProcess(rTS, qTS, spot, 0.0625, 1.0, 0.24 * 0.24, 1e-4, 0.0));
    const ext::shared_ptr<HullWhiteForwardProcess> hwProcess(new HullWhiteForwardProcess(rTS, 0.00883, 0.00526));
    hwProcess->setForwardMeasureTime(dc.yearFraction(today, today + Period(maturity + 1, Years)));

    const ext::shared_ptr<HybridHestonHullWhiteProcess> jointProcess(
        new HybridHestonHullWhiteProcess(hestonProcess, hwProcess, -0.4));

    Schedule schedule(today, today + Period(maturity, Years), Period(1, Years), TARGET(), Following, Following,
    ...
    BigNatural seed = 42;
    rsg_type rsg = PseudoRandom::make_sequence_generator(jointProcess->factors() * (grid.size() - 1), seed);
...
BOOST_AUTO_TEST_CASE(testDiscretizationError)
{
    ...
    const ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(rTS, qTS, s0, v * v, 1, v * v, 1e-6, -0.4));
    ...
    for (Real i : corr)
    {
        for (Real j : strike)
        {
            ...
            VanillaOption optionBsmHW(payoff, exercise);
            const ext::shared_ptr<HullWhite> hwModel(new HullWhite(rTS, hwProcess->a(), hwProcess->sigma()));
            optionBsmHW.setPricingEngine(
                ext::shared_ptr<PricingEngine>(new AnalyticBSMHullWhiteEngine(i, bsmProcess, hwModel)));
            ...
            if ((std::fabs(calculated - expected) > 3 * error && std::fabs(calculated - expected) > 1e-5))
            {
...
BOOST_AUTO_TEST_CASE(testFdmHestonHullWhiteEngine, *precondition(if_speed(Fast)))
{
    ...
    ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(rTS, qTS, s0, v0, 1.0, v0, 0.000001, 0.0));

    ext::shared_ptr<BlackScholesMertonProcess> stochProcess(new BlackScholesMertonProcess(s0, qTS, rTS, volTS));

    ext::shared_ptr<HullWhiteProcess> hwProcess(new HullWhiteProcess(rTS, 0.00883, 0.01));
    ext::shared_ptr<HullWhite> hwModel(new HullWhite(rTS, hwProcess->a(), hwProcess->sigma()));
    ...
    for (Real i : corr)
    {
        for (Real j : strike)
        {
            ...
            option.setPricingEngine(ext::shared_ptr<PricingEngine>(new FdHestonHullWhiteVanillaEngine(
            ...
            option.setPricingEngine(
                ext::shared_ptr<PricingEngine>(new AnalyticBSMHullWhiteEngine(i, stochProcess, hwModel)));
            const Real expected = option.NPV();
...
HullWhiteModelData hullWhiteModels[] = {{"EUR-2003", 0.00883, 0.00631}};
...
SchemeData schemes[] = {{"HV2", FdmSchemeDesc::Hundsdorfer()},
...
ext::shared_ptr<HestonProcess> makeHestonProcess(const HestonModelData& params)
{
    ...
    return ext::make_shared<HestonProcess>(rTS, qTS, spot, params.v0, params.kappa, params.theta, params.sigma,
                                           params.rho);
}
...
ext::shared_ptr<VanillaOption> makeVanillaOption(const VanillaOptionData& params)
{
    ...
    Date maturity = Date(Settings::instance().evaluationDate()) + Period(Size(params.maturity * 365), Days);
    ext::shared_ptr<Exercise> exercise(new EuropeanExercise(maturity));
    ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(params.optionType, params.strike));
...
BOOST_AUTO_TEST_CASE(testBsmHullWhitePricing)
{
    ...
    HestonModelData hestonModelData = {"BSM-HW Model", 0.09, 1.0, 0.09, QL_EPSILON, 0.0, 0.04, 0.03};
    ...
    ext::shared_ptr<HullWhiteProcess> hwProcess(
        new HullWhiteProcess(hp->riskFreeRate(), hwModelData.a, hwModelData.sigma));
    ext::shared_ptr<HullWhite> hullWhiteModel(new HullWhite(hp->riskFreeRate(), hwProcess->a(), hwProcess->sigma()));


    ext::shared_ptr<BlackScholesMertonProcess> bsmProcess(
        new BlackScholesMertonProcess(hp->s0(), hp->dividendYield(), hp->riskFreeRate(),
                                      Handle<BlackVolTermStructure>(flatVol(today, std::sqrt(hestonModelData.theta),
    ...
    for (Size l = 0; l < std::size(schemes); ++l)
    {
        ...
        for (bool i : controlVariate)
        {
            for (unsigned long u : listOfTimeStepsPerYear)
            {
                ...
                ext::shared_ptr<FdHestonHullWhiteVanillaEngine> fdEngine(new FdHestonHullWhiteVanillaEngine(
                    hestonModel, hwProcess, equityIrCorr, tSteps, 400, 2, 10, 0, i, scheme.schemeDesc));
                ...
                for (Real& strike : strikes)
                {
                    ...
                    ext::shared_ptr<VanillaOption> option = makeVanillaOption(optionData);
                    option->setPricingEngine(bsmhwEngine);
    ...
    for (unsigned long u : listOfTimeStepsPerYear)
    {
        for (Size i = 0; i < std::size(schemes); ++i)
        {
            for (auto& j : hestonModels)
            {
                ...
                ext::shared_ptr<PricingEngine> analyticEngine(new AnalyticHestonEngine(hestonModel, 172));

                Size tSteps = Size(maturity * u);

                ext::shared_ptr<FdHestonVanillaEngine> fdEngine(
                    new FdHestonVanillaEngine(hestonModel, tSteps, 200, 40, 0, schemes[i].schemeDesc));
                fdEngine->enableMultipleStrikesCaching(strikes);
...
class HestonHullWhiteCorrelationConstraint : public Constraint
{
  private:
    ...
    {
      public:
        explicit Impl(Real equityShortRateCorr) : equityShortRateCorr_(equityShortRateCorr) {}
  ...
  public:
    explicit HestonHullWhiteCorrelationConstraint(Real equityShortRateCorr)
    : Constraint(ext::shared_ptr<Constraint::Impl>(new HestonHullWhiteCorrelationConstraint::Impl(equityShortRateCorr)))
    {
    }
};
...
BOOST_AUTO_TEST_CASE(testHestonHullWhiteCalibration, *precondition(if_speed(Slow)))
{
    ...
    ext::shared_ptr<HullWhiteProcess> hwProcess(new HullWhiteProcess(rTS, 0.00883, 0.00631));
    ext::shared_ptr<HullWhite> hullWhiteModel(new HullWhite(rTS, hwProcess->a(), hwProcess->sigma()));
    ...
    const ext::shared_ptr<HestonProcess> hestonProcess(
        new HestonProcess(rTS, qTS, s0, start_v0, start_kappa, start_theta, start_sigma, start_rho));
    const ext::shared_ptr<HestonModel> analyticHestonModel(new HestonModel(hestonProcess));
    const ext::shared_ptr<PricingEngine> analyticHestonEngine(new AnalyticHestonEngine(analyticHestonModel, 164));
    const ext::shared_ptr<HestonModel> fdmHestonModel(new HestonModel(hestonProcess));
    ...
    std::vector<Volatility> vol = {
        0.482627, 0.407617, 0.366682, 0.340110, 0.314266, 0.280241, 0.252471, 0.325552, 0.464811, 0.393336, 0.354664,
        0.329758, 0.305668, 0.273563, 0.244024, 0.244886, 0.441864, 0.375618, 0.340464, 0.318249, 0.297127, 0.268839,
        0.237972, 0.225553, 0.407506, 0.351125, 0.322571, 0.305173, 0.289034, 0.267361, 0.239315, 0.213761, 0.366761,
        0.326166, 0.306764, 0.295279, 0.284765, 0.270592, 0.250702, 0.222928, 0.345671, 0.314748, 0.300259, 0.291744,
        0.283971, 0.273475, 0.258503, 0.235683, 0.324512, 0.303631, 0.293981, 0.288338, 0.283193, 0.276248, 0.266271,
        0.250506, 0.311278, 0.296340, 0.289481, 0.285482, 0.281840, 0.276924, 0.269856, 0.258609, 0.303219, 0.291534,
    ...
    for (Size i = 0; i < maturities.size(); ++i)
    {
        ...
        ext::shared_ptr<Exercise> exercise(new EuropeanExercise(today + maturity));

        for (Size j = 0; j < strikes.size(); ++j)
        {
            ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(
                strikes[j] * rTS->discount(maturities[i]) >= s0->value() * qTS->discount(maturities[i]) ? Option::Call :
                                                                                                          Option::Put,
                strikes[j]));
            RelinkableHandle<Quote> v(ext::shared_ptr<Quote>(new SimpleQuote(vol[i * strikes.size() + j])));

            ext::shared_ptr<BlackCalibrationHelper> helper(new HestonModelHelper(
                maturity, calendar, s0, strikes[j], v, rTS, qTS, BlackCalibrationHelper::PriceError));
            ...
            ext::shared_ptr<GeneralizedBlackScholesProcess> bsProcess =
                QuantLib::detail::ImpliedVolatilityHelper::clone(
                    ext::make_shared<GeneralizedBlackScholesProcess>(
                        s0, qTS, rTS, Handle<BlackVolTermStructure>(flatVol(v->value(), dc))),
                    volQuote);

            VanillaOption dummyOption(payoff, exercise);

            ext::shared_ptr<PricingEngine> bshwEngine(
                new AnalyticBSMHullWhiteEngine(equityShortRateCorr, bsProcess, hullWhiteModel));

            Volatility vt = QuantLib::detail::ImpliedVolatilityHelper::calculate(dummyOption, *bshwEngine, *volQuote,
                                                                                 marketValue, 1e-8, 100, 0.0001, 10);

            v.linkTo(ext::shared_ptr<Quote>(new SimpleQuote(vt)));

            helper->setPricingEngine(ext::shared_ptr<PricingEngine>(analyticHestonEngine));
        }
    }

    HestonHullWhiteCorrelationConstraint corrConstraint(equityShortRateCorr);
    LevenbergMarquardt om(1e-6, 1e-8, 1e-8);
    analyticHestonModel->calibrate(options, om, EndCriteria(400, 40, 1.0e-8, 1.0e-4, 1.0e-8), corrConstraint);
    ...
    for (Size i = 0; i < maturities.size(); ++i)
    {
        ...
        ext::shared_ptr<FdHestonHullWhiteVanillaEngine> engine(new FdHestonHullWhiteVanillaEngine(
            fdmHestonModel, hwProcess, equityShortRateCorr, tGrid, 45, 11, 5, 0, true));
        ...
        for (Size j = 0; j < strikes.size(); ++j)
        {
            ...
            ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Call, strikes[js]));
            Handle<Quote> v(ext::shared_ptr<Quote>(new SimpleQuote(vol[i * strikes.size() + js])));
            ext::shared_ptr<BlackCalibrationHelper> helper(new HestonModelHelper(
                maturity, calendar, s0, strikes[js], v, rTS, qTS, BlackCalibrationHelper::PriceError));
            options.push_back(helper);
    ...
    fdmHestonModel->calibrate(options, vm, EndCriteria(400, 40, 1.0e-8, 1.0e-4, 1.0e-8), corrConstraint);
...
BOOST_AUTO_TEST_CASE(testH1HWPricingEngine)
{
    ...
    const ext::shared_ptr<Exercise> exercise(new EuropeanExercise(exerciseDate));
    ...
    const ext::shared_ptr<HullWhiteProcess> hwProcess(new HullWhiteProcess(rTS, kappa_r, sigma_r));
    const ext::shared_ptr<HullWhite> hullWhiteModel(new HullWhite(Handle<YieldTermStructure>(rTS), kappa_r, sigma_r));

    const Real tol = 0.0001;
    const Real strikes[] = {40, 80, 100, 120, 180};
    const Real expected[std::size(sigma_v)][std::size(strikes)] = {{0.267503, 0.235742, 0.228223, 0.223461, 0.217855},
                                                                   {0.263626, 0.211625, 0.199907, 0.193502, 0.190025}};

    for (Size j = 0; j < std::size(sigma_v); ++j)
    {
        const ext::shared_ptr<HestonProcess> hestonProcess(
            new HestonProcess(rTS, qTS, s0, v0, kappa_v, theta, sigma_v[j], rho_sv));
        const ext::shared_ptr<HestonModel> hestonModel(new HestonModel(hestonProcess));

        for (Size i = 0; i < std::size(strikes); ++i)
        {
            const ext::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Call, strikes[i]));

            VanillaOption option(payoff, exercise);

            const ext::shared_ptr<PricingEngine> analyticH1HWEngine(
                new AnalyticH1HWEngine(hestonModel, hullWhiteModel, rho_sr, 144));
            option.setPricingEngine(analyticH1HWEngine);
            const Real impliedH1HW = option.impliedVolatility(option.NPV(), bsProcess);
