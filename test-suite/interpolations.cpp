...
void checkValues(const char* type, const CubicInterpolation& cubic, I xBegin, I xEnd, J yBegin)
{
    ...
    while (xBegin != xEnd)
    {
        ...
        {
            BOOST_ERROR(type << " interpolation failed at x = " << *xBegin << std::scientific
...
void check1stDerivativeValue(const char* type, const CubicInterpolation& cubic, Real x, Real value)
{
...
void check2ndDerivativeValue(const char* type, const CubicInterpolation& cubic, Real x, Real value)
{
...
void checkNotAKnotCondition(const char* type, const CubicInterpolation& cubic)
{
    Real tolerance = 1.0e-14;
...
void checkSymmetry(const char* type, const CubicInterpolation& cubic, Real xMin)
{
...
{
    return std::sqrt(s * std::sinh(std::log(t)) + std::exp(std::sin(u) * std::sin(3 * v)) + std::sinh(std::log(v * w)));
}
...
{
  public:
    GF(Real exponent, Real factor) : exponent_(exponent), factor_(factor) {}

    Real operator()(Real h) const
    {
        return M_PI + factor_ * std::pow(h, exponent_) + std::pow(factor_ * h, exponent_ + 1);
    }
...
BOOST_AUTO_TEST_CASE(testSplineErrorOnGaussianValues)
{
    ...
    for (Size i = 0; i < std::size(points); i++)
    {
        ...
        // Not-a-knot
        CubicInterpolation f(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, false,
                             CubicInterpolation::NotAKnot, Null<Real>(), CubicInterpolation::NotAKnot, Null<Real>());
        f.update();
        ...
        // MC not-a-knot
        f = CubicInterpolation(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, true,
                               CubicInterpolation::NotAKnot, Null<Real>(), CubicInterpolation::NotAKnot, Null<Real>());
        f.update();
...
BOOST_AUTO_TEST_CASE(testSplineOnGaussianValues)
{
    ...
    for (Real start = -1.9, j = 0; j < 2; start += 0.2, j++)
    {
        ...
        CubicInterpolation f(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, false,
                             CubicInterpolation::NotAKnot, Null<Real>(), CubicInterpolation::NotAKnot, Null<Real>());
        f.update();
        checkValues("Not-a-knot spline", f, x.begin(), x.end(), y.begin());
        ...
        f = CubicInterpolation(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, true,
                               CubicInterpolation::NotAKnot, Null<Real>(), CubicInterpolation::NotAKnot, Null<Real>());
        f.update();
        checkValues("MC not-a-knot spline", f, x.begin(), x.end(), y.begin());
        // good performance
        interpolated = f(x1_bad);
        if (interpolated < 0.0)
        {
...
BOOST_AUTO_TEST_CASE(testSplineOnRPN15AValues)
{
    ...
    const Real RPN15A_x[] = {7.99, 8.09, 8.19, 8.7, 9.2, 10.0, 12.0, 15.0, 20.0};
    const Real RPN15A_y[] = {0.0, 2.76429e-5, 4.37498e-5, 0.169183, 0.469428, 0.943740, 0.998636, 0.999919, 0.999994};

    Real interpolated;

    // Natural spline
    CubicInterpolation f =
        CubicInterpolation(std::begin(RPN15A_x), std::end(RPN15A_x), std::begin(RPN15A_y), CubicInterpolation::Spline,
                           false, CubicInterpolation::SecondDerivative, 0.0, CubicInterpolation::SecondDerivative, 0.0);
    f.update();
    checkValues("Natural spline", f, std::begin(RPN15A_x), std::end(RPN15A_x), std::begin(RPN15A_y));
    check2ndDerivativeValue("Natural spline", f, *std::begin(RPN15A_x), 0.0);
    check2ndDerivativeValue("Natural spline", f, *(std::end(RPN15A_x) - 1), 0.0);
    // poor performance
    Real x_bad = 11.0;
    interpolated = f(x_bad);
    if (interpolated < 1.0)
    {
        BOOST_ERROR("Natural spline interpolation " << "poor performance unverified\n"
                                                    << "at x = " << x_bad << "\ninterpolated value: " << interpolated
                                                    << "\nexpected value > 1.0");
    }
    ...
    // Clamped spline
    f = CubicInterpolation(std::begin(RPN15A_x), std::end(RPN15A_x), std::begin(RPN15A_y), CubicInterpolation::Spline,
                           false, CubicInterpolation::FirstDerivative, 0.0, CubicInterpolation::FirstDerivative, 0.0);
    f.update();
    checkValues("Clamped spline", f, std::begin(RPN15A_x), std::end(RPN15A_x), std::begin(RPN15A_y));
    check1stDerivativeValue("Clamped spline", f, *std::begin(RPN15A_x), 0.0);
    check1stDerivativeValue("Clamped spline", f, *(std::end(RPN15A_x) - 1), 0.0);
    // poor performance
    interpolated = f(x_bad);
    if (interpolated < 1.0)
    {
        BOOST_ERROR("Clamped spline interpolation " << "poor performance unverified\n"
                                                    << "at x = " << x_bad << "\ninterpolated value: " << interpolated
                                                    << "\nexpected value > 1.0");
    }
    ...
    // Not-a-knot spline
    f = CubicInterpolation(std::begin(RPN15A_x), std::end(RPN15A_x), std::begin(RPN15A_y), CubicInterpolation::Spline,
                           false, CubicInterpolation::NotAKnot, Null<Real>(), CubicInterpolation::NotAKnot,
                           Null<Real>());
    f.update();
    ...
    if (interpolated < 1.0)
    {
        BOOST_ERROR("Not-a-knot spline interpolation " << "poor performance unverified\n"
                                                       << "at x = " << x_bad << "\ninterpolated value: " << interpolated
                                                       << "\nexpected value > 1.0");
    }
    ...
    f = CubicInterpolation(std::begin(RPN15A_x), std::end(RPN15A_x), std::begin(RPN15A_y), CubicInterpolation::Spline,
                           true, CubicInterpolation::SecondDerivative, 0.0, CubicInterpolation::SecondDerivative, 0.0);
    f.update();
    checkValues("MC natural spline", f, std::begin(RPN15A_x), std::end(RPN15A_x), std::begin(RPN15A_y));
    // good performance
    interpolated = f(x_bad);
    if (interpolated > 1.0)
    {
        BOOST_ERROR("MC natural spline interpolation " << "good performance unverified\n"
                                                       << "at x = " << x_bad << "\ninterpolated value: " << interpolated
                                                       << "\nexpected value < 1.0");
    }
    ...
    f = CubicInterpolation(std::begin(RPN15A_x), std::end(RPN15A_x), std::begin(RPN15A_y), CubicInterpolation::Spline,
                           true, CubicInterpolation::FirstDerivative, 0.0, CubicInterpolation::FirstDerivative, 0.0);
    f.update();
    checkValues("MC clamped spline", f, std::begin(RPN15A_x), std::end(RPN15A_x), std::begin(RPN15A_y));
    check1stDerivativeValue("MC clamped spline", f, *std::begin(RPN15A_x), 0.0);
    check1stDerivativeValue("MC clamped spline", f, *(std::end(RPN15A_x) - 1), 0.0);
    // good performance
    interpolated = f(x_bad);
    if (interpolated > 1.0)
    {
        BOOST_ERROR("MC clamped spline interpolation " << "good performance unverified\n"
                                                       << "at x = " << x_bad << "\ninterpolated value: " << interpolated
                                                       << "\nexpected value < 1.0");
    }
    ...
    f = CubicInterpolation(std::begin(RPN15A_x), std::end(RPN15A_x), std::begin(RPN15A_y), CubicInterpolation::Spline,
                           true, CubicInterpolation::NotAKnot, Null<Real>(), CubicInterpolation::NotAKnot,
                           Null<Real>());
    f.update();
    checkValues("MC not-a-knot spline", f, std::begin(RPN15A_x), std::end(RPN15A_x), std::begin(RPN15A_y));
    // good performance
    interpolated = f(x_bad);
    if (interpolated > 1.0)
    {
...
BOOST_AUTO_TEST_CASE(testSplineOnGenericValues)
{
    ...
    // Natural spline
    CubicInterpolation f(std::begin(generic_x), std::end(generic_x), std::begin(generic_y), CubicInterpolation::Spline,
                         false, CubicInterpolation::SecondDerivative, generic_natural_y2[0],
                         CubicInterpolation::SecondDerivative, generic_natural_y2[n - 1]);
    f.update();
    ...
    f = CubicInterpolation(std::begin(generic_x), std::end(generic_x), std::begin(generic_y),
                           CubicInterpolation::Spline, false, CubicInterpolation::FirstDerivative, y1a,
    ...
    check1stDerivativeValue("Clamped spline", f, *std::begin(generic_x), 0.0);
    check1stDerivativeValue("Clamped spline", f, *(std::end(generic_x) - 1), 0.0);
    x35[0] = f(3.5);


    // Not-a-knot spline
    f = CubicInterpolation(std::begin(generic_x), std::end(generic_x), std::begin(generic_y),
                           CubicInterpolation::Spline, false, CubicInterpolation::NotAKnot, Null<Real>(),
...
BOOST_AUTO_TEST_CASE(testSimmetricEndConditions)
{
    ...
    // Not-a-knot spline
    CubicInterpolation f(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, false, CubicInterpolation::NotAKnot,
                         Null<Real>(), CubicInterpolation::NotAKnot, Null<Real>());
    f.update();
    checkValues("Not-a-knot spline", f, x.begin(), x.end(), y.begin());
    checkNotAKnotCondition("Not-a-knot spline", f);
...
BOOST_AUTO_TEST_CASE(testDerivativeEndConditions)
{
    ...
    // Clamped spline
    f = CubicInterpolation(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, false,
                           CubicInterpolation::FirstDerivative, 4.0, CubicInterpolation::FirstDerivative, -4.0);
    f.update();
    checkValues("Clamped spline", f, x.begin(), x.end(), y.begin());
    ...
    f = CubicInterpolation(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, false,
                           CubicInterpolation::SecondDerivative, -2.0, CubicInterpolation::SecondDerivative, -2.0);
    f.update();
    checkValues("SecondDerivative spline", f, x.begin(), x.end(), y.begin());
    ...
    f = CubicInterpolation(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, true,
                           CubicInterpolation::FirstDerivative, 4.0, CubicInterpolation::FirstDerivative, -4.0);
    f.update();
    checkValues("MC Clamped spline", f, x.begin(), x.end(), y.begin());
    ...
    f = CubicInterpolation(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, true,
                           CubicInterpolation::SecondDerivative, -2.0, CubicInterpolation::SecondDerivative, -2.0);
    f.update();
    checkValues("MC SecondDerivative spline", f, x.begin(), x.end(), y.begin());
...
BOOST_AUTO_TEST_CASE(testNonRestrictiveHymanFilter)
{
    ...
    // MC Not-a-knot spline
    CubicInterpolation f(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, true, CubicInterpolation::NotAKnot,
                         Null<Real>(), CubicInterpolation::NotAKnot, Null<Real>());
    f.update();
    interpolated = f(zero);
    if (std::fabs(interpolated - expected) > 1e-15)
    {
        BOOST_ERROR("MC not-a-knot spline" << " interpolation failed at x = " << zero << "\n    interpolated value: "
                                           << interpolated << "\n    expected value:     " << expected
                                           << "\n    error:              " << std::fabs(interpolated - expected));
    }
    ...
    if (std::fabs(interpolated - expected) > 1e-15)
    {
        BOOST_ERROR("MC clamped spline" << " interpolation failed at x = " << zero << "\n    interpolated value: "
                                        << interpolated << "\n    expected value:     " << expected
                                        << "\n    error:              " << std::fabs(interpolated - expected));
    }
    ...
    if (std::fabs(interpolated - expected) > 1e-15)
    {
        BOOST_ERROR("MC SecondDerivative spline" << " interpolation failed at x = " << zero
                                                 << "\n    interpolated value: " << interpolated
                                                 << "\n    expected value:     " << expected
                                                 << "\n    error:              " << std::fabs(interpolated - expected));
    }
...
BOOST_AUTO_TEST_CASE(testMultiSpline)
{
    ...
    Real &s = args[0] = offsets[0], &t = args[1] = offsets[1], &u = args[2] = offsets[2], &v = args[3] = offsets[3],
         &w = args[4] = offsets[4];
...
BOOST_AUTO_TEST_CASE(testFritschButland)
{
    ...
    const Real y[][5] = {{1.0, 2.0, 1.0, 1.0, 2.0}, {1.0, 2.0, 1.0, 1.0, 1.0}, {2.0, 1.0, 0.0, 2.0, 3.0}};
    ...
    for (Size i = 0; i < 3; ++i)
    {
        ...
        for (Size j = 0; j < 4; ++j)
        {
            ...
            for (Size k = 0; k < 10; ++k)
            {
                ...
                if (std::isnan(y1))
                    BOOST_ERROR("NaN detected in case " << i << ":" << std::fixed << "\n    f(" << x1 << ") = " << y1);
...
BOOST_AUTO_TEST_CASE(testBackwardFlat)
{
    ...
    for (i = 0; i < N; i++)
    {
        ...
        if (std::fabs(expected - calculated) > tolerance)
            BOOST_ERROR("failed to reproduce " << io::ordinal(i + 1) << " datum" << std::fixed << "\n    expected:   "
    ...
    Real expected = y[0];
    if (std::fabs(expected - calculated) > tolerance)
        BOOST_ERROR("failed to extrapolate correctly at " << p << std::fixed << "\n    expected:   " << expected
                                                          << "\n    calculated: " << calculated << std::scientific
                                                          << "\n    error:      " << std::fabs(calculated - expected));
    ...
    expected = 0.0;
    if (std::fabs(expected - calculated) > tolerance)
        BOOST_ERROR("failed to calculate primitive at " << x[0] << std::fixed << "\n    expected:   " << expected
                                                        << "\n    calculated: " << calculated << std::scientific
                                                        << "\n    error:      " << std::fabs(calculated - expected));
...
BOOST_AUTO_TEST_CASE(testMixedLinearCubicMatchDerivatives)
{
    ...
    MixedLinearCubicInterpolation f(x.begin(), x.end(), y.begin(), k, MixedInterpolation::SplitRanges,
                                    CubicInterpolation::Spline, false, CubicInterpolation::FirstDerivative,
                                    Null<Real>(), CubicInterpolation::SecondDerivative, 0.0);
    f.update();
    ...
    {
        BOOST_ERROR("derivatives at the switch point do not match"
...
BOOST_AUTO_TEST_CASE(testSabrInterpolation)
{
    ...
    {
        Real calculatedVol =
            sabrVolatility(strikes[i], forward, expiry, initialAlpha, initialBeta, initialNu, initialRho);
    ...
    ext::shared_ptr<EndCriteria> endCriteria(new EndCriteria(100000, 100, 1e-8, 1e-8, 1e-8));
                ...
                for (bool k_b : isBetaFixed)
                {
                    for (bool k_n : isNuFixed)
                    {
                        for (bool k_r : isRhoFixed)
                        {
                            ...
                            SABRInterpolation sabrInterpolation(strikes.begin(), strikes.end(), volatilities.begin(),
                                                                expiry, forward, k_a ? initialAlpha : alphaGuess,
                                                                k_b ? initialBeta : betaGuess,
                                                                k_n ? initialNu : nuGuess, k_r ? initialRho : rhoGuess,
                                                                k_a, k_b, k_n, k_r, i, endCriteria, method, 1E-10);
...
BOOST_AUTO_TEST_CASE(testSabrGuess)
{
    ...
    SabrTestCase cases[] = {{0.058, 2016, 0.271, 1.0, 1.010, -0.345}, {0.153, 2016, 0.256, 1.0, 0.933, -0.321},
                            {0.230, 2016, 0.256, 1.0, 0.820, -0.346}, {0.479, 2016, 0.255, 1.0, 0.629, -0.370},
                            {0.729, 2016, 0.257, 1.0, 0.528, -0.403}, {1.227, 2016, 0.260, 1.0, 0.448, -0.429},
                            {1.726, 2016, 0.261, 1.0, 0.392, -0.440}, {2.244, 2016, 0.262, 1.0, 0.355, -0.445},
                            {2.742, 2016, 0.262, 1.0, 0.329, -0.445}, {3.241, 2016, 0.262, 1.0, 0.310, -0.447},
    ...
    for (const auto& c : cases)
    {
        for (auto type : {VolatilityType::ShiftedLognormal, VolatilityType::Normal})
        {
            for (auto shift : {0.0, 0.01, 0.1})
            {
                ...
                auto [alpha, beta, nu, rho] =
                    sabrGuess(k_m, vol_m, k_0, vol_0, k_p, vol_p, c.forward, c.expiry, c.beta, shift, type);
    ...
    BOOST_CHECK_EXCEPTION(sabrGuess(99, 0.32, 100, 0.30, 101, 0.31, 100, 1.0, 0.5, 0.0, VolatilityType::Normal), Error,
...
BOOST_AUTO_TEST_CASE(testKernelInterpolation)
{
    ...
    std::vector<std::vector<Real>> yd = {{11.275, 11.125, 11.250, 11.825, 12.625},
    ...
    std::vector<std::vector<Real>> ytd = {{11.23847, 11.12003, 11.58932, 11.99168, 13.29650},
...
BOOST_AUTO_TEST_CASE(testKernelInterpolation2D)
{
    ...
    KernelInterpolation2D kernel2D(xVec.begin(), xVec.end(), yVec.begin(), yVec.end(), M, myKernel);
    ...
    for (Size i = 0; i < M.rows(); ++i)
    {
        for (Size j = 0; j < M.columns(); ++j)
        {
            ...
            if (std::fabs(expectedVal - calcVal) > tolerance)
            {
                ...
                BOOST_ERROR("2D Kernel interpolation failed at x = "
                            << xVec[i] << ", y = " << yVec[j] << "\n    interpolated value: " << calcVal
                            << "\n    expected value:     " << expectedVal
                            << "\n    error:              " << std::fabs(expectedVal - calcVal));
            }
    ...
    KernelInterpolation2D kernel2DEp(xVec1.begin(), xVec1.end(), yVec1.begin(), yVec1.end(), M1, &epanechnikovKernel);
    ...
    for (Size i = 0; i < M1.rows(); ++i)
    {
        for (Size j = 0; j < M1.columns(); ++j)
        {
            ...
            if (std::fabs(expectedVal - calcVal) > tolerance)
            {
                ...
                BOOST_ERROR("2D Epanechnkikov Kernel interpolation failed at x = "
                            << xVec1[i] << ", y = " << yVec1[j] << "\n    interpolated value: " << calcVal
                            << "\n    expected value:     " << expectedVal
                            << "\n    error:              " << std::fabs(expectedVal - calcVal));
            }
    ...
    for (Size i = 0; i < M1.rows(); ++i)
    {
        for (Size j = 0; j < M1.columns(); ++j)
        {
            ...
            if (std::fabs(expectedVal - calcVal) > tolerance)
            {
                ...
                BOOST_ERROR("2D Epanechnkikov Kernel updated interpolation failed at x = "
                            << xVec1[i] << ", y = " << yVec1[j] << "\n    interpolated value: " << calcVal
                            << "\n    expected value:     " << expectedVal
                            << "\n    error:              " << std::fabs(expectedVal - calcVal));
            }
...
BOOST_AUTO_TEST_CASE(testUnknownRichardsonExtrapolation)
{
    ...
    const std::pair<Real, Real> testCases[] = {
        std::make_pair(1.0, 1.0), std::make_pair(1.0, -1.0), std::make_pair(2.0, 0.25), std::make_pair(2.0, -1.0),
        std::make_pair(3.0, 2.0), std::make_pair(3.0, -0.5), std::make_pair(4.0, 1.0),  std::make_pair(4.0, 0.5)};
...
BOOST_AUTO_TEST_CASE(testNoArbSabrInterpolation, *precondition(if_speed(Fast)))
{
    ...
    Real initialRho = 0.01;
    // calculate SABR vols and compare with input vols
    NoArbSabrSmileSection noarbSabr(expiry, forward, {initialAlpha, initialBeta, initialNu, initialRho});
    for (Size i = 0; i < strikes.size(); i++)
    {
    ...
    { // skip simplex (gets caught in some cases)
        for (bool i : vegaWeighted)
        {
            for (bool k_a : isAlphaFixed)
            {
                for (Size k_b = 0; k_b < 1 /*std::size(isBetaFixed)*/; ++k_b)
                    ...
                    for (bool k_n : isNuFixed)
                    {
                        for (bool k_r : isRhoFixed)
                        {
                            NoArbSabrInterpolation noarbSabrInterpolation(
                                strikes.begin(), strikes.end(), volatilities.begin(), expiry, forward,
                                k_a ? initialAlpha : alphaGuess, isBetaFixed[k_b] ? initialBeta : betaGuess,
                                k_n ? initialNu : nuGuess, k_r ? initialRho : rhoGuess, k_a, isBetaFixed[k_b], k_n, k_r,
...
BOOST_AUTO_TEST_CASE(testSabrSingleCases)
{
    ...
    SABRInterpolation s0(strikes.begin(), strikes.end(), vols.begin(), tte, forward, Null<Real>(), 0.25, Null<Real>(),
                         Null<Real>(), false, true, false, false);
    s0.update();

    if (s0.maxError() > 0.01 || s0.rmsError() > 0.01)
    {
        BOOST_ERROR("Sabr case #1 failed with max error (" << s0.maxError() << ") and rms error (" << s0.rmsError()
                                                           << "), both should be < 0.01");
    }
}
...
BOOST_AUTO_TEST_CASE(testTransformations)
{
    ...
    for (Size i = 0; i < 1E6; ++i)
    {
        ...
        if (!close(z[0], y[0], N) || !close(z[1], y[1], N) || !close(z[2], y[2], N) || !close(z[3], y[3], N))
            BOOST_ERROR("SabrInterpolation: direct(inverse("
                        << y[0] << "," << y[1] << "," << y[2] << "," << y[3] << ")) = (" << z[0] << "," << z[1] << ","
                        << z[2] << "," << z[3] << "), difference is (" << z[0] - y[0] << "," << z[1] - y[1] << ","
                        << z[2] - y[2] << "," << z[3] - y[3] << ")");
        ...
        QL_REQUIRE(sigmaI >= QuantLib::detail::NoArbSabrModel::sigmaI_min &&
                   ...
                   "sigmaI = alpha*forward^(beta-1.0) (" << sigmaI << ") out of bounds, alpha=" << alpha
                                                         << " beta=" << beta << " forward=" << forward);
        QL_REQUIRE(nu >= QuantLib::detail::NoArbSabrModel::nu_min && nu <= QuantLib::detail::NoArbSabrModel::nu_max,
                   "nu (" << nu << ") out of bounds");
        QL_REQUIRE(rho >= QuantLib::detail::NoArbSabrModel::rho_min && rho <= QuantLib::detail::NoArbSabrModel::rho_max,
...
BOOST_AUTO_TEST_CASE(testLagrangeInterpolation)
{
    ...
    const Real references[] = {
        -0.5000000000000000, -0.5392414024347419, -0.5591485962711904, -0.5629199661387594, -0.5534414777017116,
        -0.5333043347921566, -0.5048221831582063, -0.4700478608272949, -0.4307896950846587, -0.3886273460669714,
        -0.3449271969711449, -0.3008572908782903, -0.2574018141928359, -0.2153751266968088, -0.1754353382192734,
        -0.1380974319209344, -0.1037459341938971, -0.0726471311765894, -0.0449608318838433, -0.0207516779521373,
        0.0000000000000000,  0.0173877793964286,  0.0315691961126723,  0.0427562482700356,  0.0512063534145595,
        0.0572137590808174,  0.0611014067405497,  0.0632132491361394,  0.0639070209989264,  0.0635474631523613,
        0.0625000000000000,  0.0611248703983366,  0.0597717119144768,  0.0587745984686508,  0.0584475313615655,
        0.0590803836865967,  0.0609352981268212,  0.0642435381368876,  0.0692027925097279,  0.0759749333281079,
        0.0846842273010179,  0.0954160004849021,  0.1082157563897290,  0.1230887474699003,  0.1400000000000001,
        0.1588747923353829,  0.1795995865576031,  0.2020234135046815,  0.2259597111862140,  0.2511886165833182,
        0.2774597108334206,  0.3044952177998833,  0.3319936560264689,  0.3596339440766487,  0.3870799592577457,
        0.4139855497299214,  0.4400000000000001,  0.4647739498001331,  0.4879657663513030,  0.5092483700116673,
        0.5283165133097421,  0.5448945133624253,  0.5587444376778583,  0.5696747433431296,  0.5775493695968156,
        0.5822972837863635,  0.5839224807103117,  0.5825144353453510,  0.5782590089582251,  0.5714498086024714,
        0.5625000000000000,  0.5519545738075141,  0.5405030652677689,  0.5289927272456703,  0.5184421566492137,
        0.5100553742352614,  0.5052363578001620,  0.5056040287552059,  0.5130076920869246};
    ...
    for (Size i = 0; i < 79; ++i)
    {
        ...
        if (std::isnan(calculated) || std::fabs(references[i] - calculated) > tol)
        {
...
{
    BOOST_TEST_MESSAGE("Testing Lagrange interpolation at supporting points...");
    ...
    for (Size i = 1; i < n - 1; ++i)
    {
        for (Real z = x[i] - 100 * QL_EPSILON; z < x[i] + 100 * QL_EPSILON; z += 2 * QL_EPSILON)
        {
            ...
            if (std::isnan(calculated) || std::fabs(expected - calculated) > relTol)
            {
...
{
    BOOST_TEST_MESSAGE("Testing Lagrange interpolation derivatives...");
    ...
    for (Real x = -1.0; x <= 0.9; x += 0.01)
    {
        ...
        const Real expected = (interpl(x + eps, true) - interpl(x - eps, true)) / (2 * eps);

        if (std::isnan(calculated) || std::fabs(expected - calculated) > 25 * eps)
        {
...
{
    BOOST_TEST_MESSAGE("Testing Lagrange interpolation on Chebyshev nodes...");
...
BOOST_AUTO_TEST_CASE(testBSplines)
{
    ...
    std::vector<std::tuple<Natural, Real, Real>> referenceValues = {
        {0, -0.95, 9.5238095238e-04}, {0, -0.01, 0.37337142857}, {0, 0.49, 0.84575238095}, {0, 1.21, 0.0},
        {1, 1.49, 0.562987654321},    {1, 1.59, 0.490888888889}, {2, 1.99, 0.62429409171}, {3, 1.19, 0.0},
    ...
    for (const auto& [idx, x, expected] : referenceValues)
    {
        ...
        if (std::isnan(calculated) || std::fabs(calculated - expected) > tol)
        {
...
BOOST_AUTO_TEST_CASE(testBackwardFlatOnSinglePoint)
{
    ...
    const Interpolation impl(BackwardFlat().interpolate(knots.begin(), knots.end(), values.begin()));
...
BOOST_AUTO_TEST_CASE(testChebyshevInterpolation)
{
    ...
    const auto fcts = std::vector<std::pair<std::function<Real(Real)>, std::string>>{
        {[](Real x) { return std::sin(x); }, "sin"},
        {[](Real x) { return std::cos(x); }, "cos"},
        {[](Real x) { return std::exp(-x * x); }, "e^(-x*x)"}};

    const auto tests = std::vector<std::pair<Size, Real>>{{11, 1e-5}, {20, 1e-11}};
...
BOOST_AUTO_TEST_CASE(testChebyshevInterpolationOnNodes)
{
    ...
    for (auto pointType : {ChebyshevInterpolation::FirstKind, ChebyshevInterpolation::SecondKind})
    {
...
BOOST_AUTO_TEST_CASE(testFlatExtrapolation)
{
    ...
    auto cubic = ext::make_shared<CubicInterpolation>(
        std::begin(x), std::end(x), std::begin(y), CubicInterpolation::Spline, false,
    ...
    for (Size i = 0; i < N; ++i)
    {
        ...
        if (std::fabs(calculated - expected) > tolerance)
            BOOST_ERROR("in-range mismatch at x=" << x[i] << std::fixed << "\n    expected:   " << expected
                                                  << "\n    calculated: " << calculated << std::scientific
                                                  << "\n    error:      " << std::fabs(calculated - expected));
    }
    ...
    for (Size i = 0; i < N - 1; ++i)
    {
        ...
        if (std::fabs(calculated - expected) > tolerance)
            BOOST_ERROR("in-range mismatch at midpoint x=" << mid << std::fixed << "\n    expected:   " << expected
                                                           << "\n    calculated: " << calculated << std::scientific
                                                           << "\n    error:      " << std::fabs(calculated - expected));
    }
    ...
    for (Real p : {-2.0, -1.0, -0.01})
    {
        ...
        Real expected = y[0];
        if (std::fabs(calculated - expected) > tolerance)
            BOOST_ERROR("flat extrapolation below range failed at "
                        << p << std::fixed << "\n    expected:   " << expected << "\n    calculated: " << calculated
                        << std::scientific << "\n    error:      " << std::fabs(calculated - expected));
    }
    ...
    for (Real p : {4.01, 5.0, 10.0})
    {
        ...
        Real expected = y[N - 1];
        if (std::fabs(calculated - expected) > tolerance)
            BOOST_ERROR("flat extrapolation above range failed at "
                        << p << std::fixed << "\n    expected:   " << expected << "\n    calculated: " << calculated
                        << std::scientific << "\n    error:      " << std::fabs(calculated - expected));
    }
    ...
    for (Real p : {0.5, 1.5, 2.5, 3.5})
    {
        ...
        if (std::fabs(calculated - expected) > tolerance)
            BOOST_ERROR("in-range derivative mismatch at x="
                        << p << std::fixed << "\n    expected:   " << expected << "\n    calculated: " << calculated
                        << std::scientific << "\n    error:      " << std::fabs(calculated - expected));
    }
    ...
    for (Real p : {0.5, 1.5, 2.5, 3.5})
    {
        ...
        if (std::fabs(calculated - expected) > tolerance)
            BOOST_ERROR("in-range second derivative mismatch at x="
                        << p << std::fixed << "\n    expected:   " << expected << "\n    calculated: " << calculated
                        << std::scientific << "\n    error:      " << std::fabs(calculated - expected));
    }
    ...
    for (Real p : {0.5, 1.5, 2.5, 3.5})
    {
        ...
        if (std::fabs(calculated - expected) > tolerance)
            BOOST_ERROR("in-range primitive mismatch at x="
                        << p << std::fixed << "\n    expected:   " << expected << "\n    calculated: " << calculated
                        << std::scientific << "\n    error:      " << std::fabs(calculated - expected));
    }
    ...
    f.update();
    {
        ...
        if (std::fabs(calculated - expected) > tolerance)
            BOOST_ERROR("mismatch after update at x=2.0" << std::fixed << "\n    expected:   " << expected
                                                         << "\n    calculated: " << calculated << std::scientific
                                                         << "\n    error:      " << std::fabs(calculated - expected));
    }
}
