...
struct CommonVars
{
    ...
    CommonVars()
    {
        ...
        process = ext::make_shared<BlackScholesMertonProcess>(underlying, dividendYield, riskFreeRate, volatility);
...
BOOST_AUTO_TEST_CASE(testBond)
{
    ...
    BOOST_TEST_MESSAGE("Testing out-of-the-money convertible bonds against vanilla bonds...");

    CommonVars vars;

    vars.conversionRatio = 1.0e-16;

    ext::shared_ptr<Exercise> euExercise = ext::make_shared<EuropeanExercise>(vars.maturityDate);
    ext::shared_ptr<Exercise> amExercise = ext::make_shared<AmericanExercise>(vars.issueDate, vars.maturityDate);

    Size timeSteps = 1001;
    ext::shared_ptr<PricingEngine> engine =
        ext::make_shared<BinomialConvertibleEngine<CoxRossRubinstein>>(vars.process, timeSteps, vars.creditSpread);

    Handle<YieldTermStructure> discountCurve(
        ext::make_shared<ForwardSpreadedTermStructure>(vars.riskFreeRate, vars.creditSpread));
    ...
    ConvertibleZeroCouponBond euZero(euExercise, vars.conversionRatio, vars.no_callability, vars.issueDate,
                                     vars.settlementDays, vars.dayCounter, schedule, vars.redemption);
    euZero.setPricingEngine(engine);

    ConvertibleZeroCouponBond amZero(amExercise, vars.conversionRatio, vars.no_callability, vars.issueDate,
                                     vars.settlementDays, vars.dayCounter, schedule, vars.redemption);
    amZero.setPricingEngine(engine);

    ZeroCouponBond zero(vars.settlementDays, vars.calendar, 100.0, vars.maturityDate, Following, vars.redemption,
                        vars.issueDate);
    ...
    ConvertibleFixedCouponBond euFixed(euExercise, vars.conversionRatio, vars.no_callability, vars.issueDate,
                                       vars.settlementDays, coupons, vars.dayCounter, schedule, vars.redemption);
    euFixed.setPricingEngine(engine);

    ConvertibleFixedCouponBond amFixed(amExercise, vars.conversionRatio, vars.no_callability, vars.issueDate,
                                       vars.settlementDays, coupons, vars.dayCounter, schedule, vars.redemption);
    amFixed.setPricingEngine(engine);

    FixedRateBond fixed(vars.settlementDays, vars.faceAmount, schedule, coupons, vars.dayCounter, Following,
    ...
    ext::shared_ptr<IborIndex> index = ext::make_shared<Euribor1Y>(discountCurve);
    std::vector<Real> gearings(1, 1.0);
    std::vector<Rate> spreads;

    ConvertibleFloatingRateBond euFloating(euExercise, vars.conversionRatio, vars.no_callability, vars.issueDate,
                                           vars.settlementDays, index, vars.fixingDays, spreads, vars.dayCounter,
                                           schedule, vars.redemption);
    euFloating.setPricingEngine(engine);

    ConvertibleFloatingRateBond amFloating(amExercise, vars.conversionRatio, vars.no_callability, vars.issueDate,
                                           vars.settlementDays, index, vars.fixingDays, spreads, vars.dayCounter,
                                           schedule, vars.redemption);
    amFloating.setPricingEngine(engine);

    ext::shared_ptr<IborCouponPricer> pricer =
        ext::make_shared<BlackIborCouponPricer>(Handle<OptionletVolatilityStructure>());

    Schedule floatSchedule(vars.issueDate, vars.maturityDate, Period(vars.frequency), vars.calendar, Following,
                           Following, DateGeneration::Backward, false);

    FloatingRateBond floating(vars.settlementDays, vars.faceAmount, floatSchedule, index, vars.dayCounter, Following,
                              vars.fixingDays, gearings, spreads, std::vector<Rate>(), std::vector<Rate>(), false,
                              vars.redemption, vars.issueDate);
...
BOOST_AUTO_TEST_CASE(testOption)
{
    ...
    BOOST_TEST_MESSAGE("Testing zero-coupon convertible bonds against vanilla option...");
    ...
    ext::shared_ptr<PricingEngine> vanillaEngine =
        ext::make_shared<BinomialVanillaEngine<CoxRossRubinstein>>(vars.process, timeSteps);

    vars.creditSpread.linkTo(ext::make_shared<SimpleQuote>(0.0));

    Real conversionStrike = vars.redemption / vars.conversionRatio;
    ext::shared_ptr<StrikedTypePayoff> payoff = ext::make_shared<PlainVanillaPayoff>(Option::Call, conversionStrike);
    ...
    Real expected =
        vars.faceAmount / 100.0 *
        (vars.redemption * vars.riskFreeRate->discount(vars.maturityDate) + vars.conversionRatio * euOption.NPV());
...
BOOST_AUTO_TEST_CASE(testRegression)
{
    ...
    BOOST_TEST_MESSAGE("Testing fixed-coupon convertible bond in known regression case...");
    ...
    Handle<YieldTermStructure> r(ext::make_shared<ForwardCurve>(dates, forwards, Actual360()));

    Handle<BlackVolTermStructure> sigma(ext::make_shared<BlackConstantVol>(tomorrow, NullCalendar(), 21.685235548092248,
                                                                           Thirty360(Thirty360::BondBasis)));

    ext::shared_ptr<BlackProcess> process = ext::make_shared<BlackProcess>(u, r, sigma);
    ...
    ConvertibleFixedCouponBond bond(exercise, conversionRatio, no_callability, issueDate, settlementDays, coupons,
                                    dayCounter, schedule, redemption);
    bond.setPricingEngine(
        ext::make_shared<BinomialConvertibleEngine<CoxRossRubinstein>>(process, 600, spread, no_dividends));
