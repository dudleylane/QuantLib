...
#define REPORT_FAILURE_FLOATING(greekName, minmax, payoff, exercise, s, q, r, today, v, expected, calculated, error, \
                                tolerance)                                                                           \
    BOOST_ERROR(exerciseTypeToString(exercise)                                                                       \
                << payoff->optionType() << " lookback option with " << payoffTypeToString(payoff) << " payoff:\n"    \
                << "    underlying value: " << s << "\n"                                                             \
...
#define REPORT_FAILURE_FIXED(greekName, minmax, payoff, exercise, s, q, r, today, v, expected, calculated, error, \
                             tolerance)                                                                           \
    BOOST_ERROR(exerciseTypeToString(exercise)                                                                    \
                << payoff->optionType() << " lookback option with " << payoffTypeToString(payoff) << " payoff:\n" \
                << "    underlying value: " << s << "\n"                                                          \
...
#define REPORT_FAILURE_MC(lookbackType, optionType, analytical, monteCarlo, tolerance)                                 \
    BOOST_ERROR("Analytical and MC " << lookbackType << " " << optionType << " values differed by more than tolerance" \
                                     << "\n"                                                                           \
...
BOOST_AUTO_TEST_CASE(testAnalyticContinuousFloatingLookback)
{
    ...
    BOOST_TEST_MESSAGE("Testing analytic continuous floating-strike lookback options...");
    ...
    for (auto& value : values)
    {
        ...
        ext::shared_ptr<PricingEngine> engine(new AnalyticContinuousFloatingLookbackEngine(stochProcess));
        ...
        if (error > value.tol)
        {
            REPORT_FAILURE_FLOATING("value", values[i].minmax, payoff, exercise, value.s, value.q, value.r, today,
...
BOOST_AUTO_TEST_CASE(testAnalyticContinuousFixedLookback)
{
    ...
    BOOST_TEST_MESSAGE("Testing analytic continuous fixed-strike lookback options...");
    ...
    for (auto& value : values)
    {
        ...
        ext::shared_ptr<PricingEngine> engine(new AnalyticContinuousFixedLookbackEngine(stochProcess));
        ...
        if (error > value.tol)
        {
            REPORT_FAILURE_FIXED("value", values[i].minmax, payoff, exercise, value.s, value.q, value.r, today, value.v,
...
BOOST_AUTO_TEST_CASE(testAnalyticContinuousPartialFloatingLookback)
{
    ...
    BOOST_TEST_MESSAGE("Testing analytic continuous partial floating-strike lookback options...");
    ...
    for (auto& value : values)
    {
        ...
        ext::shared_ptr<PricingEngine> engine(new AnalyticContinuousPartialFloatingLookbackEngine(stochProcess));

        Date lookbackEnd = today + timeToDays(value.t1);
        ContinuousPartialFloatingLookbackOption option(value.minmax, value.l, lookbackEnd, payoff, exercise);
        option.setPricingEngine(engine);
...
BOOST_AUTO_TEST_CASE(testAnalyticContinuousPartialFixedLookback)
{
    ...
    LookbackOptionData values[] = {// data from "Option Pricing Formulas, Second Edition", Haug, 2006, pg.148
                                   // type,         strike, minmax, s,    q,    r,    t,    v, l,   t1,  result,   tol
    ...
    for (auto& value : values)
    {
        ...
        ext::shared_ptr<PricingEngine> engine(new AnalyticContinuousPartialFixedLookbackEngine(stochProcess));

        Date lookbackStart = today + timeToDays(value.t1);
        ContinuousPartialFixedLookbackOption option(lookbackStart, payoff, exercise);
        option.setPricingEngine(engine);
...
BOOST_AUTO_TEST_CASE(testMonteCarloLookback, *precondition(if_speed(Slow)))
{
    ...
    for (auto type : types)
    {
        ...
        ContinuousPartialFixedLookbackOption partialFixedLookback(lookbackStart, payoff, exercise);
        ext::shared_ptr<PricingEngine> engine(new AnalyticContinuousPartialFixedLookbackEngine(stochProcess));
        ...
        ext::shared_ptr<PricingEngine> mcpartialfixedengine =
            MakeMCLookbackEngine<ContinuousPartialFixedLookbackOption, PseudoRandom>(stochProcess)
                .withSteps(2000)
        ...
        ContinuousFixedLookbackOption fixedLookback(minMax, payoff, exercise);
        ext::shared_ptr<PricingEngine> analyticalfixedengine(new AnalyticContinuousFixedLookbackEngine(stochProcess));
        ...
        ext::shared_ptr<PricingEngine> mcfixedengine =
            MakeMCLookbackEngine<ContinuousFixedLookbackOption, PseudoRandom>(stochProcess)
                .withSteps(2000)
                .withAntitheticVariate()
                .withSeed(1)
                .withAbsoluteTolerance(tolerance);
        ...
        ContinuousPartialFloatingLookbackOption partialFloating(minMax, lambda, lookbackEnd, floatingPayoff, exercise);
        ...
        ext::shared_ptr<PricingEngine> mcpartialfloatingengine =
            MakeMCLookbackEngine<ContinuousPartialFloatingLookbackOption, PseudoRandom>(stochProcess)
                .withSteps(2000)
                .withAntitheticVariate()
                .withSeed(1)
                .withAbsoluteTolerance(tolerance);
        ...
        ContinuousFloatingLookbackOption floating(minMax, floatingPayoff, exercise);
        ...
        ext::shared_ptr<PricingEngine> mcfloatingengine =
            MakeMCLookbackEngine<ContinuousFloatingLookbackOption, PseudoRandom>(stochProcess)
                .withSteps(2000)
                .withAntitheticVariate()
                .withSeed(1)
                .withAbsoluteTolerance(tolerance);
