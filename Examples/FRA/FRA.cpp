...
int main(int, char*[])
{
    ...
    try
    {
        ...
        auto fra1x4 =
            ext::make_shared<FraRateHelper>(h1x4, 1, 4, fixingDays, calendar, convention, endOfMonth, fraDayCounter);

        auto fra2x5 =
            ext::make_shared<FraRateHelper>(h2x5, 2, 5, fixingDays, calendar, convention, endOfMonth, fraDayCounter);

        auto fra3x6 =
            ext::make_shared<FraRateHelper>(h3x6, 3, 6, fixingDays, calendar, convention, endOfMonth, fraDayCounter);

        auto fra6x9 =
            ext::make_shared<FraRateHelper>(h6x9, 6, 9, fixingDays, calendar, convention, endOfMonth, fraDayCounter);

        auto fra9x12 =
            ext::make_shared<FraRateHelper>(h9x12, 9, 12, fixingDays, calendar, convention, endOfMonth, fraDayCounter);
        ...
        cout << endl;
        cout << "Test FRA construction, NPV calculation, and FRA purchase" << endl << endl;

        Size i;
        for (i = 0; i < std::size(monthsToStart); i++)
        {

            Date fraValueDate = fraCalendar.advance(settlementDate, monthsToStart[i], Months, fraBusinessDayConvention);

            Rate fraStrikeRate = threeMonthFraQuote[monthsToStart[i]];

            ForwardRateAgreement myFRA(euribor3m, fraValueDate, fraFwdType, fraStrikeRate, fraNotional);

            cout << "3m Term FRA, Months to Start: " << monthsToStart[i] << endl;
            cout << "strike FRA rate: " << io::rate(fraStrikeRate) << endl;
            cout << "FRA 3m forward rate: " << myFRA.forwardRate() << endl;
            cout << "FRA market quote: " << io::rate(threeMonthFraQuote[monthsToStart[i]]) << endl;
            cout << "FRA amount [should be zero]: " << myFRA.amount() << endl;
            cout << "FRA NPV [should be zero]: " << myFRA.NPV() << endl << endl;
        }


        cout << endl << endl;
        cout << "Now take a 100 basis-point upward shift in FRA quotes "
             << "and examine NPV" << endl
             << endl;
        ...
        for (i = 0; i < std::size(monthsToStart); i++)
        {
            ...
            Rate fraStrikeRate = threeMonthFraQuote[monthsToStart[i]] - BpsShift;

            ForwardRateAgreement myFRA(euribor3m, fraValueDate, fraFwdType, fraStrikeRate, fraNotional);

            cout << "3m Term FRA, 100 notional, Months to Start = " << monthsToStart[i] << endl;
            cout << "strike FRA rate: " << io::rate(fraStrikeRate) << endl;
            cout << "FRA 3m forward rate: " << myFRA.forwardRate() << endl;
            cout << "FRA market quote: " << io::rate(threeMonthFraQuote[monthsToStart[i]]) << endl;
            cout << "FRA amount [should be positive]: " << myFRA.amount() << endl;
            cout << "FRA NPV [should be positive]: " << myFRA.NPV() << endl << endl;
        }
