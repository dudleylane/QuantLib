...
theVegaBumps(bool factorwiseBumping, const ext::shared_ptr<MarketModel>& marketModel, bool doCaps)
{
    ...
    VegaBumpCollection possibleBumps(marketModel, factorwiseBumping);
...
int Bermudan()
{
    ...
    FlatVol calibration(volatilities, ext::make_shared<ExponentialForwardCorrelation>(correlations), evolution,
    ...
    LogNormalFwdRatePc evolver(marketModel, generatorFactory,
    ...
    LongstaffSchwartzExerciseStrategy exerciseStrategy(basisSystem, basisCoefficients, evolution, numeraires,
                                                       nullRebate, control);
    ...
    AccountingEngine accounter(evolverPtr, Clone<MarketModelMultiProduct>(allProducts), initialNumeraireValue);
    ...
    for (Size i = 0; i < 4; ++i)
    {
        ...
        LogNormalFwdRateEuler evolverEuler(marketModel, generatorFactory, numeraires);

        MarketModelPathwiseSwap receiverPathwiseSwap(rateTimes, accruals, strikes, receive);
        Clone<MarketModelPathwiseMultiProduct> receiverPathwiseSwapPtr(receiverPathwiseSwap.clone());

        //  callable receiver swap
        CallSpecifiedPathwiseMultiProduct callableProductPathwise(receiverPathwiseSwapPtr, exerciseStrategy);

        Clone<MarketModelPathwiseMultiProduct> callableProductPathwisePtr(callableProductPathwise.clone());


        std::vector<std::vector<Matrix>> theBumps(theVegaBumps(allowFactorwiseBumping, marketModel, doCaps));

        PathwiseVegasOuterAccountingEngine accountingEngineVegas(ext::make_shared<LogNormalFwdRateEuler>(evolverEuler),
                                                                 callableProductPathwisePtr, marketModel, theBumps,
    ...
    auto upperEvolver = ext::make_shared<LogNormalFwdRatePc>(ext::make_shared<FlatVol>(calibration), uFactory,
                                                             numeraires // numeraires for each step
    );
    ...
    for (Size s = 0; s < isExerciseTime.size(); ++s)
    {
        if (isExerciseTime[s])
        {
            ...
            auto e = ext::make_shared<LogNormalFwdRatePc>(ext::make_shared<FlatVol>(calibration), uFactory,
                                                          numeraires, // numeraires for each step
                                                          s);
    ...
    UpperBoundEngine uEngine(upperEvolver,  // does outer paths
                             ...
                             receiverSwap, nullRebate, receiverSwap, nullRebate, exerciseStrategy,
                             initialNumeraireValue);
    ...
    std::cout << " time to compute upper bound is,  " << (t5 - t4) / static_cast<Real>(CLOCKS_PER_SEC)
              << ", seconds.\n";
...
int InverseFloater(Real rateLevel)
{
    ...
    MultiStepInverseFloater inverseFloater(rateTimes, accruals, accruals, fixedStrikes, fixedMultipliers,
    ...
    CallSpecifiedMultiProduct dummyProduct =
        CallSpecifiedMultiProduct(inverseFloater, naifStrategy, ExerciseAdapter(nullRebate));
    ...
    for (Size i = 0; i < 4; ++i)
    {
        ...
        MarketModelPathwiseInverseFloater pathwiseInverseFloater(
            rateTimes, accruals, accruals, fixedStrikes, fixedMultipliers, floatingSpreads, paymentTimes, payer);

        Clone<MarketModelPathwiseMultiProduct> pathwiseInverseFloaterPtr(pathwiseInverseFloater.clone());

        //  callable inverse floater
        CallSpecifiedPathwiseMultiProduct callableProductPathwise(pathwiseInverseFloaterPtr, exerciseStrategy);
    ...
    UpperBoundEngine uEngine(upperEvolver,  // does outer paths
                             innerEvolvers, // for sub-simulations that do continuation values
                             inverseFloater, nullRebate, inverseFloater, nullRebate, exerciseStrategy,
                             initialNumeraireValue);
