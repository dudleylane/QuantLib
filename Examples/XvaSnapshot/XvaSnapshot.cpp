...
int main(int, char**)
{
    try
    {
        ...
        std::cout << "Loaded " << snapshot.size() << " quotes from snapshot." << std::endl;
        ...
        Handle<YieldTermStructure> discount(ext::make_shared<FlatForward>(today, Handle<Quote>(discQuote), dc));

        // 3. Price a 5y 3% semiannual fixed-rate bond.
        Date issue = today;
        Date maturity = today + 5 * Years;
        Schedule schedule(issue, maturity, 6 * Months, cal, ModifiedFollowing, ModifiedFollowing,
                          DateGeneration::Backward, false);
        FixedRateBond bond(/*settlementDays*/ 2, /*faceAmount*/ 100.0, schedule, {0.03}, dc);
        bond.setPricingEngine(ext::make_shared<DiscountingBondEngine>(discount));
        std::cout << "Bond NPV = " << std::fixed << std::setprecision(4) << bond.NPV() << std::endl;
        ...
        std::vector<Date> grid{
            today, today + 1 * Years, today + 2 * Years, today + 3 * Years, today + 4 * Years, today + 5 * Years};
        ...
        Handle<DefaultProbabilityTermStructure> cptyPd(ext::make_shared<FlatHazardRate>(today, cptyHaz, dc));
        Handle<DefaultProbabilityTermStructure> ownPd(ext::make_shared<FlatHazardRate>(today, ownHaz, dc));

        XvaCalculator xva(grid, epe, ene, cptyPd, ownPd,
                          /*cptyRec*/ 0.40, /*ownRec*/ 0.40,
                          /*fundSpread*/ 0.005, /*capSpread*/ 0.010,
                          /*margSpread*/ 0.000, discount);
        std::cout << "CVA = " << xva.cva() << ", DVA = " << xva.dva() << ", FVA = " << xva.fva()
                  << ", KVA = " << xva.kva() << ", Total = " << xva.totalXvaAdjustment() << std::endl;
        return 0;
    }
