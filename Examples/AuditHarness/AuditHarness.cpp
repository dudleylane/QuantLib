...
struct GreekDiff {
    ...
    Real relDiff() const {
        return std::abs(analytic) > 1e-12 ? absDiff() / std::abs(analytic)
                                          : absDiff();
    }
};

ext::shared_ptr<BlackScholesMertonProcess>
makeBSMProcess(Real spot, Real r, Real q, Real vol) {
    ...
    return ext::make_shared<BlackScholesMertonProcess>(
        Handle<Quote>(spotQ), Handle<YieldTermStructure>(divTS),
        Handle<YieldTermStructure>(rateTS), Handle<BlackVolTermStructure>(volTS));
}
...
void testEuropeanGreeks() {
    ...
    std::vector<GreekDiff> diffs{
        {"delta", dAnalytic, dBump},
        {"gamma", gAnalytic, gBump},
        {"vega ", vAnalytic, vBump}};
    std::cout << std::fixed << std::setprecision(10);
    for (const auto &d : diffs)
        std::cout << "  " << d.name << "  analytic=" << d.analytic
                  << "  bump=" << d.bumpAndReprice
                  << "  rel_err=" << d.relDiff() << "\n";
}
...
void testAsianFdBumpSensitivity() {
    ...
    for (int m = 1; m <= 12; ++m) fixings.push_back(today + Period(m, Months));
    ...
    for (Real spot : {0.01, 1.0, 100.0, 10000.0}) {
        ...
        std::cout << "  spot=" << std::setw(9) << spot
                  << "  NPV=" << std::setprecision(6) << npv
                  << "  delta=" << delta
                  << "  gamma=" << std::setprecision(4) << gamma
...
void testBootstrapPathological() {
    ...
    auto tryBootstrap = [&](const std::string &label,
                            const std::vector<Real> &depositRates,
        ...
        for (Size i = 0; i < depositRates.size(); ++i) {
            ...
            helpers.push_back(ext::make_shared<DepositRateHelper>(
                Handle<Quote>(q), depositPeriods[i], 2, cal, Following, false, dc));
        }
        std::cout << "  [" << label << "] rates: ";
        for (Real r : depositRates) std::cout << r << " ";
        std::cout << "\n";
        try {
            auto curve = ext::make_shared<PiecewiseYieldCurve<Discount, LogLinear>>(
                today, helpers, dc);
            Real zeroRate5Y = curve->zeroRate(today + Period(5, Years),
                                               dc, Continuous).rate();
            std::cout << "    -> bootstrapped OK, 5Y zero = " << std::fixed
                      << std::setprecision(6) << zeroRate5Y << "\n";
        } catch (const std::exception &e) {
            std::cout << "    -> THREW: " << e.what() << "\n";
        }
    };

    std::vector<Period> periods = {Period(1, Months), Period(3, Months),
                                   Period(6, Months), Period(1, Years),
                                   Period(2, Years),  Period(5, Years)};
    tryBootstrap("normal upward",
                 {0.030, 0.032, 0.034, 0.036, 0.040, 0.045}, periods);
    tryBootstrap("inverted (steep)",
                 {0.080, 0.070, 0.060, 0.050, 0.040, 0.030}, periods);
    tryBootstrap("inverted (mild)",
                 {0.045, 0.044, 0.043, 0.042, 0.041, 0.040}, periods);
    tryBootstrap("flat very-low",
                 {0.001, 0.001, 0.001, 0.001, 0.001, 0.001}, periods);
    tryBootstrap("negative rates",
                 {-0.005, -0.003, -0.001, 0.001, 0.005, 0.010}, periods);
    tryBootstrap("extreme positive",
                 {0.15, 0.20, 0.25, 0.30, 0.35, 0.40}, periods);
    tryBootstrap("humped",
                 {0.02, 0.03, 0.05, 0.07, 0.05, 0.03}, periods);
}
