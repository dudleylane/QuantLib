    ...
    try
    {
        ...
        for (Size i = 0; i < sizeof(tenorsSwapMkt) / sizeof(Size); i++)
            swapHelpers.push_back(ext::make_shared<SwapRateHelper>(
                makeQuoteHandle(ratesSwapmkt[i]), tenorsSwapMkt[i] * Years, TARGET(), Quarterly, ModifiedFollowing,
                ActualActual(ActualActual::ISDA), yieldIndx));

        auto swapTS = ext::make_shared<PiecewiseYieldCurve<Discount, LogLinear>>(2, TARGET(), swapHelpers,
                                                                                 ActualActual(ActualActual::ISDA));
        swapTS->enableExtrapolation();

        auto riskFreeEngine = ext::make_shared<DiscountingSwapEngine>(Handle<YieldTermStructure>(swapTS));
        ...
        Real intensitiesHigh[] = {0.0534, 0.0534, 0.0564, 0.06, 0.0614, 0.0696, 0.0696, 0.0696, 0.0696, 0.0696, 0.0696};
        // Recovery rates:
        ...
        {
            defaultTSDates.push_back(TARGET().advance(todaysDate, Period(defaultTenors[i], Months)));
        ...
        defaultIntensityTS.emplace_back(ext::make_shared<InterpolatedHazardRateCurve<BackwardFlat>>(
            defaultTSDates, intesitiesVLow, Actual360(), TARGET()));
        defaultIntensityTS.emplace_back(ext::make_shared<InterpolatedHazardRateCurve<BackwardFlat>>(
            defaultTSDates, intesitiesVMedium, Actual360(), TARGET()));
        defaultIntensityTS.emplace_back(ext::make_shared<InterpolatedHazardRateCurve<BackwardFlat>>(
            defaultTSDates, intesitiesVHigh, Actual360(), TARGET()));

        Volatility blackVol = 0.15;
        auto ctptySwapCvaLow = ext::make_shared<CounterpartyAdjSwapEngine>(Handle<YieldTermStructure>(swapTS), blackVol,
                                                                           defaultIntensityTS[0], ctptyRRLow);
        ...
        for (Size i = 0; i < sizeof(tenorsSwapMkt) / sizeof(Size); i++)
            riskySwaps.push_back(MakeVanillaSwap(tenorsSwapMkt[i] * Years, yieldIndxS, ratesSwapmkt[i], 0 * Days)
        ...
        */
        for (Size i = 0; i < riskySwaps.size(); i++)
        {
            ...
            cout << " | " << setw(6) << 10000. * (riskySwaps[i].fairRate() - nonRiskyFair);
